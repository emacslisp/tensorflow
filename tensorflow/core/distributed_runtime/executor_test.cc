...
protected:
	...
	void Create(const Graph* graph)
	{
		...
		params.create_kernel =
				[this, version](const NodeDef& ndef,
						OpKernel** kernel) {
		...
		runner_ =
				[this](std::function<void()> fn) {thread_pool_->Schedule(fn);};
		...
		const string& receiver, const string& name)
{
	...
	CHECK(
			Rendezvous::ParseKey(
					Rendezvous::CreateKey(sender, incarnation, receiver, name,
							FrameAndIter(0, 0)), &result).ok());
	return result;
}
