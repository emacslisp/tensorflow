		...
		std::vector<string>* hidden_ops)
{
	...
	while (s.ok()) {
		...
		StringPiece op_name;
		if (scanner.One(strings::Scanner::LETTER_DIGIT_DOT).Any(
				strings::Scanner::LETTER_DIGIT_DASH_DOT_SLASH_UNDERSCORE).GetResult(
				nullptr, &op_name)) {
...
int main(int argc, char* argv[])
{
	...
	if (argc == 2) {
		...
		TF_CHECK_OK(
				tensorflow::ParseHiddenOpsCommandLine(argv[1], &hidden_ops));
