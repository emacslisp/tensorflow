		...
		std::vector<Output>* grad_outputs)
{
	...
	grad_outputs->push_back(
			Mul(scope, grad_inputs[0], Sign(scope, op.input(0))));
	return scope.status();
}
		...
		std::vector<Output>* grad_outputs)
{
	...
	grad_outputs->push_back(
			Mul(scope, grad_inputs[0],
					Neg(scope, Square(scope, op.output(0)))));
	return scope.status();
}
		...
		std::vector<Output>* grad_outputs)
{
	...
	// dx = dy * exp(x)
	grad_outputs->push_back(
			Mul(scope, grad_inputs[0], Exp(scope, op.input(0))));
	return scope.status();
}
...
Status MatMulGradHelper(const Scope& scope, const bool is_batch,
		...
		const Output& y1, const bool adj_y1, std::vector<Output>* grad_outputs)
{
	if (is_batch == false) {
		auto dx = MatMul(scope, x0, x1,
				MatMul::TransposeA(adj_x0).TransposeB(adj_x1));
		grad_outputs->push_back(dx);
		auto dy = MatMul(scope, y0, y1,
				MatMul::TransposeA(adj_y0).TransposeB(adj_y1));
		grad_outputs->push_back(dy);
	} else {
		auto dx = BatchMatMul(scope, x0, x1,
				BatchMatMul::AdjX(adj_x0).AdjY(adj_x1));
		grad_outputs->push_back(dx);
		auto dy = BatchMatMul(scope, y0, y1,
				BatchMatMul::AdjX(adj_y0).AdjY(adj_y1));
		grad_outputs->push_back(dy);
	}
	return scope.status();
}
...
Status MatMulGradCommon(const Scope& scope, const Operation& op,
		const bool is_batch, const std::vector<Output>& grad_inputs,
		...
		std::vector<Output>* grad_outputs)
{
	...
	bool tb;
	TF_RETURN_IF_ERROR(
			GetNodeAttr(op.output(0).node()->def(), attr_adj_x, &ta));
	TF_RETURN_IF_ERROR(
			GetNodeAttr(op.output(0).node()->def(), attr_adj_y, &tb));

	if (!ta && !tb) {
		return MatMulGradHelper(scope, is_batch, grad_inputs[0], false,
				op.input(1), true, op.input(0), true, grad_inputs[0], false,
				grad_outputs);
	} else if (!ta && tb) {
		return MatMulGradHelper(scope, is_batch, grad_inputs[0], false,
				op.input(1), false, grad_inputs[0], true, op.input(0), false,
				grad_outputs);
	} else if (ta && !tb) {
		return MatMulGradHelper(scope, is_batch, op.input(1), false,
				grad_inputs[0], true, op.input(0), false, grad_inputs[0], false,
				grad_outputs);
	}
	return MatMulGradHelper(scope, is_batch, op.input(1), true, grad_inputs[0],
			true, grad_inputs[0], true, op.input(0), true, grad_outputs);
}
