...
public:
	...
	// of the tensor.
	bool HasTensor(const string& name, TensorShape* shape,
			DataType* type) const;
...
private:
	...
	const TensorSliceSet* FindTensorSlice(const string& name,
		...
		const TensorSlice& slice, T* data) const
{
	...
	const TensorSliceSet* tss;
	{
		...
		if (!tss && !all_shards_loaded_) {
			VLOG(1)
					<< "Did not find slice in preferred shard, loading all shards."
	...
	for (const auto& x : details) {
		...
		CHECK(sss_[idx]->Get(key, &value))
				<< "Failed to seek to the record for tensor " << name
				<< ", slice " << slice_s.DebugString() << ": computed key = "
				<< key;
		SavedTensorSlices sts;
		CHECK(ParseProtoUnlimited(&sts, value))
				<< "Failed to parse the record for tensor " << name
				<< ", slice " << slice_s.DebugString() << ": computed key = "
				<< key;
		CopyDataFromTensorSliceToTensorSlice(tss->shape(), slice_s, slice,
