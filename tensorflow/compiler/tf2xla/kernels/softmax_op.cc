...
public:
	explicit SoftmaxOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		auto logits_max = b->Reduce(logits, XlaHelpers::MinValue(b, type),
				max_func, { kClassDim });
		...
		if (log_) {
			...
			auto log_sum_exp = b->Log(
					b->Reduce(b->Exp(shifted_logits), XlaHelpers::Zero(b, type),
							add_func, { kClassDim }));
			...
			auto sum_exp = b->Reduce(exp_shifted, XlaHelpers::Zero(b, type),
					add_func, { kClassDim });
...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		xla::ComputationDataHandle loss = b->Reduce(
				b->Mul(b->Neg(labels), b->Sub(shifted_logits, log_sum_exp, {
						kBatchDim })), XlaHelpers::Zero(b, type), add_func, {
						kClassDim });

		// backprop: prob - labels, where
		//   prob = exp(logits - max_logits) / sum(exp(logits - max_logits))
		//     (where the division broadcasts along the batch dimension)
		xla::ComputationDataHandle backprop = b->Sub(
				b->Div(exp_shifted_logits, sum_exp, { kBatchDim }), labels);
