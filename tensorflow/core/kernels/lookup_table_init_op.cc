...
// process batches.
class KeyValueTensorIterator: public InitializableLookupTable::InitTableIterator {
public:
			...
			keys_(keys), values_(values), valid_(true), status_(Status::OK())
	{
		...
		if (!key_shape.IsSameSize(values_->shape())) {
			...
			status_ = errors::InvalidArgument(
					...
					key_shape.DebugString(), " vs ",
					values_->shape().DebugString());
		}
...
class TextFileLineIterator: public InitializableLookupTable::InitTableIterator {
public:
	TextFileLineIterator() :
			valid_(false), vocab_size_(-1), status_(
					errors::FailedPrecondition("Not initialized"))
	{
	}
			...
			int64 value_index, Env* env)
	{
		if (vocab_size == -1) {
			TF_RETURN_IF_ERROR(
					GetNumLinesInTextFile(env, filename, &vocab_size));
		}
		filename_ = filename;
	...
	void Next() override
	{
		...
		if (!status_.ok()) {
			if (errors::IsOutOfRange(status_) && next_id_ != vocab_size_) {
				status_ = errors::InvalidArgument("Invalid vocab_size in ",
						filename_, ": expected ", vocab_size_, " but got ",
						next_id_);
			}
			valid_ = false;
			return;
		...
		if (line.empty()) {
			status_ = errors::InvalidArgument("Invalid content in ", filename_,
					": empty line found at position ", input_buffer_->Tell(),
					".");
			valid_ = false;
			return;
		}
		...
		if (!ignore_split_) {
			...
			if (std::max(key_index_, value_index_) >= tokens.size()) {
				status_ = errors::InvalidArgument(
						"Invalid number of columns in ", filename_, " line ",
						next_id_, " (", line, ") : expected ",
						std::max(key_index_, value_index_), " got ",
						tokens.size());
				valid_ = false;
				return;
			}
			...
			int64 index, DataType dtype, Tensor* tensor)
	{
		...
		case DT_INT32: {
			...
			if (!strings::safe_strto32(token.c_str(), &value)) {
				valid_ = false;
				return errors::InvalidArgument("Field ", token, " in line ",
						next_id_, " is not a valid int32.");
			}
		...
		case DT_DOUBLE: {
			...
			if (!strings::safe_strtod(token.c_str(), &value)) {
				valid_ = false;
				return errors::InvalidArgument("Field ", token, " in line ",
						next_id_, " is not a valid double.");
			}
		...
		default:
			valid_ = false;
			return errors::InvalidArgument("Data type ", dtype,
					" not supported.");
		}
		return Status::OK();
	}
...
Status InitializeTableFromTextFile(const string& filename, int64 vocab_size,
		char delimiter, int32 key_index, int32 value_index, Env* env,
		InitializableLookupTable* table)
{
	...
	TF_RETURN_IF_ERROR(
			iter.Init(filename, vocab_size, delimiter, table->key_dtype(),
					key_index, table->value_dtype(), value_index, env));
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, TensorShapeUtils::IsVector(values.shape()),
				errors::InvalidArgument(
						"Values must be a vector, but received ",
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				TensorShapeUtils::IsScalar(vocab_filename_tensor.shape()),
				errors::InvalidArgument(
		...
		OP_REQUIRES_OK(ctx,
				lookup::InitializeTableFromTextFile(vocab_filename, vocab_size_,
						delimiter_, key_index_, value_index_, ctx->env(),
						table));
	}
