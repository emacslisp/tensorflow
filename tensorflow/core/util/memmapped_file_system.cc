...
public:
			...
			char* scratch) const override
	{
		...
		const uint64 region_left = std::min(length_ - offset,
				static_cast<uint64>(to_read));
		result->set(reinterpret_cast<const uint8*>(data_) + offset,
				region_left);
		...
		std::unique_ptr<RandomAccessFile>* result)
{
	...
	result->reset(
			new RandomAccessFileFromMemmapped(
		...
		const string& filename, std::unique_ptr<ReadOnlyMemoryRegion>* result)
{
	...
	result->reset(
			new ReadOnlyMemoryRegionFromMemmapped(
					GetMemoryWithOffset(dir_element->second.offset),
					dir_element->second.length));
	return Status::OK();
}
...
Status MemmappedFileSystem::InitializeFromFile(Env* env, const string& filename)
{
	TF_RETURN_IF_ERROR(
	...
	if (!ParseProtoUnlimited(&proto_directory, memory_start + directory_offset,
	...
	for (auto element_iter = proto_directory.element().rbegin();
		...
		if (element_iter->offset() >= prev_element_offset) {
			return errors::DataLoss("Corrupted memmapped model file: ",
					filename, " Invalid offset of internal component");
		}
		if (!directory_.insert(
				std::make_pair(element_iter->name(),
						FileRegion(element_iter->offset(),
								prev_element_offset - element_iter->offset()))).second) {
			return errors::DataLoss("Corrupted memmapped model file: ",
					filename, " Duplicate name of internal component ",
					element_iter->name());
		}
		...
		const string& filename)
{
	...
	for (char c : filename.substr(strlen(kMemmappedPackagePrefix),
...
MemmappedEnv::MemmappedEnv(Env* env) :
		EnvWrapper(env)
{
}
...
{
	std::unique_ptr<MemmappedFileSystem> file_system_ptr(
			new MemmappedFileSystem);
	const auto status = file_system_ptr->InitializeFromFile(target(),
			package_filename);
	if (status.ok()) {
