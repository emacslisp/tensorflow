...
Status test_empty_graph_transform(const GraphDef& graph_def,
		const TransformFuncContext& context, GraphDef* result)
{
	result->Clear();
	return Status::OK();
}
...
protected:
	void TestConstantFolding()
	{
		...
		TF_ASSERT_OK(
				WriteStringToFile(Env::Default(), in_filename_pb,
						graph_def_serialized));

		std::vector<string> args =
				{ "some_binary", "--in_graph=" + in_filename_pb, "--out_graph="
						+ out_filename_pb,
		...
		TF_EXPECT_OK(
				ReadBinaryProto(Env::Default(), out_filename_pb,
						&out_graph_def));
	...
	void TestTransformRegistration()
	{
		...
		TF_ASSERT_OK(root.ToGraphDef(&graph_def));
		Status no_such_status = TransformGraph( { }, { }, { {
				"test_no_such_transform", { } } }, &graph_def);
		EXPECT_TRUE(
				StringPiece(no_such_status.ToString()).contains(
						"not recognized"));
	}
	...
	void TestParseTransformParameters()
	{
		...
		ParseTransformParameters("bar(a=\"1\", b=\"1,2,3\", a=3)",
				&params_list);
		EXPECT_EQ(1, params_list.size());
		EXPECT_EQ("bar", params_list[0].first);
		EXPECT_EQ(1, params_list[0].second.count("a"));
	...
	void TestShouldIgnoreErrors()
	{
		bool ignore_errors;
		TF_EXPECT_OK(
				ShouldIgnoreErrors( { { "ignore_errors", { "true" } } },
						&ignore_errors));
		EXPECT_TRUE(ignore_errors);

		TF_EXPECT_OK(
				ShouldIgnoreErrors( { { "ignore_errors", { "false" } } },
						&ignore_errors));
		...
		EXPECT_FALSE(
				ShouldIgnoreErrors( { { "ignore_errors", { "foo" } } },
						&ignore_errors).ok());
	}
};
