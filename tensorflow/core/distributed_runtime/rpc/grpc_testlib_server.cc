...
Status FillServerDef(const string& job_spec, const string& job_name,
		int num_cpus, int num_gpus, int task_index, ServerDef* options)
{
...
int main(int argc, char* argv[])
{
	...
	int num_gpus = 0;
	int task_index = 0;
	std::vector<tensorflow::Flag> flag_list = { tensorflow::Flag("tf_jobs",
			&job_spec, "job specification"), tensorflow::Flag("tf_job",
			&job_name, "job name"), tensorflow::Flag("tf_task", &task_index,
			"task index"), tensorflow::Flag("num_cpus", &num_cpus,
			"number of CPUs"), tensorflow::Flag("num_gpus", &num_gpus,
			"number of GPUs"), };
	...
	tensorflow::Status s = tensorflow::FillServerDef(job_spec, job_name,
			num_cpus, num_gpus, task_index, &def);
	if (!s.ok()) {
