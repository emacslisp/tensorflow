...
public:
			...
			OpKernel(context)
	{
		...
		if (format_.empty()) {
			...
			OP_REQUIRES(context, false,
					errors::InvalidArgument(
							"format must be '', grayscale or rgb, got ",
							format_));
		}
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("optimize_size", &flags_.optimize_jpeg_size));
		OP_REQUIRES_OK(context,
		...
		string density_unit;
		OP_REQUIRES_OK(context,
				context->GetAttr("density_unit", &density_unit));
		if (density_unit == "in") {
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("x_density", &flags_.x_density));
		OP_REQUIRES_OK(context,
				context->GetAttr("y_density", &flags_.y_density));
		OP_REQUIRES_OK(context,
				context->GetAttr("xmp_metadata", &xmp_metadata_));
