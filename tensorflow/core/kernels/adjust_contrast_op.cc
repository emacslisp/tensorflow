	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<float>::value,
						TensorShape(input.shape()), &mean_values));

		if (input.NumElements() > 0) {
			const int64 batch = input.NumElements()
			...
			functor::AdjustContrast<Device, T>()(
					...
					mean_values.shaped<float, 4>(shape),
...
public:
	...
	void DoCompute(OpKernelContext* context, const ComputeOptions& options) override
	{
		...
		auto input_data = input->shaped<float, 3>(
				{ batch, image_size, channels });
		auto mean_data = mean_values.tensor<float, 2>();
		auto output_data = output->shaped<float, 3>( { batch, image_size,
				channels });
...
private:
			...
			typename TTypes<float, 3>::Tensor scratch)
	{
		...
		for (int64 i = 0; i < batch; i++) {
			auto input_plane = input_flat.slice(Index(i * plane_size),
					Index(plane_size));
			auto summation_plane = summation_scratch.slice(
					Index(i * plane_size), Index(plane_size));
			...
			do {
				...
				if (round == 0) {
					...
					summation_plane.slice(Index(0),
							Index(right_size * channels)) = input_plane.slice(
							Index(left_size * channels),
					...
					if (left_size > right_size) {
						...
						summation_plane.slice(Index(right_size * channels),
								Index(channels)) = input_plane.slice(
								Index(right_size * channels), Index(channels));
					}
				} else {
			...
			auto mean_plane = mean_flat.slice(Index(i * channels),
					Index(channels));
			mean_plane = summation_plane.slice(Index(0), Index(channels))
					* mean_scaling;
		}
	}
			...
			typename TTypes<float, 3>::Tensor outputs)
	{
		...
		for (int64 i = 0; i < batch; i++) {
			...
			while (copied < image_size) {
				...
				int64 to_copy = std::min( { copied, image_size - copied,
						kMaxToCopy });
