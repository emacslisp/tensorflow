		...
		const GraphOptions& graph_options, Item* item)
{
	...
	item->lib_def = new FunctionLibraryDefinition(OpRegistry::Global(),
			gdef.library());
	...
	for (auto& p : partition_graphs) {
		...
		// Find the device.
		Status s = worker_env_->device_mgr->LookupDevice(device_name,
				&unit->device);
		if (!s.ok()) {
		...
		unit->lib = NewFunctionLibraryRuntime(worker_env_->device_mgr,
				worker_env_->env, unit->device, subgraph->versions().producer(),
...
void GraphMgr::StartParallelExecutors(const string& handle, int64 step_id,
		Item* item, Rendezvous* rendezvous, StepStatsCollector* collector,
		CostGraphDef* cost_graph, CancellationManager* cancellation_manager,
		StatusCallback done)
{
	const int num_units = item->units.size();
	CHECK_GE(num_units, 1);
	ScopedStepContainer* step_container = new ScopedStepContainer(step_id,
			[this](const string& name) {
				worker_env_->device_mgr->ClearContainers( {name});
			});
	// NOTE: Transfer one ref of rendezvous and item.
	ExecutorBarrier* barrier = new ExecutorBarrier(num_units, rendezvous,
