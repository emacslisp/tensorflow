...
public:
	...
	{
		return shape.dims() == 0
				|| (allow_legacy_scalars() && shape.dims() == 1
						&& shape.dim_size(0) == 1);
	}
	...
	{
		return shape.dims() == 1
				|| (allow_legacy_scalars() && shape.dims() == 0);
	}
...
public:
	OpKernelConstruction(DeviceType device_type, DeviceBase* device,
			Allocator* allocator, const NodeDef* node_def, const OpDef* op_def,
			FunctionLibraryRuntime* flib, const DataTypeSlice& input_types,
			...
			Status* status) :
			device_type_(device_type), device_(device), allocator_(allocator), def_(
					node_def), op_def_(op_def), flib_(flib), input_types_(
					input_types), input_memory_types_(input_memory_types), output_types_(
					output_types), output_memory_types_(output_memory_types), graph_def_version_(
	...
	const int graph_def_version() const
	{
...
public:
	...
	OpArgIterator(const ListType* list, int i) :
			list_(list), i_(i)
	{
	}
...
struct TensorValue {
	TensorValue() :
			mutex_if_ref(nullptr), tensor(nullptr)
	{
	}
...
public:
	...
	FunctionCallFrame* call_frame() const
	{
...
std::unique_ptr<OpKernel> CreateOpKernel(DeviceType device_type,
		DeviceBase* device, Allocator* allocator, const NodeDef& def,
		int graph_def_version, Status* status);
Status CreateOpKernel(DeviceType device_type, DeviceBase* device,
		Allocator* allocator, FunctionLibraryRuntime* flib, const NodeDef& def,
