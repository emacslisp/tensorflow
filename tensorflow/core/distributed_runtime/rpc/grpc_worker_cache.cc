...
public:
	explicit GrpcWorkerCache(GrpcChannelCache* channel_cache,
			...
			local_target_(local_target), local_worker_(local_worker), channel_cache_(
					channel_cache)
	{
		// TODO(mrry): Investigate possible performance improvements by
		// replacing this thread with a threadpool.
		polling_thread_ =
				Env::Default()->StartThread(ThreadOptions(),
						"grpc_worker_cache",
						[this]() {
							void* tag;
							bool ok;
	...
	WorkerInterface* CreateWorker(const string& target) override
	{
		if (target == local_target_) {
			...
			SharedGrpcChannelPtr channel = channel_cache_->FindWorkerChannel(
					target);
			if (!channel)
				return nullptr;
			WorkerInterface* ret = NewGrpcRemoteWorker(channel,
...
WorkerCacheInterface* NewGrpcWorkerCacheWithLocalWorker(GrpcChannelCache* cc,
		WorkerInterface* local_worker, const string& local_target)
{
