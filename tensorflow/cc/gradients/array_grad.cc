		...
		std::vector<Output>* grad_outputs)
{
	...
	grad_outputs->push_back(
			ScatterNd(scope, indices, grad_inputs[0], ref_shape));
	grad_outputs->push_back(NoGradient());
	return scope.status();
}
...
{
	grad_outputs->push_back(
			CheckNumerics(scope, grad_inputs[0],
		...
		std::vector<Output>* grad_outputs)
{
	...
	grad_outputs->push_back(
			Slice(scope, grad_inputs[0], begin, Shape(scope, x)));
	grad_outputs->push_back(NoGradient());
	return scope.status();
}
		...
		std::vector<Output>* grad_outputs)
{
	int block_size;
	TF_RETURN_IF_ERROR(
			GetNodeAttr(op.node()->def(), "block_size", &block_size));
	grad_outputs->push_back(
		...
		std::vector<Output>* grad_outputs)
{
	...
	grad_outputs->push_back(
			tensorflow::ops::internal::MirrorPadGrad(scope, grad_inputs[0],
					op.input(1), mode));
	grad_outputs->push_back(NoGradient());
	return scope.status();
}
		...
		std::vector<Output>* grad_outputs)
{
	...
	TF_RETURN_IF_ERROR(GetNodeAttr(op.node()->def(), "mode", &mode));
	grad_outputs->push_back(
			MirrorPad(scope, grad_inputs[0], op.input(1), mode));
	grad_outputs->push_back(NoGradient());
	return scope.status();
}
