	...
	while (inst_it != computation->instructions().end()) {
		...
		if (instruction->opcode() == HloOpcode::kConstant) {
			...
			for (auto it = range.first; it != range.second; ++it) {
				if (LiteralUtil::Equal(instruction->literal(),
						it->second->literal())) {
					match = it->second;
					break;
				}
			...
			if (match == nullptr) {
				...
				TF_CHECK_OK(
						computation->ReplaceUsesOfInstruction(instruction,
								match));
	...
	for (auto& computation : module->computations()) {
		...
		for (auto instruction : post_order) {
			...
			for (HloInstruction* equivalent_instruction : equivalent_instructions) {
				TF_RETURN_IF_ERROR(
						computation->ReplaceUsesOfInstruction(
