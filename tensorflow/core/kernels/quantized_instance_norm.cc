...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("output_range_given", &output_range_given_));
		if (output_range_given_) {
			OP_REQUIRES_OK(context,
					context->GetAttr("given_y_min", &given_y_min_));
			OP_REQUIRES_OK(context,
					context->GetAttr("given_y_max", &given_y_max_));
	...
	void Compute(OpKernelContext* context) override
	{
...
#endif
		{
			...
			float_mean = input_tensor.cast<float>().reduce(reduction_indices,
					Eigen::internal::MeanReducer<float>());

			float_variance = (input_scale
					* ((input_tensor.cast<float>()
							- float_mean.reshape(expand_spec).broadcast(
									broadcast_spec)))).square().reduce(
			...
			Eigen::Tensor<float, 4, Eigen::RowMajor> instance_normed =
					input_scale
							* (input_tensor.cast<float>()
									- float_mean.reshape(expand_spec).broadcast(
											broadcast_spec))
							* (float_variance + variance_epsilon_).rsqrt().reshape(
									expand_spec).broadcast(broadcast_spec);
			...
			FloatToQuantizedStruct<quint8> output_f2q(normed_min(),
					normed_max());
			auto instance_normed_quantized = QUANTIZE_WITH_EIGEN(
					instance_normed, output_f2q, quint8);
