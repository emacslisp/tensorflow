...
void SetDefaultLayoutToContainer(
		tensorflow::protobuf::RepeatedField<tensorflow::protobuf_int64>* minor_to_major)
{
	...
	case DimensionOrder::kRandom:
		...
		std::shuffle(minor_to_major->begin(), minor_to_major->end(),
				std::mt19937(
						default_layout.seed != 0 ?
								default_layout.seed : std::random_device()()));
	}
}
...
/* static */void LayoutUtil::SetToDefaultLayout(Shape* shape)
{
	if (ShapeUtil::IsTuple(*shape)) {
		...
		tensorflow::protobuf::RepeatedField<tensorflow::protobuf_int64>* minor_to_major =
				shape->mutable_layout()->mutable_minor_to_major();
			...
			if (layout.padded_dimensions(i) < shape.dimensions(i)) {
				return InvalidArgument(
						"for dimension %d, dimension padding (%lld) is smaller than "
								"the dimension size (%lld) of the shape", i,
						layout.padded_dimensions(i), shape.dimensions(i));
			}
		}
	}
	return tensorflow::Status::OK();
...
/* static */bool LayoutUtil::HasLayout(const Shape& shape)
{
	if (ShapeUtil::IsTuple(shape)) {
		...
		return std::all_of(shape.tuple_shapes().begin(),
				shape.tuple_shapes().end(),
		...
		const Layout& layout)
{
	...
	for (int64 physical = 0; physical < logical_to_physical.size();
			++physical) {
...
tensorflow::Status CopyLayoutInternal(const Shape& src, Shape* dst)
{
	...
	if (ShapeUtil::IsTuple(src)) {
		...
		for (int64 i = 0; i < ShapeUtil::TupleElementCount(src); ++i) {
			TF_RETURN_IF_ERROR(
					CopyLayoutInternal(src.tuple_shapes(i),
		...
		const Shape& rhs)
{
	...
	if (ShapeUtil::IsTuple(lhs)) {
		...
		for (int i = 0; i < ShapeUtil::TupleElementCount(lhs); ++i) {
			if (!LayoutsInShapesEqual(lhs.tuple_shapes(i),
					rhs.tuple_shapes(i))) {
				return false;
			}
		}
...
/* static */bool LayoutUtil::AreDimensionsConsecutive(const Layout& layout,
