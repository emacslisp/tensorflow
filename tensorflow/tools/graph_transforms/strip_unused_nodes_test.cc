...
protected:
	void TestSimpleAdd()
	{
		...
		GraphDef result;
		TF_ASSERT_OK(
				StripUnusedNodes(graph_def, { { }, { "add_node" } }, &result));
	...
	void TestCommonAncestor()
	{
		...
		GraphDef result;
		TF_ASSERT_OK(StripUnusedNodes(graph_def, { { "dangling_input" }, {
				"add_node1" } }, &result));
	...
	void TestSimplePlaceholder()
	{
		...
		GraphDef result;
		TF_ASSERT_OK(StripUnusedNodes(graph_def, { { "mul_node" },
				{ "add_node" } }, &result));
		...
		EXPECT_EQ(TensorShape( { }),
				TensorShape(
						node_lookup["mul_node"]->attr().at("shape").shape()));
		EXPECT_EQ(1, node_lookup.count("a_node"));
	...
	void TestPlaceholderNamedArgs()
	{
		...
		GraphDef result;
		TF_ASSERT_OK(StripUnusedNodes(graph_def, { { "mul_node", "a_node" }, {
				"add_node" }, { { "name", { "a_node", "mul_node" } }, {
				"type_for_name", { "int64", "quint8" } }, { "shape_for_name", {
				"1,2", "1, 2, 3" } } } }, &result));
		...
		EXPECT_EQ("Placeholder", node_lookup["mul_node"]->op());
		EXPECT_EQ(DT_QUINT8,
				node_lookup["mul_node"]->attr().at("dtype").type());
		EXPECT_EQ(TensorShape( { 1, 2, 3 }),
				TensorShape(
						node_lookup["mul_node"]->attr().at("shape").shape()));
		EXPECT_EQ(1, node_lookup.count("a_node"));
