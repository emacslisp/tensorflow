...
void Worker::RegisterGraphAsync(const RegisterGraphRequest* request,
		RegisterGraphResponse* response, StatusCallback done)
{
	Status s = env_->graph_mgr->Register(request->session_handle(),
			request->graph_def(), request->graph_options(),
			response->mutable_graph_handle());
	done(s);
}

void Worker::DeregisterGraphAsync(const DeregisterGraphRequest* request,
		DeregisterGraphResponse* response, StatusCallback done)
{
...
Status Worker::PrepareRunGraph(RunGraphRequestWrapper* req,
		GraphMgr::NamedTensors* in, GraphMgr::NamedTensors* out)
{
...
void Worker::RunGraphAsync(CallOptions* opts, RunGraphRequestWrapper* request,
		MutableRunGraphResponseWrapper* response, StatusCallback done)
{
		...
		MutableRunGraphResponseWrapper* response, StatusCallback done)
{
	...
	CancellationToken token;
	{
		...
		bool already_cancelled = !cancellation_manager_->RegisterCallback(token,
				[cm]() {cm->StartCancel();});
	...
	env_->graph_mgr->ExecuteAsync(request->graph_handle(), step_id,
			request->exec_opts(), collector, cost_graph, cm, in,
			[this, step_id, response, cm, out, token, collector,
			opts, done](Status s) {
				if (s.ok()) {
		...
		MutableRunGraphResponseWrapper* response, StatusCallback done)
{
	...
	if (partial_run_state == nullptr) {
		...
		env_->graph_mgr->ExecuteAsync(graph_handle, step_id,
				request->exec_opts(), nullptr /* collector */,
...
void Worker::CleanupGraphAsync(const CleanupGraphRequest* request,
		CleanupGraphResponse* response, StatusCallback done)
{
	const int64 step_id = request->step_id();
...
void Worker::CleanupAllAsync(const CleanupAllRequest* request,
		CleanupAllResponse* response, StatusCallback done)
{
	std::vector < string > containers;
	for (const auto& c : request->container())
