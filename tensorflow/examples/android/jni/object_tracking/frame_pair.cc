...
void FramePair::AdjustBox(const BoundingBox box, float* const translation_x,
		...
		float* const scale_y) const
{
	...
	*translation_y = translation.y;
	...
	const int good_scale_points = FillScales(old_center, translation, weights,
			deltas);
...
int FramePair::FillWeights(const BoundingBox& box, float* const weights) const
{
	...
	for (int i = 0; i < kMaxKeypoints; ++i) {
		...
		if (!in_box) {
			...
			const float sq_x_dist = Square(
					initial.x - frame1_keypoints_[i].pos_.x);
			const float sq_y_dist = Square(
					initial.y - frame1_keypoints_[i].pos_.y);
		...
		if (max_score > min_score) {
			...
			intrinsic_score = ((frame1_keypoints_[i].score_ - min_score)
					/ (max_score - min_score)) * (1.0f - kBaseScore)
					+ kBaseScore;
		}
...
void FramePair::FillTranslations(Point2f* const translations) const
{
	for (int i = 0; i < kMaxKeypoints; ++i) {
		...
		translations[i].x = frame2_keypoints_[i].pos_.x
				- frame1_keypoints_[i].pos_.x;
		translations[i].y = frame2_keypoints_[i].pos_.y
				- frame1_keypoints_[i].pos_.y;
	}
}

int FramePair::FillScales(const Point2f& old_center, const Point2f& translation,
...
static float GetMedian(const int num_items,
		const WeightedDelta* const weighted_deltas, const float sum)
{
...
Point2f FramePair::GetWeightedMedian(const float* const weights,
		const Point2f* const deltas) const
{
	...
	// Compute median X value.
	{
		...
		median_delta.x = GetMedian(kMaxKeypoints, weighted_deltas,
				total_weight);
	}
	...
	// Compute median Y value.
	{
		...
		qsort(weighted_deltas, kMaxKeypoints, sizeof(WeightedDelta),
				WeightedDeltaCompare);
		median_delta.y = GetMedian(kMaxKeypoints, weighted_deltas,
				total_weight);
	}
...
float FramePair::GetWeightedMedianScale(const float* const weights,
		const Point2f* const deltas) const
{
	float median_delta;
	...
	// Compute median scale value across x and y.
	{
		...
		median_delta = GetMedian(kMaxKeypoints * 2, weighted_deltas,
				total_weight);
	}
