...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, params.forward_output_shape(),
						&output));
		const int32 highest = static_cast<int32>(Eigen::NumTraits < T
				> ::highest());
		const int32 lowest = static_cast<int32>(Eigen::NumTraits < T
				> ::lowest());
		...
		SpatialAvgPool<Device, int32>(context, &int32_output, int32_input,
				params, padding_);

		// Clamp the int32 output back into quantized space.
		output->flat<T>() =
				int32_output.flat<int32>().cwiseMax(lowest).cwiseMin(highest).template cast<
						T>();
