...
struct MultinomialFunctor {
	void operator()(OpKernelContext* ctx, const Device& d,
			...
			int num_classes, int num_samples, const random::PhiloxRandom& gen,
...
struct MultinomialFunctor<CPUDevice, T> {
			...
			typename TTypes<int64>::Matrix output)
	{
		...
		const int64 cost = 50
				* (num_samples * std::log(num_classes) / std::log(2)
						+ num_classes);
...
public:
	explicit MultinomialOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		Tensor* samples_t;
		OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape( { batch_size,
				num_samples }), &samples_t));
		...
		if (samples_t->NumElements() > 0) {
			...
			if (std::is_same<Device, GPUDevice>::value) {
				OP_REQUIRES_OK(ctx, ctx->allocate_temp(DT_FLOAT, TensorShape( {
						batch_size, num_samples, num_classes }), &noises));
				OP_REQUIRES_OK(ctx, ctx->allocate_temp(DT_FLOAT, TensorShape( {
						batch_size, num_samples, num_classes }), &scores));
				OP_REQUIRES_OK(ctx, ctx->allocate_temp(DT_FLOAT, TensorShape( {
						batch_size, num_samples }), &scratch));
			}

			const int num_samples_ceil_4 = (num_samples + 3) / 4 * 4;
			auto rng = generator_.ReserveRandomOutputs(
					batch_size * num_samples_ceil_4, 256);
			functor::MultinomialFunctor<Device, T>()(ctx,
					ctx->eigen_device<Device>(), logits_t.matrix<T>(),
					noises.flat<float>(), scores.flat<float>(),
					scratch.flat<float>(), batch_size, num_classes, num_samples,
					rng, samples_t->matrix<int64>());
		}
	}
