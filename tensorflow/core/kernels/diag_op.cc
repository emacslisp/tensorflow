...
public:
	explicit DiagonalGenerator(const Tensor& diagonal) :
			diagonal_(diagonal)
	{
...
public:
	explicit DiagonalExtractor(const Tensor& tensor) :
			tensor_(tensor)
	{
...
public:
	explicit DiagOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		switch (num_dims) {
		case 1:
			output_tensor->tensor<T, 2>() =
					output_tensor->tensor<T, 2>().generate(
							DiagonalGenerator<T, 1, 2>(diagonal));
			break;
		case 2:
			output_tensor->tensor<T, 4>() =
					output_tensor->tensor<T, 4>().generate(
							DiagonalGenerator<T, 2, 4>(diagonal));
			break;
		case 3:
			output_tensor->tensor<T, 6>() =
					output_tensor->tensor<T, 6>().generate(
							DiagonalGenerator<T, 3, 6>(diagonal));
			break;
		default:
			context->SetStatus(
					errors::Unimplemented("Diagonal of rank ", num_dims,
...
public:
	explicit DiagPartOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, 2 == num_dims || 4 == num_dims || 6 == num_dims,
				errors::InvalidArgument(
						"The rank of the tensor should be 2, \
                                         4, or 6, got shape ",
		...
		for (int i = 0; i < out_dims; i++) {
			OP_REQUIRES(context,
					...
					errors::InvalidArgument("Invalid shape ",
							tensor.shape().DebugString(), ": dimensions ", i,
							" and ", i + out_dims, " do not match."));
		}
