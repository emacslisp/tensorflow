...
public:
	explicit ShapeOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		Tensor* out = nullptr;
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(0, TensorShape( { rank }), &out));
		auto vec = out->vec<OutType>();
		for (int i = 0; i < rank; ++i) {
			int64 dim_size = inp.dim_size(i);
			if (out->dtype() == DT_INT32) {
				OP_REQUIRES(ctx,
						FastBoundsCheck(dim_size,
								std::numeric_limits<int32>::max()),
						errors::InvalidArgument("Shape output type is 32-bit ",
...
public:
	explicit ShapeNOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
				...
				if (out->dtype() == DT_INT32) {
					OP_REQUIRES(ctx,
							...
							errors::InvalidArgument(
									"ShapeN output type is 32-bit but shape ",
									i, " dim ", j, " is ", dim_size));
				}
...
public:
	explicit RankOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
...
public:
	explicit SizeOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		if (out->dtype() == DT_INT32) {
			OP_REQUIRES(ctx,
					...
					errors::InvalidArgument(
							"Number of elements was larger than "
...
public:
	explicit ExpandDimsOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				(dim >= -1 - ctx->input(0).dims() && dim <= ctx->input(0).dims()),
		...
		if (!output->CopyFrom(ctx->input(0), output_shape)) {
			...
			ctx->SetStatus(
					errors::Internal(
							"Could not expand dimension with input shape ",
...
public:
	explicit SqueezeOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		if (!output->CopyFrom(ctx->input(0), output_shape)) {
			...
			ctx->SetStatus(
					errors::Internal("Could not squeeze input with shape ",
							ctx->input(0).shape().DebugString(),
							" and output shape ", output_shape.DebugString()));
		}
	}
