...
private:
	mutex mu_;
	condition_variable non_empty_cond_var_;std::deque<Tuple> buf_ GUARDED_BY(mu_);
};
...
Status GetBuffer(OpKernelContext* ctx, const NodeDef& ndef, Buffer** buf)
{
	...
	TF_RETURN_IF_ERROR(
			rm->LookupOrCreate < Buffer
					> (cinfo.container(), cinfo.name(), buf, CreateBuffer));
	return Status::OK();
}
...
public:
	explicit StageOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
...
public:
	explicit UnstageOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		Buffer::Tuple tuple;
		buf->Get(&tuple);
		OP_REQUIRES(ctx, tuple.size() == ctx->num_outputs(),
				errors::InvalidArgument("Mismatch stage/unstage: ",
						tuple.size(), " vs. ", ctx->num_outputs()));
