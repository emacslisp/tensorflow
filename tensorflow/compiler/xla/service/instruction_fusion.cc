...
StatusOr<bool> InstructionFusion::Run(HloModule* module)
{
	...
	for (auto& computation : module->computations()) {
		...
		while (!post_order.empty()) {
			...
			std::sort(sorted_operand_numbers.begin(),
					sorted_operand_numbers.end(), [&](int64 i, int64 j) {
			...
			for (int64 i : sorted_operand_numbers) {
				...
				if (operand->IsFusable() && ShouldFuse(instruction, i)) {
					HloInstruction* fusion_instruction = Fuse(operand,
							instruction);
					...
					if (operand->user_count() == 0) {
						...
						// location to nullptr.
						post_order[FindOrDie(post_order_index, operand)] =
								nullptr;
		...
		HloInstruction* consumer)
{
	...
	if (consumer->opcode() == HloOpcode::kFusion) {
		...
		fusion_instruction = computation_->AddInstruction(
				HloInstruction::CreateFusion(consumer->shape(),
						ChooseKind(producer, consumer), consumer));
		TF_CHECK_OK(
