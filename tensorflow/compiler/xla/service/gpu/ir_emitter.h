...
protected:
	...
	{
		return ir_emitter_context_->buffer_assignment().GetUniqueTopLevelAllocation(
				&hlo).ConsumeValueOrDie()->index();
	}
	...
	virtual Status EmitTargetElementLoop(const HloInstruction& hlo,
...
public:
	...
	Status HandleRng(HloInstruction* random, RandomDistribution distribution)
			override;
	Status HandleSelect(HloInstruction* select, HloInstruction* pred,
			HloInstruction* on_true, HloInstruction* on_false) override;

	Status EmitTargetElementLoop(const HloInstruction& hlo,
			const llvm_ir::ElementGenerator& body_emitter) override;

	// Same as `EmitTargetElementLoop`, but in given `thunk` rather than
	// `LastThunk()`.
	Status EmitTargetElementLoopInThunk(const HloInstruction& hlo,
			const llvm_ir::ElementGenerator& body_emitter, KernelThunk* thunk);
...
private:
	...
	llvm::Function* BuildKernelPrototype(const HloInstruction& inst,
	...
	Status EmitColumnReduction(int64 height, int64 width,
	...
	Status EmitReductionToVector(HloInstruction* reduce,
