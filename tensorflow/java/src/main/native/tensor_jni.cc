		...
		size_t src_size, jarray dst)
{
	...
	if (sz > src_size) {
		throwException(env, kIllegalStateException,
				"cannot fill a Java array of %d bytes with a Tensor of %d bytes",
				sz, src_size);
		return 0;
	}
	switch (dtype) {
		...
		char* dst, size_t dst_size)
{
	if (dims_left == 1) {
		...
		size_t sz = 0;
		for (int i = 0; i < len; ++i) {
			jarray row = static_cast<jarray>(env->GetObjectArrayElement(ndarray,
					i));
			sz += writeNDArray(env, row, dtype, dims_left - 1, dst + sz,
					dst_size - sz);
		...
		size_t src_size, int dims_left, jarray dst)
{
	if (dims_left == 1) {
		...
		for (int i = 0; i < len; ++i) {
			jarray row = static_cast<jarray>(env->GetObjectArrayElement(ndarray,
					i));
			sz += readNDArray(env, dtype, src + sz, src_size - sz,
					dims_left - 1, row);
			env->DeleteLocalRef(row);
			if (env->ExceptionCheck())
				return sz;
...
JNIEXPORT jlong JNICALL Java_org_tensorflow_Tensor_allocate(JNIEnv* env,
		jclass clazz, jint dtype, jlongArray shape)
{
...
JNIEXPORT jbyteArray JNICALL Java_org_tensorflow_Tensor_scalarBytes(JNIEnv* env,
		jclass clazz, jlong handle)
{
	TF_Tensor* t = requireHandle(env, handle);
	if (t == nullptr)
