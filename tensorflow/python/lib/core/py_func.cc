...
bool IsSingleNone(PyObject* obj)
{
	...
	char* item_ptr = static_cast<char*>(PyArray_GetPtr(array_obj,
			indices.data()));
...
Status ConvertNdarrayToTensor(PyObject* obj, Tensor* ret)
{
	...
	case NPY_OBJECT: {
		...
		PyObject** input_data =
				reinterpret_cast<PyObject**>(PyArray_DATA(input));
	...
	default: {
		TF_RETURN_IF_ERROR(
				NumericNpDTypeToTfDType(PyArray_TYPE(input), &dtype));
		Tensor t(dtype, shape);
	...
	if (typenum == NPY_OBJECT) {
		...
		for (int i = 0; i < tflat.dimension(0); ++i) {
			...
			if (out[i] == nullptr) {
				...
				Py_DECREF(obj);
				return errors::Internal("Failed to allocate a copy of string ",
						i);
			}
		}
	} else {
...
public:
	explicit PyFuncOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				static_cast<int32>(call.out.size()) == ctx->num_outputs(),
				errors::InvalidArgument(token_, " returns ", call.out.size(),
						" values, but expects to see ", ctx->num_outputs(),
						" values."));
		for (size_t i = 0; i < call.out.size(); ++i) {
			...
			OP_REQUIRES(ctx, t.dtype() == output_type(i),
					errors::InvalidArgument(i, "-th value returned by ", token_,
							" is ", DataTypeString(t.dtype()), ", but expects ",
