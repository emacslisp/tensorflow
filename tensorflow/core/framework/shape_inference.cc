...
InferenceContext::InferenceContext(int graph_def_version,
		...
		graph_def_version_(graph_def_version), node_def_(
		...
		const std::vector<ShapeHandle>& shapes)
{
	...
	if (result == output_name_map_.end()) {
		...
		if (size != shapes.size()) {
			errors::InvalidArgument("Must have exactly ", shapes.size(),
					" shapes.");
		}
		...
		const std::vector<ShapeHandle>& input_tensors_as_shapes)
{
	...
	construction_status_ = NameRangesForNode(node_def_, op_def,
		...
		const std::vector<DataType>& input_handle_dtypes)
{
	...
	for (const auto& e : input_name_map_) {
		num_inputs_from_node_def = std::max(num_inputs_from_node_def,
				e.second.second);
	}
	...
	if (input_handle_shapes.empty()) {
		...
		if (input_handle_shape_.size() != inputs_.size()) {
			construction_status_ = errors::InvalidArgument(
					"Wrong number of handle shapes passed; expected ",
					inputs_.size(), " got ", input_handle_shape_.size());
		}
	}
	if (input_handle_dtypes.empty()) {
		...
		if (input_handle_dtype_.size() != inputs_.size()) {
			construction_status_ = errors::InvalidArgument(
					"Wrong number of handle dtypes passed; expected ",
					inputs_.size(), " got ", input_handle_dtype_.size());
		}
	}
...
Status InferenceContext::Merge(ShapeHandle s0, ShapeHandle s1, ShapeHandle* out)
{
	...
	if (rank != Rank(s1)) {
		*out = nullptr;
		return errors::InvalidArgument("Shapes must be equal rank, but are ",
				rank, " and ", Rank(s1));
	}
		...
		if (v0 == kUnknownDim) {
			...
			*out = nullptr;
			return errors::InvalidArgument("Dimension ", i,
					" in both shapes must be equal, but are ", Value(d0),
					" and ", Value(d1));
		}
	}
		...
		ShapeHandle* out)
{
		...
		if (start < 0) {
			*out = nullptr;
			return errors::InvalidArgument("Subshape start out of bounds: ",
					start_in, ", for shape with rank ", rank);
		}
	}
	...
	if (end < 0) {
		...
		if (end < 0) {
			*out = nullptr;
			return errors::InvalidArgument("Subshape end out of bounds: ",
					end_in, ", for shape with rank ", rank);
		}
	}
	if (start > end) {
		*out = nullptr;
		return errors::InvalidArgument(
				"Subshape must have computed start <= end, but is ", start,
				" and ", end, " (computed from start ", start_in, " and end ",
				end_in, " over shape with rank ", rank, ")");
	}
	std::vector<DimensionHandle> dims;
		...
		ShapeHandle* out)
{
	...
	std::vector<DimensionHandle> dims;
	dims.reserve(rank);
	for (int i = 0; i < s1_rank; ++i)
		dims.push_back(Dim(s1, i));
	for (int i = 0; i < s2_rank; ++i)
		dims.push_back(Dim(s2, i));
	return ReturnCreatedShape(dims, out);
}
...
Status InferenceContext::MakeDimForScalarInput(int idx, DimensionHandle* out)
{
	...
	if (rank != 0) {
		return errors::InvalidArgument("Input must be scalar but has rank ",
				rank);
	}
...
Status InferenceContext::Divide(DimensionHandle dividend,
		DimensionOrConstant divisor, bool evenly_divisible,
		DimensionHandle* out)
{
		...
		DimensionHandle* out)
{
	...
	if (first_value == 0) {
		...
		*out = UnknownDim();
		...
		if (sum < 0) {
			return errors::InvalidArgument(
					"Dimension size overflow from adding ", first_value,
					" and ", second_value);
		}
		*out = MakeDim(sum);
	}
...
Status InferenceContext::Subtract(DimensionHandle first,
		DimensionOrConstant second, DimensionHandle* out)
{
	const int64 first_value = Value(first);
	const int64 second_value = Value(second);
	// Special cases.
...
Status InferenceContext::AttachContext(const Status& status)
{
	...
	string error_context = strings::StrCat(" for '", node_def_.name(),
			"' (op: '", node_def_.op(), "') with input shapes: ",
			str_util::Join(input_shapes, ", "), ".");
