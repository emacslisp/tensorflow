...
public:
	...
	GPUDeviceContext(int stream_id, gpu::Stream* stream,
			...
			stream_id_(stream_id), stream_(stream), host_to_device_stream_(
					host_to_device_stream), device_to_host_stream_(
					device_to_host_stream), device_to_device_stream_(
	...
	gpu::Stream* host_to_device_stream() const
	{
		return host_to_device_stream_;
	}
	gpu::Stream* device_to_host_stream() const
	{
	...
	void CopyCPUTensorToDevice(const Tensor* cpu_tensor, Device* device,
			Tensor* device_tensor, StatusCallback done) const override;

	void CopyDeviceTensorToCPU(const Tensor* device_tensor,
			StringPiece edge_name, Device* device, Tensor* cpu_tensor,
