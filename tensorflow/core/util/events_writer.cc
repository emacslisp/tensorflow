...
:
		env_(Env::Default()), file_prefix_(file_prefix), num_outstanding_events_(
				0)
{
}
...
bool EventsWriter::Init()
{
	if (recordio_writer_.get() != nullptr) {
		...
		if (FileHasDisappeared()) {
			...
			if (num_outstanding_events_ > 0) {
				LOG(WARNING)
						<< "Re-initialization, attempting to open a new file, "
	...
	filename_ = strings::Printf("%s.out.tfevents.%010lld.%s",
			file_prefix_.c_str(), static_cast<long long>(time_in_seconds),
			port::Hostname().c_str());
	...
	VLOG(1) << "Successfully opened events file: " << filename_;
	{
		...
		event.set_file_version(
				strings::StrCat(kVersionPrefix, kCurrentVersion));
		WriteEvent(event);
		Flush();
	}
	return true;
...
bool EventsWriter::Flush()
{
	...
	if (!recordio_writer_->Flush().ok()) {
		LOG(ERROR) << "Failed to flush " << num_outstanding_events_
				<< " events to " << filename_;
		return false;
	}
...
bool EventsWriter::Close()
{
	...
	if (recordio_file_.get() != NULL) {
		...
		if (!s.ok()) {
			LOG(ERROR) << "Error when closing previous event file: "
					<< filename_ << ": " << s;
			return_value = false;
		}
