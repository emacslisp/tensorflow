...
void ParseSizes(OpKernelContext* context, const std::vector<int32>& strides,
		...
		int* stride_rows, int* stride_cols, int* rate_rows, int* rate_cols,
		int64* pad_top, int64* pad_left, int64* out_rows, int64* out_cols)
{
	...
	*rate_cols = rates[2];
	...
	OP_REQUIRES(context, depth == filter.dim_size(2),
			errors::InvalidArgument(
					"input and filter must have the same depth: ", depth,
					" vs ", filter.dim_size(2)));

	// Effective filter size, after introducing rate - 1 zeros between each
	// non-zero filter element.
	const int filter_rows_eff = filter_rows
			+ (filter_rows - 1) * (*rate_rows - 1);
	const int filter_cols_eff = filter_cols
			+ (filter_cols - 1) * (*rate_cols - 1);
...
public:
	explicit DilationOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		ParseSizes(context, strides_, rates_, padding_, &stride_rows,
		...
		functor::Dilation<Device, T>()(context->eigen_device<Device>(),
				input.tensor<T, 4>(), filter.tensor<T, 3>(), stride_rows,
				stride_cols, rate_rows, rate_cols, pad_top, pad_left,
				output->tensor<T, 4>());
	}
							...
							if (h_in >= 0 && h_in < input_rows) {
								for (int w = 0; w < filter_cols; ++w) {
									...
									if (w_in >= 0 && w_in < input_cols) {
										const T val = input(b, h_in, w_in, d)
												+ filter(h, w, d);
										if (val > cur_val) {
											cur_val = val;
										}
...
struct DilationBackpropInput<CPUDevice, T> {
	void operator()(const CPUDevice& d,
			...
			typename TTypes<T, 4>::ConstTensor out_backprop, int stride_rows,
			int stride_cols, int rate_rows, int rate_cols, int pad_top,
			int pad_left, typename TTypes<T, 4>::Tensor in_backprop)
	{
		const int batch = input.dimension(0);
		const int input_rows = input.dimension(1);
		const int input_cols = input.dimension(2);
			...
			for (int h_out = 0; h_out < output_rows; ++h_out) {
				...
				for (int w_out = 0; w_out < output_cols; ++w_out) {
					...
					for (int d = 0; d < depth; ++d) {
						...
						in_backprop(b, h_in_max, w_in_max, d) += out_backprop(b,
								h_out, w_out, d);
					}
				}
			}
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, filter.shape(), &filter_backprop));
...
struct DilationBackpropFilter<CPUDevice, T> {
	void operator()(const CPUDevice& d,
			...
			typename TTypes<T, 4>::ConstTensor out_backprop, int stride_rows,
			int stride_cols, int rate_rows, int rate_cols, int pad_top,
			int pad_left, typename TTypes<T, 3>::Tensor filter_backprop)
	{
		const int batch = input.dimension(0);
		const int input_rows = input.dimension(1);
		const int input_cols = input.dimension(2);
			...
			for (int h_out = 0; h_out < output_rows; ++h_out) {
				...
				for (int w_out = 0; w_out < output_cols; ++w_out) {
					...
					for (int d = 0; d < depth; ++d) {
						...
						filter_backprop(h_max, w_max, d) += out_backprop(b,
								h_out, w_out, d);
					}
				}
			}
