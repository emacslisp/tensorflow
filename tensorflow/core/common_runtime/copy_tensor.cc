...
struct RegistrationInfo {
	RegistrationInfo(DeviceType s, DeviceType r, CopyTensor::CopyFunction cf) :
			sender_device_type(std::move(s)), receiver_device_type(
					std::move(r)), copy_function(cf)
	{
	}
...
{
	static std::vector<RegistrationInfo>* registry = new std::vector<
			RegistrationInfo>;
	return registry;
}
		...
		Tensor* output, StatusCallback done)
{
	...
	const DeviceType src_device_type(
			src_alloc_attr.on_host() ?
					DEVICE_CPU : src->attributes().device_type());
	const DeviceType dst_device_type(
			dst_alloc_attr.on_host() ?
					DEVICE_CPU : dst->attributes().device_type());
	...
	if (non_cpu_src && non_cpu_dst) {
		...
		Tensor* cpu_tensor = new Tensor(cpu_allocator, input->dtype(),
				input->shape());
		auto delete_and_done = [cpu_tensor, done](const Status& status) {
			delete cpu_tensor;
			done(status);
		};
		send_dev_context->CopyDeviceTensorToCPU(input, edge_name, src,
				cpu_tensor,
...
Status CopyTensor::Register(DeviceType sender_device_type,
		DeviceType receiver_device_type, CopyFunction copy_function)
{
	std::vector<RegistrationInfo>* registry = MutableRegistry();
