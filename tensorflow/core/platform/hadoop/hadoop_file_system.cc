...
Status BindFunc(void* handle, const char* name, std::function<R(Args...)>* func)
{
	...
	*func = reinterpret_cast<R (*)(Args...)>(symbol_ptr);return
Status	::OK();
}
...
private:
	...
	{
		auto TryLoadAndBind =
				[this](const char* name, void** handle) -> Status {
...
public:
	HDFSRandomAccessFile(const string& filename, const string& hdfs_filename,
			...
			filename_(filename), hdfs_filename_(hdfs_filename), hdfs_(hdfs), fs_(
					fs), file_(file)
	{
	}
			...
			char* scratch) const override
	{
		...
		while (n > 0 && s.ok()) {
			...
			if (r > 0) {
				...
				file_ = hdfs_->hdfsOpenFile(fs_, hdfs_filename_.c_str(),
						O_RDONLY, 0, 0, 0);
				if (file_ == nullptr) {
					return IOError(filename_, errno);
				}
...
private:
	...
	mutable mutex mu_;mutable hdfsFile file_ GUARDED_BY(mu_);
};

Status HadoopFileSystem::NewRandomAccessFile(const string& fname,
		std::unique_ptr<RandomAccessFile>* result)
{
	...
	result->reset(
			new HDFSRandomAccessFile(fname, TranslateName(fname), hdfs_, fs,
					file));
	return Status::OK();
}
...
public:
	HDFSWritableFile(const string& fname, LibHDFS* hdfs, hdfsFS fs,
			hdfsFile file) :
...
Status HadoopFileSystem::NewAppendableFile(const string& fname,
		std::unique_ptr<WritableFile>* result)
{
	hdfsFS fs = nullptr;
	TF_RETURN_IF_ERROR(Connect(fname, &fs));
...
Status HadoopFileSystem::NewReadOnlyMemoryRegionFromFile(const string& fname,
		...
		std::vector<string>* result)
{
	...
	int entries = 0;
	hdfsFileInfo* info = hdfs_->hdfsListDirectory(fs,
			TranslateName(dir).c_str(), &entries);
	if (info == nullptr) {
