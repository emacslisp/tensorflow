...
GpuElementalIrEmitter::GpuElementalIrEmitter(
		...
		ElementalIrEmitter(hlo_module_config, module, ir_builder), hlo_module_config_(
				hlo_module_config), compute_nested_(std::move(compute_nested))
{
}
		...
		PrimitiveType output_type) const
{
	...
	return EmitDeviceFunctionCall(callee_name, operands, input_types,
			output_type,
...
{
	return EmitLibdeviceMathCall("__nv_erfcinv", { value }, { prim_type },
			prim_type);
}
		...
		const HloInstruction* op, llvm::Value* operand_value) const
{
	...
	switch (op->opcode()) {
	case HloOpcode::kExp:
		return EmitLibdeviceMathCall("__nv_exp", { operand_value },
				{ input_type }, output_type);
	case HloOpcode::kFloor:
		return EmitLibdeviceMathCall("__nv_floor", { operand_value }, {
				input_type }, output_type);
	case HloOpcode::kCeil:
		return EmitLibdeviceMathCall("__nv_ceil", { operand_value }, {
				input_type }, output_type);
	case HloOpcode::kLog:
		return EmitLibdeviceMathCall("__nv_log", { operand_value },
				{ input_type }, output_type);
	case HloOpcode::kTanh:
		return EmitLibdeviceMathCall("__nv_tanh", { operand_value }, {
				input_type }, output_type);
	default:
		...
		tensorflow::gtl::ArraySlice<llvm::Attribute::AttrKind> attributes) const
{
	...
	llvm::FunctionType* callee_type = llvm::FunctionType::get(
			llvm_ir::PrimitiveTypeToIrType(output_type, ir_builder_), // The return type.
...
llvm::Value* GpuElementalIrEmitter::EmitThreadId() const
{
	llvm::Value* block_id = ir_builder_->CreateIntCast(
			llvm_ir::EmitCallToIntrinsic(
					...
					ir_builder_), ir_builder_->getIntNTy(128), /*isSigned=*/
			true, "block.id");
