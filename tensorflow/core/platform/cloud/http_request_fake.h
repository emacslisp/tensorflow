...
public:
	...
	FakeHttpRequest(const string& request, const string& response,
			...
			expected_request_(request), response_(response), response_status_(
					response_status), captured_post_body_(captured_post_body), response_headers_(
					response_headers), response_code_(response_code)
	{
	}
	...
	Status SetPutFromFile(const string& body_filepath, size_t offset) override
	{
		...
		const string& content = string(std::istreambuf_iterator<char>(stream),
				std::istreambuf_iterator<char>()).substr(offset);
	...
	Status SetPostFromBuffer(const char* buffer, size_t size) override
	{
		if (captured_post_body_) {
			*captured_post_body_ = string(buffer, size);
			...
			actual_request_ += strings::StrCat("Post body: ",
					StringPiece(buffer, size), "\n");
		}
		return Status::OK();
	}
	...
	{
		EXPECT_EQ(expected_request_, actual_request_)
