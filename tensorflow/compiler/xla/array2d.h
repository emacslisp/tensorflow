...
public:
	...
	{
		int64 n1 = 0;
		for (auto n1_it = values.begin(); n1_it != values.end();
				++n1_it, ++n1) {
			int64 n2 = 0;
			for (auto n2_it = n1_it->begin(); n2_it != n1_it->end();
					++n2_it, ++n2) {
				(*this)(n1, n2) = *n2_it;
			}
		}
	...
	T* data() const
	{
		return const_cast<Array2D*>(this)->values_.data();
	}
	...
	void FillUnique(T start_value = 0)
	{
		for (int64 i0 = 0; i0 < n1(); ++i0) {
			for (int64 i1 = 0; i1 < n2(); ++i1) {
				(*this)(i0, i1) = ((i0 << tensorflow::Log2Ceiling64(n2())) | i1)
						+ start_value;
			}
		}
	}
	...
	void FillRandom(const T& value, const double mean = 0.0, const int seed =
			12345)
	{
		std::mt19937 g(seed);
	...
	string ToString() const
	{
		...
		for (int64 row = 0; row < height(); ++row) {
			...
			for (int64 col = 0; col < width(); ++col) {
				pieces.push_back(
						tensorflow::strings::StrCat((*this)(row, col)));
				pieces.push_back(", ");
			}
			pieces.pop_back();
			pieces.push_back("]");
