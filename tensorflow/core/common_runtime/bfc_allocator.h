...
private:
	...
	struct Chunk {
		...
		string DebugString(BFCAllocator* a, bool recurse) NO_THREAD_SAFETY_ANALYSIS {
			string dbg;
	...
	struct Bin {
		...
		struct ChunkComparator {
			...
			bool operator()(const ChunkHandle ha, const ChunkHandle hb) const NO_THREAD_SAFETY_ANALYSIS {
	...
	public:
		AllocationRegion(void* ptr, size_t memory_size) :
				ptr_(ptr), memory_size_(memory_size), end_ptr_(
						static_cast<void*>(static_cast<char*>(ptr_)
								+ memory_size_))
		{
			DCHECK_EQ(0, memory_size % kMinAllocationSize);
			const size_t n_handles = (memory_size + kMinAllocationSize - 1)
					/ kMinAllocationSize;
	...
	mutable mutex lock_;RegionManager
	region_manager_ GUARDED_BY(lock_);
	...
	// newly-created chunk.
	int64
	next_allocation_id_ GUARDED_BY(lock_);

	// Stats.
	AllocatorStats
	stats_ GUARDED_BY(lock_);
