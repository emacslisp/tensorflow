		...
		int depth, FunctionLibraryRuntime* lib_runtime)
{
	...
	Status status = lib_runtime->Instantiate(call_def.op(), call_def.attr(),
			&handle);
	if (!status.ok()) {
	...
	for (Node* node : fbody->graph->nodes()) {
		if (node->IsSource() || node->IsSink())
			continue;
		if (node->def().op() == "_Arg" || node->def().op() == "_Retval")
			continue;
		...
		if (!HasXLAKernel(*node, jit_device_type)
				&& !IsCompilableCall(node->def(), jit_device_type, depth + 1,
						lib_runtime)) {
			VLOG(2) << "Function marking failed: unsupported op "
...
Status FindCompilationCandidates(const Graph& graph,
		...
		std::unordered_set<Node*>* candidates)
{
	...
	std::unique_ptr<FunctionLibraryRuntime> lib_runtime(
			NewFunctionLibraryRuntime(nullptr, env, nullptr,
	...
	for (Node* node : graph.nodes()) {
		...
		if (!HasXLAKernel(*node, jit_device_type)
				&& !IsCompilableCall(node->def(), jit_device_type, 0,
						lib_runtime.get())) {
			VLOG(2) << "Compilation rejected node: unsupported op "
					<< node->name() << ": " << node->def().op();
			continue;
		}
		if (node->def().op() == "While"
...
public:
	...
	{
		return FindRoot()->representative_;
	}
...
Status MarkForCompilationPass::Run(const GraphOptimizationPassOptions& options)
{
		...
		const std::function<bool(const Node*, const DeviceType&)>& is_compilable_fn)
{
	...
	TF_RETURN_IF_ERROR(
			FindCompilationCandidates(*graph, options.flib_def,
					(options.session_options != nullptr) ?
							options.session_options->env : Env::Default(),
	...
	// exists.
	auto GetOrAddFrameNodeId =
			[&frame_nodes, &cycles](const string& frame_name) {
	...
	for (Node* n : compilation_candidates) {
		...
		} else if (options.flib_def->GetAttr(n->def(), kXlaCompileAttr,
				&compile_attr).ok()) {
			marked_for_compilation = compile_attr;
		}
