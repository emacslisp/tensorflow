...
public:
	...
	public:
		...
		{
			return Options(*this).WithAttrImpl(attr_name,
					std::forward<T>(value));
		}
		...
		{
			return WithAttr<std::initializer_list<T>>(attr_name,
					std::move(value));
		}
	...
	explicit GraphDefBuilder(const OpRegistryInterface* op_registry =
			OpRegistry::Global()) :
	...
	enum TestFailImmediatelyType {
		kFailImmediately
	};
	explicit GraphDefBuilder(TestFailImmediatelyType,
