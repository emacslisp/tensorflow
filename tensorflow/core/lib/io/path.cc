...
std::pair<StringPiece, StringPiece> SplitPath(StringPiece uri)
{
	...
	if (pos == StringPiece::npos)
		return std::make_pair(
				StringPiece(uri.begin(), host.end() - uri.begin()), path);
...
std::pair<StringPiece, StringPiece> SplitBasename(StringPiece path)
{
	...
	return std::make_pair(StringPiece(path.data(), pos),
			StringPiece(path.data() + pos + 1, path.size() - (pos + 1)));
}
}  // namespace internal
		...
		if (src[0] == '.') {
			...
			if (src[1] == '/' || !src[1]) {
				...
				if (dst != backtrack_limit) {
					...
					for (--dst; dst != backtrack_limit && dst[-1] != '/';
							--dst) {
						// Empty.
					}
		...
		StringPiece* path)
{
	...
	if (!strings::Scanner(remaining).One(strings::Scanner::LETTER).Many(
			strings::Scanner::LETTER_DIGIT_DOT).StopCapture().OneLiteral("://").GetResult(
			&remaining, scheme)) {
	...
	// 1. Parse host
	if (!strings::Scanner(remaining).ScanUntil('/').GetResult(&remaining,
			host)) {
