		...
		int task_index, ServerDef* options)
{
	...
	if (options->task_index() >= my_num_tasks) {
		return errors::InvalidArgument("Task index ", options->task_index(),
				" is invalid (job \"", options->job_name(), "\" contains ",
				my_num_tasks, " tasks");
	}
	return Status::OK();
}
...
int main(int argc, char* argv[])
{
	...
	int task_index = 0;
	std::vector<tensorflow::Flag> flag_list = { tensorflow::Flag("cluster_spec",
			&cluster_spec, "cluster spec"), tensorflow::Flag("job_name",
			&job_name, "job name"), tensorflow::Flag("task_id", &task_index,
			"task id"), };
