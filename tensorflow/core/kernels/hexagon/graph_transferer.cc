		...
		const OutputTensorMap& output_tensor_map)
{
	...
	for (const Node* const node : graph.nodes()) {
		status = RegisterNodeIfAllInputsAreCached(ops_definitions,
				shape_refiner, *node, false, input_node_info_list,
		...
		OutputTensorInfo* output_tensor_info)
{
	...
	if (dry_run_for_unknown_shape) {
		...
		status = DryRunInferenceForAllNode(graph_def, input_node_info_list,
				true, output_tensor_info);
		if (!status.ok()) {
			return status;
		}
	}
	...
	return LoadGraphFromProto(ops_definitions, graph_def, input_node_info_list,
			output_node_names, output_tensor_info->output_tensor_map);
}
 ...
 * is fixed.
 */
/* static */Status GraphTransferer::DryRunInference(const GraphDef& graph_def,
		const std::vector<InputNodeInfo>& input_node_info_list,
		const std::vector<string>& output_node_names,
		...
		std::vector<tensorflow::Tensor>* output_tensors)
{
	...
	std::unique_ptr<Session> session = std::unique_ptr<Session>(
			NewSession(session_options));
...
/* static */Status GraphTransferer::DryRunInferenceForAllNode(
		...
		const bool initialize_by_zero,
		OutputTensorInfo* const output_tensor_info)
{
	...
	const Status status = DryRunInference(graph_def, input_node_info_list,
			output_node_names, initialize_by_zero, &output_tensors);
	if (!status.ok()) {
...
void GraphTransferer::SortParams(const std::vector<string>& output_node_names)
{
	...
	for (const NodeTransferParams& params : node_transfer_params_list_) {
		...
		dependency_map.emplace(std::piecewise_construct,
				std::make_tuple(node_id), std::make_tuple());
		if (params.inputs_size == 0) {
			continue;
		}
		CHECK(input_map.count(node_id) == 1);
		for (std::tuple<int, int>& id_and_port : input_map.at(node_id)->input_node_id_and_output_port_list) {
...
int GraphTransferer::CacheNode(const Node& node)
{
	...
	std::tie(std::ignore, emplace_succeeded) =
			node_name_to_id_cache_map_.emplace(node.name(),
					node_name_cache_list_.size() - 1);
...
bool GraphTransferer::AreAllInputsCached(const Node& node) const
{
	for (const Node* const input_node : node.in_nodes()) {
		if (node_name_to_id_cache_map_.count(input_node->name()) <= 0) {
			VLOG(1) << "input_node " << input_node->name() << " of "
		...
		const std::vector<string>& output_node_names)
{
	...
	if (node.name() == SOURCE_NODE_NAME || node.name() == SINK_NODE_NAME) {
		...
		RegisterInputNode(ops_definitions, shape_refiner, output_tensor_map,
				node);
	} else if (std::find(output_node_names.begin(), output_node_names.end(),
			node.name()) != output_node_names.end()) {
		RegisterOutputNode(ops_definitions, shape_refiner, output_tensor_map,
				node);
		...
		return errors::InvalidArgument(
				node.type_string() + " has not implemented yet.");
	}
	return Status();
}
		...
		const Node& node, const OutputTensorMap& output_tensor_map)
{
	...
	CHECK(max_bytes_per_data > 0) << "dt = " << dt << ", " + DataTypeString(dt)
			<< ", " << max_bytes_per_data << ", " << (int) (DataTypeSize(dt))
			<< ",,,,,,,";
	shape_inference::InferenceContext* context = shape_refiner.GetContext(
			&node);
	...
	const_node_transfer_params_list_.emplace_back(
			ConstNodeTransferParams { node.name(), id, { { shape[0], shape[1],
					shape[2], shape[3] } }, data_name, data_size });
...
int GraphTransferer::RegisterConstantShape(const std::vector<int>& shape)
{
	...
	const string shape_name = CONST_SHAPE_PREFIX + ToString(shape.at(0)) + 'x'
			+ ToString(shape.at(1)) + 'x' + ToString(shape.at(2)) + 'x'
			+ ToString(shape.at(3));
	if (node_name_to_id_cache_map_.count(shape_name) <= 0) {
		...
		const_node_transfer_params_list_.emplace_back(ConstNodeTransferParams {
				shape_name, id, { { shape[0], shape[1], shape[2], shape[3] } },
				"", 0 });
	}
		...
		const ShapeRefiner& shape_refiner)
{
	...
	std::array<int64, SHAPE_ARRAY_SIZE> shape;
	const shape_inference::DimensionHandle dim_handle = context->NumElements(
			shape_handle);
		...
		const OutputTensorMap& output_tensor_map, const Node& node)
{
	...
	CHECK(op_type_id >= 0 && op_type_id < ops_definitions.GetTotalOpsCount())
			<< "Op " << node.type_string() << " not found in map(id = "
			<< op_type_id << ")";
	AppendNodeParamsWithIoParams(shape_refiner, output_tensor_map, node,
			node.name(), id, node.type_string(), op_type_id, padding_str,
		...
		const OutputTensorMap& output_tensor_map, const Node& node)
{
	...
	CHECK(op_type_id >= 0 && op_type_id < ops_definitions.GetTotalOpsCount());
	AppendNodeParamsWithIoParams(shape_refiner, output_tensor_map, node,
			node.name(), id, node.type_string(), op_type_id, PADDING_NA,
		...
		const OutputTensorMap& output_tensor_map, const Node& node)
{
	...
	AppendNodeParamsWithIoParams(shape_refiner, output_tensor_map, node,
			node.name(), id, node.type_string(), op_type_id, PADDING_NA,
			node.num_inputs(), { }, 0 /* outputs_size */,
...
void GraphTransferer::AppendNodeParams(const string& name, const int id,
		const string& type, const int type_id, const string& padding_str,
		...
		const int outputs_size)
{
	...
	node_transfer_params_list_.emplace_back(
			NodeTransferParams { name, id, type, type_id, PADDING_PREFIX
					+ padding_str, INPUTS_NODE_PREFIX + ToString(id),
					inputs_size + static_cast<int>(extra_inputs.size()),
							outputs_size <= 0 ? NULL_OUTPUT_NAME : output_name,
					static_cast<int>(outputs_size) });
}

void GraphTransferer::AppendNodeInputParams(const int id, const Node& node,
		const std::vector<int>& extra_inputs)
{
	VLOG(1) << "Append input params: " << node.name() << ", "
			<< node.num_inputs() << ", " << extra_inputs.size();
...
void GraphTransferer::AppendNodeOutputParams(const ShapeRefiner& shape_refiner,
		const OutputTensorMap& output_tensor_map, const int id,
		const Node& node)
{
	...
	for (int i = 0; i < node.num_outputs(); ++i) {
		...
		CHECK(output_node != nullptr) << node.name() << ", "
				<< node.type_string();
		...
		shape_inference::InferenceContext* context = shape_refiner.GetContext(
				output_node);
		shape_inference::ShapeHandle shape_handle = context->output(
				output_index);
		const shape_inference::DimensionHandle num_elements_dim =
				context->NumElements(shape_handle);
		int data_size = -1;
		if (context->ValueKnown(num_elements_dim)) {
			...
			if (!output_tensor_map.empty() && strict_check_mode_) {
				...
				const TensorShape& tensor_shape = output_tensor_map.at(
						node.name())->shape();
				CHECK(num_output_elements == tensor_shape.num_elements())
						<< "num elements of node " << node.name()
						<< " doesn't match " << num_output_elements << " vs "
...
/* static */std::array<int64, GraphTransferer::SHAPE_ARRAY_SIZE> GraphTransferer::BuildShapeArray(
		...
		shape_inference::InferenceContext* context)
{
	...
	case 1:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, 1, 1, context->Value(
				context->Dim(shape_handle, 0)) } };
	case 2:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, 1, context->Value(
				context->Dim(shape_handle, 0)), context->Value(
				context->Dim(shape_handle, 1)) } };
	case 3:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, context->Value(
				context->Dim(shape_handle, 0)), context->Value(
				context->Dim(shape_handle, 1)), context->Value(
				context->Dim(shape_handle, 2)) } };
	case 4:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { context->Value(
				context->Dim(shape_handle, 0)), context->Value(
				context->Dim(shape_handle, 1)), context->Value(
				context->Dim(shape_handle, 2)), context->Value(
				context->Dim(shape_handle, 3)) } };
	default:
...
/* static */std::array<int64, GraphTransferer::SHAPE_ARRAY_SIZE> GraphTransferer::ToTensorShapeArray(
		const TensorShape& shape)
{
	...
	case 0:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, 1, 1, 1 } };
	case 1:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, 1, 1, shape.dim_size(
				0) } };
	case 2:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, 1, shape.dim_size(0),
				shape.dim_size(1) } };
	case 3:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { 1, shape.dim_size(0),
				shape.dim_size(1), shape.dim_size(2) } };
	case 4:
		return std::array<int64, SHAPE_ARRAY_SIZE> { { shape.dim_size(0),
				shape.dim_size(1), shape.dim_size(2), shape.dim_size(3) } };
	default:
		// TODO(satok): Support more ranks?
		CHECK(false);
		return std::array<int64, SHAPE_ARRAY_SIZE>();
		...
		const std::array<int64, SHAPE_ARRAY_SIZE>& expected)
{
	...
	const std::array<int64, SHAPE_ARRAY_SIZE> actual = ToTensorShapeArray(
			output_tensor_map.at(node_name)->shape());
...
/* static */void GraphTransferer::FillDependencyRec(const int node_id,
...
void GraphTransferer::DumpNodeTransferParams() const
{
	...
	for (const ConstNodeTransferParams& params : const_node_transfer_params_list_) {
	...
	for (const NodeTransferParams& params : node_transfer_params_list_) {
		...
		LOG(INFO) << "  inputs: " << params.inputs_name << ", size = "
				<< params.inputs_size;
		LOG(INFO) << "  outputs: " << params.outputs_name << ", size = "
				<< params.outputs_size << " ]";
	}
	LOG(INFO) << "******\n";
	...
	for (const NodeInputParams& params : node_input_params_list_) {
		...
		for (const std::tuple<int, int>& pair : params.input_node_id_and_output_port_list) {
...
void GraphTransferer::DumpVerificationStringOfNodeTransferParams() const
{
	for (const ConstNodeTransferParams& params : const_node_transfer_params_list_) {
		...
		sstream << "---(CONST) [" << std::hex << params.node_id << std::dec
				...
				<< params.shape[2] << "," << params.shape[3] << ","
				<< params.data_name << "," << params.data_size << ","
				<< params.name << "]";
		LOG(INFO) << sstream.str();
	}
	LOG(INFO) << "Const node count = "
			<< const_node_transfer_params_list_.size();
	for (const NodeTransferParams& params : node_transfer_params_list_) {
		std::stringstream sstream;
