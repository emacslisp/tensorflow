...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "MergeV2Checkpoints").Input(FakeInput()) // checkpoint_prefixes
				.Input(FakeInput())  // destination_prefix
				.Attr("delete_old_dirs", delete_old_dirs).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
	...
	void RunMergeTest(bool delete_old_dirs)
	{
		...
		AddInput<string>(TensorShape( { }),
				[kMergedPrefix](int unused) -> string {
		...
		// We expect to find all saved tensors.
		{
			...
			test::ExpectTensorEqual<float>(
					Constant<float>(0, TensorShape( { 10 })), val0);
		}
		{
			Tensor val1;
			TF_EXPECT_OK(reader.Lookup("tensor1", &val1));
			test::ExpectTensorEqual<int64>(Constant<int64>(1, TensorShape( { 1,
					16, 18 })), val1);
		}
		{
			Tensor val2;
			...
			test::ExpectTensorEqual<bool>(
					Constant<bool>(true, TensorShape( { })), val2);
		}

		// Exercises "delete_old_dirs".
		for (int i = 0; i < 2; ++i) {
			int directory_found = Env::Default()->IsDirectory(
					io::Dirname(prefixes[i]).ToString()).code();
			if (delete_old_dirs) {
