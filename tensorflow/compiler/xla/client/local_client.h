...
public:
	...
	ExecutableBuildOptions& set_platform(
...
public:
	...
	const Shape& result_layout() const
	{
		return executable_->module_config().entry_computation_layout().result_layout().shape();
	}
...
private:
	...
	LocalExecutable(std::unique_ptr<Executable> executable, Backend* backend,
			int device_ordinal, const ExecutableBuildOptions& build_options);
	...
	tensorflow::Status LiteralFromShapedBuffer(
...
public:
	...
	// TODO(b/31220873): Remove ExecuteLocally methods. The path forward is to use
	// Compile and run the returned LocalExecutable.
	tensorflow::Status ExecuteLocally(const Computation& computation,
			const tensorflow::gtl::ArraySlice<const ShapedBuffer*> arguments,
	...
	// own library.
	StatusOr<std::vector<std::unique_ptr<AotCompilationResult>>>CompileAheadOfTime(
