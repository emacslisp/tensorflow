...
struct RequantizeCopier {
	RequantizeCopier(
			...
			output_min(output_min), output_max(output_max), input_min_and_max(
					input_min_and_max)
	{
	}
...
public:
	typedef std::vector<std::unique_ptr<typename TTypes<T, 2>::ConstMatrix>> ConstMatrixVector;

	explicit QuantizedConcatOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}

	void CalculateInputAndOutputRange(const OpInputList& input_mins,
			...
			float* output_min, float* output_max)
	{
		...
		if (std::is_signed<T>::value) {
			...
			const float largest_value = std::max(std::abs(overall_min),
					std::abs(overall_max));
	...
	void CalculateConcatDims(const size_t N, const TensorShape& input_shape,
			int input_dims, const OpInputList& values, OpKernelContext* context,
			const int32 concat_dim, const int64 inputs_flat_dim0,
			ConstMatrixVector* inputs_flat, int* output_concat_dim)
	{
		...
		*output_concat_dim = 0;
		...
		for (int i = 0; i < N; ++i) {
			...
			for (int j = 0; j < input_dims; ++j) {
				...
				OP_REQUIRES(context, in.dim_size(j) == input_shape.dim_size(j),
						errors::InvalidArgument(
			...
			if (in.NumElements() > 0) {
				...
				inputs_flat->emplace_back(
						new typename TTypes<T, 2>::ConstMatrix(
								in.shaped<T, 2>( { inputs_flat_dim0,
										inputs_flat_dim1 })));
			}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->input("concat_dim", &concat_dim_tensor));
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument(
						...
						0, ", ", input_dims, "), but got ", concat_dim));
		...
		CalculateInputAndOutputRange(input_mins, input_maxes, N,
				&input_mins_and_maxes, &output_min, &output_max);
		const int64 inputs_flat_dim0 = CalculateInputsDim(input_shape,
				concat_dim);
		ConstMatrixVector inputs_flat;
		int output_concat_dim;
		CalculateConcatDims(N, input_shape, input_dims, values, context,
				concat_dim, inputs_flat_dim0, &inputs_flat, &output_concat_dim);
		...
		if (output->NumElements() > 0) {
			...
			auto output_flat = output->shaped<T, 2>( { inputs_flat_dim0,
					output_dim1 });
			ConcatCPUImpl<T>(context->device(), inputs_flat,
					sizeof(T) /* cost_per_unit */,
					RequantizeCopier<T>(&input_mins_and_maxes, output_min,
							output_max), &output_flat);
		}
