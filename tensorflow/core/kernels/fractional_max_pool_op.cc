...
public:
			...
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("pooling_ratio", &pooling_ratio_));
		OP_REQUIRES_OK(context,
				context->GetAttr("pseudo_random", &pseudo_random_));
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("deterministic", &deterministic_));
	...
	{
		typedef Eigen::Map<
				const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> ConstEigenMatrixMap;
		...
		if (deterministic_) {
			if (pooling_region_generated_) {
				...
				height_cum_seq = GeneratePoolingSequence(input_size_[1],
						output_size_[1], &generator_, pseudo_random_);
				width_cum_seq = GeneratePoolingSequence(input_size_[2],
						output_size_[2], &generator_, pseudo_random_);
				mutex_lock lock(mu_);
		...
		for (int64 b = 0; b < input_size_[0]; ++b) {
			...
			for (int64 hs = 0; hs < height_cum_seq.size() - 1; ++hs) {
				...
				int64 height_end =
						overlapping_ ?
								height_cum_seq[hs + 1] :
								height_cum_seq[hs + 1] - 1;
				height_end = std::min(height_end, height_max);

				// width sequence.
				for (int64 ws = 0; ws < width_cum_seq.size() - 1; ++ws) {
					const int64 out_offset = (b * output_size_[1] + hs)
							* output_size_[2] + ws;
					// width start and end.
					...
					int64 width_end =
							overlapping_ ?
									width_cum_seq[ws + 1] :
									width_cum_seq[ws + 1] - 1;
					width_end = std::min(width_end, width_max);
					for (int64 h = height_start; h <= height_end; ++h) {
						for (int64 w = width_start; w <= width_end; ++w) {
							const int64 in_offset = (b * input_size_[1] + h)
									* input_size_[2] + w;
							out_mat.col(out_offset) =
									out_mat.col(out_offset).cwiseMax(
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<int64>::v(),
						tensor_out.shape(), &tensor_out_arg_max));
		// Find arg_max for each tensor_out
		ConstEigenMatrixMap tensor_in_mat(tensor_in.flat<T>().data(),
				input_size[3], input_size[2] * input_size[1] * input_size[0]);
		EigenMatrixMap tensor_out_dup_mat(tensor_out_dup.flat<T>().data(),
				output_size[3],
		...
		for (int64 b = 0; b < input_size[0]; ++b) {
			...
			for (int64 hs = 0; hs < height_seq_tensor.dim_size(0) - 1; ++hs) {
				...
				int64 height_end =
						overlapping_ ?
								height_seq_tensor_flat(hs + 1) :
								height_seq_tensor_flat(hs + 1) - 1;
				height_end = std::min(height_end, height_max);

				// width sequence.
				for (int64 ws = 0; ws < width_seq_tensor.dim_size(0) - 1;
						++ws) {
					const int64 out_index = (b * output_size[1] + hs)
							* output_size[2] + ws;
					// width start and end.
					const int64 width_start = width_seq_tensor_flat(ws);
					int64 width_end =
							overlapping_ ?
									width_seq_tensor_flat(ws + 1) :
									width_seq_tensor_flat(ws + 1) - 1;
					width_end = std::min(width_end, width_max);
					for (int64 h = height_start; h <= height_end; ++h) {
						for (int64 w = width_start; w <= width_end; ++w) {
							const int64 in_index = (b * input_size[1] + h)
									* input_size[2] + w;
							// Walk through each channel (depth).
							for (int64 d = 0; d < input_size[3]; ++d) {
								const T& input_ref = tensor_in_mat.coeffRef(d,
										in_index);
								T& output_ref = tensor_out_dup_mat.coeffRef(d,
										out_index);
								int64& out_arg_max_ref =
										tensor_out_arg_max_mat.coeffRef(d,
												out_index);
								if (output_ref < input_ref
										|| out_arg_max_ref
												== kInvalidMaxPoolingIndex) {
									output_ref = input_ref;
									int input_offset = in_index * input_size[3]
											+ d;
		...
		ConstEigenMatrixMap tensor_out_mat(tensor_out.flat<T>().data(),
				output_size[3],
				output_size[2] * output_size[1] * output_size[0]);
		const int64 num_reshaped_cols = output_size[2] * output_size[1]
				* output_size[0];
		...
		for (int index = 0; index < num_total_outputs; ++index) {
			...
			CHECK(
					input_backprop_index >= 0
							&& input_backprop_index < num_total_inputs)
					<< "Invalid input backprop index: " << input_backprop_index
					<< ", " << num_total_inputs;
			input_backprop_flat(input_backprop_index) += out_backprop_flat(
					index);
		}
	}
