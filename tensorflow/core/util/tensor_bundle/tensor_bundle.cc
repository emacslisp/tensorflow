...
Status ReadStringTensor(io::InputBuffer* buffered_file, size_t num_elements,
		size_t offset, size_t size, string* destination, uint32* actual_crc32c)
{
	if (size == 0)
		return Status::OK();
	CHECK_GT(size, 0);
	...
	*actual_crc32c = crc32c::Value(
			reinterpret_cast<const char*>(string_lengths.data()),
	...
	TF_RETURN_IF_ERROR(
			buffered_file->ReadNBytes(sizeof(uint32),
					reinterpret_cast<char*>(&length_checksum),
					&unused_bytes_read));
	if (crc32c::Unmask(length_checksum) != *actual_crc32c) {
		return errors::DataLoss("The length checksum does not match: expected ",
	...
	*actual_crc32c = crc32c::Extend(*actual_crc32c,
			reinterpret_cast<char*>(&length_checksum), sizeof(uint32));
	...
	for (size_t i = 0; i < num_elements; ++i) {
		...
		size_t bytes_read = 0;
		TF_RETURN_IF_ERROR(
				buffered_file->ReadNBytes(string_length, &(*buffer)[0],
						&bytes_read));
		*actual_crc32c = crc32c::Extend(*actual_crc32c, buffer->data(),
				bytes_read);
	}
	return Status::OK();
}
		...
		size_t* bytes_written, uint32* crc32c)
{
	...
	*crc32c = 0;
	for (int64 i = 0; i < val.NumElements(); ++i) {
		...
		*crc32c = crc32c::Extend(*crc32c,
				reinterpret_cast<const char*>(&elem_size), sizeof(uint32));
	}
	...
	*bytes_written = lengths.size();
	...
	TF_RETURN_IF_ERROR(
			out->Append(
					StringPiece(reinterpret_cast<const char*>(&length_checksum),
							sizeof(uint32))));
	*crc32c = crc32c::Extend(*crc32c,
		...
		size_t size, size_t buffer_size, char* destination)
{
	...
	while (bytes_read < size) {
		...
		if (!status.ok()) {
			...
			return errors::DataLoss("Requested ", desired_bytes,
					" bytes but read ", result.size(), " bytes.");
...
BundleWriter::BundleWriter(Env* env, StringPiece prefix) :
		env_(env), prefix_(prefix.ToString()), tmp_metadata_path_(
				strings::StrCat(MetaFilename(prefix_), ".tempstate",
						random::New64())), tmp_data_path_(
				strings::StrCat(DataFilename(prefix_, 0, 1), ".tempstate",
						random::New64())), out_(nullptr), size_(0)
{
	status_ = env_->CreateDir(io::Dirname(prefix_).ToString()); // Ignores errors.
	...
	out_ = std::unique_ptr<FileOutputBuffer>(
			new FileOutputBuffer(wrapper.release(),
					8 << 20 /* 8MB write buffer */));
...
Status BundleWriter::Add(StringPiece key, const Tensor& val)
{
	...
	if (val.dtype() != DT_STRING) {
		...
		status_ = WriteStringTensor(val, out_.get(), &data_bytes_written,
				&crc32c);
	}
		...
		const Tensor& slice_tensor)
{
	...
	const string slice_name = checkpoint::EncodeTensorNameSlice(
		...
		MergeState* merge_state)
{
	...
	TF_RETURN_IF_ERROR(
			table::Table::Open(table::Options(), file.get(), file_size,
					&table));
	...
	for (; iter->Valid(); iter->Next()) {
		...
		if (entry_iter != merge_state->entries.end()) {
			...
			if (to_merge_entry.slices().empty()) {
				return errors::Internal("Duplicate tensor keyed by ", key,
			...
			CHECK(
					TensorShape(existing_entry.shape())
		...
		StringPiece merged_prefix)
{
	...
	for (const auto& p : merge.shard_ids) {
		VLOG(1) << "Renaming " << p.first << " to "
				<< DataFilename(merged_prefix, p.second,
						merge.shard_ids.size());
		TF_RETURN_IF_ERROR(
				env->RenameFile(p.first,
						DataFilename(merged_prefix, p.second,
								merge.shard_ids.size())));
	}
	...
	TF_RETURN_IF_ERROR(
			env->NewWritableFile(MetaFilename(merged_prefix),
					&merged_metadata));
	Status status;
	{
...
BundleReader::BundleReader(Env* env, StringPiece prefix) :
		env_(env), prefix_(prefix.ToString()), metadata_(nullptr), table_(
				nullptr), iter_(nullptr)
{
	...
	status_ = table::Table::Open(table::Options(), metadata_, file_size,
			&table_);
	if (!status_.ok())
		return;
...
Status BundleReader::GetValue(const BundleEntryProto& entry, Tensor* val)
{
	...
	if (DataTypeCanUseMemcpy(entry.dtype())) {
		...
		TF_RETURN_IF_ERROR(
				ReadInputByChunk(buffered_file->file(), entry.offset(),
		...
		TF_RETURN_IF_ERROR(
				ReadStringTensor(buffered_file.get(), ret->NumElements(),
						entry.offset(), entry.size(),
						GetStringBackingBuffer(*ret), &actual_crc32c));
	}
	if (crc32c::Unmask(entry.crc32c()) != actual_crc32c) {
		return errors::DataLoss("Checksum does not match: stored ",
...
Status BundleReader::Lookup(StringPiece key, Tensor* val)
{
	...
	if (entry.slices().empty()) {
		...
		return GetSliceValue(key, entry,
		...
		const TensorSlice& slice_spec, Tensor* val)
{
	...
	const string full_tensor_key_string = full_tensor_key.ToString();
	const TensorSliceSet* tss = gtl::FindPtrOrNull(tensor_slices_,
			full_tensor_key_string);
	...
	for (const auto& slice_tag_pair : details) {
		...
		if (!stored_slice.IsFull()) {
			const string encoded_stored_slice_name =
					checkpoint::EncodeTensorNameSlice(full_tensor_key_string,
							stored_slice);
			status_ = GetBundleEntryProto(encoded_stored_slice_name,
					&stored_slice_entry);
			if (!status_.ok())
				return status_;
		}
		...
		if (stored_slice == slice_spec) {
			VLOG(1) << "Optimized for common case: directly copying into "
					"pre-allocated buffer; spec: " << slice_spec.DebugString();
...
#define HANDLE_COPY(T)                                                 \
  case DataTypeToEnum<T>::value:                                       \
    ...
    break;
		...
		HANDLE_COPY(qint8)
		default:
			return errors::InvalidArgument("Dtype ",
					DataTypeString(common_dtype), " not supported.");
		}
#undef HANDLE_COPY
	}
	return Status::OK();
