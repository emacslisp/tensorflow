...
public:
	explicit ExtractGlimpseOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("uniform_noise", &uniform_noise_));
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, num_dims == 4,
		...
		OP_REQUIRES(context, offsets.shape().dim_size(1) == 2,
				errors::InvalidArgument(
						"second dimension should be of size 2 (y,x)",
						offsets.shape().DebugString()));
		...
		output->tensor<float, 4>().swap_layout().device(
				context->eigen_cpu_device()) = Eigen::ExtractGlimpses(
				input.tensor<float, 4>().swap_layout(), output_width,
				output_height, offset_vec, normalized_, centered_,
				uniform_noise_);
	}
