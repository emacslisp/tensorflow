...
void DumpStatsToStdout(const Stats& stats)
{
	...
	std::vector<std::pair<string, double>> groups = { { "Best:",
			sorted_us.front() }, { "Worst:", sorted_us.back() }, { "Median:",
			sorted_us[count_us / 2] }, { "Mean:", sum_us / count_us }, {
			label_trimmed, sum_us_trimmed / count_us_trimmed }, { label_best,
			sum_us_best / count_us_best }, };
	...
	printf("Benchmark ran %zu iterations over %lld us\n", count_us,
			stats.total_us);
	for (const auto& g : groups) {
		printf("  %-*s %*.3f us\n", max_label_size, g.first.c_str(),
				max_digits + 4, g.second);
	}
}
...
void Benchmark(const Options& options, const BenchmarkFn& fn, Stats* stats)
{
	...
	const int64 max_us =
			(options.max_micros <= 0 && options.max_iters <= 0) ?
					Options::kDefaultMicros : options.max_micros;
