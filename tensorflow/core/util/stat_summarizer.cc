...
void StatSummarizer::ProcessStepStats(const StepStats& step_stats)
{
	...
	for (const auto& ds : step_stats.dev_stats()) {
		for (const auto& ns : ds.node_stats()) {
			...
			detail->start_us.UpdateStat(
					ns.all_start_micros() - first_node_start_us);
...
std::string StatSummarizer::ColumnString(const Detail& detail,
		const int64 cumulative_stat_on_node, const Stat<int64>& stat) const
{
	...
	const double cdf_percentage = (cumulative_stat_on_node * 100.0f)
			/ stat.sum();
...
void StatSummarizer::OrderNodesByMetric(SortingMetric metric,
		std::vector<const Detail*>* details) const
{
	...
	for (const auto& det : details_) {
		...
		auto it = std::find(nodes_in_def_order_.begin(),
				nodes_in_def_order_.end(), detail->name);
...
std::string StatSummarizer::GetStatsByNodeType() const
{
	...
	while (!timings.empty()) {
		...
		const float percentage = ((entry.first
				/ static_cast<float>(accumulated_us)) * 100.0f);
		cdf += percentage;
...
std::string StatSummarizer::GetStatsByMetric(const string& title,
		SortingMetric sorting_metric, int num_stats) const
{
...
std::string StatSummarizer::GetOutputString() const
{
	...
	if (options_.show_run_order) {
		stream
				<< GetStatsByMetric("Run Order", BY_RUN_ORDER,
						options_.run_order_limit);
	}
	if (options_.show_time) {
		stream
				<< GetStatsByMetric("Top by Computation Time", BY_TIME,
						options_.time_limit);
	}
	if (options_.show_memory) {
		stream
				<< GetStatsByMetric("Top by Memory Use", BY_MEMORY,
...
{
	std::priority_queue<
			std::pair<int64, const std::pair<const std::string, Detail>*>> timings;
