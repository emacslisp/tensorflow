...
Status TestMultipleWrites(size_t compress_input_buf_size,
		size_t compress_output_buf_size, size_t uncompress_input_buf_size,
		size_t uncompress_output_buf_size, int num_writes = 1, bool with_flush =
				false, int num_copies = 1, bool corrupt_compressed_file = false)
{
	Env* env = Env::Default();
	...
	if (corrupt_compressed_file) {
		string corrupt_fname = testing::TmpDir()
				+ "/snappy_buffers_test_corrupt";
		...
		while ((file_reader->Read(file_pos, bytes_to_read, &data, scratch)).ok()) {
			file_pos += data.size();
			TF_CHECK_OK(
					corrupt_file_writer->Append(
							StringPiece(buffer, buffer_size)));
		...
		TF_CHECK_OK(
				corrupt_file_writer->Append(
						StringPiece(buffer, buffer_size - 1)));
	...
	for (int attempt = 0; attempt < 2; ++attempt) {
		...
		for (int i = 0; i < num_writes; i++) {
			...
			TF_RETURN_IF_ERROR(
					in.ReadNBytes(data.size(), &decompressed_output));
