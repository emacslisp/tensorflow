...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int d = 1; d < rank; d++)
			output_shape_t(d - 1) = input_shape_t(d);
		TensorProto proto_shape;
		...
		serialized_sparse_t.chip < 1
				> (0).setConstant(proto_blank_indices_string);
		serialized_sparse_t.chip < 1
				> (1).setConstant(proto_blank_values_string);
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				TensorShapeUtils::IsMatrix(serialized_sparse.shape()),
				errors::InvalidArgument(
						"Serialized sparse should be a matrix but received shape ",
						serialized_sparse.shape().DebugString()));
		OP_REQUIRES(context, serialized_sparse.shape().dim_size(1) == 3,
				errors::InvalidArgument(
		...
		OP_REQUIRES(context, num_sparse_tensors > 0,
				errors::InvalidArgument(
		...
		for (int i = 0; i < num_sparse_tensors; ++i) {
			...
			OP_REQUIRES(context,
					ParseProtoUnlimited(&proto_indices,
							serialized_sparse_t(i, 0)),
					errors::InvalidArgument(
							"Could not parse serialized_sparse[", i, ", 0]"));
			OP_REQUIRES(context,
					ParseProtoUnlimited(&proto_values,
							serialized_sparse_t(i, 1)),
					errors::InvalidArgument(
							"Could not parse serialized_sparse[", i, ", 1]"));
			OP_REQUIRES(context,
					ParseProtoUnlimited(&proto_shape,
							serialized_sparse_t(i, 2)),
					errors::InvalidArgument(
							"Could not parse serialized_sparse[", i, ", 2]"));
			...
			OP_REQUIRES(context, output_shape.FromProto(proto_shape),
					errors::InvalidArgument(
							...
							", 2] (shape)"));
			OP_REQUIRES(context,
					TensorShapeUtils::IsVector(output_shape.shape()),
					errors::InvalidArgument("Expected serialized_sparse[", i,
		...
		for (int i = 0; i < num_sparse_tensors; ++i) {
			tensors_to_concat.emplace_back(indices_to_concat[i],
					values_to_concat[i], preconcat_shape, std_order);
		}
