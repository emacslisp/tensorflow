...
public:
explicit TestOp3Cpu(OpKernelConstruction* context) :
		OpKernel(context)
{
}
...
public:
explicit TestOp3Gpu(OpKernelConstruction* context) :
		OpKernel(context)
{
}
void Compute(OpKernelContext* context) override
		...
		const DataTypeVector& observed)
{
	...
	for (size_t i = 0; i < size; ++i) {
		...
		EXPECT_TRUE(match) << what << " i:" << i << ", expected: "
				<< expected[i] << ", observed: " << observed[i];
	}
}

void ExpectSuccess(const string& op_type, DeviceType device_type,
		const DataTypeVector& inputs, const DataTypeVector& outputs)
{
	Status status;
	std::unique_ptr<OpKernel> op(
			CreateOpKernel(device_type, &device_, cpu_allocator(),
					CreateNodeDef(op_type, inputs), TF_GRAPH_DEF_VERSION,
					&status));
		...
		error::Code code)
{
	...
	Status status;
	std::unique_ptr<OpKernel> op(
			CreateOpKernel(device_type, &device_, cpu_allocator(), node_def,
...
public:
DummyDevice(Env* env, bool save) :
		DeviceBase(env), save_(save)
{
}
...
NodeDef CreateNodeDef(const string& op_type, const std::vector<string>& attrs)
{
	NodeDef node_def;
...
std::unique_ptr<OpKernel> ExpectSuccess(const string& op_type,
		DeviceType device_type, const std::vector<string>& attrs,
		DataTypeSlice input_types = { })
{
	...
	// Test CreateOpKernel()
	std::unique_ptr<OpKernel> op(
			CreateOpKernel(device_type, &device, cpu_allocator(), def,
					TF_GRAPH_DEF_VERSION, &status));
	EXPECT_TRUE(status.ok()) << status;
	EXPECT_TRUE(op != nullptr);
	if (op != nullptr) {
		...
		const std::vector<string>& attrs, error::Code code)
{
	...
	if (!status.ok()) {
		...
		if (errors::IsNotFound(status)) {
			...
			Status status2 = SupportedDeviceTypesForNode(DeviceTypes(), def,
					&devices);
...
string GetKernelClassName(const string& op_type, DeviceType device_type,
		const std::vector<string>& attrs, DataTypeSlice input_types = { })
{
	NodeDef def = CreateNodeDef(op_type, attrs);
	for (size_t i = 0; i < input_types.size(); ++i) {
		def.add_input("a:0");
	...
	const Status status = FindKernelDef(device_type, def, &kernel_def,
			&kernel_class_name);
	if (status.ok()) {
...
public:
explicit GetAttrKernel(OpKernelConstruction* context) :
		OpKernel(context)
{
	string attr_name;
...
void ExpectOk(std::initializer_list<string> keys)
{
	for (const auto& key_status : status) {
		...
		EXPECT_EQ(in_keys, key_status.second.ok()) << "key_status: "
				<< key_status.first << ", " << key_status.second;
	}
}
