...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		Status result = ffmpeg::ReadAudioFile(input_filename, file_format_,
				samples_per_second_, channel_count_, &output_samples);
		if (result.code() == error::Code::NOT_FOUND) {
			OP_REQUIRES(context, result.ok(),
					errors::Unavailable(
							"FFmpeg must be installed to run this op. FFmpeg "
		...
		OP_REQUIRES(context, output_samples.size() % channel_count_ == 0,
				errors::Unknown(
						"FFmpeg created non-integer number of audio frames."));

		// Copy the output data to the output Tensor.
		Tensor* output = nullptr;
		const int64 frame_count = output_samples.size() / channel_count_;
		OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape( {
				frame_count, channel_count_ }), &output));
		...
		for (int32 frame = 0; frame < frame_count; ++frame) {
			for (int32 channel = 0; channel < channel_count_; ++channel) {
				matrix(frame, channel) = output_samples[frame * channel_count_
						+ channel];
			}
		}
	}
