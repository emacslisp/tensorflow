...
public:
	...
	BufferAllocation(Index index, int64 size, bool is_thread_local,
			bool is_reusable) :
			index_(index), size_(size), is_thread_local_(is_thread_local), is_reusable_(
					is_reusable)
	{
	}
	~BufferAllocation()
...
private:
	...
	std::map<const LogicalBuffer*, BufferAllocation::Index> allocation_index_for_buffer_;
...
private:
	explicit BufferAssigner(LogicalBuffer::SizeFunction buffer_size,
			bool colocate_related_buffers) :
			buffer_size_(std::move(buffer_size)), colocate_related_buffers_(
	...
	void BuildColocatedBufferSets(const HloModule* module,
