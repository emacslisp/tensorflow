...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("cast_op", "Cast").Input(FakeInput(src)).Attr(
						"SrcT", src).Attr("DstT", dst).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
	...
	void CheckCast()
	{
		...
		AddInputFromArray<INPUT>(TensorShape( { 1, 2, 2, 1 }), { INPUT(1),
				INPUT(2), INPUT(3), INPUT(4) });
		TF_ASSERT_OK(RunOpKernel());
		Tensor expected(allocator(), out_type, TensorShape( { 1, 2, 2, 1 }));
		test::FillValues<OUTPUT>(&expected, { OUTPUT(1), OUTPUT(2), OUTPUT(3),
				OUTPUT(4) });
...
static void BM_cpu_float_int64(int iters, int num)
{
	...
	testing::BytesProcessed(
			static_cast<int64>(iters) * num * (sizeof(float) + sizeof(int64)));
	testing::UseRealTime();
...
static void BM_cpu_bool_float(int iters, int num)
{
	testing::ItemsProcessed(static_cast<int64>(iters) * num);
	testing::BytesProcessed(
			static_cast<int64>(iters) * num * (sizeof(bool) + sizeof(float)));
	testing::UseRealTime();
