...
public:
	explicit ScatterAddNdim(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		// Check tensor bounds.
		if (!CheckTensorBounds(context, input_tensor))
			return;
		if (!CheckTensorBounds(context, indices_tensor))
			return;
		if (!CheckTensorBounds(context, deltas_tensor))
			return;
