...
public:
	explicit Executable(std::unique_ptr<HloModule> hlo_module,
			...
			hlo_module_(std::move(hlo_module)), module_config_(
	...
	// (result_buffer).
	virtual Status ExecuteOnStream(const ExecutableRunOptions* run_options,
			tensorflow::gtl::ArraySlice<const ShapedBuffer*> arguments,
	...
	virtual StatusOr<perftools::gputools::DeviceMemoryBase> ExecuteAsyncOnStream(
			const ExecutableRunOptions* run_options,
			tensorflow::gtl::ArraySlice<perftools::gputools::DeviceMemoryBase> arguments) = 0;
	...
	bool dumping() const
	{
		return session_module_ != nullptr;
	}
	SessionModule* session_module() const
	{
		return session_module_.get();
	}
	...
	static Status DumpToDirectory(const string& directory_path,
			const string& filename, const SessionModule& session_module);
...
protected:
	...
	ExecutionProfile
	execution_profile_ GUARDED_BY(mutex_);
