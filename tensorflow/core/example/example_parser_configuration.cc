		...
		std::vector<VarLenFeature>* var_len_features)
{
	...
	if (node.op() != "ParseExample") {
		return errors::InvalidArgument(node_name,
				" node is not a ParseExample op");
	}
	...
	for (int i = 0; i < tdense.list().type_size(); ++i) {
		...
		(*fixed_len_features)[i].shape = TensorShape(
				dense_shapes.list().shape(i));
	}
	...
	for (int i = 0; i < num_sparse; ++i) {
		...
		(*var_len_features)[i].key =
				op_input_tensors[input_idx].scalar<string>()();
	}
	...
	for (int i = 0; i < num_sparse; ++i) {
		...
		config.indices_output_tensor_name = strings::StrCat(node_output_prefix,
				indices_offset);

		int values_offset = sparse_values_output_start + i;
		config.values_output_tensor_name = strings::StrCat(node_output_prefix,
				values_offset);

		int shapes_offset = sparse_shapes_output_start + i;
		config.shapes_output_tensor_name = strings::StrCat(node_output_prefix,
				shapes_offset);
	}

	for (int i = 0; i < num_dense; ++i) {
		int output_idx = dense_values_output_start + i;
		(*fixed_len_features)[i].values_output_tensor_name = strings::StrCat(
				node_output_prefix, output_idx);
	}
	return Status::OK();
}
		...
		std::vector<VarLenFeature>* var_len_features)
{
	...
	for (auto it = feature_map.cbegin(); it != feature_map.cend(); ++it) {
		...
		if (config.has_fixed_len_feature()) {
			...
			f.values_output_tensor_name =
					fixed_config.values_output_tensor_name();
			...
			v.values_output_tensor_name =
					var_len_config.values_output_tensor_name();
			v.indices_output_tensor_name =
					var_len_config.indices_output_tensor_name();
			v.shapes_output_tensor_name =
					var_len_config.shapes_output_tensor_name();
