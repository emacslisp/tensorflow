...
public:
	explicit ReluOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
...
public:
	explicit Relu6Op(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
...
public:
	explicit ReluGradOp(OpKernelConstruction* ctx) :
			XlaBinaryMapOp(ctx)
	{
	}
...
public:
	explicit Relu6GradOp(OpKernelConstruction* ctx) :
			XlaBinaryMapOp(ctx)
	{
	}
	// Return the lhs (incoming gradient) if the rhs (input feature) > 0,
