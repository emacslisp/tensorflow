	...
	void ComputeAsync(OpKernelContext* ctx, DoneCallback callback) final
	{
		...
		if (ctx->input_dtype(0) == DT_RESOURCE) {
			OP_REQUIRES_OK_ASYNC(ctx,
					LookupResource(ctx, HandleFromInput(ctx, 0), &queue),
					callback);
		} else {
			OP_REQUIRES_OK_ASYNC(ctx,
					GetResourceFromContext(ctx, "handle", &queue), callback);
		}
...
protected:
			...
			DoneCallback callback) override
	{
		if (ctx->input_dtype(0) == DT_RESOURCE) {
			OP_REQUIRES_OK_ASYNC(ctx,
					ctx->MatchSignature( { DT_RESOURCE },
							queue->component_dtypes()), callback);
		} else {
			OP_REQUIRES_OK_ASYNC(ctx,
					ctx->MatchSignature( { DT_STRING_REF },
							queue->component_dtypes()), callback);
		}

		queue->TryDequeue(ctx,
			...
			DoneCallback callback) override
	{
		...
		OP_REQUIRES_ASYNC(ctx, num_elements >= 0,
				errors::InvalidArgument("DequeueManyOp requested ",
						num_elements, " < 0 elements"), callback);

		if (ctx->input_dtype(0) == DT_RESOURCE) {
			OP_REQUIRES_OK_ASYNC(ctx, ctx->MatchSignature( { DT_RESOURCE,
					DT_INT32 }, queue->component_dtypes()), callback);
		} else {
			OP_REQUIRES_OK_ASYNC(ctx, ctx->MatchSignature( { DT_STRING_REF,
					DT_INT32 }, queue->component_dtypes()), callback);
		}

		queue->TryDequeueMany(num_elements, ctx, false /* allow_small_batch */,
				[ctx, callback](const QueueInterface::Tuple& tuple) {
					if (!ctx->status().ok()) {
						callback();
						return;
...
protected:
			...
			DoneCallback callback) override
	{
		...
		queue->TryDequeueMany(num_elements, ctx, true /* allow_small_batch */,
				[ctx, callback](const QueueInterface::Tuple& tuple) {
					if (!ctx->status().ok()) {
						callback();
						return;
...
protected:
			...
			DoneCallback callback) override
	{
		...
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(0, TensorShape( { }), &Tqueue_size));
...
public:
	explicit FakeQueueOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
