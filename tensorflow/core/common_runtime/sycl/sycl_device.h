...
public:
	...
	SYCLDevice(const SessionOptions &options, const string &name,
			...
			LocalDevice(options,
					Device::BuildDeviceAttributes(name, DEVICE_SYCL,
							memory_limit, locality, physical_device_desc),
					nullptr), cpu_allocator_(cpu_allocator), sycl_queue_(
					new Eigen::QueueInterface(sycl_selector)), sycl_device_(
					new Eigen::SyclDevice(sycl_queue_)), sycl_allocator_(
					new SYCLAllocator(sycl_queue_)), device_context_(
					new SYCLDeviceContext())
	{
