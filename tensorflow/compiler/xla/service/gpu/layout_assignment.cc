		...
		LayoutConstraints* constraints)
{
	for (auto& instruction : constraints->computation()->instructions()) {
		...
		if (ImplementedAsDnnConvolution(*instruction)) {
			...
			*input_shape.mutable_layout() = LayoutUtil::MakeLayout(
					{ dimension_numbers.spatial_dimensions(1),
			...
			*output_shape.mutable_layout() = LayoutUtil::MakeLayout(
					{ dimension_numbers.spatial_dimensions(1),
							dimension_numbers.spatial_dimensions(0),
							dimension_numbers.feature_dimension(),
							dimension_numbers.batch_dimension() });
			...
			if (instruction->opcode() == HloOpcode::kConvolution) {
				...
				TF_RETURN_IF_ERROR(
						constraints->SetInstructionLayout(output_shape,
								output));
			} else {
				CHECK_EQ(HloOpcode::kFusion, instruction->opcode());
				switch (instruction->fusion_kind()) {
				case HloInstruction::FusionKind::kConvBackwardFilter:
					// filter = BackwardFilterConvolve(input, output)
					TF_RETURN_IF_ERROR(
							constraints->SetOperandLayout(input_shape, filter,
									0));
					TF_RETURN_IF_ERROR(
							constraints->SetInstructionLayout(filter_shape,
									filter));
					TF_RETURN_IF_ERROR(
							constraints->SetOperandLayout(output_shape, filter,
									1));
					break;
				case HloInstruction::FusionKind::kConvBackwardInput:
					// input = BackwardInputConvolve(output, filter)
					TF_RETURN_IF_ERROR(
							constraints->SetInstructionLayout(input_shape,
									input));
					TF_RETURN_IF_ERROR(
							constraints->SetOperandLayout(output_shape, input,
									0));
					TF_RETURN_IF_ERROR(
							constraints->SetOperandLayout(filter_shape, input,
									1));
					break;
				default:
					LOG(FATAL) << "Not a convolution-fusion";
