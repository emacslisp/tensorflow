		...
		std::vector<int64>* arg_sizes)
{
	...
	for (int i = 0; i < ps.parameters_size(); ++i) {
		if (i == ps.parameters_size() - 1 && compile_result.has_context_arg) {
			...
			arg_sizes->push_back(
					xla::ShapeUtil::ByteSizeOf(ps.parameters(i),
  ...
  const void *const *args() const { return args_; }
	...
	if (config.feed_size() != num_args) {
		return errors::InvalidArgument("mismatch between feed_size(",
				config.feed_size(), ") and num_args(", num_args, ")");
	}
    ...
    return (*static_cast<const {{TYPE}}(*){{DIM_SIZES}}>(
		...
		*methods += RewriteWithName(strings::StrCat(i), code, rewrites);
		if (!config.feed(i).name().empty()) {
			*methods += RewriteWithName("_" + config.feed(i).name(), code,
					rewrites);
		}
	}
	return Status::OK();
		...
		string* methods)
{
	if (ps.result().element_type() != xla::TUPLE) {
		...
		if (config.fetch_size() != 1) {
			return errors::InvalidArgument(
					"non-tuple result implies 1 fetch, but got ",
					config.fetch_size(), " fetches");
		}
		*methods +=
				R"(
    ...
    return (*static_cast<const {{TYPE}}(*){{DIM_SIZES}}>(
		...
		*methods += RewriteWithName("0", code, rewrites);
		if (!config.fetch(0).name().empty()) {
			*methods += RewriteWithName("_" + config.fetch(0).name(), code,
					rewrites);
		}
		return Status::OK();
	}
    ...
    return (*static_cast<const {{TYPE}}(*){{DIM_SIZES}}>(
		...
		*methods += RewriteWithName(strings::StrCat(i), code, rewrites);
		if (!config.fetch(i).name().empty()) {
			*methods += RewriteWithName("_" + config.fetch(i).name(), code,
					rewrites);
		}
	}
	return Status::OK();
		...
		const CompileResult& compile_result, string* header)
{
	...
	if (result_index < 0 || result_index > temp_sizes.size()) {
		return errors::InvalidArgument("result index: ", result_index,
				" is outside the range of temp sizes: [0,", temp_sizes.size(),
				")");
	}
	...
	const size_t arg_bytes_aligned = runtime::aligned_buffer_bytes(iarg.data(),
			iarg.size());
	const size_t arg_bytes_total = total_buffer_bytes(iarg.data(), iarg.size());
	const size_t temp_bytes_aligned = runtime::aligned_buffer_bytes(
			itemp.data(), itemp.size());
	const size_t temp_bytes_total = total_buffer_bytes(itemp.data(),
			itemp.size());
...
// clang-format on
	...
	const std::vector<std::pair<string, string>> rewrites = { {
			...
			"{{ARG_SIZES}}", str_util::Join(arg_sizes, ", ") }, { "{{CLASS}}",
			opts.class_name }, { "{{CONTEXT_INCLUDE}}\n", context_include }, {
			"{{CONTEXT_MEMBER_VAR}}\n", context_member_var }, {
			"{{CONTEXT_SET_ARG}}\n", context_set_arg }, {
			"{{CONTEXT_SET_THREAD_POOL}}\n", context_set_thread_pool }, {
			"{{ENTRY}}", compile_result.entry_point }, { "{{ERROR_MSG}}",
			error_msg }, { "{{METHODS_ARG}}\n", methods_arg }, {
			"{{METHODS_RESULT}}\n", methods_result },
			{ "{{NS_END}}\n", ns_end }, { "{{NS_START}}\n", ns_start }, {
					"{{PROGRAM_SHAPE}}", xla::ShapeUtil::HumanString(ps) }, {
					"{{RESULT_INDEX}}", strings::StrCat(result_index) }, {
					"{{RUN_RESULT}}", run_result }, { "{{TEMP_BYTES_ALIGNED}}",
					strings::StrCat(temp_bytes_aligned) }, {
		...
		std::vector<string>* namespaces)
{
	...
	while ((end = cpp_class.find("::", begin)) != string::npos) {
		...
		TF_RETURN_IF_ERROR(
				ValidateCppIdent(ns,
						"in namespace component of cpp_class: " + cpp_class));
