...
// capability.
std::map<string, GpuBackendInfo> gpu_info_map = { { "compute_20", {
		"libdevice.compute_20.10.bc", "sm_20" } }, { "compute_30", {
		"libdevice.compute_30.10.bc", "sm_30" } }, { "compute_35", {
...
static void ValidateGPUArchitecture(const string& value)
{
	if (!gpu_info_map.count(value)) {
		LOG(FATAL)
				<< "value for --gpu_architecture must be compute_{20,30,35,37}";
	}
}
		...
		tensorflow::StringPiece extension)
{
	...
	return tensorflow::io::JoinPath(flags->dump_temp_products_to,
...
std::unique_ptr<llvm::TargetMachine> GetTargetMachine(llvm::Triple triple,
		tensorflow::StringPiece cpu_name,
		const HloModuleConfig& hlo_module_config)
{
	std::string error;
	const llvm::Target* target = TargetRegistry::lookupTarget("", triple,
			error);
	if (target == nullptr) {
	...
	return WrapUnique(
			target->createTargetMachine(triple.str(),
					llvm_ir::AsStringRef(cpu_name), "+ptx42", target_options,
					Optional < Reloc::Model > (RelocModel), CMModel,
					codegen_opt_level));
}
...
void EmitBitcodeToFile(const Module& module, tensorflow::StringPiece filename)
{
	...
	llvm::tool_output_file outfile(filename.ToString().c_str(), error_code,
			llvm::sys::fs::F_None);
	if (error_code) {
		LOG(FATAL) << "opening bitcode file for writing: "
				<< error_code.message();
	}
	...
	std::string ptx;  // need a std::string instead of a ::string.
	{
		...
		IrDumpingPassManager codegen_passes(
				ReplaceFilenameExtension(tensorflow::io::Basename(module_id),
						"-nvptx.dummy"), flags->dump_temp_products_to,
		...
		llvm::Module* module)
{
	...
	string libdevice_bc_fullpath = tensorflow::io::JoinPath(libdevice_dir_path,
			libdevice_bc_filename);
	TF_RETURN_IF_ERROR(
			tensorflow::Env::Default()->FileExists(libdevice_bc_fullpath));
	std::unique_ptr<llvm::Module> libdevice_module = LoadIRModule(
			libdevice_bc_fullpath, &module->getContext());
		...
		const string& libdevice_dir_path)
{
	...
	if (!flags->dump_temp_products_to.empty()) {
		string linked_filename = MakeNameForTempProduct(
				module->getModuleIdentifier(), "linked.bc");
	...
	std::unique_ptr<llvm::TargetMachine> target_machine = GetTargetMachine(
			target_triple, cpu_name, hlo_module_config);
	module_passes.add(
	...
	if (!flags->dump_temp_products_to.empty()) {
		string optimized_filename = MakeNameForTempProduct(
				module->getModuleIdentifier(), "optimized.bc");
		...
		const string& libdevice_dir_path)
{
	...
	string ptx;
	{
		...
		TF_ASSIGN_OR_RETURN(ptx,
				CompileModuleToPtx(module, hlo_module_config,
						libdevice_dir_path));
	}
	return ptx;
}
