...
inline int64 LinearSubIndexToFullIndex(int64 output_index,
		const gtl::InlinedVector<int32, 8>& dim_list,
...
void MakeUnreducedIndices(gtl::InlinedVector<bool, 8> index_is_reduced,
		int32 input_dims, gtl::InlinedVector<int32, 8>* unreduced_indices)
{
	for (int32 index = 0; index < input_dims; ++index) {
		if (!index_is_reduced[index])
			unreduced_indices->push_back(index);
	}
}
...
public:
	...
	explicit ReduceJoinOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int32 i = 0; i < reduction_dims; i++) {
			...
			OP_REQUIRES(context,
					reduce_index >= -input_dims && reduce_index < input_dims,
					errors::OutOfRange("Invalid reduction dimension ",
							reduce_index, " for input with ", input_dims,
							" dimension(s)"));
		...
		gtl::InlinedVector<int32, 8> reduced_indices = GetReducedIndices(
				reduction_indices, input_dims);
		...
		TensorShape output_shape = GetOutputShape(index_is_reduced, input_shape,
				keep_dims_);
		...
		const int64 reduction_iter_size = GetReductionIterSize(reduced_indices,
				input_shape);
		...
		for (int64 output_index = 0; output_index < output_shape.num_elements();
				++output_index) {
			int64 output_full_index = LinearSubIndexToFullIndex(output_index,
					unreduced_indices, input_shape, strides);
			for (int64 reduction_index = 0;
					reduction_index < reduction_iter_size; ++reduction_index) {
				int64 reduction_full_index = LinearSubIndexToFullIndex(
						reduction_index, reduced_indices, input_shape, strides);
				curr_strings[reduction_index] = input_flat(
						output_full_index + reduction_full_index);
			}
			output_flat(output_index) = str_util::Join(curr_strings,
					separator_.c_str());
		}
	}
