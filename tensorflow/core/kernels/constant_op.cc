		...
		OpKernel(ctx), tensor_(ctx->output_type(0))
{
	...
	OP_REQUIRES_OK(ctx,
			ctx->device()->MakeTensorFromProto(*proto, AllocatorAttributes(),
					&tensor_));
		...
		OpKernel(ctx), tensor_(ctx->output_type(0))
{
	...
	OP_REQUIRES_OK(ctx, ctx->GetAttr("value", &proto));
	OP_REQUIRES_OK(ctx,
			ctx->device()->MakeTensorFromProto(*proto, alloc_attr, &tensor_));
	OP_REQUIRES(ctx, ctx->output_type(0) == tensor_.dtype(),
			errors::InvalidArgument("Type mismatch between value (",
					DataTypeString(tensor_.dtype()), ") and dtype (",
					DataTypeString(ctx->output_type(0)), ")"));
...
public:
explicit FillOp(OpKernelConstruction* context) :
		OpKernel(context)
{
}
...
void Compute(OpKernelContext* context) override
{
	...
	OP_REQUIRES(context, IsLegacyVector(Tdims.shape()),
			errors::InvalidArgument(
					"dims must be a vector of int32, got shape ",
	...
	if (expected_shape_.dims() > 0) {
		OP_REQUIRES(ctx, false,
				errors::InvalidArgument(
						...
						name(), "' with dtype ", DataTypeString(output_type(0)),
