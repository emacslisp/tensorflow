	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (deterministic_) {
			if (pooling_region_generated_) {
				...
				row_cum_seq = GeneratePoolingSequence(input_size_[1],
						output_size_[1], &generator_, pseudo_random_);
				col_cum_seq = GeneratePoolingSequence(input_size_[2],
		...
		for (int64 b = 0; b < input_size_[0]; ++b) {
			...
			for (int64 hs = 0; hs < row_cum_seq.size() - 1; ++hs) {
				...
				int64 row_end =
						overlapping_ ?
								row_cum_seq[hs + 1] : row_cum_seq[hs + 1] - 1;
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		auto orig_input_tensor_shape_flat =
				orig_input_tensor_shape.flat<int64>();
		...
		ConstEigenMatrixMap out_backprop_mat(out_backprop.flat<T>().data(),
				out_depth, out_cols * out_rows * out_batch);
		...
		for (int64 b = 0; b < out_batch; ++b) {
			for (int64 r = 0; r < out_rows; ++r) {
				...
				for (int64 c = 0; c < out_cols; ++c) {
					...
					int64 in_col_end =
							overlapping_ ?
									col_seq_tensor_flat(c + 1) :
									col_seq_tensor_flat(c + 1) - 1;
					in_col_end = std::min(in_col_end, in_max_col_index);

					const int64 num_elements_in_pooling_cell = (in_row_end
							- in_row_start + 1)
							* (in_col_end - in_col_start + 1);
					const int64 out_index = (b * out_rows + r) * out_cols + c;
					// Now we can evenly distribute out_backprop(b, h, w, *) to
					// in_backprop(b, hs:he, ws:we, *).
					for (int64 in_r = in_row_start; in_r <= in_row_end;
							++in_r) {
						for (int64 in_c = in_col_start; in_c <= in_col_end;
								++in_c) {
							const int64 in_index = (b * in_rows + in_r)
									* in_cols + in_c;
							// Walk through each channel (depth).
							for (int64 d = 0; d < out_depth; ++d) {
								const double out_backprop_element =
										static_cast<double>(out_backprop_mat.coeffRef(
												d, out_index));
								double& in_backprop_ref =
										in_backprop_tensor_temp_mat.coeffRef(d,
												in_index);
								in_backprop_ref += out_backprop_element
