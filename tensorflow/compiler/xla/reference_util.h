...
public:
	...
	static std::vector<T> ReduceR2ToR1(const Array2D<T>& input,
			int dimension_to_reduce, T init, std::function<T(T, T)> freduce)
	{
		std::vector<T> result(
				dimension_to_reduce == 0 ? input.n2() : input.n1(), init);
	...
	static std::vector<float> Reduce4DTo1D(const Array4D<float>& array,
			float init, tensorflow::gtl::ArraySlice<int64> dims,
			std::function<float(float, float)> reduce_function);
	...
	static std::unique_ptr<Array2D<float>> MapArray2D(const Array2D<float>& lhs,
	...
	// xla::MakePadding().
	static int64 WindowCount(int64 unpadded_width, int64 window_len,
	...
	static std::unique_ptr<Array4D<float>> SelectAndScatter4DGePlus(
			const Array4D<float>& operand, const Array4D<float>& source,
			float init, const tensorflow::gtl::ArraySlice<int64>& window,
	...
	static std::unique_ptr<Array2D<T>> Concat2D(const Array2D<T>& lhs,
			const Array2D<T>& rhs, int concatenate_dimension)
	{
	...
	static std::unique_ptr<Array3D<T>> Concat3D(const Array3D<T>& lhs,
			const Array3D<T>& rhs, int concatenate_dimension)
	{
		...
		auto result = MakeUnique<Array3D<T>>(out_dims[0], out_dims[1],
				out_dims[2]);
		for (int64 i0 = 0; i0 < result->n1(); ++i0) {
			for (int64 i1 = 0; i1 < result->n2(); ++i1) {
	...
	static std::unique_ptr<Array4D<T>> Concat4D(const Array4D<T>& lhs,
			const Array4D<T>& rhs, int concatenate_dimension)
	{
		...
		auto result = MakeUnique<Array4D<T>>(out_dims[0], out_dims[1],
	...
	template<typename T>
	static std::vector<T> ModSlice1D(
			const tensorflow::gtl::ArraySlice<T>& input, int64 start,
			int64 size)
	{
		std::vector<T> result;
	...
	static std::unique_ptr<Array4D<T>> Slice4D(const Array4D<T>& input,
			std::array<int64, 4> starts, std::array<int64, 4> limits)
	{
		CHECK_LE(starts[0], input.n1());
		CHECK_LE(starts[1], input.n2());
		...
		auto result = MakeUnique<Array4D<T>>(limits[0] - starts[0],
				limits[1] - starts[1], limits[2] - starts[2],
				limits[3] - starts[3]);
		for (int64 i0 = 0; i0 < result->n1(); ++i0) {
			for (int64 i1 = 0; i1 < result->n2(); ++i1) {
				for (int64 i2 = 0; i2 < result->n3(); ++i2) {
					for (int64 i3 = 0; i3 < result->n4(); ++i3) {
						(*result)(i0, i1, i2, i3) = input(starts[0] + i0,
								starts[1] + i1, starts[2] + i2, starts[3] + i3);
					}
				}
			}
		}
	...
	static std::unique_ptr<Array3D<T>> Slice3D(const Array3D<T>& input,
			std::array<int64, 3> starts, std::array<int64, 3> limits)
	{
		CHECK_LE(starts[0], input.n1());
		CHECK_LE(starts[1], input.n2());
		CHECK_LE(starts[2], input.n3());
		...
		CHECK_LE(limits[1], input.n2());
		CHECK_LE(limits[2], input.n3());
		auto result = MakeUnique<Array3D<T>>(limits[0] - starts[0],
				limits[1] - starts[1], limits[2] - starts[2]);
		for (int64 i0 = 0; i0 < result->n1(); ++i0) {
			for (int64 i1 = 0; i1 < result->n2(); ++i1) {
				for (int64 i2 = 0; i2 < result->n3(); ++i2) {
					(*result)(i0, i1, i2) = input(starts[0] + i0,
							starts[1] + i1, starts[2] + i2);
				}
			}
		}
		return result;
	...
	// the result.
	template<typename F>
	static std::unique_ptr<Array4D<float>> MapArray4D(
				...
				for (int64 height = 0; height < input.height(); ++height) {
					for (int64 width = 0; width < input.width(); ++width) {
						(*result)(plane, depth, height, width) = map_function(
								input(plane, depth, height, width), plane,
								depth, height, width);
					}
				}
			}
