...
template<typename T>
void Expect(BundleReader* reader, const string& key, const Tensor& expected_val)
{
	// Tests for Contains().
...
Status FlipEndiannessBit(const string& prefix)
{
	...
	// Reads the existing metadata file, and fills the builder.
	{
		...
		CHECK(
				header.ParseFromArray(iter->value().data(),
						iter->value().size()));
	...
	TF_RETURN_IF_ERROR(
			env->RenameFile(metadata_tmp_path, MetaFilename(prefix)));
	return file->Close();
}
...
void TestBasic()
{
	...
	}
	{
		...
		EXPECT_EQ(AllTensorKeys(&reader), std::vector<string>( { "foo_000",
				"foo_001", "foo_002", "foo_003" }));
	...
	}
	{
		...
		TF_ASSERT_OK(reader.status());
		EXPECT_EQ(AllTensorKeys(&reader), std::vector<string>( { "bar_000",
				"bar_001", "bar_002", "bar_003" }));
	...
	TF_ASSERT_OK(
			MergeBundles(Env::Default(), { Prefix("foo"), Prefix("bar") },
					Prefix("merged")));
	{
		BundleReader reader(Env::Default(), Prefix("merged"));
		TF_ASSERT_OK(reader.status());
		EXPECT_EQ(AllTensorKeys(&reader), std::vector<string>( { "bar_000",
				"bar_001", "bar_002", "bar_003", "foo_000", "foo_001",
				"foo_002", "foo_003" }));
		Expect<T>(&reader, "bar_000", Constant_2x3<T>(0));
		Expect<T>(&reader, "bar_001", Constant_2x3<T>(1));
		Expect<T>(&reader, "bar_002", Constant_2x3<T>(2));
...
void TestNonStandardShapes()
{
	...
	}
	{
		...
		Expect<T>(&reader, "non_standard0", Constant<T>(0, TensorShape( { 0,
				1618 })));
		Expect<T>(&reader, "non_standard1", Constant<T>(0, TensorShape( { 16, 0,
				18 })));
	}
}
...
void VersionTest(const VersionDef& version, StringPiece expected_error)
{
	const string path = Prefix("version_test");
	{
		...
		*header.mutable_version() = version;
		...
		std::unique_ptr<WritableFile> file;
		TF_ASSERT_OK(
				Env::Default()->NewWritableFile(MetaFilename(path), &file));
		table::TableBuilder builder(table::Options(), file.get());
	...
	EXPECT_TRUE(
			StringPiece(reader.status().error_message()).starts_with(
					expected_error));
}
