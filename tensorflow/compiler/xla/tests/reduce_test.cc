...
protected:
	ReduceTest()
	{
		...
		// clang-format on
		CHECK(ShapeUtil::Equal(literal_3d_->shape(), ShapeUtil::MakeShape(F32, {/*z=*/
				4, /*y=*/2, /*x=*/3 })))
	...
	void RunR1ToR0Test(int64 element_count)
	{
		...
		std::unique_ptr<Literal> input_literal = LiteralUtil::CreateR1(
				AsSlice(input_data));
		...
		ComputeAndCompareR0<float>(&builder, expected,
				{ input_global_data.get() }, ErrorSpec(0.001));
	}
	...
	void RunR2ToR0Test(int64 rows, int64 cols, int64 minor = 1, int64 major = 0)
	{
		...
		input_literal = LiteralUtil::Relayout(*input_literal,
				LayoutUtil::MakeLayout( { minor, major }));
		std::unique_ptr<GlobalData> input_global_data =
				client_->TransferToServer(*input_literal).ConsumeValueOrDie();
		...
		ComputeAndCompareR0<float>(&builder, expected,
				{ input_global_data.get() }, ErrorSpec(0.01, 1e-4));
	}
...
{
	*os
			<< tensorflow::strings::Printf("R%luToR%lu%s_%s_Reduce%s",
					spec.bounds.size(),
...
class ReduceR3ToR2Test: public ReduceTest, public ::testing::WithParamInterface<
		BoundsLayout> {
};
