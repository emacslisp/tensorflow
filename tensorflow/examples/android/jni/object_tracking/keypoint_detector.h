...
public:
	explicit KeypointDetector(const KeypointDetectorConfig* const config) :
			config_(config), keypoint_scratch_(
					new Image<uint8>(config_->image_size)), interest_map_(
					new Image<bool>(config_->image_size)), fast_quadrant_(0)
	{
...
private:
	...
	int AddExtraCandidatesForBoxes(const std::vector<BoundingBox>& boxes,
			const int max_num_keypoints, Keypoint* const keypoints) const;
	...
	int FindFastKeypoints(const Image<uint8>& frame, const int quadrant,
			const int downsample_factor, const int max_num_keypoints,
