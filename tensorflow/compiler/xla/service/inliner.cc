		...
		tensorflow::gtl::ArraySlice<HloInstruction*> /*static_operands*/)
{
	...
	if (hlo_query::AllOperandsAreParameters(root)) {
		...
		VLOG(10)
				<< "inlining map({X ... Y}, op) => : op(X ... Y) with function "
				<< root.ToShortString();
		...
		if (root.opcode() != HloOpcode::kConstant) {
			...
			HloInstruction* constant = computation_->AddInstruction(
					root.Clone());
			HloInstruction* placed_instruction = computation_->AddInstruction(
...
StatusOr<bool> Inliner::Run(HloModule* module)
{
	...
	bool changed = false;
	for (const std::unique_ptr<HloComputation>& computation : module->computations()) {
