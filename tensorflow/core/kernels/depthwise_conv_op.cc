...
struct DepthwiseConv2DKernel {
			...
			T* output)
	{
		...
		const int64 output_vectorized_size = (out_depth / kPacketSize)
				* kPacketSize;
		const int64 base_output_index = (out_r * args.out_cols + out_c)
				* out_depth;
		...
		if (output_scalar_size > 0) {
			...
			const int64 last_output_index = base_output_index
					+ output_vectorized_size;
...
struct LaunchDepthwiseConvOp<CPUDevice, T> {
			...
			const T* input, const T* depthwise_filter, T* output)
	{
		...
		auto shard =
				[&ctx, &args, &input, &filter_data, &output](int64 start,
						int64 limit) {
					static const int64 kPacketSize = (sizeof(Packet) / sizeof(T));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, out_shape.num_elements() <= 2147483647,
				errors::InvalidArgument(
						"total number of outputs should be within the "
		...
		VLOG(2) << "DepthwiseConv2dNative: " << " Input: [" << batch << ", "
				<< input_rows << ", " << input_cols << ", " << in_depth
				<< "]; Filter: [" << filter_rows << ", " << filter_cols << ", "
				<< in_depth << ", " << depth_multiplier << "]; stride = "
				<< stride << ", pad_rows = " << pad_rows << ", pad_cols = "
				<< pad_cols << ", output: [" << batch << ", " << out_rows
				<< ", " << out_cols << ", " << out_depth << "]";
		...
		if (std::is_same<T, float>::value && in_depth == 1) {
			launcher_.launch(context, use_cudnn_, cudnn_use_autotune_, input,
					filter, stride, stride, BrainPadding2EigenPadding(padding_),
					output, FORMAT_NHWC);
			return;
		}
