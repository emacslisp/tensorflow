...
ForLoop::ForLoop(tensorflow::StringPiece suffix, llvm::Value* start_index,
		...
		suffix_(suffix.ToString()), start_index_(start_index), end_index_(
				end_index), step_(step), insert_before_bb_(nullptr)
{
}
...
void ForLoop::Emit(llvm::IRBuilder<>* ir_builder)
{
	...
	if (insert_point == preheader_bb_->end()) {
		...
		exit_bb_ = preheader_bb_->splitBasicBlock(insert_point,
				GetNameWithSuffix("loop_exit").c_str());
	...
	llvm::Value* indvar_address = ir_builder->CreateAlloca(
			ir_builder->getInt64Ty(), nullptr,
	...
	llvm::Value* indvar_inc = ir_builder->CreateAdd(indvar, step, "invar.inc",
		...
		llvm::Value* start_index, llvm::Value* end_index)
{
	if (inner_loop_body_bb_ != nullptr) {
		...
		ir_builder_->SetInsertPoint(
				&*inner_loop_body_bb_->getFirstInsertionPt());
	}
	std::unique_ptr<ForLoop> loop = ForLoop::EmitForLoop(suffix, start_index,
			end_index, ir_builder_->getInt64(1), ir_builder_);
...
std::unique_ptr<ForLoop> ForLoopNest::AddLoop(int64 start_index,
		int64 end_index, tensorflow::StringPiece suffix)
{
...
IrArray::Index ForLoopNest::AddLoopsForShapeOnDimensions(const Shape& shape,
		...
		tensorflow::StringPiece suffix)
{
	...
	for (int64 dimension : dimensions) {
		...
		/*end_index=*/shape.dimensions(dimension),
				/*suffix=*/tensorflow::strings::Printf("%s%lld",
						suffix.ToString().c_str(), dimension));
