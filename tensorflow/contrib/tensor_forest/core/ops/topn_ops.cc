...
public:
	explicit TopNInsert(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, new_scores.shape().dims() == 1,
				errors::InvalidArgument(
						"new_scores should be one-dimensional"));

		OP_REQUIRES(context,
				ids.shape().dim_size(0) == scores.shape().dim_size(0),
				errors::InvalidArgument(
						"ids and scores should be the same length"));
		OP_REQUIRES(context,
		...
		if (overflow > 0) {
			...
			std::iota(shortlist_indices.begin() + 1, shortlist_indices.end(),
					1);
			auto cmp2 = [&flat_scores](int a, int b) {
				return flat_scores(a) < flat_scores(b);
			};
			std::sort(shortlist_indices.begin() + 1, shortlist_indices.end(),
					cmp2);
			...
			while (i < num_updates && j <= shortlist_max_size) {
				...
				if (flat_new_scores(update_indices[i])
					...
					// update score_cutoff.
					score_cutoff = std::max(score_cutoff,
							flat_scores(shortlist_indices[j]));
					updates.push_back(
							std::make_tuple(shortlist_indices[j],
									flat_new_ids(update_indices[i]),
									flat_new_scores(update_indices[i])));
			...
			for (int i = 0; i < num_updates; i++) {
				...
				updates.push_back(
						std::make_tuple(j, flat_new_ids(i),
								flat_new_scores(i)));
				j++;
				shortlist_size++;
			}
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, shortlist_ids_shape,
...
public:
	explicit TopNRemove(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, ids.shape().dims() == 1,
				errors::InvalidArgument("ids should be one-dimensional"));
		OP_REQUIRES(context, remove_ids.shape().dims() == 1,
				errors::InvalidArgument(
						"remove_ids should be one-dimensional"));
		...
		std::unordered_set<int> ids_to_remove(flat_remove_ids.data(),
				flat_remove_ids.data() + num_to_remove);
