...
public:
	explicit SparseAddOp(OpKernelConstruction *ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext *ctx) override
	{
		...
		for (int i = 0; i < a_shape->NumElements(); ++i) {
			OP_REQUIRES(ctx, a_shape_flat(i) == b_shape_flat(i),
					errors::InvalidArgument(
							"Operands' shapes do not match: got ",
							a_shape_flat(i), " and ", b_shape_flat(i),
							" for dimension ", i));
		}
		...
		while (i < a_nnz && j < b_nnz) {
			switch (sparse::DimComparator::cmp(a_indices_mat, b_indices_mat, i,
					j, num_dims)) {
			case -1:
