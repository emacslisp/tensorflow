...
protected:
	SubgraphTest() :
			g_(new Graph(OpRegistry::Global()))
	{
			...
			const string& targets_str)
	{
		...
		std::vector<string> fed = str_util::Split(fed_str, ',',
				str_util::SkipEmpty());
		std::vector<string> fetch = str_util::Split(fetch_str, ',',
				str_util::SkipEmpty());
		std::vector<string> targets = str_util::Split(targets_str, ',',
				str_util::SkipEmpty());
...
static void BM_Subgraph(int iters, int num_nodes)
{
	...
	{  // Scope for temporary variables used to construct g.
		...
		for (int i = 0; i < num_nodes; i++) {
			...
			if (i > 0) {
				last_node = ops::UnaryOp("Op", last_node,
						b.opts().WithName(name));
			} else {
	...
	while (--iters > 0) {
		...
		TF_CHECK_OK(
				subgraph::RewriteGraphForExecution(subgraph, fed, fetch,
						targets, device_info));
		delete subgraph;
	}
}
