...
public:
	explicit DynamicPartitionOp_Shared(OpKernelConstruction* c) :
			OpKernel(c)
	{
			...
			const Tensor** partitions, OpOutputList* Tout)
	{
		...
		for (int64 i = 0; i < N; i++) {
			...
			OP_REQUIRES(c, FastBoundsCheck(p, num_partitions_),
					errors::InvalidArgument("partitions",
							SliceDebugString((*partitions)->shape(), i), " = ",
...
public:
	...
	void Compute(OpKernelContext* c) override
	{
		...
		if (partitions->dims() == data->dims()) {
			...
			std::vector<
					Eigen::TensorMap<Eigen::Tensor<T, 1, Eigen::RowMajor>,
			...
			for (int64 i = 0; i < N; i++) {
				...
				OP_REQUIRES(c, FastBoundsCheck(oi, out_vec[p].size()),
						errors::InvalidArgument("out_vec[", p, "] size: ",
								out_vec[p].size(), " is not LTE output_index[",
								p, "] : ", oi));
			...
			for (int64 i = 0; i < N; i++) {
				...
				out_flat[p].slice(out_indices, sizes) = data_flat.slice(
						data_indices, sizes);
				output_index[p]++;
			}
		}
