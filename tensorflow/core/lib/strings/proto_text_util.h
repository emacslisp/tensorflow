...
public:
	...
	ProtoTextOutput(string* output, bool short_debug) :
			output_(output), short_debug_(short_debug), field_separator_(
					short_debug ? " " : "\n")
	{
	}
	...
	{
		if (value != 0)
			AppendNumeric(field_name, value);
	}
	...
	void AppendString(const char field_name[], const string& value)
	{
		AppendFieldAndValue(field_name,
				StrCat("\"", ::tensorflow::str_util::CEscape(value), "\""));
	}
...
bool ProtoParseNumericFromScanner(Scanner* scanner, T* value)
{
	...
	if (!scanner->Many(Scanner::LETTER_DIGIT_DOT_PLUS_MINUS).GetResult(nullptr,
			&numeric_str)) {
		return false;
	}
