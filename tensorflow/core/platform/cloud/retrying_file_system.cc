...
public:
	RetryingRandomAccessFile(std::unique_ptr<RandomAccessFile> base_file,
			...
			base_file_(std::move(base_file)), initial_delay_microseconds_(
					delay_microseconds)
	{
	}
			...
			char* scratch) const override
	{
		return RetryingUtils::CallWithRetries(
				std::bind(&RandomAccessFile::Read, base_file_.get(), offset, n,
						result, scratch), initial_delay_microseconds_);
	}
...
Status RetryingFileSystem::NewRandomAccessFile(const string& filename,
		std::unique_ptr<RandomAccessFile>* result)
{
	std::unique_ptr<RandomAccessFile> base_file;
	TF_RETURN_IF_ERROR(
			RetryingUtils::CallWithRetries(
					std::bind(&FileSystem::NewRandomAccessFile,
							base_file_system_.get(), filename, &base_file),
					initial_delay_microseconds_));
	result->reset(
			new RetryingRandomAccessFile(std::move(base_file),
					initial_delay_microseconds_));
	return Status::OK();
}

Status RetryingFileSystem::NewWritableFile(const string& filename,
		std::unique_ptr<WritableFile>* result)
{
	std::unique_ptr<WritableFile> base_file;
	TF_RETURN_IF_ERROR(
			RetryingUtils::CallWithRetries(
					std::bind(&FileSystem::NewWritableFile,
							base_file_system_.get(), filename, &base_file),
					initial_delay_microseconds_));
	result->reset(
			new RetryingWritableFile(std::move(base_file),
					initial_delay_microseconds_));
	return Status::OK();
}

Status RetryingFileSystem::NewAppendableFile(const string& filename,
		std::unique_ptr<WritableFile>* result)
{
	std::unique_ptr<WritableFile> base_file;
	TF_RETURN_IF_ERROR(
			RetryingUtils::CallWithRetries(
					std::bind(&FileSystem::NewAppendableFile,
							base_file_system_.get(), filename, &base_file),
					initial_delay_microseconds_));
	result->reset(
			new RetryingWritableFile(std::move(base_file),
		...
		std::vector<string>* result)
{
	return RetryingUtils::CallWithRetries(
			std::bind(&FileSystem::GetChildren, base_file_system_.get(), dir,
					result), initial_delay_microseconds_);
}
		...
		std::vector<string>* result)
{
	return RetryingUtils::CallWithRetries(
			std::bind(&FileSystem::GetMatchingPaths, base_file_system_.get(),
					pattern, result), initial_delay_microseconds_);
}
...
Status RetryingFileSystem::GetFileSize(const string& fname, uint64* file_size)
{
	return RetryingUtils::CallWithRetries(
			std::bind(&FileSystem::GetFileSize, base_file_system_.get(), fname,
					file_size), initial_delay_microseconds_);
}
...
Status RetryingFileSystem::RenameFile(const string& src, const string& target)
{
	return RetryingUtils::CallWithRetries(
			std::bind(&FileSystem::RenameFile, base_file_system_.get(), src,
					target), initial_delay_microseconds_);
}

Status RetryingFileSystem::IsDirectory(const string& dirname)
{
	return RetryingUtils::CallWithRetries(
			std::bind(&FileSystem::IsDirectory, base_file_system_.get(),
					dirname), initial_delay_microseconds_);
}
