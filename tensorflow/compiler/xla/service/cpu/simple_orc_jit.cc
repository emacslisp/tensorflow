...
struct SimpleResolver: public llvm::JITSymbolResolver {
	llvm::JITSymbol findSymbol(const std::string &name) override
	{
		...
		if (canonical_name == runtime::kEigenMatmulF32SymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_EigenMatMulF32);
		} else if (canonical_name
				== runtime::kEigenSingleThreadedMatmulF32SymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_EigenSingleThreadedMatMulF32);
		} else if (canonical_name == runtime::kEigenConvF32SymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_EigenConvF32);
		} else if (canonical_name
				== runtime::kEigenSingleThreadedConvF32SymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_EigenSingleThreadedConvF32);
		} else if (canonical_name
				== runtime::kAcquireInfeedBufferForDequeueSymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_AcquireInfeedBufferForDequeue);
		} else if (canonical_name
				== runtime::kReleaseInfeedBufferAfterDequeueSymbolName) {
			func_addr =
					reinterpret_cast<void *>(__xla_cpu_runtime_ReleaseInfeedBufferAfterDequeue);
...
SimpleOrcJIT::SimpleOrcJIT(const llvm::TargetOptions &target_options,
		...
		target_machine_(
				CHECK_NOTNULL(
						llvm::EngineBuilder().setTargetOptions(target_options).setOptLevel(
								opt_level).selectTarget(
						/*TargetTriple=*/llvm::Triple(), /*MArch=*/"",
						/*MCPU=*/llvm::sys::getHostCPUName(),
						/*MAttrs=*/DetectMachineAttributes()))), disassembler_(
				*target_machine_), data_layout_(
				target_machine_->createDataLayout()), compile_layer_(
				object_layer_,
		...
		std::unique_ptr<llvm::Module> module)
{
	...
	auto handle = compile_layer_.addModuleSet(std::move(module_set),
			MakeUnique<llvm::SectionMemoryManager>(),
...
void SimpleOrcJIT::RemoveModule(SimpleOrcJIT::ModuleHandleT handle)
{
	module_handles_.erase(
			std::remove(module_handles_.begin(), module_handles_.end(),
					handle));
...
llvm::JITSymbol SimpleOrcJIT::FindSymbol(const std::string &name)
{
	std::string mangled_name;
	{
		...
		llvm::Mangler::getNameWithPrefix(mangled_name_stream, name,
				data_layout_);
	}

	// Resolve symbol from last module to first, allowing later redefinitions of
	// symbols shadow earlier ones.
	for (auto &handle : llvm::make_range(module_handles_.rbegin(),
			module_handles_.rend())) {
		if (auto symbol = compile_layer_.findSymbolIn(handle, mangled_name,
