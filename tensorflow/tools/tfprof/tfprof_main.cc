...
int main(int argc, char** argv)
{
	...
	std::vector<tensorflow::Flag> flag_list = { tensorflow::Flag("graph_path",
			&FLAGS_graph_path, "GraphDef proto text file name"),
			tensorflow::Flag("run_meta_path", &FLAGS_run_meta_path,
					"RunMetadata proto binary file name"), tensorflow::Flag(
					"op_log_path", &FLAGS_op_log_path,
					"tensorflow::tfprof::OpLog proto binary file name"),
			tensorflow::Flag("checkpoint_path", &FLAGS_checkpoint_path,
					"TensorFlow Checkpoint file name"), tensorflow::Flag(
					"max_depth", &FLAGS_max_depth, "max depth"),
			...
			tensorflow::Flag("min_float_ops", &FLAGS_min_float_ops,
					"min float ops"), tensorflow::Flag("device_regexes",
					&FLAGS_device_regexes, "device regexes"), tensorflow::Flag(
					"order_by", &FLAGS_order_by, "order by"), tensorflow::Flag(
					"account_type_regexes", &FLAGS_start_name_regexes,
					"start name regexes"), tensorflow::Flag("trim_name_regexes",
					&FLAGS_trim_name_regexes, "trim name regexes"),
			tensorflow::Flag("show_name_regexes", &FLAGS_show_name_regexes,
					"show name regexes"), tensorflow::Flag("hide_name_regexes",
					&FLAGS_hide_name_regexes, "hide name regexes"),
			tensorflow::Flag("account_displayed_op_only",
					&FLAGS_account_displayed_op_only,
					"account displayed op only"), tensorflow::Flag("select",
					&FLAGS_select, "select"), tensorflow::Flag("dump_to_file",
					&FLAGS_dump_to_file, "dump to file"), };
	...
	std::vector<tensorflow::string> device_regexes = Split(FLAGS_device_regexes,
			',', tensorflow::str_util::SkipEmpty());
	std::vector<tensorflow::string> account_type_regexes = Split(
			FLAGS_account_type_regexes, ',', tensorflow::str_util::SkipEmpty());
	std::vector<tensorflow::string> start_name_regexes = Split(
			FLAGS_start_name_regexes, ',', tensorflow::str_util::SkipEmpty());
	std::vector<tensorflow::string> trim_name_regexes = Split(
			FLAGS_trim_name_regexes, ',', tensorflow::str_util::SkipEmpty());
	std::vector<tensorflow::string> show_name_regexes = Split(
			FLAGS_show_name_regexes, ',', tensorflow::str_util::SkipEmpty());
	std::vector<tensorflow::string> hide_name_regexes = Split(
			FLAGS_hide_name_regexes, ',', tensorflow::str_util::SkipEmpty());
	std::vector < tensorflow::string > select = Split(FLAGS_select, ',',
			tensorflow::str_util::SkipEmpty());
	...
	if (argc == 1 && FLAGS_graph_path.empty()) {
		...
		printf("3) tfprof --graph_path <GraphDef proto text file>: "
		...
		if (tensorflow::string(argv[1]) == tensorflow::tfprof::kCmds[0]
				|| tensorflow::string(argv[1])
						== tensorflow::tfprof::kCmds[1]) {
			cmd = argv[1];
		}
	}
	...
	TF_CHECK_OK(
			tensorflow::tfprof::ReadGraphDef(tensorflow::Env::Default(),
	...
	if (!FLAGS_checkpoint_path.empty()) {
		ckpt_reader.reset(
				new tensorflow::checkpoint::CheckpointReader(
	...
	tensorflow::tfprof::TFStats tf_stat(std::move(graph), std::move(run_meta),
			std::move(op_log), std::move(ckpt_reader));
	tensorflow::tfprof::Options opts(FLAGS_max_depth, FLAGS_min_bytes,
			FLAGS_min_micros, FLAGS_min_params, FLAGS_min_float_ops,
			device_regexes, FLAGS_order_by, account_type_regexes,
			start_name_regexes, trim_name_regexes, show_name_regexes,
			hide_name_regexes, FLAGS_account_displayed_op_only, select,
			FLAGS_viz, FLAGS_dump_to_file);
	...
	for (char* line = nullptr; (line = linenoise("tfprof> ")) != nullptr;) {
		...
		tensorflow::Status s = tensorflow::tfprof::ParseCmdLine(line_s, &cmd,
				&new_opts);
		if (!s.ok()) {
			fprintf(stderr, "E: %s\n", s.ToString().c_str());
			continue;
