...
Status DebugIO::PublishDebugTensor(const string& tensor_name,
		const string& debug_op, const Tensor& tensor, const uint64 wall_time_us,
		const gtl::ArraySlice<string>& debug_urls)
{
	...
	for (const string& url : debug_urls) {
		if (str_util::Lowercase(url).find(kFileURLScheme) == 0) {
			...
			Status s = DebugFileIO::DumpTensorToDir(node_name, output_slot,
					debug_op, tensor, wall_time_us, dump_root_dir, nullptr);
			if (!s.ok()) {
				num_failed_urls++;
				fail_statuses.push_back(s);
			}
		} else if (str_util::Lowercase(url).find(kGrpcURLScheme) == 0) {
			const string grpc_server_stream_addr = url.substr(
					strlen(kGrpcURLScheme));
			Status s = DebugGrpcIO::SendTensorThroughGrpcStream(node_name,
					output_slot, debug_op, tensor, wall_time_us,
	...
	if (num_failed_urls == 0) {
		...
		string error_message = strings::StrCat("Publishing to ",
				num_failed_urls, " of ", debug_urls.size(),
				" debug target URLs failed, due to the following errors:");
		for (Status& status : fail_statuses) {
			error_message = strings::StrCat(error_message, " ",
		...
		const std::unordered_set<string>& debug_urls)
{
	...
	for (const string& debug_url : debug_urls) {
		if (debug_url.find(kFileURLScheme) == 0) {
			const string dump_root_dir = debug_url.substr(
					strlen(kFileURLScheme));
			const string file_name = strings::StrCat("_tfdbg_graph_",
					now_micros);

			status.Update(
					DebugFileIO::DumpEventProtoToFile(event, dump_root_dir,
							file_name));
		} else if (debug_url.find(kGrpcURLScheme) == 0) {
			DebugGrpcIO::SendEventProtoThroughGrpcStream(event,
					debug_url.substr(strlen(kGrpcURLScheme)));
		}
	}
...
// static
Status DebugFileIO::DumpTensorToDir(const string& node_name,
		const int32 output_slot, const string& debug_op, const Tensor& tensor,
...
string DebugFileIO::GetDumpFilePath(const string& dump_root_dir,
		const string& node_name, const int32 output_slot,
		const string& debug_op, const uint64 wall_time_us)
{
	return io::JoinPath(dump_root_dir,
			strings::StrCat(node_name, "_", output_slot, "_", debug_op, "_",
					wall_time_us));
}

// static
Status DebugFileIO::DumpEventProtoToFile(const Event& event_proto,
		const string& dir_name, const string& file_name)
{
	Env* env(Env::Default());
...
// static
Status DebugFileIO::DumpTensorToEventFile(const string& node_name,
		const int32 output_slot, const string& debug_op, const Tensor& tensor,
		const uint64 wall_time_us, const string& file_path)
{
	const string tensor_name = strings::StrCat(node_name, ":", output_slot);
	Event event = WrapTensorAsEvent(tensor_name, debug_op, tensor,
			wall_time_us);
...
DebugGrpcChannel::DebugGrpcChannel(const string& server_stream_addr) :
		ctx_(), channel_(
				::grpc::CreateCustomChannel(server_stream_addr,
						::grpc::InsecureChannelCredentials(),
						::grpc::ChannelArguments())), stub_(
				EventListener::NewStub(channel_)), reader_writer_(
				stub_->SendEvents(&ctx_)), mu_()
{
}
...
std::unordered_map<string, std::shared_ptr<DebugGrpcChannel>> DebugGrpcIO::stream_channels;

// static
Status DebugGrpcIO::SendTensorThroughGrpcStream(const string& node_name,
		const int32 output_slot, const string& debug_op, const Tensor& tensor,
		const uint64 wall_time_us, const string& server_stream_addr)
{
	const string tensor_name = strings::StrCat(node_name, ":", output_slot);
...
// static
Status DebugGrpcIO::SendEventProtoThroughGrpcStream(const Event& event_proto,
		const string& server_stream_addr)
{
		...
		if (stream_channels.find(server_stream_addr) == stream_channels.end()) {
			...
			if (!debug_grpc_channel->is_channel_ready()) {
				return errors::FailedPrecondition(
						strings::StrCat(
								"Channel at the following gRPC address is ",
	...
	if (!write_ok) {
		return errors::Cancelled(
				strings::StrCat("Write event to stream URL ",
