...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		functor::MatrixBandPart<Device, T>::Compute(
				context->eigen_device<Device>(), num_lower, num_upper,
				...
				for (int64 i = 0; i < output.dimension(1); ++i) {
					...
					const int64 band_end =
							num_upper < 0 ?
									output.dimension(2) :
									std::min(
											static_cast<int64>(output.dimension(
													2)), i + num_upper + 1);
					if (band_start < band_end) {
						const Eigen::DSizes<Eigen::DenseIndex, 3> indices(r, i,
								band_start);
						const Eigen::DSizes<Eigen::DenseIndex, 3> sizes(1, 1,
								band_end - band_start);
						output.slice(indices, sizes) = input.slice(indices,
								sizes);
					}
				}
			}
