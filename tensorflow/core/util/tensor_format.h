...
inline TensorShape ShapeFromFormat(TensorFormat dst_format,
		const TensorShape& src_shape, TensorFormat src_format)
{
	if (src_format == dst_format) {
		return src_shape;
	}
	std::vector<int64> dim_sizes(4);
	dim_sizes[GetTensorDimIndex<2>(dst_format, 'N')] = GetTensorDim(src_shape,
			src_format, 'N');
	dim_sizes[GetTensorDimIndex<2>(dst_format, 'H')] = GetTensorDim(src_shape,
			src_format, 'H');
	dim_sizes[GetTensorDimIndex<2>(dst_format, 'W')] = GetTensorDim(src_shape,
			src_format, 'W');
	dim_sizes[GetTensorDimIndex<2>(dst_format, 'C')] = GetTensorDim(src_shape,
			src_format, 'C');
	return TensorShape(dim_sizes);
}
