...
public:
			...
			OpOutputList* decoded_shape) const
	{
		...
		Status s = ctx->allocate_output("log_probability", TensorShape( {
				batch_size, top_paths_ }), log_prob);
		if (!s.ok())
			return s;
			...
			OpOutputList* decoded_shape) const
	{
		...
		for (int p = 0; p < top_paths_; ++p) {
			...
			Status s = decoded_indices->allocate(p, TensorShape( { p_num, 2 }),
					&p_indices);
			if (!s.ok())
				return s;
...
public:
	explicit CTCGreedyDecoderOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				decode_helper_.ValidateInputsGenerateOutputs(ctx, &inputs,
						&seq_len, &log_prob, &decoded_indices, &decoded_values,
						&decoded_shape));
		...
		OP_REQUIRES(ctx,
				FastBoundsCheck(num_classes_raw,
		...
		for (std::size_t t = 0; t < max_time; ++t) {
			input_list_t.emplace_back(
					inputs_t.data() + t * batch_size * num_classes, batch_size,
					num_classes);
		}
		auto seq_len_t = seq_len->vec<int32>();
		...
		for (int b = 0; b < batch_size; ++b) {
			...
			for (int t = 0; t < seq_len_t(b); ++t) {
				...
				log_prob_t(b, 0) += -RowMax(input_list_t[t], b,
						&max_class_indices);
		...
		OP_REQUIRES_OK(ctx,
				decode_helper_.StoreAllDecodedSequences(sequences,
...
public:
	explicit CTCBeamSearchDecoderOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("merge_repeated", &merge_repeated_));
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		ctc::CTCBeamSearchDecoder<> beam_search(num_classes, beam_width_,
				&beam_scorer_, 1 /* batch_size */, merge_repeated_);
		...
		for (int b = 0; b < batch_size; ++b) {
			...
			for (int t = 0; t < seq_len_t(b); ++t) {
				...
				auto input_bi = Eigen::Map<const Eigen::ArrayXf>(
		...
		OP_REQUIRES_OK(ctx,
				decode_helper_.StoreAllDecodedSequences(best_paths,
						&decoded_indices, &decoded_values, &decoded_shape));
	}
