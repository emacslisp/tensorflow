...
public:
	explicit EncodePngOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument(
						"image cannot have >= int32 max elements"));
		...
		OP_REQUIRES(context, channels >= 1 && channels <= 4,
				errors::InvalidArgument(
						"image must have 1, 2, 3, or 4 channels, got ",
						channels));
		...
		if (desired_channel_bits_ == 8) {
			...
			OP_REQUIRES(context,
					png::WriteImageToBuffer(image.flat<uint16>().data(), width,
							height, width * channels * 2, channels,
							desired_channel_bits_, compression_,
							&output->scalar<string>()(), nullptr),
					errors::Internal("PNG encoding failed"));
		}
