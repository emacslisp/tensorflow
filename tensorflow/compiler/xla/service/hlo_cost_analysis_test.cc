...
protected:
	HloCostAnalysisTest() :
			...
			service_(
					static_cast<Service*>(ClientLibrary::GetXlaService(
							static_cast<LocalClient*>(client_)->platform()))), computation_tracker_(
	...
	std::unique_ptr<HloModule> BuildHloGraph(ComputationBuilder* builder)
	{
		...
		auto user_computation_status = computation_tracker_.Resolve(
				computation.handle());
