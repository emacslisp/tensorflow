...
AsyncExecution::AsyncExecution(Backend* backend,
		...
		backend_(CHECK_NOTNULL(backend)), streams_(std::move(streams)), profile_(
				profile), result_(result)
{
		...
		const ExecutionProfile& profile, GlobalDataHandle result)
{
	...
	auto inserted = handle_to_execution_.emplace(handle,
			MakeUnique<AsyncExecution>(backend, std::move(streams), profile,
					result));
