...
public:
	explicit SparseSplitOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int slice_index = 0; slice_index < num_split_; ++slice_index) {
			...
			OP_REQUIRES_OK(context,
					context->allocate_output(slice_index + 2 * num_split_, {
							outputs[slice_index].shape().dims() }, &shape));
			for (int dim = 0; dim < outputs[slice_index].shape().dims();
					++dim) {
				shape->vec<int64>()(dim) =
						outputs[slice_index].shape().dim_size(dim);
			}
		}
	}
