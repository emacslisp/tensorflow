...
private:
	...
	Status BuildParallelCheckOp(
			...
			const Subgraph& subgraph, Graph* graph_out,
...
Status Encapsulator::SplitIntoSubgraphs()
{
	...
	for (Node* node : graph_in_->nodes()) {
		...
		string device =
				node->assigned_device_name().empty() ?
						node->def().device() : node->assigned_device_name();
	...
	for (const Edge* edge : graph_in_->edges()) {
		...
		Node* src_image = gtl::FindWithDefault(node_images, edge->src(),
				nullptr);
		Node* dst_image = gtl::FindWithDefault(node_images, edge->dst(),
				nullptr);
		...
		if (!src_func_id.empty() && src_func_id == dst_func_id) {
			...
			if (edge->IsControlEdge()) {
				...
				g->AddEdge(src_image, edge->src_output(), dst_image,
						edge->dst_input());
			}
			continue;
		}
		...
		if (!src_func_id.empty()) {
			...
			if (src_subgraph.results.emplace(
					NodeSlot(edge->src(), edge->src_output()), ret_index).second) {
				...
				src_subgraph.graph->AddEdge(src_image, edge->src_output(), ret,
						0);
			}
		}
		...
		if (!dst_func_id.empty()) {
			...
			std::tie(iter, inserted) = dst_subgraph.args_by_src.emplace(
					NodeSlot(edge->src(), edge->src_output()),
					dst_subgraph.args.size());
			...
			if (inserted) {
				...
				NodeDef arg_def;
				NodeDefBuilder builder(dst_subgraph.graph->NewName("input"),
						kArgOp);
			...
			dst_subgraph.args_by_dst[NodeSlot(edge->dst(), edge->dst_input())] =
					arg_index;
			dst_subgraph.graph->AddEdge(dst_subgraph.args[arg_index], 0,
					dst_image, edge->dst_input());
		}
	}
	...
	for (auto& subgraph_entry : subgraphs_) {
		...
		if (VLOG_IS_ON(1)) {
			...
			dump_graph::DumpGraphToFile(
					strings::StrCat("encapsulate_fdef_graph_", name),
					*subgraph.graph, library);
		...
		Node** parallel_check_op)
{
	...
	for (int i = 0; i < num_results; ++i) {
		...
		expected_outputs[i] = NodeDefBuilder::NodeOut(
				node_images.at(node_slot.node)->name(), node_slot.slot,
				result_dtypes[i]);
		actual_outputs[i] = NodeDefBuilder::NodeOut(
				subgraph.call_node_def.name(), i, result_dtypes[i]);
	}
	// Assign the parallel check op to a CPU on the same task as the cluster it is
	// checking.
	string device, dummy;
	if (!DeviceNameUtils::SplitDeviceName(
			subgraph.call_node_inputs->assigned_device_name(), &device,
			&dummy)) {
	...
	TF_RETURN_IF_ERROR(
			NodeDefBuilder(
					graph_out->NewName(
							strings::StrCat(subgraph.call_node_def.name(),
									"_parallel_check")), "ParallelCheck").Device(
					device).Attr("T", result_dtypes).Input(expected_outputs).Input(
					actual_outputs).Finalize(&check_def));
	...
	for (int i = 0; i < num_results; ++i) {
		const NodeSlot& node_slot = results_by_num[i];
		graph_out->AddEdge(node_images.at(node_slot.node), node_slot.slot,
				check_op, i);
		graph_out->AddEdge(subgraph.call_node_inputs, i, check_op,
				num_results + i);
	}
...
Status Encapsulator::BuildOutputGraph(bool parallel_checking, Graph* graph_out)
{
	...
	for (Node* node : graph_in_->nodes()) {
		...
		// is enabled.
		if (!func_id.empty() && !parallel_checking)
			continue;
	...
	for (auto& subgraph_entry : subgraphs_) {
		...
		subgraph.call_node_inputs = graph_out->AddNode(subgraph.call_node_def,
				&s);
		if (!s.ok())
			return s;
		...
		if (parallel_checking) {
			TF_RETURN_IF_ERROR(
					BuildParallelCheckOp(node_images, subgraph, graph_out,
	...
	for (const Edge* edge : graph_in_->edges()) {
		...
		if (edge->IsControlEdge()) {
			...
			if (edges_added.emplace(NodeSlot(src_image, -1),
					NodeSlot(dst_image, -1)).second) {
				graph_out->AddControlEdge(src_image, dst_image);
			}
			...
			if (parallel_checking) {
				graph_out->AddControlEdge(src_image,
						node_images.at(edge->dst()));
			}
			continue;
		}
		...
		if (!src_func_id.empty()) {
			...
			src_output = src_subgraph.results.at(
					NodeSlot(edge->src(), edge->src_output()));
		}
		...
		if (!dst_func_id.empty()) {
			...
			dst_input = dst_subgraph.args_by_dst.at(
					NodeSlot(edge->dst(), edge->dst_input()));

			// If we are parallel checking, also feed the tensor as an input to the
			// corresponding parallel check subgraph.
			if (parallel_checking) {
				graph_out->AddEdge(src_image, src_output,
						node_images.at(edge->dst()), edge->dst_input());
			}
		}
		// Add the edge, if we have not already added it.
		if (edges_added.emplace(NodeSlot(src_image, src_output),
				NodeSlot(dst_image, dst_input)).second) {
...
Status EncapsulateSubgraphsInFunctions(string group_attribute,
		const Graph& graph_in, const RewriteSubgraphFn& rewrite_subgraph_fn,
		bool parallel_checking, std::unique_ptr<Graph>* graph_out,
		FunctionLibraryDefinition* library)
{
	Status s;
		...
		const GraphOptimizationPassOptions& options)
{
	...
	if (VLOG_IS_ON(1)) {
		dump_graph::DumpGraphToFile("before_encapsulate_subgraphs",
	...
	std::unique_ptr<FunctionLibraryRuntime> flr(
			NewFunctionLibraryRuntime(nullptr, options.session_options->env,
					nullptr, TF_GRAPH_DEF_VERSION, library, opts));
	...
	TF_RETURN_IF_ERROR(
			EncapsulateSubgraphsInFunctions(kXlaClusterAttr, **options.graph,
...
bool IsXlaCompiledKernel(const Node& node)
{
	...
	bool has_compilation_attr = GetNodeAttr(node.def(), kXlaCompiledKernelAttr,
			&is_compiled).ok() && is_compiled;
