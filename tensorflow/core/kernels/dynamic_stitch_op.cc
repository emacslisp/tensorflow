...
public:
	explicit DynamicStitchOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	...
	void Compute(OpKernelContext* c) override
	{
		...
		for (int input_num = 0; input_num < indices_inputs.size();
			...
			OP_REQUIRES(c,
					...
					errors::InvalidArgument("Need data[0].shape[",
							indices0.dims(), ":] = data[", input_num,
							"].shape[", indices.dims(),
							":], got data[0].shape = ",
							data0.shape().DebugString(), ", data[", input_num,
							"].shape = ", data.shape().DebugString(),
							", indices[0].shape = ",
							indices0.shape().DebugString(), ", indices[",
							input_num, "].shape = ",
		...
		if (first_dim_size > 0) {
			...
			for (int input_num = 0; input_num < indices_inputs.size();
				...
				const Tensor& data = data_inputs[input_num];
				auto data_flat = data.shaped<T, 2>( { indices_vec.dimension(0),
						slice_size });
					...
					for (int i = 0; i < indices_vec.size(); i++) {
						...
						OP_REQUIRES(c, FastBoundsCheck(index, first_dim_size),
								errors::InvalidArgument("indices[", i,
										"] is out of range"));
						memcpy(merged_base + index * slice_size,
