...
private:
	...
	typedef std::unordered_map<uint64, ReffedClientGraph*> RCGMap;RCGMap
	run_graphs_ GUARDED_BY(mu_);RCGMap
	partial_run_graphs_ GUARDED_BY(mu_);
	...
	condition_variable num_running_is_zero_;int32
	num_running_ GUARDED_BY(mu_) = 0;
	...
	int64
	next_node_id_ GUARDED_BY(mu_) = 0;
