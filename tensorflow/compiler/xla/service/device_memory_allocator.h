...
public:
	...
	virtual tensorflow::Status Deallocate(int device_ordinal,
			perftools::gputools::DeviceMemoryBase* mem) = 0;

	// Return the platform that the allocator allocates memory on.
	const perftools::gputools::Platform* platform() const
	{
		return platform_;
	}
...
public:
	StreamExecutorMemoryAllocator(perftools::gputools::Platform* platform,
	...
	StatusOr<perftools::gputools::DeviceMemoryBase> Allocate(int device_ordinal,
			uint64 size, bool retry_on_failure = true) override;
