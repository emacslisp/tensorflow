...
public:
	...
	FakeLibCurl(const string& response_content, uint64 response_code,
			...
			response_content_(response_content), response_code_(response_code), response_headers_(
					response_headers)
	{
	}
	...
	CURLcode curl_easy_perform(CURL* curl) override
	{
		if (read_data_) {
			...
			posted_content_ = "";
			do {
				bytes_read = read_callback_(buffer, 1, sizeof(buffer),
						read_data_);
				posted_content_ = strings::StrCat(posted_content_,
						StringPiece(buffer, bytes_read));
			} while (bytes_read > 0);
		}
		if (write_data_) {
			write_callback_(response_content_.c_str(), 1,
					response_content_.size(), write_data_);
		}
	...
	{
		std::vector<string>* v =
				list ? reinterpret_cast<std::vector<string>*>(list) : new std::vector<
								string>();
		v->push_back(str);
	...
	char* curl_easy_escape(CURL* curl, const char* str, int length) override
	{
		...
		char* out_char_str = reinterpret_cast<char*>(port::Malloc(
				sizeof(char) * temp_str.size() + 1));
