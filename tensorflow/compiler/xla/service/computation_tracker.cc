		...
		const string& computation_name)
{
	...
	opaque_to_computation_[handle_value] = MakeUnique<UserComputation>(
			computation_name, computation_handle);
		...
		const SessionModule& session_module)
{
	...
	for (const SessionComputation& computation : session_module.embedded_computations()) {
	...
	const int64 old_handle =
			session_module.entry().computation_handle().handle();
	TF_ASSIGN_OR_RETURN(old_to_new[old_handle],
			LoadSessionComputation(session_module.entry(), &old_to_new));
	return old_to_new[old_handle];
}

StatusOr<std::unique_ptr<SessionModule>> ComputationTracker::SnapshotComputation(
		const ComputationHandle& computation)
{
	...
	{
		tensorflow::mutex_lock lock(computation_mutex_);
		ComputeComputationPostOrder(entry_versioned_handle, &visited,
				&post_order);
	}
	auto session_module = MakeUnique<SessionModule>();
	*session_module->mutable_entry() =
			Resolve(entry_versioned_handle.handle).ValueOrDie()->CloneSessionComputation(
					entry_versioned_handle.version);
	for (auto it = ++post_order.rbegin(); it != post_order.rend(); ++it) {
		*session_module->add_embedded_computations() =
				Resolve(it->handle).ValueOrDie()->CloneSessionComputation(
						it->version);
	}
		...
		std::map<int64, ComputationHandle>* old_to_new)
{
	...
	(*old_to_new)[session_computation.computation_handle().handle()] =
			new_handle;
	TF_ASSIGN_OR_RETURN(opaque_to_computation_[new_handle.handle()],
			UserComputation::MakeWithRemapping(session_computation, new_handle,
					*old_to_new));
	return new_handle;
}
		...
		const ComputationHandle& computation) const
{
	...
	if (it == opaque_to_computation_.end()) {
		return NotFound("computation handle not found: %lld",
				computation.handle());
	}
		...
		bool include_unused_parameters) const
{
	...
	string module_name = tensorflow::strings::StrCat(entry_computation->name(),
			"_module");
	...
	for (auto versioned_handle : post_order) {
		...
		TF_ASSIGN_OR_RETURN(std::unique_ptr < HloComputation > hlo_computation,
				computation->BuildHloComputation(versioned_handle.version,
						resolver, include_unused_parameters));
