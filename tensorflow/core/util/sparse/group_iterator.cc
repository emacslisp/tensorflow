...
{
	CHECK_EQ(rhs.iter_, iter_)
			<< "Can't compare steps from different iterators";
...
GroupIterable::IteratorStep GroupIterable::IteratorStep::operator++(int)
{  // postfix ++
...
TTypes<int64>::UnalignedConstMatrix Group::indices() const
{
	return TTypes<int64>::UnalignedConstMatrix(
			&(iter_->ix_.matrix<int64>()(loc_, 0)), next_loc_ - loc_,
			iter_->dims_);
}
