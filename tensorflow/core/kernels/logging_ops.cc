...
public:
	explicit AssertOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		for (int i = 1; i < ctx->num_inputs(); ++i) {
			strings::StrAppend(&msg, "[",
					ctx->input(i).SummarizeValue(summarize_), "]");
			if (i < ctx->num_inputs() - 1)
				strings::StrAppend(&msg, " ");
		}
...
private:
	mutex mu_;int64
	call_counter_ GUARDED_BY(mu_) = 0;
	int64 first_n_ = 0;
	int32 summarize_ = 0;
	string message_;
};
