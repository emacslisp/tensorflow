...
public:
	...
	virtual void NextFrame(const uint8* const new_frame, const int64 timestamp,
...
protected:
	...
	virtual TrackedObject* MaybeAddObject(const std::string& id,
			const Image<uint8>& image, const BoundingBox& bounding_box,
		...
		const ObjectTracker& tracker)
{
	...
	stream << "first frame: " << first_frame_index << ","
			<< first_frame_pair.end_time_ << "    " << "last frame: "
			<< last_frame_index << "," << last_frame_pair.end_time_
			<< "   diff: "
