...
bool IsConstantFoldable(const FunctionLibraryDefinition* flib_def,
		const Node* n, std::function<bool(const Node*)> consider)
{
...
void FindConstantFoldableNodes(const Graph* graph,
		const FunctionLibraryDefinition* flib_def, ConstantFoldingOptions opts,
		...
		for (const Edge* out_edge : added_nodes.first->out_edges()) {
			if (node_map.count(out_edge->dst()) == 0) {
				...
				tensors_to_fetch->insert(
						{ { added_nodes.second, out_edge->src_output() },
								added_nodes.first });
			}
		}
	}
		...
		NodeAndOutput tensor, const Tensor& constant)
{
	...
	DeviceType device_type =
			partition_device ?
					DeviceType { partition_device->device_type() } : DEVICE_CPU;
	if (partition_device && device_type != DEVICE_CPU) {
		MemoryType memory_type;
		if (!MemoryTypeForOutput(device_type, graph, tensor.first,
				tensor.second, &memory_type).ok()) {
			return false;
		}
	...
	Node* constant_node;
	auto builder = NodeDefBuilder(
			strings::StrCat(graph->NewName(node_name), "__cf__",
					UniqueConstantId()), "Const").Attr("dtype",
			constant.dtype()).Attr("value", constant);
	if (partition_device) {
...
{
	bool was_mutated;
	Status unused_status = DoConstantFoldingWithStatus(opts, function_library,
			env, partition_device, graph, &was_mutated);
	return was_mutated;
}
		...
		Device* partition_device, Graph* graph, bool* was_mutated)
{
	...
	std::unique_ptr<Graph> constant_graph(
			GetConstantGraph(graph, constant_foldable_nodes,
					&tensors_to_fetch));
