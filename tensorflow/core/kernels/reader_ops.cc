...
public:
	...
	explicit ReaderVerbAsyncOpKernel(OpKernelConstruction* context) :
			AsyncOpKernel(context), thread_pool_(
...
public:
	...
	void ComputeWithReader(OpKernelContext* context, ReaderInterface* reader) override
	{
...
public:
	...
	void ComputeWithReader(OpKernelContext* context, ReaderInterface* reader) override
	{
		...
		OP_REQUIRES_OK(context,
				context->input("num_records", &num_records_tensor));
		...
		int64 num_actually_read = reader->ReadUpTo(num_records, queue,
				&keys_vec, &values_vec, context);
		...
		Tensor* keys = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output("keys", TensorShape( {
				num_actually_read }), &keys));

		Tensor* values = nullptr;
		OP_REQUIRES_OK(context,
				context->allocate_output("values", TensorShape( {
						num_actually_read }), &values));
...
public:
	...
	void ComputeWithReader(OpKernelContext* context, ReaderInterface* reader) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output("records_produced", TensorShape( { }),
						&output));
...
public:
	...
	void ComputeWithReader(OpKernelContext* context, ReaderInterface* reader) override
	{
		Tensor* output = nullptr;
		OP_REQUIRES_OK(context,
				context->allocate_output("units_completed", TensorShape( { }),
						&output));
	...
	void ComputeWithReader(OpKernelContext* context, ReaderInterface* reader) override
	{
		...
		OP_REQUIRES(context, TensorShapeUtils::IsScalar(tensor->shape()),
				errors::InvalidArgument(
						"Reader state must be scalar, but had shape: ",
						tensor->shape().DebugString()));
		OP_REQUIRES_OK(context,
				reader->RestoreState(tensor->scalar<string>()()));
	}
};
