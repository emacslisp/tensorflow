...
string Name(int node_index)
{
	return strings::StrCat("n", node_index);
}
...
Status BuildInputArgIndex(const OpDef::ArgDef& arg_def,
		const InstantiateAttrValueMap& attr_values, NameInfoIndex* name_info,
		InstantiationResult* result)
{
	...
	TF_RETURN_IF_ERROR(AddArgName(name_info, arg_def.name(), { true, arg_index,
			0, is_type_list, dtypes }));
	// Creates dtypes.size() nodes in the gdef.
	for (size_t i = 0; i < dtypes.size(); ++i) {
		TF_RETURN_IF_ERROR(
				AddArgName(name_info, strings::StrCat(arg_def.name(), ":", i), {
		...
		const int arg_index, NameInfoIndex* name_info)
{
	...
	for (int i = 0; i < num_retval; ++i) {
		TF_RETURN_IF_ERROR(
				ArgNumType(attrs, node_sig->output_arg(i), &is_type_list,
						&dtypes));
		// Note that we rely on the backwards-compatibility test enforcing
		// that output_arg(*).name() doesn't change here.
		const string base_name = strings::StrCat(node.name(), ":",
				node_sig->output_arg(i).name());
		TF_RETURN_IF_ERROR(AddRetName(name_info, base_name, { false, arg_index,
				start, is_type_list, dtypes }));
		...
		const NameInfoIndex& name_info, GraphDef* gdef)
{
	...
	for (int i = 0; i < num_args; ++i) {
		TF_RETURN_IF_ERROR(
				ArgNumType(attrs, fnode_sig->input_arg(i), &is_type_list,
						&dtypes));
		...
		for (size_t j = 0; j < dtypes.size(); ++fnode_arg_index) {
			if (fnode_arg_index >= fnode.input_size()) {
				...
				return errors::InvalidArgument(
						...
						fnode_arg_index, " >= ", fnode.input_size());
			}
			...
			if (item == nullptr) {
				return errors::InvalidArgument("input ", input_name,
						" is not found: ", SummarizeNodeDef(fnode));
			}
			if (item->dtypes.size() > dtypes.size() - j) {
				return errors::InvalidArgument("Input ", input_name,
						" too long for ", fnode_sig->input_arg(i).name());
			}
			...
			for (int k = 0; k < item->dtypes.size(); ++k, ++j) {
				if (item->dtypes[k] != dtypes[j]) {
					return errors::InvalidArgument("input ",
							fnode_sig->input_arg(i).name(), "[", j,
							"] expected type ", DataTypeString(dtypes[j]),
							" != ", DataTypeString(item->dtypes[k]),
							", the type of ", input_name, "[", k, "]");
				}
				if (item->is_func_arg) {
	...
	for (int i = fnode_arg_index; i < fnode.input_size(); ++i) {
		...
		if (input.empty() || input[0] != '^') {
			return errors::InvalidArgument("Expected input[", i, "] == '",
					input, "' to be a control input.");
		}
		int nid = -1;
		...
		for (const auto& p : name_info) {
			if (p.first == node_name
					|| tensorflow::StringPiece(p.first).starts_with(
							node_colon)) {
				nid = p.second.nid;
				break;
			}
	...
	if (sig.attr_size() > 0) {
		...
		for (int i = 0; i < sig.attr_size(); ++i) {
			...
			if (i > 0)
				strings::StrAppend(&out, ", ");
			if (a.type() == "type") {
				strings::StrAppend(&out, a.name(), ":",
						Print(a.allowed_values()));
			} else {
		...
		GetFunctionSignature get_function, InstantiationResult* result)
{
	...
	for (int i = 0; i < fdef.node_def_size(); ++i) {
		...
		TF_RETURN_IF_ERROR(
				AddDefaultAttrs(fdef.node_def(i).op(), get_function,
						&node_attrs[i]));
	}
	...
	for (int i = 0; i < fdef.node_def_size(); ++i) {
		...
		if (!s.ok()) {
			errors::AppendToMessage(&s, "In ",
					SummarizeNodeDef(fdef.node_def(i)));
			return s;
		}
	}
	...
	for (const OpDef::ArgDef& ret_def : sig.output_arg()) {
		s = AddReturnNode(ret_def, attr_values, fdef.ret(), name_info,
				&ret_index, result);
		if (!s.ok()) {
...
FunctionCallFrame::FunctionCallFrame(DataTypeSlice arg_types,
		...
		arg_types_(arg_types.begin(), arg_types.end()), ret_types_(
				ret_types.begin(), ret_types.end())
{
...
Status FunctionCallFrame::SetArgs(gtl::ArraySlice<Tensor> args)
{
	...
	if (args.size() != arg_types_.size()) {
		return errors::InvalidArgument("Expects ", arg_types_.size(),
				" arguments, but ", args.size(), " is provided");
	}
	for (size_t i = 0; i < args.size(); ++i) {
		if (arg_types_[i] != args[i].dtype()) {
			return errors::InvalidArgument("Expects arg[", i, "] to be ",
					DataTypeString(arg_types_[i]), " but ",
...
Status FunctionCallFrame::SetRetval(int index, const Tensor& val)
{
	if (index < 0 || static_cast<size_t>(index) >= rets_.size()) {
		return errors::InvalidArgument("SetRetval ", index,
				" is not within [0, ", rets_.size(), ")");
	}
	if (val.dtype() != ret_types_[index]) {
		return errors::InvalidArgument("Expects ret[", index, "] to be ",
				DataTypeString(ret_types_[index]), ", but ",
				DataTypeString(val.dtype()), " is provided.");
	}
...
FunctionLibraryDefinition::FunctionLibraryDefinition(
		...
		default_registry_(default_registry), function_defs_(
				def_lib.function_size())
{
...
Status FunctionLibraryDefinition::AddFunctionDef(const FunctionDef& fdef)
{
	...
	if (ptr != nullptr) {
		return errors::InvalidArgument("Function with name: ",
				fdef.signature().name(), " already exists in function library.");
	}
	ptr.reset(new FunctionDefAndOpRegistration(fdef));
	return Status::OK();
}
...
Status FunctionLibraryDefinition::LookUp(const string& op,
		...
		const NodeDef& ndef) const
{
	...
	if (!GetNodeAttr(AttrSlice(&ndef.attr()), kFuncAttr, &forward_func_attrs).ok()) {
		return nullptr;
	}
		...
		gtl::ArraySlice<std::pair<string, AttrValueWrapper>> attrs)
{
	...
	for (const auto& a : attrs) {
		ret.proto.mutable_func()->mutable_attr()->insert(
				{ a.first, a.second.proto });
	}
	return ret;
}
...
/* static */
FunctionDef FunctionDefHelper::Create(const string& function_name,
		gtl::ArraySlice<string> in_def, gtl::ArraySlice<string> out_def,
		gtl::ArraySlice<string> attr_def, gtl::ArraySlice<Node> node_def,
...
FunctionDef FunctionDefHelper::Define(const string& name,
		gtl::ArraySlice<string> arg_def, gtl::ArraySlice<string> ret_def,
		gtl::ArraySlice<string> attr_def, gtl::ArraySlice<Node> node_def)
{
	FunctionDef fdef;
	OpDefBuilder b(name);
	...
	for (const auto& src : node_def) {
		...
		for (const auto& o : output_names) {
			CHECK_LE(o.second.second, src.ret.size())
					<< "Missing ret for output '" << o.first << "' in '"
					<< src.ret[0] << "' of " << name;
			for (int i = o.second.first; i < o.second.second; ++i) {
				ret_index[src.ret[i]] = strings::StrCat(src.ret[0], ":",
						o.first, ":", i - o.second.first);
			}
		}
	}
	...
	for (const auto& r : fdef.signature().output_arg()) {
		...
		CHECK(iter != ret_index.end()) << "Return '" << r.name() << "' in "
				<< name;
...
FunctionDef FunctionDefHelper::Define(gtl::ArraySlice<string> arg_def,
		gtl::ArraySlice<string> ret_def, gtl::ArraySlice<string> attr_def,
