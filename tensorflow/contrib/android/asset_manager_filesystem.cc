...
public:
	ScopedAssetDir(AAssetDir* asset_dir) :
...
public:
			...
			char* scratch) const override
	{
		auto asset = ScopedAsset(
				AAssetManager_open(asset_manager_, file_name_.c_str(),
						AASSET_MODE_RANDOM));
		...
		const off64_t region_left = std::min(length - new_offset,
				static_cast<off64_t>(to_read));
...
Status AssetManagerFileSystem::FileExists(const string& fname)
{
	...
	auto asset = ScopedAsset(
			AAssetManager_open(asset_manager_, path.c_str(),
					AASSET_MODE_RANDOM));
	if (asset.get() == nullptr) {
...
Status AssetManagerFileSystem::NewRandomAccessFile(const string& fname,
		...
		const string& fname, std::unique_ptr<ReadOnlyMemoryRegion>* result)
{
	...
	if (fd >= 0) {
		...
		if (asset_buffer == nullptr) {
			return errors::Internal("Error reading ", fname,
					" from asset manager.");
		}
		...
		std::vector<string>* r)
{
	...
	auto dir = ScopedAssetDir(
			AAssetManager_openDir(asset_manager_, path.c_str()));
	if (dir.get() == nullptr) {
...
Status AssetManagerFileSystem::NewWritableFile(const string& fname,
		std::unique_ptr<WritableFile>* result)
{
	return errors::Unimplemented("Asset storage is read only.");
}
Status AssetManagerFileSystem::NewAppendableFile(const string& fname,
		std::unique_ptr<WritableFile>* result)
{
	return errors::Unimplemented("Asset storage is read only.");
}
