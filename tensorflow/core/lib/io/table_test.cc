...
namespace test {
static StringPiece RandomString(random::SimplePhilox* rnd, int len, string* dst)
{
	dst->resize(len);
	for (int i = 0; i < len; i++) {
...
static StringPiece CompressibleString(random::SimplePhilox* rnd,
		double compressed_fraction, size_t len, string* dst)
{
...
public:
	...
	void Finish(const Options& options, std::vector<string>* keys, KVMap* kvmap)
	{
		*kvmap = data_;
		keys->clear();
		for (KVMap::const_iterator it = data_.begin(); it != data_.end();
				++it) {
...
static const TestArgs kTestArgList[] = { { TABLE_TEST, 16 }, { TABLE_TEST, 1 },
		{ TABLE_TEST, 1024 }, { BLOCK_TEST, 16 }, { BLOCK_TEST, 1 }, {
				BLOCK_TEST, 1024 }, };
