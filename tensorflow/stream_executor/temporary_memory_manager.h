...
public:
	explicit TemporaryMemoryManager(Stream* stream) :
			stream_(stream)
	{
	}
...
template<typename T>
port::StatusOr<std::unique_ptr<TemporaryDeviceMemory<T>>>TemporaryMemoryManager::AllocateArray(uint64 element_count) {
