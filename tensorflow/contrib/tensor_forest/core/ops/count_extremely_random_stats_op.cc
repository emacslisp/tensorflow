...
void Evaluate(const EvaluateParams& params, int32 start, int32 end)
{
	...
	const auto split_thresholds = params.candidate_split_thresholds.tensor<
			float, 2>();

	const int32 num_splits =
			static_cast<int32>(params.candidate_split_features.shape().dim_size(
					1));
	const int32 num_nodes =
			static_cast<int32>(params.tree_tensor.shape().dim_size(0));
	...
	for (int32 i = start; i < end; ++i) {
		...
		while (true) {
			...
			if (left_child == LEAF_NODE) {
				...
				if (accumulator >= 0
						&& IsAllInitialized(
								params.candidate_split_features.Slice(
										accumulator, accumulator + 1))) {
					CHECK_LT(accumulator, num_accumulators);
					params.results[i].splits_initialized = true;
					for (int split = 0; split < num_splits; split++) {
						const int32 feature = split_features(accumulator,
								split);
						if (!DecideNode(params.dense_input,
								params.sparse_indices, params.sparse_values, i,
								feature, split_thresholds(accumulator, split),
								params.input_spec)) {
			...
			node_index = left_child
					+ DecideNode(params.dense_input, params.sparse_indices,
...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context, context->GetAttr("regression", &regression_));
		string serialized_proto;
		OP_REQUIRES_OK(context,
				context->GetAttr("input_spec", &serialized_proto));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		// Check inputs.
		if (sparse_input) {
			const auto sparse_shape =
					sparse_input_shape.unaligned_flat<int64>();
			...
			OP_REQUIRES(context, sparse_input_shape.shape().dim_size(0) == 2,
					errors::InvalidArgument(
		...
		OP_REQUIRES(context, tree_tensor.shape().dims() == 2,
				errors::InvalidArgument("tree should be two-dimensional"));
		...
		OP_REQUIRES(context,
				candidate_split_features.shape()
						== candidate_split_thresholds.shape(),
				errors::InvalidArgument(
						"candidate_split_features and candidate_split_thresholds should be "
								"the same shape."));
		OP_REQUIRES(context, current_epoch.shape().dim_size(0) == 1,
				errors::InvalidArgument(
						"The current_epoch should be a tensor of shape (1)."));

		// Check tensor bounds.
		if (!CheckTensorBounds(context, input_data))
			return;
		if (!CheckTensorBounds(context, sparse_input_indices))
			return;
		if (!CheckTensorBounds(context, sparse_input_values))
			return;
		if (!CheckTensorBounds(context, sparse_input_shape))
			return;
		if (!CheckTensorBounds(context, input_labels))
			return;
		if (!CheckTensorBounds(context, input_weights))
			return;
		if (!CheckTensorBounds(context, tree_tensor))
			return;
		if (!CheckTensorBounds(context, tree_thresholds))
			return;
		if (!CheckTensorBounds(context, node_to_accumulator))
			return;
		if (!CheckTensorBounds(context, candidate_split_features))
			return;
		if (!CheckTensorBounds(context, candidate_split_thresholds))
			return;
		if (!CheckTensorBounds(context, birth_epochs))
			return;
		if (!CheckTensorBounds(context, current_epoch))
			return;
		...
		auto work =
				[&params, num_data](int64 start, int64 end) {
					CHECK(start <= end);
					CHECK(end <= num_data);
					Evaluate(params, static_cast<int32>(start), static_cast<int32>(end));
				};
		Shard(num_threads, worker_threads->workers, num_data, costPerUnit,
				work);

		const int32 num_nodes = static_cast<int32>(tree_tensor.shape().dim_size(
				0));
		if (regression_) {
			ProcessResultsRegression(context, input_labels, input_weights,
					birth_epochs, epoch, std::move(results), num_nodes);
		} else {
...
protected:
	void ProcessResultsClassification(OpKernelContext* context,
			...
			std::unique_ptr<InputDataResult[]> results, int32 num_nodes)
	{
		const int32 num_data = static_cast<int32>(input_labels.shape().dim_size(
				0));
		...
		auto out_candidate_indices = output_candidate_pcw_indices->tensor<int32,
				2>();
		...
		auto out_candidate =
				output_candidate_pcw_delta->unaligned_flat<float>();
			...
			std::unique_ptr<InputDataResult[]> results, int32 num_nodes)
	{
		...
		auto out_node_squares =
				output_node_pcw_squares_delta->tensor<float, 2>();
		...
		auto out_split_squares =
				output_candidate_pcw_squares->tensor<float, 2>();
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(5, total_pcw_shape,
						&output_total_pcw_indices));
		auto out_total_indices =
				output_total_pcw_indices->unaligned_flat<int32>();
