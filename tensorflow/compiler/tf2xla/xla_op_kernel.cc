		...
		xla::Literal* constant_literal)
{
	return ConstantInputReshaped(index,
			context_->input(index).shape().dim_sizes(), constant_literal);
}

Status XlaOpKernelContext::ConstantInputReshaped(int index,
		gtl::ArraySlice<int64> new_dims, xla::Literal* constant_literal)
{
	const Tensor& tensor = context_->input(index);
	TensorShape new_shape(new_dims);
	if (tensor.NumElements() != new_shape.num_elements()) {
		return errors::InvalidArgument(context_->op_kernel().name(), " input ",
				index, " has shape ", tensor.shape().DebugString(),
				" but was asked to be reshaped to incompatible shape ",
				new_shape.DebugString());
	}
	const XlaExpression* expression = XlaContext::CastExpressionFromTensor(
			tensor);
	...
	if (expression->has_constant_value()) {
		...
		if (!temp.CopyFrom(expression->constant_value(), new_shape)) {
			...
			return errors::Internal(
					"Incompatible shapes in ConstantInputReshaped.");
		}
	...
	if (!computed.ok()) {
		return errors::InvalidArgument("Error evaluating ",
...
Status XlaOpKernelContext::ConstantInputList(StringPiece name,
		...
		const xla::ComputationDataHandle& handle)
{
	...
	OP_REQUIRES_OK(context_,
			context_->allocate_output(index,
					XLAShapeToTensorShape(*shape.ValueOrDie()), &output));
...
void XlaOpKernelContext::CtxFailure(Status s)
{
...
const xla::Computation* XlaOpKernelContext::GetOrCreateMax(const DataType type)
{
	return XlaContext::Get(context_).GetOrCreateMax(type);
}

const xla::Computation* XlaOpKernelContext::GetOrCreateAdd(const DataType type)
{
...
XlaOpKernel::XlaOpKernel(OpKernelConstruction* context) :
		OpKernel(context)
{
}
