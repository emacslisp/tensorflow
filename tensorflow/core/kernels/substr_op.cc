...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (is_scalar || input_shape == pos_shape) {
			...
			BCast bcast(BCast::FromShape(input_shape),
					BCast::FromShape(pos_shape));
			...
			case 1: {
				...
				auto output = output_tensor->shaped<string, 1>(
						bcast.result_shape());
				...
				Tensor input_buffer;
				OP_REQUIRES_OK(context,
						context->allocate_temp(DT_STRING, output_shape,
								&input_buffer));
				...
				for (int i = 0; i < output_shape.dim_size(0); ++i) {
					...
					const T len = tensorflow::internal::SubtleMustCopy(
							len_bcast(i));
					OP_REQUIRES(context,
							FastBoundsCheck(pos, input_bcast(i).size()),
							errors::InvalidArgument("pos ", pos,
									" out of range for string", "b'", in,
									"' at index ", i));
					output(i) = in.substr(pos, len);
			...
			case 2: {
				...
				auto output = output_tensor->shaped<string, 2>(
						bcast.result_shape());
			...
			default: {
				context->SetStatus(
						errors::Unimplemented(
								"Substr broadcast not implemented for ", ndims,
								" dimensions"));
			}
			}
		}
