...
static void Convert8to16(const uint8* p8, int num_comps, int p8_row_bytes,
		int width, int height_in, uint16* p16, int p16_row_bytes)
{
...
{
	DecodeContext* const ctx = bit_cast<DecodeContext*>(
		...
		int desired_channel_bits, DecodeContext* context)
{
	...
	png_get_IHDR(context->png_ptr, context->info_ptr, &context->width,
			&context->height, &context->bit_depth, &context->color_type, 0, 0,
			0);
	...
	context->need_to_synthesize_16 = (context->bit_depth <= 8
			&& desired_channel_bits == 16);
		...
		const std::vector<std::pair<string, string> >* metadata)
{
	...
	if (channel_bits > 8 && port::kLittleEndian)
