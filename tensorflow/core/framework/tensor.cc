...
struct Helper<ResourceHandle> {
	...
	static void Encode(TensorBuffer* in, int64 n, Destination* out)
	{
		port::EncodeResourceHandleList(in->base<const ResourceHandle>(), n,
				out);
	}
...
struct ProtoHelper<ResourceHandle> {
	...
	{
		return reinterpret_cast<const ResourceHandle*>(&(*proto.resource_handle_val().begin()));
	}
	static size_t NumElements(const TensorProto& proto)
	{
...
Tensor::Tensor(DataType type) :
		shape_( { 0 }), buf_(nullptr)
{
	set_dtype(type);
}
	...
	if (N > 0 && proto.dtype()) {
		...
		if (!proto.tensor_content().empty()) {
			...
			CASES_WITH_DEFAULT(proto.dtype(),
					p = Helper<T>::Decode(a, content, N), dtype_error = true,
					dtype_error = true);
		} else {
			CASES_WITH_DEFAULT(proto.dtype(),
					p = FromProtoField<T>(a, proto, N), dtype_error = true,
					dtype_error = true);
		}
		if (dtype_error || p == nullptr)
			return false;
	}
	shape_ = shape;
...
void Tensor::AsProtoTensorContent(TensorProto* proto) const
{
	...
	if (buf_) {
		CASES(dtype(),
				Helper<T>::Encode(buf_, shape_.num_elements(),
...
size_t Tensor::TotalBytes() const
{
	if (shape_.num_elements() == 0)
		return 0;
	CHECK(buf_) << "null buf_ with non-zero shape size "
			<< shape_.num_elements();
