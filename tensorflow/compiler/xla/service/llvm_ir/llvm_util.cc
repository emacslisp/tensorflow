		...
		llvm::IRBuilder<>* ir_builder)
{
	...
	llvm::Function* intrinsic = llvm::Intrinsic::getDeclaration(module,
			intrinsic_id, types);
		...
		llvm::IRBuilder<>* ir_builder)
{
	...
	llvm::PointerType* array_type_as_pointer = llvm::cast < llvm::PointerType
			> (array_type);
	VLOG(2) << "EmitBufferIndexingGEP with type="
			<< llvm_ir::DumpToString(*array_type) << " array="
			<< llvm_ir::DumpToString(*array) << " index="
			<< llvm_ir::DumpToString(*index);

	return ir_builder->CreateInBoundsGEP(
			array_type_as_pointer->getElementType(), array,
			llvm::isa < llvm::GlobalVariable > (array) ?
					llvm::ArrayRef<llvm::Value*>( { ir_builder->getInt64(0),
							index }) :
					index);
}
...
{
	llvm::Type* result_type = PrimitiveTypeToIrType(shape.element_type(),
			ir_builder);
	if (ShapeUtil::IsTuple(shape)) {
		// A tuple buffer is an array of pointers.
		result_type = llvm::ArrayType::get(result_type,
...
llvm::Constant* LiteralToConstant(const Literal& literal, int64 dimension_index,
		std::vector<int64>* multi_index, llvm::IRBuilder<>* ir_builder)
{
	const Shape& shape = literal.shape();
	llvm::Type* ir_element_type = llvm_ir::PrimitiveTypeToIrType(
			shape.element_type(), ir_builder);
	if (dimension_index == -1) {
		...
		case PRED:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<bool>(literal, *multi_index));
			break;
		case U8:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<uint8>(literal, *multi_index));
			break;
		case S32:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<int32>(literal, *multi_index));
			break;
		case U32:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<uint32>(literal, *multi_index));
			break;
		case S64:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<int64>(literal, *multi_index));
			break;
		case U64:
			value = llvm::ConstantInt::get(ir_element_type,
					LiteralUtil::Get<uint64>(literal, *multi_index));
			break;
		case F32:
			value = llvm::ConstantFP::get(ir_element_type,
					LiteralUtil::Get<float>(literal, *multi_index));
			break;
		case F64:
			value = llvm::ConstantFP::get(ir_element_type,
					LiteralUtil::Get<double>(literal, *multi_index));
			break;
		default:
	...
	for (int64 i = 0; i < shape.dimensions(dimension); ++i) {
		...
		elements.push_back(
				LiteralToConstant(literal, dimension_index - 1, multi_index,
						ir_builder));
	}
	...
	if (elements.empty()) {
		...
		for (int i = 0; i < dimension_index; ++i) {
			...
			element_type = llvm::ArrayType::get(element_type,
					shape.dimensions(index));
		}
	} else {
		element_type = elements[0]->getType();
	}
	llvm::ArrayType* aggregate_type = llvm::ArrayType::get(element_type,
			shape.dimensions(dimension));
		...
		llvm::IRBuilder<>* ir_builder)
{
	...
	llvm::Constant* value = LiteralToConstant(literal, /*dimension_index=*/
			ShapeUtil::Rank(literal.shape()) - 1, &multi_index, ir_builder);
	return value;
}
		...
		llvm::IRBuilder<>* ir_builder, int alignment)
{
	...
	llvm::AllocaInst* alloca = ir_builder->CreateAlloca(type, element_count,
			AsStringRef(name));
	if (alignment != 0) {
		...
		llvm::IRBuilder<>* ir_builder, bool emit_else)
{
	...
	if_data.true_block = CreateBasicBlock(nullptr,
			tensorflow::strings::StrCat(name, "-true"), ir_builder);
	if_data.false_block =
			emit_else ?
	...
	ir_builder->CreateCondBr(condition, if_data.true_block,
		...
		llvm::IRBuilder<>* ir_builder)
{
	...
	if (lhs_value->getType()->isIntegerTy()) {
		comparison_result = ir_builder->CreateICmp(predicate, lhs_value,
				rhs_value);
	} else {
		comparison_result = ir_builder->CreateFCmp(predicate, lhs_value,
				rhs_value);
	}
	// comparison_result is i1, but the NVPTX codegen incorrectly lowers i1
	// arrays. So we extend it to i8 so that it's addressable.
	return ir_builder->CreateZExt(comparison_result,
			llvm_ir::PrimitiveTypeToIrType(PRED, ir_builder));
}
		...
		llvm::IRBuilder<>* ir_builder)
{
	llvm::FunctionType* log_function_type = llvm::FunctionType::get(
			...
			{ ir_builder->getInt64Ty(), ir_builder->getInt64Ty() }, /*isVarArg=*/
			false);
	ir_builder->CreateCall(log_function_type,
		...
		bool is_pointer_to)
{
	...
	llvm::MDNode* tbaa_node = metadata_builder.createTBAANode(
			llvm_ir::AsStringRef(type_name), root);
	instruction->setMetadata(llvm::LLVMContext::MD_tbaa,
			metadata_builder.createTBAAStructTagNode(tbaa_node, tbaa_node, /*Offset=*/
					0));
}
...
void SetAlignmentMetadataForLoad(llvm::LoadInst* load, uint64_t alignment)
{
	...
	llvm::Constant* alignment_constant = llvm::ConstantInt::get(int64_ty,
			alignment);
	llvm::MDBuilder metadata_builder(context);
	auto* alignment_metadata = metadata_builder.createConstant(
			alignment_constant);
		...
		uint64_t dereferenceable_bytes)
{
	llvm::LLVMContext& context = load->getContext();
	llvm::Type* int64_ty = llvm::Type::getInt64Ty(context);
	llvm::Constant* dereferenceable_bytes_constant = llvm::ConstantInt::get(
			int64_ty, dereferenceable_bytes);
	llvm::MDBuilder metadata_builder(context);
	auto* dereferenceable_bytes_metadata = metadata_builder.createConstant(
	...
	inst->setMetadata(llvm::LLVMContext::MD_range,
			llvm::MDNode::get(context,
					{ llvm::ConstantAsMetadata::get(
							llvm::ConstantInt::get(i32, lower)),
							llvm::ConstantAsMetadata::get(
									llvm::ConstantInt::get(i32, upper)) }));
	return inst;
}
		...
		llvm::IRBuilder<>* builder)
{
	...
	return builder->CreateOr(
			builder->CreateShl(rotand,
					mod(builder->CreateSub(size_value, rotor))),
