...
/* static */StatusOr<std::vector<se::Platform*>> PlatformUtil::GetSupportedPlatforms()
{
	...
	if (platform_map.empty()) {
		LOG(WARNING)
				<< "no executor platforms available: platform map is empty";
	}
	...
	for (auto& platform_pair : platform_map) {
		...
		if (compiler_status.ok()) {
			if (platform->VisibleDeviceCount() > 0) {
				...
				LOG(WARNING) << "platform " << platform->Name()
						<< " present but no " << "visible devices found";
			}
...
static bool IsDeviceSupported(se::StreamExecutor* executor)
{
	...
	if (executor->platform()->id() == se::cuda::kCudaPlatformId) {
		...
		if (description.cuda_compute_capability(&major_version,
				&minor_version)) {
...
/* static */StatusOr<std::vector<se::StreamExecutor*>> PlatformUtil::GetStreamExecutors(
