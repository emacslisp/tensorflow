...
protected:
	...
	std::unique_ptr<HloComputation> BuildMapComputationPlus1(const string& name)
	{
		...
		builder.AddInstruction(
				HloInstruction::CreateBinary(r0f32_, HloOpcode::kAdd, param,
						value));
  ...
  //   param[(s32,f32[4])] --- get-tuple-element[0] --- less-than
			...
			const string& name)
	{
		...
		auto index = builder.AddInstruction(
				HloInstruction::CreateGetTupleElement(const4->shape(), param,
						0));
		builder.AddInstruction(
				HloInstruction::CreateBinary(r0f32_, HloOpcode::kLt, index,
						const4));
		return builder.Build();
	}
  ...
  //   param[(s32,f32[4])] ---|                                    tuple
			...
			const string& name)
	{
		...
		auto indexc = builder.AddInstruction(
				HloInstruction::CreateGetTupleElement(const1->shape(), param,
						0));
		auto addc = builder.AddInstruction(
				HloInstruction::CreateBinary(indexc->shape(), HloOpcode::kAdd,
						indexc, const1));
		auto indexv = builder.AddInstruction(
				HloInstruction::CreateGetTupleElement(constv->shape(), param,
						1));
		auto addv = builder.AddInstruction(
				HloInstruction::CreateBinary(constv->shape(), HloOpcode::kAdd,
						indexv, constv));
			...
			const BufferAssignment& buffers, HloInstruction* hlo)
	{
		...
		const BufferAllocation& buffer = *buffers.GetUniqueTopLevelAllocation(
				hlo).ConsumeValueOrDie();
	...
	const BufferAllocation& GetTopLevelAllocation(
			const BufferAssignment& buffers, const HloInstruction* hlo)
	{
			...
			const BufferAssignment& buffers)
	{
		...
		for (const HloInstruction* hlo : instructions) {
			if (!buffers.HasTopLevelAllocation(hlo)) {
				...
				// parameter.
				EXPECT_TRUE(
						HloOpcode::kConstant == hlo->opcode()
			...
			const BufferAssignment& assignment)
	{
		...
		for (const HloInstruction* instruction : a) {
			if (assignment.HasTopLevelAllocation(instruction)) {
				a_buffers.insert(
						assignment.GetUniqueTopLevelAllocation(instruction).ConsumeValueOrDie()->index());
			}
		}
...
std::unique_ptr<BufferAssignment> RunBufferAssignment(HloModule* module)
{
	return BufferAssigner::Run(module,
			...
			/*pointer_size=*/sizeof(void*)).ConsumeValueOrDie();
}
}
