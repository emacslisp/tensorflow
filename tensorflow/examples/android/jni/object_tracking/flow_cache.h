...
public:
	explicit FlowCache(const OpticalFlowConfig* const config) :
			config_(config), image_size_(config->image_size), optical_flow_(
					config), fullframe_matrix_(NULL)
	{
			...
			float* const flow_y) const
	{
		...
		*flow_y = guess_from_cache.y;
		...
		for (int pyramid_level = kMinNumPyramidLevelsToUseForAdjustment - 1;
			...
			if (!optical_flow_.FindFlowAtPointSingleLevel(pyramid_level, u_x,
					u_y, false, flow_x, flow_y)) {
				return false;
			}
		}
			...
			const int grid_height) const
	{
		...
		const BoundingBox valid_box = bounding_box.Intersect(
				BoundingBox(0, 0, image_size_.width - 1,
...
private:
			...
			const float y) const
	{
		...
		if (fullframe_matrix_ != NULL && cache_level == kCacheCutoff) {
			const float xnew = x * fullframe_matrix_[0]
					+ y * fullframe_matrix_[1] + fullframe_matrix_[2];
			const float ynew = x * fullframe_matrix_[3]
					+ y * fullframe_matrix_[4] + fullframe_matrix_[5];

			return Point2f(xnew - x, ynew - y);
		}

		const int level_dim = BlockDimForCacheLevel(cache_level);
		const int pixels_per_cache_block_x = (image_size_.width + level_dim - 1)
				/ level_dim;
		...
		if (!(*has_cache_[cache_level])[index_y][index_x]) {
			...
			if (!success) {
				LOGV("Computation of cached value failed for level %d!",
						cache_level);
			}
	...
	{
		if (x < 0 || x >= image_size_.width || y < 0
				|| y >= image_size_.height) {
			return Point2f(0, 0);
		}
