...
public:
	explicit BestSplits(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (regression_) {
			OP_REQUIRES(context, split_sums.shape() == split_squares.shape(),
					errors::InvalidArgument(
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, finished.shape(), &output_splits));
		...
		std::function<int32(int32)> best_feature_func = std::bind(
				BestFeatureClassification, accumulator_sums, split_sums, _1);
		if (regression_) {
			best_feature_func = std::bind(BestFeatureRegression,
		...
		for (int32 i = 0; i < num_finished; i++) {
			...
			if (accumulator < 0) {
				LOG(ERROR)
						<< "Something has gone wrong, we got a finished node that "
