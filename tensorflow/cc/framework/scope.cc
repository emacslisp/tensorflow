...
Scope::Scope(Graph* graph, Status* status, Scope::NameMap* name_map,
		...
		graph_(graph), status_(status), name_map_(name_map), refiner_(refiner), scope_used_(
				nullptr), colocation_constraints_()
{
}
...
Scope Scope::NewRootScope()
{
	...
	ShapeRefiner* refiner = new ShapeRefiner(graph->versions().producer(),
			graph->op_registry());
...
Scope::Scope(const Scope& other, Scope::Tags::ScopeName, const string& name,
		bool copy_names) :
		graph_(other.graph_), status_(other.status_), name_map_(
				copy_names ?
						other.name_map_ :
						std::shared_ptr<NameMap>(new NameMap)), refiner_(
				other.refiner_), scope_used_(nullptr), control_deps_(
				other.control_deps_), name_(name), op_name_(""), exit_on_error_(
				other.exit_on_error_), kernel_label_(other.kernel_label_), device_(
				other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
}
...
Scope::Scope(const Scope& other, Scope::Tags::OpName, const string& name,
		...
		graph_(other.graph_), status_(other.status_), name_map_(
				other.name_map_), refiner_(other.refiner_), scope_used_(
				other.scope_used_), control_deps_(other.control_deps_), name_(
				name), op_name_(op_name), exit_on_error_(other.exit_on_error_), kernel_label_(
				other.kernel_label_), device_(other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
}
...
Scope::Scope(const Scope& other, Scope::Tags::ControlDeps,
		...
		graph_(other.graph_), status_(other.status_), name_map_(
				other.name_map_), refiner_(other.refiner_), scope_used_(
				other.scope_used_), control_deps_(
				clear_control_deps ?
						std::vector<Operation>() :
						(control_deps.insert(control_deps.begin(),
								other.control_deps_.begin(),
								other.control_deps_.end()), control_deps)), name_(
				other.name_), op_name_(other.op_name_), exit_on_error_(
				other.exit_on_error_), kernel_label_(other.kernel_label_), device_(
				other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
...
Scope::Scope(const Scope& other, Scope::Tags::Device, const string& device) :
		graph_(other.graph_), status_(other.status_), name_map_(
				...
				other.name_), op_name_(other.op_name_), exit_on_error_(
				other.exit_on_error_), kernel_label_(other.kernel_label_), device_(
				device), colocation_constraints_(other.colocation_constraints_)
{
}
...
Scope::Scope(const Scope& other, Scope::Tags::SingleUseScope,
		...
		graph_(other.graph_), status_(other.status_), name_map_(
				other.name_map_), refiner_(other.refiner_), scope_used_(
				new bool(false)), control_deps_(other.control_deps_), name_(
				other.name_), op_name_(op_name), exit_on_error_(
				other.exit_on_error_), kernel_label_(other.kernel_label_), device_(
				other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
...
Scope::Scope(const Scope& other, Scope::Tags::ExitOnError) :
		graph_(other.graph_), status_(other.status_), name_map_(
				other.name_map_), refiner_(other.refiner_), scope_used_(
				other.scope_used_), control_deps_(other.control_deps_), name_(
				other.name_), op_name_(other.op_name_), exit_on_error_(true), kernel_label_(
				other.kernel_label_), device_(other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
}
...
Scope::Scope(const Scope& other, Scope::Tags::KernelLabel,
		...
		graph_(other.graph_), status_(other.status_), name_map_(
				...
				other.scope_used_), control_deps_(other.control_deps_), name_(
				other.name_), op_name_(other.op_name_), exit_on_error_(
				other.exit_on_error_), kernel_label_(kernel_label), device_(
				other.device_), colocation_constraints_(
				other.colocation_constraints_)
{
}
...
void Scope::UpdateBuilder(NodeBuilder* builder) const
{
	...
	if (!colocation_constraints_.empty()) {
		...
		std::transform(constraints.begin(), constraints.end(),
				constraints.begin(), [](const string& s) {
...
string Scope::GetUniqueName(const string& prefix, bool check_single_use) const
{
	if (check_single_use && single_use_scope()) {
		if (*scope_used_) {
			*status_ = errors::AlreadyExists(prefix,
...
{
	const string unique_name = GetUniqueName(default_name,
			true /* check_single_use */);
...
string Scope::GetUniqueNameForOp(const string& default_name) const
{
	if (single_use_scope()) {
		if (op_name_.empty() || *scope_used_) {
			*status_ = errors::InvalidArgument(
					"Cannot get a unique name in this scope");
			return "";
		}
		*scope_used_ = true;
		return op_name_;
	...
	return op_name_.empty() ?
			GetNameForOp(default_name) : GetNameForOp(op_name_);
}

Scope Scope::NewSubScope(const string& child_scope_name) const
{
	if (child_scope_name.empty()) {
		return Scope(*this, Scope::Tags::ScopeName(), name_,
				true /* copy_names */);
	}
	const string unique_name = GetUniqueName(child_scope_name,
			false /* check_single_use */);
	const string sep = name_.empty() || unique_name.empty() ? "" : "/";
	return Scope(*this, Scope::Tags::ScopeName(),
			strings::StrCat(name_, sep, unique_name), false /* copy_names */);
}
...
Scope Scope::WithOpName(const string& op_name) const
{
	if (single_use_scope()) {
		UpdateStatus(
				errors::InvalidArgument("Cannot set op name ", op_name,
						" on this scope"));
		return *this;
	}
		...
		const string& composite_op_name) const
{
	...
	if (!single_use_scope()) {
		Scope child = NewSubScope(
				op_name_.empty() ? composite_op_name : op_name_);
