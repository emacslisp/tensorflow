...
private:
	...
	mutable mutex mu_;
	DataType elem_type_;
	Tensor handle_;bool closed_ GUARDED_BY(mu_);std::vector<TensorAndAllocation> stack_ GUARDED_BY(mu_);
...
// ensures that correct garbage collection on error or successful completion.
	...
	public:
				...
				OpKernel(context)
		{
			...
			OP_REQUIRES_OK(context,
					context->GetAttr("stack_name", &stack_name_));
			if (stack_name_ == "")
				stack_name_ = name();
		}
		...
		void Compute(OpKernelContext* ctx) override
		{
			...
			OP_REQUIRES_OK(ctx,
					ctx->allocate_temp(tensorflow::DT_STRING,
							tensorflow::TensorShape( { 2 }), &stack_handle,
							alloc_attr));
	...
	public:
		explicit StackPushOp(OpKernelConstruction* context) :
				AsyncOpKernel(context)
		{
			OP_REQUIRES_OK(context,
					context->GetAttr("swap_memory", &swap_memory_));
		}
		...
		void ComputeAsync(OpKernelContext* ctx, DoneCallback done) override
		{
			...
			if (ctx->input_dtype(1) != stack->ElemType()) {
				ctx->CtxFailure(
						errors::InvalidArgument("Must have type ",
			...
			if (swap_memory_ && !alloc_attrs.on_host()
				...
				if (stats.bytes_in_use > (stats.bytes_limit * kOccupancy)) {
					...
					Allocator* cpu_allocator = device->GetAllocator(
							host_alloc_attrs);
					Tensor* cpu_tensor = new Tensor(cpu_allocator,
							tensor.dtype(), tensor.shape());
					device_ctxt->CopyDeviceTensorToCPU(&tensor, "StackPush",
							device, cpu_tensor,
			...
			OP_REQUIRES_OK_ASYNC(ctx,
					stack->Push( { tensor, alloc_attrs, false }), done);
	...
	public:
		...
		void ComputeAsync(OpKernelContext* ctx, DoneCallback done) override
		{
			...
			if (value.swapped_to_cpu) {
				...
				Allocator* gpu_allocator = device->GetAllocator(
						value.alloc_attrs);
				Tensor* device_tensor = new Tensor(gpu_allocator,
						cpu_tensor->dtype(), cpu_tensor->shape());
				device_ctxt->CopyCPUTensorToDevice(cpu_tensor, device,
						device_tensor,
