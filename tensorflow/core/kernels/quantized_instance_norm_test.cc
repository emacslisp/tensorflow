		...
		const TensorShape& shape, float var_eps, float* out)
{
	...
	for (int n = 0; n < N; ++n) {
		for (int c = 0; c < C; ++c) {
			float sum = 0.0;
			for (int i = 0; i < H * W; ++i) {
				float tmp = dequantized[n * H * W * C + i * C + c]
						- inp_mean[n * C + c];
				sum += tmp * tmp;
			}
	...
	for (int n = 0; n < N; ++n) {
		for (int c = 0; c < C; ++c) {
			for (int i = 0; i < H * W; ++i) {
				out[n * H * W * C + i * C + c] = (dequantized[n * H * W * C
						+ i * C + c] - inp_mean[n * C + c])
		...
		bool output_range_given, float give_y_min, float given_y_max)
{
	...
	auto instance_norm = QuantizedInstanceNorm(root, input_ph, x_min, x_max,
	...
	Eigen::Tensor<float, 0, Eigen::RowMajor> max_diff = (expected.flat<float>()
			- (out_min + out_scale * out.cast<float>())).abs().maximum();
...
void TestMaxInput()
{
	...
	Expect(input_tensor, 0.0f,
			std::numeric_limits<float>::max() / static_cast<float>(2 * 16),
			false, 0.0f, 0.0f);
}
