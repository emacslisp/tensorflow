...
protected:
	void TestMapNamesToNodes()
	{
		...
		Output b_const = Const(root.WithOpName("b"),
				Input::Initializer(b_data));
	...
	void TestNodeNamePartsFromInput()
	{
		...
		NodeNamePartsFromInput("some_node_name/with/slashes", &prefix,
				&node_name, &suffix);
		EXPECT_EQ("", prefix);
		EXPECT_EQ("some_node_name/with/slashes", node_name);
		EXPECT_EQ("", suffix);

		NodeNamePartsFromInput("some_node_name:0", &prefix, &node_name,
				&suffix);
		EXPECT_EQ("", prefix);
		EXPECT_EQ("some_node_name", node_name);
		EXPECT_EQ(":0", suffix);
		...
		NodeNamePartsFromInput("^some_node_name:99", &prefix, &node_name,
				&suffix);
		EXPECT_EQ("^", prefix);
		EXPECT_EQ("some_node_name", node_name);
		EXPECT_EQ(":99", suffix);
	...
	void TestGetOpTypeMatches()
	{
		...
		for (const NodeMatch& match : const_matches) {
			...
			EXPECT_TRUE(
					("a" == match.node.name()) || ("b" == match.node.name()))
		...
		TF_ASSERT_OK(matcher.GetOpTypeMatches( { "Add", { { "Const" },
				{ "Const" } } }, &add_child_matches));
		...
		TF_ASSERT_OK(
				matcher.GetOpTypeMatches( { "Mul", { { "Add", { { "Const" }, {
						"Const" } } }, { "Placeholder" } } }, &all_matches));
	...
	void TestReplaceMatchingOpTypes()
	{
		...
		TF_ASSERT_OK(
				ReplaceMatchingOpTypes(graph_def, { "*" },
		...
		for (const NodeDef& node : replaced_graph_def.node()) {
			if (node.name() == "output") {
				...
				EXPECT_EQ(true, false) << "Unexpected node name found: "
						<< node.name();
			}
		}
	}
	...
	void TestRenameNodeInputs()
	{
		...
		TF_ASSERT_OK(
				RenameNodeInputs(graph_def, { { "a", "b" } },
	...
	void TestRenameNodeInputsWithRedirects()
	{
		...
		Output c_const = Const(root.WithOpName("c"),
				Input::Initializer(c_data));
		...
		GraphDef renamed_graph_def;
		TF_ASSERT_OK(
				RenameNodeInputs(graph_def, { { "a", "f" }, { "f", "e" }, { "e",
						"d" }, { "d", "c" } }, std::unordered_set<string>(),
						&renamed_graph_def));
	...
	void TestRenameNodeInputsWithCycle()
	{
		...
		GraphDef renamed_graph_def;
		Status rename_status = RenameNodeInputs(graph_def, { { "a", "d" }, {
				"d", "a" } }, std::unordered_set<string>(), &renamed_graph_def);
	...
	void TestRenameNodeInputsWithWildcard()
	{
		...
		QuantizeV2 quantize_a(root.WithOpName("quantize_a"), a_const, a_const,
				a_const, DT_QUINT8, QuantizeV2::Attrs().Mode("MIN_FIRST"));
		...
		QuantizeV2 quantize_b(root.WithOpName("quantize_b"), b_const, b_const,
				b_const, DT_QUINT8, QuantizeV2::Attrs().Mode("MIN_FIRST"));
		...
		GraphDef renamed_graph_def;
		TF_ASSERT_OK(
				RenameNodeInputs(graph_def,
						{ { "quantize_a:*", "quantize_b" } },
						std::unordered_set<string>(), &renamed_graph_def));
	...
	void TestRenameNodeInputsWithIgnores()
	{
		...
		GraphDef renamed_graph_def;
		TF_ASSERT_OK(
				RenameNodeInputs(graph_def, { { "a", "b" } }, { "add2" },
						&renamed_graph_def));
	...
	void TestFindInvalidInputs()
	{
		...
		*(add_node2->mutable_input()->Add()) = "^const_node2";
		...
		for (const std::pair<string, string>& invalid_input : invalid_inputs) {
			EXPECT_TRUE(
					(invalid_input.first == "add_node1")
							|| (invalid_input.first == "add_node2"));
			if (invalid_input.first == "add_node1") {
				EXPECT_TRUE(
						(invalid_input.second == "missing_input1")
	...
	void TestGetInOutTypes()
	{
		...
		Output float_const = Const(root.WithOpName("float_const"),
				Input::Initializer(float_data));

		Tensor int_data(DT_INT32, TensorShape( { width }));
		test::FillIota<int32>(&int_data, 1);
		Output int_const = Const(root.WithOpName("int_const"),
				Input::Initializer(int_data));
		...
		TF_EXPECT_OK(
				GetInOutTypes(*float_const_def, &float_const_inputs,
						&float_const_outputs));
		...
		TF_EXPECT_OK(
				GetInOutTypes(*int_const_def, &int_const_inputs,
						&int_const_outputs));
		...
		TF_EXPECT_OK(
				GetInOutTypes(*float_relu_def, &float_relu_inputs,
						&float_relu_outputs));
		...
		TF_EXPECT_OK(
				GetInOutTypes(*int_relu_def, &int_relu_inputs,
						&int_relu_outputs));
	...
	void TestHashNodeDef()
	{
		...
		Tensor a_data(DT_FLOAT, TensorShape( { width }));
		test::FillIota<float>(&a_data, 1.0f);
		Output a_const = Const(a_root.WithOpName("a"),
				Input::Initializer(a_data));
		...
		Tensor b_data(DT_FLOAT, TensorShape( { width }));
		test::FillIota<float>(&b_data, 1.0f);
		Output b_const = Const(b_root.WithOpName("a"),
				Input::Initializer(b_data));
		...
		Output c_const = Const(c_root.WithOpName("a"),
				Input::Initializer(c_data));
		...
		Output d_const = Const(d_root.WithOpName("d"),
				Input::Initializer(d_data));
		...
		Output e_const = Const(e_root.WithOpName("a"),
				Input::Initializer(e_data));
		...
		Output f_const = Const(f_root.WithOpName("a"),
				Input::Initializer(f_data));
	...
	void TestGetOneInt32Parameter()
	{
		...
		EXPECT_EQ(10, value);
		EXPECT_FALSE(
				context.GetOneInt32Parameter("not_a_number", 0, &value).ok());
	...
	void TestGetOneInt64Parameter()
	{
		...
		EXPECT_EQ(10, value);
		EXPECT_FALSE(
				context.GetOneInt64Parameter("not_a_number", 0, &value).ok());
	...
	void TestGetOneFloatParameter()
	{
		...
		TF_EXPECT_OK(
				context.GetOneFloatParameter("not_present", 10.5f, &value));
		EXPECT_NEAR(10.5f, value, 1e-5f);
		EXPECT_FALSE(
				context.GetOneFloatParameter("not_a_number", 0, &value).ok());
	}
	...
	void TestGetOneBoolParameter()
	{
		...
		EXPECT_FALSE(
				context.GetOneBoolParameter("not_a_bool", false, &value).ok());
	...
	void TestLoadTextOrBinaryGraphFile()
	{
		...
		const string text_file = io::JoinPath(testing::TmpDir(),
				"text_graph.pbtxt");
		TF_ASSERT_OK(WriteTextProto(Env::Default(), text_file, graph_def));

		const string binary_file = io::JoinPath(testing::TmpDir(),
				"binary_graph.pb");
		TF_ASSERT_OK(WriteBinaryProto(Env::Default(), binary_file, graph_def));

		const string bogus_file = io::JoinPath(testing::TmpDir(),
				"bogus_graph.pb");
		TF_ASSERT_OK(
				WriteStringToFile(Env::Default(), bogus_file,
						"Not a !{ proto..."));
		...
		EXPECT_FALSE(
				LoadTextOrBinaryGraphFile("____non_existent_file_____",
						&no_graph_def).ok());

		GraphDef bogus_graph_def;
		EXPECT_FALSE(
				LoadTextOrBinaryGraphFile(bogus_file, &bogus_graph_def).ok());
	}
};
