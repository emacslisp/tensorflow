...
void MemoryTypesHelper(const NameRangeMap& name_map,
		std::vector<string>* host_memory_args, MemoryTypeVector* memory_types)
{
		...
		MemoryTypeVector* inp_mtypes, MemoryTypeVector* out_mtypes)
{
	...
	Status status = FindKernelDef(device_type, ndef, &kdef,
	...
	if (!host_memory_args.empty()) {
		return errors::InvalidArgument("HostMemory args '",
				str_util::Join(host_memory_args, "', '"),
