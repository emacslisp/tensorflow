...
void GetTwoBest(int max, std::function<float(int)> score_fn, float* best_score,
		int* best_index, float* second_best_score, int* second_best_index)
{
	*best_index = -1;
	*second_best_index = -1;
	*best_score = FLT_MAX;
...
void GetTwoBestClassification(const Tensor& total_counts,
		const Tensor& split_counts, int32 accumulator, float* best_score,
		int* best_index, float* second_best_score, int* second_best_index)
{
	const int32 num_splits =
			static_cast<int32>(split_counts.shape().dim_size(1));
	const int32 num_classes = static_cast<int32>(split_counts.shape().dim_size(
			2)) - 1;
	...
	const auto tc =
			total_counts.Slice(accumulator, accumulator + 1).unaligned_flat<
					float>();
	...
	bcast[0] = num_splits;
	const Eigen::Tensor<float, 1, Eigen::RowMajor> rights = tc.broadcast(bcast)
			- splits;

	std::function<float(int)> score_fn = std::bind(ClassificationSplitScore,
			splits, rights, num_classes, std::placeholders::_1);
		...
		const Tensor& split_counts, int32 accumulator)
{
	...
	GetTwoBestClassification(total_counts, split_counts, accumulator,
		...
		int32 accumulator, int32 num_regression_dims, int i)
{
	...
	if (left_count > 0) {
		score += WeightedVariance(splits_sum.slice(offsets, extents),
		...
		float* second_best_score, int* second_best_index)
{
	...
	const auto tc_square =
			total_squares.Slice(accumulator, accumulator + 1).unaligned_flat<
					float>();
	const auto splits_sum =
			split_sums.Slice(accumulator, accumulator + 1).unaligned_flat<float>();
	const auto splits_square =
			split_squares.Slice(accumulator, accumulator + 1).unaligned_flat<
					float>();
	...
	GetTwoBest(num_splits,
			std::bind(RegressionSplitScore, splits_count_accessor,
					...
					right_sums, right_squares, accumulator, num_regression_dims,
					std::placeholders::_1), best_score, best_index,
			second_best_score, second_best_index);
}

int32 BestFeatureRegression(const Tensor& total_sums,
		const Tensor& total_squares, const Tensor& split_sums,
		const Tensor& split_squares, int32 accumulator)
{
	...
	GetTwoBestRegression(total_sums, total_squares, split_sums, split_squares,
			accumulator, &best_score, &best_feature_index, &second_best_score,
			&second_best_index);
	return best_feature_index;
}
...
int MakeBootstrapWeights(const Tensor& total_counts, const Tensor& split_counts,
		int32 accumulator, int index, std::vector<float>* weights)
{
	const int32 num_classes = static_cast<int32>(split_counts.shape().dim_size(
			2)) - 1;
...
bool BestSplitDominatesClassificationBootstrap(const Tensor& total_counts,
		const Tensor& split_counts, int32 accumulator, float dominate_fraction,
		random::SimplePhilox* rand)
{
	float best_score;
	float second_best_score;
		...
		const Tensor& split_counts, int32 accumulator, float dominate_fraction)
{
	...
	float hoeffding_bound = range
			* sqrt(log(1.0 / (1.0 - dominate_fraction)) / (2.0 * n));
...
double DirichletCovarianceTrace(const Tensor& total_counts,
		const Tensor& split_counts, int32 accumulator, int index)
{
	const int32 num_classes = static_cast<int32>(split_counts.shape().dim_size(
			2)) - 1;
		...
		const Tensor& split_counts, int32 accumulator, float dominate_fraction)
{
	...
	double trace = DirichletCovarianceTrace(total_counts, split_counts,
			...
			+ DirichletCovarianceTrace(total_counts, split_counts, accumulator,
					second_best_index);

	std::vector<float> mu1;
	getDirichletMean(total_counts, split_counts, accumulator,
			best_feature_index, &mu1);
...
bool DecideNode(const Tensor& dense_features,
		const Tensor& sparse_input_indices, const Tensor& sparse_input_values,
...
void GetParentWeightedMean(float leaf_sum, const float* leaf_data,
		float parent_sum, const float* parent_data, float valid_leaf_threshold,
		int num_outputs, std::vector<float>* mean)
{
	...
	if (leaf_sum < valid_leaf_threshold && parent_sum >= 0) {
		...
		parent_weight = std::min(1.0f,
				(valid_leaf_threshold - leaf_sum) / parent_sum);
