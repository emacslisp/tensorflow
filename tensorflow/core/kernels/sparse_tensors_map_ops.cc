...
public:
	SparseTensorsMap(const string& name) :
			...
			int64* handle)
	{
		...
		Tensor* ix;
		TF_RETURN_IF_ERROR(
				ctx->allocate_persistent(sp.indices().dtype(),
						sp.indices().shape(), &persistent_ix, &ix));
		*ix = sp.indices();
		...
		TF_RETURN_IF_ERROR(
				ctx->allocate_persistent(sp.indices().dtype(),
						sp.indices().shape(), &persistent_values, &values));
		*values = sp.values();
		{
			mutex_lock l(mu_);
			int64 unique_st_handle = counter_++; // increment is guarded on purpose
			sp_tensors_[unique_st_handle] = PersistentSparseTensor {
					persistent_ix, persistent_values, sp.shape() };
			*handle = unique_st_handle;
		}
		return Status::OK();
	}

	Status RetrieveAndClearSparseTensors(OpKernelContext* ctx,
			const TTypes<int64>::ConstVec& handles,
			...
			for (size_t i = 0; i < handles.size(); ++i) {
				...
				if (sp_iter == sp_tensors_.end()) {
					return errors::InvalidArgument(
							"Unable to find SparseTensor: ", handle,
							" in map: ", name_);
				}
...
private:
	...
	mutex mu_;int64
	counter_ GUARDED_BY(mu_);std::unordered_map<int64, PersistentSparseTensor> sp_tensors_ GUARDED_BY(mu_);
};
			...
			SparseTensorsMap** sparse_tensors_map)
	{
		...
		TF_RETURN_IF_ERROR(
				cinfo_.resource_manager()->LookupOrCreate < SparseTensorsMap
						> (cinfo_.container(), cinfo_.name(), &sparse_tensors_map_, sparse_tensors_map_creator));
...
private:
	...
	mutex mu_;SparseTensorsMap* sparse_tensors_map_ PT_GUARDED_BY(mu_);
};
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->input("sparse_indices", &input_indices));
		...
		OP_REQUIRES_OK(context,
				TensorShapeUtils::MakeShape(input_shape->vec<int64>().data(),
						input_shape->NumElements(), &input_shape_object));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, rank > 1,
				errors::InvalidArgument(
						"Rank of input SparseTensor should be > 1, but saw rank: ",
						rank));
		...
		OP_REQUIRES_OK(context,
				TensorShapeUtils::MakeShape(input_shape_t.data() + 1,
		...
		for (const auto& subset : minibatch) {
			...
			visited.insert(b);
			OP_REQUIRES(context, b > -1 && b < N,
					errors::InvalidArgument(
							"Received unexpected column 0 value in input SparseTensor: ",
							b, " < 0 or >= N (= ", N, ")"));
			...
			Tensor output_values = Tensor(DataTypeToEnum<T>::value, {
					num_entries });
			...
			int64 handle;
			OP_REQUIRES_OK(context,
					map->AddSparseTensor(context, st_i, &handle));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				map->RetrieveAndClearSparseTensors(context, sparse_handles_t,
						&sparse_tensors));
		...
		for (int64 i = 0; i < N; ++i) {
			...
			Tensor expanded_indices(DT_INT64,
					TensorShape(
							{ num_entries, 1 + output_indices.dim_size(1) }));
			...
			Eigen::DSizes < Eigen::DenseIndex, 2
					> indices_sizes(num_entries, rank);
		...
		for (int i = 0; i < N; ++i) {
			if (rank < 0)
				rank = shapes_to_concat[i].dims();
			OP_REQUIRES(context, rank == shapes_to_concat[i].dims(),
					errors::InvalidArgument(
							"Inconsistent rank across SparseTensors: rank prior to "
									"SparseTensor[", i, "] was: ", rank,
							" but rank of SparseTensor[", i, "] is: ",
							shapes_to_concat[i].dims()));
		}
		...
		for (int i = 0; i < N; ++i) {
			for (int d = 0; d < rank; ++d) {
				preconcat_shape.set_dim(d,
						std::max(preconcat_shape.dim_size(d),
		...
		for (int i = 0; i < N; ++i) {
			tensors_to_concat.emplace_back(std::move(indices_to_concat[i]),
					std::move(values_to_concat[i]), preconcat_shape, std_order);
		}
