...
std::unique_ptr<Device> GetCPUDevice(Env* env)
{
	...
	Status s = DeviceFactory::GetFactory(DEVICE_CPU)->CreateDevices(
...
public:
	...
	Status Send(const ParsedKey& parsed, const Args& send_args,
		...
		const std::vector<string>& output_names, std::vector<Tensor>* outputs)
{
	...
	for (const auto& in : inputs) {
		...
		string full_key = Rendezvous::CreateKey("/cpu:0", 1, "/cpu:1",
				tensor_name, FrameAndIter(0, 0));
		Rendezvous::ParsedKey parsed;
		TF_RETURN_IF_ERROR(Rendezvous::ParseKey(full_key, &parsed));
		TF_RETURN_IF_ERROR(
				rendez->Send(parsed, Rendezvous::Args(), in.second,
						false /* is_dead */));
	}

	// Call RewriteGraphForExecution
	TF_RETURN_IF_ERROR(
			subgraph::RewriteGraphForExecution(graph_to_run.get(), input_names,
					output_names, { } /* target nodes */,
					device->attributes()));
	...
	params.create_kernel =
			[&device, g](const NodeDef& ndef, OpKernel** kernel) {
	...
	for (size_t i = 0; i < output_names.size(); ++i) {
		const string& output_key = Rendezvous::CreateKey("/cpu:0", 1, "/cpu:1",
				output_names[i], FrameAndIter(0, 0));
		Rendezvous::ParsedKey parsed;
		TF_RETURN_IF_ERROR(Rendezvous::ParseKey(output_key, &parsed));
		bool is_dead;
		TF_RETURN_IF_ERROR(
				rendez->Recv(parsed, Rendezvous::Args(), &(*outputs)[i],
						&is_dead));
	}
