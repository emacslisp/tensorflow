...
/* static */Literal LiteralUtil::MinValue(PrimitiveType primitive_type)
{
	...
	case U32:
		return *LiteralUtil::CreateR0<uint32>(
				std::numeric_limits<uint32>::min());
	case U64:
		return *LiteralUtil::CreateR0<uint64>(
...
{
	auto literal = MakeUnique<Literal>();
	*literal->mutable_shape() = ShapeUtil::MakeShape(U8, {
...
/* static */std::unique_ptr<Literal> LiteralUtil::Slice(const Literal& literal,
		...
		tensorflow::gtl::ArraySlice<int64> limit_indices)
{
	...
	*result_literal->mutable_shape() = result_shape;
	...
	case F32:
		LiteralUtil::EachCell<float>(*result_literal,
				[&](tensorflow::gtl::ArraySlice<int64> indices, float /*value*/) {
	...
	case S32:
		LiteralUtil::EachCell<int32>(*result_literal,
				[&](tensorflow::gtl::ArraySlice<int64> indices, int32 /*value*/) {
					for (int64 i = 0; i < ShapeUtil::Rank(result_shape); ++i) {
						new_indices[i] = indices[i] + start_indices[i];
					}
		...
		return result_literal;
	case U32:
		LiteralUtil::EachCell<uint32>(*result_literal,
				[&](tensorflow::gtl::ArraySlice<int64> indices, uint32 /*value*/) {
					for (int64 i = 0; i < ShapeUtil::Rank(result_shape); ++i) {
						new_indices[i] = indices[i] + start_indices[i];
					}
...
/* static */string LiteralUtil::GetAsString(const Literal& literal,
...
/* static */int64 LiteralUtil::LinearIndex(const Literal& literal,
		tensorflow::gtl::ArraySlice<int64> multi_index)
{
...
/* static */string LiteralUtil::ToString(const Literal& literal)
{
	...
	if (ShapeUtil::IsTuple(shape)) {
		...
		for (int64 i0 = 0; i0 < shape.dimensions(0); ++i0) {
			pieces.push_back(
					tensorflow::strings::Printf("  {  // i0=%lld\n", i0));
			for (int64 i1 = 0; i1 < shape.dimensions(1); ++i1) {
				pieces.push_back(
...
/* static */void LiteralUtil::EachCellAsString(const Literal& literal,
		std::function<
				void(tensorflow::gtl::ArraySlice<int64> indices,
						const string& value)> per_cell)
{
	...
	if (ShapeUtil::Rank(literal.shape()) == 4) {
		for (int64 i0 = 0; i0 < literal.shape().dimensions(0); ++i0) {
			for (int64 i1 = 0; i1 < literal.shape().dimensions(1); ++i1) {
				for (int64 i2 = 0; i2 < literal.shape().dimensions(2); ++i2) {
					for (int64 i3 = 0; i3 < literal.shape().dimensions(3);
							++i3) {
						per_cell( { i0, i1, i2, i3 }, GetAsString(literal, { i0,
								i1, i2, i3 }));
					}
				}
			}
		...
		const Literal& literal2)
{
	...
	if (ShapeUtil::IsTuple(literal1.shape())) {
		...
		// tuple elements.
		CHECK_EQ(literal1.tuple_literals_size(),
				literal2.tuple_literals_size());
		for (int i = 0; i < literal1.tuple_literals_size(); ++i) {
			if (!Equal(literal1.tuple_literals(i),
					literal2.tuple_literals(i))) {
				return false;
			}
		}
		return true;
...
template<>
/* static */tensorflow::gtl::ArraySlice<uint32> LiteralUtil::GetArraySlice<
		uint32>(const Literal& literal)
{
...
template<>
/* static */tensorflow::gtl::ArraySlice<uint64> LiteralUtil::GetArraySlice<
		uint64>(const Literal& literal)
{
...
template<>
/* static */tensorflow::gtl::ArraySlice<int32> LiteralUtil::GetArraySlice<int32>(
		const Literal& literal)
{
...
template<>
/* static */tensorflow::gtl::ArraySlice<int64> LiteralUtil::GetArraySlice<int64>(
		const Literal& literal)
{
...
template<>
/* static */tensorflow::gtl::ArraySlice<float> LiteralUtil::GetArraySlice<float>(
		const Literal& literal)
{
...
template<>
/* static */tensorflow::gtl::ArraySlice<double> LiteralUtil::GetArraySlice<
		double>(const Literal& literal)
{
...
static bool AllElementsEqualValue(const Literal& literal, NativeT value)
{
	for (int64 i = 0; i < ShapeUtil::ElementsIn(literal.shape()); ++i) {
		auto multi_index = IndexUtil::LinearIndexToMultidimensionalIndex(
				literal.shape(), i);
...
/* static */bool LiteralUtil::IsZero(const Literal& literal,
...
template<>
/* static */void LiteralUtil::PopulateWithValue(int64 value,
		tensorflow::gtl::ArraySlice<int64> dimensions, Literal* literal)
{
	*literal->mutable_shape() = ShapeUtil::MakeShape(
			primitive_util::NativeToPrimitiveType<int64>(), dimensions);
	tensorflow::protobuf::RepeatedField<tensorflow::protobuf_int64>* repeated_field =
...
template<>
/* static */void LiteralUtil::PopulateWithValue(uint64 value,
		tensorflow::gtl::ArraySlice<int64> dimensions, Literal* literal)
{
	*literal->mutable_shape() = ShapeUtil::MakeShape(
			primitive_util::NativeToPrimitiveType<uint64>(), dimensions);
	tensorflow::protobuf::RepeatedField<tensorflow::protobuf_uint64>* repeated_field =
