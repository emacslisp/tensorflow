...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				Conv2DBackpropComputeDimensions("Conv2DFastBackpropFilter",
		...
		functor::SpatialConvolutionBackwardKernel<Device, T>()(
				...
				filter_backprop->tensor<T, 4>(), input.tensor<T, 4>(),
				out_backprop.tensor<T, 4>(), dims.rows.filter_size,
				dims.cols.filter_size, dims.rows.stride, dims.cols.stride);
	}
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		Conv2DBackpropDimensions dims;
		OP_REQUIRES_OK(context,
				Conv2DBackpropComputeDimensions("Conv2DCustomBackpropFilter",
						input.shape(), filter_shape, out_backprop.shape(),
						strides_, padding_, data_format_, &dims));
		...
		const size_t shard_size = (target_working_set_size + work_unit_size - 1)
				/ work_unit_size;
		...
		for (int image_id = 0; image_id < dims.batch_size; image_id +=
			...
			auto shard =
					[&input_data, &col_buffer_data, &dims, &pad_top, &pad_left,
