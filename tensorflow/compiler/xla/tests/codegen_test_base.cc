...
{
	std::unique_ptr<Executable> executable = CompileToExecutable(
			std::move(hlo_module));
		...
		std::unique_ptr<HloModule> hlo_module)
{
	...
	return backend_->compiler()->Compile(std::move(hlo_module),
			...
			backend_->default_stream_executor()).ConsumeValueOrDie();
}
...
void CodegenTestBase::RunFileCheck(const string& input, const string& pattern)
{
	...
	string pattern_path = tensorflow::io::JoinPath(tempdir_name,
			"xla_hlo_test_ir_pattern");
	TF_CHECK_OK(
	...
	string file_check_path = tensorflow::io::JoinPath(test_srcdir,
			"external/llvm/FileCheck");
	file_check_process.SetProgram(file_check_path, { file_check_path,
			pattern_path });
