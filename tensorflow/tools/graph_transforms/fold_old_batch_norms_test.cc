...
protected:
	void TestFoldOldBatchNorms()
	{
		...
		Output conv_op = Conv2D(root.WithOpName("conv_op"), input_op,
				weights_op, { 1, 1, 1, 1 }, "VALID");

		Tensor mean_data(DT_FLOAT, TensorShape( { 2 }));
		test::FillValues<float>(&mean_data, { 10.0f, 20.0f });
		Output mean_op = Const(root.WithOpName("mean_op"),
				Input::Initializer(mean_data));
		...
		Output beta_op = Const(root.WithOpName("beta_op"),
				Input::Initializer(beta_data));

		Tensor gamma_data(DT_FLOAT, TensorShape( { 2 }));
		test::FillValues<float>(&gamma_data, { 1.0f, 2.0f });
		Output gamma_op = Const(root.WithOpName("gamma_op"),
				Input::Initializer(gamma_data));
		...
		*(original_graph_def.mutable_node()->Add()) = batch_norm_node;
		...
		TF_ASSERT_OK(
				FoldOldBatchNorms(original_graph_def, { { }, { "output" } },
						&fused_graph_def));
