...
Status HandleSliceToElement(const Tensor& parent, Tensor* element, int64 index)
{
...
QueueBase::QueueBase(int32 capacity, const DataTypeVector& component_dtypes,
		...
		capacity_(capacity), component_dtypes_(component_dtypes), component_shapes_(
				component_shapes), name_(name), closed_(false)
{
}
...
Status QueueBase::ValidateTupleCommon(const Tuple& tuple) const
{
	if (tuple.size() != static_cast<size_t>(num_components())) {
		return errors::InvalidArgument(
				"Wrong number of components in tuple. Expected ",
				num_components(), ", got ", tuple.size());
	}
	for (size_t i = 0; i < tuple.size(); ++i) {
		if (tuple[i].dtype() != component_dtypes_[i]) {
			return errors::InvalidArgument("Type mismatch in tuple component ",
					i, ". Expected ", DataTypeString(component_dtypes_[i]),
					", got ", DataTypeString(tuple[i].dtype()));
		}
	}
	return Status::OK();
}
...
{
	if (node_def.op() != op) {
		return errors::InvalidArgument("Shared queue '", name_, "' has type '",
				op, "' that does not match type of Node '", node_def.name(),
				"': ", node_def.op());
	}
	return Status::OK();
}
		...
		int32 capacity) const
{
	...
	if (requested_capacity < 0)
		requested_capacity = kUnbounded;
	if (requested_capacity != capacity) {
		return errors::InvalidArgument("Shared queue '", name_,
				"' has capacity ", capacity, " but requested capacity was ",
				requested_capacity);
	}
	return Status::OK();
}
		...
		for (size_t i = 1; i < tuple.size(); ++i) {
			if (tuple[i].dim_size(0) != batch_size) {
				return errors::InvalidArgument(
						...
						"dimension. Component ", i, " has ",
						tuple[i].dim_size(0), ", and should have ", batch_size);
			}
		}
	}
	return Status::OK();
			...
			if (attempt.cancellation_manager == cancellation_manager
				...
				if (!attempt.is_cancelled) {
					...
					if (action == kEnqueue) {
						...
						attempt.context->SetStatus(
								errors::Cancelled(
										"Dequeue operation was cancelled"));
					}
...
bool QueueBase::TryAttemptLocked(Action action, std::vector<CleanUp>* clean_up)
{
std::deque<Attempt>* attempts =
		action == kEnqueue ? &enqueue_attempts_ : &dequeue_attempts_;
