...
IrEmitter::IrEmitter(const HloModule& hlo_module,
		...
		assignment_(assignment), module_(llvm_module), arch_type_(
				llvm::Triple(llvm_module->getTargetTriple()).getArch()), ir_builder_(
				llvm_module->getContext()), hlo_to_profile_idx_(
				hlo_to_profile_idx), alias_analysis_(hlo_module, assignment,
				&llvm_module->getContext()), hlo_module_config_(
				hlo_module_config)
{
		...
		std::vector<const HloInstruction*>* instruction_order)
{
	...
	if (instruction_order != nullptr) {
		TF_RETURN_IF_ERROR(
				computation->root_instruction()->AcceptOrdered(this,
						*instruction_order));
	} else {
		...
		bool is_entry_computation)
{
  ...
  //   prof counters ->  | counter 0 | counter 1 | ..... | counter N-1 |
	...
	std::vector<llvm::Type*> compute_function_params( { i8_ptr_type,
			i8_ptr_type, i8_ptr_ptr_type, i8_ptr_ptr_type });
		...
		const Literal& literal)
{
	...
	llvm::Constant* initializer = llvm_ir::ConvertLiteralToIrConstant(literal,
			&ir_builder_);
...
Status IrEmitter::HandleCopy(HloInstruction* copy, HloInstruction* operand)
{
	if (ShapeUtil::IsTuple(copy->shape())) {
		...
		TF_ASSIGN_OR_RETURN(llvm::Value * copy_value,
				EmitTargetAddressForOp(copy));
		...
		HloInstruction* operand)
{
	...
	emitted_value_[get_tuple_element] = llvm_ir::EmitGetTupleElement(shape,
			get_tuple_element->tuple_index(), MinimumAlignmentForShape(shape),
			GetEmittedValueFor(operand), &ir_builder_);
	return Status::OK();
}

Status IrEmitter::HandleSelect(HloInstruction* select, HloInstruction* pred,
		HloInstruction* on_true, HloInstruction* on_false)
{
	...
	if (ShapeUtil::IsTuple(select->shape())) {
		...
		llvm_ir::EmitTupleSelect(
				llvm_ir::IrArray(output_address, select->shape()),
...
Status IrEmitter::HandleInfeed(HloInstruction* infeed)
{
	...
	llvm::FunctionType* acquire_type = llvm::FunctionType::get(i8_ptr_type, {
			int32_type },
	/*isVarArg=*/false);

	llvm::Function* acquire_func = llvm::cast < llvm::Function
	...
	llvm::Function* release_func = llvm::cast < llvm::Function
			> (module_->getOrInsertFunction(
	...
	llvm::Value* acquired_pointer = ir_builder_.CreateCall(acquire_func, {
			ir_builder_.getInt32(length_32) });
	...
	ir_builder_.CreateCall(release_func, { ir_builder_.getInt32(length_32),
			acquired_pointer });
		...
		tensorflow::gtl::ArraySlice<HloInstruction*> /*static_operands*/)
{
	...
	llvm::Function* mapped_ir_function = FindOrDie(emitted_functions_,
			function);
...
Status IrEmitter::HandleReduceWindow(HloInstruction* reduce_window,
		HloInstruction* operand, const Window& window, HloComputation* function)
{
	...
	// that works.
	return EmitTargetElementLoop(reduce_window,
			[this, reduce_window, operand, window,
...
Status IrEmitter::HandleSelectAndScatter(HloInstruction* select_and_scatter)
{
	...
	llvm::Function* select_function = FindOrDie(emitted_functions_,
			select_and_scatter->select());
	llvm::Function* scatter_function = FindOrDie(emitted_functions_,
			select_and_scatter->scatter());
	...
	TF_RETURN_IF_ERROR(
			EmitTargetElementLoop(select_and_scatter,
	...
	const llvm_ir::IrArray::Index source_index = source_loops.AddLoopsForShape(
			source->shape(), "source");
	...
	for (int64 i = 0; i < rank; ++i) {
		llvm::Value* strided_index = ir_builder_.CreateNSWMul(source_index[i],
				ir_builder_.getInt64(window.dimensions(i).stride()));
		operand_index[i] = ir_builder_.CreateNSWSub(
				ir_builder_.CreateNSWAdd(strided_index, window_index[i]),
				ir_builder_.getInt64(window.dimensions(i).padding_low()));
		llvm::Value* index_condition = ir_builder_.CreateICmpULT(
				operand_index[i],
				ir_builder_.getInt64(
						ShapeUtil::GetDimension(operand->shape(), i)));
		in_bounds_condition = ir_builder_.CreateAnd(in_bounds_condition,
				index_condition);
	}
	CHECK(in_bounds_condition != nullptr);
	...
	llvm::Value* operand_data = operand_array.EmitReadArrayElement(
			operand_index, &ir_builder_);
	...
	llvm::Value* operand_address = operand_array.EmitArrayElementAddress(
			operand_index, &ir_builder_);
	llvm::Value* result = EmitElementFunctionCall(select_function, output_shape,
			{ selected_value_address, operand_address }, "select_function");

	// If the 'select' function returns false, update the selected value and the
	// index to the currently visiting operand.
	llvm::Value* cond = ir_builder_.CreateICmpNE(result,
			llvm::ConstantInt::get(
					llvm_ir::PrimitiveTypeToIrType(PRED, &ir_builder_), 0),
			"boolean_predicate");
	llvm_ir::LlvmIfData if_select_lhs = llvm_ir::EmitIfThenElse(cond,
			"if-select-lhs", &ir_builder_);
	...
	llvm::Value* source_value_address = source_array.EmitArrayElementAddress(
			source_index, &ir_builder_);
	llvm_ir::IrArray output_array(GetIrArrayForOp(select_and_scatter));
	llvm::Value* output_value_address = output_array.EmitArrayElementAddress(
			selected_index, &ir_builder_);
	llvm::Value* scatter_value = EmitElementFunctionCall(scatter_function,
			source->shape(), { output_value_address, source_value_address },
			"scatter_function");
		...
		HloInstruction* rhs)
{
	...
	TF_RETURN_IF_ERROR(
			DotOpEmitter::EmitDotOperation(*dot, /*transpose_lhs=*/false, /*transpose_rhs=*/
					false, target_array, lhs_array, rhs_array,
		...
		HloInstruction* lhs, HloInstruction* rhs, const Window& window)
{
	...
	if (PotentiallyImplementedAsEigenConvolution(*convolution)) {
		...
		if (LayoutUtil::IsMonotonicWithDim0Major(lhs_shape.layout())
			...
			int64 input_rows = input_shape.dimensions(
					dnums.spatial_dimensions(0));
			int64 input_cols = input_shape.dimensions(
					dnums.spatial_dimensions(1));
			int64 input_channels = input_shape.dimensions(
					dnums.feature_dimension());

			// Kernel tensor.
			const Shape& kernel_shape = convolution->operand(1)->shape();
			int64 kernel_rows = kernel_shape.dimensions(
					dnums.kernel_spatial_dimensions(0));
			int64 kernel_cols = kernel_shape.dimensions(
					dnums.kernel_spatial_dimensions(1));
			int64 kernel_channels = kernel_shape.dimensions(
					dnums.kernel_input_feature_dimension());
			int64 kernel_filters = kernel_shape.dimensions(
					dnums.kernel_output_feature_dimension());

			// Output tensor.
			const Shape& convolution_shape = convolution->shape();
			int64 output_rows = convolution_shape.dimensions(
					dnums.spatial_dimensions(0));
			int64 output_cols = convolution_shape.dimensions(
					dnums.spatial_dimensions(1));
			...
			llvm::FunctionType* conv_type = llvm::FunctionType::get(
					ir_builder_.getVoidTy(), { int8_ptr_type, float_ptr_type,
					...
					/*isVarArg=*/false);
			legacy_flags::CpuRuntimeFlags* flags =
			...
			ir_builder_.CreateCall(conv_func,
					{ GetExecutableRunOptionsArgument(),
							ir_builder_.CreateBitCast(target_address,
									float_ptr_type), ir_builder_.CreateBitCast(
									lhs_address, float_ptr_type),
							ir_builder_.CreateBitCast(rhs_address,
									float_ptr_type), ir_builder_.getInt64(
									input_batch), ir_builder_.getInt64(
									input_rows), ir_builder_.getInt64(
									input_cols), ir_builder_.getInt64(
									input_channels), ir_builder_.getInt64(
									kernel_rows), ir_builder_.getInt64(
									kernel_cols), ir_builder_.getInt64(
									kernel_channels), ir_builder_.getInt64(
									kernel_filters), ir_builder_.getInt64(
									output_rows), ir_builder_.getInt64(
									output_cols), ir_builder_.getInt64(
									row_stride), ir_builder_.getInt64(
									col_stride), ir_builder_.getInt64(
									padding_top), ir_builder_.getInt64(
									padding_bottom), ir_builder_.getInt64(
									padding_left), ir_builder_.getInt64(
									padding_right), ir_builder_.getInt64(
									lhs_row_dilation), ir_builder_.getInt64(
									lhs_col_dilation), ir_builder_.getInt64(
									rhs_row_dilation), ir_builder_.getInt64(
									rhs_col_dilation), });
	...
	return EmitTargetElementLoop(convolution,
			[this, convolution, lhs, rhs, window,
...
Status IrEmitter::HandleParameter(HloInstruction* parameter)
{
	...
	llvm::Value* param_address_offset = llvm_ir::EmitBufferIndexingGEP(params,
			param_number, &ir_builder_);
	llvm::LoadInst* param_address_untyped = ir_builder_.CreateLoad(
	...
	VLOG(2) << "  emitted value: "
			<< llvm_ir::DumpToString(*param_address_typed);
	return Status::OK();
}
		...
		tensorflow::gtl::ArraySlice<int64> dimensions, HloComputation* function)
{
	...
	return EmitTargetElementLoop(reduce,
			[this, reduce, arg, init_value, dimensions,
			reducer_function](const llvm_ir::IrArray::Index& index) {
...
Status IrEmitter::HandlePad(HloInstruction* pad)
{
	...
	const llvm_ir::IrArray::Index operand_index = loops.AddLoopsForShape(
	...
	for (int64 i = 0; i < operand_index.size(); ++i) {
		llvm::Value* offset = ir_builder_.CreateMul(operand_index[i],
				ir_builder_.getInt64(
						padding_config.dimensions(i).interior_padding() + 1));
		llvm::Value* index = ir_builder_.CreateAdd(offset,
				ir_builder_.getInt64(
	...
	output_array.EmitWriteArrayElement(output_index, operand_data,
			&ir_builder_);
...
Status IrEmitter::HandleFusion(HloInstruction* fusion)
{
	if (fusion->fusion_kind() == HloInstruction::FusionKind::kTransposeDot) {
		...
		DCHECK(
				lhs_parameter->opcode() == HloOpcode::kParameter
						&& rhs_parameter->opcode() == HloOpcode::kParameter);
		const HloInstruction* lhs = fusion->operand(
				lhs_parameter->parameter_number());
		const HloInstruction* rhs = fusion->operand(
				rhs_parameter->parameter_number());
		...
		TF_RETURN_IF_ERROR(
				DotOpEmitter::EmitDotOperation(*dot,
						dot->operand(0)->IsRank2Transpose(),
						dot->operand(1)->IsRank2Transpose(), target_array,
						lhs_array, rhs_array, GetExecutableRunOptionsArgument(),
						&ir_builder_));
		...
		CpuElementalIrEmitter elemental_emitter(hlo_module_config_,
				&ir_builder_, module_);
		FusedIrEmitter fused_emitter(parameter_arrays, &elemental_emitter);
		TF_RETURN_IF_ERROR(
				fusion->fused_expression_root()->Accept(&fused_emitter));
...
{
	llvm::Function* call_ir_function = FindOrDie(emitted_functions_,
			computation);
		...
		tensorflow::StringPiece custom_call_target)
{
	...
	llvm::AllocaInst* operands_alloca =
			llvm_ir::EmitAllocaAtFunctionEntryWithCount(i8_ptr_type,
					ir_builder_.getInt32(operands.size()), "cc_operands_alloca",
					&ir_builder_);
	for (int i = 0; i < operands.size(); ++i) {
		const HloInstruction* operand = operands[i];
		llvm::Value* operand_as_i8ptr = ir_builder_.CreatePointerCast(
				GetEmittedValueFor(operand), i8_ptr_type);
	...
	auto* custom_call_ir_function = llvm::cast < llvm::Function
			> (module_->getOrInsertFunction(
	...
	auto* output_address_arg = ir_builder_.CreatePointerCast(output_address,
			i8_ptr_type);

	ir_builder_.CreateCall(custom_call_ir_function, { output_address_arg,
			operands_alloca });
		...
		HloComputation* condition, HloComputation* body)
{
	...
	TF_RET_CHECK(
			ShapeUtil::IsScalar(condition->root_instruction()->shape())
					&& condition->root_instruction()->shape().element_type()
							== PRED)
			<< "While condition computation must return bool";
	// Check that all while-related buffers share an allocation.
	TF_RETURN_IF_ERROR(
			ShapeUtil::ForEachSubshape(xla_while->shape(),
	...
	// The called computation should have been emitted previously.
	llvm::Function* condition_ir_function = FindOrDie(emitted_functions_,
			condition);
	...
	llvm::Value* while_predicate = ir_builder_.CreateICmpNE(while_condition,
			llvm::ConstantInt::get(
					llvm_ir::PrimitiveTypeToIrType(PRED, &ir_builder_), 0));

	// Branches to the body or to the while exit depending on the condition.
	llvm::BasicBlock* body_bb = llvm::BasicBlock::Create(module_->getContext(),
			"while_body", compute_function_);
	llvm::BasicBlock* exit_bb = llvm::BasicBlock::Create(module_->getContext(),
			"while__exit");
...
llvm::Value* IrEmitter::GetProfileCounterFor(const HloInstruction* hlo)
{
	...
	if (hlo) {
		...
		counter_name = tensorflow::strings::StrCat("prof_counter_0x",
				tensorflow::strings::Hex(hlo_address,
						tensorflow::strings::PadSpec(sizeof(hlo_address))));
	} else {
		...
		llvm::Value* cycle_end, llvm::Value* cycle_start)
{
	...
	llvm::LoadInst* old_cycle_count = ir_builder->CreateLoad(prof_counter,
			"old_cycle_count");
	auto* new_cycle_count = ir_builder->CreateAdd(cycle_diff, old_cycle_count,
			"new_cycle_count");
		...
		llvm::IRBuilder<>* ir_builder)
{
	...
	llvm::Function* func_llvm_x86_rdtscp = llvm::Intrinsic::getDeclaration(
			module, llvm::Intrinsic::x86_rdtscp);
	if (!aux_i8ptr_) {
		llvm::AllocaInst* rdtscp_aux = llvm_ir::EmitAllocaAtFunctionEntry(
				ir_builder->getInt32Ty(), "rdtscp_aux", ir_builder);
		aux_i8ptr_ = ir_builder->CreateBitCast(rdtscp_aux,
				ir_builder->getInt8PtrTy());
	}
	llvm::ConstantInt* alloca_size = ir_builder->getInt64(4);
	llvm::Function* func_llvm_lifetime_start = llvm::Intrinsic::getDeclaration(
			module, llvm::Intrinsic::lifetime_start);
	ir_builder->CreateCall(func_llvm_lifetime_start,
			{ alloca_size, aux_i8ptr_ });
	llvm::Value* rdtscp_call = ir_builder->CreateCall(func_llvm_x86_rdtscp,
			aux_i8ptr_);
	llvm::Function* func_llvm_lifetime_end = llvm::Intrinsic::getDeclaration(
			module, llvm::Intrinsic::lifetime_end);
...
void IrEmitter::ProfilingState::RecordCycleDelta(llvm::IRBuilder<>* ir_builder,
		HloInstruction* hlo, llvm::Value* prof_counter)
{
		...
		BufferAllocation::Index temp_buf_index, const Shape& target_shape)
{
	...
	const BufferAllocation& allocation = assignment_.GetAllocation(
			temp_buf_index);
	...
	llvm::LoadInst* tempbuf_address_untyped = ir_builder_.CreateLoad(
			tempbuf_address_offset);
	...
	tempbuf_address_untyped->setMetadata(llvm::LLVMContext::MD_invariant_load,
...
llvm::Value* IrEmitter::EmitElementFunctionCall(llvm::Function* function,
		const Shape& return_shape,
		tensorflow::gtl::ArraySlice<llvm::Value*> parameter_addresses,
		tensorflow::StringPiece name)
{
	llvm::Value* return_value_buffer = EmitArrayFunctionCall(function,
			return_shape, 1, parameter_addresses, name);
	return ir_builder_.CreateLoad(return_value_buffer,
			llvm_ir::AsStringRef(
					tensorflow::strings::StrCat(name, "_return_value")));
}
...
void IrEmitter::EmitArrayFunctionCallInto(llvm::Function* function,
		tensorflow::gtl::ArraySlice<llvm::Value*> parameter_addresses,
		llvm::Value* return_value_buffer, tensorflow::StringPiece name)
{
	...
	for (int i = 0; i < parameter_addresses.size(); ++i) {
		llvm::Value* parameter_as_i8ptr = ir_builder_.CreateBitCast(
				...
				llvm_ir::AsStringRef(
						tensorflow::strings::StrCat(name, "_parameter_", i,
								"_address_as_i8ptr")));
	...
	std::vector<llvm::Value*> arguments { to_int8_ptr(return_value_buffer),
...
llvm::Value* IrEmitter::EmitArrayFunctionCall(llvm::Function* function,
		const Shape& return_shape, int64 element_count,
		tensorflow::gtl::ArraySlice<llvm::Value*> parameter_addresses,
		tensorflow::StringPiece name)
{
	llvm::Value* elements = llvm::ConstantInt::get(ir_builder_.getInt64Ty(),
			element_count);
	PrimitiveType return_type = return_shape.element_type();
	llvm::Value* return_value_buffer =
			llvm_ir::EmitAllocaAtFunctionEntryWithCount(
					llvm_ir::PrimitiveTypeToIrType(return_type, &ir_builder_),
					elements,
					tensorflow::strings::StrCat(name, "_return_value_address"),
					&ir_builder_,
					MinimumAlignmentForPrimitiveType(return_type));
	EmitArrayFunctionCallInto(function, parameter_addresses,
		...
		const HloInstruction* op)
{
	...
	if (op == op->parent()->root_instruction()) {
		...
		if (!ShapeUtil::HasZeroElements(target_shape)) {
			...
			attr_builder.addAlignmentAttr(
					MinimumAlignmentForShape(target_shape));
			attr_builder.addDereferenceableAttr(ByteSizeOf(target_shape));
			retval->addAttr(
					llvm::AttributeSet::get(retval->getContext(),
							retval->getArgNo() + 1, attr_builder));
		}
...
Status IrEmitter::EmitTargetElementLoop(HloInstruction* target_op,
		const llvm_ir::ElementGenerator& element_generator)
{
	...
	TF_RETURN_IF_ERROR(
			llvm_ir::LoopEmitter(element_generator, target_array, &ir_builder_).EmitLoop());
		...
		tensorflow::gtl::ArraySlice<PrimitiveType> supported_types)
{
	for (auto operand : operands) {
		TF_RET_CHECK(
				ShapeUtil::SameElementType(operands[0]->shape(),
						operand->shape()));
	}
