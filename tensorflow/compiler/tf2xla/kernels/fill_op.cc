...
public:
	explicit FillOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, IsLegacyVector(dims_shape),
		...
		OP_REQUIRES_OK(ctx,
				ctx->ConstantInputReshaped(0, { dims_shape.num_elements() },
						&dims_literal));
		...
		for (int i = 0; i < dims_literal.shape().dimensions(0); ++i) {
			broadcast.push_back(
					xla::LiteralUtil::Get<int>(dims_literal, { i }));
		}
