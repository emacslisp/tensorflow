...
Status InitializeSession(int num_threads, const string& graph,
		std::unique_ptr<Session>* session, std::unique_ptr<GraphDef>* graph_def)
{
...
void CreateTensorsFromInputInfo(const std::vector<InputLayerInfo>& inputs,
		std::vector<std::pair<string, tensorflow::Tensor> >* input_tensors)
{
	for (const InputLayerInfo& input : inputs) {
		...
		case DT_QUINT8: {
			InitializeTensor<quint8>(input.initialization_values,
					&input_tensor);
			break;
		}
		case DT_UINT8: {
		...
		std::unordered_map<string, TensorShape>* node_shapes)
{
	...
	std::vector<string> output_tensor_names(wanted_shapes.begin(),
			wanted_shapes.end());
	TF_RETURN_IF_ERROR(
			session->Run(input_tensors, output_tensor_names, { },
					&output_tensors));
...
Status CalculateFlops(const GraphDef& graph,
		const std::vector<InputLayerInfo>& inputs, Session* session,
		int64* total_flops, std::unordered_map<string, int64>* flops_by_op)
{
	std::unordered_set<string> floppable_ops = { "Conv2D", "MatMul",
			"QuantizedConv2D", "QuantizedMatMul" };
	...
	for (const NodeDef& node : graph.node()) {
		if (floppable_ops.count(node.op())) {
			...
			if ((node.op() == "Conv2D") || (node.op() == "QuantizedConv2D")) {
				...
				current_flops = output_count * filter_in_depth * filter_height
						* filter_width * 2;
			} else if ((node.op() == "MatMul")
					|| (node.op() == "QuantizedMatMul")) {
...
int Main(int argc, char** argv)
{
	...
	std::vector<Flag> flag_list = { Flag("graph", &graph, "graph file name"),
			Flag("input_layer", &input_layer_string, "input layer names"), Flag(
					"input_layer_shape", &input_layer_shape_string,
					"input layer shape"), Flag("input_layer_type",
					&input_layer_type_string, "input layer type"), Flag(
					"input_layer_values", &input_layer_values_string,
					"values to initialize the inputs with"), Flag(
					"output_layer", &output_layer_string, "output layer name"),
			Flag("num_runs", &num_runs, "number of runs"), Flag("run_delay",
					&run_delay, "delay between runs in seconds"), Flag(
					"num_threads", &num_threads, "number of threads"), Flag(
					"benchmark_name", &benchmark_name, "benchmark name"), Flag(
					"output_prefix", &output_prefix, "benchmark output prefix"),
			Flag("show_sizes", &show_sizes, "whether to show sizes"), Flag(
					"show_run_order", &show_run_order,
					"whether to list stats by run order"), Flag(
					"run_order_limit", &run_order_limit,
					"how many items to show by run order"), Flag("show_time",
					&show_time, "whether to list stats by time taken"), Flag(
					"time_limit", &time_limit,
					"how many items to show by time taken"), Flag("show_memory",
					&show_memory, "whether to list stats by memory used"), Flag(
					"memory_limit", &memory_limit,
					"how many items to show by memory used"), Flag("show_type",
					&show_time, "whether to list stats by op type"), Flag(
					"show_summary", &show_time,
					"whether to show a summary of the stats"), Flag(
	...
	std::vector<string> input_layer_shapes = str_util::Split(
			input_layer_shape_string, ':');
	std::vector<string> input_layer_types = str_util::Split(
			input_layer_type_string, ',');
	std::vector<string> input_layer_values = str_util::Split(
			input_layer_values_string, ':');
	...
	Status initialize_status = InitializeSession(num_threads, graph, &session,
			&graph_def);
	...
	for (int n = 0; n < inputs_count; ++n) {
		...
		if (n < input_layer_values.size()) {
			CHECK(
					str_util::SplitAndParseAsFloats(input_layer_values[n], ',',
	...
	Status time_status = TimeMultipleRuns(sleep_seconds, num_runs, inputs,
			output_layers, session.get(), stats.get());
	...
	if (show_flops) {
		...
		LOG(INFO) << "FLOPs estimate: "
				<< strings::HumanReadableNum(total_flops);
	...
	if (!benchmark_name.empty() && !output_prefix.empty()) {
		...
		const double throughput = DataTypeSize(inputs[0].data_type) * total_size
				* num_runs / static_cast<double>(wall_time) / (1024 * 1024);
