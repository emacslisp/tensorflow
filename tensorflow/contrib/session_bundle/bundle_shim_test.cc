...
void ValidateHalfPlusTwo(const SavedModelBundle& saved_model_bundle,
		const string& input_tensor_name, const string& output_tensor_name)
{
	...
	Tensor input = test::AsTensor<string>(serialized_examples,
			TensorShape( { 4 }));
	...
	test::ExpectTensorEqual<float>(outputs[0], test::AsTensor<float>( { 2, 2.5,
			3, 3.5 }, TensorShape( { 4, 1 })));
}

void LoadAndValidateSavedModelBundle(const string& export_dir,
		const std::unordered_set<string>& tags, const string& signature_def_key)
{
	SessionOptions session_options;
	RunOptions run_options;
	SavedModelBundle saved_model_bundle;
	TF_ASSERT_OK(
			LoadSessionBundleOrSavedModelBundle(session_options, run_options,
					export_dir, tags, &saved_model_bundle));
	...
	ASSERT_FALSE(
			regression_signature_def.inputs().find(kRegressInputs)
					== regression_signature_def.inputs().end());
	TensorInfo input_tensor_info = regression_signature_def.inputs().find(
			kRegressInputs)->second;
	EXPECT_EQ(1, regression_signature_def.outputs_size());

	ASSERT_FALSE(
			regression_signature_def.outputs().find(kRegressOutputs)
					== regression_signature_def.outputs().end());
	TensorInfo output_tensor_info = regression_signature_def.outputs().find(
			kRegressOutputs)->second;
		...
		const string& expected_string_signature_def)
{
	...
	CHECK(
			protobuf::TextFormat::ParseFromString(expected_string_signature_def,
					&expected_signature));
