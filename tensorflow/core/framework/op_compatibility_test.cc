...
public:
	explicit TestKernel(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output("ndef", TensorShape( { }),
						&out_tensor));
...
protected:
	...
	void ExpectSuccess(const OpDef& old_op_def)
	{
		...
		TF_ASSERT_OK(
				InOutTypesForNode(*node_def(), old_op_def, &old_in_types,
						&old_out_types));
		...
		TF_ASSERT_OK(
				InOutTypesForNode(*node_def(), *new_op_def, &new_in_types,
						&new_out_types));
		...
		if (new_out_types.size() == old_out_types.size()) {
			...
			for (int i = 0; i < new_out_types.size(); ++i) {
				if (!IsRefType(old_out_types[i])
						&& IsRefType(new_out_types[i])) {
	...
	string Result()
	{
		return GetOutput(0)->scalar<string>()();
	}
			...
			const string& compatibility_error)
	{
		...
		if (status.ok()) {
			...
			EXPECT_TRUE(
					StringPiece(status.error_message()).contains(
							validation_error)) << status << " does not contain "
					<< validation_error;
		}
