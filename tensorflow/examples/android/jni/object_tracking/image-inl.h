...
Image<T>::Image(const int width, const int height) :
		width_less_one_(width - 1), height_less_one_(height - 1), data_size_(
				width * height), own_data_(true), width_(width), height_(
				height), stride_(width)
{
	Allocate();
}
...
Image<T>::Image(const Size& size) :
		width_less_one_(size.width - 1), height_less_one_(size.height - 1), data_size_(
				size.width * size.height), own_data_(true), width_(size.width), height_(
				size.height), stride_(size.width)
{
	Allocate();
}
...
Image<T>::Image(const int width, const int height, T* const image_data,
		const bool own_data) :
		width_less_one_(width - 1), height_less_one_(height - 1), data_size_(
				width * height), own_data_(own_data), width_(width), height_(
				height), stride_(width)
{
...
bool Image<T>::ExtractPatchAtSubpixelFixed1616(const int fp_x, const int fp_y,
		const int patchwidth, const int patchheight, DstType* to_data) const
{
	// Calculate weights.
	...
	for (int y = 0; y < patchheight; ++y, to_data += patchwidth) {
		for (int x = 0; x < patchwidth; ++x) {
			to_data[x] = static_cast<DstType>(GetPixelInterpFixed1616(
					fp_x + (x << 16), fp_y + (y << 16)));
		}
	}
...
void Image<T>::DownsampleInterpolateLinear(const Image<T>& original)
{
	...
	// copy method.
	if (original.GetWidth() == GetWidth() && original.GetHeight() == GetHeight()
	...
	for (int y = 0; y < height_; ++y) {
		...
		for (int x = 0; x < width_; ++x) {
			...
			if (trunc_x != last_trunc_x) {
				...
				const int trunc_x_b = MIN(original.width_less_one_,
						trunc_x + 1);
	...
	for (int y = 0; y < height_; ++y) {
		...
		for (int x = 0; x < width_; ++x) {
			...
			pixel_sum += (original[orig_y][max_x] + original[orig_y][min_x]
					+ original[max_y][orig_x] + original[min_y][orig_x]) * 2;
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			...
			for (int window_y = 0; window_y < window_size; ++window_y) {
				const int start_y = Clip((y << 1) - window_radius + window_y,
						ZERO, max_y);
		...
		const int center_y) const
{
	...
	return (3
			* (original[min_y][max_x] + original[max_y][max_x]
					- original[min_y][min_x] - original[max_y][min_x])
			+ 10 * (original[center_y][max_x] - original[center_y][min_x])) / 32;
}
		...
		const int center_y) const
{
	...
	// Convolution loop unrolled for performance...
	return (3
			* (original[max_y][min_x] + original[max_y][max_x]
					- original[min_y][min_x] - original[min_y][max_x])
			+ 10 * (original[max_y][center_x] - original[min_y][center_x])) / 32;
}
...
void Image<T>::DerivativeX(const Image<U>& original)
{
	for (int y = 0; y < height_; ++y) {
		...
		for (int x = 1; x < width_less_one_; ++x) {
			*dest_pixel++ = HalfDiff(*source_prev_pixel++,
					*source_next_pixel++);
		}

		// Last pixel. Approximated with backward difference.
		dest_row[width_less_one_] = source_row[width_less_one_]
				- source_row[width_less_one_ - 1];
	}
}
...
inline T Image<T>::ConvolvePixel3x3(const Image<U>& original,
		const int* const filter, const int center_x, const int center_y,
