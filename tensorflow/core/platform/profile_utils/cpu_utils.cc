...
{
	static const double micro_sec_per_clock = (1000.0 * 1000.0)
			/ static_cast<double>(GetCycleCounterFrequency());
		...
		const int64 clock_cycle)
{
	return std::chrono::duration<double>(
			static_cast<double>(clock_cycle) / GetCycleCounterFrequency());
}
...
// TODO(satok): do not switch by macro here
	...
	int64 freq_hz;
	FILE* fp = popen("sysctl hw | grep hw.cpufrequency_max: | cut -d' ' -f 2",
			"r");
	if (fp == nullptr) {
		return INVALID_FREQUENCY;
	}
