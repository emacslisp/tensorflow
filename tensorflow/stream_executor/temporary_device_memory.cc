...
{
	return parent_->temporary_memory_manager()->IsFinalized(device_memory_,
			allocation_generation_);
}

bool TemporaryDeviceMemoryBase::IsAllocated() const
{
	return parent_->temporary_memory_manager()->HasAllocated(device_memory_,
			allocation_generation_);
}

TemporaryDeviceMemoryBase::TemporaryDeviceMemoryBase(Stream* parent,
		DeviceMemoryBase device_memory, uint64 allocation_generation) :
		device_memory_(device_memory), allocation_generation_(
				allocation_generation), parent_(parent)
{
	DCHECK(IsAllocated());
}
