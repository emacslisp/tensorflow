...
enum class MetricKind
	: int {kGauge = 0, kCumulative
};

// The type of the metric values.
enum class ValueType
	: int {kInt64 = 0, kHistogram
};
...
public:
	...
	template<int N>
	StringLiteral(const char (&data)[N]) :
			literal_(data, N - 1)
	{
	}
...
private:
	...
	AbstractMetricDef(const MetricKind kind, const ValueType value_type,
			...
			kind_(kind), value_type_(value_type), name_(name), description_(
					description), label_descriptions_(
					std::vector<StringPiece>(label_descriptions.begin(),
...
public:
	...
	MetricDef(const internal::StringLiteral name,
			...
			AbstractMetricDef(metric_kind, internal::GetValueType<Value>(),
					name, description, { label_descriptions... })
	{
