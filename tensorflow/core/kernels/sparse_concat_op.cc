...
public:
	explicit SparseConcatOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("concat_dim", &concat_dim_attr_));
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int i = 0; i < N; i++) {
			OP_REQUIRES(context, TensorShapeUtils::IsVector(shapes[i].shape()),
					errors::InvalidArgument(
							...
							shapes[i].shape().DebugString(), " at position ",
							i));
		}
			...
			for (int j = 0; j < input_rank; ++j) {
				if (j != concat_dim) {
					OP_REQUIRES(context,
							...
							errors::InvalidArgument(
									"Input shapes must match: expected ",
									input_shape.dim_size(j), " for dimension ",
									j, " but got ", current_shape.dim_size(j),
									" at position ", i));
				}
			}
		}
		...
		for (int i = 0; i < N; ++i) {
			...
			sp_inputs.emplace_back(tensor::DeepCopy(inds[i]),
					tensor::DeepCopy(vals[i]), current_shape, std_order);
			sp_inputs[i].Reorder<T>(concat_order);
		}

		sparse::SparseTensor concat = sparse::SparseTensor::Concat<T>(
				sp_inputs);
		...
		OP_REQUIRES_OK(context, context->allocate_output(2, TensorShape( {
				concat.shape().dims() }), &output_shape_out));
