...
public:
	Allocation(Backend* backend, int device_ordinal,
			...
			backend_(backend), device_ordinal_(device_ordinal), device_memory_(
					device_memory), shape_(shape), tag_(tag), ref_count_(
					initial_ref_count)
	{
	}
...
private:
	...
	GlobalDataHandle RegisterInternal(Backend* backend, int device_ordinal,
			perftools::gputools::DeviceMemoryBase device_memory,
			const Shape& shape, const string& tag, int initial_ref_count)
	EXCLUSIVE_LOCKS_REQUIRED (allocation_mutex_);
	...
	// nested tuple.
	tensorflow::Status DeallocateShape(Backend* backend, int device_ordinal,
			perftools::gputools::DeviceMemoryBase* device_memory,
	...
	int64
	next_handle_ GUARDED_BY(allocation_mutex_);
