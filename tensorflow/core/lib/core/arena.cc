...
Arena::Arena(const size_t block_size) :
		remaining_(0), block_size_(block_size), freestart_(NULL), // set for real in Reset()
		blocks_alloced_(1), overflow_blocks_(NULL)
{
	assert(block_size > kDefaultAlignment);

	first_blocks_[0].mem = reinterpret_cast<char*>(port::AlignedMalloc(
...
{
	const size_t overage = reinterpret_cast<size_t>(freestart_)
			& (alignment - 1);
	if (overage > 0) {
		...
		const uint32 alignment)
{
	...
	uint32 adjusted_alignment = (
			alignment > 1 ?
					LeastCommonMultiple(alignment, kDefaultAlignment) : 1);
	// Required minimum alignment for port::AlignedMalloc().
	adjusted_alignment = std::max(adjusted_alignment,
	...
	block->mem = reinterpret_cast<char*>(port::AlignedMalloc(
			adjusted_block_size, adjusted_alignment));
	block->size = adjusted_block_size;
	CHECK(NULL != block->mem) << "block_size=" << block_size
			<< " adjusted_block_size=" << adjusted_block_size << " alignment="
			<< alignment << " adjusted_alignment=" << adjusted_alignment;
...
void Arena::FreeBlocks()
{
	...
	if (overflow_blocks_ != NULL) {
		...
		for (it = overflow_blocks_->begin(); it != overflow_blocks_->end();
				++it) {
