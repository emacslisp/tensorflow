		...
		int64* mask)
{
	...
	for (int i = 0; i < MASK_WORDS; ++i) {
		...
		if (!ok) {
			LOG(ERROR) << "i=" << i << " mask="
					<< reinterpret_cast<const void*>(mask[i]) << " field="
					<< reinterpret_cast<const void*>(tmp[i]);
		}
	}
...
void InitMask(perftools::gputools::StreamExecutor* exec, void* ptr, int64* mask)
{
	gpu::DeviceMemory<int64> gpu_ptr { gpu::DeviceMemoryBase { ptr, MASK_BYTES } };
...
bool GPUDebugAllocator::TracksAllocationSizes()
{
	return true;
}
...
{
	auto req_size = base_allocator_->RequestedSize(
			static_cast<char*>(ptr) - MASK_BYTES);
...
void* GPUNanResetAllocator::AllocateRaw(size_t alignment, size_t num_bytes)
{
	...
	gpu::DeviceMemory<float> nan_ptr { gpu::DeviceMemoryBase {
			static_cast<float*>(allocated_ptr), req_size } };
...
void GPUNanResetAllocator::DeallocateRaw(void* ptr)
{
	...
	std::vector<float> nans(req_size / sizeof(float), std::nanf(""));
	gpu::DeviceMemory<float> nan_ptr { gpu::DeviceMemoryBase {
			static_cast<float*>(ptr), req_size } };
	if (!stream_exec_->SynchronousMemcpy(&nan_ptr, &nans[0], req_size)) {
		LOG(ERROR) << "Could not initialize to NaNs";
	}
