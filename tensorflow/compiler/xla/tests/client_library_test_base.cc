		...
		client_(GetOrCreateLocalClientOrDie(platform))
{
	...
	flags->xla_disable_hlo_passes = tensorflow::str_util::Join(
			disabled_pass_names, ",");
}
...
string ClientLibraryTestBase::ExecuteToString(ComputationBuilder* builder,
		tensorflow::gtl::ArraySlice<GlobalData*> arguments)
{
...
{
	EXPECT_IS_OK(
			ComputeAndCompareLiteralWithStatus(builder, expected, arguments,
					shape_with_layout));
}
		...
		const Shape* shape_with_layout)
{
	...
	if (ShapeUtil::ElementIsFloating(expected.shape())) {
		...
		TF_RET_CHECK(
				ShapeUtil::ElementIsIntegral(expected.shape())
...
void ClientLibraryTestBase::ComputeAndCompareR1U8(ComputationBuilder* builder,
		...
		tensorflow::gtl::ArraySlice<GlobalData*> arguments)
{
	...
	std::unique_ptr<Literal> expected_literal = LiteralUtil::CreateR1U8(
			expected);
...
void ClientLibraryTestBase::ComputeAndCompareTuple(ComputationBuilder* builder,
...
Computation ClientLibraryTestBase::CreateScalarRelu()
{
	...
	auto z_value = builder.Parameter(0, ShapeUtil::MakeShape(F32, { }),
			"z_value");
...
Computation ClientLibraryTestBase::CreateScalarReluSensitivity()
{
	...
	auto activation = builder.Parameter(0, ShapeUtil::MakeShape(F32, { }),
			"activation");
	auto backprop = builder.Parameter(1, ShapeUtil::MakeShape(F32, { }),
			"backprop");
	auto zero = builder.ConstantR0<float>(0.0);
...
std::unique_ptr<Array2D<float>> ClientLibraryTestBase::CreatePatternedMatrixWithZeroPadding(
