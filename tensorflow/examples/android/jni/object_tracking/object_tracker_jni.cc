...
jboolean JNICALL OBJECT_TRACKER_METHOD(isObjectVisible)(JNIEnv* env,
		jobject thiz, jstring object_id);
...
JNIEXPORT
jbyteArray JNICALL OBJECT_TRACKER_METHOD(getKeypointsPacked)(JNIEnv* env,
		jobject thiz, jfloat scale_factor);

JNIEXPORT
jfloatArray JNICALL OBJECT_TRACKER_METHOD(getKeypointsNative)(JNIEnv* env,
		jobject thiz, jboolean only_found_);
...
JNIEXPORT
void JNICALL OBJECT_TRACKER_METHOD(initNative)
...
{
const char* const id_str = env->GetStringUTFChars(object_id, 0);
const TrackedObject* const object = get_object_tracker(env, thiz)->GetObject(
		id_str);
env->ReleaseStringUTFChars(object_id, id_str);
...
JNIEXPORT
void JNICALL OBJECT_TRACKER_METHOD(nextFrameNative)
(JNIEnv* env, jobject thiz,
                                                    jbyteArray y_data,
                                                    jbyteArray uv_data,
                                                    jlong timestamp,
...
JNIEXPORT
void JNICALL OBJECT_TRACKER_METHOD(forgetNative)
(JNIEnv* env, jobject thiz,
	jstring object_id) {
const char* const id_str = env->GetStringUTFChars(object_id, 0);
...
JNIEXPORT
jfloatArray JNICALL OBJECT_TRACKER_METHOD(getKeypointsNative)(JNIEnv* env,
	jobject thiz, jboolean only_found)
{
jfloat keypoint_arr[kMaxKeypoints * kKeypointStep];

const int number_of_keypoints = get_object_tracker(env, thiz)->GetKeypoints(
		only_found, keypoint_arr);

// Create and return the array that will be passed back to Java.
jfloatArray keypoints = env->NewFloatArray(number_of_keypoints * kKeypointStep);
if (keypoints == NULL) {
	LOGE("null array!");
	return NULL;
}
...
// Create and return the array that will be passed back to Java.
jbyteArray keypoints = env->NewByteArray(
		number_of_keypoints * bytes_per_keypoint);
...
env->SetByteArrayRegion(keypoints, 0, number_of_keypoints * bytes_per_keypoint,
		keypoint_arr);
