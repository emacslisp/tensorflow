...
/* static */XlaContext& XlaContext::Get(const OpKernelContext* ctx)
{
	...
	XlaContext* context;
	TF_CHECK_OK(
			ctx->resource_manager()->Lookup(ctx->step_container()->name(),
		...
		bool use_tuple_arg)
{
	...
	if (use_tuple_arg_ && num_parameters_ > 0) {
		xla::Shape tuple_shape = xla::ShapeUtil::MakeTupleShape(
				parameter_shapes);
		xla::ComputationDataHandle tuple = builder().Parameter(0, tuple_shape,
				"arg_tuple");
		for (int i = 0; i < args_.size(); ++i) {
			const XlaCompiler::Argument& arg = args_[i];
			if (arg.parameter < 0)
				continue;
			parameters_[arg.parameter] = builder().GetTupleElement(tuple,
					arg.parameter);
		}
	} else {
		for (int i = 0; i < args_.size(); ++i) {
			...
			if (arg.parameter < 0)
				continue;
			parameters_[arg.parameter] = builder().Parameter(arg.parameter,
					parameter_shapes[arg.parameter], strings::StrCat("arg", i));
		}
	}
	return Status::OK();
}

Status XlaContext::CollectResults(xla::Computation* computation,
		...
		int* num_nonconst_outputs)
{
	...
	if (handle.handle() > 0) {
		...
		xla::StatusOr < xla::Computation > computation_status =
				builder().Build();
...
XlaContext::XlaContext(XlaCompiler* compiler, xla::Client* client,
		...
		compiler_(compiler), xla_builder_(client, computation_name), allow_cpu_custom_calls_(
				allow_cpu_custom_calls), resolve_compile_time_constants_(
...
XlaContext::GetOrCreateRuntimeContextParameter()
{
	...
	if (has_context_parameter_)
		return context_parameter_;
	has_context_parameter_ = true;
	context_parameter_ = xla_builder_.Parameter(num_parameters_,
			xla::ShapeUtil::MakeOpaqueShape(), "tf_context");
	return context_parameter_;
}

string XlaContext::DebugString()
{
	return "TLA JIT context";
}
		...
		const xla::Literal& literal)
{
	...
	if (resolve_compile_time_constants_) {
		...
		mutex_lock l(mu_);
		retval_.emplace_back(retval_index,
				xla_builder_.ConstantLiteral(literal));
	}
	return Status::OK();
}
...
xla::ComputationBuilder& XlaContext::builder()
{
	return xla_builder_;
}
...
{
	return LookupOrCreate(type, &max_func_,
			[this, type] {
...
{
	return LookupOrCreate(type, &add_func_,
			[this, type] {
				const string type_string = DataTypeString(type);
...
const xla::Computation* XlaContext::LookupOrCreate(DataType type,
		ComputationMap* out, const std::function<xla::Computation()>& create)
{
	{
		mutex_lock l(mu_);
