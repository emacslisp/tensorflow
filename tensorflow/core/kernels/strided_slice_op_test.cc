...
static void SliceHelper(int iters, int size)
{
	...
	Node* node;
	TF_CHECK_OK(
			NodeBuilder(g->NewName("n"), "StridedSlice").Input(
					test::graph::Constant(g, input)).Input(
					test::graph::Constant(g, begin)).Input(
					test::graph::Constant(g, end)).Input(
					test::graph::Constant(g, strides)).Attr("T", dt).Finalize(g,
					&node));
...
static void BM_ValidateStridedSliceOp(int iters)
{
	...
	for (int i = 0; i < iters; ++i) {
		...
		TF_CHECK_OK(
				ValidateStridedSliceOp(&begin, &end, strides,
						...
						&slice_dim0, &begin_out, &end_out, &strides_out));
	}
}
