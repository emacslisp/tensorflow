...
/* static */port::StatusOr<std::unique_ptr<MachineManager>> MachineManager::Create(
		PlatformKind kind, DeviceOptions options, const PluginConfig &config)
{
	std::unique_ptr<MachineManager> machine_manager { new MachineManager { kind,
			options, config } };
...
MachineManager::MachineManager(PlatformKind platform,
		DeviceOptions device_options, const PluginConfig &config) :
		platform_(platform), device_options_(device_options), plugin_config_(
				config), min_numa_node_(0), limit_numa_node_(0)
{
}
...
port::Status MachineManager::Init()
{
	...
	auto status = executors_[0]->Init(0 /* = device_ordinal */,
			device_options_);
	if (!status.ok()) {
	...
	for (int device_ordinal = 1; device_ordinal < device_count;
			++device_ordinal) {
		StreamExecutor *stream_exec = new StreamExecutor { platform_,
				plugin_config_ };
		...
		limit_numa_node_ = std::max(limit_numa_node_,
				stream_exec->GetDeviceDescription().numa_node() + 1);

		if (!stream_exec->GetDeviceDescription().ecc_enabled()) {
			LOG(WARNING) << "ECC not enabled for device ordinal: "
					<< device_ordinal;
		}
...
int MachineManager::device_count() const
{
...
std::unique_ptr<std::map<std::pair<int, int>, bool>> MachineManager::GetPeerAccessMap()
{
...
StreamExecutor *MachineManager::executor_for_device(int device_ordinal) const
{
	...
	CHECK(0 <= device_ordinal && device_ordinal < device_count()) << "device "
			<< device_ordinal << " out of range with device count "
			<< device_count();
...
int MachineManager::ExecutorToNumaNode(const StreamExecutor *stream_exec) const
{
...
/* static */port::StatusOr<MachineManager *> MachineManager::CreateSingletonInternal(
		PlatformKind platform, DeviceOptions options,
		const PluginConfig &config)
{
	...
	VLOG(1) << "machine manager singleton is " << singleton_
			<< " with platform " << PlatformKindString(platform)
			<< " and device options " << options.ToString();
...
/* static */MachineManager *MachineManager::singleton()
{
	...
	if (singleton_ == nullptr) {
		...
		auto status = CreateSingletonInternal(platform, options,
				PluginConfig());
		if (!status.ok()) {
			LOG(FATAL)
