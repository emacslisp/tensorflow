...
tensorflow::Status LiteralFromAllocation(const Allocation* allocation,
		const Shape& literal_shape, Literal* literal)
{
	TF_ASSIGN_OR_RETURN(se::StreamExecutor * executor,
			allocation->backend()->stream_executor(
		...
		SessionModule* module)
{
	...
	for (const Allocation* allocation : arg_allocations) {
		TF_RETURN_IF_ERROR(
				LiteralFromAllocation(allocation, allocation->shape(),
		...
		const ServiceOptions& options)
{
	...
	TF_ASSIGN_OR_RETURN(execute_backend,
	...
	std::unique_ptr<Service> service(
			new Service(std::move(execute_backend),
					std::move(compute_constant_backend)));
	return std::move(service);
}

/* static */StatusOr<std::unique_ptr<Backend>> Service::CreateComputeConstantBackend()
{
...
/* static */void Service::DumpExecutedHlo(const HloModule& module,
		const string& label, const HloExecutionProfile* profile)
{
	...
	if (!flags->xla_generate_hlo_graph.empty()
		...
		hlo_graph_dumper::DumpGraph(*module.entry_computation(), label,
				flags->xla_hlo_graph_addresses, flags->xla_hlo_graph_layout,
				profile);
	}
...
Service::Service(std::unique_ptr<Backend> execute_backend,
		...
		execute_backend_(std::move(execute_backend)), compute_constant_backend_(
				std::move(compute_constant_backend))
{
		...
		ComputationResponse* result)
{
	...
	*result->mutable_computation() = computation_tracker_.NewComputation(
			arg->name());
	return tensorflow::Status::OK();
}
		...
		if (allocation->backend() != backend
			...
			return InvalidArgument(
					"argument %d is on device %s but computation will be executed "
							"on device %s", i,
					allocation->backend()->device_name(
							allocation->device_ordinal()).c_str(),
		...
		const ExecutionOptions& execution_options)
{
	...
	if (!execution_options.has_shape_with_output_layout()) {
		...
		TF_RETURN_IF_ERROR(
				ValidateResultShapeWithLayout(shape_with_output_layout,
	...
	module_config->set_fast_math_disabled(
			execution_options.disable_fast_math());
...
StatusOr<std::unique_ptr<Executable>> Service::BuildExecutable(
		...
		const tensorflow::gtl::ArraySlice<perftools::gputools::DeviceMemoryBase> arguments,
		Backend* backend, se::StreamExecutor* executor)
{
	...
	if (!executable_for_compute_constant
		...
		TF_ASSIGN_OR_RETURN(session_module,
				computation_tracker_.SnapshotComputation(
		...
		if (!directory_path.empty()) {
			string filename = tensorflow::strings::Printf(
					...
					versioned_handle.version);
			TF_RETURN_IF_ERROR(
					Executable::DumpToDirectory(directory_path, filename,
							*session_module));
		}
	}

	VLOG(1)
			<< tensorflow::strings::Printf("building executable %lld@v%lld",
					versioned_handle.handle.handle(), versioned_handle.version);
	...
	TF_ASSIGN_OR_RETURN(std::unique_ptr < Executable > executable,
			backend->compiler()->Compile(std::move(module),
					std::move(module_config), hlo_dumper, executor));
...
{
	std::shared_ptr<Executable> executable = compilation_cache_.LookUp(
	...
	TF_ASSIGN_OR_RETURN(std::unique_ptr < Executable > executable_unique_ptr,
...
StatusOr<std::vector<GlobalDataHandle>> Service::ExecuteParallelAndRegisterResult(
		...
		tensorflow::gtl::ArraySlice<string> result_tags)
{
	...
	auto stream_releaser = ::tensorflow::gtl::MakeCleanup(
			[backend, &streams]() {
	...
	for (int64 i = 0; i < executables.size(); i++) {
		...
		result_handles.push_back(
				allocation_tracker_.Register(backend,
						executors[i]->device_ordinal(), result,
	...
	for (int64 i = 0; i < result_handles.size(); ++i) {
		if (!streams[i]->BlockHostUntilDone()) {
			return InternalError("failed to complete execution for stream %lld",
					i);
		}
	}
		...
		const string& result_tag, ExecutionProfile* profile)
{
	...
	if (backend->Replicas().size() == 1) {
		...
		std::vector<
				tensorflow::gtl::ArraySlice<
						perftools::gputools::DeviceMemoryBase>> repeated_arguments(
				backend->Replicas().size(), arguments);
	...
	return allocation_tracker_.Register(backend, executor->device_ordinal(),
			result, executable->result_shape(), result_tag);
}
		...
		ExecuteParallelResponse* result)
{
	...
	for (int64 i = 0; i < arg->requests_size(); ++i) {
		...
		if (arg->requests(i).has_device_handle()) {
			executor =
					execute_backend_->stream_executors()[arg->requests(i).device_handle().handle()];
		} else {
		...
		VLOG(3)
				<< "ExecuteParallel created HloModuleConfig computation layout: "
	...
	TF_ASSIGN_OR_RETURN(std::vector < GlobalDataHandle > outputs,
			ExecuteParallelAndRegisterResult(executable_ptrs, all_arguments,
					execute_backend_.get(), executors, computation_names));
	...
	if (available_device_count < arg->device_count() * replicas) {
		return ResourceExhausted(
				"Requested device count (%lld) exceeds the number of available devices "
						"on the target (%lld)", arg->device_count(),
		...
		ExecuteResponse* result)
{
	...
	TF_ASSIGN_OR_RETURN(*result->mutable_output(),
			ExecuteAndRegisterResult(executable.get(), arguments,
		...
		ExecuteAsyncResponse* result)
{
	...
	auto output = allocation_tracker_.Register(execute_backend_.get(),
			execute_backend_->default_device_ordinal(), result_data,
			executable->result_shape(),
			"result of " + user_computation->name());
		...
		TransferToServerResponse* result)
{
	...
	if (arg->has_device_handle()) {
		TF_ASSIGN_OR_RETURN(stream_executor,
				execute_backend_->stream_executor(
	...
	*result->mutable_data() = allocation_tracker_.Register(
			execute_backend_.get(), stream_executor->device_ordinal(),
			allocation, shape,
			tensorflow::strings::StrCat("TransferToServer literal of size ",
					allocation_size));
		...
		TransferToInfeedResponse* result)
{
	...
	if (arg->replica_id() < 0 || arg->replica_id() >= replica_count) {
		return FailedPrecondition("%s",
				tensorflow::strings::StrCat("The replica_id=",
						arg->replica_id(),
		...
		ComputeConstantResponse* result)
{
	...
	if (!is_constant) {
		return InvalidArgument(
				"Operand to ComputeConstant depends on parameter.");
	}
	...
	*execution_options.mutable_shape_with_output_layout() =
	...
	if (arg->has_output_layout()) {
		TF_RETURN_IF_ERROR(
				LayoutUtil::ValidateLayoutForShape(arg->output_layout(),
						execution_options.shape_with_output_layout()));
...
tensorflow::Status Service::AddInstruction(const RequestT* arg,
		ResponseT* result,
		const std::function<StatusOr<ComputationDataHandle>(UserComputation*)>& adder)
{
	TF_ASSIGN_OR_RETURN(UserComputation * computation,
			computation_tracker_.Resolve(arg->computation()));
...
tensorflow::Status Service::Op(const OpRequest* arg, OpResponse* result)
{
	...
	case OpRequest::kCallRequest: {
		...
		handle = computation->AddCallInstruction(arg->call_request(),
				*to_apply);
		break;
	}
	case OpRequest::kConcatenateRequest:
		handle = computation->AddConcatenateInstruction(
				arg->concatenate_request());
		break;
	...
	case OpRequest::kCustomCallRequest:
		handle = computation->AddCustomCallInstruction(
				arg->custom_call_request());
		break;
	case OpRequest::kDynamicSliceRequest:
		handle = computation->AddDynamicSliceInstruction(
				arg->dynamic_slice_request());
		break;
	...
	case OpRequest::kReduceRequest: {
		...
		handle = computation->AddReduceInstruction(arg->reduce_request(),
				*to_apply);
		break;
	}
	...
	case OpRequest::kVariadicOpRequest:
		handle = computation->AddVariadicInstruction(
				arg->variadic_op_request());
		break;
	case OpRequest::kWhileRequest: {
		TF_ASSIGN_OR_RETURN(UserComputation * condition,
				computation_tracker_.Resolve(arg->while_request().condition()));
		TF_ASSIGN_OR_RETURN(UserComputation * body,
	...
	case OpRequest::kSendRequest: {
		TF_RETURN_IF_ERROR(
				channel_tracker_.RegisterSend(
						arg->send_request().channel_handle()));
		TF_RETURN_IF_ERROR(
				computation->AddSendInstruction(arg->send_request()));
		return tensorflow::Status::OK();
	}
	case OpRequest::kRecvRequest: {
		TF_RETURN_IF_ERROR(
				channel_tracker_.RegisterRecv(
						arg->recv_request().channel_handle()));
...
{
	TF_ASSIGN_OR_RETURN(std::unique_ptr < SessionModule > module,
