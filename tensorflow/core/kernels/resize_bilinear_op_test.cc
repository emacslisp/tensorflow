...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("resize_bilinear_op", "ResizeBilinear").Input(
		...
		for (int b = 0; b < batch; ++b) {
			for (int64 y = 0; y < out_height; ++y) {
				...
				const int64 bottom_y_index = std::min(
						static_cast<int64>(ceilf(in_y)), in_height - 1);
				const float y_lerp = in_y - top_y_index;
				for (int64 x = 0; x < out_width; ++x) {
					const float in_x = x * width_scale;
					const int64 left_x_index = static_cast<int64>(floorf(in_x));
					const int64 right_x_index = std::min(
							static_cast<int64>(ceilf(in_x)), in_width - 1);
					const float x_lerp = in_x - left_x_index;
					for (int c = 0; c < channels; ++c) {
						const float top_left = images(b, top_y_index,
								left_x_index, c);
						const float top_right = images(b, top_y_index,
								right_x_index, c);
						const float bottom_left = images(b, bottom_y_index,
								left_x_index, c);
						const float bottom_right = images(b, bottom_y_index,
								right_x_index, c);
			...
			int output_width, int output_height)
	{
		...
		AddInputFromArray<int32>(TensorShape( { 2 }), { output_width,
				output_height });
...
protected:
	ResizeBilinearOpAlignCornersTest()
	{
		TF_EXPECT_OK(
				NodeDefBuilder("resize_bilinear_op", "ResizeBilinear").Input(
						FakeInput(DT_FLOAT)).Input(FakeInput(DT_INT32)).Attr(
						"align_corners", true).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
};
