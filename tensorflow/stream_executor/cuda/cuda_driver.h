...
public:
	...
	static bool HostRegister(CudaContext* context, void *location,
			uint64 bytes);
	...
	static port::Status ContextSetSharedMemConfig(CudaContext* context,
	...
	static bool GetModuleSymbol(CudaContext* context, CUmodule module,
			const char *symbol_name, CUdeviceptr *dptr, size_t *bytes);
	...
	static bool SynchronousMemsetUint8(CudaContext* context,
	...
	static bool AsynchronousMemsetUint8(CudaContext* context,
			CUdeviceptr location, uint8 value, size_t uint32_count,
			CUstream stream);
	...
	static bool AsynchronousMemcpyD2H(CudaContext* context, void *host_dst,
			CUdeviceptr gpu_src, uint64 size, CUstream stream);
	static bool AsynchronousMemcpyH2D(CudaContext* context, CUdeviceptr gpu_dst,
			const void *host_src, uint64 size, CUstream stream);
	...
	typedef void (*StreamCallback)(CUstream stream, CUresult status,
			void *data);
	...
	static port::StatusOr<int> GetMaxOccupiedBlocksPerCore(CudaContext* context,
			CUfunction kernel, int threads_per_block,
...
class CudaContext {
public:
	CudaContext(CUcontext context, int64 id) :
