		...
		std::unordered_map<int, int>* node_to_stream_id)
{
	...
	if (node_to_stream_id == nullptr) {
		status.Update(
				errors::InvalidArgument(
						"Bad node_to_stream_id argument supplied."));
	}
	...
	if (VLOG_IS_ON(2)) {
		for (Node* n : order) {
			...
			for (const Edge* e : n->in_edges()) {
				VLOG(2) << "  Edge from " << e->src()->id() << "  "
						<< e->src()->name() << " fanout "
						<< e->src()->out_edges().size();
			}
		}
	}
	...
	for (Node* n : order) {
		...
		if (op == "_Send") {
			if (opts.send_stream >= 0)
				stream_id = opts.send_stream;
		} else if (op == "_Recv") {
			if (opts.recv_stream >= 0)
				stream_id = opts.recv_stream;
		} else if (op == "Const") {
			if (opts.const_stream >= 0)
				stream_id = opts.const_stream;
		} else {
			if (opts.compute_stream >= 0)
				stream_id = opts.compute_stream;
		}
