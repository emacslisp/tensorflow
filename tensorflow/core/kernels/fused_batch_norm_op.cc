...
struct FusedBatchNorm<CPUDevice, T> {
	void operator()(OpKernelContext* context, const Tensor& x_input,
			...
			const Tensor& estimated_variance_input, T epsilon, Tensor* y_output,
			Tensor* batch_mean_output, Tensor* batch_var_output,
			Tensor* saved_mean_output, Tensor* saved_var_output,
			TensorFormat tensor_format, bool is_training)
	{
		...
		typename TTypes<T>::ConstVec estimated_mean(
				estimated_mean_input.vec<T>());
		...
		T rest_size_adjust = static_cast<T>(rest_size)
				/ static_cast<T>(rest_size_minus_one);
		...
		auto scaling_factor =
				((variance + epsilon).rsqrt() * scale).eval().reshape(
						one_by_depth).broadcast(bcast_spec);
		auto x_scaled = x_centered * scaling_factor;
		auto x_shifted = x_scaled
...
struct FusedBatchNormGrad<CPUDevice, T> {
	void operator()(OpKernelContext* context, const Tensor& y_backprop_input,
			...
			Tensor* offset_backprop_output, TensorFormat tensor_format)
	{
		CHECK(tensor_format == FORMAT_NHWC)
		...
		typename TTypes<T, 4>::Tensor x_backprop(
				x_backprop_output->tensor<T, 4>());
		typename TTypes<T>::Vec scale_backprop(scale_backprop_output->vec<T>());
		typename TTypes<T>::Vec offset_backprop(
				offset_backprop_output->vec<T>());
		...
		auto x_mean_rest_by_depth = mean.reshape(one_by_depth).broadcast(
				bcast_spec);
		auto x_centered = (x_rest_by_depth - x_mean_rest_by_depth).eval();
		auto coef0 = (variance + epsilon).rsqrt();
		auto coef0_rest_by_depth = coef0.eval().reshape(one_by_depth).broadcast(
				bcast_spec);
		auto x_scaled = x_centered * coef0_rest_by_depth;

		auto y_backprop_rest_by_depth = y_backprop.eval().reshape(
				rest_by_depth);
		scale_backprop.device(d) = (y_backprop_rest_by_depth * x_scaled).sum(
				reduce_dims);
		auto y_backprop_sum = y_backprop_rest_by_depth.sum(reduce_dims);
		offset_backprop.device(d) = y_backprop_sum;

		auto y_backprop_sum_one_by_depth = y_backprop_sum.eval().reshape(
				one_by_depth);
		auto y_backprop_mean_one_by_depth = y_backprop_sum_one_by_depth
				* rest_size_inv;
		auto y_backprop_mean_rest_by_depth =
				y_backprop_mean_one_by_depth.broadcast(bcast_spec);
		auto y_backprop_centered = y_backprop_rest_by_depth
				- y_backprop_mean_rest_by_depth;
		auto coef1 = (scale * coef0).eval().reshape(one_by_depth).broadcast(
				bcast_spec);
		auto coef2 =
				(coef0.square()
						* (y_backprop_rest_by_depth * x_centered).mean(
								reduce_dims)).eval().reshape(one_by_depth).broadcast(
						bcast_spec);
		x_backprop.reshape(rest_by_depth).device(d) = coef1
				* (y_backprop_centered - x_centered * coef2);
	}
};
...
void Compute(OpKernelContext* context) override
{
	...
	if (is_training_) {
		OP_REQUIRES(context, estimated_mean.dim_size(0) == 0,
	...
	functor::FusedBatchNorm<Device, T>()(context, x, scale, offset,
			estimated_mean, estimated_variance, epsilon_, y, batch_mean,
			batch_var, saved_mean, saved_inv_var, tensor_format_, is_training_);
}
...
void Compute(OpKernelContext* context) override
{
	...
	OP_REQUIRES(context, saved_maybe_inv_var.dims() == 1,
	...
	OP_REQUIRES_OK(context,
			context->allocate_output(1, scale_offset_shape, &scale_backprop));
	Tensor* offset_backprop = nullptr;
	OP_REQUIRES_OK(context,
			context->allocate_output(2, scale_offset_shape, &offset_backprop));
	// two placeholders for estimated_mean and estimated_variance, which are
	// used for inference and thus not needed here for gradient computation.
	Tensor* placeholder_1 = nullptr;
	OP_REQUIRES_OK(context,
			context->allocate_output(3, TensorShape( { }), &placeholder_1));
	Tensor* placeholder_2 = nullptr;
	OP_REQUIRES_OK(context,
			context->allocate_output(4, TensorShape( { }), &placeholder_2));

	functor::FusedBatchNormGrad<Device, T>()(context, y_backprop, x, scale,
			saved_mean, saved_maybe_inv_var, epsilon_, x_backprop,
			scale_backprop, offset_backprop, tensor_format_);
}
