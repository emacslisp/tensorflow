		...
		const string& history_version)
{
	return io::JoinPath(ops_prefix,
			strings::StrCat("compat/ops_history.", history_version, ".pbtxt"));
}

OpCompatibilityLib::OpCompatibilityLib(const string& ops_prefix,
		const string& history_version, const std::set<string>* stable_ops) :
		ops_file_(io::JoinPath(ops_prefix, "ops.pbtxt")), op_history_file_(
				OpsHistoryFile(ops_prefix, history_version)), stable_ops_(
				stable_ops)
{
...
Status OpCompatibilityLib::ValidateCompatible(Env* env, int* changed_ops,
		int* added_ops, OpList* out_op_history)
{
	*changed_ops = 0;
	*added_ops = 0;
	...
	if (stable_ops_ != nullptr) {
		...
		for (int cur = 0;
				iter != stable_ops_->end() && cur < op_list_.op_size(); ++cur) {
	...
	{  // Read op history.
		...
		Status status = ReadFileToString(env, op_history_file_,
				&op_history_str);
		if (!errors::IsNotFound(status)) {
			if (!status.ok())
				return status;
			protobuf::TextFormat::ParseFromString(op_history_str,
					&in_op_history);
		}
	}
	...
	while (cur < op_list_.op_size() && start < in_op_history.op_size()) {
		...
		if (stable_ops_ != nullptr && stable_ops_->count(op_name) == 0) {
			...
			for (++cur; cur < op_list_.op_size(); ++cur) {
				if (op_list_.op(cur).name() != op_name)
					break;
			}
			...
			for (; end < in_op_history.op_size(); ++end) {
				if (in_op_history.op(end).name() != op_name)
					break;
			}
			...
			if (history_str != cur_str) {
				...
				for (int i = start; i < end; ++i) {
					TF_RETURN_IF_ERROR(
							OpDefCompatible(in_op_history.op(i),
									op_list_.op(cur)));
				}
				...
				if (start < end - 1) {
					TF_RETURN_IF_ERROR(
							OpDefAddedDefaultsUnchanged(in_op_history.op(start),
