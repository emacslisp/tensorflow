		...
		int64 parameter_number, const Shape& shape, const string& name)
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kParameter, shape));
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kTrace, ShapeUtil::MakeNil()));
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kConstant, literal->shape()));
	instruction->literal_ = std::move(literal);
	return instruction;
}

/* static */std::unique_ptr<HloInstruction> HloInstruction::CreateGetTupleElement(
		const Shape& shape, HloInstruction* operand, int64 index)
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kGetTupleElement, shape));
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kConvolution, shape));
	...
	instruction->convolution_dimension_numbers_ = MakeUnique<
			ConvolutionDimensionNumbers>(dimension_numbers);
	return instruction;
}

/* static */std::unique_ptr<HloInstruction> HloInstruction::CreateCrossReplicaSum(
		const Shape& shape, HloInstruction* operand)
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kCrossReplicaSum, shape));
	instruction->AppendOperand(operand);
	return instruction;
}
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kInfeed, shape));
...
{
	std::unique_ptr<HloInstruction> instruction = WrapUnique(
			new HloInstruction(HloOpcode::kOutfeed, ShapeUtil::MakeNil()));
	instruction->AppendOperand(operand);
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kSend, ShapeUtil::MakeNil()));
	instruction->AppendOperand(operand);
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kReverse, shape));
	instruction->AppendOperand(operand);
		...
		tensorflow::gtl::ArraySlice<int64> limit_indices)
{
	...
	instruction->AppendOperand(operand);
	instruction->slice_starts_.assign(start_indices.begin(),
			start_indices.end());
	instruction->slice_limits_.assign(limit_indices.begin(),
			limit_indices.end());
	return instruction;
}
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kDynamicSlice, shape));
	instruction->AppendOperand(operand);
...
/* static */std::unique_ptr<HloInstruction> HloInstruction::CreateDynamicUpdateSlice(
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kConcatenate, shape));
	for (auto operand : operands) {
		instruction->AppendOperand(operand);
	}
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kConvert, shape));
	instruction->AppendOperand(operand);
	return instruction;
}
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kReduce, shape));
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kReduceWindow, shape));
	instruction->AppendOperand(operand);
	instruction->AppendOperand(init_value);
	instruction->to_apply_ = reduce_computation;
	instruction->window_ = MakeUnique<Window>(window);
...
/* static */std::unique_ptr<HloInstruction> HloInstruction::CreateSelectAndScatter(
		const Shape& shape, HloInstruction* operand, HloComputation* select,
		const Window& window, HloInstruction* source,
		HloInstruction* init_value, HloComputation* scatter)
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kSelectAndScatter, shape));
	instruction->AppendOperand(operand);
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kBroadcast, shape));
	instruction->AppendOperand(operand);
		...
		const Shape& shape, HloInstruction* operand)
{
	...
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kReshape, shape));
	instruction->AppendOperand(operand);
	return instruction;
}
		...
		tensorflow::gtl::ArraySlice<int64> dimensions)
{
	...
	CHECK(
			std::equal(operand->shape().dimensions().begin(),
					operand->shape().dimensions().end(),
					Permute(dimensions, shape.dimensions()).begin()));
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kTranspose, shape));
	instruction->AppendOperand(operand);
	instruction->dimensions_.assign(dimensions.begin(), dimensions.end());
	return instruction;
}
...
{
	auto instruction = WrapUnique(
			new HloInstruction(HloOpcode::kFusion, shape));
...
/* static */std::unique_ptr<HloInstruction> HloInstruction::CreateFusionForBackwardConvolution(
		const Shape& shape, FusionKind fusion_kind, const Window& window,
		const ConvolutionDimensionNumbers& conv_dnums,
		HloInstruction* fused_root)
{
	std::unique_ptr<HloInstruction> fusion = CreateFusion(shape, fusion_kind,
			fused_root);
	fusion->window_ = MakeUnique<Window>(window);
	fusion->convolution_dimension_numbers_ = MakeUnique<
		...
		HloInstruction* instruction_to_merge)
{
	...
	TF_CHECK_OK(
			instruction_to_merge->ReplaceUseWith(this,
		...
		HloInstruction* instruction_to_fuse)
{
	...
	HloInstruction* fused_instruction = CloneAndFuseInternal(
			instruction_to_fuse);
		...
		HloInstruction* instruction_to_fuse)
{
	...
	if (new_fusion_instruction) {
		...
		CHECK(
				std::find(operands_.begin(), operands_.end(),
						instruction_to_fuse) != operands_.end());
		for (int64 operand_num = 0; operand_num < operand_count();
				++operand_num) {
			if (instruction_to_fuse == operands_[operand_num]) {
				...
				// respective vectors.
				fused_parameters_.erase(
						fused_parameters_.begin() + operand_num);
				...
				while (operand_num < fused_parameters_.size()) {
					fused_parameters_[operand_num]->parameter_number_ =
							operand_num;
					operand_num++;
				}
				// Throw removed fused parameter instruction away.
				auto inst_it = std::find_if(fused_instructions_.begin(),
	...
	for (int64 operand_num = 0; operand_num < clone->operand_count();
		...
		if (fused_param == nullptr) {
			...
			std::unique_ptr<HloInstruction> param_instruction = CreateParameter(
					param_no, operand->shape(), "fusion_param");
...
void CalledComputationsInternal(const HloInstruction& instruction,
...
void HloInstruction::CheckFusionInstruction() const
{
	...
	for (auto fused_param : fused_parameters_) {
		...
		CHECK(
				ShapeUtil::Compatible(fused_param->shape(),
...
{
	std::unique_ptr<HloInstruction> instruction = WrapUnique(
			new HloInstruction(HloOpcode::kCall, shape));
	for (auto operand : operands) {
		instruction->AppendOperand(operand);
	}
...
{
	std::unique_ptr<HloInstruction> instruction = WrapUnique(
			new HloInstruction(HloOpcode::kCustomCall, shape));
	for (auto operand : operands) {
		instruction->AppendOperand(operand);
	}
		...
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	...
	case HloOpcode::kTrace:
		LOG(FATAL) << "Not yet implemented, clone: "
...
{
	std::unique_ptr<HloInstruction> clone = CloneWithNewOperands(shape_,
			operands_);
		...
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	...
	auto new_instruction = WrapUnique(
			new HloInstruction(HloOpcode::kFusion, shape));
	...
	for (HloInstruction* old_fused_parameter : fused_parameters_) {
		...
		HloInstruction* new_fusion_parameter =
				new_fused_instructions.back().get();
		new_fusion_parameter->parent_fusion_instruction_ =
				new_instruction.get();
	...
	for (auto old_fused_instruction_iter = fused_instructions_.rbegin();
		...
		HloInstruction* old_fused_instruction =
				old_fused_instruction_iter->get();
		...
		for (int64 operand_idx = 0;
				operand_idx < old_fused_instruction->operand_count();
				++operand_idx) {
		...
		HloInstruction* new_fused_instruction =
				new_fused_instructions.back().get();
		new_fused_instruction->parent_fusion_instruction_ =
				new_instruction.get();
...
bool HloInstruction::Identical(const HloInstruction& other,
		std::function<bool(const HloInstruction*, const HloInstruction*)> eq_operands,
		std::function<bool(const HloComputation*, const HloComputation*)> eq_computations) const
{
...
bool HloInstruction::IsRank2Transpose() const
{
	return (opcode_ == HloOpcode::kTranspose)
			...
			&& std::equal(shape_.dimensions().begin(),
					shape_.dimensions().end(),
		...
		HloInstruction* new_producer)
{
	...
	TF_RET_CHECK(user_it != users_.end()) << "Instruction " << user->name()
			<< " not a use of instruction " << name();
	users_.erase(user_it);
	...
	TF_RET_CHECK(
			std::count(user->operands_.begin(), user->operands_.end(), this)
					>= 0);
		...
		HloInstruction* new_operand)
{
	...
	TF_RET_CHECK(
			...
			<< old_operand->shape().ShortDebugString()
			<< " is not compatible with "
			<< new_operand->shape().ShortDebugString();
	operands_[operand_num] = new_operand;

	VLOG(3) << "Replacing operand " << operand_num << " of " << name()
...
{
	string operands = tensorflow::str_util::Join(operands_, ", ",
...
string HloInstruction::ToString(bool compact_operands) const
{
	...
	if (opcode() == HloOpcode::kConstant) {
		...
		if (ShapeUtil::ElementsIn(shape()) <= 10) {
			...
			for (const auto& s : v) {
				if (s.empty())
					continue;
				tensorflow::strings::StrAppend(&operands, (first ? "" : " "),
						s);
				first = false;
			}
		} else {
		...
		operands =
				tensorflow::str_util::Join(slice, ", ",
						[&](string* out, HloInstruction* operand) {
	...
	if (CanHaveDimensionsField()) {
		tensorflow::strings::StrAppend(&extra, ", dimensions={",
	...
	if (!slice_starts_.empty() && !slice_limits_.empty()) {
		...
		for (int i = 0; i < slice_starts_.size(); ++i) {
			bounds.push_back(
					tensorflow::strings::StrCat("[", slice_starts_[i], ":",
							slice_limits_[i], "]"));
		}
		tensorflow::strings::StrAppend(&extra, ", slice={",
				tensorflow::str_util::Join(bounds, ", "), "}");
	}
	if (convolution_dimension_numbers_ != nullptr) {
		...
		for (int64 i = 0; i < dnums.spatial_dimensions().size(); ++i) {
			lhs_dims[dnums.spatial_dimensions(i)] = tensorflow::strings::StrCat(
					i);
		}

		std::vector<string> rhs_dims(
				2 + dnums.kernel_spatial_dimensions().size());
	...
	if (to_apply_ != nullptr) {
		tensorflow::strings::StrAppend(&extra, ", computation=",
				to_apply_->name());
	}
	...
	return tensorflow::strings::Printf("%s = %s %s(%s)%s", name().c_str(),
...
{
	return tensorflow::strings::Printf("%s = %s(%s)", name().c_str(),
...
Status HloInstruction::Visit(DfsHloVisitor* visitor)
{
	...
	case HloOpcode::kDynamicUpdateSlice:
		return visitor->HandleDynamicUpdateSlice(this, operands_[0],
				operands_[1], operands_[2]);
	case HloOpcode::kSort:
...
Status HloInstruction::AcceptInternal(DfsHloVisitor* visitor)
{
	...
	for (auto operand : operands_) {
		VLOG(3) << "Going to visit HLO " << operand->name()
				<< " as operand of HLO " << name();
	...
	for (auto* instruction : order) {
		for (auto* operand : instruction->operands()) {
			if (order_position.count(operand) == 0
					|| order_position.at(operand)
							>= order_position.at(instruction)) {
				return false;
			}
		}
	}
...
Status HloInstruction::AcceptOrdered(DfsHloVisitor* visitor,
		const std::vector<const HloInstruction*>& order)
{
	...
	TF_RETURN_IF_ERROR(
			this->Accept([&predecessors](HloInstruction* instruction) {
		...
		const HloInstruction* operand)
{
	...
	return std::all_of(operand_indices.begin(), operand_indices.end(),
...
std::tuple<bool, std::vector<int64>, std::vector<int64>> HloInstruction::ReshapeMerelyInsertsOrDeletes1SizedDimensions() const
{
	if (HloOpcode::kReshape != opcode_) {
		return std::make_tuple(false, std::vector<int64>(),
				std::vector<int64>());
	}
