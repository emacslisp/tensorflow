...
int main(int argc, char **argv)
{
	...
	if (argc < 2) {
		LOG(QFATAL) << "Usage: " << argv[0]
				<< " <path-to-serialized-literal-text>";
	}

	std::unique_ptr<xla::Literal> literal = xla::TextLiteralReader::ReadPath(
			argv[1]).ConsumeValueOrDie();

	LOG(INFO) << "literal: " << literal->ShortDebugString();
	fprintf(stderr, "%s\n", xla::LiteralUtil::ToString(*literal).c_str());
	if (literal->shape().element_type() == xla::F32) {
		float min = *std::min_element(literal->f32s().begin(),
				literal->f32s().end());
		float max = *std::max_element(literal->f32s().begin(),
				literal->f32s().end());
