		...
		const MetaGraphDefTwiddler& twiddler)
{
	...
	}
	{
		...
		twiddler(&graph_def);
		TF_RETURN_IF_ERROR(
				WriteStringToFile(Env::Default(), sink,
						graph_def.SerializeAsString()));
	}
	return Status::OK();
}
		...
		const SessionBundle& bundle)
{
	...
	const string input_name =
			signatures.named_signatures().at("inputs").generic_signature().map().at(
					"x").tensor_name();
	const string output_name =
			signatures.named_signatures().at("outputs").generic_signature().map().at(
					"y").tensor_name();

	// Validate the half plus two behavior.
	Tensor input = test::AsTensor<float>( { 0, 1, 2, 3 },
			TensorShape( { 4, 1 }));
	std::vector<Tensor> outputs;
	TF_ASSERT_OK(
			bundle.session->Run( { { input_name, input } }, { output_name },
...
void CheckSessionBundle(const string& export_path, const SessionBundle& bundle)
{
	...
	TF_ASSERT_OK(
			bundle.session->Run( { }, { "filename1:0", "filename2:0" }, { },
					&path_outputs));
...
protected:
			...
			const string& meta_graph_def_filename)
	{
		...
		const string export_path = io::JoinPath(testing::TmpDir(),
				strings::StrCat(test_info->test_case_name(),
						test_info->name()));
		TF_CHECK_OK(
				CopyExport(export_path, variables_filename,
