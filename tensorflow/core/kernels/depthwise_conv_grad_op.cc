...
static void CopyOutputBackpropRegion(const DepthwiseArgs& args,
		const int64 padded_filter_inner_dim_size, const int64 in_r,
		const int64 in_c, const T* out_backprop, T* buffer)
{
	...
	const int64 out_r_start = std::max(static_cast<int64>(0),
			(in_r - filter_rows + pad_rows + stride) / stride);
	const int64 out_r_end = std::min(out_rows - 1, (in_r + pad_rows) / stride);
	const int64 out_c_start = std::max(static_cast<int64>(0),
	...
	for (int out_r = out_r_start; out_r <= out_r_end; ++out_r) {
		...
		for (int out_c = out_c_start; out_c <= out_c_end; ++out_c) {
			...
			const int64 buf_base = (f_r * filter_cols + f_c)
					* padded_filter_inner_dim_size;
			// Calculate index into 'out_backprop' for coordinate (out_r, out_c).
			auto* out_bprop = out_backprop
					+ (out_r * args.out_cols + out_c) * args.out_depth;
			...
			for (int64 d = 0; d < scalar_size; ++d) {
				buffer[buf_base + vectorized_size + d] =
						out_bprop[vectorized_size + d];
			}
...
static void ComputeBackpropInput(const DepthwiseArgs& args,
		const int64 padded_filter_inner_dim_size, const int64 in_r,
		const int64 in_c, const T* filter, const T* buffer, T* out_buffer,
		T* output)
{
	typedef typename Eigen::internal::packet_traits<T>::type Packet;
	static const int64 kPacketSize = (sizeof(Packet) / sizeof(T));
	...
	const int64 output_vectorized_size = (out_depth / kPacketSize)
			* kPacketSize;
	...
	const int64 dm_vectorized_size = (depth_multiplier / kPacketSize)
			* kPacketSize;
	...
	for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
		...
		for (int j = 0; j < filter_spatial_size; ++j) {
			...
			// Load filter.
			const auto filter_block = Eigen::internal::ploadu < Packet
					> (filter + index);
			// Load input.
			const auto data_block = Eigen::internal::ploadu < Packet
					> (buffer + index);
			// Vector multiply-add.
			vaccum = Eigen::internal::pmadd < Packet
					> (filter_block, data_block, vaccum);
		}
		if (depth_multiplier == 1) {
			// Write directly to the output.
			Eigen::internal::pstoreu < T
					> (output + base_output_index + i, vaccum);
		} else {
	...
	if (output_scalar_size > 0) {
		auto vaccum = Eigen::internal::pset1 < Packet > (0);
		for (int j = 0; j < filter_spatial_size; ++j) {
			const int64 index = output_vectorized_size
					+ j * padded_filter_inner_dim_size;
			const auto filter_block = Eigen::internal::ploadu < Packet
					> (filter + index);
			const auto data_block = Eigen::internal::ploadu < Packet
					> (buffer + index);
		...
		if (depth_multiplier == 1) {
			...
			for (int j = 0; j < output_scalar_size; ++j) {
				output[base_output_index + output_vectorized_size + j] =
						out_buf[j];
			}
		} else {
			// Buffer output for subsequent reduction step.
	...
	if (depth_multiplier > 1) {
		for (int64 d = 0; d < in_depth; ++d) {
			...
			for (int64 dm = 0; dm < dm_vectorized_size; dm += kPacketSize) {
				const auto v = Eigen::internal::ploadu < Packet
						> (out_buffer + index + dm);
...
struct LaunchDepthwiseConvBackpropInputOp<CPUDevice, T> {
	...
	static void launch(OpKernelContext* ctx, const DepthwiseArgs& args,
			const T* out_backprop, const T* depthwise_filter, T* in_backprop)
	{
		static const int64 kPacketSize = (sizeof(Packet) / sizeof(T));

		// Pad 'depthwise_filter' to vector register width (if needed).
		const bool pad_filter =
				(args.out_depth % kPacketSize) == 0 ? false : true;
		Tensor padded_filter;
		if (pad_filter) {
			// Allocate space for padded filter.
			const int64 filter_spatial_size = args.filter_rows
					* args.filter_cols;
			const int64 padded_filter_inner_dim_size = ((args.out_depth
			...
			functor::DepthwiseFilterPadOp<T>()(args, depthwise_filter,
					padded_filter.template flat<T>().data());
		}
		const T* filter_data = pad_filter ? padded_filter.template flat<T>().data() : depthwise_filter;

		// Computes one shard of depthwise conv2d backprop input.
		auto shard =
				[&ctx, &args, &out_backprop, &filter_data, &in_backprop](
			...
			for (int in_c = 0; in_c < args.in_cols; ++in_c) {
				for (int in_d = 0; in_d < args.in_depth; ++in_d) {
					...
					for (int out_d = out_d_start; out_d < out_d_end; ++out_d) {
						const int out_r_start = std::max(0,
								(in_r - args.filter_rows + args.pad_rows
										+ stride) / stride);
						const int out_r_end = std::min(args.out_rows - 1,
								(in_r + args.pad_rows) / stride);

						for (int out_r = out_r_start; out_r <= out_r_end;
								++out_r) {
							const int out_c_start = std::max(0,
									(in_c - args.filter_cols + args.pad_cols
											+ stride) / stride);
							const int out_c_end = std::min(args.out_cols - 1,
									(in_c + args.pad_cols) / stride);

							for (int out_c = out_c_start; out_c <= out_c_end;
									++out_c) {
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, TensorShapeUtils::IsVector(input_sizes.shape()),
				errors::InvalidArgument(
		...
		LaunchDepthwiseConvBackpropInputOp<Device, T>::launch(context, args,
				out_backprop_ptr, filter_ptr, in_backprop_ptr);
	}
...
static void ComputeBackpropFilter(const DepthwiseArgs& args,
		const int64 padded_out_depth_size, const int64 out_r, const int64 out_c,
		const T* out_backprop, const T* input_buffer, T* output_buffer)
{
	typedef typename Eigen::internal::packet_traits<T>::type Packet;
	static const int64 kPacketSize = (sizeof(Packet) / sizeof(T));
	...
	const int64 out_depth = args.out_depth;
	const int64 filter_spatial_size = args.filter_rows * args.filter_cols;
	const int64 output_vectorized_size = (padded_out_depth_size / kPacketSize)
			* kPacketSize;
	const int64 base_output_index = (out_r * args.out_cols + out_c) * out_depth;
	// Determine whether we can execute fast or slow code path.
	const int64 output_image_size = args.out_rows * args.out_cols
			* args.out_depth;
	const int64 output_last_vector_index = output_image_size
	...
	if (fast_path) {
		...
		for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
			...
			const auto out_bprop_block = Eigen::internal::ploadu < Packet
					> (out_backprop + base_output_index + i);
			for (int j = 0; j < filter_spatial_size; ++j) {
				const int64 index = i + j * padded_out_depth_size;
				// Load vector register from 'input_buffer'.
				const auto input_block = Eigen::internal::ploadu < Packet
						> (input_buffer + index);
				// Load output block into vector register.
				auto out_block_data = output_buffer + index;
				auto out_block = Eigen::internal::ploadu < Packet
						> (out_block_data);
				// Vector multiply-add.
				out_block = Eigen::internal::pmadd < Packet
						> (out_bprop_block, input_block, out_block);
		...
		for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
			...
			const int64 out_bprop_limit = std::min(output_image_size,
					out_bprop_index + kPacketSize);
			T out_buf[kPacketSize];
...
struct LaunchDepthwiseConvBackpropFilterOp<CPUDevice, T> {
			...
			const T* out_backprop, const T* input, T* filter_backprop)
	{
		...
		const int64 filter_spatial_size = args.filter_rows * args.filter_cols;
		const int64 padded_out_depth_size = ((args.out_depth + kPacketSize - 1)
		...
		const int64 padded_filter_size = filter_spatial_size
		...
		for (int64 i = 0; i < filter_spatial_size; ++i) {
			...
			for (int64 j = 0; j < vectorized_size; j += kPacketSize) {
				...
				for (int b = 0; b < args.batch; ++b) {
					...
					const auto* output_buffer = output_buffer_data
							+ b * padded_filter_size;
					const auto v = Eigen::internal::ploadu < Packet
							> (output_buffer + buffer_base + j);
					// Add 'v' to 'out_block'.
						...
						for (int f_c = 0; f_c < args.filter_cols; ++f_c) {
							...
							if (in_r >= 0 && in_r < args.in_rows && in_c >= 0
								...
								filter_backprop[filter_backprop_offset] +=
										input[input_offset]
												* out_backprop[out_backprop_offset];
							}
						}
					}
				}
...
public:
	explicit DepthwiseConv2dNativeBackpropFilterOp(
	...
	void Compute(OpKernelContext* context) override
	{
		...
		LaunchDepthwiseConvBackpropFilterOp<Device, T>::launch(context, args,
				out_backprop_ptr, input_ptr, filter_backprop_ptr);
	}
