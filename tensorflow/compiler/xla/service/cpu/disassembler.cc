		...
		subtarget_info_(*target_machine.getMCSubtargetInfo())
{
	...
	mc_context_.reset(
			new llvm::MCContext(target_machine.getMCAsmInfo(),
					target_machine.getMCRegisterInfo(), objfile_info_.get()));
	disassembler_.reset(
			target_machine.getTarget().createMCDisassembler(subtarget_info_,
					*mc_context_));
	inst_printer_.reset(
			target_machine.getTarget().createMCInstPrinter(
	...
	inst_analysis_.reset(
			target_machine.getTarget().createMCInstrAnalysis(
					target_machine.getMCInstrInfo()));
}
		...
		const llvm::object::ObjectFile& object_file) const
{
	...
	for (auto& section : object_file.sections()) {
		...
		for (int i = 0; i < symbols.size(); ++i) {
			...
			uint64_t end_index;
			if (i < symbols.size() - 1) {
				llvm::Expected<uint64_t> next_address =
						symbols[i + 1].getAddress();
				CHECK(next_address);
			...
			while (index < end_index) {
				...
				if (decode_status == llvm::MCDisassembler::Success) {
					...
					string annotation;
					if (inst_analysis_
							&& (inst_analysis_->isUnconditionalBranch(
									instruction)
									|| inst_analysis_->isConditionalBranch(
											instruction))) {
						uint64_t target;
						if (inst_analysis_->evaluateBranch(instruction,
								section_address + index, size, target)) {
							annotation = tensorflow::strings::Printf(
									"[0x%08lx]", target);
						}
					}
					inst_printer_->printInst(&instruction, ostream,
							annotation.c_str(), subtarget_info_);
				} else {
					ostream << " <unknown>";
				}
