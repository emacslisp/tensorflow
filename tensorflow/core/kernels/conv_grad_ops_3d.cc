...
public:
	explicit Conv3DBackpropInputOp(OpKernelConstruction* context) :
			OpKernel(context), takes_shape_(
					type_string().find("V2") != std::string::npos)
	{
		OP_REQUIRES_OK(context, context->GetAttr("strides", &stride_));
		OP_REQUIRES(context, stride_.size() == 5,
				errors::InvalidArgument("Sliding window strides field must "
						"specify 5 dimensions"));
		OP_REQUIRES(context, (stride_[0] == 1 && stride_[4] == 1),
				errors::InvalidArgument(
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (takes_shape_) {
			...
			OP_REQUIRES_OK(context,
					TensorShapeUtils::MakeShape(input_sizes.vec<int32>(),
							&input_shape));
		} else {
		...
		Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 5> pad_dims {
				{ 0, 0 }, { top_pad_planes, bottom_pad_planes }, { top_pad_rows,
		...
		TensorShape padded_out_shape( { batch, padded_out_planes,
				padded_out_rows, padded_out_cols, out_depth });
		Tensor padded_output;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::v(), padded_out_shape,
						&padded_output));
		Eigen::DSizes < Eigen::DenseIndex, 5 > no_op_shuffle { 0, 1, 2, 3, 4 };
		Eigen::DSizes < Eigen::DenseIndex, 5 > eigen_strides { 1, strides[0],
				strides[1], strides[2], 1 };
		functor::InflatePadAndShuffle<Device, T, 5, Eigen::DenseIndex>()(
				context->eigen_device<Device>(), out_backprop.tensor<T, 5>(),
				eigen_strides, pad_dims, no_op_shuffle,
				padded_output.tensor<T, 5>());
		...
		TensorShape r_filter_shape( { filter_size[0], filter_size[1],
				filter_size[2], out_depth, in_depth });
		Tensor r_filter;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::v(), r_filter_shape,
						&r_filter));
		...
		functor::ShuffleAndReverse<Device, T, 5, Eigen::DenseIndex>()(
				context->eigen_device<Device>(), filter.tensor<T, 5>(),
				filter_order, filter_rev_dims, r_filter.tensor<T, 5>());
		const Tensor& r_filter_cref = r_filter;

		// Now we can call conv_3d directly.
		functor::CuboidConvolution<Device, T>()(context->eigen_device<Device>(),
				in_backprop->tensor<T, 5>(), padded_output_cref.tensor<T, 5>(),
				r_filter_cref.tensor<T, 5>(), 1, 1, 1,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (takes_shape_) {
			...
			OP_REQUIRES_OK(context,
					TensorShapeUtils::MakeShape(filter_sizes.vec<int32>(),
							&filter_shape));
		} else {
		...
		TensorShape padded_out_shape( { out_depth, padded_out_planes,
				padded_out_rows, padded_out_cols, batch });
		Tensor padded_output;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::v(), padded_out_shape,
						&padded_output));
		...
		functor::InflatePadAndShuffle<Device, T, 5, Eigen::DenseIndex>()(
				context->eigen_device<Device>(), out_backprop.tensor<T, 5>(),
				eigen_strides, pad_dims, out_order,
				padded_output.tensor<T, 5>());
		const Tensor& padded_output_cref = padded_output;
		...
		TensorShape in_shuffle_shape( { input_size[0], input_size[1],
				input_size[2], batch, in_depth });
		Tensor in_shuffle;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::v(), in_shuffle_shape,
						&in_shuffle));
		...
		// output.
		TensorShape filter_shuffle_shape( { out_depth, filter_size[0],
				filter_size[1], filter_size[2], in_depth });
		Tensor filter_shuffle;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::v(),
						filter_shuffle_shape, &filter_shuffle));
		functor::CuboidConvolution<Device, T>()(context->eigen_device<Device>(),
				filter_shuffle.tensor<T, 5>(),
				padded_output_cref.tensor<T, 5>(),
				in_shuffle_cref.tensor<T, 5>(), 1, 1, 1,
				BrainPadding2EigenPadding(VALID));
		...
		functor::ShuffleAndReverse<Device, T, 5, Eigen::DenseIndex>()(
				...
				filter_shuffle_cref.tensor<T, 5>(), filter_order,
				filter_rev_dims, filter_backprop->tensor<T, 5>());
	}
