...
public:
	explicit PoolingOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		...
		padding_ =
				(padding == VALID) ? xla::Padding::kValid : xla::Padding::kSame;
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		xla::ComputationDataHandle pooled = ctx->builder()->ReduceWindow(input,
				InitValue(ctx->builder(), type), *Reduction(ctx, type), ksize_,
				stride_, padding_);
...
public:
	explicit MaxPoolOp(OpKernelConstruction* ctx) :
			PoolingOp(ctx)
	{
	}
	...
	const xla::Computation* Reduction(XlaOpKernelContext* ctx, DataType dtype) override
	{
		return ctx->GetOrCreateMax(dtype);
	}

	xla::ComputationDataHandle PostProcessOutput(XlaOpKernelContext* ctx,
			const xla::ComputationDataHandle& output, DataType dtype,
...
static xla::ComputationDataHandle AvgPoolDivideByCount(XlaOpKernelContext* ctx,
		const xla::ComputationDataHandle& output, DataType dtype,
		...
		TensorFormat data_format)
{
	if (padding == xla::Padding::kValid) {
		...
		auto divisor = XlaHelpers::IntegerLiteral(ctx->builder(), dtype,
				window_size);
		...
		auto ones = ctx->builder()->Broadcast(
				...
				{ input_shape.dim_size(height_dim), input_shape.dim_size(
						width_dim) });
		...
		auto counts = ctx->builder()->ReduceWindow(ones,
				XlaHelpers::Zero(ctx->builder(), dtype),
				*ctx->GetOrCreateAdd(dtype), { ksize[height_dim],
						ksize[width_dim] }, { stride[height_dim],
						stride[width_dim] }, xla::Padding::kSame);
...
public:
	explicit AvgPoolOp(OpKernelConstruction* ctx) :
			PoolingOp(ctx)
	{
...
public:
	explicit MaxPoolGradOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		string data_format;
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		xla::Padding xla_padding =
				(padding_ == VALID) ?
						xla::Padding::kValid : xla::Padding::kSame;

		xla::PrimitiveType element_type;
		OP_REQUIRES_OK(ctx,
				DataTypeToPrimitiveType(input_type(2), &element_type));
		xla::ComputationDataHandle init_value = XlaHelpers::Zero(ctx->builder(),
				input_type(2));
		...
		xla::ComputationDataHandle gradients = ctx->builder()->SelectAndScatter(
				input, select, ksize_, stride_, xla_padding, out_backprop,
				init_value, scatter);
...
public:
	explicit AvgPoolGradOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		string data_format;
		OP_REQUIRES_OK(ctx, ctx->GetAttr("data_format", &data_format));
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, gradients_shape.dims() == 4,
				errors::InvalidArgument(
						"orig_input_shape must have 4 elements"));
		...
		TensorShape filter_shape( { ksize_[height_dim], ksize_[width_dim],
				depth, depth });

		// Reuse the logic from Conv2DBackpropInput to compute padding.
		Conv2DBackpropDimensions dims;
		OP_REQUIRES_OK(ctx,
				Conv2DBackpropComputeDimensions("AvgPoolGrad", gradients_shape,
						filter_shape, out_backprop_shape, stride_, padding_,
						data_format_, &dims));
		...
		auto out_backprop_div = AvgPoolDivideByCount(ctx, out_backprop, dtype,
				gradients_shape, xla_padding, ksize_, stride_int64s,
				data_format_);
		...
		auto padded_gradients = ctx->builder()->Pad(out_backprop_div, zero,
				padding_config);
