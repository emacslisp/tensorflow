...
public:
	...
	CURLcode curl_easy_setopt(CURL* curl, CURLoption option, uint64 param) override
	{
		return ::curl_easy_setopt(curl, option, param);
	}

	CURLcode curl_easy_setopt(CURL* curl, CURLoption option, const char* param) override
	{
		return ::curl_easy_setopt(curl, option, param);
	}

	CURLcode curl_easy_setopt(CURL* curl, CURLoption option, void* param) override
	{
		return ::curl_easy_setopt(curl, option, param);
	}

	CURLcode curl_easy_setopt(CURL* curl, CURLoption option,
			size_t (*param)(void*, size_t, size_t, FILE*)) override
	{
		return ::curl_easy_setopt(curl, option, param);
	}

	CURLcode curl_easy_setopt(CURL* curl, CURLoption option,
			size_t (*param)(const void*, size_t, size_t, void*)) override
	{
		return ::curl_easy_setopt(curl, option, param);
	}
	...
	CURLcode curl_easy_getinfo(CURL* curl, CURLINFO info, uint64* value) override
	{
...
Status HttpRequest::Init()
{
	...
	libcurl_->curl_easy_setopt(curl_, CURLOPT_USERAGENT,
...
Status HttpRequest::AddHeader(const string& name, const string& value)
{
	TF_RETURN_IF_ERROR(CheckInitialized());
	TF_RETURN_IF_ERROR(CheckNotSent());
	curl_headers_ = libcurl_->curl_slist_append(curl_headers_,
			strings::StrCat(name, ": ", value).c_str());
	return Status::OK();
}
...
Status HttpRequest::AddAuthBearerHeader(const string& auth_token)
{
	...
	if (!auth_token.empty()) {
		return AddHeader("Authorization",
				strings::StrCat("Bearer ", auth_token));
	}
	return Status::OK();
}
...
Status HttpRequest::SetPutFromFile(const string& body_filepath, size_t offset)
{
	...
	if (!put_body_) {
		return errors::InvalidArgument(
				"Couldn't open the specified file: " + body_filepath);
	}
	...
	curl_headers_ = libcurl_->curl_slist_append(curl_headers_,
			strings::StrCat("Content-Length: ", size).c_str());
		...
		void* this_object)
{
	...
	that->response_buffer_->insert(that->response_buffer_->end(),
		...
		FILE* this_object)
{
	...
	const size_t bytes_to_copy = std::min(size * nmemb,
			that->post_body_buffer_.size() - that->post_body_read_);
		...
		void* this_object)
{
	...
	if (strings::Scanner(header).ScanEscapedUntil(':').StopCapture().OneLiteral(
...
Status HttpRequest::Send()
{
	...
	const auto& error_message = strings::StrCat(
			...
			response_code_, ", error code ", curl_result, ", error message '",
			error_buffer, "')");
...
uint64 HttpRequest::GetResponseCode() const
{
