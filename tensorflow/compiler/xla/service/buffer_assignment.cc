...
{
	DCHECK(
			std::find(assigned_buffers_.begin(), assigned_buffers_.end(),
					&buffer) == assigned_buffers_.end()) << "LogicalBuffer "
			<< buffer.ToString() << " already assigned to allocation "
			<< index();
...
string BufferAllocation::ToString() const
{
	...
	tensorflow::strings::StrAppend(&output,
			tensorflow::strings::Printf("allocation %lld: %p, size %lld",
					index_, this, size()));
	if (is_entry_computation_parameter()) {
		tensorflow::strings::StrAppend(&output, ", parameter ",
				parameter_number());
	}
	if (is_thread_local()) {
...
{
	CHECK(index >= 0 && index < allocations_.size()) << "Allocation index "
...
bool BufferAssignment::HasTopLevelAllocation(
		const HloInstruction* instruction) const
{
	for (const LogicalBuffer* buffer : GetPointsToSet(instruction).element(
			/*index=*/{ })) {
		if (allocation_index_for_buffer_.count(buffer) > 0) {
			return true;
		}
...
StatusOr<const BufferAllocation*> BufferAssignment::GetUniqueTopLevelOutputAllocation() const
{
...
void BufferAssignment::AddAssignment(const LogicalBuffer& buffer,
		BufferAllocation* allocation, bool colocated_buffer)
{
	CHECK_EQ(0, allocation_index_for_buffer_.count(&buffer)) << "LogicalBuffer "
			<< buffer << " already has an allocation.";
	CHECK(
			allocation->is_reusable() || allocation->assigned_buffers().empty()
					|| colocated_buffer)
...
tensorflow::Status GatherComputationsByAllocationType(const HloModule* module,
		...
		if ((is_thread_local && in_global_set)
			...
			return InvalidArgument(
					"computation %s has conflicting allocation requirements (global "
							"and thread-local)", computation->name().c_str());
		}
		...
		for (auto& instruction : computation->instructions()) {
			for (auto* subcomputation : instruction->MakeCalledComputationsSet()) {
				...
				case HloOpcode::kWhile:
					...
					worklist.push_back(std::make_pair(subcomputation, false)); // Not thread local.
					break;
				case HloOpcode::kMap:
				...
				case HloOpcode::kFusion:
					...
					worklist.push_back(std::make_pair(subcomputation, true)); // Thread local.
					break;
				default:
...
bool BufferAssigner::MaybeAssignBuffer(BufferAllocation* allocation,
		const LogicalBuffer& buffer, BufferAssignment* assignment)
{
	CHECK(!assignment->HasAllocation(buffer)) << "buffer " << buffer
			<< " already has an allocation assigned.";

	VLOG(4) << "Trying to assign " << buffer.ToString() << " to allocation: "
			<< allocation->ToString();
...
tensorflow::Status BufferAssigner::AssignBuffersForComputation(
		...
		const tensorflow::gtl::FlatSet<BufferAllocation::Index>& colocated_allocations,
		BufferAssignment* assignment)
{
	...
	for (auto& instruction : computation->instructions()) {
		if (hlos_to_allocate == nullptr
			...
			// any allocations.
			for (const LogicalBuffer* buffer : assignment->points_to_analysis().GetBuffersDefinedByInstruction(
					instruction.get())) {
	...
	for (const auto* buffer : sorted_buffers) {
		...
		if (buffer->instruction()->opcode() == HloOpcode::kParameter
			...
			// callers.
			BufferAllocation* allocation = assignment->NewAllocation(*buffer,
					buffer_size_(*buffer),
					/*is_thread_local=*/false,
					/*is_reusable=*/false);
		...
		if (buffer->IsTopLevel() && !buffer->IsTuple()) {
			for (auto* operand : buffer->instruction()->operands()) {
				...
				for (const auto& operand_allocation : assignment->GetAllocations(
						operand, /*index=*/{ })) {
					BufferAllocation* allocation =
							assignment->GetMutableAllocation(
									operand_allocation.index());
		...
		if (!assignment->HasAllocation(*buffer)) {
			auto* allocation = assignment->NewAllocation(*buffer,
					buffer_size_(*buffer), is_thread_local, /*is_reusable=*/
					true);
		...
		std::vector<ColocatedBufferSet>* colocated_buffer_sets)
{
	...
	ColocatedBufferSet* first =
			&(*colocated_buffer_sets)[overlap_set_indices[0]];
...
void BufferAssigner::BuildColocatedBufferSets(const HloModule* module,
		const TuplePointsToAnalysis& points_to_analysis,
		std::vector<ColocatedBufferSet>* colocated_buffer_sets)
{
	for (auto& computation : module->computations()) {
		for (auto& instruction : computation->instructions()) {
			...
			if (opcode == HloOpcode::kWhile) {
				...
				TF_CHECK_OK(
						ShapeUtil::ForEachSubshape(while_hlo->shape(),
				...
				HloInstruction* root_hlo =
						call_hlo->to_apply()->root_instruction();
				TF_CHECK_OK(
						ShapeUtil::ForEachSubshape(call_hlo->shape(),
	...
	for (const ColocatedBufferSet& colocated_buffer_set : colocated_buffer_sets) {
		...
		for (const LogicalBuffer* buffer : colocated_buffer_set) {
			if (allocation == nullptr) {
				...
				allocation = assignment->NewAllocation(*buffer,
						buffer_size_(*buffer),
						/*is_thread_local=*/false,
						/*is_reusable=*/true);
		...
		const std::vector<const HloInstruction*>* hlos_to_allocate)
{
	...
	TF_RETURN_IF_ERROR(
			GatherComputationsByAllocationType(module,
					&thread_local_computations, &global_computations));
	...
	for (auto* computation : global_computations) {
		TF_RETURN_IF_ERROR(
				AssignBuffersForComputation(computation, /*is_thread_local=*/
						false, hlo_set.get(), colocated_buffers,
	...
	for (auto* computation : thread_local_computations) {
		...
		TF_RETURN_IF_ERROR(
				AssignBuffersForComputation(computation, /*is_thread_local=*/
						true, hlo_set.get(), colocated_buffers,
						colocated_allocations, assignment.get()));
	}
	...
	const PointsToSet& root_points_to = assignment->GetPointsToSet(
			root_instruction);
	TF_RETURN_IF_ERROR(
			root_points_to.ForEachElement(
	...
	TF_RETURN_IF_ERROR(
			ShapeUtil::ForEachSubshape(root->shape(),
					[this, &output_size, root, &assignment](
							const Shape& /*subshape*/, const ShapeIndex& index) {
