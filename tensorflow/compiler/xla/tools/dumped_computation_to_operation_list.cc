...
public:
	explicit OperationDumper(const string& path) :
			path_(path)
	{
	}
	...
	{
		string params = tensorflow::str_util::Join(hlo->operands(), ", ",
				[](string* out, const HloInstruction* operand) {
					tensorflow::strings::StrAppend(
							out, ShapeUtil::HumanString(operand->shape()));
				});
		// Spit `op_name(params...) -> result_type :: path` to stdout.
		std::cout
				<< tensorflow::strings::Printf("%s :: (%s) -> %s :: %s\n",
						HloOpcodeString(hlo->opcode()).c_str(), params.c_str(),
						ShapeUtil::HumanString(hlo->shape()).c_str(),
						path_.c_str());
		return Status::OK();
	}
...
void RealMain(tensorflow::gtl::ArraySlice<char*> args)
{
	...
	LocalService* local_service = ClientLibrary::GetXlaService(
			client->platform());
	for (char* arg : args) {
