		...
		*input_max = 1;
		...
		for (const Edge* edge : node->in_edges()) {
			if (edge->src_output() != Graph::kControlSlot
					&& edge->dst_input() == 0) {
				FindType(graph, edge->src(), signed_input, range_given,
						input_min, input_max);
			}
		}
		...
		const std::vector<EdgeToConvert>& target_edges)
{
	...
	for (const EdgeToConvert edge : target_edges) {
		Node* convert_node;
		string name = strings::StrCat(edge.edge->src()->name(),
				"/QuantizeAndDequantize");

		auto iter = name_index.find(name);
		if (iter == name_index.end()) {
			TF_RETURN_IF_ERROR(
					NodeBuilder(name, "QuantizeAndDequantize").Input(
							edge.edge->src()).Attr("signed_input",
							edge.signed_input).Attr("num_bits", edge.num_bits).Attr(
							"range_given", edge.range_given).Attr("input_min",
							edge.input_min).Attr("input_max", edge.input_max).Finalize(
							graph, &convert_node));
		...
		graph->AddEdge(convert_node, 0, edge.edge->dst(),
				edge.edge->dst_input());
...
Status DoQuantizeTraining(int32 num_bits, Graph* graph)
{
	...
	if (num_bits < 1 || num_bits > 63) {
		return errors::OutOfRange(
				"num_bits should be in range [1, 63] but is: ", num_bits);
	}
		...
		if (nodes_to_rewrite.find(node->type_string()) != nodes_to_rewrite.end()
			...
			for (const Edge* edge : node->in_edges()) {
				if (edge->src_output() == Graph::kControlSlot) {
					...
					if (!known_op) {
						...
						if (potential_input > 0) {
							return errors::Unimplemented(
									"Find a second unknown op: ",
									edge->src()->name(), " with type: ",
					...
					target_edges.emplace_back(
							EdgeToConvert(edge, num_bits, signed_input,
									range_given, input_min, input_max));
				}
			}
		}
...
Status DoQuantizeTrainingOnSerializedGraphDef(const string& input_graph,
		int32 num_bits, string* result_graph)
{
