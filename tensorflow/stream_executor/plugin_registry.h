...
public:
	typedef blas::BlasSupport* (*BlasFactory)(
...
private:
	...
	port::Status RegisterFactoryInternal(PluginId plugin_id,
			const string& plugin_name, FactoryT factory,
			std::map<PluginId, FactoryT>* factories);

	// Actually performs the work of factory retrieval.
	template<typename FactoryT>
	port::StatusOr<FactoryT> GetFactoryInternal(PluginId plugin_id,
			const std::map<PluginId, FactoryT>& factories,
