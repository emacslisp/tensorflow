		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	for (const NodeDef& input_node : input_graph_def.node()) {
		...
		for (const string& input_name : input_node.input()) {
			...
			string suffix;
			NodeNamePartsFromInput(input_name, &prefix, &input_node_name,
					&suffix);
			if (new_names.count(input_node_name) == 0) {
				return errors::InvalidArgument("No node named ",
						input_node_name, " for input to ", old_name);
			}
			string new_input_name = prefix + new_names[input_node_name]
					+ suffix;
