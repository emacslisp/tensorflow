...
public:
	explicit GraphNode(TFNode* node) :
			ShowNode(node)
	{
	...
	void AggregateTotalStats(GraphNode* node)
	{
		...
		mutable_proto()->set_total_inputs(
				proto().total_inputs() + node->proto().total_inputs() + 1);
	}
	...
	void AddSelfToTotalStats()
	{
		...
		mutable_proto()->set_total_inputs(
				proto().total_inputs() + proto().inputs());
	}
...
private:
	...
	bool ShouldShowIfExtra(ShowNode* node, const Options& opts, int depth) override
	{
		return true;
	}
	...
	std::vector<GraphNode*> GenerateGraphDot(GraphNode* root,
			GraphNode* last_shown, const Options& opts, int depth, int hidden,
			std::set<string>* declared_nodes, std::set<string>* declared_edges,
			TFProfNode* parent);
