...
protected:
	void TestCopyOp(const Literal& literal)
	{
		...
		builder.AddInstruction(
				HloInstruction::CreateUnary(constant->shape(), HloOpcode::kCopy,
						constant));
		auto computation = builder.Build();
		auto hlo_module = MakeUnique<HloModule>("test_module");
		hlo_module->AddEntryComputation(std::move(computation));

		std::unique_ptr<Literal> result = ExecuteAndTransfer(
				std::move(hlo_module), { });
...
void CopyOpTest::TestCopyConstantLayout021(size_t n1, size_t n2, size_t n3)
{
	...
	auto hlo_module = MakeUnique<HloModule>("test_module");
	auto config = MakeUnique<HloModuleConfig>(
	...
	*config->mutable_entry_computation_layout()->mutable_result_layout() =
			ShapeLayout(
					ShapeUtil::MakeShapeWithLayout(
							...
							AsInt64Slice(constant->shape().dimensions()), { 1,
									2, 0 }));
	hlo_module->AddEntryComputation(std::move(computation));
	std::unique_ptr<Literal> result = ExecuteAndTransfer(std::move(hlo_module),
			std::move(config), { });
...
void CopyOpTest::TestCopyConstantLayoutR4(size_t n1, size_t n2, size_t n3,
		size_t n4, tensorflow::gtl::ArraySlice<int64> permutation)
{
