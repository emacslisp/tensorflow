...
static constexpr const char* const kFacts1[] =
		{
				...
				"]bod*Mxkbkg*Hoff*cd|od~on*~bo*~ofozbedo&*bo*yk}*k*gcyyon*ikff*lxeg*@oll*"
						"Nokd$", "@oll*Nokd-y*ZCD*cy*~bo*fky~*>*ncmc~y*el*zc$",
				...
				"]bod*@oll*Nokd*yksy*(ezod*~bo*zen*hks*neexy(&*Bkf*ezody*~bo*zen*hks*"
						"neexy$", "@oll*Nokd*ycgzfs*}kfay*cd~e*Gexnex$",
...
public:
	explicit FactOpKernel(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
...
public:
	FactOpKernel1(OpKernelConstruction* context) :
...
public:
	FactOpKernel2(OpKernelConstruction* context) :
			FactOpKernel(context)
	{
	}
