...
public:
	explicit BetaincOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
		...
		if (b_shape.dims() > 0 && x_shape.dims() > 0) {
			OP_REQUIRES(ctx, b_shape == x_shape,
					errors::InvalidArgument(
							...
							b_shape.DebugString(), " vs. ",
							x_shape.DebugString()));
		}

		TensorShape merged_shape(a_shape);
		if (b_shape.dims() > 0)
			merged_shape = b_shape;
		if (x_shape.dims() > 0)
			merged_shape = x_shape;
...
#define CASE(NDIM)                                                        \
  case NDIM: {                                                            \
    ...
    return;                                                               \
		...
		default: {
			ctx->SetStatus(
					errors::InvalidArgument("Broadcasting rank not supported: ",
							ndims));
			return;
		}
		}
