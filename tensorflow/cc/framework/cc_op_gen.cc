...
string PrintAttrValue(string op, const AttrValue& attr_value)
{
	...
	case AttrValue::kTensor:
		return strings::StrCat("Input::Initializer(", "{",
				PrintTensor(attr_value.tensor()), "}, ",
				PrintTensorShape(
						TensorShape(attr_value.tensor().tensor_shape())),
				").AsTensorProto()");
		...
		if (attr_value.list().s_size() > 0) {
			...
			for (int i = 0; i < attr_value.list().b_size(); ++i) {
				if (i > 0)
					strings::StrAppend(&ret, ", ");
				strings::StrAppend(&ret,
						attr_value.list().b(i) ? "true" : "false");
			}
			...
			for (int i = 0; i < attr_value.list().shape_size(); ++i) {
				...
				strings::StrAppend(&ret,
						PrintTensorShape(
								TensorShape(attr_value.list().shape(i))));
			}
		}
		strings::StrAppend(&ret, "}");
		return ret;
...
std::pair<const char*, bool> AttrTypeName(StringPiece attr_type)
{
	static const std::unordered_map<StringPiece, std::pair<const char*, bool>,
			StringPiece::Hasher> attr_type_map { { "string", { "StringPiece",
			false } }, { "list(string)", { "gtl::ArraySlice<string>", true } },
			{ "int", { "int64", false } }, { "list(int)", {
					"gtl::ArraySlice<int>", true } }, { "float", { "float",
					false } }, { "list(float)",
					{ "gtl::ArraySlice<float>", true } }, { "bool", { "bool",
					false } },
			{ "list(bool)", { "gtl::ArraySlice<bool>", true } }, { "type", {
					"DataType", false } }, { "list(type)", { "DataTypeSlice",
					true } }, { "shape", { "TensorShape", false } }, {
					"list(shape)", { "gtl::ArraySlice<TensorShape>", true } }, {
					"tensor", { "TensorProto", true } }, { "list(tensor)", {
					"gtl::ArraySlice<TensorProto>", true } }, { "func", {
					"NameAttrList", true } }, };
...
bool IsCPPKeyword(StringPiece name)
{
	...
	kCPPReserved { "alignas", "alignof", "and", "and_eq", "asm",
...
void InferOpAttributes(const OpDef& op_def,
...
bool HasOptionalAttrs(const OpDef& op_def,
		const std::unordered_map<string, string>& inferred_input_attrs)
{
	for (int i = 0; i < op_def.attr_size(); ++i) {
		...
		if ((inferred_input_attrs.find(attr.name())
				== inferred_input_attrs.end()) && attr.has_default_value()) {
			return true;
		}
	}
	return false;
		...
		graph_op_def(g_op_def), op_def(i_op_def), aliases(a)
{
	...
	for (int i = 0; i < op_def.input_arg_size(); ++i) {
		const auto& arg(op_def.input_arg(i));
		arg_types.push_back(
				strings::StrCat("::tensorflow::",
						ArgIsList(arg) ? "InputList" : "Input"));
	...
	for (int i = 0; i < op_def.attr_size(); ++i) {
		...
		if (inferred_input_attrs.count(attr.name()) > 0)
			continue;
		...
		if (attr.has_default_value()) {
			...
			arg_types.push_back(
					strings::StrCat(use_const ? "const " : "", attr_type_name,
							use_const ? "&" : ""));
	...
	if (op_def.output_arg_size() == 0) {  // No outputs.
		...
		for (int i = 0; i < op_def.output_arg_size(); ++i) {
			...
			if (op_def.output_arg(i).description().empty()) {
				...
				// TODO(josh11b): Word wrap this.
				strings::StrAppend(&comment, ": ",
						op_def.output_arg(i).description(), "\n");
			}
		}
	}
...
string OpInfo::GetOpAttrStruct() const
{
	...
	for (int i = 0; i < op_def.attr_size(); ++i) {
		...
		const string suffix =
				(camel_case_name == op_name || camel_case_name == "Attrs") ?
						"_" : "";
		const string attr_func_def = strings::StrCat(camel_case_name, suffix,
				"(", use_const ? "const " : "", attr_type_name,
				use_const ? "&" : "");
...
void OpInfo::WriteClassDecl(WritableFile* h) const
{
	...
	if (has_optional_attrs) {
		...
		for (int i = 0; i < op_def.attr_size(); ++i) {
			...
			const string suffix =
					(camel_case_name == op_name || camel_case_name == "Attrs") ?
							"_" : "";
			const string attr_func_def = strings::StrCat(camel_case_name,
					suffix, "(", use_const ? "const " : "", attr_type_name,
					use_const ? "&" : "");
			strings::StrAppend(&class_decl, "  static Attrs ", attr_func_def,
					" x) {\n");
			strings::StrAppend(&class_decl, "    return Attrs().",
					camel_case_name, suffix, "(x);\n");
			strings::StrAppend(&class_decl, "  }\n");
		}
	}
	...
	for (int i = 0; i < output_types.size(); ++i) {
		strings::StrAppend(&class_decl, "  ", output_types[i], " ",
				output_names[i], ";\n");
	}
...
void OpInfo::GetOutput(string* out) const
{
	...
	string return_on_error = strings::StrCat("if (!", scope_str,
			".ok()) return;");
	...
	strings::StrAppend(out, "  ::tensorflow::Status _status_ = "
	...
	for (int i = 0; i < op_def.output_arg_size(); ++i) {
		const string arg_range = strings::StrCat("_outputs_range[\"",
				graph_op_def.output_arg(i).name(), "\"]");
		if (is_list_output[i]) {
			strings::StrAppend(out, "  for (int64 i = ", arg_range,
					".first; i < ", arg_range, ".second; ++i)\n");
...
string OpInfo::GetConstructorBody() const
{
	...
	for (int i = 0; i < op_def.input_arg_size(); ++i) {
		const auto& arg(op_def.input_arg(i));
		strings::StrAppend(&body, "  auto _", arg.name(),
				" = ::tensorflow::ops::",
				ArgIsList(arg) ? "AsNodeOutList" : "AsNodeOut", "(", scope_str,
				", ", AvoidCPPKeywords(arg.name()), ");\n");
		strings::StrAppend(&body, "  ", return_on_error, "\n");
	}
	...
	strings::StrAppend(&body, "  ", scope_str,
			".UpdateStatus(builder.Finalize(", scope_str,
			".graph(), &ret));\n");
	strings::StrAppend(&body, "  ", return_on_error, "\n");
...
void WriteCCOp(const OpDef& graph_op_def, const OpDef& interface_op_def,
		const std::vector<string>& aliases, WritableFile* h, WritableFile* cc)
{
		...
		const string& dot_cc_fname, const string& overrides_fnames)
{
	...
	for (const auto& graph_op_def : ops.op()) {
		...
		// code depends on it.
		if (graph_op_def.name() == "Const")
			continue;

		// Incorporate overrides from override_map.
		OpDef interface_op_def = graph_op_def;
		const OpGenOverride* op_override = override_map.ApplyOverride(
				&interface_op_def);
		std::vector<string> aliases;
		if (op_override) {
			if (op_override->skip())
				continue;
			aliases.assign(op_override->alias().begin(),
					op_override->alias().end());
			if (op_override->hide()) {
				// Write hidden ops to _internal.h and _internal.cc.
				WriteCCOp(graph_op_def, interface_op_def, aliases,
						internal_h.get(), internal_cc.get());
				continue;
			}
		}
