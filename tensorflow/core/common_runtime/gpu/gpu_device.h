...
public:
	BaseGPUDevice(const SessionOptions& options, const string& name,
			...
			Allocator* cpu_allocator, bool sync_every_op, int32 max_streams);
	...
	void ConsumeListOfAccessedTensors(DeviceContext* device_context,
	...
	gpu::StreamExecutor* executor() const
	{
		return executor_;
	}
