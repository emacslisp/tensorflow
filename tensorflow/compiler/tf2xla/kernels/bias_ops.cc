...
public:
	explicit BiasOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		string data_format;
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		int feature_dim =
				(data_format_ == FORMAT_NHWC) ?
						input_shape.dims() - 1 : input_shape.dims() - 3;
		OP_REQUIRES(ctx, feature_dim >= 0,
				errors::InvalidArgument(
						"Input tensor does not have enough dimensions "
								"to contain the feature dimension"));
		OP_REQUIRES(ctx,
				bias_shape.dim_size(0) == input_shape.dim_size(feature_dim),
				errors::InvalidArgument(
						"Must provide as many biases as the last dimension "
								"of the input tensor: ",
						bias_shape.DebugString(), " vs. ",
						input_shape.DebugString()));

		xla::ComputationDataHandle result = ctx->builder()->Add(ctx->Input(0),
				ctx->Input(1), { feature_dim });
...
public:
	explicit BiasAddGradOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		string data_format;
		if (ctx->GetAttr("data_format", &data_format).ok()) {
