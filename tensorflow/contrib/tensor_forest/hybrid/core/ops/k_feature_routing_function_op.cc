...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("num_features_per_node",
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int i = 0; i < num_data; i++) {
			...
			for (int j = 0; j < max_nodes_ / 2; j++) {
				feature_set.clear();
				tensorforest::GetFeatureSet(layer_num_, i, random_seed_,
						num_features, num_features_per_node_, &feature_set);

				int32 left_child = 2 * j + 1;
				int32 right_child = left_child + 1;

				float prob = out_probs(i, j);
				float left_prob = LeftProbabilityK(point, feature_set,
						tree_parameters_tensor.Slice(j, j + 1), tree_biases(j),
						num_features, num_features_per_node_);
