		...
		llvm_ir::IrArray* array)
{
	...
	if (alias_scope_md == nullptr) {
		alias_scope_md = GetAliasScopeMetadataForBuffer(buffer_index,
				GetAliasDomain());
	}
	...
	if (hlo.opcode() == HloOpcode::kParameter) {
		...
		if (std::find(parameter_instructions.begin(),
				parameter_instructions.end(), &hlo)
		...
		const BufferAssignment& assignment, const HloInstruction& hlo)
{
	...
	for (BufferAllocation::Index noalias_index : buffers) {
		llvm::MDNode* scope = metadata_builder.createAliasScope(
				AsStringRef(
						tensorflow::strings::StrCat("buffer: ", noalias_index)),
				domain);
		scopes.push_back(scope);
	}
	llvm::MDNode* noalias_list = llvm::MDNode::get(domain->getContext(),
			AsArrayRef(scopes));
	return noalias_list;
}
