...
static void SetArgvFromEnv(EnvArgv* a)
{
	if (!a->initialized) {
		...
		AppendToEnvArgv(kDummyArgv, strlen(kDummyArgv), nullptr, 0, a); // dummy argv[0]
...
bool ParseFlagsFromEnv(const std::vector<tensorflow::Flag>& flag_list)
{
	...
	bool result = tensorflow::Flags::Parse(&env_argv->argc, &env_argv->argv[0],
			flag_list);
	env_argv_mu.unlock();
	return result;
}
