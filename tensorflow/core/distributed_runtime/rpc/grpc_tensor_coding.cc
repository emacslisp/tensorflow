...
static void EncodeSkeleton(const Tensor& val, io::ProtoEncodeHelper* e)
{
	...
	if (tensor_shape_bytes > 0) {
		...
		for (int d = 0; d < ndims; d++) {
			...
			int64 dim_varlen = 1 +  // TensorShapeProto_Dim::kSizeFieldNumber
					core::VarintLength(dim_size);
			e->WriteVarlengthBeginning(TensorShapeProto::kDimFieldNumber,
					dim_varlen);
...
#ifndef NDEBUG
	{
		...
		DCHECK_EQ(tensor_except_contents, out) << skeleton.DebugString()
		...
		::grpc::ByteBuffer* result)
{
	...
	if (!DataTypeCanUseMemcpy(val.dtype())) {
		...
		// not the actual data
		gtl::InlinedVector<char, 128> skeleton(
		...
		uint32 overall_tensor_proto_bytesize = (e_skeleton.size()
		...
		int num_slices = 0;
		{
			...
			if (!tensor_data_is_large) {
				// (E)
				memcpy(GPR_SLICE_START_PTR(s0) + e.size(), tdata.data(),
						tdata.size());
			}
