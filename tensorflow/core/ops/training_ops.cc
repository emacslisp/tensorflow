...
static Status ApplyAdadeltaShapeFn(InferenceContext* c, bool sparse)
{
	...
	TF_RETURN_IF_ERROR(c->Merge(s, ShapeOrHandleShape(c, 2), &s)); // accum update
...
static Status ApplyAdagradDAShapeFn(InferenceContext* c, bool sparse)
{
	ShapeHandle unused;
	ShapeHandle s = ShapeOrHandleShape(c, 0);  // var
	TF_RETURN_IF_ERROR(c->Merge(s, ShapeOrHandleShape(c, 1), &s)); // grad_accumulator
	TF_RETURN_IF_ERROR(c->Merge(s, ShapeOrHandleShape(c, 2), &s)); // gradient_squared_accumulator
	TF_RETURN_IF_ERROR(
			HandleGradAndIndicesInputs(c, sparse, 3 /* grad_idx */, &s));
	int idx = sparse ? 5 : 4;
