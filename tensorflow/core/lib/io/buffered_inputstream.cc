...
BufferedInputStream::BufferedInputStream(InputStreamInterface* input_stream,
		size_t buffer_size, bool owns_input_stream) :
		input_stream_(input_stream), size_(buffer_size), owns_input_stream_(
				owns_input_stream)
{
	buf_.reserve(size_);
}
...
Status BufferedInputStream::ReadNBytes(int64 bytes_to_read, string* result)
{
	...
	while (result->size() < static_cast<size_t>(bytes_to_read)) {
		...
		const int64 bytes_to_copy = std::min<int64>(limit_ - pos_,
				bytes_to_read - result->size());
