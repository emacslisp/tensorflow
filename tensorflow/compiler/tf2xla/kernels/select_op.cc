...
public:
	explicit SelectOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, then_shape.IsSameSize(else_shape),
				errors::InvalidArgument(
						"'then' and 'else' must have the same size.  but received: ",
						then_shape.DebugString(), " vs. ",
		...
		if (broadcasting) {
			...
			OP_REQUIRES(ctx,
					then_shape.dim_size(0) == cond_shape.num_elements(),
		...
		ctx->SetOutput(0,
				builder->Select(cond_handle, then_handle, else_handle));
	}
