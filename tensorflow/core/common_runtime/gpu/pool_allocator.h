...
private:
	...
	mutex mutex_;std::multimap<const size_t, PtrRecord*> pool_ GUARDED_BY(mutex_);PtrRecord* lru_head_ GUARDED_BY(mutex_) = nullptr;PtrRecord* lru_tail_ GUARDED_BY(mutex_) = nullptr;int64
	get_from_pool_count_ GUARDED_BY(mutex_) = 0;int64
	put_count_ GUARDED_BY(mutex_) = 0;int64
	allocated_count_ GUARDED_BY(mutex_) = 0;int64
	evicted_count_ GUARDED_BY(mutex_) = 0;
