...
/* static */StatusOr<std::unique_ptr<ShapedBuffer>> ShapedBuffer::MakeShapedBuffer(
...
/* static */StatusOr<std::unique_ptr<ShapedBuffer>> ShapedBuffer::MakeArrayShapedBuffer(
		const Shape& shape, const perftools::gputools::Platform* platform,
...
/* static */StatusOr<std::unique_ptr<ShapedBuffer>> ShapedBuffer::MakeUnnestedTupleShapedBuffer(
		const Shape& shape, const perftools::gputools::Platform* platform,
		int device_ordinal,
		const tensorflow::gtl::ArraySlice<perftools::gputools::DeviceMemoryBase> buffers)
{
...
ShapedBuffer::ShapedBuffer(const Shape& shape,
		const perftools::gputools::Platform* platform, int device_ordinal) :
		shape_(shape), shape_index_to_buffer_entry_(shape), platform_(platform), device_ordinal_(
				device_ordinal)
{
}
...
/* static */StatusOr<std::unique_ptr<ScopedShapedBuffer>> ScopedShapedBuffer::MakeScopedShapedBuffer(
		const Shape& shape, DeviceMemoryAllocator* allocator,
		int device_ordinal)
{
	if (!LayoutUtil::HasLayout(shape)) {
		return InvalidArgument("Shape must have a layout: %s",
	...
	auto shaped_buffer = WrapUnique(
			new ScopedShapedBuffer(shape, allocator, device_ordinal));
...
ScopedShapedBuffer::ScopedShapedBuffer(const Shape& shape,
		...
		ShapedBuffer(shape, allocator->platform(), device_ordinal), allocator_(
				allocator)
{
}
...
ScopedShapedBuffer::~ScopedShapedBuffer()
{
	...
	for (perftools::gputools::DeviceMemoryBase& memory_base : buffers_) {
		if (!memory_base.is_null()
			...
			TF_CHECK_OK(
					this->allocator_->Deallocate(this->device_ordinal(),
							&memory_base));
		}
	}
}
