...
void WorkerCacheLogger::SetLogging(bool v)
{
	...
	if (v) {
		...
		if (want_logging_count_ < 0)
			want_logging_count_ = 0;
	}
}
...
void WorkerCacheLogger::RecordRecvTensor(int64 step_id, int64 start_usecs,
		int64 end_usecs, const string& tensor_name, const string& src_device,
		const string& dst_device, int64 bytes)
{
	...
	ns->set_timeline_label(
			strings::StrCat(byte_string, tensor_name, " from ", src_device,
					" to ", dst_device));
	...
	no->mutable_tensor_description()->mutable_allocation_description()->set_requested_bytes(
			bytes);
