...
port::StatusOr<DriverVersion> StringToDriverVersion(const string &value)
{
	...
	if (pieces.size() != 2 && pieces.size() != 3) {
		return port::Status { port::error::INVALID_ARGUMENT,
				port::Printf(
						"expected %%d.%%d or %%d.%%d.%%d form for driver version; got \"%s\"",
						value.c_str()) };
	}
	...
	if (!port::safe_strto32(pieces[0], &major)) {
		return port::Status { port::error::INVALID_ARGUMENT, port::Printf(
				"could not parse major version number \"%s\" as an "
						"integer from string \"%s\"", pieces[0].c_str(),
				value.c_str()) };
	}
	if (!port::safe_strto32(pieces[1], &minor)) {
		return port::Status { port::error::INVALID_ARGUMENT, port::Printf(
				"could not parse minor version number \"%s\" as an "
						"integer from string \"%s\"", pieces[1].c_str(),
				value.c_str()) };
	}
	if (pieces.size() == 3 && !port::safe_strto32(pieces[2], &patch)) {
		return port::Status { port::error::INVALID_ARGUMENT, port::Printf(
				"could not parse patch version number \"%s\" as an "
						"integer from string \"%s\"", pieces[2].c_str(),
				value.c_str()) };
	}
...
void Diagnostician::LogDiagnosticInformation()
{
	...
	CFArrayRef kext_id_query = CFArrayCreate(nullptr, (const void**) kext_ids,
			1, &kCFTypeArrayCallBacks);
	CFDictionaryRef kext_infos = KextManagerCopyLoadedKextInfo(kext_id_query,
			nullptr);
	...
	if (CFDictionaryGetValueIfPresent(kext_infos, kDriverKextIdentifier,
		...
		bool started = CFBooleanGetValue(
				(CFBooleanRef) CFDictionaryGetValue(cuda_driver_info,
						CFSTR("OSBundleStarted")));
		if (!started) {
			LOG(INFO)
					<< "kernel driver is installed, but does not appear to be running on this host "
					<< "(" << port::Hostname() << ")";
		}
	} else {
		LOG(INFO)
				<< "kernel driver does not appear to be installed on this host "
				<< "(" << port::Hostname() << ")";
	}
		...
		const string &driver_version_file_contents)
{
	...
	if (offset == string::npos) {
		return port::Status { port::error::NOT_FOUND, port::StrCat(
				"could not find kernel module information in "
	...
	// TODO(b/22689637): Eliminate the explicit namespace if possible.
	auto stripped_kernel_version = port::StripSuffixString(kernel_version,
			".ld64");
...
port::StatusOr<DriverVersion> Diagnostician::FindKernelDriverVersion()
{
	...
	CFRelease(kext_infos);
	auto status =
			port::Status { port::error::INTERNAL, port::StrCat(
					"failed to read driver bundle version: ",
					CFStringGetCStringPtr(kDriverKextIdentifier,
