 ...
 */
class AccumulatorSetGlobalStepOp: public ConditionalAccumulatorBaseSyncOpKernel {
public:
...
protected:
	void Compute(OpKernelContext* ctx, ConditionalAccumulatorBase* accumulator) override
	{
		// Check signature
		OP_REQUIRES_OK(ctx,
				ctx->MatchSignature( { DT_STRING_REF, DT_INT64 }, { }));

		// Get input new_global_step
		const Tensor* new_global_step_tensor;
		OP_REQUIRES_OK(ctx,
				ctx->input("new_global_step", &new_global_step_tensor));
		...
		Status status = accumulator->SetGlobalStep(
				new_global_step_tensor->scalar<int64>()());
		if (!status.ok())
 ...
 * as an output tensor.
 */
class AccumulatorNumAccumulatedOp: public ConditionalAccumulatorBaseSyncOpKernel {
public:
...
protected:
	void Compute(OpKernelContext* ctx, ConditionalAccumulatorBase* accumulator) override
	{
		...
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(0, TensorShape( { }), &Taccumulator_size));
