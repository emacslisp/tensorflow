		...
		const OpRegistryInterface& op_registry, int node_offset)
{
	if (node_offset > graph_def->node_size()) {
		return errors::InvalidArgument("Tried to add default attrs to GraphDef "
				"starting at offset ", node_offset,
				" with total nodes in graph: ", graph_def->node_size());
	}
...
static Status RemoveNewDefaultAttrsFromNodeDef(NodeDef* node_def,
		...
		std::set<std::pair<string, string>>* op_attr_removed)
{
	...
	for (const auto& attr : node_def->attr()) {
		...
		if (!StringPiece(attr.first).starts_with("_")
			...
			const OpDef::AttrDef* producer_attr_def = FindAttr(attr.first,
					*producer_op_def);
			if (producer_attr_def == nullptr) {
				return errors::InvalidArgument("Attr '", attr.first,
		...
		std::set<std::pair<string, string>>* op_attr_removed)
{
	...
	for (int n = 0; n < graph_def->node_size(); ++n) {
		...
		if (!IsFunction(*graph_def, node_def->op())) {
			TF_RETURN_IF_ERROR(
					RemoveNewDefaultAttrsFromNodeDef(node_def,
							consumer_op_registry, producer_op_registry,
							op_attr_removed));
		}
	}
	for (int f = 0; f < graph_def->library().function_size(); ++f) {
		FunctionDef* func_def = graph_def->mutable_library()->mutable_function(
				f);
