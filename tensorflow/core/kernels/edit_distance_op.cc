...
Status ValidateShapes(OpKernelContext* ctx, const Tensor& hypothesis_indices,
		const Tensor& hypothesis_values, const Tensor& hypothesis_shape,
	...
	if (truth_shape.NumElements() < 2)
		return errors::InvalidArgument(
				"Input SparseTensors must have rank at least 2, but truth_shape "
						"rank is: ", truth_shape.NumElements());
	if (truth_shape.NumElements() != truth_indices.dim_size(1))
		return errors::InvalidArgument("Expected truth_shape.NumElements == "
	...
	if (truth_shape.NumElements() != hypothesis_shape.NumElements())
		return errors::InvalidArgument(
				"Expected truth and hypothesis to have matching ranks, but "
						"their shapes are: ", truth_shape.shape().DebugString(),
				" and ", hypothesis_shape.shape().DebugString());
...
public:
	explicit EditDistanceOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				ValidateShapes(ctx, *hypothesis_indices, *hypothesis_values,
		...
		OP_REQUIRES_OK(ctx,
				TensorShapeUtils::MakeShape(
						hypothesis_shape->vec<int64>().data(),
						hypothesis_shape->NumElements(), &hypothesis_st_shape));
		TensorShape truth_st_shape;
		OP_REQUIRES_OK(ctx,
				TensorShapeUtils::MakeShape(truth_shape->vec<int64>().data(),
		...
		sparse::SparseTensor truth(*truth_indices, *truth_values,
				truth_st_shape, sorted_order);
		...
		while (hypothesis_iter != hypothesis_grouper.end()
			...
			if (g_truth == g_hypothesis) {
				...
				output_t(loc) = gtl::LevenshteinDistance<T>(truth_seq,
						hypothesis_seq, cmp);
				if (normalize_)
				...
				auto loc = std::inner_product(g_hypothesis.begin(),
						g_hypothesis.end(), output_strides.begin(),
						int64 { 0 });
		...
		while (hypothesis_iter != hypothesis_grouper.end()) { // zero-length truths
			...
			auto hypothesis_seq = hypothesis_j.values<T>();
			auto loc = std::inner_product(g_hypothesis.begin(),
					g_hypothesis.end(), output_strides.begin(), int64 { 0 });
			output_t(loc) = hypothesis_seq.size();
			if (normalize_ && output_t(loc) != 0.0f) {
				output_t(loc) = std::numeric_limits<float>::infinity();
			}
