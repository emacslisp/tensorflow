...
enum DataColumnTypes {
	kDataFloat = 0, kDataCategorical = 1
};
...
bool BestSplitDominatesClassificationBootstrap(const Tensor& total_counts,
		...
		tensorflow::random::SimplePhilox* rand);
...
bool DecideSparseNode(const T1& sparse_input_indices,
		const T2& sparse_input_values, int32 i, int32 feature, float bias,
		const tensorforest::TensorForestDataSpec& spec)
{
	const float val = FindSparseValue(sparse_input_indices, sparse_input_values,
			i, feature);
...
inline bool CheckTensorBounds(OpKernelContext* context, const Tensor& tensor)
{
	for (int i = 0; i < (tensor).dims(); ++i) {
		if (!TF_PREDICT_TRUE(
				tensor.shape().dim_size(i)
						< std::numeric_limits<int32>::max())) {
			context->CtxFailure(
					(errors::InvalidArgument(
							strings::StrCat(
									"Tensor has a dimension that is greater than 2^31: ",
									tensor.DebugString()))));
			return false;
		}
	}
