...
typedef std::map<std::string, std::pair<std::string, tensorflow::Tensor> > InputMap;
...
{
	static std::map<int64, SessionVariables*>* sessions
	PT_GUARDED_BY(mutex_) = new std::map<int64, SessionVariables*>();
	return sessions;
}
...
JNIEXPORT jint JNICALL TENSORFLOW_METHOD (initializeTensorFlow)(JNIEnv* env,
		jobject thiz, jobject java_asset_manager, jstring model)
{
...
JNIEXPORT jint JNICALL TENSORFLOW_METHOD (runInference)(JNIEnv* env,
		jobject thiz, jobjectArray output_name_strings)
{
	SessionVariables* vars = GetSessionVars(env, thiz);
	...
	for (int i = 0; i < env->GetArrayLength(output_name_strings); i++) {
		jstring java_string = (jstring)(
				env->GetObjectArrayElement(output_name_strings, i));
	...
	if (vars->log_stats) {
		...
		s = vars->session->Run(run_options, input_tensors,
				vars->output_tensor_names, { }, &(vars->output_tensors),
				&run_metadata);
		...
		jobject thiz)
{
	...
	std::stringstream ss;
	ss
			<< vars->summarizer->GetStatsByMetric("Top 10 CPU",
