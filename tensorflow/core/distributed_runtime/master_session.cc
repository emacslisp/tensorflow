...
public:
	ReffedClientGraph(const string& handle, const BuildGraphOptions& bopts,
			...
			session_handle_(handle), client_graph_(std::move(cg)), session_opts_(
					session_opts), is_partial_(is_partial), worker_cache_(
					worker_cache)
	{
	...
	const SimpleClientGraph* client_graph()
	{
		return client_graph_.get();
	}

	std::unique_ptr<ProfileHandler> GetProfileHandler(uint64 step,
			int64 execution_count, const RunOptions& ropts)
	{
	...
	Status RunPartitions(const MasterEnv* env, int64 step_id,
			int64 execution_count, SimpleGraphExecutionState* execution_state,
	...
	string DetailText(const NodeDef& def, const NodeExecStats& ns)
	{
		...
		return strings::StrCat(bytes, def.name(), " = ", def.op(), "(",
				str_util::Join(
						std::vector<StringPiece>(def.input().begin(),
								def.input().end()), ", "), ")");
	}
...
private:
	...
	Status DoBuildPartitions(PartitionOptions pots,
			std::unordered_map<string, GraphDef>* out_partitions);
	Status DoRegisterPartitions(const PartitionOptions& popts,
			const FunctionDefLibrary& func_def_lib,
		...
		const PartitionOptions& popts, const FunctionDefLibrary& func_def_lib)
{
	{  // Ensure register once.
		...
		if (!init_started_) {
			...
			s = DoRegisterPartitions(popts, func_def_lib,
					std::move(graph_defs));
			mu_.lock();
			init_result_ = s;
			init_done_.Notify();
...
static string SplitByWorker(const Node* node)
{
	...
	string device;
	CHECK(
			DeviceNameUtils::SplitDeviceName(node->assigned_device_name(),
					&task, &device)) << "node: " << node->name() << " dev: "
			<< node->assigned_device_name();
	return task;
}

void MasterSession::ReffedClientGraph::TrackFeedsAndFetches(Part* part,
		const GraphDef& graph_def, const PartitionOptions& popts)
{
	for (int i = 0; i < graph_def.node_size(); ++i) {
		...
		if (is_recv || is_send) {
			...
			TF_CHECK_OK(
					GetNodeAttr(ndef, "client_terminated", &client_terminated));
			if (client_terminated) {
				...
				const string& key = Rendezvous::CreateKey(send_device,
						send_device_incarnation, recv_device, name,
						FrameAndIter(0, 0));
...
static bool CopyIfNeeded(TensorProto* in, TensorProto* out)
{
	if (in->tensor_content().empty()) {
		...
		Tensor t(in->dtype());
		if (!t.FromProto(cpu_allocator(), *in))
			return false;
...
class RunManyGraphs {
public:
	explicit RunManyGraphs(int num) :
			calls_(num), pending_(num)
	{
	}
...
private:
	...
	mutable mutex mu_;Status
	status_ GUARDED_BY(mu_);
	...
	Status MasterSession::ReffedClientGraph::RunPartitions(const MasterEnv* env,
			...
			const bool is_last_partial_run)
	{
		...
		for (size_t i = 0; i < req.num_feeds(); ++i) {
			if (!feeds.insert( { req.feed_name(i), i }).second) {
				return errors::InvalidArgument("Duplicated feeds: ",
						req.feed_name(i));
			}
		}
			...
			*c->req->mutable_exec_opts() = exec_opts;
			...
			if (is_partial_) {
				for (int i = 0; i < req.num_feeds(); ++i) {
					...
					if (feeds_iter == feeds.end()) {
						return errors::InvalidArgument(
								"No feed is provided for feed=", name, ", key=",
								key);
					} else if (feeds_iter->second != i) {
						return errors::Internal("Cannot find feed named \"",
								name, " in request.");
					}
					TF_RETURN_IF_ERROR(
							c->req->AddSendFromRunStepRequest(req, i, key));
				}
				...
				for (const auto& feed_key : part.feed_key) {
					...
					TF_RETURN_IF_ERROR(
							c->req->AddSendFromRunStepRequest(req, feed_index,
									key));
				}
				for (const auto& key_fetch : part.key_fetch) {
		...
		for (int i = 0; i < num; ++i) {
			...
			part.worker->RunGraphAsync(&call->opts, call->req.get(),
					call->resp.get(),
		...
		bool success = cm->RegisterCallback(token,
				[&calls]() {calls.StartCancel();});
		if (!success) {
			calls.StartCancel();
		}
		calls.Wait();
		...
		if (status.ok()) {
			for (int i = 0; i < num; ++i) {
				...
				for (size_t j = 0; j < calls.get(i)->resp->num_recvs(); ++j) {
					auto iter = part.key_fetch.find(
							calls.get(i)->resp->recv_key(j));
					if (iter == part.key_fetch.end()) {
						status.Update(
								errors::Internal("Unexpected fetch key: ",
										calls.get(i)->resp->recv_key(j)));
						break;
					}
					const string& fetch = iter->second;
					status.Update(
							resp->AddTensorFromRunGraphResponse(fetch,
									calls.get(i)->resp.get(), j));
					if (!status.ok()) {
						break;
					}
				}
				if (pss->collect_timeline) {
					pss->step_stats[i].Swap(
							calls.get(i)->resp->mutable_step_stats());
				}
				if (pss->collect_costs) {
					CostGraphDef* cost_graph =
							calls.get(i)->resp->mutable_cost_graph();
	...
	public:
		CleanupBroadcastHelper(int64 step_id, int num_calls,
				StatusCallback done) :
				resps_(num_calls), num_pending_(num_calls), done_(
						std::move(done))
		{
	...
	void MasterSession::ReffedClientGraph::CleanupPartitionsAsync(int64 step_id,
			StatusCallback done)
	{
		const int num = partitions_.size();
		// Helper object will be deleted when the final call completes.
		CleanupBroadcastHelper* helper = new CleanupBroadcastHelper(step_id,
				num, std::move(done));
		for (int i = 0; i < num; ++i) {
			const Part& part = partitions_[i];
			part.worker->CleanupGraphAsync(helper->request(),
					helper->response(i),
	...
	void MasterSession::ReffedClientGraph::ProcessStats(int64 step_id,
			PerStepState* pss, SimpleGraphExecutionState* execution_state,
			ProfileHandler* ph, const RunOptions& options, RunMetadata* resp)
	{
		if (!pss->collect_costs && !pss->collect_timeline)
			return;
		...
		for (size_t i = 0; i < partitions_.size(); ++i) {
			...
			if (ph) {
				for (const auto& ds : ss.dev_stats()) {
					ProcessDeviceStats(ph, execution_state, ds,
							false /*is_rpc*/);
				}
			}
		}
			...
			const DeviceStepStats& ds, bool is_rpc)
	{
		...
		for (const auto& ns : ds.node_stats()) {
			if (is_rpc) {
				...
				ph->RecordOneOp(dev_name, ns, true /*is_copy*/, "",
						ns.node_name(), ns.timeline_label());
			} else {
				const Node* node = execution_state->get_node_by_name(
						ns.node_name());
				...
				ph->RecordOneOp(dev_name, ns, false /*is_copy*/, ns.node_name(),
						optype, details);
			}
		}
	}
			...
			SimpleGraphExecutionState* execution_state)
	{
		...
		while (!stack.empty()) {
			...
			for (const Edge* in_edge : n->in_edges()) {
				...
				if (pending_feeds.count( { in_node->name(),
						in_edge->src_output() }) > 0) {
...
// result.
	...
	MasterSession::MasterSession(const SessionOptions& opt,
			const MasterEnv* env, std::vector<Device*>* remote_devs,
			StatsPublisherFactory stats_publisher_factory) :
			session_opts_(opt), env_(env), handle_(
					strings::FpToString(random::New64())), stats_publisher_factory_(
					std::move(stats_publisher_factory)), graph_version_(0), run_graphs_(
					5), partial_run_graphs_(5), cancellation_manager_(
					new CancellationManager)
	{
		UpdateLastAccessTime();

		swap(remote_devs_, *remote_devs);
		VLOG(1) << "Session " << handle_ << " #local "
				<< env->local_devices.size() << " #remote "
				<< remote_devs_.size();
		...
		LOG(INFO) << "Start master session " << handle_ << " with config: "
				<< std::endl << session_opts_.config.DebugString();
	}

	MasterSession::~MasterSession()
	{
		delete cancellation_manager_;
		for (const auto& iter : run_graphs_)
			iter.second->Unref();
		for (const auto& iter : partial_run_graphs_)
			iter.second->Unref();
		for (Device* dev : remote_devs_)
			delete dev;
	}
	...
	Status MasterSession::Create(GraphDef* graph_def)
	{
		if (session_opts_.config.graph_options().place_pruned_graph()) {
			...
			session_opts_.config.mutable_graph_options()->set_place_pruned_graph(
					false);
		}
		...
		TF_RETURN_IF_ERROR(
				SimpleGraphExecutionState::MakeForBaseGraph(graph_def, options,
						&execution_state_));
		return Status::OK();
	}
		...
		std::unique_ptr < SimpleGraphExecutionState > extended_execution_state;
		{
			...
			if (graph_version_ != req->current_graph_version()) {
				return errors::Aborted("Current version is ", graph_version_,
						" but caller expected ", req->current_graph_version(),
						".");
			}

			CHECK (execution_state_);
			TF_RETURN_IF_ERROR(
					execution_state_->Extend(req->graph_def(),
							&extended_execution_state));
			...
			*count = (*c)++;
			...
			if (iter == m->end()) {
				...
				VLOG(1) << "Unseen hash " << hash << " for "
						<< BuildGraphOptionsString(opts) << " is_partial = "
						<< is_partial << "\n";
				std::unique_ptr < SimpleClientGraph > client_graph;
				TF_RETURN_IF_ERROR(
						execution_state_->BuildGraph(opts, &client_graph));
				auto entry = new ReffedClientGraph(handle_, opts,
						std::move(client_graph), session_opts_,
						stats_publisher_factory_, execution_state_.get(),
						is_partial, env_->worker_cache);
			...
			PartialRunSetupResponse* resp)
	{
		...
		string handle = std::to_string(
				partial_run_handle_counter_.fetch_add(1));
		...
		rcg->Ref();
		RunState* run_state = new RunState(inputs, outputs, rcg, step_id,
				count);
		{
			mutex_lock l(mu_);
	...
	Status MasterSession::Run(CallOptions* opts,
			const RunStepRequestWrapper& req,
	...
	Status MasterSession::BuildAndRegisterPartitions(ReffedClientGraph* rcg)
	{
		...
		TF_RETURN_IF_ERROR(
				rcg->RegisterPartitions(popts,
						rcg->client_graph()->flib_def->ToProto()));
			...
			MutableRunStepResponseWrapper* resp)
	{
		...
		if (!run_state->step_started) {
			...
			pss.collect_timeline = req.options().trace_level()
			...
			pss.collect_costs = build_cost_model_every > 0
					&& ((count + 1 - build_cost_model_after)
							% build_cost_model_every == 0);

			std::unique_ptr < ProfileHandler > ph =
					run_state->rcg->GetProfileHandler(run_state->step_id, count,
							req.options());
			if (ph) {
		...
		TF_RETURN_IF_ERROR(
				run_state->rcg->CheckFetches(req, run_state,
		...
		bool is_last_partial_run = (run_state->pending_inputs.empty()
				&& run_state->pending_outputs.empty());

		Status s = run_state->rcg->RunPartitions(env_, run_state->step_id,
				run_state->count, execution_state_.get(), &run_state->pss, opts,
				req, resp, cancellation_manager_, is_last_partial_run);
		...
		if (!s.ok() || is_last_partial_run) {
			...
			rcg->ProcessStats(run_state->step_id, &run_state->pss,
					execution_state_.get(), run_state->ph.get(), req.options(),
					resp->mutable_metadata());
			rcg->CleanupPartitionsAsync(run_state->step_id,
					[this, rcg, prun_handle](const Status& s) {
						if (!s.ok()) {
			...
			MutableRunStepResponseWrapper* resp)
	{
		...
		// Keeps the highest 8 bits 0x01: we reserve some bits of the
		// step_id for future use.
		const uint64 step_id = (random::New64() & ((1uLL << 56) - 1))
				| (1uLL << 56);
		TRACEPRINTF("stepid %llu", step_id);
		...
		std::unique_ptr < ProfileHandler > ph = rcg->GetProfileHandler(step_id,
				count, req.options());
		if (ph) {
			pss.collect_timeline = true;
			pss.collect_rpcs = ph->should_collect_rpcs();
		...
		TF_RETURN_IF_ERROR(
				rcg->RunPartitions(env_, step_id, count, execution_state_.get(),
						&pss, opts, req, resp, cancellation_manager_, false));
	...
	Status MasterSession::Close()
	{
		...
		for (ReffedClientGraph* rcg : to_unref)
			rcg->Unref();
		return Status::OK();
	}
