...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, dim < input_shape.dims(),
		...
		OP_REQUIRES(ctx, dim_size > 0,
				errors::InvalidArgument("Reduction axis ", dim,
		...
		args.push_back(
				b.ConstantLiteral(
						*xla::LiteralUtil::CreateR1 < int64
		...
		if (input_shape.dims() > 1) {
			...
			args.push_back(
					b.ConstantLiteral(
							*xla::LiteralUtil::CreateR0 < int32 > (dim)));
		}

		xla::Shape xla_shape = xla::ShapeUtil::MakeShape(xla::S64,
				output_shape.dim_sizes());
