...
public:
	explicit SummaryAudioOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		...
		has_sample_rate_attr_ = context->GetAttr("sample_rate",
				&sample_rate_attr_).ok();
	}
	...
	void Compute(OpKernelContext* c) override
	{
		...
		for (int i = 0; i < N; ++i) {
			...
			auto values = tensor.shaped<float, 3>( { batch_size, length_frames,
					num_channels });
			auto channels_by_frames = typename TTypes<float>::ConstMatrix(
					&values(i, 0, 0),
					Eigen::DSizes<Eigen::DenseIndex, 2>(length_frames,
							num_channels));
			...
			OP_REQUIRES_OK(c,
					wav::EncodeAudioAsS16LEWav(channels_by_frames.data(),
							sample_rate_truncated, num_channels, length_frames,
							sa->mutable_encoded_audio_string()));
		}
