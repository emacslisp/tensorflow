...
template<typename T>
Eigen::Map<
		const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> ToEigenMatrix(
		const Tensor& tensor)
{
...
// efficient.
template<typename T>
bool ExplicitVectorMatrixOptimization(const Tensor& a, const Tensor& b,
...
template<>
bool ExplicitVectorMatrixOptimization<Eigen::half>(const Tensor& a,
		const Tensor& b,
		const Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 1>& dim_pair,
		Tensor* out)
{
...
struct LaunchMatMulCPU {
	static void launch(OpKernelContext* ctx, OpKernel* kernel, const Tensor& a,
			const Tensor& b,
			const Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 1>& dim_pair,
			Tensor* out)
	{
		...
		bool was_vector = ExplicitVectorMatrixOptimization<T>(a, b, dim_pair,
				out);
		if (!was_vector) {
			functor::MatMulFunctor<CPUDevice, T>()(
					ctx->eigen_device<CPUDevice>(), out->matrix<T>(),
					a.matrix<T>(), b.matrix<T>(), dim_pair);
		}
	}
};
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		LaunchMatMul<Device, T, USE_CUBLAS>::launch(ctx, this, a, b, dim_pair,
				out);
	}
...
struct MatMulFunctor<CPUDevice, T> {
	void operator()(const CPUDevice& d, typename MatMulTypes<T>::out_type out,
