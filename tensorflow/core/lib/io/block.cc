...
static inline const char* DecodeEntry(const char* p, const char* limit,
		uint32* shared, uint32* non_shared, uint32* value_length)
{
	if (limit - p < 3)
		return NULL;
	...
	*value_length = reinterpret_cast<const unsigned char*>(p)[2];
	if ((*shared | *non_shared | *value_length) < 128) {
		...
		if ((p = core::GetVarint32Ptr(p, limit, shared)) == NULL)
			return NULL;
		if ((p = core::GetVarint32Ptr(p, limit, non_shared)) == NULL)
			return NULL;
		if ((p = core::GetVarint32Ptr(p, limit, value_length)) == NULL)
			return NULL;
	}
...
public:
	Iter(const char* data, uint32 restarts, uint32 num_restarts) :
			data_(data), restarts_(restarts), num_restarts_(num_restarts), current_(
					restarts_), restart_index_(num_restarts_)
	{
