...
TempBufferOffsets::TempBufferOffsets(const BufferAssignment& buffer_assignment)
{
	...
	for (auto i = 0; i < buffer_assignment.Allocations().size(); ++i) {
		...
		if (allocation.IsPreallocatedTempBuffer()) {
			InsertOrDie(&buffer_index_to_offset_, i,
					total_size_of_temp_buffers_);
			total_size_of_temp_buffers_ += RoundUpToAlignment(
					allocation.size());
		}
	}
}
