...
public:
	...
	{
		return GetOpIdFor("INPUT");
	}
	int GetOutputNodeOpId() const final
	{
		return GetOpIdFor("OUTPUT");
	}
...
static GraphDef CreateConvGraphDef()
{
	...
	Output conv = ops::Conv2D(root.WithOpName("conv"), input, filter, strides,
			"SAME");
...
static GraphDef CreatePoolGraphDef()
{
	...
	const std::vector<int> strides { 1, 1, 1, 1 };
	Output max_pool = ops::MaxPool(root.WithOpName("maxpool"), input, ksize,
			strides, "SAME");
...
{
	for (const GraphTransferer::ConstNodeTransferParams& params : gt.GetConstNodeParams()) {
		if (params.name == name) {
			return &params;
		}
	}
...
static const GraphTransferer::NodeTransferParams* FindOpNodeParams(
		const GraphTransferer& gt, const string& name)
{
	for (const GraphTransferer::NodeTransferParams& params : gt.GetOpNodeParams()) {
		if (params.name == name) {
			return &params;
		}
	}
...
{
	for (const GraphTransferer::NodeInputParams& params : gt.GetNodeInputParams()) {
...
static const GraphTransferer::NodeOutputParams* FindNodeOutputParams(
		const GraphTransferer& gt, const int node_id)
{
	for (const GraphTransferer::NodeOutputParams& params : gt.GetNodeOutputParams()) {
		if (params.node_id == node_id) {
			return &params;
		}
	}
...
static void SanityCheckNodes(const GraphTransferer& gt)
{
	for (const GraphTransferer::NodeTransferParams& params : gt.GetOpNodeParams()) {
		if (params.inputs_size > 0) {
			...
			for (const std::tuple<int, int>& pair : input_params->input_node_id_and_output_port_list) {
