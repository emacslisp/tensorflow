...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(2, out_weights_shape, &out_weights));
		...
		for (int i = 0; i < num_data; i++) {
			...
			for (int j = num_nodes - 1; j >= 0; j--) {
				tensorforest::GetFeatureSet(layer_num_, j, random_seed_,
						num_features, num_features_per_node, &feature_set);
