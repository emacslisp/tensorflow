...
void SaveTensors(OpKernelContext* context,
	...
	OP_REQUIRES(context, context->num_inputs() == N + kFixedInputs,
			errors::InvalidArgument("Expected totally ", N + kFixedInputs,
					" inputs as input #1 (which is a string "
							"tensor of saved names) contains ", N,
					" names, but received ", context->num_inputs(), " inputs"));
	...
	for (int i = 0; i < N; ++i) {
		...
		if (save_slices && !tensor_shapes_and_slices_ptr[i].empty()) {
			...
			OP_REQUIRES_OK(context,
					checkpoint::ParseShapeAndSlice(shape_spec, &shape, &slice,
							&slice_shape));
			OP_REQUIRES(context, slice_shape.IsSameSize(input.shape()),
					errors::InvalidArgument("Slice in shape_and_slice "
							"specification does not match the "
							"shape of the tensor to  save: ", shape_spec,
							", tensor: ", input.shape().DebugString()));
		}
...
#define WRITER_ADD(T)                                           \
  case DataTypeToEnum<T>::value:                                \
	...
	default:
		context->SetStatus(
				errors::Unimplemented("Saving data type ",
						DataTypeString(input.dtype()), " not yet supported"));
		return;
		}
#undef WRITER_ADD
		if (!s.ok()) {
		...
		int preferred_shard, bool restore_slice)
{
	...
	if (restore_slice) {
		...
		OP_REQUIRES(context, tensor_shape_and_slice_t.NumElements() == 1,
				errors::InvalidArgument("Expected 1 element for the tensor "
						"shape and slice but got ",
						tensor_shape_and_slice_t.NumElements()));
		tensor_shape_and_slice_ptr =
				tensor_shape_and_slice_t.flat<string>().data();
	}
	...
	if (!reader) {
		allocated_reader.reset(
				new checkpoint::TensorSliceReader(file_pattern, open_func,
						preferred_shard));
	...
	OP_REQUIRES(context, type == context->expected_output_dtype(0),
	...
	if (restore_slice && !tensor_shape_and_slice_ptr[0].empty()) {
		...
		OP_REQUIRES_OK(context,
				checkpoint::ParseShapeAndSlice(shape_spec, &parsed_shape,
						&slice_to_load, &output_shape));
		OP_REQUIRES(context, parsed_shape.IsSameSize(saved_shape),
				errors::InvalidArgument(
						"Shape in shape_and_slice spec does not match the shape in the "
								"save file: ", parsed_shape.DebugString(),
...
default:
	context->SetStatus(
			errors::Unimplemented("Restoring data type ", DataTypeString(type),
					" not yet supported"));
	}
#undef READER_COPY
}
...
Status RestoreTensorsV2(OpKernelContext* context, const Tensor& prefix,
		const Tensor& tensor_names, const Tensor& shape_and_slices,
		gtl::ArraySlice<DataType> dtypes)
{
	...
	for (size_t i = 0; i < tensor_names_flat.size(); ++i) {
		...
		if (shape_and_slice.empty()) {
			...
			TF_RETURN_IF_ERROR(
					context->allocate_output(i, restored_full_shape,
							&restored_tensor));
			...
			TF_RETURN_IF_ERROR(
					checkpoint::ParseShapeAndSlice(shape_and_slice,
							&parsed_full_shape, &parsed_slice,
							&parsed_slice_shape));
			if (!restored_full_shape.IsSameSize(parsed_full_shape)) {
				return errors::InvalidArgument("Shape in shape_and_slice spec ",
			...
			TF_RETURN_IF_ERROR(
					context->allocate_output(i, parsed_slice_shape,
							&restored_tensor));
			TF_RETURN_IF_ERROR(
					reader.LookupSlice(tensor_name, parsed_slice,
							restored_tensor));
		}
