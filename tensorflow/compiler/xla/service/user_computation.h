...
public:
	...
	explicit UserComputation(const string& name,
			const ComputationHandle& handle);
...
private:
	...
	// computation.
	const OperationRequest& GetRoot(
			VersionedComputationHandle::Version version) const
	EXCLUSIVE_LOCKS_REQUIRED (mutex_);
	...
	SessionComputation
	session_computation_ GUARDED_BY(mutex_);
	...
	// sequentially.
	int64
	next_handle_value_ GUARDED_BY(mutex_);
	...
	handle_to_return_ GUARDED_BY(mutex_);
	...
	mutable int64 program_shape_version_ GUARDED_BY(mutex_) = 0;mutable std::shared_ptr<const ProgramShape> program_shape_ GUARDED_BY(mutex_);
