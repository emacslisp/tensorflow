...
Status PosixFileSystem::NewReadOnlyMemoryRegionFromFile(const string& fname,
		std::unique_ptr<ReadOnlyMemoryRegion>* result)
{
	...
	if (fd < 0) {
		...
		struct stat st;
		::fstat(fd, &st);
		const void* address = mmap(nullptr, st.st_size, PROT_READ, MAP_PRIVATE,
				fd, 0);
...
{
	Status result;
	if (rename(TranslateName(src).c_str(), TranslateName(target).c_str())
			!= 0) {
