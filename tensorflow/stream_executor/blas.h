...
enum class Transpose {
	kNoTranspose, kTranspose, kConjugateTranspose
};
...
public:
	...
	virtual bool DoBlasRotg(Stream *stream,
			DeviceMemory<std::complex<float>> *a,
			DeviceMemory<std::complex<float>> *b, DeviceMemory<float> *c,
			DeviceMemory<std::complex<float>> *s) = 0;
	virtual bool DoBlasRotg(Stream *stream,
			DeviceMemory<std::complex<double>> *a,
	...
	virtual bool DoBlasRotm(Stream *stream, uint64 elem_count,
			DeviceMemory<double> *x, int incx, DeviceMemory<double> *y,
			int incy, const DeviceMemory<double> &param) = 0;
	...
	virtual bool DoBlasRotmg(Stream *stream, DeviceMemory<float> *d1,
			...
			const DeviceMemory<float> &y1, DeviceMemory<float> *param) = 0;
	virtual bool DoBlasRotmg(Stream *stream, DeviceMemory<double> *d1,
			DeviceMemory<double> *d2, DeviceMemory<double> *x1,
			const DeviceMemory<double> &y1, DeviceMemory<double> *param) = 0;
	...
	virtual bool DoBlasScal(Stream *stream, uint64 elem_count,
			std::complex<float> alpha, DeviceMemory<std::complex<float>> *x,
			int incx) = 0;
	virtual bool DoBlasScal(Stream *stream, uint64 elem_count,
			std::complex<double> alpha, DeviceMemory<std::complex<double>> *x,
			int incx) = 0;

	// Swaps a vector with another vector.
	virtual bool DoBlasSwap(Stream *stream, uint64 elem_count,
			DeviceMemory<float> *x, int incx, DeviceMemory<float> *y,
			int incy) = 0;
	virtual bool DoBlasSwap(Stream *stream, uint64 elem_count,
			DeviceMemory<double> *x, int incx, DeviceMemory<double> *y,
			int incy) = 0;
	...
	virtual bool DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
			...
			int incx, float beta, DeviceMemory<float> *y, int incy) = 0;
	virtual bool DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
	...
	virtual bool DoBlasGer(Stream *stream, uint64 m, uint64 n, float alpha,
			const DeviceMemory<float> &x, int incx,
			const DeviceMemory<float> &y, int incy, DeviceMemory<float> *a,
			int lda) = 0;
	virtual bool DoBlasGer(Stream *stream, uint64 m, uint64 n, double alpha,
			const DeviceMemory<double> &x, int incx,
			const DeviceMemory<double> &y, int incy, DeviceMemory<double> *a,
			int lda) = 0;
	...
	virtual bool DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
			...
			const DeviceMemory<float> &a, int lda, DeviceMemory<float> *x,
			int incx) = 0;
	virtual bool DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
			blas::Transpose trans, blas::Diagonal diag, uint64 n, uint64 k,
			const DeviceMemory<double> &a, int lda, DeviceMemory<double> *x,
			int incx) = 0;
	virtual bool DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
			blas::Transpose trans, blas::Diagonal diag, uint64 n, uint64 k,
			const DeviceMemory<std::complex<float>> &a, int lda,
	...
	virtual bool DoBlasGemm(Stream *stream, blas::Transpose transa,
			...
			int ldb, float beta, DeviceMemory<float> *c, int ldc) = 0;
	virtual bool DoBlasGemm(Stream *stream, blas::Transpose transa,
	...
	virtual bool DoBlasGemm(Stream *stream, blas::Transpose transa,
			...
			std::complex<float> beta, DeviceMemory<std::complex<float>> *c,
			int ldc) = 0;
	virtual bool DoBlasGemm(Stream *stream, blas::Transpose transa,
			blas::Transpose transb, uint64 m, uint64 n, uint64 k,
			std::complex<double> alpha,
	...
	virtual bool DoBlasGemmBatched(Stream *stream, blas::Transpose transa,
			...
			const port::ArraySlice<DeviceMemory<float> *> &b, int ldb,
			float beta, const port::ArraySlice<DeviceMemory<float> *> &c,
			int ldc, int batch_count, ScratchAllocator *scratch_allocator) = 0;
	virtual bool DoBlasGemmBatched(Stream *stream, blas::Transpose transa,
			blas::Transpose transb, uint64 m, uint64 n, uint64 k, double alpha,
			const port::ArraySlice<DeviceMemory<double> *> &a, int lda,
			const port::ArraySlice<DeviceMemory<double> *> &b, int ldb,
			double beta, const port::ArraySlice<DeviceMemory<double> *> &c,
			int ldc, int batch_count, ScratchAllocator *scratch_allocator) = 0;
	virtual bool DoBlasGemmBatched(Stream *stream, blas::Transpose transa,
			blas::Transpose transb, uint64 m, uint64 n, uint64 k,
			std::complex<float> alpha,
			const port::ArraySlice<DeviceMemory<std::complex<float>> *> &a,
			int lda,
			const port::ArraySlice<DeviceMemory<std::complex<float>> *> &b,
			int ldb, std::complex<float> beta,
			const port::ArraySlice<DeviceMemory<std::complex<float>> *> &c,
			int ldc, int batch_count, ScratchAllocator *scratch_allocator) = 0;
	virtual bool DoBlasGemmBatched(Stream *stream, blas::Transpose transa,
			blas::Transpose transb, uint64 m, uint64 n, uint64 k,
			std::complex<double> alpha,
	...
	virtual bool DoBlasTrmm(Stream *stream, blas::Side side,
			...
			uint64 m, uint64 n, float alpha, const DeviceMemory<float> &a,
			int lda, DeviceMemory<float> *b, int ldb) = 0;
	virtual bool DoBlasTrmm(Stream *stream, blas::Side side,
			blas::UpperLower uplo, blas::Transpose transa, blas::Diagonal diag,
			uint64 m, uint64 n, double alpha, const DeviceMemory<double> &a,
			int lda, DeviceMemory<double> *b, int ldb) = 0;
	virtual bool DoBlasTrmm(Stream *stream, blas::Side side,
			blas::UpperLower uplo, blas::Transpose transa, blas::Diagonal diag,
