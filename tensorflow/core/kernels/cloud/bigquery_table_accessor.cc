...
Status BigQueryTableAccessor::New(const string& project_id,
		const string& dataset_id, const string& table_id,
		...
		std::unique_ptr<BigQueryTableAccessor>* accessor)
{
	...
	if (auth_provider == nullptr && http_request_factory == nullptr) {
		accessor->reset(
				new BigQueryTableAccessor(project_id, dataset_id, table_id,
						timestamp_millis, row_buffer_size, big_query_end_point,
						columns, partition));
	} else {
		accessor->reset(
				new BigQueryTableAccessor(project_id, dataset_id, table_id,
...
BigQueryTableAccessor::BigQueryTableAccessor(const string& project_id,
		const string& dataset_id, const string& table_id,
		int64 timestamp_millis, int64 row_buffer_size, const string& end_point,
		const std::vector<string>& columns,
		const BigQueryTablePartition& partition) :
		BigQueryTableAccessor(project_id, dataset_id, table_id,
				timestamp_millis, row_buffer_size, end_point, columns,
				partition,
...
BigQueryTableAccessor::BigQueryTableAccessor(const string& project_id,
		...
		std::unique_ptr<AuthProvider> auth_provider,
		std::unique_ptr<HttpRequest::Factory> http_request_factory) :
		project_id_(project_id), dataset_id_(dataset_id), table_id_(table_id), timestamp_millis_(
				timestamp_millis), columns_(columns.begin(), columns.end()), bigquery_end_point_(
				end_point), partition_(partition), auth_provider_(
				std::move(auth_provider)), http_request_factory_(
				std::move(http_request_factory))
{
	row_buffer_.resize(row_buffer_size);
	Reset();
	...
	if (next_row_in_buffer_ != -1
		...
		*example = row_buffer_[next_row_in_buffer_];
		...
		if (!next_page_token_.empty()) {
			TF_RETURN_IF_ERROR(
					request->SetUri(
							strings::StrCat(BigQueryUriPrefix(),
									"data?maxResults=", ComputeMaxResultsArg(),
									"&pageToken=",
									request->EscapeString(next_page_token_))));
		...
		TF_RETURN_WITH_CONTEXT_IF_ERROR(request->Send(),
				" when reading rows from ", FullTableName());

		// Parse the returned row.
		StringPiece response_piece = StringPiece(&output_buffer[0],
				output_buffer.size());
		Json::Value root;
		...
		for (unsigned int i = 0; i < root["rows"].size(); ++i) {
			row_buffer_[i].Clear();
			TF_RETURN_IF_ERROR(
					ParseColumnValues(root["rows"][i], schema_root_,
							&row_buffer_[i]));
		}
...
int64 BigQueryTableAccessor::ComputeMaxResultsArg()
{
	...
	return std::min(static_cast<int64>(row_buffer_.size()),
			static_cast<int64>(partition_.end_index() - partition_.start_index()
					+ 1));
}

Status BigQueryTableAccessor::ParseColumnValues(const Json::Value& value,
		const SchemaNode& root_schema_node, Example* example)
{
	if (value.empty()) {
		return Status::OK();
	}
	...
	for (const auto& schema_node : root_schema_node.schema_nodes) {
		...
		if (schema_node.type == ColumnType::kRecord) {
			TF_RETURN_IF_ERROR(
					ParseColumnValues(value["f"][value_index]["v"], schema_node,
							example));
		} else {
			...
			if (columns_.empty()
				...
				TF_RETURN_IF_ERROR(
						AppendValueToExample(schema_node.name,
								value["f"][value_index]["v"], schema_node.type,
								example));
			}
		}
		value_index++;
...
Status BigQueryTableAccessor::ReadSchema()
{
	...
	TF_RETURN_IF_ERROR(request->SetResultBuffer(&output_buffer));
	TF_RETURN_WITH_CONTEXT_IF_ERROR(request->Send(),
			" when reading schema for ", FullTableName());
...
Status BigQueryTableAccessor::ExtractColumnType(const Json::Value& columns,
		const string& column_name_prefix, SchemaNode* root)
{
	for (auto columns_it = columns.begin(); columns_it != columns.end();
		...
		ColumnType type;
		const string current_column_name = strings::StrCat(column_name_prefix,
				(*columns_it)["name"].asString().c_str());
		TF_RETURN_IF_ERROR(
				ParseColumnType((*columns_it)["type"].asString().c_str(),
						&type));
		...
		if (type == ColumnType::kRecord) {
			...
			TF_RETURN_IF_ERROR(
					ExtractColumnType((*columns_it)["fields"], new_prefix,
...
Status BigQueryTableAccessor::AppendValueToExample(const string& column_name,
