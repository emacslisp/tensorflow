...
const string DebuggerState::SummarizeDebugTensorWatches()
{
	...
	for (const DebugTensorWatch& watch : watches) {
		string tensor_name = strings::StrCat(watch.node_name(), ":",
				watch.output_slot());
		oss << tensor_name << "|";
		...
		Graph* graph, Device* device)
{
	...
	for (Node* src_node : graph->nodes()) {
		...
		for (const Edge* edge : src_node->out_edges()) {
			...
			if (output_slot_to_edges.find(src_output)
					== output_slot_to_edges.end()) {
		...
		for (int src_output_slot = 0; src_output_slot < src_node->num_outputs();
				++src_output_slot) {
			const string tensor_name = strings::StrCat(src_node->name(), ":",
					src_output_slot);
			...
			Node* copy_node;
			Status copy_s = CreateCopyNode(graph, device_type,
					memory_type == HOST_MEMORY, src_node->name(),
					src_output_slot, src_dt, tensor_name, &copy_node);
			if (!copy_s.ok()) {
				return Status(error::FAILED_PRECONDITION,
						strings::StrCat(
								"Failed to create Copy/CopyHost node for tensor ",
								tensor_name, ", due to: ",
								copy_s.error_message()));
			}
			...
			for (size_t i = 0; i < tensor_watches[tensor_name].size(); ++i) {
				...
				Node* debug_node;
				Status debug_s = CreateDebugNode(graph, device_type,
						copy_node->name(), src_dt, tensor_name,
						tensor_watch_urls[tensor_name], i, debug_op_name,
						&debug_node);
				if (!debug_s.ok()) {
					return Status(error::FAILED_PRECONDITION,
							strings::StrCat("Failed to create debug node ",
									debug_op_name, " for tensor ", tensor_name,
			...
			const bool is_ref = IsRefType(
					src_node->output_type(src_output_slot));
			...
			for (const Edge* edge : output_slot_to_edges[src_output_slot]) {
				...
				if (!is_ref) {
					...
					graph->AddEdge(copy_node, 0, edge->dst(),
							edge->dst_input());
				}
...
// static
Status DebugNodeInserter::CreateCopyNode(Graph* graph,
		const DeviceType device_type, const bool is_host_memory,
		const string& src_node_name, const int src_output,
		const DataType src_dt, const string& tensor_name, Node** copy_node)
{
	NodeDef node_def;
	...
	auto builder = NodeDefBuilder(copy_node_name, copy_op_name).Input(
			src_node_name, src_output, src_dt);

	if (!builder.Finalize(&node_def).ok()) {
		return Status(error::FAILED_PRECONDITION,
				strings::StrCat("Failed to create node definition ",
						"for copy op ", copy_node_name, " on watched tensor ",
						tensor_name));
	}
...
// static
Status DebugNodeInserter::CreateDebugNode(Graph* graph,
		const DeviceType device_type, const string& src_copy_node_name,
		...
		const string& debug_op_name, Node** debug_node)
{
	...
	const string debug_node_name = GetDebugNodeName(tensor_name, debug_op_num,
			debug_op_name);
	auto builder =
			NodeDefBuilder(debug_node_name, debug_op_name).Input(
					src_copy_node_name, 0, src_dt).Attr("tensor_name",
					tensor_name).Attr("debug_urls", debug_urls);

	if (!builder.Finalize(&node_def).ok()) {
		return Status(error::FAILED_PRECONDITION,
				strings::StrCat("Failed to create node definition ",
						"for debug op ", debug_op_name, " on watched tensor ",
						tensor_name));
	}
