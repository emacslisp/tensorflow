...
Status HloCostAnalysis::HandleClamp(HloInstruction* clamp,
		HloInstruction* min_instruction, HloInstruction* arg_instruction,
...
Status HloCostAnalysis::HandleDynamicSlice(HloInstruction* slice,
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	return Status::OK();
}

Status HloCostAnalysis::HandleDynamicUpdateSlice(HloInstruction* dynamic_update,
...
Status HloCostAnalysis::HandleConcatenate(HloInstruction* concatenate,
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	return Status::OK();
}
		...
		HloInstruction* operand, const Window& window, HloComputation* function)
{
	...
	auto hlo_flop_count = output_size * (window_size - 1)
			* visitor.flop_count();
	hlo_to_flop_count_[reduce_window] = hlo_flop_count;
	flop_count_ += hlo_flop_count;
	transcendental_count_ += output_size * (window_size - 1)
			* visitor.transcendental_count();
	return Status::OK();
}
...
Status HloCostAnalysis::HandleSelectAndScatter(HloInstruction* instruction)
{
	...
	auto hlo_flop_count = source_element_count
			* ((window_size - 1) * select_visitor.flop_count()
					+ scatter_visitor.flop_count());
	hlo_to_flop_count_[instruction] = hlo_flop_count;
	flop_count_ += hlo_flop_count;
	transcendental_count_ += source_element_count
...
Status HloCostAnalysis::HandlePad(HloInstruction* pad)
{
	return Status::OK();
}
		...
		const Window& window)
{
	...
	const int64 output_features = convolution->shape().dimensions(
			dnums.feature_dimension());

	// For each output element, we do one fma per element in the
	// kernel at some given output feature index.
	const int64 fmas_per_output_element = ShapeUtil::ElementsIn(
			rhs_instruction->shape()) / output_features;
...
Status HloCostAnalysis::HandleCustomCall(HloInstruction* custom_call,
		tensorflow::gtl::ArraySlice<HloInstruction*> operands,
