...
public:
	...
	DeviceMemory<T> AllocateSubBuffer(DeviceMemory<T> *parent,
			uint64 element_offset, uint64 element_count);
	...
	ScopedDeviceMemory<T> AllocateOwnedSubBuffer(DeviceMemory<T> *parent,
			uint64 element_offset, uint64 element_count)
	{
		return ScopedDeviceMemory<T>(this,
				AllocateSubBuffer<T>(parent, element_offset, element_count));
	}
	...
	bool SynchronousMemZero(DeviceMemoryBase *location, uint64 size)
	SE_MUST_USE_RESULT;

	// Blocks the caller while "size" bytes are initialized to "value" (in POD
	// fashion) at the given location in GPU memory.
	bool SynchronousMemSet(DeviceMemoryBase *location, int value, uint64 size)
	SE_MUST_USE_RESULT;
	...
	port::Status SynchronousMemcpyD2H(const DeviceMemoryBase &gpu_src,
	...
	bool MemZero(Stream *stream, DeviceMemoryBase *location, uint64 size)
	SE_MUST_USE_RESULT;
		...
		ScopedDeviceMemory(parent, parent->AllocateArray<ElemT>(values.size()))
{
	if (ptr() != nullptr) {
		...
		if (!parent->SynchronousMemcpy(ptr(),
				const_cast<const ElemT *>(&local[0]), ptr()->size())) {
			Reset(nullptr);
		}
	}
}
...
DeviceMemory<T> StreamExecutor::AllocateZeroed()
{
	...
	DeviceMemory<T> result = DeviceMemory<T>::MakeFromByteSize(opaque,
			sizeof(T));
		...
		uint64 element_offset, uint64 element_count)
{
	...
	void *opaque = implementation_->AllocateSubBuffer(parent,
			sizeof(T) * element_offset, sizeof(T) * element_count);
	if (opaque == nullptr) {
		return DeviceMemory<T> { };
	}
	CreateAllocRecord(opaque, sizeof(T) * element_count);
	return DeviceMemory<T>(
			DeviceMemoryBase(opaque, sizeof(T) * element_count,
					true /* = is_sub_buffer */));
}
...
{
	KernelInvocationChecker<std::tuple<Params...>, std::tuple<Args...>>::CheckAllStaticAssert();
	if (ok()) {
		...
		bool ok = parent_->Launch(this, thread_dims, block_dims, kernel,
				kernel_args);
		if (!ok) {
			SetError();
