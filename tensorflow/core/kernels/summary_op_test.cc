...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "ScalarSummary").Input(FakeInput()).Input(
						FakeInput(dt)).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "HistogramSummary").Input(FakeInput()).Input(
						FakeInput(dt)).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "MergeSummary").Input(
						FakeInput(num_inputs)).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
