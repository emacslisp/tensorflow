...
typedef std::unique_ptr<TF_Tensor, decltype(&TF_DeleteTensor)> Safe_TF_TensorPtr;
		...
		size_t* size, void** buffer)
{
	...
	*size = 0;
	TF_RETURN_IF_ERROR(
			PyBytesArrayMap(array,
					[&size](char* ptr, Py_ssize_t len) {
						*size +=
	...
	TF_RETURN_IF_ERROR(
			PyBytesArrayMap(array,
					[&base, &data_start, &dst, &offsets](char* ptr, Py_ssize_t len) {
...
static Status TF_StringTensor_GetPtrAndLen(const TF_Tensor* src,
		tensorflow::int64 num_elements, tensorflow::int64 i, const char** ptr,
		tensorflow::uint64* len)
{
	...
	const char* p = tensorflow::core::GetVarint64Ptr(data_start + offset, limit,
			len);
	if (static_cast<int64>(offset) >= (limit - data_start) || !p
			|| static_cast<int64>(*len) > (limit - p)) {
		return errors::InvalidArgument("Malformed TF_STRING tensor; element ",
				i, " out of range");
	}
	*ptr = p;
	return Status::OK();
...
static Status CopyStringToPyArrayElement(PyArrayObject* pyarray, void* i_ptr,
		TF_Tensor* tensor, tensorflow::int64 num_elements, tensorflow::int64 i)
{
	...
	int success = PyArray_SETITEM(pyarray,
			static_cast<char*>(PyArray_ITER_DATA(i_ptr)), py_string.get());
	if (success != 0) {
...
Status TF_Tensor_to_PyObject(TF_Tensor* tensor, PyObject** out_array)
{
	...
	Safe_PyObjectPtr safe_out_array = tensorflow::make_safe(
			PyArray_Empty(ndims, dims.data(), descr, 0));
	if (!safe_out_array) {
	...
	if (PyArray_NBYTES(py_array)
		...
		if (TF_TensorType(tensor) == TF_STRING) {
			...
			auto iter = tensorflow::make_safe(
					PyArray_IterNew(safe_out_array.get()));
			for (tensorflow::int64 i = 0; i < nelems; ++i) {
				auto s = CopyStringToPyArrayElement(py_array, iter.get(),
						tensor, nelems, i);
				if (!s.ok()) {
					return s;
				}
...
void TF_Run_wrapper_helper(TF_DeprecatedSession* session, const char* handle,
		...
		const NameVector& output_names, const NameVector& target_nodes,
		...
		TF_Buffer* run_outputs)
{
	...
	while (PyDict_Next(feed_dict, &pos, &key, &value)) {
		...
		Safe_PyObjectPtr array_safe(
				make_safe(
						PyArray_FromAny(value, nullptr, 0, 0, NPY_ARRAY_CARRAY,
								nullptr)));
		if (!array_safe) {
			Set_TF_Status_from_Status(out_status,
					errors::InvalidArgument(kFeedDictErrorMsg));
		...
		PyArrayObject* array =
				reinterpret_cast<PyArrayObject*>(array_safe.get());
		...
		if (dtype != TF_STRING) {
			...
			TF_Tensor* tensor = TF_AllocateTensor(dtype, dims.data(),
					dims.size(), size);
			...
			inputs_safe.emplace_back(
					make_safe(
							TF_NewTensor(dtype, dims.data(), dims.size(),
									encoded, size,
	...
	if (handle == nullptr) {
		TF_Run(session, run_options, input_names.data(), inputs_unsafe.data(),
				...
				output_names.size(),
				const_cast<const char**>(target_nodes.data()),
				target_nodes.size(), run_outputs, out_status);
	} else {
...
{
	TF_Run_wrapper_helper(session, nullptr, run_options, feed_dict,
			output_names, target_nodes, out_status, out_values, run_outputs);
}
...
{
	TF_Reset(opt, const_cast<const char**>(containers.data()),
			containers.size(), out_status);
}
