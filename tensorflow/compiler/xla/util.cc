...
Status MaybeAddBacktrace(Status prior)
{
	...
	if (legacy_flags::GetUtilFlags()->xla_status_add_backtrace) {
		return Status { prior.code(), tensorflow::strings::StrCat(
				prior.error_message(), " :: ", tensorflow::CurrentStackTrace()) };
	} else {
		return prior;
	}
}
...
{
	CHECK(!prior.ok());
	return Status { prior.code(), tensorflow::strings::StrCat(context, ": ",
			prior.error_message()) };
}
		...
		tensorflow::gtl::ArraySlice<int64> input_permutation)
{
	...
	DCHECK_EQ(0,
			std::count(output_permutation.begin(), output_permutation.end(),
					-1));
	DCHECK(
			std::is_permutation(input_permutation.begin(),
					input_permutation.end(), output_permutation.begin()));
