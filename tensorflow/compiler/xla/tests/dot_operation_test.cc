...
protected:
	...
	void TestVectorDot();
	template<typename Element>
	void TestSquareMatrixDot(bool lhs_row_major = false, bool rhs_row_major =
			false);
	template<typename Element>
	void TestNonsquareMatrixDot(bool lhs_row_major = false, bool rhs_row_major =
			false);
};
		...
		bool rhs_row_major)
{
	auto lhs_handle =
			client_->TransferToServer(
					*test_utils::CreateR2LiteralWithLayout<Element>( { { 1.0,
							2.0 }, { 3.0, -4.0 } },
							MinorToMajorForIsRowMajor(lhs_row_major))).ConsumeValueOrDie();
	auto rhs_handle =
			client_->TransferToServer(
					*test_utils::CreateR2LiteralWithLayout<Element>( { { 1.0,
							6.0 }, { 7.0, -4.0 } },
							MinorToMajorForIsRowMajor(rhs_row_major))).ConsumeValueOrDie();

	ComputationBuilder builder(client_, TestName());
	auto prim_type = primitive_util::NativeToPrimitiveType<Element>();
	auto result = builder.Dot(
			builder.Parameter(0, ShapeUtil::MakeShape(prim_type, { 2, 2 }),
					"lhs"),
			builder.Parameter(1, ShapeUtil::MakeShape(prim_type, { 2, 2 }),
					"rhs"));

	Array2D<Element> expected( { { 15.0, -2.0 }, { -25.0, 34.0 } });
	ComputeAndCompareR2<Element>(&builder, expected, { lhs_handle.get(),
			rhs_handle.get() }, error_spec_);
}
		...
		bool rhs_row_major)
{
	...
	auto prim_type = primitive_util::NativeToPrimitiveType<Element>();
	auto result = builder.Dot(
			builder.Parameter(0, ShapeUtil::MakeShape(prim_type, { 2, 3 }),
					"lhs"),
			builder.Parameter(1, ShapeUtil::MakeShape(prim_type, { 3, 2 }),
					"rhs"));
