	...
	ComputeOptions(OpKernelConstruction* const context)
	{
		...
		if (loss_type == "logistic_loss") {
			...
			OP_REQUIRES(context, false,
					errors::InvalidArgument("Unsupported loss type: ",
							loss_type));
		}
		OP_REQUIRES_OK(context, context->GetAttr("adaptative", &adaptative));
		OP_REQUIRES_OK(context,
				context->GetAttr("num_sparse_features", &num_sparse_features));
		OP_REQUIRES_OK(context,
		...
		OP_REQUIRES(context, num_sparse_features + num_dense_features > 0,
				errors::InvalidArgument(
						"Requires at least one feature to train."));
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument(
						strings::Printf("Too many feature groups: %lld > %d",
								...
								std::numeric_limits<int>::max())));
		OP_REQUIRES_OK(context,
				context->GetAttr("num_loss_partitions", &num_loss_partitions));
		OP_REQUIRES_OK(context,
				context->GetAttr("num_inner_iterations",
...
void DoCompute(const ComputeOptions& options, OpKernelContext* const context)
{
	...
	Examples examples;
	OP_REQUIRES_OK(context,
			examples.Initialize(context, model_weights,
					options.num_sparse_features,
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (weights_inputs.size() > 0) {
			...
			for (int i = 0; i < weights_inputs.size(); ++i) {
				num_weights +=
						weights_inputs.at(i, /*lock_held=*/true).NumElements();
			}
			// TODO(sibyl-Aix6ihai): Tune this value.
			const int64 kCostPerUnit = (num_weights * 50)
					/ weights_inputs.size();
			const DeviceBase::CpuWorkerThreads& worker_threads =
					*context->device()->tensorflow_cpu_worker_threads();
			Shard(worker_threads.num_threads, worker_threads.workers,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int64 i = 0; i < num_elements; ++i) {
			...
			out_values(i, 0) =
					TF_PREDICT_TRUE(fprint.low64 >= 2) ?
							fprint.low64 :
