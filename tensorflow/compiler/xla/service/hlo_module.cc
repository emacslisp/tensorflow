		...
		const std::unordered_map<HloComputation*, HloComputation*>& replacements)
{
	...
	for (std::unique_ptr<HloComputation>& computation : computations_) {
		for (auto& instruction : computation->instructions()) {
			...
			case HloOpcode::kWhile: {
				HloComputation* new_condition =
						tensorflow::gtl::FindWithDefault(replacements,
								instruction->while_condition(), nullptr);
	...
	entry_computation_ = tensorflow::gtl::FindWithDefault(replacements,
			entry_computation_, entry_computation_);
		...
		const string& outlined_computation_name, HloComputation* computation)
{
	...
	for (HloInstruction* instruction_to_outline : instructions_to_outline) {
		...
		HloInstruction* outlined_instruction = builder.AddInstruction(
				instruction_to_outline->Clone());

		// Replace its operands to their counterparts in the new function.
		for (int64 operand_num = 0;
				operand_num < outlined_instruction->operand_count();
				++operand_num) {
			HloInstruction* old_operand = outlined_instruction->mutable_operand(
					operand_num);

			HloInstruction** operand_slot =
					&(outlined_instructions[old_operand]);
			if (*operand_slot == nullptr) {
				...
				*operand_slot = builder.AddInstruction(
						HloInstruction::CreateParameter(parameter_count,
								old_operand->shape(), ""));
				++parameter_count;
			}
			TF_CHECK_OK(
	...
	HloInstruction* call = computation->AddInstruction(
			HloInstruction::CreateCall(output->shape(), arguments,
					nested_computation));
...
std::list<HloComputation*> HloModule::MakeComputationPostOrder() const
{
	...
	for (auto& computation : computations_) {
		if (nonroot_computations.count(computation.get()) == 0) {
			for (HloComputation* embedded_computation : computation->MakeEmbeddedComputationsList()) {
