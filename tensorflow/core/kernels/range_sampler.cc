...
{
	SampleBatchGetExpectedCount(rnd, unique, batch,
			gtl::MutableArraySlice<float>(), gtl::ArraySlice<int64>(),
			gtl::MutableArraySlice<float>());
}

void RangeSampler::SampleBatchGetExpectedCount(random::SimplePhilox* rnd,
		bool unique, gtl::MutableArraySlice<int64> batch,
		...
		gtl::MutableArraySlice<float> extras_expected_count) const
{
	SampleBatchGetExpectedCountAvoid(rnd, unique, batch, batch_expected_count,
			extras, extras_expected_count, gtl::ArraySlice<int64>());
}
...
void RangeSampler::SampleBatchGetExpectedCountAvoid(random::SimplePhilox* rnd,
		bool unique, MutableArraySlice<int64> batch,
		...
		ArraySlice<int64> avoided_values) const
{
	...
	if (batch_expected_count.size() > 0) {
		...
		for (int i = 0; i < batch_size; i++) {
			batch_expected_count[i] = ExpectedCountHelper(Probability(batch[i]),
					batch_size, num_tries);
		}
	}
	CHECK_EQ(extras.size(), extras_expected_count.size());
	for (size_t i = 0; i < extras.size(); i++) {
		extras_expected_count[i] = ExpectedCountHelper(Probability(extras[i]),
				batch_size, num_tries);
	}
}
...
void AllSampler::SampleBatchGetExpectedCountAvoid(random::SimplePhilox* rnd,
		bool unique, MutableArraySlice<int64> batch,
		MutableArraySlice<float> batch_expected_count, ArraySlice<int64> extras,
		MutableArraySlice<float> extras_expected_count,
		ArraySlice<int64> avoided_values) const
...
float UniformSampler::Probability(int64 value) const
{
	return inv_range_;
}
...
{
	const int64 value = static_cast<int64>(exp(rnd->RandDouble() * log_range_))
			- 1;
	CHECK_GE(value, 0);
...
{
	return static_cast<float>(picker_.get_weight(value))
			/ picker_.total_weight();
}
...
void UnigramSampler::SampleBatchGetExpectedCountAvoid(random::SimplePhilox* rnd,
		bool unique, MutableArraySlice<int64> batch,
		MutableArraySlice<float> batch_expected_count, ArraySlice<int64> extras,
		MutableArraySlice<float> extras_expected_count,
		ArraySlice<int64> avoided_values) const
{
	mutex_lock lock(mu_);  // could use reader lock
	unsafe_sampler_.SampleBatchGetExpectedCountAvoid(rnd, unique, batch,
			batch_expected_count, extras, extras_expected_count,
			avoided_values);
}
...
FixedUnigramSampler::FixedUnigramSampler(Env* env, int64 range,
		const string& vocab_file, float distortion, int32 num_reserved_ids,
		int32 num_shards, int32 shard) :
		RangeSampler(range), total_weight_(0.0), num_shards_(num_shards), shard_(
				shard)
{
...
FixedUnigramSampler::FixedUnigramSampler(int64 range,
		const std::vector<float>& unigrams, float distortion,
		int32 num_reserved_ids, int32 num_shards, int32 shard) :
		RangeSampler(range), total_weight_(0.0), num_shards_(num_shards), shard_(
				shard)
{
	FillReservedIds(num_reserved_ids);
		...
		if (word_id % num_shards_ == shard_) {
			...
			if (!strings::safe_strtof(cols.at(cols.size() - 1).c_str(), &w)) {
				return errors::InvalidArgument(
						"Wrong vocabulary format at line: ", line);
			}
			w = pow(w, distortion);
			total_weight_ += w;
