		...
		std::vector<MinMaxRecord>* records)
{
	...
	for (const string& file_line : file_lines) {
		...
		std::vector<string> min_max_parts = str_util::Split(min_max_string,
				'[');
		if ((min_max_parts.size() != 3)
				|| (min_max_parts[0] != requant_prefix)) {
			continue;
		}
		...
		string name = name_string.substr(0,
				name_string.size() - print_suffix.size()).ToString();
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	TF_RETURN_IF_ERROR(
			context.GetOneStringParameter("min_max_log_file", "",
					&min_max_log_file));
	if (min_max_log_file == "") {
	...
	TF_RETURN_IF_ERROR(
			context.GetOneFloatParameter("min_percentile", 5.0f,
					&min_percentile));
	float max_percentile;
	TF_RETURN_IF_ERROR(
			context.GetOneFloatParameter("max_percentile", 5.0f,
					&max_percentile));
	...
	for (const auto& record_info : records_by_node) {
		...
		int max_index = std::round(
				maxs.size() * (1.0f - (max_percentile / 100.0f)));
	...
	for (const NodeDef& node : input_graph_def.node()) {
		if (range_for_nodes.count(node.name())) {
			if (node.op() != "RequantizationRange") {
				return errors::InvalidArgument(
						"Node is expected to be a RequantizationRange op: ",
