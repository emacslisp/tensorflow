...
void TensorSliceWriteTestHelper::GetData(TensorSliceReader::Table* table,
	const string& name, const TensorSlice& slice, SavedSlice* ss)
{
...
ASSERT_TRUE(table->Get(kSavedTensorSlicesKey, &value));
{
	...
	// created.
	{
		...
		EXPECT_EQ("dim { size: 5 } "
				"dim { size: 10 }", ssm.shape().ShortDebugString());
	...
	}
	{
		...
		EXPECT_EQ("dim { size: 3 } "
				"dim { size: 2 }", ssm.shape().ShortDebugString());
