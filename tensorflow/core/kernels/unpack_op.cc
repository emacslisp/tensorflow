...
public:
	explicit UnpackOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument("Input shape axis ", axis,
						" must equal ", num, ", got shape ",
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument(
						"output size must fit in Eigen DenseIndex"));
		...
		auto input_reshaped = input.shaped<T, 3>(
				{ 1, before_dim, axis_dim * after_dim });
		...
		for (int i = 0; i < num; ++i) {
			...
			if (output_shape.num_elements() > 0) {
				auto output_shaped = output->shaped<T, 3>( { 1, before_dim,
						after_dim });
				Eigen::DSizes < Eigen::DenseIndex, 3
						> indices { 0, 0, i * after_dim };
				Eigen::DSizes < Eigen::DenseIndex, 3 > sizes { 1, before_dim,
						after_dim };
				functor::Split<Device, T>()(context->eigen_device<Device>(),
						output_shaped, input_reshaped, indices, sizes);
			}
		}
	}
