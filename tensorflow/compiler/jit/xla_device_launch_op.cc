...
Status BuildCompilationCache(ResourceMgr* rm, XlaCompilationCache** compiler)
{
	...
	Status s = rm->Lookup < XlaDevice::Metadata
			> (rm->default_container(), "xla_metadata", &metadata);
	if (!s.ok()) {
		return s;
	}
...
void XlaDeviceLaunchOp::Compute(OpKernelContext* ctx)
{
	...
	OP_REQUIRES_OK(ctx,
			rm->LookupOrCreate < XlaCompilationCache
					> (rm->default_container(), "xla_compiler", &compiler, [rm](XlaCompilationCache** compiler) {
	...
	if (!kernel->computation.IsNull()) {
		...
		std::vector < std::shared_ptr
				< xla::GlobalData
						>> arg_handles(kernel->xla_input_shapes.size());
		...
		*execution_options.mutable_shape_with_output_layout() =
		...
		if (xla::ShapeUtil::IsTuple(kernel->xla_output_shape)) {
			auto outputs_or_error = compiler->client()->DeconstructTuple(
					*result.ValueOrDie());
	...
	XlaDeviceContext* device_context =
			ctx->op_device_context<XlaDeviceContext>();
	...
	for (int i = 0; i < ctx->num_outputs(); ++i) {
		...
		if (kernel->outputs[i].is_constant) {
			...
			XlaTransferManager::SetTensorGlobalData(
					std::shared_ptr<xla::GlobalData>(
							std::move(outputs[output_num])), output);
			++output_num;
		}
	}
