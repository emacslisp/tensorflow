...
/* static */std::map<perftools::gputools::Platform::Id, TransferManager::State>*
	...
	if (it == managers->end()) {
		return NotFound(
				"could not find registered transfer manager for platform %s -- check "
						"target linkage", platform->Name().c_str());
	}
...
Status TransferManager::TransferBufferFromDevice(se::StreamExecutor* executor,
		const se::DeviceMemoryBase& source, int64 size, void* destination)
{
	if (source.size() < size) {
		return FailedPrecondition(
				"Source allocation on device not large enough for data tranfer: "
						"%lld < %lld", source.size(), size);
	}
	auto copy_status = executor->SynchronousMemcpyD2H(source, size,
			destination);
	if (!copy_status.ok()) {
		return AddStatus(
...
Status TransferManager::TransferBufferToDevice(se::StreamExecutor* executor,
		int64 size, const void* source, se::DeviceMemoryBase* destination)
{
	if (destination->size() < size) {
		return FailedPrecondition(
				"Destination allocation on device not large enough for data tranfer: "
						"%lld < %lld", destination->size(), size);
	}
	auto copy_status = executor->SynchronousMemcpyH2D(source, size,
			destination);
	if (!copy_status.ok()) {
		return AddStatus(
				Status(static_cast<tensorflow::error::Code>(copy_status.code()),
...
StatusOr<std::set<se::DeviceMemoryBase>> TransferManager::GatherBufferPointersFromTuple(
		...
		const Shape& shape)
{
	...
	for (auto i = 0; i < tuple_elements.size(); ++i) {
		...
		if (ShapeUtil::IsTuple(element_shape)) {
			TF_ASSIGN_OR_RETURN(
					std::set < se::DeviceMemoryBase
							> buffer_pointers_in_element,
