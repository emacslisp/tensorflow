...
public:
	explicit MirrorPadOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		MirrorPadMode mode;
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, dims == in1.dim_size(0),
				errors::InvalidArgument(
			...
			if (offset_ == 0) {  // SYMMETRIC mode.
				OP_REQUIRES(context,
						...
						errors::InvalidArgument("paddings must be no greater "
								...
								after, " greater than ", in0.dim_size(d)));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int d = 0; d < dims; ++d) {
			...
			if (offset_ == 0) {  // SYMMETRIC mode.
				OP_REQUIRES(context, before <= out_size && after <= out_size,
						errors::InvalidArgument("paddings must be no greater "
								"than the output dimension size: ", before,
								", ", after, " greater than ", out_size));
			} else if (offset_ == 1) {  // REFLECT mode.
				OP_REQUIRES(context, before < out_size && after < out_size,
						errors::InvalidArgument("paddings must be less than"
								" the output dimension size: ", before, ", ",
								after, " not less than ", out_size));
			}
		...
		Tensor scratch;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::value, in0.shape(),
						&scratch));
