		...
		LayoutConstraints* constraints)
{
	...
	for (auto& instruction : computation->instructions()) {
		if (instruction->opcode() == HloOpcode::kConvolution
			...
			TF_RETURN_IF_ERROR(
					constraints->SetOperandLayout(filter_shape, convolution,
							1));
			TF_RETURN_IF_ERROR(
			...
			// Set layouts of the instructions' shapes.
			TF_RETURN_IF_ERROR(
					constraints->SetOperandLayout(lhs_shape, dot, 0));
			TF_RETURN_IF_ERROR(
					constraints->SetOperandLayout(rhs_shape, dot, 1));
			TF_RETURN_IF_ERROR(
					constraints->SetInstructionLayout(output_shape, dot));
		} else {
			for (int64 operand_no = 0;
					operand_no < instruction->operand_count(); ++operand_no) {
				...
				TF_RETURN_IF_ERROR(
						constraints->SetOperandLayout(operand_shape,
								instruction.get(), operand_no));
			}
			...
			tensorflow::gtl::ArraySlice<const LogicalBuffer*> buffers =
					constraints->points_to_analysis().GetPointsToSet(
							instruction.get()).element( { });
