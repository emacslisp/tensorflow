...
Options Options::FromProtoStr(const string& opts_proto_str)
{
	...
	Options opts(opts_pb.max_depth(), opts_pb.min_bytes(), opts_pb.min_micros(),
			opts_pb.min_params(), opts_pb.min_float_ops(),
			std::vector<string>(opts_pb.device_regexes().begin(),
					opts_pb.device_regexes().end()), opts_pb.order_by(),
			...
			std::vector<string>(opts_pb.select().begin(),
					opts_pb.select().end()), opts_pb.viz(),
...
{
	const string s = strings::Printf("%-28s%d\n"
			"%-28s%lld\n"
			"%-28s%lld\n"
			"%-28s%lld\n"
			"%-28s%lld\n"
			...
			"%-28s%s\n"
			"%-28s%s\n"
			"%-28s%s\n", kOptions[0], max_depth, kOptions[1], min_bytes,
			kOptions[2], min_micros, kOptions[3], min_params, kOptions[4],
			min_float_ops, kOptions[5],
