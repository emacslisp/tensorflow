...
{
	void* f;
	port::Status s = port::Env::Default()->GetSymbolFromLibrary(GetDsoHandle(),
			"cudnnGetVersion", &f);
	if (f == nullptr) {
...
size_t cudnnGetVersion()
{
	...
	auto callable = reinterpret_cast<size_t (*)(void)>(f);return
callable	();
}
...
case CUDNN_CONVOLUTION_BWD_DATA_ALGO_FFT_TILING:
	...
	return algo;
default:
	LOG(FATAL) << "Unsupported Cudnn convolution backward algorithm for data: "
			<< algorithm;
}
}
...
cudnnConvolutionBwdFilterAlgo_t ToConvBackwardFilterAlgo(
	dnn::AlgorithmType algorithm)
{
cudnnConvolutionBwdFilterAlgo_t algo = cudnnConvolutionBwdFilterAlgo_t(
		algorithm);
switch (algo) {
...
{
auto status = dynload::cudnnCreate(parent_,
		reinterpret_cast<cudnnHandle_t*>(&dnn_handle_));
if (status == CUDNN_STATUS_SUCCESS) {
	...
	bool library_loaded_matches_source = (loaded_compat_version
			== compiled_compat_version);
	if (!library_loaded_matches_source) {
		const string error = port::StrCat("Loaded runtime CuDNN library: ",
				loaded_version, " (compatibility version ",
				loaded_compat_version, ") but source was compiled with ",
				CUDNN_VERSION, " (compatibility version ",
				compiled_compat_version,
...
if (status == CUDNN_STATUS_NOT_INITIALIZED) {
	...
	if (!result.ok()) {
		...
		LOG(INFO) << "running driver version: "
...
return port::Status { port::error::INTERNAL, port::StrCat(
		"cudnn library could not create a handle: ", ToString(status)) };
}
...
ScopedTensorDescriptor(CUDAExecutor* parent,
		const BatchDescriptor& batch_descriptor, cudnnDataType_t elem_type) :
		parent_(parent), handle_(nullptr)
{
	cudnnStatus_t status = dynload::cudnnCreateTensorDescriptor(parent_,
			&handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
	...
	std::vector<int64> strides64 = batch_descriptor.full_strides(
			dnn::DataLayout::kBatchDepthYX);
	std::vector<int64> dims64 = batch_descriptor.full_dims(
			dnn::DataLayout::kBatchDepthYX);
	...
	std::transform(dims64.cbegin(), dims64.cend(), dims.begin(),
			&CheckedNarrowing<int64, int>);
	status = dynload::cudnnSetTensorNdDescriptor(parent_, handle_, elem_type,
			nd, dims.data(), strides.data());
...
{
	cudnnStatus_t status = dynload::cudnnDestroyTensorDescriptor(parent_,
			handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
...
ScopedFilterDescriptor(CUDAExecutor* parent,
		...
		parent_(parent), handle_(nullptr)
{
	cudnnStatus_t status = dynload::cudnnCreateFilterDescriptor(parent_,
			&handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
...
{
	cudnnStatus_t status = dynload::cudnnDestroyFilterDescriptor(parent_,
			handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
...
public:
ScopedConvolutionDescriptor(CUDAExecutor* parent,
		const ConvolutionDescriptor& convolution_descriptor,
		cudnnDataType_t data_type) :
		parent_(parent), handle_(nullptr)
{
	cudnnStatus_t status = dynload::cudnnCreateConvolutionDescriptor(parent_,
			&handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
	...
	status = dynload::cudnnSetConvolutionNdDescriptor(parent_, handle_,
			convolution_descriptor.ndims(), padding.data(), strides.data(),
			upscale.data(),
...
{
	cudnnStatus_t status = dynload::cudnnDestroyConvolutionDescriptor(parent_,
			handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
...
ScopedPoolingDescriptor(CUDAExecutor* parent,
		...
		parent_(parent), handle_(nullptr)
{
	cudnnStatus_t status = dynload::cudnnCreatePoolingDescriptor(parent_,
			&handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
	...
	std::transform(shape64.cbegin(), shape64.cend(), shape.begin(),
			&CheckedNarrowing<int64, int>);
	status = dynload::cudnnSetPoolingNdDescriptor(parent_, handle_,
...
{
	cudnnStatus_t status = dynload::cudnnDestroyPoolingDescriptor(parent_,
			handle_);
	if (status != CUDNN_STATUS_SUCCESS) {
		...
		parent_(parent), handle_(nullptr)
{
	...
	status = dynload::cudnnSetLRNDescriptor(parent_, handle_, lrnN, lrnAlpha,
			lrnBeta, lrnK);
	if (status != CUDNN_STATUS_SUCCESS) {
		LOG(FATAL) << "could not set cudnn LRN descriptor: "
				<< ToString(status);
	}
}
...
port::StatusOr<std::unique_ptr<dnn::RnnDescriptor>> CudnnSupport::createRnnDescriptor(
	int num_layers, int hidden_size, int input_size,
...
string error_msg = port::StrCat(
		"createRnnDescriptor needs at least Cudnn 5.0 to work. ",
...
port::StatusOr<std::unique_ptr<dnn::RnnSequenceTensorDescriptor>> CudnnSupport::createRnnSequenceTensorDescriptor(
	int seq_length, int batch_size, int data_size, dnn::DataType data_type)
{
#if CUDNN_VERSION >= 5000
...
port::StatusOr<std::unique_ptr<dnn::RnnStateTensorDescriptor>> CudnnSupport::createRnnStateTensorDescriptor(
	int num_layer, int batch_size, int data_size, dnn::DataType data_type)
{
#if CUDNN_VERSION >= 5000
...
bool CudnnSupport::DoRnnForward(Stream* stream,
...
bool CudnnSupport::DoRnnBackward(Stream* stream,
	const dnn::RnnDescriptor& rnn_desc,
	const dnn::RnnSequenceTensorDescriptor& input_desc,
	const DeviceMemory<float>& input_data,
	const dnn::RnnStateTensorDescriptor& input_h_desc,
...
template<class T>
bool CudnnSupport::DoConvolveImpl(Stream* stream,
...
if (algorithm_config.algorithm() == dnn::kDefaultAlgorithm) {
	...
	if (scratch_allocator != nullptr) {
		...
		status = dynload::cudnnGetConvolutionForwardWorkspaceSize(parent_,
		...
		if (status == CUDNN_STATUS_SUCCESS && size_in_bytes != 0) {
			auto allocated = scratch_allocator->AllocateBytes(stream,
					size_in_bytes);
			if (allocated.ok()) {
	...
	if (size_in_bytes != 0) {
		if (scratch_allocator == nullptr) {
			LOG(FATAL)
					<< "An allocator must be specified when scratch memory is "
							"needed";
		}
		auto allocated = scratch_allocator->AllocateBytes(stream,
				size_in_bytes);
...
status = dynload::cudnnConvolutionForward(parent_, ToHandle(dnn_handle_),
...
bool CudnnSupport::DoBatchNormalizationForward(Stream* stream,
...
{
return DoBatchNormalizationForwardImpl<float>(stream, dnn::DataType::kFloat, x,
		scale, offset, estimated_mean, estimated_variance, x_desc,
...
template<class T>
bool CudnnSupport::DoBatchNormalizationForwardImpl(Stream* stream,
	dnn::DataType data_type, const DeviceMemory<T>& x,
...
ScopedTensorDescriptor x_descriptor { parent_, x_desc, ToCudnnDataType(
		data_type) };
...
if (is_training) {
	...
	status = dynload::cudnnBatchNormalizationForwardTraining(parent_,
			ToHandle(dnn_handle_), mode, &one, &zero, x_descriptor.handle(),
			x.opaque(), x_descriptor.handle(), y->opaque(),
...
#endif
	status = dynload::cudnnBatchNormalizationForwardInference(parent_,
			ToHandle(dnn_handle_), mode, &one, &zero, x_descriptor.handle(),
			x.opaque(), x_descriptor.handle(), y->opaque(),
			scale_offset_descriptor.handle(), scale.opaque(), offset.opaque(),
...
bool CudnnSupport::DoBatchNormalizationBackward(Stream* stream,
...
{
return DoBatchNormalizationBackwardImpl(stream, CUDNN_DATA_FLOAT, y_backprop, x,
		scale, mean, variance, x_desc, scale_offset_desc, epsilon, x_backprop,
		scale_backprop, offset_backprop);
}

template<class T>
bool CudnnSupport::DoBatchNormalizationBackwardImpl(Stream* stream,
	int cudnn_type, const DeviceMemory<T>& y_backprop, const DeviceMemory<T>& x,
	const DeviceMemory<T>& scale, const DeviceMemory<T>& mean,
	const DeviceMemory<T>& variance, const dnn::BatchDescriptor& x_desc,
	const dnn::BatchDescriptor& scale_offset_desc, const double epsilon,
...
status = dynload::cudnnBatchNormalizationBackward(parent_,
		ToHandle(dnn_handle_), mode, &one, &zero, &one, &zero,
...
bool CudnnSupport::DoConvolve(Stream* stream,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveImpl<float>(stream, CUDNN_DATA_FLOAT, batch_descriptor,
		input_data, filter_descriptor, filter_data, convolution_descriptor,
...
bool CudnnSupport::DoConvolve(Stream* stream,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveImpl<Eigen::half>(stream, CUDNN_DATA_HALF, batch_descriptor,
		input_data, filter_descriptor, filter_data, convolution_descriptor,
		output_descriptor, output_data, scratch_allocator, algorithm_config,
		output_profile_result);
}
...
template<class T>
DeviceMemory<T> CudnnSupport::MaybeTransformLayout(Stream* stream,
	int cudnn_type,  // Actually cudnnDataType_t.
...
*transform_scratch = stream->AllocateTemporaryArray<T>(
		backward_output_data.ElementCount()).ConsumeValueOrDie();
...
ScopedTensorDescriptor orig_out_back_nd { parent_, *output_descriptor,
		static_cast<cudnnDataType_t>(cudnn_type) };
ScopedTensorDescriptor transformed_out_back_nd { parent_,
		transformed_output_descriptor, static_cast<cudnnDataType_t>(cudnn_type) };

float alpha = 1.0f;
float beta = 0.0f;
auto status = dynload::cudnnTransformTensor(parent_, ToHandle(dnn_handle_),
		&alpha, orig_out_back_nd.handle(), backward_output_data.opaque(), &beta,
...
template<class T>
bool CudnnSupport::DoConvolveBackwardDataImpl(Stream* stream,
	int cudnn_type,  // Actually cudnnDataType_t.
	const FilterDescriptor& filter_descriptor,
	const DeviceMemory<T>& filter_data,
...
std::unique_ptr<TemporaryDeviceMemory<T>> transform_scratch;
backward_output_data = MaybeTransformLayout(stream, cudnn_type,
...
if (algorithm_config.algorithm() == dnn::kDefaultAlgorithm) {
	...
	if (scratch_allocator != nullptr) {
		size_t size_in_bytes;
		status = dynload::cudnnGetConvolutionBackwardDataWorkspaceSize(parent_,
...
bool CudnnSupport::DoConvolveBackwardData(Stream* stream,
	const FilterDescriptor& filter_descriptor,
	const DeviceMemory<float>& filter_data,
	const BatchDescriptor& output_descriptor_in,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveBackwardDataImpl(stream, CUDNN_DATA_FLOAT, filter_descriptor,
		filter_data, output_descriptor_in, backward_output_data,
...
bool CudnnSupport::DoConvolveBackwardData(Stream* stream,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveBackwardDataImpl(stream, CUDNN_DATA_HALF, filter_descriptor,
		filter_data, output_descriptor_in, backward_output_data,
		convolution_descriptor, input_descriptor, backward_input_data,
		scratch_allocator, algorithm_config, output_profile_result);
}
...
template<class T>
bool CudnnSupport::DoConvolveBackwardFilterImpl(Stream* stream,
	int cudnn_type,  // Actually cudnnDataType_t.
...
output_descriptor.CloneFrom(output_descriptor_in);
std::unique_ptr<TemporaryDeviceMemory<T>> transform_scratch;
backward_output_data = MaybeTransformLayout(stream,
		static_cast<cudnnDataType_t>(cudnn_type), &output_descriptor,
...
if (algorithm_config.algorithm() == dnn::kDefaultAlgorithm) {
	...
	size_t size_in_bytes;
	status = dynload::cudnnGetConvolutionBackwardFilterWorkspaceSize(parent_,
			ToHandle(dnn_handle_), /*srcDesc=*/input_nd.handle(),
			/*diffDesc=*/out_back_nd.handle(), /*convDesc=*/conv.handle(),
			/*gradDesc=*/filter.handle(), /*algo=*/algo,
			/*sizeInBytes=*/&size_in_bytes);
...
bool CudnnSupport::DoConvolveBackwardFilter(Stream* stream,
	const dnn::BatchDescriptor& input_descriptor,
	const DeviceMemory<float>& input_data,
	const dnn::BatchDescriptor& output_descriptor_in,
	DeviceMemory<float> backward_output_data,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveBackwardFilterImpl(stream, CUDNN_DATA_FLOAT, input_descriptor,
		input_data, output_descriptor_in, backward_output_data,
...
bool CudnnSupport::DoConvolveBackwardFilter(Stream* stream,
	...
	dnn::ProfileResult* output_profile_result)
{
return DoConvolveBackwardFilterImpl(stream, CUDNN_DATA_HALF, input_descriptor,
		input_data, output_descriptor_in, backward_output_data,
		convolution_descriptor, filter_descriptor, backward_filter_data,
		scratch_allocator, algorithm_config, output_profile_result);
}
...
template<class T>
bool CudnnSupport::DoConvolveBackwardBiasImpl(Stream* stream,
	int cudnn_type,  // Actually cudnnDataType_t.
	const dnn::BatchDescriptor& input_descriptor,
	const DeviceMemory<T>& input_data,
...
status = dynload::cudnnConvolutionBackwardBias(parent_, ToHandle(dnn_handle_),
		&alpha, input_nd.handle(), input_data.opaque(), &beta, bias_nd.handle(),
...
bool CudnnSupport::DoConvolveBackwardBias(Stream* stream,
	const BatchDescriptor& input_descriptor,
	const DeviceMemory<double>& input_data,
...
bool CudnnSupport::DoMatMul(Stream* stream,
	const DeviceMemory<float>& input_data, const DeviceMemory<float>& weights,
...
if (output_dimensions.width() == 1 && output_dimensions.height() == 1) {
	...
	stream->ThenBlasGemm(blas::Transpose::kNoTranspose,
			blas::Transpose::kNoTranspose, m, n, k, alpha, weights, m,
			input_data, k, beta, output_data, m);
} else {
	...
	for (int i = 0; i < batch_count; ++i) {
		...
		a[i] =
				DeviceMemory<float>::MakeFromByteSize(
		...
		c[i] =
				DeviceMemory<float>::MakeFromByteSize(
						const_cast<float*>(reinterpret_cast<const float*>(output_data->opaque()))
								+ output_offset,
	...
	stream->ThenBlasGemmBatched(blas::Transpose::kNoTranspose,
			blas::Transpose::kNoTranspose, m, n, k, alpha, toPtrs(a), lda,
			toPtrs(b), ldb, beta, toPtrs(c), ldc, batch_count);
}
...
bool CudnnSupport::DoBiasAdd(Stream* stream,
	const DeviceMemory<float>& input_data, const DeviceMemory<float>& biases,
	const dnn::BatchDescriptor& dimensions, DeviceMemory<float>* output_data)
{
ScopedTensorDescriptor input_descriptor { parent_, dimensions, CUDNN_DATA_FLOAT };

BatchDescriptor bias_dimensions;
bias_dimensions.set_count(1).set_feature_map_count(
		dimensions.feature_map_count()).set_height(1).set_width(1).set_layout(
...
bool CudnnSupport::DoActivate(Stream* stream,
	dnn::ActivationMode activation_mode, const dnn::BatchDescriptor& dimensions,
...
float beta = 0.0;
status = dynload::cudnnActivationForward(parent_, ToHandle(dnn_handle_),
#if CUDNN_VERSION >= 5000
		activation_desc.handle(),
#else
		mode,
#endif
		&alpha, input_nd.handle(), input_data.opaque(), &beta,
		input_nd.handle(), output_data->opaque());
if (status != CUDNN_STATUS_SUCCESS) {
...
bool CudnnSupport::DoPoolForward(Stream* stream,
...
ScopedTensorDescriptor dest_desc { parent_, output_dimensions, CUDNN_DATA_FLOAT };
ScopedPoolingDescriptor pooling_desc { parent_, pooling_dimensions };
status = dynload::cudnnPoolingForward(parent_, ToHandle(dnn_handle_),
		pooling_desc.handle(), &alpha, src_desc.handle(), input_data.opaque(),
		&beta, dest_desc.handle(), output_data->opaque());
if (status != CUDNN_STATUS_SUCCESS) {
...
bool CudnnSupport::DoPoolBackward(Stream* stream,
	const dnn::PoolingDescriptor& pooling_dimensions,
	const dnn::BatchDescriptor& input_dimensions,
	const DeviceMemory<float>& input_data,
	const dnn::BatchDescriptor& output_dimensions,
...
ScopedTensorDescriptor dest_desc { parent_, output_dimensions, CUDNN_DATA_FLOAT };
ScopedPoolingDescriptor pooling_desc { parent_, pooling_dimensions };
status = dynload::cudnnPoolingBackward(parent_, ToHandle(dnn_handle_),
		pooling_desc.handle(), &alpha, dest_desc.handle(), output_data.opaque(),
		dest_desc.handle(), input_diff_data.opaque(), src_desc.handle(),
		input_data.opaque(), &beta, src_desc.handle(),
...
bool CudnnSupport::DoNormalize(Stream* stream,
...
bool CudnnSupport::DoNormalizeWithDimensions(Stream* stream,
	const dnn::NormalizeDescriptor& normalize_descriptor,
...
status = dynload::cudnnLRNCrossChannelForward(parent_, ToHandle(dnn_handle_),
		normalize.handle(), CUDNN_LRN_CROSS_CHANNEL_DIM1, &alpha, dims.handle(),
		input_data.opaque(), &beta, dims.handle(), output_data->opaque());
if (status != CUDNN_STATUS_SUCCESS) {
...
bool CudnnSupport::DoNormalizeBackwardWithDimensions(Stream* stream,
	const dnn::NormalizeDescriptor& normalize_descriptor,
...
status = dynload::cudnnLRNCrossChannelBackward(parent_, ToHandle(dnn_handle_),
		normalize.handle(), CUDNN_LRN_CROSS_CHANNEL_DIM1, &alpha, dims.handle(),
...
dnn::BatchDescriptor output_dimensions =
		dnn::BatchDescriptor::DepthConcatenateOutputDescriptor(
				input_dimensions);
...
for (size_t i = 0; i < input_data.size(); ++i) {
	...
	for (int64 batch = 0; batch < output_dimensions.count(); ++batch) {
		for (int64 yx = 0; yx < area; ++yx) {
			for (int64 depth = 0; depth < dimensions.feature_map_count();
					++depth) {
				LOG(INFO) << output_dimensions.ElementCount() << ' ' << batch
						<< ' ' << yx << ' ' << depth;
				output_host[index(batch, depth + depth_sum, yx,
						output_dimensions.feature_map_count())] = tmp[index(
						batch, depth, yx, dimensions.feature_map_count())];
			}
		}
	}
...
bool CudnnSupport::DoElementwiseOperate(Stream* stream,
...
bool CudnnSupport::DoMemcpyD2HQuantized(Stream* stream,
...
bool CudnnSupport::DoMemcpyH2DQuantized(Stream* stream, const void* host_src,
	int64 size, dnn::QuantizedActivationMode mode,
...
auto status = dynload::cudnnGetConvolutionNdForwardOutputDim(parent_,
...
output_batch_descriptor->set_count(dims[0]).set_feature_map_count(dims[1]).set_layout(
		batch_descriptor.layout());
...
gpu::port::Status status = gpu::PluginRegistry::Instance()->RegisterFactory<
		gpu::PluginRegistry::DnnFactory>(gpu::cuda::kCudaPlatformId,
		gpu::cuda::kCuDnnPlugin, "cuDNN",
...
gpu::PluginRegistry::Instance()->SetDefaultFactory(gpu::cuda::kCudaPlatformId,
		gpu::PluginKind::kDnn, gpu::cuda::kCuDnnPlugin);
}
