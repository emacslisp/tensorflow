...
void FillRandomsWithSingles(PhiloxRandom gen,
		typename Distribution::ResultElementType* p, int64 size)
{
	int granularity = Distribution::kResultElementCount;

	CHECK(size % granularity == 0) << " size: " << size << " granularity: "
			<< granularity;
...
bool CheckSamplesMoments(const std::vector<T>& samples,
		std::function<double(int)> theoretical_moments, int max_moments,
		int stride, T z_limit)
{
	...
	for (int i = 1; i <= max_moments; ++i) {
		...
		const double moments_i_var = moments_i_squared
				- moments_i_mean * moments_i_mean;
		...
		const double total_variance = moments_i_var / moments_sample_count[i]
				+ error_per_moment;
		// z_test is approximately a unit normal distribution.
		const double z_test = fabs(
				(moments[i] - moments_i_mean) / sqrt(total_variance));

		if (z_test > z_limit) {
			LOG(ERROR) << "failing z_test:" << " moment: " << i << " stride: "
					<< stride << " z_test: " << z_test << " z_limit: "
					<< z_limit << " measured moments: " << moments[i]
