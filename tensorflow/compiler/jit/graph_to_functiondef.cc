		...
		FunctionDef* fdef)
{
	...
	for (Node const* node : graph.nodes()) {
		...
		if (node->type_string() == kRetValOp) {
			...
			return_values[normalized] = strings::StrCat(edge->src()->name(),
					":", edge->src_output());
			continue;
		}
			...
			if (edge == nullptr) {
				return errors::InvalidArgument(
						"Nonconsecutive input edges; missing "
								"input edge ", i, " for node ", node->name());
			}
			node_def->add_input(
					strings::StrCat(edge->src()->name(), ":",
							edge->src_output()));
		}
		...
		TF_RETURN_IF_ERROR(
				NameRangesForNode(node->def(), node->op_def(), nullptr,
						&output_ranges));
		for (const auto& output : output_ranges) {
			for (int i = output.second.first; i < output.second.second; ++i) {
				const string tensor_name = strings::StrCat(node_def->name(),
						":", output.first, ":", i - output.second.first);
				tensor_renaming[strings::StrCat(node->name(), ":", i)] =
						tensor_name;
			}
		}
	}
	...
	for (int n_index = 0; n_index < fdef->node_def_size(); ++n_index) {
		...
		for (int i = 0; i < node_def->input_size(); ++i) {
			if (StringPiece(node_def->input(i)).starts_with("^")) {
				// Control input
				const string normalized = node_names.Renormalize(
						node_def->input(i).substr(1));
				if (normalized.empty()) {
					return errors::InvalidArgument(
							"Could not remap control input ", i, ", '",
							node_def->input(i), "', of node '",
							node_def->name(), "' in function ", name);
				}
				*node_def->mutable_input(i) = strings::StrCat("^", normalized);
				...
				if (iter == tensor_renaming.end()) {
					return errors::InvalidArgument("Could not remap input ", i,
							", '", node_def->input(i), "', of node '",
							node_def->name(), "' in function ", name);
				}
	...
	for (int r = 0; r < fdef->signature().output_arg_size(); ++r) {
		...
		if (ret_name.empty()) {
			return errors::InvalidArgument("Missing output ", r,
					" to function ", name);
		}
		...
		if (iter == tensor_renaming.end()) {
			return errors::InvalidArgument("Could not remap return value ", r,
					", '", ret_name, "', of '", return_value, "' in function ",
					name);
		}
