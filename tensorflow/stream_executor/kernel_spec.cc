...
CudaPtxInMemory::CudaPtxInMemory(port::StringPiece ptx,
		...
		KernelLoaderSpec(kernel_name), ptx_by_compute_capability_(
				CompareComputeCapability)
{
	if (ptx_compressed) {
				...
				CompareComputeCapability)
{
	for (const auto &spec : spec_list) {
		...
		ptx_by_compute_capability_[std::tuple<int, int> { major, minor }] =
				ptx.data();
	}
}
...
MultiKernelLoaderSpec::MultiKernelLoaderSpec(size_t arity) :
		arity_(arity)
{
}
