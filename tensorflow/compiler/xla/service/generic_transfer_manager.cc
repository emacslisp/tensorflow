		...
		const Shape& device_shape, const Shape& literal_shape, Literal* literal)
{
	VLOG(2) << "transferring literal shape from device: "
			<< ShapeUtil::HumanString(literal_shape) << "; device location: "
			<< source.opaque();
	...
	if (device_shape.element_type() == TUPLE) {
		*literal->mutable_shape() = literal_shape;
		...
		for (int64 i = 0; i < element_buffers.size(); ++i) {
			...
			// element array.
			TF_RETURN_IF_ERROR(
					TransferLiteralFromDevice(executor, element_buffers[i], /*device_shape=*/
							element_device_shape,
							/*literal_shape=*/element_literal_shape,
							element_literal));
		}
		return Status::OK();
	}
	...
	*literal->mutable_shape() = device_shape;
	...
	TF_RETURN_IF_ERROR(
			TransferBufferFromDevice(executor, source, /*size=*/
...
StatusOr<std::vector<se::DeviceMemoryBase>> GenericTransferManager::ShallowCopyTupleFromDevice(
		se::StreamExecutor* executor, const se::DeviceMemoryBase& source,
		const Shape& shape)
{
	...
	std::vector<void*> element_pointers(ShapeUtil::TupleElementCount(shape),
			nullptr);
	int64 tuple_size = ShapeUtil::ByteSizeOf(shape, /*pointer_size=*/
			sizeof(void*));
	...
	if (!copy_status.ok()) {
		return AddStatus(
				...
				"failed transfer of tuple buffer "
