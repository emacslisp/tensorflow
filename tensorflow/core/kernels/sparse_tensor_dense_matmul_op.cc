...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, a_shape->NumElements() == 2,
				errors::InvalidArgument(
						"Tensor 'a_shape' must have 2 elements"));
		...
		if (std::is_same<Device, GPUDevice>::value) {
			...
			OP_REQUIRES(ctx,
					FastBoundsCheck(inner_left, std::numeric_limits<int>::max())
							&& FastBoundsCheck(inner_right,
									std::numeric_limits<int>::max())
							&& FastBoundsCheck(outer_left,
									std::numeric_limits<int>::max())
							&& FastBoundsCheck(outer_right,
									std::numeric_limits<int>::max())
							...
							&& FastBoundsCheck(a_values->NumElements(),
									std::numeric_limits<int>::max()),
					errors::InvalidArgument(
							"Cannot use GPU for > 2^31 entry inputs"));
			...
			typename TTypes<T>::ConstMatrix b, typename TTypes<T>::Vec scratch)
	{
		...
		if (rhs_right < kNumVectorize) {
			...
			for (std::size_t i = 0; i < nnz; ++i) {
				const int64 m = internal::SubtleMustCopy(
						a_indices(i, lhs_index_a));
				const int64 k = internal::SubtleMustCopy(
						a_indices(i, rhs_index_a));
