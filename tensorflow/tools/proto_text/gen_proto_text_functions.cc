...
namespace {
class CrashOnErrorCollector: public tensorflow::protobuf::compiler::MultiFileErrorCollector {
public:
...
int MainImpl(int argc, char** argv)
{
	if (argc < 4) {
		LOG(ERROR)
				<< "Pass output path, relative path, and at least proto file";
		return -1;
	}

	const string output_root = argv[1];
	const string output_relative_path = kTensorFlowHeaderPrefix
			+ string(argv[2]);
	...
	for (int i = 3; i < argc; ++i) {
		if (IsPlaceholderFile(argv[i])) {
			...
			src_relative_path = s.substr(0,
					s.size() - strlen(kPlaceholderFile));
	...
	source_tree.MapPath("",
			src_relative_path.empty() ? "." : src_relative_path);
	...
	for (int i = 3; i < argc; i++) {
		if (IsPlaceholderFile(argv[i]))
			continue;
		const string proto_path = string(argv[i]).substr(
				src_relative_path.size());

		const tensorflow::protobuf::FileDescriptor* fd = importer.Import(
				proto_path);

		const int index = proto_path.find_last_of(".");
		string proto_path_no_suffix = proto_path.substr(0, index);

		proto_path_no_suffix = proto_path_no_suffix.substr(
				output_relative_path.size());

		const auto code = tensorflow::GetProtoTextFunctionCode(*fd,
				kTensorFlowHeaderPrefix);
		...
		for (int pass = 0; pass < 3; ++pass) {
			...
			const string path = output_root + "/" + proto_path_no_suffix
					+ suffix;
