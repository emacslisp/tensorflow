...
public:
	...
	void OutputToStream(std::ostream* stream) const
	{
		if (empty()) {
			...
			*stream << "count=" << count_ << " first=" << first_ << " curr="
					<< newest_ << " min=" << min_ << " max=" << max_ << " avg="
					<< avg() << " std=" << std_deviation();
		}
	}
...
public:
	StatSummarizerOptions() :
			show_run_order(true), run_order_limit(0), show_time(true), time_limit(
					10), show_memory(true), memory_limit(10), show_type(true), show_summary(
					true)
	{
	}
...
public:
	enum SortingMetric {
		BY_NAME, BY_DEFINITION_ORDER, BY_RUN_ORDER, BY_TIME, BY_MEMORY, BY_TYPE,
	};
