...
static inline void ParseAndCheckBoxSizes(OpKernelContext* context,
		const Tensor& boxes, const Tensor& scores, int* num_boxes)
{
		...
		int i, int j)
{
	...
	const float intersection_area = std::max<float>(
...
public:
			...
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("iou_threshold", &iou_threshold_));
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				TensorShapeUtils::IsScalar(max_output_size.shape()),
		...
		const int output_size = std::min(max_output_size.scalar<int>()(),
				num_boxes);
		typename TTypes<float, 2>::ConstTensor boxes_data = boxes.tensor<float,
				2>();

		std::vector<float> scores_data(num_boxes);
		std::copy_n(scores.flat<float>().data(), num_boxes,
				scores_data.begin());
		...
		for (int i = 0; i < num_boxes; ++i) {
			if (num_active == 0 || selected.size() >= output_size)
				break;
			if (active[i]) {
			...
			for (int j = i + 1; j < num_boxes; ++j) {
				if (active[j]) {
					float iou = ComputeIOU(boxes_data, sorted_indices[i],
							sorted_indices[j]);
		...
		typename TTypes<int, 1>::Tensor selected_indices_data = output->tensor<
				int, 1>();
