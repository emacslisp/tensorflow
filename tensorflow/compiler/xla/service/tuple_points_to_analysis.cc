...
{
	const std::vector<const LogicalBuffer*>& pointed_to_buffers = element(
			index);
...
/* static */StatusOr<std::unique_ptr<TuplePointsToAnalysis>> TuplePointsToAnalysis::Run(
...
Status TuplePointsToAnalysis::Analyze()
{
	...
	for (auto& computation : module_->computations()) {
		...
		for (auto& instruction : computation->instructions()) {
			TF_RETURN_IF_ERROR(
					GatherBuffersDefinedByInstruction(instruction.get(),
							&instruction_defined_buffers_[instruction.get()]));
		...
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	...
	for (int64 i = 0; i < operands.size(); ++i) {
		const PointsToSet& operand_points_to_set = *FindOrDie(points_to_,
				operands[i]);
...
Status TuplePointsToAnalysis::HandleSelect(HloInstruction* select,
		HloInstruction* /*pred*/, HloInstruction* on_true,
...
{
	const std::vector<const LogicalBuffer*>& buffers = GetPointsToSet(
...
{
	if (!InstructionDefinesBufferAtIndex(buffer.instruction(),
			buffer.index())) {
		return FailedPrecondition(
				"LogicalBuffer %s is ill-defined: instruction %s does not define a "
						"buffer at that index", buffer.ToString().c_str(),
				buffer.instruction()->name().c_str());
	}
	...
	if (GetBuffer(buffer.id()).instruction() != buffer.instruction()
		...
		return FailedPrecondition(
				...
				GetBuffer(buffer.id()).ToString().c_str());
	}
...
Status TuplePointsToAnalysis::GatherBuffersDefinedByInstruction(
		...
		std::vector<const LogicalBuffer*>* buffers)
{
	return GetPointsToSet(instruction).ForEachElement(
...
string TuplePointsToAnalysis::ToString() const
{
	...
	for (auto& computation : module_->computations()) {
		...
		for (const HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
		...
		for (auto& buffer : logical_buffers_) {
			tensorflow::strings::StrAppend(&output, "  buffer ",
					buffer->ToString(), ":\n");
			for (const BufferAlias& buffer_alias : buffer_aliases_.at(
					buffer.get())) {
