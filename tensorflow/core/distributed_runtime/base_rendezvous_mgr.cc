...
BaseRendezvousMgr::BaseRendezvousMgr(const WorkerEnv* env) :
		worker_env_(env)
{
}
...
void BaseRendezvousMgr::RecvLocalAsync(int64 step_id,
		const Rendezvous::ParsedKey& parsed, Rendezvous::DoneCallback done)
{
...
Status BaseRendezvousMgr::RecvLocal(int64 step_id,
		const Rendezvous::ParsedKey& parsed, Tensor* val, bool* is_dead)
{
	Status ret;
	Notification n;
	RecvLocalAsync(step_id, parsed, [val, is_dead, &ret, &n](const Status& s,
...
BaseRemoteRendezvous::BaseRemoteRendezvous(const WorkerEnv* env, int64 step_id,
		...
		env_(env), step_id_(step_id), local_(
				NewLocalRendezvous(tolerate_dup_recv))
{
}
...
Status BaseRemoteRendezvous::Send(const Rendezvous::ParsedKey& parsed,
		const Rendezvous::Args& args, const Tensor& val, const bool is_dead)
{
		...
		StatusCallback done)
{
	...
	if (!DMAHelper::CanUseDMA(&in)) {
				...
				errors::InvalidArgument("Non-DMA-safe ",
	...
	attr.set_gpu_compatible(
			send_args.alloc_attrs.gpu_compatible()
	...
	*out = copy;
	...
	CopyTensor::ViaDMA(parsed.edge_name, send_args.device_context,
			...
			send_args.alloc_attrs, recv_args.alloc_attrs, &in, out, done);
}
