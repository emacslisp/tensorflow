...
PaddingFIFOQueue::PaddingFIFOQueue(int capacity,
		...
		FIFOQueue(capacity, component_dtypes,
				ConvertShapesPartialDimensionsToZero(partial_shapes), name), partial_shapes_(
				partial_shapes)
{
}
...
Status PaddingFIFOQueue::Initialize()
{
	...
	if (component_dtypes_.size() != partial_shapes_.size()) {
		return errors::InvalidArgument(
				...
				component_dtypes_.size(), " dtypes and ",
				partial_shapes_.size(), " shapes.");
	}
...
Status PaddingFIFOQueue::GetElementComponent(
		const PaddingFIFOQueue::Tuple& tuple, int component,
		...
		bool allow_small_batch, CallbackWithTuple callback)
{
	if (num_elements == 0) {
		...
		for (int i = 0; i < num_components(); ++i) {
			...
			ctx->allocate_temp(component_dtypes_[i], ManyOutShape(i, 0),
					&element);
...
for (size_t i = 0; i < tuple.size(); ++i) {
	if (!partial_shapes_[i].IsCompatibleWith(tuple[i].shape())) {
		return errors::InvalidArgument("Shape mismatch in tuple component ", i,
				". Expected ", partial_shapes_[i].DebugString(), ", got ",
...
for (size_t i = 0; i < tuple.size(); ++i) {
	...
	const PartialTensorShape expected_shape = PartialTensorShape(
			{ batch_size }).Concatenate(partial_shapes_[i]);
	if (!expected_shape.IsCompatibleWith(tuple[i].shape())) {
		return errors::InvalidArgument("Shape mismatch in tuple component ", i,
				". Expected ", expected_shape.DebugString(), ", got ",
				tuple[i].shape().DebugString());
	}
}
return Status::OK();
...
Status PaddingFIFOQueue::CompatibleNodeDefShapes(const NodeDef& node_def) const
{
...
if (parent->dims() != element.dims() + 1) {
	return errors::Internal("Mismatched ranks.  Element's rank is: ",
			element.dims(),
...
for (size_t i = 0; i < shapes.size(); ++i) {
	...
	for (int64 s : partial.dim_sizes())
		shape.AddDim(s < 0 ? 0 : s);
}
return shapes;
}
