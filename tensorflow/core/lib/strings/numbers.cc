...
{
	static const std::unordered_map<string, T> special_nums = { { "inf",
			std::numeric_limits<T>::infinity() }, { "+inf", std::numeric_limits<
			T>::infinity() }, { "-inf", -std::numeric_limits<T>::infinity() }, {
			"infinity", std::numeric_limits<T>::infinity() }, { "+infinity",
			std::numeric_limits<T>::infinity() }, { "-infinity",
			-std::numeric_limits<T>::infinity() }, { "nan", std::numeric_limits<
			T>::quiet_NaN() }, { "+nan", std::numeric_limits<T>::quiet_NaN() },
	...
	for (int i = 0; i < special_num_str.length(); ++i) {
		special_num_str[i] = std::tolower(special_num_str[i],
				std::locale::classic());
	}
	...
	if (entry != special_nums.end()) {
		*endptr = str
				+ (s.eof() ?
						static_cast<std::iostream::pos_type>(strlen(str)) :
						s.tellg());
		return entry->second;
	} else {
		...
		*endptr =
				str
						+ (s.fail() ?
								...
								(s.eof() ?
										static_cast<std::iostream::pos_type>(strlen(
												str)) :
										s.tellg()));
	}
	return result;
...
char* DoubleToBuffer(double value, char* buffer)
{
	...
	if (std::abs(value) <= kDoublePrecisionCheckMax) {
		...
		full_precision_needed = locale_independent_strtonum<double>(buffer,
				NULL) != value;
	}
...
char* FloatToBuffer(float value, char* buffer)
{
	...
	int snprintf_result = snprintf(buffer, kFastToBufferSize, "%.*g", FLT_DIG,
			value);
	...
	if (!safe_strtof(buffer, &parsed_value) || parsed_value != value) {
		snprintf_result = snprintf(buffer, kFastToBufferSize, "%.*g",
				FLT_DIG + 2, value);
