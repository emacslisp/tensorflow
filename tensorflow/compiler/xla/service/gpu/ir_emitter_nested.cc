		...
		IrEmitter(hlo_module_config, ir_emitter_context, /*is_nested=*/true)
{
	...
	emitted_function_ = EmitBasePointersForNestedComputation(nested_computation,
			&io_hlos);
}
		...
		std::vector<const HloInstruction*>* io_hlos)
{
	...
	}
	{
		...
		int64 root_size = llvm_ir::ByteSizeOf(root_shape,
				ir_emitter_context_->llvm_module()->getDataLayout());
	...
	llvm::FunctionType* function_type = llvm::FunctionType::get(
			ir_builder_.getVoidTy(), argument_types, false);
	...
	llvm::BasicBlock* entry_bb = llvm::BasicBlock::Create(
			function->getContext(), "entry", function);
...
Status IrEmitterNested::EmitTargetElementLoop(const HloInstruction& hlo,
		const llvm_ir::ElementGenerator& element_generator)
{
	return llvm_ir::LoopEmitter(element_generator, GetIrArray(hlo),
			&ir_builder_).EmitLoop();
}
