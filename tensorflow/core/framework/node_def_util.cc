...
string SummarizeNodeDef(const NodeDef& node_def)
{
	...
	for (const string& attr_name : attr_names) {
		...
		strings::StrAppend(&ret, attr_name, "=",
				SummarizeAttrValue(iter->second));
	}
...
DEFINE_GET_ATTR(PartialTensorShape, shape, "shape", emplace_back,
		...
		;)
DEFINE_GET_ATTR(Tensor, tensor, "tensor", emplace_back, t,
		Tensor t; if (!t.FromProto(v)) { return errors::InvalidArgument( "Attr ", attr_name, " has value ", ProtoShortDebugString(v), " that can't be converted to a Tensor"); })
		...
		DataTypeVector* sig)
{
	...
	if (!arg_def.number_attr().empty()) {
		...
		int32 repeats = -1;
		TF_RETURN_IF_ERROR(
				GetNodeAttr(node_def, arg_def.number_attr(), &repeats));
		if (repeats < 0) {
		...
		const AttrValue* attr_value;
		TF_RETURN_IF_ERROR(
				AttrSlice(node_def).Find(arg_def.type_list_attr(),
						&attr_value));
...
Status ValidateNodeDef(const NodeDef& node_def, const OpDef& op_def)
{
	if (node_def.op() != op_def.name()) {
		return errors::InvalidArgument("NodeDef op '", node_def.op(),
				"' does not match ", SummarizeOpDef(op_def), "; NodeDef: ",
	...
	for (const auto& attr : op_def.attr()) {
		if (!gtl::InsertIfNotPresent(&op_attrs, attr.name(), &attr)) {
			return errors::InvalidArgument("OpDef has duplicate attr name '",
					attr.name(), "': ", SummarizeOpDef(op_def));
		}
	}
	for (const auto& attr : node_def.attr()) {
		...
		if (iter == op_attrs.end()) {
			return errors::InvalidArgument("NodeDef mentions attr '",
					attr.first, "' not in ", SummarizeOpDef(op_def),
	...
	if (num_inputs != inputs.size()) {
		return errors::InvalidArgument("NodeDef expected inputs '",
				DataTypeVectorString(inputs), "' do not match ", num_inputs,
				" inputs specified; ", SummarizeOpDef(op_def), "; NodeDef: ",
				SummarizeNodeDef(node_def));
	}
...
{
	return Scanner(sp).One(Scanner::LETTER_DIGIT_DOT).Any(
...
bool IsValidDataInputName(StringPiece sp)
{
	...
	Scanner scan(sp);
	scan.One(Scanner::LETTER_DIGIT_DOT).Any(
...
{
	return Scanner(sp).OneLiteral("^").One(Scanner::LETTER_DIGIT_DOT).Any(
			Scanner::LETTER_DIGIT_DASH_DOT_SLASH_UNDERSCORE).Eos().GetResult();
}
...
{
	Status ret = status;
	errors::AppendToMessage(&ret,
			strings::StrCat(" [[Node: ", SummarizeNodeDef(node_def), "]]"));
	return ret;
}
