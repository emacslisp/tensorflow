	...
	for (const NodeDef& node : rewritten_graph_def.node()) {
		...
		for (const string& old_input : node.input()) {
			...
			if (new_node_names.count(input_node_name) > 0) {
				new_input = input_prefix + new_node_names[input_node_name]
						+ input_suffix;
			} else {
				new_input = old_input;
			}
	...
	while (!current_nodes.empty()) {
		...
		for (const string& node_name : current_nodes) {
			...
			if (node_map.count(node_name) == 0) {
				LOG(ERROR) << "Bad graph structure, no node named '"
						<< node_name << "' found for input lookup";
				return errors::InvalidArgument(
						"Bad graph structure, no node named '", node_name,
		...
		current_nodes = std::vector<string>(next_nodes.begin(),
				next_nodes.end());
	}
