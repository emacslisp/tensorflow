...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("sparsetodense", "SparseToDense").Input(
						FakeInput(index_type)).Input(FakeInput(index_type)).Input(
						FakeInput(value_type)).Input(FakeInput(value_type)).Finalize(
						node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
static int BM_Arg(int ndim, int n)
{
return (ndim * 1000000) + n;
}
static int NDIM_from_arg(int bm_arg)
{
return bm_arg / 1000000;
}
static int N_from_arg(int bm_arg)
{
return bm_arg % 1000000;
}
...
for (auto* ptr : { &sparse_indices, &output_shape, &sparse_values,
		&default_value }) {
...
TF_CHECK_OK(
		NodeDefBuilder("sparsetodense", "SparseToDense").Input(
				FakeInput(DT_INT32)).Input(FakeInput(DT_INT32)).Input(
				FakeInput(DT_FLOAT)).Input(FakeInput(DT_FLOAT)).Finalize(
				&sparse_node_def));

Status status;
std::unique_ptr<OpKernel> op(
		CreateOpKernel(DEVICE_CPU, device.get(), cpu_allocator(),
				sparse_node_def, TF_GRAPH_DEF_VERSION, &status));
