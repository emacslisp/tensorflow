...
ConvolveScratchAllocator::ConvolveScratchAllocator(int device_ordinal,
...
ConvolveScratchAllocator::~ConvolveScratchAllocator()
{
	for (auto& allocated_buffer : allocated_buffers_) {
		if (!memory_allocator_->Deallocate(device_ordinal_, &allocated_buffer).ok()) {
...
se::port::StatusOr<se::DeviceMemory<uint8>> ConvolveScratchAllocator::AllocateBytes(
		se::Stream* stream, int64 byte_size)
{
	...
	auto status_or_memory = memory_allocator_->Allocate(device_ordinal_,
			byte_size,
...
ConvolutionThunk::ConvolutionThunk(ConvolutionKind convolution_kind,
		Index input_buffer, Index filter_buffer, Index output_buffer,
		...
		Thunk(Kind::kConvolution, hlo), convolution_kind_(convolution_kind), input_buffer_(
				input_buffer), filter_buffer_(filter_buffer), output_buffer_(
				output_buffer), input_shape_(input_shape), filter_shape_(
				filter_shape), output_shape_(output_shape), window_(window), dim_nums_(
				dim_nums)
{
}
		...
		const BufferAllocations& buffer_allocations, se::Stream* stream)
{
	...
	input_descriptor.set_layout(DataLayout::kBatchDepthYX).set_height(
			input_shape_.dimensions(dim_nums_.spatial_dimensions(0))).set_width(
			input_shape_.dimensions(dim_nums_.spatial_dimensions(1))).set_feature_map_count(
			input_shape_.dimensions(dim_nums_.feature_dimension())).set_count(
			input_shape_.dimensions(dim_nums_.batch_dimension()));

	FilterDescriptor filter_descriptor;
	filter_descriptor.set_layout(FilterLayout::kOutputInputYX).set_input_feature_map_count(
			filter_shape_.dimensions(
					dim_nums_.kernel_input_feature_dimension())).set_output_feature_map_count(
			filter_shape_.dimensions(
					dim_nums_.kernel_output_feature_dimension())).set_input_filter_height(
			filter_shape_.dimensions(dim_nums_.kernel_spatial_dimensions(0))).set_input_filter_width(
			filter_shape_.dimensions(dim_nums_.kernel_spatial_dimensions(1)));

	ConvolutionDescriptor convolution_descriptor;
	convolution_descriptor.set_zero_padding_width(width.padding_low()).set_zero_padding_height(
			height.padding_low()).set_horizontal_filter_stride(width.stride()).set_vertical_filter_stride(
			height.stride());

	BatchDescriptor output_descriptor;
	output_descriptor.set_layout(DataLayout::kBatchDepthYX).set_height(
			output_shape_.dimensions(dim_nums_.spatial_dimensions(0))).set_width(
			output_shape_.dimensions(dim_nums_.spatial_dimensions(1))).set_feature_map_count(
			output_shape_.dimensions(dim_nums_.feature_dimension())).set_count(
			output_shape_.dimensions(dim_nums_.batch_dimension()));
		...
		se::dnn::ProfileResult* profile_result)
{
	...
	case ConvolutionKind::kBackwardFilter:
		launch_ok = stream->ThenConvolveBackwardFilterWithAlgorithm(
				input_descriptor, input_data, output_descriptor, output_data,
				convolution_descriptor, filter_descriptor, &filter_data,
				scratch_allocator, algorithm_config, profile_result).ok();
		break;
	case ConvolutionKind::kBackwardInput:
		launch_ok = stream->ThenConvolveBackwardDataWithAlgorithm(
				filter_descriptor, filter_data, output_descriptor, output_data,
				convolution_descriptor, input_descriptor, &input_data,
				scratch_allocator, algorithm_config, profile_result).ok();
		break;
	case ConvolutionKind::kForward:
		launch_ok = stream->ThenConvolveWithAlgorithm(input_descriptor,
	...
	return InternalError(
			"Unable to launch convolution for thunk %p with type %s and algorithm "
					"(%lld, %lld)", this,
			ConvolutionKindToString(convolution_kind_).c_str(),
		...
		const BufferAllocations& buffer_allocations, se::Stream* stream)
{
	...
	if (flags->xla_gpu_autotune_convolution_algorithm
		...
		for (se::dnn::AlgorithmType algorithm : GetAlgorithms(stream->parent())) {
			...
			se::dnn::ProfileResult profile_result;
			bool launch_ok = Convolve(input_descriptor, input_data,
					filter_descriptor, filter_data, output_descriptor,
					output_data, convolution_descriptor,
					se::dnn::AlgorithmConfig(algorithm, algorithm), stream,
					&scratch_allocator, &profile_result).ok();
		...
		if (best_result.is_valid()) {
			...
			LOG(ERROR)
					<< "No convolution algorithm works with profiling. Fall back "
