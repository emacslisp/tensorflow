...
void StepStatsCollector::BuildCostModel(CostModelManager* cost_model_manager,
		const std::unordered_map<string, const Graph*>& device_map)
{
	...
	std::unordered_map<StringPiece, DeviceStats, StringPiece::Hasher> per_device_stats;
	...
	for (int i = 0; i < step_stats_->dev_stats_size(); ++i) {
		...
		if (gpu_id >= 0) {
			...
			per_device_stats.emplace(device_name, DeviceStats { &device_stats,
					nullptr });
		}
	}
	...
	for (auto& itr : per_device_stats) {
		...
		if (gpu_id >= 0) {
			...
			if (gpu_hardware_stats.find(gpu_id) != gpu_hardware_stats.end()) {
				itr.second.hardware_stats =
						gpu_hardware_stats.find(gpu_id)->second;
			}
		}
	}
			...
			if (node) {
				for (int i = 0; i < stats.output_size(); ++i) {
					...
					cm->RecordMaxMemorySize(node, i,
							Bytes(
									output.tensor_description().allocation_description().allocated_bytes()),
							stats.output(i).tensor_description().shape(),
							node->output_types()[i]);
					cm->RecordAllocationId(node, i,
							output.tensor_description().allocation_description().allocation_id());
				}
				...
				if (dev_stats.hardware_stats
					...
					cm->RecordMaxExecutionTime(node,
							Microseconds(hw_stats.op_end_rel_micros()));
				} else {
					cm->RecordMaxExecutionTime(node,
...
void StepStatsCollector::Save(const string& device, NodeExecStats* nt)
{
	VLOG(1) << "Save dev " << device << " nt " << nt;
	{
		...
		if (!step_stats_ || collectedNodes >= kMaxCollectedNodes) {
			VLOG(1)
					<< "step_stats_ nullptr or already collected too many nodes.";
			delete nt;
			return;
		}
