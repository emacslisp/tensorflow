		...
		tensorflow::StringPiece path)
{
	...
	Status s = tensorflow::Env::Default()->NewRandomAccessFile(path.ToString(),
			&file);
	if (!s.ok()) {
		return s;
	}
		...
		std::vector<tensorflow::StringPiece>* result)
{
	...
	for (size_t i = 0; i < text.size() + 1; i++) {
		if (i == text.size() || text[i] == delim) {
			tensorflow::StringPiece token(text.data() + token_start,
					i - token_start);
...
StatusOr<std::unique_ptr<Literal>> TextLiteralReader::ReadAllLines()
{
	...
	LiteralUtil::PopulateWithValue<float>(fill,
			AsInt64Slice(shape.dimensions()), result.get());
	...
	while (buf.ReadLine(&line).ok()) {
		...
		if (!coordinates_string.Consume("(")) {
			return InvalidArgument(
					"expected '(' at the beginning of coordinates: \"%s\"",
					line.c_str());
		}
		if (!tensorflow::str_util::ConsumeSuffix(&coordinates_string, ")")) {
			return InvalidArgument(
					"expected ')' at the end of coordinates: \"%s\"",
					line.c_str());
		}
		float value;
		...
		if (coordinate_values.size() != shape.dimensions_size()) {
			return InvalidArgument(
					"line did not have expected number of coordinates; want %d got %zu: "
							"\"%s\"", shape.dimensions_size(),
					coordinate_values.size(), line.c_str());
		}
