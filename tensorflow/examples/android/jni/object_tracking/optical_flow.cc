...
OpticalFlow::OpticalFlow(const OpticalFlowConfig* const config) :
		config_(config), frame1_(NULL), frame2_(NULL), working_size_(
				config->image_size)
{
}
...
bool OpticalFlow::FindFlowAtPoint_LK(const Image<uint8>& img_I,
		const Image<uint8>& img_J, const Image<int32>& I_x,
		const Image<int32>& I_y, const float p_x, const float p_y,
		float* out_g_x, float* out_g_y)
{
	float g_x = *out_g_x;
	float g_y = *out_g_y;
	...
	const float kWindowSizeFloat =
			static_cast<float>(kFlowIntegrationWindowSize);
...
bool OpticalFlow::FindFlowAtPoint_ESM(const Image<uint8>& img_I,
		const Image<uint8>& img_J, const Image<int32>& I_x,
		const Image<int32>& I_y, const Image<int32>& J_x,
		const Image<int32>& J_y, const float p_x, const float p_y,
		float* out_g_x, float* out_g_y)
{
	float g_x = *out_g_x;
	float g_y = *out_g_y;
	...
	if (!img_I.ExtractPatchAtSubpixelFixed1616(src_left_fixed, src_top_fixed,
			...
			|| !I_x.ExtractPatchAtSubpixelFixed1616(src_left_fixed,
					src_top_fixed, patch_size, patch_size, src_gradient_x)
			|| !I_y.ExtractPatchAtSubpixelFixed1616(src_left_fixed,
					src_top_fixed, patch_size, patch_size, src_gradient_y)) {
		return false;
	}
	...
	if (kDoBrightnessNormalize && left_trunc >= 0 && top_trunc >= 0
		...
		bright_offset =
				static_cast<int>(static_cast<float>(sum_diff) * area_inv);
	}
	...
	for (int iteration = 0; iteration < kNumIterations; ++iteration) {
		...
		for (int y = 0; y < patch_size;
				++y, templ_row += patch_size, src_dx_row += patch_size, src_dy_row +=
						patch_size, extract_row += patch_size) {
			...
			for (int x = 0; x < patch_size; ++x) {
				...
				int32 diff = static_cast<int32>(extract_row[x])
						- static_cast<int32>(templ_row[x]) - bright_offset;
		...
		const float jtj_1[4] = { static_cast<float>(jtj[2]),
				static_cast<float>(-jtj[1]), static_cast<float>(-jtj[1]),
		...
		if (kDoBrightnessNormalize) {
			bright_offset += static_cast<int>(area_inv
					* static_cast<float>(sum_diff) + 0.5f);
		}
...
bool OpticalFlow::FindFlowAtPointReversible(const int level, const float u_x,
		const float u_y, const bool reverse_flow, float* flow_x,
		float* flow_y) const
{
	...
	const bool success =
			kUseEsm ?
					FindFlowAtPoint_ESM(img_I, img_J, I_x, I_y, J_x, J_y,
							scaled_p_x, scaled_p_y, &scaled_flow_x,
							&scaled_flow_y) :
					FindFlowAtPoint_LK(img_I, img_J, I_x, I_y, scaled_p_x,
							scaled_p_y, &scaled_flow_x, &scaled_flow_y);
...
bool OpticalFlow::FindFlowAtPointSingleLevel(const int level, const float u_x,
		const float u_y, const bool filter_by_fb_error, float* flow_x,
		float* flow_y) const
{
	...
	if (filter_by_fb_error) {
		...
		if (!FindFlowAtPointReversible(level, new_position_x, new_position_y,
				true, &reverse_flow_x, &reverse_flow_y)) {
			LOGE("Backward error!");
			return false;
		}

		const float discrepancy_length = sqrtf(
				Square(*flow_x + reverse_flow_x)
...
bool OpticalFlow::FindFlowAtPointPyramidal(const float u_x, const float u_y,
		const bool filter_by_fb_error, float* flow_x, float* flow_y) const
{
	...
	for (int l = max_level - 1; l >= 0; --l) {
		if (!FindFlowAtPointSingleLevel(l, u_x, u_y, filter_by_fb_error, flow_x,
				flow_y)) {
			return false;
		}
	}
