...
class ExprTree {
public:
	explicit ExprTree(HloOpcode opcode) :
			opcode_(opcode)
	{
	}
	ExprTree(HloOpcode opcode, const string& tag) :
			opcode_(opcode), tag_(tag)
	{
	}
	ExprTree(HloOpcode opcode, const ExprTree& operand0) :
			opcode_(opcode)
	{
	...
	typedef std::unordered_map<string, const HloInstruction*> TaggedInstructionMap;
			...
			TaggedInstructionMap* tagged_instructions) const
	{
		...
		for (auto& pair : operands_) {
			TF_RETURN_IF_ERROR(
					pair.second->Match(instruction->operand(pair.first),
							tagged_instructions));
		}
...
public:
			...
			int64* const_value) const
	{
		...
		if (type == S32) {
			*const_value =
					static_cast<int64>(LiteralUtil::GetFirstElement<int32>(
							literal));
		} else if (type == S64) {
	...
	StatusOr<const HloInstruction*> GetTaggedInstruction(const string& tag,
...
private:
	...
	Status MatchExprTree(const ExprTree& expr_tree) override
	{
		ExprTree::TaggedInstructionMap tagged_instructions;
		TF_RETURN_IF_ERROR(
				expr_tree.Match(computation_->root_instruction(),
						&tagged_instructions));
...
private:
	...
	{
		// Build ExprTree for fused instructions.
		ExprTree gte1 = ExprTree(HloOpcode::kGetTupleElement, "gte",
		...
		for (const auto& pair : tagged_instructions) {
			...
			if (tag == "gte" && inst->tuple_index() != tuple_index_) {
				...
				return InvalidArgument(
						"Unexpected tuple index instruction : %s",
						inst->name().c_str());
				...
				CHECK(inst->IsFused());
				if (inst->fusion_instruction()->operand(
