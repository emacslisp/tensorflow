...
std::vector<string> FfmpegCommandLine(const string& input_filename,
		const string& output_filename, const string& input_format_id,
...
std::vector<float> ReadPcmFile(const string& filename)
{
	...
	for (int32 i = 0; i < sample_count; ++i) {
		...
		const double normalized = static_cast<double>(host_order)
		...
		const std::vector<float>& samples)
{
	...
	header += LittleEndianDataInt(
			samples_per_second * channel_count * sizeof(int16));
		...
		const std::vector<float>& samples)
{
	...
	for (float value : samples) {
		const int16 quantized = static_cast<int16>(value
				* std::numeric_limits<int16>::max());
		char raw[2];
...
{
	for (const char* dir : std::vector<const char*>( { getenv("TEST_TMPDIR"),
			getenv("TMPDIR"), getenv("TMP"), "/tmp" })) {
		if (!dir || !dir[0]) {
			continue;
		}
		struct stat statbuf;
		if (!stat(dir, &statbuf) && S_ISDIR(statbuf.st_mode)) {
			return io::JoinPath(dir,
					StrCat("tmp_file_", getpid(), ".", extension));
		}
	}
		...
		std::vector<float>* output_samples)
{
	...
	const std::vector<string> args = FfmpegCommandLine(filename,
			output_filename, audio_format_id, samples_per_second,
			channel_count);
	...
	if (!IsBinaryInstalled(kFfmpegExecutable)) {
		return Status(error::Code::NOT_FOUND,
				StrCat("FFmpeg could not be found."));
	}
