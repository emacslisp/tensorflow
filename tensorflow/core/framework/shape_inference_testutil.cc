...
Status ShapeInferenceTestutil::InferShapes(ShapeInferenceTestOp op,
		const string& ins, const string& expected_outs)
{
	...
	shape_inference::InferenceContext c(op.graph_def_version, &op.node_def,
			op_reg_data->op_def, in_shapes, op.input_tensors, { }, { }, { });
	...
	for (int i = 0; i < num_outputs; ++i) {
		...
		string err_suffix = strings::StrCat(". Output shape was ",
				c.DebugString(out));
		...
		if (expected == "?") {
			if (c.RankKnown(out)) {
				return Unknown(err_prefix, " expected to be unknown",
						err_suffix);
			}
			continue;
		}
		...
		for (int j = 0; j < expected_dims.size(); ++j) {
			...
			if (expected_dim == "?") {
				...
				if (std::find(v.begin(), v.end(),
					...
					return Unknown(err_prefix, " matched input d",
							in_dim_idx.first, "_", in_dim_idx.second,
				...
				if (!strings::safe_strto64(expected_dim, &value)) {
					return Unknown(err_prefix,
							": the expected dimension value '", expected_dim,
				...
				if (in_dim_idx.first != -1) {
					...
					return Unknown(err_prefix, " expected to be ", value,
							" but was ", c.DebugString(out_dim), err_suffix);
				}
			}
		}
	}
	...
	while (scanner.Peek() != ']') {
		if (scanner.Peek() == '?') {
			...
			if (!scanner.GetResult(nullptr, &match)
				...
				return errors::InvalidArgument("Could not parse number in ",
						spec);
			}
