...
void RecordMatchedNodes(const NodeMatch& match, std::set<string>* matched_nodes)
{
...
{
	CHECK_NE(0, source.attr().count(source_key)) << "No key '" << source_key
			<< "' found in " << source.DebugString();
...
void RemoveAttributes(const GraphDef& input_graph_def,
		const std::vector<string>& attributes, GraphDef* output_graph_def)
{
	output_graph_def->mutable_node()->Clear();
	for (const NodeDef& node : input_graph_def.node()) {
...
bool GraphMatcher::DoesOpTypeMatch(const NodeDef& node,
		...
		const std::set<string>& previously_matched_nodes, NodeMatch* match)
{
	...
	for (int i = 0; i < pattern.inputs.size(); ++i) {
		const string& input_node_name = NodeNameFromInput(
				non_control_inputs[i]);
		...
		if (!DoesOpTypeMatch(input_node, input_pattern,
				previously_matched_nodes, input_match)) {
			return false;
		}
	}
	return true;
...
Status ReplaceMatchingOpTypes(const GraphDef& input_graph_def,
		const OpTypePattern& pattern,
		const std::function<
		...
		if (matches_by_head_name.count(input_node.name())) {
			...
			for (const NodeDef& matched_node : matched_nodes_array) {
				...
				if (outputs_map.count(matched_node.name())) {
					for (const NodeDef* dependent_node : outputs_map[matched_node.name()]) {
						if (!matched_nodes_lookup.count(
								dependent_node->name())) {
			...
			TF_RETURN_IF_ERROR(
					node_generator(*match, input_nodes, output_nodes,
							&new_nodes));
			...
			if (abort_replacement) {
				LOG(WARNING)
						<< "Generator function didn't preserve needed nodes, "
				...
				for (const NodeDef& old_node : old_nodes) {
					NodeDef* added_node =
							output_graph_def->mutable_node()->Add();
...
Status RenameNodeInputs(const GraphDef& input_graph_def,
		...
		GraphDef* output_graph_def)
{
	std::map<string, std::vector<std::pair<string, string>>>canonical_inputs_to_rename;
...
Status IsGraphValid(const GraphDef& graph_def)
{
	...
	if (!invalid_inputs.empty()) {
		...
		for (const std::pair<string, string>& invalid_input : invalid_inputs) {
			LOG(ERROR) << "Invalid input " << invalid_input.second
					<< " for node " << invalid_input.first << " - "
...
{
	string file_data;
	Status load_file_status = ReadFileToString(Env::Default(), file_name,
			&file_data);
...
Status TransformFuncContext::GetOneStringParameter(const string& name,
		const string& default_value, string* result) const
{
	...
	if (params_count == 0) {
		...
		*result = params.at(name).at(0);
		...
		return errors::InvalidArgument("Expected a single '", name,
				"' parameter, but found ", params_count, " occurrences");
	}
}

Status TransformFuncContext::GetOneInt32Parameter(const string& name,
		int32 default_value, int32* result) const
{
	const int params_count = CountParameters(name);
	if (params_count == 0) {
		*result = default_value;
...
Status TransformFuncContext::GetOneInt64Parameter(const string& name,
		int64 default_value, int64* result) const
{
	const int params_count = CountParameters(name);
	if (params_count == 0) {
		*result = default_value;
...
Status TransformFuncContext::GetOneBoolParameter(const string& name,
		bool default_value, bool* result) const
{
	const int params_count = CountParameters(name);
	if (params_count == 0) {
		*result = default_value;
