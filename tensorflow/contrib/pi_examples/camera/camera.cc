...
Status SendCameraCommand(int fh, int request, void* arg)
{
	...
	if (r == -1) {
		LOG(ERROR) << "SendCameraCommand error " << errno << " ("
				<< strerror(errno) << ")";
...
Status SetCameraFormat(int camera_handle, int wanted_width, int wanted_height)
{
	...
	Status set_format_status = SendCameraCommand(camera_handle, VIDIOC_S_FMT,
			&fmt);
	...
	if ((fmt.fmt.pix.width != wanted_width)
			|| (fmt.fmt.pix.height != wanted_height)) {
		LOG(WARNING) << "Warning: driver is sending image at "
		...
		CameraBuffer** buffers)
{
	...
	Status request_buffers_status = SendCameraCommand(camera_handle,
			VIDIOC_REQBUFS, &req);
	...
	*buffers = (CameraBuffer*) (calloc(buffer_count, sizeof(*buffers)));
	for (int n_buffers = 0; n_buffers < buffer_count; ++n_buffers) {
		...
		buf.index = n_buffers;
		Status query_buffer_status = SendCameraCommand(camera_handle,
				VIDIOC_QUERYBUF, &buf);
		...
		(*buffers)[n_buffers].start = v4l2_mmap(NULL, buf.length,
				PROT_READ | PROT_WRITE, MAP_SHARED, camera_handle,
				buf.m.offset);
	...
	for (int i = 0; i < buffer_count; ++i) {
		...
		buf.memory = V4L2_MEMORY_MMAP;
		buf.index = i;
		Status set_buffer_status = SendCameraCommand(camera_handle, VIDIOC_QBUF,
				&buf);
	...
	Status stream_on_status = SendCameraCommand(camera_handle, VIDIOC_STREAMON,
			&type);
...
{
	enum v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	Status stream_off_status = SendCameraCommand(camera_handle,
...
Status CaptureNextFrame(int camera_handle, CameraBuffer* buffers,
		uint8_t** frame_data, int* frame_data_size, v4l2_buffer* buf)
{
	int r;
	do {
		fd_set fds;
	...
	if (r == -1) {
		LOG(
				ERROR) << "select() failed while waiting for the camera with " << errno;
	...
	buf->type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	buf->memory = V4L2_MEMORY_MMAP;
	Status get_buffer_status = SendCameraCommand(camera_handle, VIDIOC_DQBUF,
			buf);
...
{
	Status release_buffer_status = SendCameraCommand(camera_handle, VIDIOC_QBUF,
			buf);
		...
		Tensor* out_indices, Tensor* out_scores)
{
	...
	for (int i = 0; i < unsorted_scores_flat.size(); ++i) {
		scores.push_back(
				std::pair<int, float>( { i, unsorted_scores_flat(i) }));
	}
...
Status TensorFromFrame(uint8_t* image_data, int image_width, int image_height,
		int image_channels, const int wanted_height, const int wanted_width,
		...
		std::vector<Tensor>* out_tensors)
{
	...
	if (image_channels < wanted_channels) {
		return tensorflow::errors::FailedPrecondition(
				"Image needs to have at least ", wanted_channels,
				" but only has ", image_channels);
	}
	...
	tensorflow::Tensor image_tensor(tensorflow::DT_FLOAT,
			tensorflow::TensorShape( { 1, wanted_height, wanted_width,
					wanted_channels }));
	...
	for (int y = 0; y < wanted_height; ++y) {
		...
		const int bottom_y_index = std::min(static_cast<int>(ceilf(in_y)),
				(image_height - 1));
		...
		for (int x = 0; x < wanted_width; ++x) {
			...
			const int right_x_index = std::min(static_cast<int>(ceilf(in_x)),
					(image_width - 1));
			tensorflow::uint8* in_top_left_pixel = in_top_row
					+ (left_x_index * wanted_channels);
			tensorflow::uint8* in_top_right_pixel = in_top_row
					+ (right_x_index * wanted_channels);
			tensorflow::uint8* in_bottom_left_pixel = in_bottom_row
					+ (left_x_index * wanted_channels);
			tensorflow::uint8* in_bottom_right_pixel = in_bottom_row
					+ (right_x_index * wanted_channels);
			const float x_lerp = in_x - left_x_index;
			float* out_pixel = out_row + (x * wanted_channels);
			for (int c = 0; c < wanted_channels; ++c) {
				const float top_left(
						(in_top_left_pixel[c] - input_mean) / input_std);
				const float top_right(
						(in_top_right_pixel[c] - input_mean) / input_std);
				const float bottom_left(
						(in_bottom_left_pixel[c] - input_mean) / input_std);
				const float bottom_right(
						(in_bottom_right_pixel[c] - input_mean) / input_std);
...
{
	string graph = "tensorflow/contrib/pi_examples/label_image/data/"
			"tensorflow_inception_stripped.pb";
	string labels_file_name = "tensorflow/contrib/pi_examples/label_image/data/"
	...
	string root_dir = "";
	std::vector<Flag> flag_list = { Flag("graph", &graph, "graph file name"),
			Flag("labels", &labels_file_name, "labels file name"), Flag(
					"input_width", &input_width, "image input width"), Flag(
					"input_height", &input_height, "image input height"), Flag(
					"input_mean", &input_mean,
					"transformed mean of input pixels"), Flag("input_std",
					&input_std, "transformed std dev of input pixels"), Flag(
					"input_layer", &input_layer, "input layer name"), Flag(
					"output_layer", &output_layer, "output layer name"), Flag(
					"video_width", &video_width,
					"video width expected from device"), Flag("video_height",
					&video_height, "video height expected from device"), Flag(
					"print_threshold", &print_threshold,
					"print labels with scoe exceeding this"), Flag("root_dir",
					&root_dir,
	...
	Status format_status = SetCameraFormat(camera_handle, video_width,
			video_height);
	if (!format_status.ok()) {
	...
	CameraBuffer* buffers;
	Status start_capture_status = StartCameraCapture(camera_handle,
			how_many_buffers, &buffers);
	...
	for (int i = 0; i < 200; i++) {
		...
		int frame_data_size;
		v4l2_buffer buf;
		Status capture_next_status = CaptureNextFrame(camera_handle, buffers,
				&frame_data, &frame_data_size, &buf);
		if (!capture_next_status.ok()) {
			LOG(ERROR) << "CaptureNextFrame failed with "
					<< capture_next_status;
			return -1;
		}

		std::vector<Tensor> resized_tensors;
		Status tensor_from_frame_status = TensorFromFrame(frame_data,
				video_width, video_height, 3, input_height, input_width,
		...
		Status print_status = PrintTopLabels(outputs, labels, label_count,
				print_threshold * 0.01f);
		if (!print_status.ok()) {
	...
	Status end_capture_status = EndCameraCapture(camera_handle, buffers,
			how_many_buffers);
