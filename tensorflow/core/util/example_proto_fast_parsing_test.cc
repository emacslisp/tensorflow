...
void TestCorrectnessJson(const string& json)
{
	...
	string serialized;
	auto s = protobuf::util::JsonToBinaryString(resolver,
			"type.googleapis.com/tensorflow.Example", json, &serialized);
	EXPECT_TRUE(s.ok()) << s;
	delete resolver;
	TestCorrectness(serialized);
}
...
{
	static const char key_char_lookup[] = "0123456789{}~`!@#$%^&*()"
	...
	while (len-- > 0) {
		str.push_back(
				key_char_lookup[rng->Rand32()
						% (sizeof(key_char_lookup) / sizeof(key_char_lookup[0]))]);
	}
	return str;
}
...
void Fuzz(random::SimplePhilox* rng)
{
	...
	std::vector<Feature::KindCase> feat_types( { Feature::kBytesList,
			Feature::kFloatList, Feature::kInt64List });
	...
	while (num_concats--) {
		...
		for (auto i = 0; i < num_active_keys; ++i) {
			...
			case Feature::kBytesList: {
				BytesList* bytes_list =
						(*example.mutable_features()->mutable_feature())[fkey].mutable_bytes_list();
				while (num_features--) {
			...
			case Feature::kFloatList: {
				FloatList* float_list =
						(*example.mutable_features()->mutable_feature())[fkey].mutable_float_list();
				while (num_features--) {
			...
			case Feature::kInt64List: {
				Int64List* int64_list =
						(*example.mutable_features()->mutable_feature())[fkey].mutable_int64_list();
				while (num_features--) {
...
string MakeSerializedExample()
{
	...
	for (int i = 0; i < kBytesFeatureCount; ++i) {
		fmap[strings::StrCat(string('b', kFeatureNameLength), i)].mutable_bytes_list()->add_value(
				string('v', kFeatureValueLength));
	}
	for (int i = 0; i < kFloatFeatureCount; ++i) {
		fmap[strings::StrCat(string('f', kFeatureNameLength), i)].mutable_float_list()->add_value(
				123123123.123);
	}
	for (int i = 0; i < kInt64FeatureCount; ++i) {
		fmap[strings::StrCat(string('i', kFeatureNameLength), i)].mutable_int64_list()->add_value(
				10 * i);
	}
	string serialized;
	example.SerializeToString(&serialized);
