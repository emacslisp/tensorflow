...
public:
			...
			ifs_(file_name.c_str(), std::ios::in | std::ios::binary)
	{
		CHECK(ifs_.good()) << "Failed to open file \"" << file_name
				<< "\" or file is 0 length! Use prefix \"" << ASSET_PREFIX
		...
		const char* const filename, google::protobuf::MessageLite* message)
{
	...
	AAsset* asset = AAssetManager_open(asset_manager, asset_filename,
	...
	if (fd >= 0) {
		...
		if (data_size > 64 * 1024 * 1024) {
			LOG(WARNING)
					<< "Compressed proto is larger than 64mb; if problems occur "
