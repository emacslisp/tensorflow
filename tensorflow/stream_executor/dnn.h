...
enum class QuantizedActivationMode {
	k8Bit = 1, k16Bit = 2, k32Bit = 4,
};

// Specifies the data type used by an operation.
enum class DataType {
	kFloat = 0, kDouble = 1, kHalf = 2,
};
...
enum class RnnMode {
	kRnnRelu = 0, kRnnTanh = 1, kRnnLstm = 2, kRnnGru = 3,
};
...
enum class RnnInputMode {
	kRnnLinearSkip = 0, kRnnSkipInput = 1,
};
...
enum class RnnDirectionMode {
	kRnnUnidirectional = 0, kRnnBidirectional = 1,
};
...
public:
	...
	{
		return feature_map_count_;
	}
	int64 height() const
	{
		return GetDim(spatial_size_, DimIndex::Y);
	}
	int64 width() const
	{
		return GetDim(spatial_size_, DimIndex::X);
	}
	int64 spatial_dim(DimIndex dim) const
	{
		return GetDim(spatial_size_, dim);
	}
	int ndims() const
	{
		return ndims_;
...
public:
	...
	{
		return output_feature_map_count_;
	}
	...
	{
		return input_feature_map_count_;
	}
	...
	std::vector<int64> input_filter_dims() const
	{
...
public:
	...
	{
		return GetDim(zero_padding_, dim);
	}
	int filter_stride(DimIndex dim) const
	{
		return GetDim(filter_strides_, dim);
	}
	int ndims() const
	{
		return ndims_;
	...
	std::vector<int64> strides() const
	{
		return filter_strides_;
	}
	std::vector<int64> padding() const
	{
...
public:
	...
	AlgorithmType algorithm_no_scratch() const
	{
...
public:
	...
	virtual bool DoBatchNormalizationForward(Stream* stream,
	...
	virtual bool DoBatchNormalizationBackward(Stream* stream,
			...
			const DeviceMemory<float>& variance,
			const dnn::BatchDescriptor& x_desc,
			const dnn::BatchDescriptor& scale_offset_desc, const double epsilon,
			DeviceMemory<float>* x_backprop,
	...
	virtual bool DoConvolve(Stream* stream,
	...
	virtual bool DoSeparableConvolve(Stream* stream,
	...
	//  scratch_allocator: un-owned, may-be-null object that may allocate scratch
	//    space in order to speed up the convolution operation.
	virtual bool DoConvolveBackwardData(Stream* stream,
	...
	//  scratch_allocator: un-owned, may-be-null object that may allocate scratch
	//    space in order to speed up the convolution operation.
	virtual bool DoConvolveBackwardFilter(Stream* stream,
			const BatchDescriptor& input_descriptor,
			const DeviceMemory<float>& input_data,
			const BatchDescriptor& output_descriptor,
			DeviceMemory<float> backward_output_data,
	...
	virtual bool DoBiasAdd(Stream* stream,
			const DeviceMemory<float>& input_data,
	...
	// normalization.
	virtual bool DoNormalizeWithDimensions(Stream* stream,
			const dnn::NormalizeDescriptor& normalize_descriptor,
			const dnn::BatchDescriptor& dimensions,
			const DeviceMemory<float>& input_data,
			DeviceMemory<float>* output_data)
	...
	// See comments on NormalizeDescriptor for a description of local response
	// normalization.
	virtual bool DoNormalizeBackwardWithDimensions(Stream* stream,
			const dnn::NormalizeDescriptor& normalize_descriptor,
			const dnn::BatchDescriptor& dimensions,
	...
	virtual bool DoDepthConcatenate(Stream* stream,
	...
	//  output_data: un-owned device memory region in which to place the
	//    operation result.
	virtual bool DoElementwiseOperate(Stream* stream,
	...
	//  output_data: un-owned device memory region in which to place the
	//    padded result.
	virtual bool DoXYSlice(Stream* stream,
			const dnn::BatchDescriptor &dimensions,
	...
	virtual bool DoMemcpyD2HQuantized(Stream* stream,
	...
	//    succeed.
	virtual bool DoMemcpyH2DQuantized(Stream* stream, const void* host_src,
			int64 size, QuantizedActivationMode mode,
	...
	//    is no longer in use.
	virtual port::StatusOr<std::unique_ptr<dnn::RnnDescriptor>> createRnnDescriptor(
	...
	virtual port::StatusOr<std::unique_ptr<dnn::RnnSequenceTensorDescriptor>> createRnnSequenceTensorDescriptor(
			int seq_length, int batch_size, int data_size,
	...
	virtual port::StatusOr<std::unique_ptr<dnn::RnnStateTensorDescriptor>> createRnnStateTensorDescriptor(
			int num_layer, int batch_size, int data_size,
			dnn::DataType data_type)
	{
		return port::Status { port::error::UNIMPLEMENTED,
	...
	virtual bool DoRnnForward(Stream* stream,
	...
	//    afterwards.
	virtual bool DoRnnBackward(Stream* stream,
			const dnn::RnnDescriptor& rnn_desc,
			const dnn::RnnSequenceTensorDescriptor& input_desc,
			const DeviceMemory<float>& input_data,
			const dnn::RnnStateTensorDescriptor& input_h_desc,
