...
{
	gtl::STLDeleteValues(&node_to_context_);
}
...
Status ShapeRefiner::AddNode(const Node* node)
{
	...
	for (const Edge* e : node->in_edges()) {
		...
		if (it == node_to_context_.end()) {
			return errors::FailedPrecondition("Input ", e->dst_input(), " ('",
					input->name(), "') for '", node->name(),
		...
		if (e->src()->output_type(e->src_output()) == DT_RESOURCE) {
			input_handle_dtypes[e->dst_input()] = c->output_handle_dtype(
					e->src_output());
			input_handle_shapes[e->dst_input()] = c->output_handle_shape(
					e->src_output());
		}
	}
	...
	TF_RETURN_IF_ERROR(
			ops_registry_->LookUp(node->type_string(), &op_reg_data));
	...
	std::unique_ptr<InferenceContext> c(
			new InferenceContext(graph_def_version_, &node->def(),
					node->op_def(), input_shapes, input_tensors,
	...
	do {
		...
		for (int i = 0; i < c->num_inputs(); ++i) {
			...
			if (!attempted_materialization[i]) {
				...
				TF_RETURN_IF_ERROR(
						EvaluateConstantTensorForEdge(node, i, &evaluated,
								&result));
				if (evaluated) {
		...
		ShapeHandle shape)
{
	...
	if (output_port < 0 || output_port >= node->num_outputs()) {
		return errors::InvalidArgument("output_port '", output_port,
				"' is out of range, ", "node '", node->name(), "' has ",
				node->num_outputs(), " outputs");
	}
...
Status ShapeRefiner::EvaluateConstantTensorForEdge(const Node* node,
		int dst_idx, bool* evaluated, Tensor* result)
{
	*evaluated = false;
	...
	TF_RETURN_IF_ERROR(
			ExtractConstantSubgraph(input_edge->src(), &subgraph,
					&is_constant_graph, &const_inputs));
	if (!is_constant_graph) {
		return Status::OK();
	}
	const string output_tensor_name = strings::StrCat(input_edge->src()->name(),
	...
	Status s = GraphRunner::Run(&subgraph, nullptr /* function_library */,
			Env::Default(), const_inputs, { output_tensor_name }, &outputs);
...
Status ShapeRefiner::ExtractConstantSubgraph(Node* target_node,
		Graph* out_graph, bool* is_constant_graph,
		std::vector<std::pair<string, Tensor>>* const_inputs)
{
	...
	*is_constant_graph = true;
	...
	while (!edges_to_visit.empty()) {
		...
		const string& output_tensor_name = strings::StrCat(current_node->name(),
				":", current_edge->src_output());
...
Status ShapeRefiner::ConstantPartialShape(InferenceContext* target_context,
		const Node* node, int dst_idx, ShapeHandle* result)
{
	const Edge* input_edge;
	TF_RETURN_IF_ERROR(node->input_edge(dst_idx, &input_edge));
	...
	if (src_context == nullptr)
		return errors::Internal("Missing src context");
	...
	if (src_context->Value(src_context->Dim(src_shape, 0)) == 0) {
		...
		*result = src_context->input(0);
		...
		for (int i = 0; i < src_context->num_inputs(); ++i) {
			...
			bool evaluated = false;
			TF_RETURN_IF_ERROR(
					EvaluateConstantTensorForEdge(input_edge->src(), i,
							&evaluated, &scalar));
			if (evaluated) {
				int64 size;
				if (scalar.dtype() == DT_INT32) {
					...
					return errors::InvalidArgument(
							"Pack input must be int32 or int64");
				}
				dims.push_back(
						size < 0 ?
								target_context->UnknownDim() :
		...
		*result = target_context->Scalar();
		...
		for (int i = 0; i < src_context->num_inputs(); ++i) {
			...
			TF_RETURN_IF_ERROR(
					ConstantPartialShape(target_context, input_edge->src(), i,
							&sub_result));
		...
		TF_RETURN_IF_ERROR(
				target_context->MakeShapeFromTensor(evaluated ? &t : nullptr,
						src_shape, result));
	}
	return Status::OK();
}
