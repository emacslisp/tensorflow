...
Status DumpGraph(const MainFlags& flags, const string& name, const Graph& graph)
{
		...
		const protobuf::RepeatedPtrField<Feed>& feeds)
{
	for (int arg_index = 0; arg_index < feeds.size(); ++arg_index) {
		...
		if (it == node_map.end()) {
			return errors::NotFound("Can't find feed id: ",
					TensorIdToString(id));
		}
		const Node* feed_node = it->second;
		if (id.output_index() >= feed_node->num_outputs()) {
			return errors::InvalidArgument("Invalid feed id: ",
					TensorIdToString(id), ", output index should be < ",
		...
		TF_RETURN_IF_ERROR(
				NodeBuilder(strings::StrCat("_arg_", arg_index), kArgOp).Attr(
						"T",
						BaseType(feed_node->output_type(id.output_index()))).Attr(
						"index", arg_index).Attr(kFeedIdAttr,
						TensorIdToString(id)).Attr(kShapeAttr,
						TensorShape(feed.shape())).Attr(kDebugNameAttr,
						feed.name()).Finalize(graph, &arg_node));
		...
		std::unordered_set<const Node*>* retval_nodes)
{
	for (int ret_index = 0; ret_index < fetches.size(); ++ret_index) {
		...
		auto it = node_map.find(id.node_name());
		if (it == node_map.end()) {
			return errors::NotFound("Can't find fetch id: ",
					TensorIdToString(id));
		}
		Node* fetch_node = it->second;
		if (id.output_index() >= fetch_node->num_outputs()) {
			return errors::InvalidArgument("Invalid fetch id: ",
					TensorIdToString(id), ", output index should be < ",
		...
		TF_RETURN_IF_ERROR(
				NodeBuilder(strings::StrCat("_retval_", ret_index), kRetvalOp).Input(
						fetch_node, id.output_index()).Attr("T",
						BaseType(fetch_node->output_type(id.output_index()))).Attr(
						"index", ret_index).Attr(kFetchIdAttr,
						TensorIdToString(id)).Finalize(graph, &retval_node));
		...
		const MainFlags& flags)
{
	...
	if (!missing_feeds.empty() || !missing_fetches.empty()) {
		return errors::Aborted("Post graph-pruning", ", missing feeds: ",
				str_util::Join(missing_feeds, ", "), ", missing fetches: ",
...
Status CollectArgNodes(const Graph& graph, std::vector<Node*>* arg_nodes)
{
	...
	for (Node* n : graph.nodes()) {
		if (n->type_string() == kArgOp) {
			...
			if (!insert_result.second) {
				...
				return errors::InvalidArgument("Multiple ", kArgOp,
						" nodes with index ", index, ", ", n->DebugString(),
						" and ", dup->DebugString());
			}
		}
	}
	...
	for (const auto& index_node : indexed_arg_nodes) {
		if (index_node.first != arg_nodes->size()) {
			return errors::InvalidArgument("Expected ", kArgOp,
					" node with index ", arg_nodes->size(), ", but got index ",
					index_node.first);
		}
		...
		xla::Computation* computation, bool* has_context_arg)
{
	...
	std::unique_ptr<FunctionLibraryRuntime> flib_run(
			NewFunctionLibraryRuntime(compiler.device_mgr(), Env::Default(),
					compiler.device(), graph->versions().producer(), flib_def,
					OptimizerOptions()));
	XlaCompiler::CompilationResult result;
	TF_RETURN_IF_ERROR(
			compiler.CompileGraph("tfcompile", std::move(graph), flib_run.get(),
					xla_args, false /* use_tuple_arg */, &result));
	...
	*computation = std::move(result.computation);
	...
	for (int i = 0; i < result.outputs.size(); ++i) {
		...
		if (result.outputs[i].is_constant) {
			++num_const_results;
			LOG(ERROR) << "ConstRetVal index:" << i << " value:"
					<< result.outputs[i].constant_value.DebugString();
		}
	}
	if (num_const_results > 0) {
		return errors::Unimplemented(
				"Conversion from TensorFlow graph to XLA resulted in ",
				num_const_results, " constant results.  The configuration of "
		...
		CompileResult* compile_result)
{
	...
	compile_result->aot = xla::unique_ptr_static_cast
			< xla::cpu::CpuAotCompilationResult
			> (std::move(aot_or.ValueOrDie().back()));
	compile_result->entry_point = aot_opts.entry_point_name();
	compile_result->pointer_size = xla::LocalClient::PointerSizeForTriple(
			aot_opts.triple());
	return Status::OK();
}
		...
		std::unique_ptr<Graph>* graph)
{
	...
	TF_RETURN_IF_ERROR(
			AddDefaultAttrsToGraphDef(&copy_def, *g->op_registry(),
					0 /*node_offset*/));
	TF_RETURN_IF_ERROR(
			ConvertGraphDefToGraph(GraphConstructorOptions(), copy_def,
					g.get()));
...
Status CompileGraph(std::unique_ptr<Graph> graph, const MainFlags& flags,
		const FunctionLibraryDefinition* flib, CompileResult* compile_result)
{
	...
	gpu::Platform* cpu_platform = gpu::MultiPlatformManager::PlatformWithName(
			"Host").ValueOrDie();
	xla::LocalClient* client = xla::ClientLibrary::GetOrCreateLocalClient(
			cpu_platform).ValueOrDie();
	xla::Computation computation;
	TF_RETURN_IF_ERROR(
			ConvertGraphToXla(client, std::move(graph), flib, &computation,
	...
	xla::cpu::CpuAotCompilationOptions aot_opts(flags.target_triple,
			flags.target_cpu, flags.target_features, flags.entry_point,
