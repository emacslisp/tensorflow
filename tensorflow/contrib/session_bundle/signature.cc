...
Status GetNamedClassificationSignature(const string& name,
		const tensorflow::MetaGraphDef& meta_graph_def,
		ClassificationSignature* signature)
{
	Signatures signatures;
	...
	if (!it->second.has_classification_signature()) {
		return errors::FailedPrecondition(
				strings::StrCat(
						"Expected a classification signature for name \"", name,
...
Status RunClassification(const ClassificationSignature& signature,
		const Tensor& input, Session* session, Tensor* classes, Tensor* scores)
{
	...
	const Status run_status = session->Run( { { signature.input().tensor_name(),
			input } }, output_tensor_names, { }, &outputs);
	if (!run_status.ok()) {
		return run_status;
	}
		...
		Tensor* regression_output)
{
	...
	std::vector<Tensor> outputs;
	const Status run_status = session->Run( { { signature.input().tensor_name(),
			regression_input } }, output_tensor_names, { }, &outputs);
	if (!run_status.ok()) {
		return run_status;
	}
	...
	if (regression_output) {
		*regression_output = outputs[0];
		TF_RETURN_IF_ERROR(
				BatchSizesMatch(regression_input, *regression_output));
	}
	return Status::OK();
}
		...
		GenericSignature* signature)
{
	...
	if (it == signatures.named_signatures().end()) {
		return errors::InvalidArgument(
				strings::StrCat("Missing generic signature named \"", name,
						"\" in ", DebugStringIfAvailable(signatures)));
	}
	if (!it->second.has_generic_signature()) {
		return errors::InvalidArgument(
				strings::StrCat("Expected a generic signature: ",
						DebugStringIfAvailable(it->second)));
	}
...
Status GetNamedSignature(const string& name,
		const tensorflow::MetaGraphDef& meta_graph_def, Signature* signature)
{
	Signatures signatures;
	TF_RETURN_IF_ERROR(GetSignatures(meta_graph_def, &signatures));
	const auto& it = signatures.named_signatures().find(name);
		...
		std::vector<std::pair<string, Tensor>>* bound_inputs)
{
	...
	for (const auto& entry : inputs) {
		...
		if (mapped == bindings.end()) {
			return errors::NotFound(
					strings::StrCat("Could not find generic binding for: ",
							entry.first));
		}
		bound_inputs->push_back(
				{ mapped->second.tensor_name(), entry.second });
	}
	return Status::OK();
}
