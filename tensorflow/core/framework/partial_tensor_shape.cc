		...
		const PartialTensorShape& shape) const
{
	...
	if (!out.is_unknown_ && !shape.is_unknown_) {
		for (auto s : shape.dim_sizes_)
			out.dim_sizes_.push_back(s);
	}
	return out;
}
...
bool PartialTensorShape::IsCompatibleWith(const PartialTensorShape& shape) const
{
	if (is_unknown_ || shape.is_unknown_)
		return true;
	if (dims() != shape.dims())
		return false;
	for (int i = 0; i < dims(); i++) {
		if (dim_size(i) == -1 || shape.dim_size(i) == -1)
			continue;
		if (dim_size(i) != shape.dim_size(i))
			return false;
	}
	return true;
}
