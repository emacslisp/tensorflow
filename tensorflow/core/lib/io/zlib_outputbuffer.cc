...
ZlibOutputBuffer::ZlibOutputBuffer(WritableFile* file, int32 input_buffer_bytes, // size of z_stream.next_in buffer
		int32 output_buffer_bytes, const ZlibCompressionOptions& zlib_options) // size of z_stream.next_out buffer
:
		file_(file), init_status_(), input_buffer_capacity_(input_buffer_bytes), output_buffer_capacity_(
				output_buffer_bytes), z_stream_input_(
				new Bytef[input_buffer_bytes]), z_stream_output_(
				new Bytef[output_buffer_bytes]), zlib_options_(zlib_options), z_stream_(
				new z_stream)
{
}
...
ZlibOutputBuffer::~ZlibOutputBuffer()
{
	if (z_stream_.get()) {
		LOG(WARNING)
				<< "ZlibOutputBuffer::Close() not called. Possible data loss";
	}
}
...
Status ZlibOutputBuffer::Init()
{
	...
	int status = deflateInit2(z_stream_.get(), zlib_options_.compression_level,
...
void ZlibOutputBuffer::AddToInputBuffer(StringPiece data)
{
	...
	int32 free_tail_bytes = input_buffer_capacity_
			- (read_bytes + unread_bytes);

	if (bytes_to_write > free_tail_bytes) {
		memmove(z_stream_input_.get(), z_stream_->next_in, z_stream_->avail_in);
		z_stream_->next_in = z_stream_input_.get();
	}
	memcpy(z_stream_->next_in + z_stream_->avail_in, data.data(),
			bytes_to_write);
...
Status ZlibOutputBuffer::FlushOutputBufferToFile()
{
	...
	if (bytes_to_write > 0) {
		Status s = file_->Append(
				StringPiece(reinterpret_cast<char*>(z_stream_output_.get()),
						bytes_to_write));
		if (s.ok()) {
			z_stream_->next_out = z_stream_output_.get();
			z_stream_->avail_out = output_buffer_capacity_;
