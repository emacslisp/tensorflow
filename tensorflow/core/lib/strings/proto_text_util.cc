...
bool ProtoParseBoolFromScanner(Scanner* scanner, bool* value)
{
	...
	if (!scanner->RestartCapture().Many(Scanner::LETTER_DIGIT).GetResult(
			nullptr, &bool_str)) {
		return false;
	}
...
bool ProtoParseStringLiteralFromScanner(Scanner* scanner, string* value)
{
	...
	if (!scanner->One(Scanner::ALL).RestartCapture().ScanEscapedUntil(quote).StopCapture().One(
			Scanner::ALL).GetResult(nullptr, &value_sp)) {
		return false;
	}
	ProtoSpaceAndComments(scanner);
