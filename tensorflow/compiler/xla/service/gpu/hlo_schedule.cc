...
GpuHloOrdering::GpuHloOrdering(const HloModule* module,
		const StreamAssignment& stream_assignment,
		const std::vector<const HloInstruction*>& thunk_launch_order) :
...
tensorflow::Status DFSLaunchOrder(const HloComputation* computation,
		...
		std::vector<const HloInstruction*>* launch_order)
{
	...
	for (const auto& hlo : computation->instructions()) {
		if (hlo->operand_count() == 0) {
			...
			incoming_edge_count[hlo.get()] = std::set<HloInstruction*>(
					hlo->operands().begin(), hlo->operands().end()).size();
		}
	}
		...
		const HloModule& module, const StreamAssignment& stream_assignment)
{
	...
	schedule->hlo_ordering_ = MakeUnique<GpuHloOrdering>(&module,
			stream_assignment, schedule->thunk_launch_order_);
