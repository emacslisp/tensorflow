...
protected:
	...
	bool Compare()
	{
		...
		Eigen::Tensor<float, 4, Eigen::RowMajor> expected(batch_size, rows,
				cols, depth);
		...
		Eigen::Tensor<float, 0, Eigen::RowMajor> sum =
				((expected - actual).abs() > actual.constant(tol_)).select(
						actual.constant(1), actual.constant(0)).sum();
		return sum() == 0;
	}
