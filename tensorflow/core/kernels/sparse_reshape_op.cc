...
public:
	explicit SparseReshapeOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int d = 0; d < output_rank; ++d) {
			...
			if (size == -1) {
				OP_REQUIRES(context, unknown_index == -1,
						errors::InvalidArgument(
								"only one output shape size may be -1, "
										"not both ", unknown_index, " and ",
								d));
				unknown_index = d;
		...
		if (unknown_index != -1) {
			OP_REQUIRES(context, product > 0,
					errors::InvalidArgument(
							"SparseReshape cannot infer the missing "
									"input size for an empty tensor unless all "
									"specified input sizes are non-zero"));
			const int64 missing = dense_size / product;
			OP_REQUIRES(context, product * missing == dense_size,
					errors::InvalidArgument(
		...
		OP_REQUIRES(context, output_shape.num_elements() == dense_size,
				errors::InvalidArgument("Input to reshape is a tensor with ",
						dense_size, " dense values, but the "
								"requested shape has ",
		...
		for (int d = input_rank - 2; d >= 0; --d) {
			input_strides[d] = input_strides[d + 1]
					* input_shape.dim_size(d + 1);
		}
