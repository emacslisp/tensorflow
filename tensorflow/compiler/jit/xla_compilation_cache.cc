		...
		const XlaCompilationCache::Signature& signature) const
{
	...
	for (const auto& arg : signature.arg_values) {
		h = Hash64Combine(h, std::hash<int>()(static_cast<int>(arg.first)));
		h = Hash64Combine(h,
				Hash64(arg.second.tensor_data().data(),
...
Status XlaCompilationCache::Compile(const NameAttrList& function,
		int num_constant_args, OpKernelContext* ctx,
		...
		xla::LocalExecutable** executable)
{
	...
	if (VLOG_IS_ON(2)) {
		...
		for (int i = 0; i < ctx->num_inputs(); i++) {
			...
			VLOG(2) << i << ": dtype=" << ctx->input_dtype(i) << " present="
					<< ctx->has_input(i) << " shape=" << shape.DebugString();
	...
	if (!entry->compiled) {
		...
		// a long time.)
		std::vector<XlaCompiler::Argument> args = BuildArguments(
				num_constant_args, ctx);

		std::unique_ptr<FunctionLibraryRuntime> flr(
				NewFunctionLibraryRuntime(compiler_.device_mgr(), ctx->env(),
						compiler_.device(), TF_GRAPH_DEF_VERSION,
						ctx->function_library()->GetFunctionLibraryDefinition(),
						OptimizerOptions(),
						nullptr /* custom_kernel_creator */));

		entry->compiled = true;
		entry->compilation_status = compiler_.CompileFunction(flr.get(),
