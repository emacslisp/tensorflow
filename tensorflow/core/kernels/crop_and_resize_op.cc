...
static inline void ParseAndCheckBoxSizes(OpKernelContext* context,
		const Tensor& boxes, const Tensor& box_ind, int* num_boxes)
{
...
inline void CheckValidBoxInd(OpKernelContext* context,
...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("extrapolation_value", &extrapolation_value_));
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape( {
				num_boxes, crop_height, crop_width, depth }), &output));

		typename TTypes<T, 4>::ConstTensor image_data = image.tensor<T, 4>();
		typename TTypes<float, 2>::ConstTensor boxes_data = boxes.tensor<float,
				2>();
		typename TTypes<int32, 1>::ConstTensor box_ind_data = box_ind.tensor<
				int32, 1>();
		typename TTypes<float, 4>::Tensor crops_data =
				output->tensor<float, 4>();

		CheckValidBoxInd<Device>(context, box_ind_data, batch);

		bool status = functor::CropAndResize<Device, T>()(
				context->eigen_device<Device>(), image_data, boxes_data,
				box_ind_data, extrapolation_value_, crops_data);
		if (!status) {
			context->SetStatus(
...
struct CropAndResize<CPUDevice, T> {
	bool operator()(const CPUDevice& d,
			...
			float extrapolation_value, typename TTypes<float, 4>::Tensor crops)
	{
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, grads.dim_size(0) == num_boxes,
		...
		OP_REQUIRES(context, image_height > 0 && image_width > 0,
				errors::InvalidArgument("image dimensions must be positive"));
		OP_REQUIRES(context, grads.dim_size(3) == depth,
				errors::InvalidArgument(
		...
		Tensor* output = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape( {
				batch, image_height, image_width, depth }), &output));

		typename TTypes<float, 4>::ConstTensor grads_data = grads.tensor<float,
				4>();
		typename TTypes<float, 2>::ConstTensor boxes_data = boxes.tensor<float,
				2>();
		typename TTypes<int32, 1>::ConstTensor box_ind_data = box_ind.tensor<
		...
		bool status = functor::CropAndResizeBackpropImage<Device, T>()(
				context->eigen_device<Device>(), grads_data, boxes_data,
				box_ind_data, output_data);
		if (!status) {
			context->SetStatus(
					errors::Internal(
							"Failed launch CropAndResizeBackpropImageKernel."));
		}
	}
};
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		bool status = functor::CropAndResizeBackpropBoxes<Device, T>()(
				context->eigen_device<Device>(), grads_data, image_data,
				boxes_data, box_ind_data, output_data);
		if (!status) {
			context->SetStatus(
					errors::Internal(
							"Failed launch CropAndResizeBackpropBoxesKernel."));
		}
	}
};
...
struct CropAndResizeBackpropBoxes<CPUDevice, T> {
			...
			typename TTypes<float, 2>::Tensor grads_boxes)
	{
		...
		for (int b = 0; b < num_boxes; ++b) {
			...
			const float width_scale =
					(crop_width > 1) ? (x2 - x1) * width_ratio : 0;
			...
			for (int y = 0; y < crop_height; ++y) {
				...
				for (int x = 0; x < crop_width; ++x) {
					...
					for (int d = 0; d < depth; ++d) {
						...
						float image_grad_y = (1 - x_lerp)
								* (bottom_left - top_left)
								+ x_lerp * (bottom_right - top_right);
						float image_grad_x = (1 - y_lerp)
								* (top_right - top_left)
						...
						// dy1, dy2
						if (crop_height > 1) {
							grads_boxes(b, 0) += image_grad_y
									* (image_height - 1 - y * height_ratio);
							grads_boxes(b, 2) += image_grad_y
									* (y * height_ratio);
						} else {
							grads_boxes(b, 0) += image_grad_y * 0.5
									* (image_height - 1);
							grads_boxes(b, 2) += image_grad_y * 0.5
									* (image_height - 1);
						}
						// dx1, dx2
						if (crop_width > 1) {
							grads_boxes(b, 1) += image_grad_x
									* (image_width - 1 - x * width_ratio);
							grads_boxes(b, 3) += image_grad_x
									* (x * width_ratio);
						} else {
							grads_boxes(b, 1) += image_grad_x * 0.5
									* (image_width - 1);
							grads_boxes(b, 3) += image_grad_x * 0.5
									* (image_width - 1);
						}
					}
				}
...
template<>
inline void CheckValidBoxInd<CPUDevice>(OpKernelContext* context,
