...
GlobalDataHandle AllocationTracker::Register(Backend* backend,
		int device_ordinal, se::DeviceMemoryBase device_memory,
...
GlobalDataHandle AllocationTracker::RegisterInternal(Backend* backend,
		int device_ordinal, se::DeviceMemoryBase device_memory,
		const Shape& shape, const string& tag, int initial_ref_count)
{
	VLOG(2) << "RegisterInternal(" << "tag: \"" << tag << "\" "
			<< "device_ordinal: " << device_ordinal << " " << "device_memory: "
			<< device_memory.opaque() << " " << "shape: "
	...
	if (handle_it != handle_map.end()) {
		...
		auto inserted = handle_to_allocation_.emplace(handle,
				MakeUnique<Allocation>(backend, device_ordinal, device_memory,
...
tensorflow::Status AllocationTracker::DeallocateShape(Backend* backend,
		int device_ordinal, se::DeviceMemoryBase* device_memory,
		const Shape& shape, std::set<void*>* deallocated_buffers)
{
	VLOG(2) << "DeallocateShape(" << "shape: \"" << shape.ShortDebugString()
	...
	if (ShapeUtil::IsTuple(shape)) {
		...
		TF_RET_CHECK(ShapeUtil::TupleElementCount(shape) == elements.size())
				<< "tuple has unexpected number of elements: "
				<< elements.size() << " != "
				<< ShapeUtil::TupleElementCount(shape);
		for (int i = 0; i < elements.size(); ++i) {
			VLOG(2) << "recursing onto the tuple elements";
			TF_RETURN_IF_ERROR(
					DeallocateShape(backend, device_ordinal, &elements[i],
							shape.tuple_shapes(i), deallocated_buffers));
		}
	}

	return backend->memory_allocator()->Deallocate(device_ordinal,
			device_memory);
}
		...
		const GlobalDataHandle& data)
{
	...
	TF_ASSIGN_OR_RETURN(std::vector < se::DeviceMemoryBase > element_bases,
			allocation->backend()->transfer_manager()->ShallowCopyTupleFromDevice(
					executor, allocation->device_memory(),
					allocation->shape()));

	std::vector<GlobalDataHandle> element_handles;
	for (int i = 0; i < element_bases.size(); ++i) {
		element_handles.push_back(
				RegisterInternal(allocation->backend(),
						allocation->device_ordinal(), element_bases[i],
						ShapeUtil::GetSubshape(allocation->shape(), { i }),
						tensorflow::strings::StrCat(allocation->tag(),
								".element_", i),
						/*initial_ref_count=*/2));
	}
		...
		const GlobalDataHandle& data)
{
	...
	if (allocation->is_deallocated()) {
		return InvalidArgument(
				"global data handle %lld was previously deallocated",
				data.handle());
	}
