...
static void SegmentReductionValidationHelper(OpKernelContext* context,
		const Tensor& input, const Tensor& segment_ids)
{
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const Index output_rows =
				num_indices > 0 ?
						internal::SubtleMustCopy(segment_vec(num_indices - 1))
								+ 1 :
						0;
		...
		while (end <= num_indices) {
			...
			if (end < num_indices) {
				...
				OP_REQUIRES(context, out_index + 1 == next_index,
						errors::InvalidArgument(
								"segment ids are not increasing by 1"));
			}
			...
			OP_REQUIRES(context, FastBoundsCheck(out_index, output_rows),
					errors::InvalidArgument("Segment id ", out_index,
							" out of range [0, ", output_rows,
			...
			if (start == end - 1) {
				typedef Eigen::TensorMap<
						Eigen::Tensor<const T, 1, Eigen::RowMajor>,
						Eigen::Unaligned> InT;
				InT in_slice(in_slice_ptr, out_slice_shape);
				out_slice = in_slice;
			} else {
				Eigen::DSizes < Eigen::DenseIndex, 2
						> in_slice_shape(end - start, num_col);
				typedef Eigen::TensorMap<
						Eigen::Tensor<const T, 2, Eigen::RowMajor>,
						Eigen::Unaligned> InT;
...
struct UnsortedSegmentSumFunctor<CPUDevice, T, Index> {
			...
			typename TTypes<T, 2>::Tensor output)
	{
		...
		auto data_flat = typename TTypes<T, 2>::ConstTensor(data, N,
				data_size / N);
		for (int64 i = 0; i < N; ++i) {
			Index j = internal::SubtleMustCopy(segment_ids(i));
			OP_REQUIRES(ctx, FastBoundsCheck(j, output_rows),
					errors::InvalidArgument("segment_ids",
							SliceDebugString(segment_ids_shape, i), " = ", j,
							" is out of range [0, ", output_rows, ")"));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		functor::UnsortedSegmentSumFunctor<Device, T, Index>()(context,
				context->template eigen_device<Device>(), output_rows,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		while (true) {
			...
			const int bad_offset = Reduce(input_flat, indices_vec, start,
					end - start, out);
			OP_REQUIRES(context, bad_offset < 0,
					errors::InvalidArgument("Bad: indices[", start + bad_offset,
...
template<typename Device, class T>
class SparseSegmentReductionMeanOp: public SparseSegmentReductionOpBase<Device,
		T> {
public:
...
template<typename Device, class T>
class SparseSegmentReductionSqrtNOp: public SparseSegmentReductionOpBase<Device,
		T> {
public:
...
template<typename Device, class T>
class SparseSegmentReductionSumOp: public SparseSegmentReductionOpBase<Device, T> {
public:
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const SegmentId M = internal::SubtleMustCopy(
				output_dim0.scalar<SegmentId>()());
		...
		const SegmentId num_segments = internal::SubtleMustCopy(
				segment_vec(N - 1)) + 1;
		...
		for (int64 i = 0; i < N; ++i) {
			...
			OP_REQUIRES(context, FastBoundsCheck(idx, num_segments),
					errors::InvalidArgument("Segment id ", idx,
