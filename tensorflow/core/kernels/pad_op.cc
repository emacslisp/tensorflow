...
public:
	explicit PadOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const int fixed_dims =
				(allow_legacy_scalars() && dims == 0 && in1.dim_size(0) == 1) ?
						1 : dims;
		OP_REQUIRES(context, fixed_dims == in1.dim_size(0),
				errors::InvalidArgument(
		...
		for (int d = 0; d < fixed_dims; ++d) {
			...
			OP_REQUIRES(context, before_d >= 0 && after_d >= 0,
					errors::InvalidArgument("Paddings must be non-negative: ",
							before_d, " ", after_d));
			const int64 size_d =
					(allow_legacy_scalars() && d == in0.dims()) ?
							1 : in0.dim_size(d);
...
private:
			...
			TTypes<int32>::ConstMatrix paddings, Tensor* output)
	{
		...
		functor(context->eigen_device<Device>(), output->tensor<T, Dims>(),
				input, paddings_array);
	}
};
