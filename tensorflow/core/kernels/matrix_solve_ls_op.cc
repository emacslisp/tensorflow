...
public:
	...
	virtual void ValidateInputMatrixShapes(OpKernelContext* context,
	...
	{
		return TensorShapes(
				{ TensorShape( { input_matrix_shapes[0].dim_size(1),
	...
	int64 GetCostPerUnit(const TensorShapes& input_matrix_shapes) const final
	{
		...
		double num_rhss =
				static_cast<double>(input_matrix_shapes[1].dim_size(1));
		double cost = std::max(m, n) * std::min(m, n)
				* (std::min(m, n) + num_rhss);
		return cost >= static_cast<double>(kint64max) ?
				kint64max : static_cast<int64>(cost);
	}
			...
			MatrixMaps* outputs) final
	{
		...
		OP_REQUIRES(context,
				TensorShapeUtils::IsScalar(l2_regularizer_in.shape()),
				errors::InvalidArgument(
						"l2_regularizer must be scalar, got shape ",
		...
		if (fast_) {
			...
			if (matrix.rows() >= matrix.cols()) {
				...
				gramian.template triangularView<Eigen::Lower>() = matrix
						* matrix.transpose();
				if (l2_regularizer > 0) {
					gramian +=
			...
			outputs->at(0) = matrix.completeOrthogonalDecomposition().solve(
					rhs);
		}
	}
