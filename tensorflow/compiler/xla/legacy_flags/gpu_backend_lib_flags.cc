...
static void AllocateFlags()
{
	...
	flags->opt_level = 3;
	flag_list =
			new std::vector<tensorflow::Flag>(
					{
							tensorflow::Flag("dump_temp_products_to",
									&flags->dump_temp_products_to,
									"dump temporary compilation products to this directory. "
											"If empty, no dump is produced"),
							tensorflow::Flag("ftz", &flags->ftz,
									"flush to zero semantics"),
							tensorflow::Flag("fma", &flags->fma,
									"use FMA synthesis"), tensorflow::Flag(
									"gpu_architecture",
									&flags->gpu_architecture,
									"GPU architecture"), tensorflow::Flag(
									"verbose_ptx_asm", &flags->verbose_ptx_asm,
									"emit PTX assembly with extra comments"),
							tensorflow::Flag("kernel", &flags->kernel,
									"only emit the IR and PTX for this kernel"),
							tensorflow::Flag("llvm_dump_passes",
									&flags->llvm_dump_passes,
									"dump the passes LLVM runs to stderr"),
							tensorflow::Flag("llvm_cl_opts",
									&flags->llvm_cl_opts,
									"comma-separated list of command line options to pass to "
											"LLVM.  For example, --llvm_cl_opts=--print-before=loop-unroll"),
							tensorflow::Flag("dump_ir_before_passes",
