...
static Status ReadEntireFile(Env* env, const string& filename, string* contents)
{
	uint64 file_size = 0;
...
public:
	WholeFileReader(Env* env, const string& node_name) :
			ReaderBase(strings::StrCat("WholeFileReader '", node_name, "'")), env_(
					env)
	{
	}
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output("contents", TensorShape( { }),
						&output));
		OP_REQUIRES_OK(context,
