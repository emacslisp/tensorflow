		...
		OutT output_mat, Random& uniform)
{
	...
	for (IntT i = 0; i < size; i++) {
		output_mat.template chip<0>(i) = input_mat.template chip<0>(
				permutation[i]);
	}
}
...
public:
	explicit RandomShuffleOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (input.NumElements() <= 1 || input.dim_size(0) <= 1) {
			...
			random::SingleSampleAdapter<random::PhiloxRandom> single(
					&local_gen);
