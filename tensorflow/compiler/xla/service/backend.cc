	...
	explicit EigenThreadPoolWrapper() :
			pool(
					new tensorflow::thread::ThreadPool(
							...
							tensorflow::port::NumSchedulableCPUs())), wrapper(
					new tensorflow::EigenThreadPoolWrapper(pool.get())), device(
					new Eigen::ThreadPoolDevice(wrapper.get(),
							wrapper->NumThreads()))
	{
	}
		...
		perftools::gputools::Platform* platform, int64 replica_count)
{
	...
	std::unique_ptr<Backend> backend(
			new Backend(replica_count, platform, compiler, stream_executors,
					transfer_manager));
	TF_RETURN_IF_ERROR(
			backend->PoolStreams(kInitialStreamsToPool,
					backend->default_stream_executor()));
	return std::move(backend);
}

/* static */StatusOr<std::unique_ptr<Backend>> Backend::CreateDefaultBackend()
{
...
void Backend::ReleaseStream(std::unique_ptr<perftools::gputools::Stream> stream)
{
	tensorflow::mutex_lock lock(mutex_);
...
Backend::Backend(int64 replica_count, perftools::gputools::Platform* platform,
		Compiler* compiler,
		tensorflow::gtl::ArraySlice<se::StreamExecutor*> stream_executors,
		TransferManager* transfer_manager) :
		platform_(platform), compiler_(compiler), transfer_manager_(
				transfer_manager), replica_count_(replica_count)
{
	...
	memory_allocator_ = MakeUnique<StreamExecutorMemoryAllocator>(platform,
			stream_executors);
	...
	CHECK(!stream_executors_.empty()) << "Service found no devices for backend "
			<< platform_->Name() << '.';
...
const Eigen::ThreadPoolDevice* Backend::eigen_intra_op_thread_pool_device() const
{
