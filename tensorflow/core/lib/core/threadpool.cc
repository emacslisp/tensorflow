...
struct EigenEnvironment {
	...
	Task CreateTask(std::function<void()> f)
	{
		...
		if (port::Tracing::IsActive()) {
			...
			port::Tracing::RecordEvent(
					port::Tracing::EventCategory::kScheduleClosure, id);
		}
		return Task { std::unique_ptr<TaskImpl>(new TaskImpl { std::move(f),
				Context(ContextKind::kThread), id, }), };
	}
...
struct ThreadPool::Impl: Eigen::ThreadPoolTempl<EigenEnvironment> {
			...
			std::function<void(int64, int64)> fn)
	{
		...
		device.parallelFor(total, Eigen::TensorOpCost(0, 0, cost_per_unit),
		...
		const string& name, int num_threads)
{
	...
	impl_.reset(
			new ThreadPool::Impl(env, thread_options, "tf_" + name,
					num_threads));
}
...
void ThreadPool::ParallelForWithWorkerId(int64 total, int64 cost_per_unit,
