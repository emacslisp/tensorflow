...
Status GetTableAttrs(OpKernelConstruction* context, string* project_id,
		string* dataset_id, string* table_id, int64* timestamp_millis,
		std::vector<string>* columns, string* test_end_point)
{
...
public:
	explicit BigQueryReader(BigQueryTableAccessor* bigquery_table_accessor,
			...
			ReaderBase(strings::StrCat("BigQueryReader '", node_name, "'")), bigquery_table_accessor_(
					CHECK_NOTNULL(bigquery_table_accessor))
	{
	}
	...
	Status ReadLocked(string* key, string* value, bool* produced, bool* at_end) override
	{
		...
		*produced = false;
		...
		int64 row_id;
		TF_RETURN_IF_ERROR(
				bigquery_table_accessor_->ReadRow(&row_id, &example));
...
public:
			...
			ReaderOpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				BigQueryTableAccessor::New(project_id, dataset_id, table_id,
						timestamp_millis, kDefaultRowBufferSize, test_end_point,
						columns, BigQueryTablePartition(),
