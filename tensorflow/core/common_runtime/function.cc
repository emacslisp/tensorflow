...
public:
	...
	Status Instantiate(const string& function_name,
			const InstantiateAttrValueMap& attrs, Handle* handle) override;
	...
	const FunctionLibraryDefinition* GetFunctionLibraryDefinition() const override
	{
		return lib_def_;
	}
...
private:
	...
	Status FunctionDefToBody(const FunctionDef& fdef,
			const InstantiateAttrValueMap& attrs, FunctionBody** fbody);
...
FunctionLibraryRuntimeImpl::FunctionLibraryRuntimeImpl(const DeviceMgr* dmgr,
		Env* env, Device* device, int graph_def_version,
		const FunctionLibraryDefinition* lib_def,
		const OptimizerOptions& optimizer_options,
		CustomKernelCreator custom_kernel_creator) :
		device_mgr_(dmgr), device_(device), env_(env), graph_def_version_(
				graph_def_version), lib_def_(lib_def), optimizer_(
				optimizer_options), custom_kernel_creator_(
...
{
	for (FunctionBody* p : func_graphs_)
		delete p;
		...
		OpKernel** kernel)
{
	...
	for (const auto& t : fbody->arg_types) {
		input_memory_types.push_back(
				t == DT_INT32 ? HOST_MEMORY : DEVICE_MEMORY);
	}
	MemoryTypeVector output_memory_types;
	for (const auto& t : fbody->ret_types) {
		output_memory_types.push_back(
				t == DT_INT32 ? HOST_MEMORY : DEVICE_MEMORY);
	}
	...
	OpKernelConstruction construction(device_type, device_,
			device_->GetAllocator(AllocatorAttributes()), &ndef,
			&fbody->fdef.signature(), this, fbody->arg_types,
		...
		const InstantiateAttrValueMap& attrs, FunctionBody** fbody)
{
	...
	TF_RETURN_IF_ERROR(
			InstantiateFunction(fdef, attrs, get_func_sig_, &result));
		...
		const NameAttrList& func, FunctionBody** g_body)
{
	...
	if (fdef == nullptr) {
		...
		TF_RETURN_IF_ERROR(
				gradient::GetOpGradientCreator(func.name(), &creator));
		if (creator == nullptr) {
...
Status FunctionLibraryRuntimeImpl::Instantiate(const string& function_name,
		const InstantiateAttrValueMap& attrs, Handle* handle)
{
	...
	if (function_name == kGradientOp) {
		...
		if (f == nullptr) {
			return errors::InvalidArgument(
					"SymbolicGradient is missing attr: f");
		}
		...
		if (fdef == nullptr) {
			return errors::NotFound("Function ", function_name,
					" is not defined.");
		}
		...
		DoneCallback done)
{
	...
	const FunctionBody* fbody = GetFunctionBody(handle);
	FunctionCallFrame* frame = new FunctionCallFrame(fbody->arg_types,
			fbody->ret_types);
...
FunctionLibraryRuntime* NewFunctionLibraryRuntime(const DeviceMgr* dmgr,
		Env* env, Device* device, int graph_def_version,
		const FunctionLibraryDefinition* lib_def,
		const OptimizerOptions& optimizer_options,
		CustomKernelCreator custom_kernel_creator)
{
	return new FunctionLibraryRuntimeImpl(dmgr, env, device, graph_def_version,
			lib_def, optimizer_options, custom_kernel_creator);
}
		...
		const OptimizerOptions& optimizer_options)
{
	return NewFunctionLibraryRuntime(dmgr, env, device, graph_def_version,
			lib_def, optimizer_options, GetCustomCreatorSingleton()->Get());
}
		...
		for (Node* n : matches) {
			...
			for (const Edge* out : n->out_edges()) {
				if (out->IsControlEdge()) {
					...
					g->AddEdge(in->src(), in->src_output(), out->dst(),
							out->dst_input());
				}
			}
...
bool RemoveListArrayConverter(Graph* g)
{
	...
	if (!matches.empty()) {
		for (Node* n : matches) {
			...
			gtl::InlinedVector<Node*, 8> identity_nodes(n->num_inputs(),
					nullptr);
			...
			for (const Edge* e : n->out_edges()) {
				if (e->IsControlEdge()) {
					...
					if (id_node == nullptr) {
						LOG(ERROR)
								<< "RemoveListArrayConverter unexpected missing input: "
								<< e->src_output();
						return removed_any;
					}
					CHECK(id_node);
		...
		const FunctionBody* fbody)
{
	if (!ValidateInlining(caller, fbody)) {
		LOG(WARNING) << "Inlining mismatch: " << caller->DebugString()
				<< " vs. " << DebugString(fbody->graph);
		return;
	}
...
bool ExpandInlineFunctions(FunctionLibraryRuntime* lib, Graph* graph)
{
	...
	for (Node* node : graph->nodes()) {
		...
		bool noinline;
		if (fld->GetAttr(node->def(), kNoInlineAttr, &noinline).ok()
				&& noinline) {
			VLOG(3) << "noinline: " << node->DebugString();
			continue;
		}
		FunctionLibraryRuntime::Handle handle;
		Status s = lib->Instantiate(node->type_string(), node->def().attr(),
				&handle);
		if (!s.ok()) {
		...
		for (const Edge* e : n->in_edges()) {
			if (e->IsControlEdge()) {
				...
				if (inputs[e->dst_input()] == nullptr) {
					...
					LOG(WARNING)
							<< "Malformed graph node. multiple input edges: "
							<< n->DebugString();
				}
			}
		}
...
FunctionBody::FunctionBody(const FunctionDef& f, DataTypeSlice arg_t,
		...
		fdef(f), graph(g), arg_types(arg_t.begin(), arg_t.end()), ret_types(
				ret_t.begin(), ret_t.end())
{
...
public:
	explicit SymbolicGradientHelper(const FunctionBody& f) :
			fbody_(&f)
	{
	}
...
FunctionBody* SymbolicGradientHelper::Compute()
{
	...
	TF_CHECK_OK(
			AddSymbolicGradients(y_node_outputs, x_node_outputs,
					y_grad_node_outputs, &x_grad_node_outputs, g));
	...
	for (size_t i = 0; i < fbody_->arg_types.size(); ++i) {
		Endpoint grad = { x_grad_node_outputs[i].node,
				x_grad_node_outputs[i].index };
