...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, in_depth == filter_shape.dim_size(2),
				errors::InvalidArgument(
		...
		xla::ComputationDataHandle conv =
				ctx->builder()->ConvWithGeneralDimensions(ctx->Input(0),
						ctx->Input(1), window_strides, xla_padding, dims);
...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				Conv2DBackpropComputeDimensions("Conv2DBackpropInput",
		...
		xla::ComputationDataHandle mirrored_weights = ctx->builder()->Rev(
				filter,
		...
		xla::ComputationDataHandle in_backprop =
				ctx->builder()->ConvGeneralDilated(out_backprop,
						mirrored_weights, /*window_strides=*/{ 1, 1 },
						/*padding=*/{ { dims.rows.pad_before,
								dims.rows.pad_after }, { dims.cols.pad_before,
								dims.cols.pad_after } },
...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		Conv2DBackpropDimensions dims;
		OP_REQUIRES_OK(ctx,
				Conv2DBackpropComputeDimensions("Conv2DBackpropFilter",
						activations_shape, filter_shape, out_backprop_shape,
		...
		//
		const int padded_in_rows = dims.rows.expanded_output_size
				+ dims.rows.filter_size - 1;
		const int padded_in_cols = dims.cols.expanded_output_size
				+ dims.cols.filter_size - 1;
		...
		auto filter_backprop = ctx->builder()->ConvGeneralDilated(activations,
				gradients,
				/*window_strides=*/{ 1, 1 },
				/*padding=*/{ { top_pad_in_rows, total_pad_in_rows
						- top_pad_in_rows }, { left_pad_in_cols,
						total_pad_in_cols - left_pad_in_cols } },
				/*lhs_dilation=*/{ 1, 1 },
		...
		xla::ComputationDataHandle filter_backprop_reshaped =
				ctx->builder()->Transpose(filter_backprop, { h_dim, w_dim,
						c_dim, n_dim });
