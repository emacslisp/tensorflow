...
ALWAYS_INLINE void ScalarMulAdd3Way(const float a1, const float a2,
		const float a3, const bfloat16** inp1, const bfloat16** inp2,
...
ALWAYS_INLINE void ScalarMulAdd3Way(const float a1, const float a2,
		const float a3, const float** inp1, const float** inp2,
...
ALWAYS_INLINE void LoadTwoScalars(const bfloat16** data, Packet* l1, Packet* l2)
{
	if (kNumOperands >= 2) {
...
ALWAYS_INLINE void MulAdd3Way(const Packet a1, const Packet a2, const Packet a3,
		const bfloat16** binp1, const bfloat16** binp2, const bfloat16** binp3,
		float** out)
{
...
ALWAYS_INLINE void TwoMulAdd3Way(const Packet a1, const Packet a2,
		const Packet a3, const bfloat16** binp1, const bfloat16** binp2,
...
ALWAYS_INLINE void MulAdd3Way128(const Packet a1, const Packet a2,
		const Packet a3, const bfloat16** inp1, const bfloat16** inp2,
		const bfloat16** inp3, float** out)
{
...
// Vectorized version of ScalarMulAdd3Way
ALWAYS_INLINE void MulAdd3Way(const Packet a1, const Packet a2, const Packet a3,
		const float** inp1, const float** inp2, const float** inp3, float** out)
{
	auto c = LOAD(*out)
	;
	const auto b1 = LOAD(*inp1)
...
// Unroll MulAdd3Way for two iterations
ALWAYS_INLINE void TwoMulAdd3Way(const Packet a1, const Packet a2,
		const Packet a3, const float** inp1, const float** inp2,
		const float** inp3, float** out)
{
	auto c1 = LOAD(*out)
	;
...
inline void GEPP(const std::vector<SparseSlice<TL>*>& left_slices,
		const Eigen::TensorMap<Eigen::Tensor<const TR, 2, Eigen::RowMajor>,
				Eigen::Aligned>& right, const int num_cols, Matrix* output)
{
			...
			for (; j + 1 < end3; j += 2) {
				...
				if (cols == 128) {
					...
					for (int k = 0; k < cols_mod; ++k) {
						...
						ScalarMulAdd3Way(nsl1, nsl2, nsl3, &nr1, &nr2, &nr3,
								&nout);
					}
				}
			}
...
public:
	static inline void Compute(TensorInfoCache* cache,
			const ConstMatrixMapL& left, const ConstMatrixMapR& right,
			bool transpose_left,
...
private:
	...
	static inline void ComputeOutputBlock(
			...
			const ConstMatrixMapR& right, int num_cols, int output_row_offset,
			int output_col_offset, bool assign, bool transpose_output,
			MatrixMap* output);
	...
	static inline BlockingCounter* CreateDenseSlices(const ConstMatrixMapR& mat,
			int row_start, int num_rows, int col_start, int num_cols,
	...
	// complete.
	static inline BlockingCounter* ShuffleMatrix(const ConstMatrixMapR& mat,
	...
	static inline void SliceMatrix(const MatrixR& mat, const int num_rows,
			const int num_slices, std::vector<ConstMatrixMapR*>* slices);
	...
	static inline void ComputeBlockSizes(const ConstMatrixMapL& left,
			const ConstMatrixMapR& right, bool transpose_left, int num_threads,
			int* KR, int* NR, int* KL, int* JB, int* IB);
...
public:
	explicit SparseMatMulOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(0, TensorShape( { m, n }), &output));
		...
		if (!a_is_sparse_ && !b_is_sparse_) {
			...
			out.device(ctx->template eigen_device<CPUDevice>()) = left->matrix<
					float>().contract(right->matrix<float>(), dim_pair);
			return;
		}
		...
		if (transpose_b) {
			...
			right_tr.reset(
					new Tensor(right->dtype(),
							TensorShape(
									{ right->dim_size(1), right->dim_size(0) })));

			Eigen::array<int, 2> perm( { 1, 0 });
			if (transpose_output) {
				right_tr->matrix<TL>().device(
						ctx->template eigen_device<CPUDevice>()) =
						right->matrix<TL>().shuffle(perm);
			} else {
				right_tr->matrix<TR>().device(
						ctx->template eigen_device<CPUDevice>()) =
		...
		bool transpose_output, MatrixMap* output)
{
	...
	if (!assign) {
		const Eigen::array<int, 2> begin = { output_row_offset,
				output_col_offset };
		...
		if (transpose_output) {
			if (num_cols == rhs_num_cols) {
				...
				output->shuffle(perm).slice(begin, sizes) += out.slice(zero,
						sizes);
			}
		} else {
			if (num_cols == rhs_num_cols) {
...
inline BlockingCounter* SparseMatMul<TL, TR>::CreateSparseSlices(
		const typename SparseMatMul<TL, TR>::ConstMatrixMapL& mat,
		bool transpose, int slice_num_rows, int slice_block_size,
		int slice_num_cols,
		std::vector<std::vector<SparseSlice<TL>*>>* mat_slices,
		const DeviceBase::CpuWorkerThreads* thread_pool)
{
	...
	const int num_slices_dim0 = std::max(1,
			(mat_num_rows + slice_num_rows - 1) / slice_num_rows);
	const int num_slices_dim1 = std::max(1,
			(mat_num_cols + slice_num_cols - 1) / slice_num_cols);
	mat_slices->resize(num_slices_dim0);
	BlockingCounter* counter = new BlockingCounter(
			num_slices_dim0 * num_slices_dim1);
	...
	for (int i = 0; i < num_slices_dim0; ++i) {
		...
		int num_rows = std::min<int>(slice_num_rows,
				mat_num_rows - i * slice_num_rows);
		for (int j = 0; j < num_slices_dim1; ++j) {
			int num_cols = std::min<int>(slice_num_cols,
					mat_num_cols - j * slice_num_cols);
			...
			auto* sparse_slice = new SparseSlice<TL>(num_rows, num_cols,
					slice_block_size);
...
ALWAYS_INLINE void CopyAndMayBeInterleaveBfloat16(void* bdst, const void* bsrc,
  ...
  for (int index = 0; index + kStep <= num; index += kStep) {
    auto in = LOAD(src);auto tmp = INTERLEAVE(in);STORE(dst, tmp);
src += kNumOperands;
dst += kNumOperands;
}
if (num % kStep != 0) {
...
template<typename TL, typename TR>
inline void SparseMatMul<TL, TR>::SliceMatrix(const MatrixR& mat,
const int num_rows, const int num_slices,
...
std::vector<typename SparseMatMul<TL, TR>::ConstMatrixMapR*>* slices)
{
BlockingCounter* shuffle_counter = ShuffleMatrix(mat, row_start, num_rows,
	col_start, num_cols, N, thread_pool, buffer);
...
if (*KR % *KL == 0
		&& std::max<int>(1, left_dim0 / 64) * (left_dim1 / *KL)
				> est_num_cores) {
	break;
}
}
...
const int num_threads = thread_pool->num_threads;
int KR, NR, KL, JB, IB;
ComputeBlockSizes(left, right, transpose_left, num_threads, &KR, &NR, &KL, &JB,
	&IB);
...
const int buffer_num_rows = std::min(KR, right_dim0)
	* (std::min(NR, right_dim1) + N - 1) / N;
...
const int right_num_cols = std::min(NR, static_cast<int>(right_dim1 - NR * nb));
for (int kb = 0; kb < num_k_blocks; ++kb) {
	const int right_num_rows = std::min(KR,
			static_cast<int>(right_dim0 - KR * kb));
	dense_slice_counter.reset(
			CreateDenseSlices(right, kb * KR, right_num_rows, nb * NR,
					right_num_cols, thread_pool, &buffer, &right_slices));
	...
	for (int j_outer = 0; j_outer < num_right_slices; j_outer += JB) {
		for (int i_outer = 0; i_outer < num_left_slices; i_outer += IB) {
			for (int j_inner = j_outer;
					j_inner < std::min(num_right_slices, j_outer + JB);
					++j_inner) {
				const int num_cols = std::min(N, right_num_cols - N * j_inner);
				for (int i_inner = i_outer;
						i_inner < std::min(num_left_slices, i_outer + IB);
						++i_inner) {
					...
					tasks.push_back(
							std::bind(&ComputeOutputBlock, block_left_slices,
									std::ref(*right_slices[j_inner]), num_cols,
									M * i_inner, N * j_inner + nb * NR, kb == 0,
