...
protected:
  ...
  //  OnesVec  GTE(0) GTE(1) GTE(2)
	...
	HloComputation* BuildComputation0()
	{
		...
		auto out0 = builder.AddInstruction(
		...
		auto add1 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd, gte1,
						gte2));
		...
		auto add2 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd, add1,
						one_vec));
		auto out1 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kMultiply,
						add2, one_vec));

		// Second sub-computation: out2 = Add(Mul(add1, one_vec), one_vec)
		auto mul0 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kMultiply,
						add1, one_vec));
		auto out2 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd, mul0,
						one_vec));

		// Create output Tuple.
		builder.AddInstruction(
				HloInstruction::CreateTuple( { out0, out1, out2 }));
  ...
  //            GTE(0)   GTE(1)
	...
	HloComputation* BuildComputation1()
	{
		...
		Shape tuple_shape2_ = ShapeUtil::MakeTupleShape( { data_shape_,
				data_shape_ });
		// Create param instruction to access computation state.
		...
		for (int i = 0; i < iters; ++i) {
			mul0 = builder.AddInstruction(
					HloInstruction::CreateBinary(data_shape_,
							HloOpcode::kMultiply, gte0, mul0));
		}
		...
		auto mul1 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kMultiply,
						mul0, one_vec));
		auto out1 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd, mul1,
						one_vec));
  ...
  //      GTE(0) GTE(1) GTE(2) GTE(3)
	...
	HloComputation* BuildComputation2(bool add_extra_input)
	{
		...
		auto reduce0 = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd, gte0,
						gte1));
		auto reduce_out = builder.AddInstruction(
				HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd,
						reduce0, gte2));
		if (add_extra_input) {
			auto gte3 = builder.AddInstruction(
					HloInstruction::CreateGetTupleElement(data_shape_, state,
							3));
			reduce_out = builder.AddInstruction(
					HloInstruction::CreateBinary(data_shape_, HloOpcode::kAdd,
							reduce_out, gte3));
		}
	...
	Shape tuple_shape2_ = ShapeUtil::MakeTupleShape(
			{ data_shape_, data_shape_ });
	Shape tuple_shape3_ = ShapeUtil::MakeTupleShape( { data_shape_, data_shape_,
			data_shape_ });
	Shape tuple_shape4_ = ShapeUtil::MakeTupleShape( { data_shape_, data_shape_,
			data_shape_, data_shape_ });
