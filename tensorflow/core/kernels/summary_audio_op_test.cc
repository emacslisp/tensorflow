...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "AudioSummaryV2").Input(FakeInput()).Input(
						FakeInput()).Input(FakeInput()).Attr("max_outputs",
						max_outputs).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
	...
	void CheckAndRemoveEncodedAudio(Summary* summary)
	{
		for (int i = 0; i < summary->value_size(); ++i) {
			...
			ASSERT_TRUE(value->has_audio()) << "No audio for value: "
					<< value->tag();
			...
			if (VLOG_IS_ON(2)) {
				...
				TF_CHECK_OK(
						WriteStringToFile(Env::Default(),
								strings::StrCat("/tmp/", value->tag(), ".wav"),
