...
public:
	...
	typedef std::function<void(const Tensor&, const Tensor&, const Tuple&)> IndicesKeysValuesCallback;

	Barrier(const DataTypeVector& value_component_types,
			const std::vector<TensorShape>& value_component_shapes,
			const string& name) :
			closed_(false), queue_closed_(false), queue_cancelled_(false), cancel_pending_enqueues_(
					false), value_component_types_(value_component_types), value_component_shapes_(
					value_component_shapes), name_(name), input_index_(
					std::numeric_limits<int64>::min())
	{
		...
		queue_component_shapes.insert(queue_component_shapes.end(),
				value_component_shapes.begin(), value_component_shapes.end());

		ready_queue_ = new PriorityQueue(QueueBase::kUnbounded /* capacity */,
				queue_component_types, queue_component_shapes,
				strings::StrCat(name_, "_queue"));
	}
	...
	void TryInsertMany(const Tensor& keys, int component_index,
			const Tensor& values, OpKernelContext* ctx, DoneCallback callback)
	{
		TensorShape element_shape = values.shape();
		OP_REQUIRES_ASYNC(ctx,
				keys.NumElements() == 0 || element_shape.num_elements() > 0,
				errors::InvalidArgument(
						"Tensors with no elements are not supported ", name_,
						": received shape ", element_shape.DebugString()),
				callback);
		if (element_shape.dims() > 0)
...

		{
			...
			if (closed_) {
				OP_REQUIRES_ASYNC(ctx,
						...
						errors::Cancelled("Barrier ", name_,
								" is closed.  Pending enqueues cancelled: ",
								cancel_pending_enqueues_,
								".  Number of new insertions: ", num_inserted,
								".  Number of incomplete keys: ",
								incomplete_.size(), "."), callback);
			}
			...
			for (int i = 0; i < num_inserted; ++i) {
				OP_REQUIRES_OK_ASYNC(ctx,
						InsertOneLocked < T
								> (ctx, keys, values, element_shape, component_index, i, &ready_tuples, &new_elements),
						callback);
			}
			...
			OP_REQUIRES_ASYNC(ctx,
					input_index_ != std::numeric_limits<int64>::max(),
					errors::Internal("Barrier has had ", input_index_,
			...
			for (int i = 0; i < 2 + num_components(); ++i) {
				...
				for (int b = 0; b < insertion_size; ++b) {
					OP_REQUIRES_OK_ASYNC(ctx,
							QueueBase::CopyElementToSlice(ready_tuples[b][i],
		...
		ready_queue_->TryEnqueueMany(insert_tuple, ctx,
			...
			OpKernelContext* ctx, IndicesKeysValuesCallback callback)
	{
		int num_elements_to_deliver = num_elements;
		{
			...
			if (closed_) {
				...
				if (allow_small_batch) {
					...
					num_elements_to_deliver = std::min(num_elements,
							available_elements);
				} else {
				...
				if (available_elements < std::max(num_elements_to_deliver, 1)) {
					ctx->SetStatus(
							errors::OutOfRange("Barrier '", name_,
									"' is closed and has ",
									"insufficient elements (requested ",
									num_elements_to_deliver, ", total size ",
									available_elements, ")"));
		...
		ready_queue_->TryDequeueMany(num_elements_to_deliver, ctx,
				allow_small_batch, [this, ctx, callback](const Tuple& t) {
	...
	~Barrier() override EXCLUSIVE_LOCKS_REQUIRED(mu_) {
		mutex_lock lock(mu_);incomplete_.clear();ready_queue_->Unref();
	}

	string DebugString()
	override {return "A barrier";}
...
private:
	...
	private:
		...
		EXCLUSIVE_LOCKS_REQUIRED(mu_) {
			*barrier = new Barrier(value_component_types_, value_component_shapes_,
					cinfo_.name());if (*barrier == nullptr) {
	...
	public:
		...
		void ComputeAsync(OpKernelContext* ctx, DoneCallback callback) final
		{
			...
			OP_REQUIRES_OK_ASYNC(ctx,
					GetResourceFromContext(ctx, "handle", &barrier), callback);
	...
	protected:
		...
		{
			OP_REQUIRES_ASYNC(ctx, component_index_ < barrier->num_components(),
					errors::InvalidArgument("The component ID is out of range ",
							component_index_, " > num_components", " (= ",
							barrier->num_components(), ")"), callback);
			OP_REQUIRES_OK_ASYNC(ctx,
					ctx->MatchSignature(
							{ DT_STRING_REF, DT_STRING, barrier->component_type(
									component_index_) }, { }), callback);

			const Tensor* keys;
			const Tensor* values;
			OP_REQUIRES_OK_ASYNC(ctx, ctx->input("keys", &keys), callback);
			OP_REQUIRES_OK_ASYNC(ctx, ctx->input("values", &values), callback);
			barrier->TryInsertMany < T
					> (*keys, component_index_, *values, ctx, callback);
		}
...
#undef REGISTER_INSERTMANY
	...
	protected:
				...
				DoneCallback callback) override
		{
			...
			OP_REQUIRES_OK_ASYNC(ctx,
					ctx->input("num_elements", &Tnum_elements), callback);
			OP_REQUIRES_ASYNC(ctx,
					TensorShapeUtils::IsScalar(Tnum_elements->shape()),
			...
			barrier->TryTakeMany(num_elements, allow_small_batch_, timeout_,
					ctx,
	...
	public:
		...
		{
			OP_REQUIRES_OK(context,
					context->GetAttr("cancel_pending_enqueues",
							&cancel_pending_enqueues_));
		}
	...
	protected:
				...
				DoneCallback callback) override
		{
			...
			OP_REQUIRES_OK_ASYNC(ctx,
					ctx->allocate_output(0, TensorShape( { }), &Tsize),
					callback);
