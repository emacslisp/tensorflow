			...
			for (const auto& x : slices_) {
				if (slice.Overlaps(x.second.slice)) {
					return errors::Internal(
							"Overlapping slices: existing slice = ", x.first,
							", new slice = ", str);
				}
			}
		}
...
bool TensorSliceSet::Query(const TensorSlice& slice, float* data) const
{
	...
	if (info) {
		...
		if (total_size == overlap_size) {
			...
			if (data) {
				for (const auto& x : slices_) {
					CopyDataFromTensorSliceToTensorSlice(shape_, x.second.slice,
							slice, x.second.data, data);
				}
			}
			return true;
		} else {
...
bool TensorSliceSet::QueryMeta(const TensorSlice& slice,
		...
		for (const auto& x : slices_) {
			if (slice.Intersect(x.second.slice, &intersection)) {
				...
				results->emplace_back(
						std::make_pair(x.second.slice, x.second.tag));
			}
		}
		if (total_size == overlap_size) {
			// We have it!
...
Status RegisterTensorSlice(const string& name, const TensorShape& shape,
		DataType type, const string& tag, const TensorSlice& slice,
		std::unordered_map<string, TensorSliceSet*>* tensor_slices)
{
	...
	if (!tss) {
		...
		if (!shape.IsSameSize(tss_shape)) {
			return errors::Internal(
					"Incompatible tensor shapes detected for tensor ", name,
					": existing = ", tss_shape.DebugString(), ", new = ",
					shape.DebugString());
		}
		if (type != tss->type()) {
			return errors::Internal(
					"Incompatible tensor types detected for tensor ", name,
					": existing = ", DataTypeString(tss->type()), ", new = ",
					DataTypeString(type));
		}
	}
