...
struct TensorZeroPadding {
	void operator()(const Device& d, const int64 time_idx,
			...
			typename TTypes<float>::Vec mask, typename TTypes<float>::Matrix m)
	{
		...
		Eigen::array < Eigen::DenseIndex, 2 > broadcast_shape( { 1,
				m.dimensions()[1] });
...
struct LSTMBlockCell {
	LSTMBlockCell(const int batch_size, const int input_size,
			const int cell_size) :
			batch_size_(batch_size), input_size_(input_size), cell_size_(
					cell_size)
	{
	}
...
struct LSTMBlockCellFprop: public LSTMBlockCell {
	...
	void operator()(OpKernelContext* ctx, const Device& d, const T forget_bias,
			const T cell_clip, bool use_peephole,
			...
			typename TTypes<T>::Matrix icfo, typename TTypes<T>::Matrix h)
	{
		...
		TensorBlasGemm<Device, T, USE_CUBLAS>::compute(ctx, d, false, false,
				T(1), const_xh, w, T(0), icfo);
		Eigen::array < Eigen::DenseIndex, 2 > b_shape( { 1, b.dimensions()[0] });
		Eigen::array < Eigen::DenseIndex, 2
				> broadcast_shape( { batch_size_, 1 });
		icfo.device(d) += b.reshape(b_shape).broadcast(broadcast_shape);

		Eigen::array < Eigen::DenseIndex, 2 > p_shape( { 1, cell_size_ });
		Eigen::array < Eigen::DenseIndex, 2 > p_broadcast_shape( { batch_size_,
				1 });

		// Input gate.
		if (use_peephole) {
			auto i_peep = cs_prev
					* wci.reshape(p_shape).broadcast(p_broadcast_shape);
			i.device(d) =
					(icfo.slice(icfo_i_offsets(), cell_extents()) + i_peep).sigmoid();
		} else {
			i.device(d) =
					icfo.slice(icfo_i_offsets(), cell_extents()).sigmoid();
		}
		...
		if (use_peephole) {
			auto f_peep = cs_prev
					* wcf.reshape(p_shape).broadcast(p_broadcast_shape);
			f.device(d) = (icfo.slice(icfo_f_offsets(), cell_extents())
					+ f.constant(forget_bias) + f_peep).sigmoid();
		} else {
			f.device(d) = (icfo.slice(icfo_f_offsets(), cell_extents())
					+ f.constant(forget_bias)).sigmoid();
		}
		...
		if (cell_clip > 0.0f) {
			cs.device(d) = cs.binaryExpr(cs.constant(cell_clip),
					Eigen::scalar_clip_op<T>());
		}
		...
		if (use_peephole) {
			auto o_peep = cs
					* wco.reshape(p_shape).broadcast(p_broadcast_shape);
			o.device(d) =
					(icfo.slice(icfo_o_offsets(), cell_extents()) + o_peep).sigmoid();
		} else {
			o.device(d) =
					icfo.slice(icfo_o_offsets(), cell_extents()).sigmoid();
		}
...
struct LSTMBlockCellBprop: public LSTMBlockCell {
	...
	void operator()(OpKernelContext* ctx, const Device& d, bool use_peephole,
			typename TTypes<T>::ConstMatrix x,
			typename TTypes<T>::ConstMatrix cs_prev,
			typename TTypes<T>::ConstMatrix h_prev,
			typename TTypes<T>::ConstMatrix w, typename TTypes<T>::ConstVec wci,
			...
			typename TTypes<T>::ConstMatrix cs,
			typename TTypes<T>::ConstMatrix f,
			typename TTypes<T>::ConstMatrix o,
			typename TTypes<T>::ConstMatrix ci,
			...
			typename TTypes<T>::Vec wco_grad)
	{
		...
		if (use_peephole) {
			dcs.device(d) = dcs
					+ do_ * wco.reshape(p_shape).broadcast(p_broadcast_shape);
		}
		...
		if (use_peephole) {
			cs_prev_grad.device(d) = cs_prev_grad
		...
		if (use_peephole) {
			wci_grad.device(d) = (di * cs_prev).sum(
					Eigen::array<int, 1>( { 0 }));
			wcf_grad.device(d) = (df * cs_prev).sum(
					Eigen::array<int, 1>( { 0 }));
...
struct BlockLSTMBprop: public LSTMBlockCell {
	...
	void operator()(OpKernelContext* ctx, const Device& d, bool use_peephole,
			...
			typename TTypes<T>::ConstVec wcf, typename TTypes<T>::ConstVec wco,
			typename TTypes<T>::ConstVec b, typename TTypes<T>::Matrix xh,
			typename TTypes<T>::ConstMatrix i,
			typename TTypes<T>::ConstMatrix cs,
			typename TTypes<T>::ConstMatrix f,
			typename TTypes<T>::ConstMatrix o,
			typename TTypes<T>::ConstMatrix ci,
			...
			typename TTypes<T>::Vec b_grad)
	{
		...
		typename TTypes<T>::ConstMatrix const_dicfo(dicfo.data(),
				dicfo.dimensions());
		TensorBlasGemm<Device, T, USE_CUBLAS>::compute(ctx, d, false, true,
				T(1), const_dicfo, w, T(0), xh_grad);
		...
		// w_grad.
		TensorBlasGemm<Device, T, USE_CUBLAS>::compute(ctx, d, true, false,
				T(1), const_xh, const_dicfo, T(1), w_grad);
