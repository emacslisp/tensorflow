...
IrEmitter::IrEmitter(const HloModuleConfig& hlo_module_config,
		...
		ir_emitter_context_(ir_emitter_context), ir_builder_(
				ir_emitter_context->llvm_module()->getContext()), bindings_(
				ir_emitter_context->hlo_module(),
				&ir_emitter_context->buffer_assignment(),
				&ir_emitter_context->temp_buffer_offsets(), &ir_builder_,
				is_nested), hlo_module_config_(hlo_module_config)
{
...
Status IrEmitter::DefaultAction(HloInstruction* hlo)
{
	...
	return EmitTargetElementLoop(*hlo,
			GpuElementalIrEmitter(hlo_module_config_,
					...
					GetNestedComputer()).MakeElementGenerator(hlo,
...
Status IrEmitter::HandleBitcast(HloInstruction* bitcast)
{
	...
	if (bindings_.BoundToIrValue(*operand)) {
		bindings_.BindHloToIrValue(*bitcast,
				bindings_.GetBasePointer(*operand));
	}
	return Status::OK();
}
		...
		HloInstruction* operand)
{
	...
	bindings_.BindHloToIrValue(*get_tuple_element,
			llvm_ir::EmitGetTupleElement(get_tuple_element->shape(),
		...
		tensorflow::gtl::ArraySlice<llvm::Value*> operands, llvm::Value* output)
{
	...
	if (emitted_function == nullptr) {
		IrEmitterNested ir_emitter_nested(hlo_module_config_,
				nested_computation, ir_emitter_context_);
		TF_RETURN_IF_ERROR(
				nested_computation.root_instruction()->Accept(
						&ir_emitter_nested));
		...
		llvm::Value* source_address)
{
	...
	if (root_opcode == HloOpcode::kAdd) {
		...
		if (element_type == F32) {
			...
			llvm_ir::EmitCallToIntrinsic(
					llvm::Intrinsic::nvvm_atomic_load_add_f32, { output_address,
							source }, { output_address->getType() },
					&ir_builder_);
			return true;
		}
		if (primitive_util::IsIntegralType(element_type)) {
			// integral + integral
			ir_builder_.CreateAtomicRMW(llvm::AtomicRMWInst::Add,
	...
	if (root_opcode == HloOpcode::kMaximum
		...
		ir_builder_.CreateAtomicRMW(llvm::AtomicRMWInst::Max, output_address,
				source, llvm::AtomicOrdering::SequentiallyConsistent);
		return true;
	}
		...
		llvm::Value* source_address)
{
	if (computation.num_parameters() != 2) {
		...
		return Unimplemented(
				"We only support atomic functions with exactly two parameters, but "
						"computation %s has %lld.", computation.name().c_str(),
	...
	// old_output = *output_address;
	llvm::Value* old_output_location = ir_builder_.CreateAlloca(element_ir_type, /*ArraySize=*/
			nullptr, "old_output_location");
	ir_builder_.CreateStore(
			ir_builder_.CreateLoad(output_address, "old_output"),
			old_output_location);
	...
	llvm::BasicBlock* loop_body_bb = llvm::BasicBlock::Create(
			ir_builder_.getContext(), "atomic_op_loop_body",
	...
	// new_output = computation(old_output, source);
	llvm::Value* new_output_location = ir_builder_.CreateAlloca(element_ir_type, /*ArraySize=*/
			nullptr, "new_output_location");
	TF_RETURN_IF_ERROR(EmitCallToNestedComputation(computation, {
			old_output_location, source_address }, new_output_location));

	// (old_output, success) = atomicCAS(output_address, old_output, new_output);
	llvm::Type* element_int_ir_type = ir_builder_.getIntNTy(
			element_ir_type->getScalarSizeInBits());
	...
	llvm::Value* old_output = ir_builder_.CreateLoad(old_output_location,
			"old_output");
	llvm::Value* new_output = ir_builder_.CreateLoad(new_output_location,
			"new_output");
	...
	ir_builder_.CreateStore(
			ir_builder_.CreateBitCast(
					...
					element_ir_type), old_output_location);
	ir_builder_.CreateCondBr(
			ir_builder_.CreateExtractValue(ret_value, 1, "success"),
		...
		HloInstruction* lhs_instruction, HloInstruction* rhs_instruction)
{
	...
	if (ShapeUtil::IsScalar(lhs_shape) && ShapeUtil::IsScalar(rhs_shape)) {
		...
		llvm::Value* lhs_value = lhs_array.EmitReadArrayElement(/*index=*/{ },
				&ir_builder_);
		llvm::Value* rhs_value = rhs_array.EmitReadArrayElement(/*index=*/{ },
				&ir_builder_);
	...
	TF_RET_CHECK(
			!ShapeUtil::IsScalar(lhs_shape) && !ShapeUtil::IsScalar(rhs_shape));
	...
	// scalar.
	const int64 lhs_reduction_dimension = ShapeUtil::GetDimensionNumber(
			lhs_shape, -1);
	const int64 rhs_reduction_dimension =
			ShapeUtil::Rank(rhs_shape) >= 2 ?
					ShapeUtil::GetDimensionNumber(rhs_shape, -2) : 0;
	...
	llvm_ir::IrArray::Index lhs_index = EmitOperandArrayLoopNest(lhs_array,
			lhs_reduction_dimension, "lhs", &loop_nest);
	llvm_ir::IrArray::Index rhs_index = EmitOperandArrayLoopNest(rhs_array,
			rhs_reduction_dimension, "rhs", &loop_nest);
	...
	llvm::Value* accum_address = llvm_ir::EmitAllocaAtFunctionEntry(accum_type, // The pointee type of the alloca instruction.
			"accum_address",  // The name of the alloca instuction.
			&ir_builder_);

	// Initialize the accumulator in the preheader to zero.
	new llvm::StoreInst(llvm::ConstantFP::get(accum_type, 0.0), // The value stored.
	accum_address,                           // The address.
			reduction_loop->GetPreheaderBasicBlock()->getTerminator()); // The instruction this store is inserted before.
	...
	llvm::Value* lhs_element = lhs_array.EmitReadArrayElement(lhs_index,
			&ir_builder_);
	llvm::Value* rhs_element = rhs_array.EmitReadArrayElement(rhs_index,
			&ir_builder_);
	...
	target_array.EmitWriteArrayElement(target_index,
			ir_builder_.CreateLoad(accum_address), // The value written to the target array.
			&ir_builder_);
		...
		RandomDistribution /*distribution*/)
{
	...
	return llvm_ir::LoopEmitter(
			GpuElementalIrEmitter(hlo_module_config_,
					ir_emitter_context_->llvm_module(), &ir_builder_,
					GetNestedComputer()).MakeElementGenerator(random,
					operand_to_generator), GetIrArray(*random), &ir_builder_).EmitLoop();
}
		...
		tensorflow::StringPiece name_suffix, llvm_ir::ForLoopNest* loop_nest)
{
	...
	// output.
	llvm_ir::IrArray::Index index = loop_nest->AddLoopsForShapeOnDimensions(
			shape, dimensions, name_suffix);
		...
		tensorflow::gtl::ArraySlice<llvm::Value*> parameter_elements)
{
	llvm::Value* return_buffer = llvm_ir::EmitAllocaAtFunctionEntry(
			llvm_ir::PrimitiveTypeToIrType(
					computation.root_instruction()->shape().element_type(),
					&ir_builder_), "return_buffer", &ir_builder_);
	std::vector<llvm::Value*> parameter_buffers;
	for (llvm::Value* parameter_element : parameter_elements) {
		parameter_buffers.push_back(
				llvm_ir::EmitAllocaAtFunctionEntry(parameter_element->getType(),
						"parameter_buffer", &ir_builder_));
		ir_builder_.CreateStore(parameter_element, parameter_buffers.back());
	}
	TF_RETURN_IF_ERROR(
			EmitCallToNestedComputation(computation, parameter_buffers,
					return_buffer));
