...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("rgb_to_hsv_op", "RGBToHSV").Input(
						FakeInput(data_type)).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("hsv_to_rgb_op", "HSVToRGB").Input(
						FakeInput(data_type)).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
	...
	void CheckRedMax(DataType data_type)
	{
		...
		AddInputFromArray<T>(TensorShape( { 3 }), { expected_h, expected_s,
				expected_v });
		TF_ASSERT_OK(RunOpKernel());
