...
public:
	...
	virtual std::unique_ptr<ProfileHandler> GetProfileHandler(uint64 step,
			int64 execution_count, const RunOptions& ropts) = 0;
...
typedef std::function<
		std::unique_ptr<StatsPublisherInterface>(const string&,
				const BuildGraphOptions&, const SessionOptions&)> StatsPublisherFactory;
