...
GraphNode* TFGraph::CreateParentNode(const string& name)
{
	...
	parent_nodes_[name] = std::unique_ptr<TFNode>(
			new TFNode(node_defs_.back().get()));
	nodes_map_[name] = std::unique_ptr<GraphNode>(
			new GraphNode(parent_nodes_[name].get()));
...
std::vector<GraphNode*> TFGraph::SearchRoot(
		...
		const std::vector<string>& regexes, std::set<string>* visited)
{
	...
	for (GraphNode* root : roots) {
		if (visited->find(root->name()) != visited->end())
			continue;
		...
		std::vector<GraphNode*> nroot = SearchRoot(root->children, regexes,
				visited);
...
std::vector<GraphNode*> TFGraph::PrintGraph(const std::vector<GraphNode*> roots,
		const Options& opts, int depth, int hidden, int last_ident,
		std::set<string>* visits)
{
	std::vector<GraphNode*> show_nodes;

	for (GraphNode* node : roots) {
		if (visits->find(node->name()) != visits->end())
			continue;
		...
		if (show) {
			...
			nhidden = 0;
			nlast_ident = (
					hidden && opts.select.find(kShown[4]) != opts.select.end() ?
							last_ident + 4 : last_ident + 2);
		} else {
			++nhidden;
		}
		...
		if (show) {
			...
			if (hidden && opts.select.find(kShown[4]) != opts.select.end()) {
				node->formatted_str = strings::Printf("%s...hidden %d...\n",
						string(last_ident, ' ').c_str(), hidden);
				node->formatted_str += strings::Printf("  %s%s\n",
