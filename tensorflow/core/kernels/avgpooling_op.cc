...
public:
	explicit AvgPoolingOp(OpKernelConstruction* context) :
			UnaryOp<T>(context)
	{
		string data_format;
		...
		OP_REQUIRES(context, data_format_ == FORMAT_NHWC,
				errors::InvalidArgument(
						"Default AvgPoolingOp only supports NHWC."));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const int64 shard_cost = window_rows * window_cols * depth_window
				* in_rows * in_rows * in_cols;
