...
private:
	...
	{
		static auto singleton = new std::map<CUcontext,
...
VLOG(3) << "ScopedActivateContext switching context from " << tls->id << " to "
		<< cuda_context->id();
...
return CUDADriver::CanEnablePeerAccess(from_context.ValueOrDie(),
		to_context.ValueOrDie()) ? "true" : "false";
}
...
return port::Status { port::error::ABORTED, port::StrCat(
		"failed call to cuInit: ", ToString(res)) };
}
...
return port::Status { port::error::INTERNAL, port::StrCat(
		"failed call to cuDeviceGet: ", ToString(res)) };
}

/* static */bool CUDADriver::GetDeviceName(CUdevice device, string *device_name)
{
...
/* static */port::Status CUDADriver::CreateContext(CUdevice device,
	DeviceOptions device_options, CudaContext** context)
{
*context = nullptr;
...
/* static */bool CUDADriver::FuncGetAttribute(CUfunction_attribute attribute,
	CUfunction func, int *attribute_value)
{
...
/* static */port::StatusOr<CUsharedconfig> CUDADriver::ContextGetSharedMemConfig(
	CudaContext* context)
{
...
if (result != CUDA_SUCCESS) {
	...
	LOG(ERROR) << "failed to get CUDA device shared memory config. "
			<< "Context device ID: " << device << ", result: "
			<< ToString(result);
	return port::Status { port::error::INTERNAL, port::StrCat(
			"failed to get shared memory config: ", ToString(result)) };
}
...
if (result != CUDA_SUCCESS) {
	...
	LOG(ERROR) << "failed to set CUDA device shared memory config. "
			...
			<< shared_mem_config << ", result: " << ToString(result);
	return port::Status { port::error::INTERNAL, port::StrCat(
			"failed to set shared memory config: ", ToString(result)) };
}
return port::Status::OK();
}

/* static */bool CUDADriver::LaunchKernel(CudaContext* context,
...
VLOG(2) << "launching kernel: " << function << "; gdx: " << grid_dim_x
		<< " gdy: " << grid_dim_y << " gdz: " << grid_dim_z << " bdx: "
		<< block_dim_x << " bdy: " << block_dim_y << " bdz: " << block_dim_z;
CUresult res = dynload::cuLaunchKernel(function, grid_dim_x, grid_dim_y,
		grid_dim_z, block_dim_x, block_dim_y, block_dim_z, shared_mem_bytes,
		stream, kernel_params, extra);
if (res != CUDA_SUCCESS) {
	LOG(ERROR) << "failed to launch CUDA kernel: " << function << "; result: "
			<< ToString(res);
	return false;
}
...
/* static */port::Status CUDADriver::LoadCubin(CudaContext* context,
	const char *cubin_bytes, CUmodule *module)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::LoadPtx(CudaContext* context,
	const char *ptx_contents, CUmodule *module)
{
...
/* static */bool CUDADriver::SynchronousMemsetUint8(CudaContext* context,
	CUdeviceptr location, uint8 value, size_t size)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::SynchronousMemsetUint32(CudaContext* context,
	CUdeviceptr location, uint32 value, size_t uint32_count)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::AsynchronousMemsetUint8(CudaContext* context,
	CUdeviceptr location, uint8 value, size_t uint32_count, CUstream stream)
{
ScopedActivateContext activation { context };
CUresult res = dynload::cuMemsetD8Async(location, value, uint32_count, stream);
if (res != CUDA_SUCCESS) {
...
/* static */bool CUDADriver::AsynchronousMemsetUint32(CudaContext* context,
	CUdeviceptr location, uint32 value, size_t uint32_count, CUstream stream)
{
ScopedActivateContext activation { context };
CUresult res = dynload::cuMemsetD32Async(location, value, uint32_count, stream);
if (res != CUDA_SUCCESS) {
	LOG(ERROR) << "failed to enqueue async memset operation: " << ToString(res);
	return false;
}
...
/* static */bool CUDADriver::AddStreamCallback(CudaContext* context,
	CUstream stream, StreamCallback callback, void *data)
{
// Note: flags param is required to be zero according to CUDA 6.0.
CUresult res = dynload::cuStreamAddCallback(stream, callback, data,
		0 /* = flags */);
if (res != CUDA_SUCCESS) {
...
/* static */bool CUDADriver::GetModuleFunction(CudaContext *context,
	CUmodule module, const char *kernel_name, CUfunction *function)
{
...
/* static */bool CUDADriver::GetModuleSymbol(CudaContext* context,
	CUmodule module, const char *symbol_name, CUdeviceptr *dptr, size_t *bytes)
{
ScopedActivateContext activated { context };
CHECK(
		module != nullptr && symbol_name != nullptr
				&& (dptr != nullptr || bytes != nullptr));
CUresult res = dynload::cuModuleGetGlobal_v2(dptr, bytes, module, symbol_name);
if (res != CUDA_SUCCESS) {
	// symbol may not be found in the current module, but it may reside in
	// another module.
	VLOG(2) << "failed to get symbol \"" << symbol_name << "\" from module: "
			<< ToString(res);
	return false;
}
...
/* static */void CUDADriver::UnloadModule(CudaContext *context, CUmodule module)
{
ScopedActivateContext activated { context };
CUresult res = dynload::cuModuleUnload(module);
if (res != CUDA_SUCCESS) {
	LOG(ERROR) << "failed to unload module " << module << "; leaking: "
			<< ToString(res);
}
}
...
return port::Status { port::error::INTERNAL, port::StrCat(
		"failed to get device for context: ", ToString(result)) };
}

/* static */bool CUDADriver::CreateStream(CudaContext *context, CUstream *out)
{
...
VLOG(2) << "successfully created stream " << *out << " for context " << context
		<< " on thread";
return true;
}
...
/* static */void *CUDADriver::DeviceAllocate(CudaContext *context, uint64 bytes)
{
ScopedActivateContext activated { context };
...
VLOG(2) << "allocated " << ptr << " for context " << context << " of " << bytes
		<< " bytes";
return ptr;
}
...
if (res != CUDA_SUCCESS) {
	LOG(ERROR) << "failed to free device memory at " << location << "; result: "
			<< ToString(res);
} else {
...
/* static */void *CUDADriver::HostAllocate(CudaContext *context, uint64 bytes)
{
ScopedActivateContext activation { context };
void *host_mem = nullptr;
// "Portable" memory is visible to all CUDA contexts. Safe for our use model.
CUresult res = dynload::cuMemHostAlloc(&host_mem, bytes,
		CU_MEMHOSTALLOC_PORTABLE);
if (res != CUDA_SUCCESS) {
	LOG(ERROR) << "failed to alloc " << bytes << " bytes on host: "
			<< ToString(res);
}
return host_mem;
}
...
ScopedActivateContext activation { context };
// "Portable" memory is visible to all CUDA contexts. Safe for our use model.
CUresult res = dynload::cuMemHostRegister(location, bytes,
...
case CUDA_ERROR_NOT_INITIALIZED:
	return port::Status { port::error::FAILED_PRECONDITION, port::Printf(
			"error destroying CUDA event in context %p: %s", context,
			ToString(res).c_str()) };
default:
	return port::Status { port::error::INTERNAL, port::Printf(
			"error destroying CUDA event in context %p: %s", context,
			ToString(res).c_str()) };
}
}
...
case CUDA_ERROR_NOT_INITIALIZED:
	return port::Status { port::error::FAILED_PRECONDITION, port::Printf(
			"error recording CUDA event on stream %p: %s", stream,
			ToString(res).c_str()) };
default:
	return port::Status { port::error::INVALID_ARGUMENT, port::Printf(
			"error recording CUDA event on stream %p: %s", stream,
			ToString(res).c_str()) };
}
}
...
if (res != CUDA_SUCCESS && res != CUDA_ERROR_NOT_READY) {
	return port::Status { port::error::INTERNAL, port::Printf(
			"failed to query event: %s", ToString(res).c_str()) };
}
...
/* static */bool CUDADriver::GetEventElapsedTime(CudaContext* context,
	float *elapsed_milliseconds, CUevent start, CUevent stop)
{
ScopedActivateContext activated { context };
...
/* static */bool CUDADriver::IsStreamIdle(CudaContext *context, CUstream stream)
{
ScopedActivateContext activated { context };
CHECK(stream != nullptr);
...
/* static */bool CUDADriver::SynchronousMemcpyD2H(CudaContext* context,
	void *host_dst, CUdeviceptr gpu_src, uint64 size)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::SynchronousMemcpyH2D(CudaContext* context,
	CUdeviceptr gpu_dst, const void *host_src, uint64 size)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::SynchronousMemcpyD2D(CudaContext* context,
	CUdeviceptr gpu_dst, CUdeviceptr gpu_src, uint64 size)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::AsynchronousMemcpyD2H(CudaContext* context,
	void *host_dst, CUdeviceptr gpu_src, uint64 size, CUstream stream)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::AsynchronousMemcpyH2D(CudaContext* context,
	CUdeviceptr gpu_dst, const void *host_src, uint64 size, CUstream stream)
{
ScopedActivateContext activation { context };
...
/* static */bool CUDADriver::AsynchronousMemcpyD2D(CudaContext* context,
	CUdeviceptr gpu_dst, CUdeviceptr gpu_src, uint64 size, CUstream stream)
{
ScopedActivateContext activation { context };
CUresult result = dynload::cuMemcpyDtoDAsync_v2(gpu_dst, gpu_src, size, stream);
if (result != CUDA_SUCCESS) {
	LOG(ERROR)
			<< port::Printf(
					...
					CUDAPointersToCanAccessString(gpu_src, gpu_dst).c_str(),
					size, size);
...
/* static */port::Status CUDADriver::CreateEvent(CudaContext* context,
	CUevent *result, EventFlags flags)
{
int cuflags;
switch (flags) {
...
if (res == CUDA_SUCCESS) {
	...
	return port::Status { port::error::FAILED_PRECONDITION, port::StrCat(
			"could not create CUDA event: ", ToString(res)) };
}
}
...
CUresult result = dynload::cuPointerGetAttribute(&context,
		CU_POINTER_ATTRIBUTE_CONTEXT, pointer);
if (result == CUDA_SUCCESS) {
...
return port::Status { port::error::INTERNAL, port::StrCat(
		"failed to query device pointer for context: ", ToString(result)) };
}
...
{
unsigned int value;
CUresult result = dynload::cuPointerGetAttribute(&value,
		CU_POINTER_ATTRIBUTE_MEMORY_TYPE, pointer);
if (result == CUDA_SUCCESS) {
	switch (value) {
	...
	default:
		return port::Status { port::error::INTERNAL, port::StrCat(
				"unknown memory space provided by CUDA API: ", value) };
	}
}
...
if (result == CUDA_SUCCESS) {
	...
	return port::Status { port::error::NOT_FOUND, port::Printf(
			"not a device pointer %p; %s", reinterpret_cast<void *>(dptr),
			ToString(result).c_str()) };
}

return port::Status { port::error::INTERNAL, port::Printf(
		"failed to get pointer into for device pointer %p; %s",
...
/* static */port::Status CUDADriver::GetComputeCapability(int *cc_major,
	int *cc_minor, CUdevice device)
{
*cc_major = 0;
*cc_minor = 0;
CUresult result = dynload::cuDeviceComputeCapability(cc_major, cc_minor,
		device);
if (result == CUDA_SUCCESS) {
	return port::Status::OK();
}
...
return port::Status { port::error::INTERNAL, port::Printf(
		"failed to get compute capability for device: %s; %d",
...
if (result != CUDA_SUCCESS) {
	return port::Status { port::error::NOT_FOUND, port::StrCat(
			"could not retrieve CUDA device attribute (", attribute, "): ",
			ToString(result)) };
}
T converted = value;
return converted;
...
/* static */port::StatusOr<int> CUDADriver::GetMultiprocessorCount(
	CUdevice device)
{
return GetSimpleAttribute<int>(device, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT);
}
...
/* static */port::StatusOr<int64> CUDADriver::GetThreadsPerWarp(CUdevice device)
{
...
{
int value;
CUresult res = dynload::cuDeviceGetAttribute(&value,
...
res = dynload::cuDeviceGetAttribute(&value, CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Y,
		device);
if (res != CUDA_SUCCESS) {
...
res = dynload::cuDeviceGetAttribute(&value, CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Z,
		device);
if (res != CUDA_SUCCESS) {
...
/* static */bool CUDADriver::GetDeviceProperties(CUdevprop *device_properties,
	int device_ordinal)
{
CUresult res = dynload::cuDeviceGetProperties(device_properties,
		device_ordinal);
if (res != CUDA_SUCCESS) {
...
/* static */bool CUDADriver::GetDeviceMemoryInfo(CudaContext* context,
	int64 *free_out, int64 *total_out)
{
ScopedActivateContext activation { context };
size_t free = 0;
size_t total = 0;
...
CUresult res = dynload::cuDeviceGetPCIBusId(chars.begin(), kBufferSize - 1,
		device);
if (res != CUDA_SUCCESS) {
...
CUresult res = dynload::cuDeviceCanAccessPeer(&can_access_peer,
		from_device.ValueOrDie(), to_device.ValueOrDie());
if (res != CUDA_SUCCESS) {
...
CUresult result = dynload::cuCtxEnablePeerAccess(to->context(),
		0 /* = flags */);
if (result != CUDA_SUCCESS
		&& result != CUDA_ERROR_PEER_ACCESS_ALREADY_ENABLED) {
	return port::Status { port::error::INTERNAL, port::Printf(
			"failed to enable peer access from %p to %p: %s", from, to,
			ToString(result).c_str()) };
}
...
if (result != CUDA_SUCCESS) {
	return port::Status { port::error::INTERNAL, port::Printf(
			"failed to calculate occupancy of kernel %p: %s", kernel,
			ToString(result).c_str()) };
}
