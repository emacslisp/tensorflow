...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, out_shape.num_elements() <= 2147483647,
				errors::InvalidArgument(
		...
		if (!custom_function_name.empty()) {
			...
			OP_REQUIRES_OK(ctx,
					TensorShapeToXLAShape(input_type(0), out_shape,
							&xla_out_shape));
			...
			output =
					b.CustomCall(custom_function_name,
							{ input, filter,
									b.ConstantR1 < int64
											> ( {	batch, input_rows, input_cols, in_depth}),
									b.ConstantR1 < int64
											> (
													{	filter_rows, filter_cols, in_depth, depth_multiplier}),
									b.ConstantR1 < int64
											> ( {	batch, out_rows, out_cols, out_depth}),
									b.ConstantR0 < int64 > (stride),
									b.ConstantR0 < int64 > (pad_rows),
			...
			for (int i = 0; i < in_depth; ++i) {
				...
				xla::ComputationDataHandle filter_slice = b.Slice(filter,
						filter_begin, filter_limits);
				xla::ComputationDataHandle input_slice = b.Slice(input,
						input_begin, input_limits);
				convs.push_back(
						b.ConvWithGeneralDimensions(input_slice, filter_slice,
