...
void MakeGRPCCluster(const SessionOptions& options, int n,
		std::vector<string>* workers, std::vector<DeviceAttributes>* devices)
{
	CHECK_GE(n, 1);
	...
	worker_threads = new thread::ThreadPool(Env::Default(), "worker_threads",
			n);
...
{
	BM_Helper(iters, width, 2 /*num_stages*/, tensor_size,
			true /*multi-device*/);
}
