...
bool ReshapeIsBitcast(const HloInstruction* reshape,
...
public:
	...
	Status HandleConvert(HloInstruction* convert, HloInstruction* operand)
			override;
	...
	static bool Run(HloComputation* computation, bool is_layout_sensitive,
...
private:
	explicit AlgebraicSimplifierVisitor(HloComputation* computation,
			...
			AlgebraicSimplifier::ValidBitcastCallback valid_bitcast_callback) :
			computation_(computation), is_layout_sensitive_(
					is_layout_sensitive), valid_bitcast_callback_(
	...
	bool TransformToClampIfSameShape(HloInstruction* root, HloInstruction* min,
			HloInstruction* min_operand, HloInstruction* operand,
			HloInstruction* max, HloInstruction* max_operand);
...
void AlgebraicSimplifierVisitor::ReplaceWithBitcast(HloInstruction* instruction)
{
	...
	auto bitcast = computation_->AddInstruction(
			HloInstruction::CreateUnary(instruction->shape(),
					HloOpcode::kBitcast, instruction->mutable_operand(0)));
		...
		HloInstruction* lhs, HloInstruction* rhs)
{
	...
	if (lhs->opcode() == HloOpcode::kExp && rhs->opcode() == HloOpcode::kExp) {
		VLOG(10) << "transform [exp(A)/exp(B) => exp(A-B)]: "
				<< divide->ToString();
		HloInstruction* subtract = computation_->AddInstruction(
				HloInstruction::CreateBinary(divide->shape(),
						HloOpcode::kSubtract, lhs->mutable_operand(0),
						rhs->mutable_operand(0)));
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(divide,
				HloInstruction::CreateUnary(divide->shape(), HloOpcode::kExp,
						subtract));
	}
		...
		HloInstruction* operand)
{
	...
	if (operand->opcode() == HloOpcode::kExp
			&& ReplaceInstructionIfSameShape(log,
		...
		HloInstruction* get_tuple_element, HloInstruction* operand)
{
	if (operand->opcode() == HloOpcode::kTuple) {
		...
		if (ReplaceInstructionIfSameShape(get_tuple_element,
...
Status AlgebraicSimplifierVisitor::HandleBroadcast(HloInstruction* broadcast)
{
	...
	if (std::is_sorted(broadcast->dimensions().begin(),
		...
		VLOG(10) << "transform broadcast(X) -> reshape(X) where "
				"n(broadcast(X)) == n(X)";
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(broadcast,
				HloInstruction::CreateReshape(broadcast->shape(), operand));
	}
	...
	if (ShapeUtil::IsScalar(operand->shape())) {
		for (HloInstruction* user : broadcast->users()) {
			...
			if (user->user_count() == 0
					&& user != computation_->root_instruction()) {
				continue;
			}
			if (OutputIsPermutationOfOperandElements(user, broadcast)
				...
				// instruction.
				changed_ = true;
				return computation_->ReplaceUsesOfInstruction(user,
						new_broadcast);
			}
		}
	}
		...
		HloInstruction* operand)
{
	...
	if (operand->opcode() == HloOpcode::kConstant) {
		...
		std::unique_ptr<HloInstruction> new_constant = ConvertIfSrcTypeMatches(
				src_literal, dest_type);
		changed_ = true;
...
Status AlgebraicSimplifierVisitor::HandlePad(HloInstruction* pad)
{
	...
	if (has_negative) {
		...
		TF_RETURN_IF_ERROR(
				LayoutUtil::CopyLayoutBetweenShapes(pad->shape(),
						&nonzero_pad_shape));
		HloInstruction* nonzero_pad = computation_->AddInstruction(
				HloInstruction::CreatePad(nonzero_pad_shape,
						pad->mutable_operand(0), pad->mutable_operand(1),
						nonzero_padding));
		...
		HloInstruction* lhs, HloInstruction* rhs)
{
	...
	if (IsLiteralWithValue(rhs, 0)) {
		...
		if (ShapeUtil::IsScalar(power->shape())) {
			...
			ones = HloInstruction::CreateBroadcast(power->shape(),
					computation_->AddInstruction(std::move(one)), { });
		}
		changed_ = true;
	...
	if (IsLiteralWithValue(rhs, 2)) {
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(power,
				HloInstruction::CreateBinary(power->shape(),
	...
	if (IsLiteralWithValue(rhs, -1)) {
		...
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(power,
				HloInstruction::CreateBinary(power->shape(), HloOpcode::kDivide,
						one, lhs));
	}
	return Status::OK();
}
...
Status AlgebraicSimplifierVisitor::HandleReshape(HloInstruction* reshape)
{
	...
	if (HloOpcode::kReshape == operand->opcode()) {
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(reshape,
				HloInstruction::CreateReshape(reshape->shape(),
						operand->mutable_operand(0)));
	}

	if (HloOpcode::kBroadcast == reshape->operand(0)->opcode()) {
		auto opt_dims = ReshapeLeavesDimensionsUnmodified(reshape,
				reshape->operand(0)->dimensions());
		if (opt_dims.first) {
			changed_ = true;
			return computation_->ReplaceWithNewInstruction(reshape,
					HloInstruction::CreateBroadcast(reshape->shape(),
							reshape->mutable_operand(0)->mutable_operand(0),
							opt_dims.second));
		}
	}
...
Status AlgebraicSimplifierVisitor::HandleReduce(HloInstruction* reduce,
		HloInstruction* arg, HloInstruction* init_value,
		tensorflow::gtl::ArraySlice<int64> dimensions, HloComputation* function)
{
	if (ShapeUtil::ElementsIn(reduce->shape())
		...
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(reduce,
				HloInstruction::CreateMap(reduce->shape(),
...
Status AlgebraicSimplifierVisitor::HandleTranspose(HloInstruction* transpose)
{
	...
	if (HloOpcode::kTranspose == operand->opcode()) {
		changed_ = true;
		return computation_->ReplaceWithNewInstruction(transpose,
				HloInstruction::CreateTranspose(transpose->shape(),
						operand->mutable_operand(0),
		...
		const Window& window)
{
	...
	auto add_bitcast =
			[&](Shape shape, HloInstruction* operand) {
	...
	const int64 input_channels = input_shape.dimensions(
			dnums.feature_dimension());
	const int64 output_channels = filter_shape.dimensions(
			dnums.kernel_output_feature_dimension());
	...
	const Shape new_filter_shape =
			ShapeUtil::MakeShapeWithMonotonicDim0MajorLayout(
					filter_shape.element_type(), { input_channels,
							output_channels });
	const Shape dot_output_shape =
			ShapeUtil::MakeShapeWithMonotonicDim0MajorLayout(
					convolution_shape.element_type(), { conv_width,
							output_channels });
	...
	auto dot = computation_->AddInstruction(
			HloInstruction::CreateBinary(dot_output_shape, HloOpcode::kDot,
					new_lhs, new_rhs));
	changed_ = true;
		...
		HloInstruction* lhs, HloInstruction* rhs)
{
	...
	// where max_operand and min_operand are scalar constants.
	{
		...
		if (hlo_query::MatchBinaryInstructionOperandOpcode(HloOpcode::kMinimum,
				maximum,
				...
				&& TransformToClampIfSameShape(maximum, min, min_operand,
						operand, maximum, max_operand)) {
			return Status::OK();
		}
	}
		...
		HloInstruction* lhs, HloInstruction* rhs)
{
	...
	// where max_operand and min_operand are scalar constants.
	{
		...
		if (hlo_query::MatchBinaryInstructionOperandOpcode(HloOpcode::kMaximum,
				minimum,
				...
				&& hlo_query::MatchBinaryInstructionOperand(
						...
						/*other_operand=*/&operand)
				&& TransformToClampIfSameShape(minimum, minimum, min_operand,
						operand, max, max_operand)) {
			return Status::OK();
		}
	}
...
StatusOr<bool> AlgebraicSimplifier::Run(HloModule* module)
{
	...
	bool changed =
			std::any_of(module->computations().begin(),
