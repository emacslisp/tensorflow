...
public:
	...
	StatusOr<llvm::Function*> EmitComputation(HloComputation* computation,
			const string& function_name_prefix, bool is_entry_computation,
...
protected:
	...
	Status HandleConstant(HloInstruction* constant, const Literal& literal)
			override;
	Status HandleCopy(HloInstruction* copy, HloInstruction* operand) override;
	Status HandleGetTupleElement(HloInstruction* get_tuple_element,
			HloInstruction* operand) override;
	Status HandleSelect(HloInstruction* select, HloInstruction* pred,
			HloInstruction* on_true, HloInstruction* on_false) override;
	...
	Status HandleTuple(HloInstruction* tuple,
...
private:
	...
	StatusOr<llvm::Function*> EmitFunction(HloComputation* function, // The function to emit.
			tensorflow::StringPiece function_name_suffix); // Used for LLVM IR register names.
	...
	llvm::Value* EmitElementFunctionCall(llvm::Function* function,
	...
	void EmitArrayFunctionCallInto(llvm::Function* function,
			tensorflow::gtl::ArraySlice<llvm::Value*> parameter_addresses,
	...
	// function.
	llvm::Value* EmitArrayFunctionCall(llvm::Function* function,
	...
	Status ElementTypesSameAndSupported(const HloInstruction& instruction,
	...
	Status EmitTargetElementLoop(HloInstruction* target_op,
	...
	llvm::Constant* CreateInitializerForConstantArray(
			const std::vector<llvm::Constant*>& array_elements,
			const Shape& shape, int64 dimension_index);
	...
	std::map<std::pair<llvm::Function*, BufferAllocation::Index>,
			llvm::AllocaInst*> thread_local_buffers_;
	...
	public:
		ProfilingState() :
				is_entry_computation_(false), use_rdtscp_(false), prof_counters_(
						nullptr)
		{
		}
		ProfilingState(bool is_entry_computation, bool use_rdtscp,
				llvm::Argument* prof_counters) :
				is_entry_computation_(is_entry_computation), use_rdtscp_(
						use_rdtscp), prof_counters_(prof_counters)
		{
		}

		// Record the cycle counter before an HLO executes.
		void RecordCycleStart(llvm::IRBuilder<>* ir_builder,
				HloInstruction* hlo);
		// Record the number of cycles it took for an HLO to execute.
		void RecordCycleDelta(llvm::IRBuilder<>* ir_builder,
