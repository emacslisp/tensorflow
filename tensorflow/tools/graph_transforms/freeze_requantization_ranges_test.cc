...
protected:
	void TestFreezeRequantizationRanges()
	{
		...
		const string min_max_log_file_name = io::JoinPath(testing::TmpDir(),
				"min_max_log_file.txt");
		{
			std::unique_ptr<WritableFile> file;
			TF_ASSERT_OK(
					Env::Default()->NewWritableFile(min_max_log_file_name,
							&file));
			TF_ASSERT_OK(file->Append("Something irrelevant\n"));
			TF_ASSERT_OK(file->Append("[SomePrefix] "
		...
		TF_EXPECT_OK(
				FreezeRequantizationRanges(graph_def, context,
						&frozen_graph_def));
		...
		const string& min_input = NodeNameFromInput(
				node_map.at("requantize_op")->input(3));
		ASSERT_EQ(1, node_map.count(min_input));
		EXPECT_EQ("Const", node_map.at(min_input)->op());
		const string& max_input = NodeNameFromInput(
				node_map.at("requantize_op")->input(4));
		...
		TF_ASSERT_OK(
				original_session->Run( { }, { "dequantize_op" }, { },
						&original_outputs));
		...
		TF_ASSERT_OK(
				frozen_session->Run( { }, { "dequantize_op" }, { },
						&frozen_outputs));

		ASSERT_EQ(original_outputs.size(), frozen_outputs.size());
		ASSERT_EQ(1, frozen_outputs.size());
		test::ExpectTensorNear<float>(original_outputs[0], frozen_outputs[0],
				0.5);
	}
	...
	void TestExtractMinMaxRecords()
	{
				...
				"min_max_log_file2.txt");
		{
			...
			TF_ASSERT_OK(file->Append("[SomePrefix] "
					...
					"[-2.4313571][10.584145]\n"));
			TF_ASSERT_OK(file->Append("Something else irrelevant\n"));
			TF_ASSERT_OK(file->Append("[SomeOtherPrefix] "
