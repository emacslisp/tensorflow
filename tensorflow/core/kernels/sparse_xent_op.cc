...
Status CheckInvalidLabelIndex(const Tensor& labels, int64 max_index)
{
	...
	int64 bad_index;
	auto min_max_dim_value = std::minmax_element(label_values.data(),
			label_values.data() + label_values.size());
	if (*min_max_dim_value.first < 0
			|| *min_max_dim_value.second >= max_index) {
		bad_index =
				(*min_max_dim_value.first < 0) ?
						*min_max_dim_value.first : *min_max_dim_value.second;
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::value, labels.shape(),
						&scratch));
		...
		if (logits.dim_size(0) > 0) {
			if (std::is_same<Device, CPUDevice>::value) {
				OP_REQUIRES_OK(context,
						CheckInvalidLabelIndex<Index>(labels,
								logits.dim_size(1)));
			}
