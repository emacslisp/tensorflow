...
/* static */Status XlaDevice::Create(const string& platform_name,
		const string& device_name, int device_ordinal,
		...
		const string& name_prefix, std::unique_ptr<XlaDevice>* device)
{
	...
	const DeviceAttributes attrs = Device::BuildDeviceAttributes(
			strings::StrCat(name_prefix, "/device:", device_name, ":",
					device_ordinal), DeviceType(device_name),
			Bytes(16ULL << 30), DeviceLocality(),
			strings::StrCat("device: ", device_name, " device"));

	static Allocator* allocator = new XlaDeviceAllocator;
	device->reset(
			new XlaDevice(options, attrs, device_ordinal,
					DeviceType(jit_device_name), platform.ValueOrDie(),
					allocator));
	return Status::OK();
}

XlaDevice::Metadata::Metadata(int device_ordinal,
		perftools::gputools::Platform* platform, const DeviceType& device_type) :
		device_ordinal_(device_ordinal), device_type_(device_type), platform_(
				platform)
{
}
...
string XlaDevice::Metadata::DebugString()
{
	return "XLA device metadata";
}
...
XlaDevice::XlaDevice(const SessionOptions& options,
		...
		LocalDevice(options, attrs, xla_allocator), device_ordinal_(
				device_ordinal), jit_device_name_(jit_device_name), xla_allocator_(
				xla_allocator), platform_(platform)
{
	// Store the platform in the resource manager so Ops can retrieve it
	// e.g., to lazily create a XlaCompilationCache object.
	TF_CHECK_OK(
			resource_manager()->Create < Metadata
					> (resource_manager()->default_container(), "xla_metadata", new Metadata(
							device_ordinal_, platform_, jit_device_name_)));
}
XlaDevice::~XlaDevice()
{
}
