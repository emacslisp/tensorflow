...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "Save").Input(FakeInput()).Input(
						FakeInput()).Input(FakeInput( { DT_BOOL, DT_INT32,
						DT_FLOAT, DT_DOUBLE, DT_QINT8, DT_QINT32, DT_UINT8,
						DT_INT8, DT_INT16, DT_INT64, DT_STRING, DT_COMPLEX64,
						DT_COMPLEX128, DT_HALF })).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "SaveSlices").Input(FakeInput()).Input(
						FakeInput()).Input(FakeInput()).Input(FakeInput( {
						DT_INT32, DT_FLOAT, DT_DOUBLE, DT_QINT8, DT_QINT32 })).Finalize(
						node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
protected:
	void MakeOp()
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "SaveSlices").Input(FakeInput()).Input(
						FakeInput()).Input(FakeInput()).Input(FakeInput( {
						DT_INT32, DT_INT32, DT_FLOAT })).Finalize(node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
static void BM_LargeTensorWrite(int iters, int num_elements)
{
	...
	// Builds the graph.
	const string temp_filename = io::JoinPath(testing::TmpDir(),
	...
	session_options.config.mutable_graph_options()->mutable_optimizer_options()->set_opt_level(
			tensorflow::OptimizerOptions_Level_L0);
