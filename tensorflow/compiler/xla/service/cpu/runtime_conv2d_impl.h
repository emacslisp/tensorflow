...
void EigenConvF32Impl(const EigenDevice& device, float* out, float* lhs,
		float* rhs, int64 input_batch, int64 input_rows, int64 input_cols,
		...
		int64 padding_top, int64 padding_bottom, int64 padding_left,
		int64 padding_right, int64 lhs_row_dilation, int64 lhs_col_dilation,
		int64 rhs_row_dilation, int64 rhs_col_dilation)
{
	const Eigen::TensorMap<Eigen::Tensor<const float, 4, Eigen::RowMajor>,
			Eigen::Aligned> input(lhs, input_batch, input_rows, input_cols,
			input_channels);

	const Eigen::TensorMap<Eigen::Tensor<const float, 4, Eigen::RowMajor>,
			Eigen::Aligned> kernel(rhs, kernel_rows, kernel_cols,
			kernel_channels, kernel_filters);

	Eigen::TensorMap<Eigen::Tensor<float, 4, Eigen::RowMajor>, Eigen::Aligned> output(
			out, input_batch, output_rows, output_cols, kernel_filters);
	...
	output.device(device) = input.extract_image_patches(kernel_cols,
			kernel_rows, col_stride, row_stride, rhs_col_dilation,
			rhs_row_dilation, lhs_col_dilation, lhs_row_dilation, padding_left,
			padding_right, padding_top, padding_bottom, 0.0f).reshape(
			pre_contract_dims).contract(kernel.reshape(kernel_dims),
