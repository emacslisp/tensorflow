...
CUDAFftPlan::CUDAFftPlan(CUDAExecutor *parent, uint64 num_x, uint64 num_y,
		...
		parent_(parent), fft_type_(type)
{
	auto ret = dynload::cufftPlan2d(parent, &plan_, num_x, num_y,
			CUDAFftType(type));
	if (ret != CUFFT_SUCCESS) {
		...
		parent_(parent), fft_type_(type)
{
	...
	auto ret = dynload::cufftPlanMany(parent, &plan_, rank, elem_count_,
...
{
	dynload::cufftDestroy(parent_, plan_);
}
...
std::unique_ptr<fft::Plan> CUDAFft::Create2dPlan(Stream *stream, uint64 num_x,
		uint64 num_y, fft::Type type, bool in_place_fft)
{
	std::unique_ptr<fft::Plan> plan { new CUDAFftPlan(parent_, num_x, num_y,
			type) };
	return plan;
}

std::unique_ptr<fft::Plan> CUDAFft::Create3dPlan(Stream *stream, uint64 num_x,
		uint64 num_y, uint64 num_z, fft::Type type, bool in_place_fft)
{
	std::unique_ptr<fft::Plan> plan { new CUDAFftPlan(parent_, num_x, num_y,
			num_z, type) };
	return plan;
...
std::unique_ptr<fft::Plan> CUDAFft::CreateBatchedPlan(Stream *stream, int rank,
		uint64 *elem_count, uint64 *input_embed, uint64 input_stride,
		uint64 input_distance, uint64 *output_embed, uint64 output_stride,
		uint64 output_distance, fft::Type type, bool in_place_fft,
		int batch_count)
{
	std::unique_ptr<fft::Plan> plan { new CUDAFftPlan(parent_, rank, elem_count,
			input_embed, input_stride, input_distance, output_embed,
			output_stride, output_distance, type, batch_count) };
	return plan;
}

template<typename FuncT, typename InputT, typename OutputT>
bool CUDAFft::DoFftInternal(Stream *stream, fft::Plan *plan, FuncT cufftExec,
		const DeviceMemory<InputT> &input, DeviceMemory<OutputT> *output)
{
...
bool CUDAFft::DoFftWithDirectionInternal(Stream *stream, fft::Plan *plan,
		FuncT cufftExec, const DeviceMemory<InputT> &input,
