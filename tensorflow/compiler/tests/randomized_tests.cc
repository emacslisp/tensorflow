...
public:
	...
	Status BuildGraph(string name_prefix, string device, bool use_jit,
			...
			std::vector<string>* inputs, std::vector<string>* outputs) const;
		...
		std::vector<string>* inputs, std::vector<string>* outputs) const
{
	...
	*test_def = node_def_;
	...
	for (int i = 0; i < input_types.size(); ++i) {
		...
		TF_RETURN_IF_ERROR(
				NodeDefBuilder(name, "Placeholder").Device(device).Attr("dtype",
						input_types[i]).Finalize(def));
	...
	for (int i = 0; i < output_types.size(); ++i) {
		...
		TF_RETURN_IF_ERROR(
				NodeDefBuilder(name, "Identity").Device(device).Attr("T",
						output_types[i]).Input(test_def->name(), i,
						output_types[i]).Finalize(def));
...
public:
	...
	std::vector<int64> RandomDims(int min_rank = 0, int max_rank =
			kDefaultMaxRank, int64 min_size = 0, int64 max_size =
			kDefaultMaxDimensionSize);
...
OpTest::OpTest()
{
	...
	LOG(INFO) << "Random seed for test case: " << seed << ". To reproduce the "
			"results of this test, pass flag --tf_xla_random_seed=" << seed;
		...
		std::vector<int64> dims)
{
	...
	size_t skip = std::uniform_int_distribution<size_t>(0, dims.size() - 1)(
			generator());
...
OpTest::WindowedDims OpTest::ChooseWindowedDims()
{
	...
	do {
		...
		d.kernel_rows = random_int(generator()), d.input_rows = RandomDim(
				d.kernel_rows);
		d.stride_rows = std::uniform_int_distribution<int>(1, d.kernel_rows)(
				generator());
		int64 pad_dummy;
	...
	do {
		...
		d.stride_cols = std::uniform_int_distribution<int>(1, d.kernel_cols)(
				generator());
		int64 pad_dummy;
	...
	for (int i = 0; i < Tx.size(); ++i) {
		if (!IsClose(Tx(i), Ty(i), atol, rtol)) {
			return errors::InvalidArgument(
					strings::StrCat(i, "-th tensor element isn't close: ",
							Tx(i), " vs. ", Ty(i), ". x = ", x.DebugString(),
							"y = ", y.DebugString(), "atol = ", atol,
							" rtol = ", rtol, " tol = ",
							atol + rtol * std::fabs(Tx(i))));
		}
	}
	return Status::OK();
}
...
Status TensorsAreEqualImpl(const Tensor& x, const Tensor& y)
{
	...
	for (int i = 0; i < Tx.size(); ++i) {
		if (Tx(i) != Ty(i)) {
			return errors::InvalidArgument(
					strings::StrCat(i, "-th tensor element isn't equal: ",
							Tx(i), " vs. ", Ty(i), ". x = ", x.DebugString(),
							"y = ", y.DebugString()));
		}
	}
		...
		double rtol)
{
	if (a.dtype() != b.dtype()) {
		return errors::InvalidArgument(
				strings::StrCat("Tensors have different types: ",
						DataTypeString(a.dtype()), " and ",
						DataTypeString(b.dtype())));
	}
	if (!a.IsSameSize(b)) {
		return errors::InvalidArgument(
				strings::StrCat("Tensors have different shapes: ",
...
{
	string cpu_device = LocalDeviceToFullDeviceName(
			strings::StrCat(DEVICE_CPU, ":0"));
	string test_device = LocalDeviceToFullDeviceName(*tf_xla_test_device_ptr);

	DeviceNameUtils::ParsedName parsed_name;
	ASSERT_TRUE(
			DeviceNameUtils::ParseLocalName(*tf_xla_test_device_ptr,
					&parsed_name));
	...
	TF_ASSERT_OK(
			builder.BuildGraph(strings::StrCat("test", num_tests_, "_expected"),
					cpu_device,
	...
	NodeDef* node_def;
	TF_ASSERT_OK(
			builder.BuildGraph(strings::StrCat("test", num_tests_, "_test"),
					test_device, tf_xla_test_use_jit, &graph, &node_def,
					&test_inputs, &test_fetches));

	// Check that there's a kernel corresponding to 'node_def' on the device under
	// test.
	Status status = FindKernelDef(test_device_type, *node_def, nullptr,
			nullptr);
	if (!status.ok()) {
	...
	std::vector<std::pair<string, Tensor>> expected_feeds(
	...
	Status s = session_->Run(expected_feeds, expected_fetches, { },
			&expected_outputs);
	if (!s.ok()) {
		VLOG(1) << "Expected graph failed with status: " << s
				<< ". Skipping test";
		return;
	}
...
// configurable time bound.
		tensorflow::Flag("tf_xla_test_repetitions",
				...
				"Number of repetitions for each test."), tensorflow::Flag(
				"tf_xla_test_device", tensorflow::tf_xla_test_device_ptr,
				"Tensorflow device type to use for test"), tensorflow::Flag(
				"tf_xla_test_use_jit", &tensorflow::tf_xla_test_use_jit,
