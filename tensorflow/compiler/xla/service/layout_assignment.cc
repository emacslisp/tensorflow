...
OperandLayoutConstraint::OperandLayoutConstraint(
		...
		int64 operand_no) :
		shape_layout_(shape_layout), instruction_(instruction), operand_no_(
				operand_no)
{
	CHECK(shape_layout_.LayoutIsSet());
	CHECK(
			ShapeUtil::Compatible(shape_layout.shape(),
		...
		const HloInstruction* instruction, int64 operand_no) const
{
	...
	auto operand_buffers = points_to_analysis_.GetPointsToSet(
			instruction->operand(operand_no)).CreateFlattenedSet();
		...
		const LogicalBuffer& buffer)
{
	...
	auto new_constraint_it = buffer_constraints_.insert( { &buffer,
			BufferLayoutConstraint(layout, buffer) });
		...
		const HloInstruction* instruction, int64 operand_no)
{
	...
	const ShapeLayout* curr_shape_layout = OperandLayout(instruction,
			operand_no);
...
Status LayoutConstraints::SetArrayOperandLayout(const Layout& layout,
		const HloInstruction* instruction, int64 operand_no)
{
...
Status LayoutConstraints::SetInstructionLayout(const Shape& shape_with_layout,
		const HloInstruction* instruction)
{
	...
	// instruction.
	return ShapeUtil::ForEachSubshape(shape_with_layout,
...
const Layout* LayoutConstraints::BufferLayout(const LogicalBuffer& buffer) const
{
...
const ShapeLayout* LayoutConstraints::OperandLayout(
		const HloInstruction* instruction, int64 operand_no) const
{
	auto it = operand_constraints_.find(
			std::make_pair(instruction, operand_no));
	return it == operand_constraints_.end() ?
			nullptr : &it->second.shape_layout();
}
...
string LayoutConstraints::ToString() const
{
	...
	tensorflow::strings::StrAppend(&output,
			"LayoutConstraints for computation ", computation_->name(), ":\n");
	for (auto* instruction : computation_->MakeInstructionPostOrder()) {
		...
		for (int64 i = 0; i < instruction->operand_count(); ++i) {
			if (OperandLayout(instruction, i) != nullptr) {
				tensorflow::strings::StrAppend(&output, "    operand (", i,
						"): ", OperandLayout(instruction, i)->ToString(), "\n");
			}
		}
		for (const LogicalBuffer* buffer : points_to_analysis_.GetBuffersDefinedByInstruction(
				instruction)) {
		...
		HloComputation* computation, LayoutConstraints* constraints)
{
	...
	for (auto& instruction : computation->instructions()) {
		...
		if (instruction->opcode() == HloOpcode::kConstant) {
			...
			// ComputationLayout.
			shape_with_layout = &computation_layout.parameter_layout(
					instruction->parameter_number()).shape();
		}
	...
	for (auto& instruction : computation->instructions()) {
		if (instruction->opcode() == HloOpcode::kCall) {
			...
			const ComputationLayout& called_computation_layout = FindOrDie(
					computation_layouts_, instruction->to_apply());
			TF_RETURN_IF_ERROR(
					constraints->SetInstructionLayout(
			...
			const ComputationLayout& body_layout = FindOrDie(
					computation_layouts_, body);
			const ComputationLayout& condition_layout = FindOrDie(
					computation_layouts_, condition);
			...
			DCHECK(
					ShapeUtil::Compatible(body_layout.result_shape(),
			...
			if (!ShapeUtil::Equal(body->root_instruction()->shape(),
				...
				return InternalError(
						...
						ShapeUtil::HumanString(
								condition_layout.parameter_shape(0)).c_str(),
						ShapeUtil::HumanString(body_layout.result_shape()).c_str());
			}
			...
			TF_RETURN_IF_ERROR(
					constraints->SetOperandLayout(body_layout.result_shape(),
							instruction.get(), 0));
			...
			TF_RETURN_IF_ERROR(
					constraints->SetInstructionLayout(result_shape,
							instruction.get()));
			for (int64 i = 0; i < instruction->operand_count(); ++i) {
		...
		const ComputationLayout& computation_layout)
{
	...
	for (int64 i = 0; i < computation->num_parameters(); ++i) {
		TF_RET_CHECK(
				computation_layout.parameter_layout(i).MatchesLayoutInShape(
						call->operand(i)->shape()));
	}
	TF_RET_CHECK(
			computation_layout.result_layout().MatchesLayoutInShape(
					call->shape()));
	return Status::OK();
}
		...
		const ComputationLayout& body_computation_layout)
{
	...
	TF_RET_CHECK(
			condition_computation_layout.parameter_layout(0).MatchesLayoutInShape(
					init_shape));
	TF_RET_CHECK(
			body_computation_layout.parameter_layout(0).MatchesLayoutInShape(
					init_shape));
	TF_RET_CHECK(
			body_computation_layout.result_layout().MatchesLayoutInShape(
					init_shape));
	TF_RET_CHECK(
...
Status CheckFusionLayout(HloInstruction* fusion)
{
	...
	TF_RET_CHECK(
			LayoutUtil::LayoutsInShapesEqual(fusion->shape(),
					fusion->fused_expression_root()->shape()));
	for (int64 i = 0; i < fusion->operand_count(); ++i) {
		TF_RET_CHECK(
				LayoutUtil::LayoutsInShapesEqual(
						fusion->fused_parameter(i)->shape(),
...
Status CheckParameterLayout(HloInstruction* parameter,
		const ComputationLayout& computation_layout)
{
	const ShapeLayout& parameter_layout = computation_layout.parameter_layout(
	...
	if (!parameter_layout.MatchesLayoutInShape(parameter->shape())) {
		return InternalError(
				"parameter instruction %s does not match layout of computation "
						"shape: %s", parameter->ToString().c_str(),
				parameter_layout.ToString().c_str());
	}
	return Status::OK();
}
		...
		const std::map<HloComputation*, ComputationLayout>& computation_layouts)
{
	...
	for (auto& computation : module->computations()) {
		for (auto& instruction : computation->instructions()) {
			...
			case HloOpcode::kCall:
				TF_RETURN_IF_ERROR(
						CheckCallLayout(instruction.get(),
								FindOrDie(computation_layouts,
			...
			case HloOpcode::kParameter:
				TF_RETURN_IF_ERROR(
						CheckParameterLayout(instruction.get(),
								FindOrDie(computation_layouts,
										instruction->parent())));
				break;
			...
			case HloOpcode::kWhile:
				TF_RETURN_IF_ERROR(
						CheckWhileLayout(instruction.get(),
								FindOrDie(computation_layouts,
	...
	TF_RET_CHECK(
			ShapeUtil::Equal(
					...
					FindOrDie(computation_layouts, module->entry_computation()).result_layout().shape()));
		...
		entry_computation_layout_(entry_computation_layout)
{
	...
	for (const ShapeLayout& parameter_layout : entry_computation_layout_->parameter_layouts()) {
...
tensorflow::protobuf::RepeatedField<tensorflow::protobuf_int64> DeleteAndInsertIndices(
		...
		int64 operand_no)
{
	...
	if (instruction->opcode() == HloOpcode::kReshape) {
		...
		if (merely_inserts_or_deletes_1_sized_dims) {
			Layout operand_layout = LayoutUtil::MakeLayout(
					AsInt64Slice(
							DeleteAndInsertIndices(inserted_indices,
									deleted_indices,
									output_layout.minor_to_major())));
			TF_CHECK_OK(
					LayoutUtil::ValidateLayoutForShape(operand_layout,
							operand->shape()));
	...
	if (instruction->opcode() == HloOpcode::kTranspose) {
		...
		std::vector<int64> perm = ComposePermutations(instruction->dimensions(),
		...
		int64 operand_no)
{
	...
	if (user->opcode() == HloOpcode::kReshape) {
		...
		if (merely_inserts_or_deletes_1_sized_dims) {
			Layout user_layout = LayoutUtil::MakeLayout(
					AsInt64Slice(
							DeleteAndInsertIndices(deleted_indices,
									inserted_indices,
									operand_layout.minor_to_major())));
			TF_CHECK_OK(
					LayoutUtil::ValidateLayoutForShape(user_layout,
							user->shape()));
	...
	if (user->opcode() == HloOpcode::kTranspose) {
		...
		std::vector<int64> perm = Permute(
				InversePermutation(user->dimensions()),
	...
	while (!worklist.empty()) {
		...
		if (auto* buffer_constraint =
			...
			TF_RETURN_IF_ERROR(
				...
				dynamic_cast<const OperandLayoutConstraint*>(layout_constraint)) {
			TF_RETURN_IF_ERROR(
					PropagateOperandConstraint(*operand_constraint,
							constraints));
		} else if (auto* result_constraint =
				dynamic_cast<const ResultLayoutConstraint*>(layout_constraint)) {
			TF_RETURN_IF_ERROR(
		...
		const TuplePointsToAnalysis& points_to_analysis)
{
	...
	for (const auto& buffer_alias : points_to_analysis.GetBufferAliases(buffer)) {
		...
		for (const HloInstruction* user : buffer_alias.instruction()->users()) {
			for (int64 operand_no : user->OperandIndices(
					buffer_alias.instruction())) {
		...
		LayoutConstraints* constraints)
{
		...
		for (int64 operand_no = 0; operand_no < instruction->operand_count();
			...
			if (constraints->OperandLayout(instruction, operand_no) == nullptr
				...
				if (operand_layout != nullptr) {
					TF_RETURN_IF_ERROR(
							constraints->SetArrayOperandLayout(*operand_layout,
									instruction, operand_no));
				}
			}
		}
	...
	for (const auto& user_operand_no : GetArrayUsesOfBuffer(buffer,
		...
		if (constraints->OperandLayout(user, operand_no) == nullptr
			...
			TF_RETURN_IF_ERROR(
					constraints->SetArrayOperandLayout(
		...
		LayoutConstraints* constraints)
{
	...
	return PropagateUseConstraintToDefs(result_constraint.shape_layout(),
		...
		HloInstruction* instruction, const ShapeIndex& index)
{
	...
	TF_RET_CHECK(
			!points_to_analysis.InstructionDefinesBufferAtIndex(instruction,
					index));
		...
		const Shape& shape_with_layout, HloInstruction* instruction)
{
	...
	if (ShapeUtil::IsTuple(instruction->shape())) {
		...
		for (int64 i = 0;
				i < ShapeUtil::TupleElementCount(instruction->shape()); ++i) {
		...
		TF_RETURN_IF_ERROR(
				LayoutUtil::CopyLayoutBetweenShapes(shape_with_layout,
						tuple_copy->mutable_shape()));
		return tuple_copy;
	} else if (ShapeUtil::IsArray(instruction->shape())) {
		HloInstruction* copy = instruction->parent()->AddInstruction(
...
Status CopyOperandIfLayoutsDiffer(const ShapeLayout& operand_layout,
		HloInstruction* instruction, int64 operand_no)
{
...
Status SetFusionLayouts(HloInstruction* fusion)
{
	...
	for (auto& fused_instruction : fusion->fused_instructions()) {
		if (fused_instruction->opcode() == HloOpcode::kParameter) {
			const HloInstruction* fusion_operand = fusion->operand(
					fused_instruction->parameter_number());
			DCHECK(
					ShapeUtil::Compatible(fusion_operand->shape(),
							fused_instruction->shape()));
			TF_RETURN_IF_ERROR(
					LayoutUtil::CopyLayoutBetweenShapes(fusion_operand->shape(),
			...
			// instruction layout.
			DCHECK(
					ShapeUtil::Compatible(fusion->shape(),
							fused_instruction->shape()));
			TF_RETURN_IF_ERROR(
					LayoutUtil::CopyLayoutBetweenShapes(fusion->shape(),
							fused_instruction->mutable_shape()));
		...
		HloComputation* computation)
{
	...
	for (HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
		...
		for (int64 operand_no = 0; operand_no < instruction->operand_count();
				++operand_no) {
			const ShapeLayout* operand_layout = constraints.OperandLayout(
					instruction, operand_no);
			if (operand_layout != nullptr) {
				TF_RETURN_IF_ERROR(
						CopyOperandIfLayoutsDiffer(*operand_layout, instruction,
								operand_no));
			}
		}
		...
		// inference.
		for (const LogicalBuffer* buffer : constraints.points_to_analysis().GetBuffersDefinedByInstruction(
				instruction)) {
		...
		TF_RETURN_IF_ERROR(
				ShapeUtil::ForEachMutableSubshape(instruction->mutable_shape(),
		...
		HloComputation* computation)
{
	...
	TF_RETURN_IF_ERROR(
			AddMandatoryConstraints(computation_layout, computation,
					&constraints));
...
StatusOr<bool> LayoutAssignment::Run(HloModule* module)
{
	...
	for (auto* computation : module->MakeComputationPostOrder()) {
		if (computation == module->entry_computation()) {
			TF_RETURN_IF_ERROR(
					RunOnComputation(*entry_computation_layout_,
			...
			TF_RETURN_IF_ERROR(
					RunOnComputation(computation_layout, computation));
		}
	}
