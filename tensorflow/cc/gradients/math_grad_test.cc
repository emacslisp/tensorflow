...
protected:
			...
			std::function<float(float, float)> dx_fn)
	{
		...
		TF_ASSERT_OK(test::CallGradFunction(scope_, Operation(y.node()), {
				ops::Const(scope_, dy) }, &grad_outputs));
		Tensor output;
		test::GetTensor(scope_, grad_outputs[0], &output);
		test::ExpectClose(output, dx);
	}

	float RV(std::vector<float> v)
	{
...
protected:
	MathGradTest() :
			root_(Scope::NewRootScope().WithDevice("/cpu:0"))
	{
	}
			...
			std::vector<Tensor>* out)
	{
		...
		TF_ASSERT_OK(
				test::CallGradFunction(root_, Operation(z.node()), { dz },
						&grad_outputs));
			...
			std::vector<Tensor>* data)
	{
		...
		if (is_batch) {
			// x.shape = [b, m, k]
			x_shape =
					tx ? TensorShape( { b, k, m }) : TensorShape( { b, m, k });
		} else {
			// x.shape = [m, k]
		...
		if (is_batch) {
			// y.shape = [b, k, n]
			y_shape =
					ty ? TensorShape( { b, n, k }) : TensorShape( { b, k, n });
		} else {
			// y.shape = [k, n]
	...
	{
		test::FillFn<float>(t,
				[this](const int i) {return static_cast<float>(Rand());});
	}
