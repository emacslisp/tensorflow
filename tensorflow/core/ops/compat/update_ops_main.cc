...
void WriteUpdateTo(const string& directory)
{
	OpCompatibilityLib compatibility(directory,
			strings::StrCat("v", TF_MAJOR_VERSION), nullptr);
	...
	Env* env = Env::Default();
	{
		...
		TF_QCHECK_OK(
				WriteStringToFile(env, ops_file, compatibility.OpsString()));
	}
	...
	int added_ops = 0;
	TF_QCHECK_OK(
			compatibility.ValidateCompatible(env, &changed_ops, &added_ops,
					&out_op_history));
