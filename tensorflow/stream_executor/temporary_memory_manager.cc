...
void TemporaryMemoryManager::MarkFinalized(
		const DeviceMemoryBase& device_memory, uint64 generation,
		bool must_exist)
{
	mutex_lock lock(mutex_);
...
port::StatusOr<std::unique_ptr<TemporaryDeviceMemoryBase>> TemporaryMemoryManager::AllocateArrayBase(
		uint64 element_count, uint64 element_size)
{
	uint64 byte_size = element_count * element_size;
	DeviceMemoryBase device_memory = stream_->parent()->AllocateArray<uint8>(
			byte_size);
	...
	VLOG(1)
			<< port::Printf(
					"stream %p allocated temporary device memory at %p (size %llu) in "
							"generation %llu", stream_, device_memory.opaque(),
