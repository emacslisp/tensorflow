...
CpuAotCompilationOptions::CpuAotCompilationOptions(string triple,
		string cpu_name, string features, string entry_point_name,
		RelocationModel relocation_model) :
		triple_(std::move(triple)), cpu_name_(std::move(cpu_name)), features_(
				std::move(features)), entry_point_name_(
				std::move(entry_point_name)), relocation_model_(
				relocation_model)
{
}
...
CpuAotCompilationResult::CpuAotCompilationResult(
		...
		object_file_data_(std::move(object_file_data)), buffer_sizes_(
				std::move(buffer_sizes)), result_buffer_index_(
				result_buffer_index)
{
}
...
/* static */void CpuCompiler::InitializeLLVMTarget()
{
	...
	if (!flags->xla_cpu_llvm_cl_opts.empty()) {
		std::vector<string> opts = tensorflow::str_util::Split(
...
public:
	static StatusOr<std::unordered_map<const HloInstruction*, size_t>> GetCandidatesForComputation(
...
private:
	...
	Status DefaultAction(HloInstruction* hlo_instruction) override
	{
		hlo_to_profile_idx_->insert(
				{ hlo_instruction, hlo_to_profile_idx_->size() });
		return Status::OK();
	}
		...
		HloModuleConfig* module_config, HloDumper dump_hlo)
{
	...
	pipeline.AddPass<ConvCanonicalization>();
	{
		...
		pass.AddPass < AlgebraicSimplifier
				> (
				/*is_layout_sensitive=*/false, [](const Shape&, const Shape&) {return false;});
		...
		se::StreamExecutor* stream_exec)
{
	...
	auto llvm_module = MakeUnique<llvm::Module>("__compute_module",
			*llvm_context);
	...
	if (module_config->hlo_profiling_enabled()) {
		TF_ASSIGN_OR_RETURN(hlo_to_profile_idx,
	...
	if (flags->xla_cpu_parallel) {
		...
		std::unordered_map<const HloInstruction*,
				std::unique_ptr<unsigned char[]>> aligned_constants;
		for (auto instruction : computation->MakeInstructionPostOrder()) {
			...
			if (instruction->opcode() == HloOpcode::kConstant) {
				...
				// higher alignment.
				const void* data = LiteralUtil::InternalData(
						instruction->literal());
				int64 size = llvm_ir::ByteSizeOf(instruction->shape(),
						data_layout);
				auto iter = aligned_constants.emplace(instruction,
						MakeUnique<unsigned char[]>(size));
		...
		for (auto embedded_computation : computation->MakeEmbeddedComputationsList()) {
			auto parallel_computation_iter = parallel_computations.find(
					embedded_computation);
			// All parallel computations are considered to be an entry computation for
			// IR generation purposes.
			bool computation_is_parallel = parallel_computation_iter
					!= parallel_computations.end();
			TF_ASSIGN_OR_RETURN(llvm::Function * ir_function,
			...
			if (computation_is_parallel) {
				HloInstruction* call_instruction =
						parallel_computation_iter->second;
		...
		cpu_executable.reset(
				new ParallelCpuExecutable(std::move(jit), std::move(assignment),
		...
		if (flags->xla_cpu_embed_ir) {
			static_cast<CpuExecutable&>(*cpu_executable).set_ir_module_string(
					ir_module_string);
		}
	} else {
...
StatusOr<std::vector<std::unique_ptr<AotCompilationResult>>>CpuCompiler::CompileAheadOfTime(
		std::vector<std::unique_ptr<HloModule>> hlo_modules,
		std::vector<std::unique_ptr<HloModuleConfig>> module_configs,
