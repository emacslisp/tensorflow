...
Output Const(const Scope& scope, const Input::Initializer& val)
{
	...
	auto builder =
			NodeBuilder(unique_name, "Const").Attr("value", val.tensor).Attr(
					"dtype", val.tensor.dtype());
...
NodeBuilder::NodeOut AsNodeOut(const Scope& scope, const Input& inp)
{
	...
	if (!inp.node_name().empty()) {
		return NodeBuilder::NodeOut(inp.node_name(), inp.index(),
				inp.data_type());
	}
	auto transformed = Input { Const(scope.NewSubScope("Const"),
			Input::Initializer(inp.tensor())) };
