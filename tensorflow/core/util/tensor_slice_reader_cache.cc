		...
		TensorSliceReader::OpenTableFunction open_function, int preferred_shard)
{
	...
	TensorSliceReaderCache::OpenFuncType* func_ptr = open_function.target<
			TensorSliceReaderCache::OpenFuncType>();
#else   // __GXX_RTTI
	...
	if (!func_ptr) {
		...
		LOG(WARNING)
				<< "Caching disabled because the open function is a lambda or "
	...
	if (readers_.find(filepattern) == readers_.end()) {
		...
		TensorSliceReader* tmp_reader(
				new TensorSliceReader(filepattern, open_function,
						preferred_shard));
