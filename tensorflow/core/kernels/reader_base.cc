...
ReaderBase::ReaderBase(const string& name) :
		name_(name)
{
}
		...
		OpKernelContext* context)
{
	...
	while (true) {
		...
		Status status = ReadUpToLocked(remaining, keys, values,
				&num_records_produced, &at_end);
		// This call so far.
		...
		if (!at_end && status.ok() && num_records_produced == 0) {
			status =
					errors::Internal("ReadManyLocked() for ", name(),
...
Status ReaderBase::ReadUpToLocked(int64 num_records, std::vector<string>* keys,
		std::vector<string>* values, int64* num_read, bool* at_end)
{
	bool produced = false;
	string key;
	string value;
		...
		OpKernelContext* context)
{
	...
	while (true) {
		...
		if (!at_end && status.ok() && !produced) {
			status =
					errors::Internal("ReadLocked() for ", name(),
...
{
	Notification n;
	queue->TryDequeue(context,
			[this, context, &n](const QueueInterface::Tuple& tuple) {
				if (context->status().ok()) {
					if (tuple.size() != 1) {
						context->SetStatus(
...
Status ReaderBase::RestoreBaseState(const ReaderBaseState& state)
{
	...
	if (work_started_ > work_finished_) {
		return errors::InvalidArgument(
				"Inconsistent work started vs. finished when restoring in ",
				name(), ": ", state.DebugString());
	}
	return Status::OK();
}
