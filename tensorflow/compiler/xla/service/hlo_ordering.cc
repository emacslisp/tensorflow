...
string PredecessorHloOrdering::ToStringHelper(const string& name) const
{
	...
	for (auto& computation : module_->computations()) {
		pieces.push_back(
				tensorflow::strings::Printf("computation %s:",
						computation->name().c_str()));
		const auto all = computation->MakeInstructionPostOrder();
		for (auto instruction : all) {
			pieces.push_back(
					tensorflow::strings::Printf("  %s strict predecessors:",
							instruction->name().c_str()));
			for (auto predecessor : all) {
				if (strict_predecessors_.at(computation.get())->IsReachable(
						instruction, predecessor)) {
					pieces.push_back(
							tensorflow::strings::Printf("  %s",
									predecessor->name().c_str()));
				}
			}
		}
	}
...
SequentialHloOrdering::SequentialHloOrdering(const HloModule* module,
...
string SequentialHloOrdering::ToString() const
{
	...
	for (auto& computation : module_->computations()) {
		pieces.push_back(
				tensorflow::strings::Printf("computation %s order:",
						computation->name().c_str()));
...
private:
	...
	ListScheduler(const HloComputation& computation,
			const TuplePointsToAnalysis& points_to_analysis,
			const LogicalBuffer::SizeFunction& size_function) :
			computation_(computation), points_to_analysis_(points_to_analysis), size_function_(
					size_function)
	{
		...
		for (auto& instruction : computation.instructions()) {
			...
			for (auto* operand : instruction->operands()) {
				for (const LogicalBuffer* buffer : points_to_analysis.GetBuffersDefinedByInstruction(
						operand)) {
		...
		for (auto& instruction : computation.instructions()) {
			for (const LogicalBuffer* buffer : buffer_uses_.at(
					instruction.get())) {
		...
		// the computation.
		for (const LogicalBuffer* live_out_buffer : points_to_analysis.GetPointsToSet(
				computation.root_instruction()).CreateFlattenedSet()) {
	...
	int64 BytesFreedIfScheduled(const HloInstruction* instruction)
	{
		...
		for (auto* buffer : points_to_analysis_.GetBuffersDefinedByInstruction(
				instruction)) {
			if (!IgnoreBuffer(*buffer)) {
				freed_bytes -= size_function_(*buffer);
			}
	...
	std::vector<const HloInstruction*> CreateSchedule()
	{
		...
		while (!ready_list.empty()) {
			...
			for (auto* successor : successors) {
				std::set<HloInstruction*> predecessors(
						successor->operands().begin(),
	...
	std::unordered_map<const HloInstruction*,
			std::unordered_set<const LogicalBuffer*>> buffer_uses_;
...
StatusOr<SequentialHloOrdering::HloModuleSequence> CreateMemoryMinimizingSequence(
