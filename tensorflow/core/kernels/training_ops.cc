...
struct ApplyAdadelta<CPUDevice, T> {
	void operator()(const CPUDevice& d, typename TTypes<T>::Flat var,
			...
			typename TTypes<T>::ConstFlat grad)
	{
		accum.device(d) = accum * rho()
				+ grad.square() * (static_cast<T>(1) - rho());
		const auto update = (accum_update + epsilon()).sqrt()
				* (accum + epsilon()).rsqrt() * grad;
		accum_update.device(d) = accum_update * rho()
				+ update.square() * (static_cast<T>(1) - rho());
...
struct ApplyAdagradDA<CPUDevice, T> {
			...
			typename TTypes<T>::ConstFlat grad)
	{
		...
		if (l1() > 0) {
			var.device(d) = lr() * var.constant(-1.0) * gradient_accum.sign()
					* (gradient_accum.abs()
							- var.constant(static_cast<float>(global_step))
									* var.constant(l1())).cwiseMax(T(0.0))
					/ (var.constant(l2())
							* var.constant(
									static_cast<float>(global_step) * lr())
							+ gradient_squared_accum.sqrt());
		} else {
			var.device(d) = lr() * gradient_accum * var.constant(-1.0)
					/ (var.constant(l2())
							* var.constant(
									static_cast<float>(global_step) * lr())
							+ gradient_squared_accum.sqrt());
...
struct ApplyProximalAdagrad<CPUDevice, T> {
			...
			typename TTypes<T>::ConstFlat grad)
	{
		...
		if (l1() > 0) {
			var.device(d) = prox_var.abs()
					- learning_rate * prox_var.constant(l1());
			// compute sign(v) * max(|v| - lr * l1, 0)
			var.device(d) = prox_var.sign() * var.cwiseMax(T(0.0));
		} else {
			var.device(d) = prox_var;
...
struct ApplyFtrl<CPUDevice, T> {
			...
			typename TTypes<T>::ConstScalar lr_power)
	{
		...
		if (lr_power() == static_cast<T>(-0.5)) {
			linear.device(d) += grad
					- (new_accum.sqrt() - accum.sqrt()) / lr() * var;
		} else {
			linear.device(d) += grad
					- (new_accum.pow(-lr_power()) - accum.pow(-lr_power()))
							/ lr() * var;
		}
		...
		var.device(d) = (linear.abs() > linear.constant(l1())).select(var,
...
struct ApplyRMSProp<CPUDevice, T> {
			...
			typename TTypes<T>::ConstFlat grad)
	{
		...
		mom.device(d) = mom * momentum()
				+ (grad * lr()) / ((ms + epsilon()).sqrt());
		var.device(d) -= mom;
	}
};
...
std::vector<mutex_lock> MaybeLockMutexesInOrder(OpKernelContext* ctx,
		bool do_lock, const std::vector<int>& input_ids)
{
...
public:
	explicit ApplyGradientDescentOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, var.shape().IsSameSize(delta.shape()),
		...
		functor::ApplyGradientDescent<Device, T>()(device, var.flat<T>(),
				alpha.scalar<T>(), delta.flat<T>());
...
public:
	explicit ApplyAdadeltaOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
...
private:
	...
	void DoValidate(OpKernelContext* ctx)
	{
		...
		Tensor accum;
		OP_REQUIRES_OK(ctx,
				GetInputTensor(ctx, 1, use_exclusive_lock_, &accum));
		Tensor accum_update;
		OP_REQUIRES_OK(ctx,
		...
		OP_REQUIRES(ctx, var.shape().IsSameSize(accum.shape()),
				errors::InvalidArgument(
	...
	void DoCompute(OpKernelContext* ctx)
	{
		...
		functor::ApplyAdadelta<Device, T>()(device, var.flat<T>(),
				accum.flat<T>(), accum_update.flat<T>(), lr.scalar<T>(),
				rho.scalar<T>(), epsilon.scalar<T>(), grad.flat<T>());
	}
};
...
public:
	explicit SparseApplyAdadeltaOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		const Device& device = ctx->template eigen_device<Device>();
		functor::ApplyProximalGradientDescent<Device, T>()(device,
				var.flat<T>(), alpha.scalar<T>(), l1.scalar<T>(),
...
public:
	explicit ApplyAdagradOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		const Device& device = ctx->template eigen_device<Device>();
		functor::ApplyAdagrad<Device, T>()(device, var.flat<T>(),
				accum.flat<T>(), lr.scalar<T>(), grad.flat<T>());
...
public:
	explicit ApplyProximalAdagradOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				...
				errors::InvalidArgument("l1 regularization strength is not a "
						"non-negative scalar: ", l1.shape().DebugString()));
		...
		OP_REQUIRES(ctx,
				...
				errors::InvalidArgument("l2 regularization strength is not a "
						"non-negative scalar: ", l2.shape().DebugString()));
		...
		const Device& device = ctx->template eigen_device<Device>();
		functor::ApplyProximalAdagrad<Device, T>()(device, var.flat<T>(),
				accum.flat<T>(), lr.scalar<T>(), l1.scalar<T>(), l2.scalar<T>(),
				grad.flat<T>());
		...
		const T& l2, const T& lr_power)
{
	...
	if (lr_power == static_cast<T>(-0.5)) {
		...
		quadratic = Eigen::numext::pow(accum, -lr_power) / lr
				+ static_cast<T>(2) * l2;
	}
...
public:
	explicit SparseApplyAdagradOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
...
public:
	explicit ApplyAdagradDAOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				GetInputTensor(ctx, 1, use_exclusive_lock_, &gradient_accum));
		Tensor gradient_squared_accum;
		OP_REQUIRES_OK(ctx,
				GetInputTensor(ctx, 2, use_exclusive_lock_,
						&gradient_squared_accum));
		OP_REQUIRES(ctx, var.IsInitialized(),
				errors::FailedPrecondition(
						"Attempting to use uninitialized variables: ",
						def().input(0)));
		OP_REQUIRES(ctx, gradient_accum.IsInitialized(),
				errors::FailedPrecondition(
						"Attempting to use uninitialized variables: ",
						def().input(1)));
		OP_REQUIRES(ctx, gradient_squared_accum.IsInitialized(),
				errors::FailedPrecondition(
						"Attempting to use uninitialized variables: ",
						def().input(2)));
		...
		const Device& device = ctx->template eigen_device<Device>();
		functor::ApplyAdagradDA<Device, T>()(device, var.flat<T>(),
				gradient_accum.flat<T>(), gradient_squared_accum.flat<T>(),
				lr.scalar<T>(), global_step.scalar<int64>()(), l1.scalar<T>(),
...
public:
	explicit SparseApplyAdagradDAOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
...
public:
	explicit ApplyFtrlOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		Tensor linear;
		OP_REQUIRES_OK(ctx,
				GetInputTensor(ctx, 2, use_exclusive_lock_, &linear));
		OP_REQUIRES(ctx, var.IsInitialized(),
				errors::FailedPrecondition(
						"Attempting to use uninitialized variables: ",
						def().input(0)));
		...
		OP_REQUIRES(ctx, var.shape().IsSameSize(accum.shape()),
				errors::InvalidArgument(
						...
						var.shape().DebugString(), " ",
						accum.shape().DebugString()));
		OP_REQUIRES(ctx, var.shape().IsSameSize(linear.shape()),
				errors::InvalidArgument(
		...
		functor::ApplyFtrl<Device, T>()(device, var.flat<T>(), accum.flat<T>(),
				linear.flat<T>(), grad.flat<T>(), lr.scalar<T>(),
				l1.scalar<T>(), l2.scalar<T>(), lr_power.scalar<T>());
...
public:
	explicit SparseApplyFtrlOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
...
public:
	explicit ApplyMomentumOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
...
public:
	explicit SparseApplyMomentumOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_nesterov", &use_nesterov_));
...
public:
	explicit ApplyAdamOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		functor::ApplyAdam<Device, T>()(device, var.flat<T>(), m.flat<T>(),
				v.flat<T>(), beta1_power.scalar<T>(), beta2_power.scalar<T>(),
...
public:
	explicit ApplyRMSPropOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		functor::ApplyRMSProp<Device, T>()(device, var.flat<T>(), ms.flat<T>(),
				mom.flat<T>(), lr.scalar<T>(), rho.scalar<T>(),
				momentum.scalar<T>(), epsilon.scalar<T>(), grad.flat<T>());
...
public:
	explicit ApplyCenteredRMSPropOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		const Device& device = ctx->template eigen_device<Device>();
		functor::ApplyCenteredRMSProp<Device, T>()(device, var.flat<T>(),
				mg.flat<T>(), ms.flat<T>(), mom.flat<T>(), lr.scalar<T>(),
				rho.scalar<T>(), momentum.scalar<T>(), epsilon.scalar<T>(),
				grad.flat<T>());
		MaybeForwardRefInputToRefOutput(ctx, 0, 0);
	}
...
public:
	explicit SparseApplyRMSPropOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
	}
