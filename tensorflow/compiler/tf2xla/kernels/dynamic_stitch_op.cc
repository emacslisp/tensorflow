...
public:
	explicit DynamicStitchOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		OP_REQUIRES(ctx, ctx->num_inputs() > 0,
				errors::InvalidArgument(
						"DynamicStitchOp: Must have some inputs"));
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		const TensorShape indices0_shape = XLAShapeToTensorShape(
				indices_input[0].shape());
		for (int input_num = 0; input_num < indices_input.size(); input_num++) {
			const TensorShape indices_shape = XLAShapeToTensorShape(
					indices_input[input_num].shape());
			const TensorShape& data_shape = data_shapes[input_num];
			OP_REQUIRES(ctx,
					TensorShapeUtils::StartsWith(data_shape, indices_shape),
			...
			OP_REQUIRES(ctx,
					input_num == 0
							|| SameExtraShape(data0_shape, indices0_shape,
									data_shape, indices_shape),
					errors::InvalidArgument("Need data[0].shape[",
							indices0_shape.dims(), ":] = data[", input_num,
							"].shape[", indices_shape.dims(),
							":], got data[0].shape = ",
							data0_shape.DebugString(), ", data[", input_num,
							"].shape = ", data_shape.DebugString(),
							", indices[0].shape = ",
							indices0_shape.DebugString(), ", indices[",
							input_num, "].shape = ",
		...
		for (int input_num = 0; input_num < indices.size(); input_num++) {
			for (int i = 0; i < indices[input_num].shape().dimensions(0); ++i) {
				max_index = std::max(max_index,
						xla::LiteralUtil::Get<int>(indices[input_num], { i }));
			}
		}
		...
		for (int input_num = 0; input_num < indices.size(); input_num++) {
			for (int i = 0; i < indices[input_num].shape().dimensions(0); ++i) {
				int index = xla::LiteralUtil::Get<int>(indices[input_num],
						{ i });
		...
		for (int input_num = 0; input_num < indices.size(); input_num++) {
			...
			if (new_shape == data_shapes[input_num]) {
				...
				input[input_num] = ctx->builder()->Reshape(handle,
						new_shape.dim_sizes());
			}
		}
		...
		std::vector<int64> slice_start(
				1 + data0_shape.dims() - indices0_shape.dims());
		std::vector<int64> slice_limit(
				1 + data0_shape.dims() - indices0_shape.dims());
		for (int d = indices0_shape.dims(); d < data0_shape.dims(); d++) {
			slice_limit[1 + d - indices0_shape.dims()] = data0_shape.dim_size(
					d);
		}
		...
		for (int index_num = 0; index_num < number_of_indices; index_num++) {
			...
			// the index.
			to_concat[index_num] = ctx->builder()->Slice(expression,
...
private:
	...
	static bool SameExtraShape(const TensorShape& data0_shape,
			const TensorShape& indices0, const TensorShape& data1_shape,
