		...
		const OpRegistryInterface* op_registry)
{
	...
	const Status status = op_registry->LookUpOpDef(op_name.ToString(),
			&op_def_);
	if (status.ok()) {
		Initialize();
	} else {
...
bool NodeDefBuilder::NextArgAvailable()
{
	if (op_def_ == nullptr) {
		...
		errors_.push_back(
				strings::StrCat("More Input() calls than the ",
...
{
	if (NextArgAvailable()) {
		Status status = fake_input(*op_def_, inputs_specified_, node_def_,
				this);
		if (!status.ok())
			errors_.push_back(status.error_message());
	}
...
void NodeDefBuilder::SingleInput(const OpDef::ArgDef* input_arg,
		StringPiece src_node, int src_index, DataType dt)
{
	...
	if (!input_arg->number_attr().empty()
		...
		errors_.push_back(
				strings::StrCat("Single tensor passed to '", input_arg->name(),
						"', expected list"));
		return;
	}
		...
		gtl::ArraySlice<NodeOut> src_list)
{
	...
	if (!input_arg->number_attr().empty()) {
		...
		errors_.push_back(
				strings::StrCat("List provided to input '", input_arg->name(),
...
void NodeDefBuilder::AddInput(StringPiece src_node, int src_index)
{
	if (src_node.empty()) {
		...
		errors_.push_back(
				strings::StrCat("Non-control input starting with ^: ",
						src_node));
	} else if (src_index > 0) {
		...
		DataType expected, DataType dt)
{
	if (!TypesCompatible(expected, dt)) {
		errors_.push_back(
				strings::StrCat("Input '", input_arg->name(), "' passed ",
...
void NodeDefBuilder::VerifyInputRef(const OpDef::ArgDef* input_arg, DataType dt)
{
	if (input_arg->is_ref() && !IsRefType(dt)) {
		errors_.push_back(
				strings::StrCat("Input '", input_arg->name(), "' passed ",
						DataTypeString(dt), " expected ref type"));
	}
}
		...
		if (errors_ptr->size() == 1) {
			if (op_def_ == nullptr) {
				return errors::InvalidArgument((*errors_ptr)[0],
						" while building NodeDef '", node_def_.name(), "'");
			}
			return errors::InvalidArgument((*errors_ptr)[0],
					" while building NodeDef '", node_def_.name(), "' using ",
					SummarizeOpDef(*op_def_));
		} else {
			return errors::InvalidArgument(errors_ptr->size(),
					" errors while building NodeDef '", node_def_.name(),
...
void NodeDefBuilder::CheckInconsistency(StringPiece attr_name,
		const AttrValue& found, const AttrValue& attr_value)
{
	if (!AreAttrValuesEqual(found, attr_value)) {
		errors_.push_back(
				strings::StrCat("Inconsistent values for attr '", attr_name,
						"' ", SummarizeAttrValue(found), " vs. ",
