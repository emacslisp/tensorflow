...
StatusOr<bool> ConvCanonicalization::Run(HloModule* module)
{
	...
	bool changed = false;
	for (HloInstruction* hlo : module->entry_computation()->MakeInstructionPostOrder()) {
		if (hlo->opcode() == HloOpcode::kConvolution
			...
			auto kernel_input_feature_dim =
					dnums.kernel_input_feature_dimension();
			auto kernel_output_feature_dim =
			...
			for (int i = 0; i < num_spatial_dims; ++i) {
				...
				new_input_dims[i + 1] = input->shape().dimensions(
						dnums.spatial_dimensions(i));
			}
			new_input_dim_order[num_dims - 1] = feature_dim;
			new_input_dims[num_dims - 1] = input->shape().dimensions(
					feature_dim);

			Shape new_input_shape = ShapeUtil::MakeShape(
					input->shape().element_type(), new_input_dims);
			HloInstruction* new_input =
					module->entry_computation()->AddInstruction(
							HloInstruction::CreateTranspose(new_input_shape,
									input, new_input_dim_order));
			...
			HloInstruction* new_conv =
					module->entry_computation()->AddInstruction(
							HloInstruction::CreateConvolve(new_conv_shape,
									new_input, new_kernel, hlo->window(),
									new_dnums));
			...
			// input reshape.
			TF_RETURN_IF_ERROR(
					module->entry_computation()->ReplaceWithNewInstruction(hlo,
							HloInstruction::CreateTranspose(hlo->shape(),
									new_conv,
