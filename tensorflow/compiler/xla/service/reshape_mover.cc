		...
		HloInstruction* instruction)
{
	if (IsElementwiseOfEquivalentReshapesOrTransposes(instruction)) {
		...
		auto new_elementwise = computation->AddInstruction(
		...
		case HloOpcode::kReshape:
			...
			break;
		case HloOpcode::kTranspose:
			new_reshape = HloInstruction::CreateTranspose(instruction->shape(),
					new_elementwise, old_reshape->dimensions());
			break;
		default:
			LOG(FATAL) << "Bad opcode";
		}
		TF_CHECK_OK(
				computation->ReplaceWithNewInstruction(instruction,
...
{
	return std::any_of(module->computations().begin(),
