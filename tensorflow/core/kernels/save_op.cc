...
public:
	explicit SaveOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
...
public:
	explicit SaveSlicesOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
...
public:
	explicit ShardedFilenameOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		out->scalar<string>()() = strings::Printf("%s-%05d-of-%05d",
				ctx->input(0).scalar<string>()().c_str(),
...
public:
	explicit ShardedFilespecOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		Tensor* out = nullptr;
		OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape( { }), &out));
		out->scalar<string>()() = strings::Printf("%s-\?\?\?\?\?-of-%05d",
				ctx->input(0).scalar<string>()().c_str(),
				ctx->input(1).scalar<int32>()());
	}
};
