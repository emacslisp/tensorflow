...
public:
	explicit HloModule(const string& name,
			...
			name_(name), entry_computation_(nullptr), has_entry_computation_handle_(
					true), entry_computation_handle_(entry_computation_handle)
	{
	}
	...
	HloInstruction* OutlineExpressionFromComputation(
			...
			const string& outlined_computation_name,
