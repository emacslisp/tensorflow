...
public:
	explicit SelectOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
			...
			const Tensor* then, const Tensor* else_)
	{
		...
		if (output->NumElements() > 0) {
			...
			func(ctx->eigen_device<Device>(), output->flat<T>(),
					cond->flat<bool>(), then->flat<T>(), else_->flat<T>());
		}
	}
...
struct BatchSelectFunctor<CPUDevice, T> {
			...
			typename TTypes<T>::ConstMatrix else_flat_outer_dims)
	{
		...
		const Eigen::DenseIndex all_but_batch = then_flat_outer_dims.dimension(
				1);

#if !defined(EIGEN_HAS_INDEX_LIST)
		Eigen::array < Eigen::DenseIndex, 2 > broadcast_dims { { 1,
				all_but_batch } };
		Eigen::Tensor<Eigen::DenseIndex, 2>::Dimensions reshape_dims { { batch,
				1 } };
#else
		...
		Assign(d, output_flat_outer_dims,
				cond_vec.reshape(reshape_dims).broadcast(broadcast_dims).select(
						then_flat_outer_dims, else_flat_outer_dims));
	}
};
