...
IrArray::Index::Index(llvm::Value* linear, const Shape& shape,
		...
		multidim_(ShapeUtil::Rank(shape)), linear_(linear), layout_(
				shape.layout()), dims_(shape.dimensions().begin(),
				shape.dimensions().end())
{
	CHECK(LayoutUtil::HasLayout(shape)) << "Shape "
...
IrArray::Index::Index(tensorflow::gtl::ArraySlice<llvm::Value*> multidim,
		...
		multidim_(multidim.begin(), multidim.end()), linear_(linear), layout_(
				shape.layout()), dims_(shape.dimensions().begin(),
				shape.dimensions().end())
{
...
IrArray::Index::Index(tensorflow::gtl::ArraySlice<llvm::Value*> multidim,
		...
		multidim_(multidim.begin(), multidim.end()), layout_(shape.layout()), dims_(
		...
		base_ptr_(base_ptr), shape_(&shape)
{
	...
	int depth = 0;
	element_type_ = llvm::cast < llvm::PointerType
			> (base_ptr_->getType())->getElementType();
	while (llvm::ArrayType* array_type = llvm::dyn_cast < llvm::ArrayType
			> (element_type_)) {
...
bool IrArray::Index::LinearValidOnShape(const Shape& a) const
{
	...
	*b.mutable_layout() = layout_;
	return linear_ != nullptr
			...
			&& LayoutUtil::Equal(
					ShapeUtil::StripDegenerateDimensions(a).layout(),
					ShapeUtil::StripDegenerateDimensions(b).layout());
}

IrArray::Index IrArray::Index::SourceIndexOfReshape(const Shape& output_shape,
		const Shape& input_shape, llvm::IRBuilder<>* builder) const
{
	const auto& target_index = *this;
	CHECK_EQ(target_index.size(), ShapeUtil::Rank(output_shape));
	std::vector<std::pair<int64, int64>> common_factors = CommonFactors(
	...
	for (ssize_t k = common_factors.size() - 2; k >= 0; --k) {
		llvm::Value* logical_linear_index = Index(
				tensorflow::gtl::ArraySlice<llvm::Value*>(multidim_,
						common_factors[k].second,
						common_factors[k + 1].second
								- common_factors[k].second)).Linearize(
				tensorflow::gtl::ArraySlice<int64>(
						AsInt64Slice(output_shape.dimensions()),
						common_factors[k].second,
						common_factors[k + 1].second
								- common_factors[k].second), builder);
		...
		for (int64 i = common_factors[k + 1].first - 1;
			...
			if (input_shape.dimensions(i) == 1) {
				...
				source_multidim_index[i] = builder->CreateURem(
						logical_linear_index, divisor);
			}
			logical_linear_index = builder->CreateUDiv(logical_linear_index,
					divisor);
		}
	}
...
IrArray::Index IrArray::Index::SourceIndexOfTranspose(const Shape& shape,
		const Shape& operand_shape,
		tensorflow::gtl::ArraySlice<int64> dimension_mapping,
		llvm::IRBuilder<>* builder) const
{
	std::vector<llvm::Value*> operand_multidim_index = Permute(
			dimension_mapping, multidim());
	if (linear() != nullptr
			&& ShapeUtil::TransposeIsBitcast(operand_shape, shape,
					dimension_mapping)) {
		...
		llvm::IRBuilder<>* builder) const
{
	...
	for (ssize_t i = size() - 1; i >= 0; --i) {
		llvm::Value* addend = builder->CreateMul((*this)[i],
				builder->getInt64(multiplier), "",
...
llvm::Value* IrArray::EmitArrayElementAddress(const IrArray::Index& index,
		llvm::IRBuilder<>* ir_builder, tensorflow::StringPiece name) const
{
	...
	if (!is_implicit_broadcast && index.LinearValidOnShape(*shape_)) {
		return ir_builder->CreateInBoundsGEP(
				ir_builder->CreateBitCast(base_ptr_,
						PrimitiveTypeToIrType(shape_->element_type(),
								ir_builder)->getPointerTo()),
		...
		llvm::IRBuilder<>* ir_builder) const
{
	...
	return IrArray(
			ir_builder->CreatePointerCast(base_ptr_,
					new_ir_type->getPointerTo()), new_shape);
}

/* static */IrArray::Index IrArray::BumpIndex(const Index& index,
		int64 which_dimension, int64 addend, llvm::IRBuilder<>* ir_builder)
{
	Index new_index = index;
	new_index[which_dimension] = ir_builder->CreateAdd(index[which_dimension],
			ir_builder->getInt64(addend), "", /*HasNUW=*/true,
			/*HasNSW=*/true);
	return new_index;
}
