...
public:
	explicit RandomCropOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const auto output_shape = TensorShape( { target_height, target_width,
				channels });
		...
		for (int y = 0; y < target_height; ++y) {
			for (int x = 0; x < target_width; ++x) {
				for (int c = 0; c < channels; ++c) {
					output_data(y, x, c) = input_data(y + offset_height,
							x + offset_width, c);
				}
			}
		}
