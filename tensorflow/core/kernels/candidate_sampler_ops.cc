...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape( {
				num_sampled_ }), &out_sampled_candidates));

		Tensor* out_true_expected_count = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output(1, TensorShape( {
				batch_size, num_true_ }), &out_true_expected_count));
		Tensor* out_sampled_expected_count = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output(2, TensorShape( {
				num_sampled_ }), &out_sampled_expected_count));

		gtl::ArraySlice<int64> true_candidate(
				true_classes.matrix<int64>().data(), batch_size * num_true_);
		...
		sampler_->SampleBatchGetExpectedCount(&random, unique_,
				&sampled_candidate, &sampled_expected_count, true_candidate,
				&true_expected_count);
...
public:
			...
			BaseCandidateSamplerOp(context)
	{
		...
		if (!vocab_file.empty()) {
			set_sampler(
					new FixedUnigramSampler(context->env(), range_max,
							vocab_file, distortion, num_reserved_ids,
							num_shards, shard));
		} else {
			set_sampler(
					new FixedUnigramSampler(range_max, unigrams, distortion,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const TensorShape& in_true_candidates_shape =
				in_true_candidates.shape();
		OP_REQUIRES(context,
		...
		for (int64 i = 0; i < batch_size; ++i) {
			for (int64 j = 0; j < num_true_; ++j) {
				const int64 true_candidate = in_true_candidates.matrix<int64>()(
						i, j);
		...
		OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape( {
				static_cast<int>(indices.size()) }), &out_indices));
		Tensor* out_ids = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output(1, TensorShape( {
				static_cast<int>(ids.size()) }), &out_ids));
		Tensor* out_weights = nullptr;
		OP_REQUIRES_OK(context, context->allocate_output(2, TensorShape( {
				static_cast<int>(weights.size()) }), &out_weights));
