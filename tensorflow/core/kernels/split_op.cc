...
public:
	explicit SplitOpBase(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
	...
	void ComputeEasyCases(OpKernelContext* context, bool* done)
	{
		...
		OP_REQUIRES(context, 0 <= split_dim && split_dim < input_shape.dims(),
		...
		OP_REQUIRES(context, input_shape.dim_size(split_dim) % num_split == 0,
				errors::InvalidArgument(
						"Number of ways to split should evenly divide the split "
								"dimension, but got split_dim ", split_dim,
						" (size = ", input_shape.dim_size(split_dim), ") ",
...
public:
	...
	explicit SplitOpCPU(OpKernelConstruction* c) :
			Base(c)
	{
	}
