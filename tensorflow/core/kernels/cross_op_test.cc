...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("cross_op", "Cross").Input(FakeInput(DT_FLOAT)).Input(
						FakeInput(DT_FLOAT)).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
};
...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("cross_int_op", "Cross").Input(
						FakeInput(DT_INT32)).Input(FakeInput(DT_INT32)).Finalize(
						node_def()));
		TF_EXPECT_OK(InitOp());
	}
};
