...
public:
	static constexpr bool HasGet()
	{
		...
		return !std::is_same<Result, None>()
				&& Checker::template valid<Result>();
	}
};

// Defines HasGet() for a particular method, container, and checker. If
// HasGet()==true, provides Get() that delegates to the method.
template<typename M, typename Checker, typename C, bool /*has_get*/=
		HasGetHelper<M, Checker, C>::HasGet()>
struct Wrapper {
	static constexpr bool HasGet()
	{
		return false;
...
struct Wrapper<M, Checker, C, true> {
	...
	static decltype(M::Get(std::declval<C*>())) Get(C* v)
	{
		return M::Get(v);
	}
};
...
public:
	...
	ArraySliceImplBase(const ArraySliceImplBase& x, size_type pos,
			size_type len) :
...
public:
	...
	explicit ArraySliceImpl(const C& v) :
			ArraySliceImplBase<const T>(
					ContainerData<T, C>::Get(std::addressof(v)),
