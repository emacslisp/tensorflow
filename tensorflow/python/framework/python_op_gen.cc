...
bool IsPythonReserved(const string& s)
{
	static const std::set<string>* const kPythonReserved = new std::set<string>(
			...
			"and", "as", "assert", "break", "class", "continue", "def", "del",
					...
					"_op_def_lib", "text_format", "op_def_pb2",
					"op_def_library", "ops" });
...
{
	if (IsPythonReserved(s))
		return strings::StrCat(s, "_");
	return s;
}
		...
		bool is_output)
{
	if (!arg.number_attr().empty()) {
		...
		const string* original_arg = gtl::FindOrNull(inferred_attrs,
				arg.number_attr());
		string prefix;
		if (original_arg == nullptr) {
			...
			if (attr->has_minimum() && attr->minimum() > 0) {
				prefix = strings::StrCat("A list of at least ",
						attr->minimum());
			} else {
				prefix = "A list of";
			}
		...
		if (arg.type() != DT_INVALID) {
			...
			if (original_arg == nullptr) {
				...
				if (attr->has_allowed_values()) {
					...
					return strings::StrCat(prefix,
							" `Tensor` objects of the same type.");
				}
			} else {
				return strings::StrCat(prefix,
		...
		const string prefix =
				is_list ?
						strings::StrCat("A list of ", mutable_str,
								"`Tensor` objects") :
		...
		if (num_outs == 1) {
			...
			if (ConsumeEquals(&description)) {  // Skip the generated type info.
				strings::Appendf(&result, "%s",
						Indent(4, 4, description).c_str());
			} else {
				// Special case of one output, don't use the name of the output unless
				// there is no description.
				string desc =
						output_type_string.empty() ?
								kUnknownTensorType : output_type_string[0];
				if (desc == kUnknownTensorType) {
					...
					if (!description.empty()) {
						...
						desc = strings::StrCat(" The ",
								op_def.output_arg(0).name(), " `Tensor`.");
					}
				} else if (!description.empty()) {
					AppendWithinWidth(&desc, description,
				...
				if (ConsumeEquals(&description)) { // Skip the generated type info.
					...
					const string type =
							static_cast<size_t>(i) < output_type_string.size() ?
									output_type_string[i] : kUnknownTensorType;
					if (!description.empty()) {
...
static string GetPythonOp(const OpDef& op_def, bool is_hidden, string op_name)
{
	...
	for (int i = 0; i < op_def.input_arg_size(); ++i) {
		...
		if (!arg.type_attr().empty()) {
			gtl::InsertIfNotPresent(&inferred_attrs, arg.type_attr(),
					arg.name());
		...
		if (!arg.number_attr().empty()) {
			gtl::InsertIfNotPresent(&inferred_attrs, arg.number_attr(),
					arg.name());
		}
	}
	...
	attrs.reserve(
			args_no_default.size() - op_def.input_arg_size()
	...
	const bool has_args = args_no_default.size() + args_with_defaults.size()
			> 0;
	...
	string out_names_list = strings::StrCat("[\"",
			str_util::Join(out_names, "\", \""), "\"]");

	// Provide the output names as a Python list
	string lower_op_name_outputs = strings::StrCat("_", lower_op_name,
			"_outputs");
	...
	if (num_outs > 1) {
		const string tuple_type_prefix = strings::StrCat("_", op_def.name(),
				"Output = _collections.namedtuple(");
		const string tuple_type_suffix = strings::StrCat("\"", op_def.name(),
				"\", ", lower_op_name_outputs, ")");
		strings::Appendf(&result, "%s\n",
	...
	const string def_suffix = strings::StrCat(parameters, has_args ? ", " : "",
			"name=None):");
	...
	for (const string& name : attrs) {
		...
		static const char* const kAttrTypeName[][2] =
				{ { "string", "`string`" }, { "list(string)",
						...
						"list(int)", "list of `ints`" }, { "float", "`float`" },
						{ "list(float)", "list of `floats`" }, { "bool",
								"`bool`" }, { "list(bool)", "list of `bools`" },
						{ "type", "`tf.DType`" }, { "list(type)",
								"list of `tf.DTypes`" }, { "shape",
		...
		if (attr.has_minimum()) {
			if (attr.type() == "int") {
				...
				strings::StrAppend(&desc, " that has length `>= ",
						attr.minimum(), "`");
			}
		}
	...
	for (int i = 0; i < op_def.output_arg_size(); ++i) {
		output_type_string.push_back(
				ArgTypeName(op_def, op_def.output_arg(i), inferred_attrs,
						true));
	}
	...
	if (num_outs <= 1) {
		...
		string return_tuple = strings::StrCat("  return _", op_def.name(),
				"Output._make(result)\n");
		...
		bool require_shapes)
{
	...
	strings::Appendf(&result,
			R"("""Python wrappers around Brain.
