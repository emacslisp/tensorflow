...
ExpectedCalls CreateRetriableErrors(const string& method, int n)
{
	...
	for (int i = 0; i < n; i++) {
		expected_calls.emplace_back(
				std::make_tuple(method,
						errors::Unavailable(
								strings::StrCat("Retriable error #", i))));
	}
...
public:
	MockCallSequence(const ExpectedCalls& calls) :
			calls_(calls)
	{
	}
	...
	~MockCallSequence()
	{
		EXPECT_TRUE(calls_.empty()) << "Not all expected calls have been made, "
...
public:
	MockRandomAccessFile(const ExpectedCalls& calls) :
			calls_(calls)
	{
	}
...
public:
	MockWritableFile(const ExpectedCalls& calls) :
			calls_(calls)
	{
	}
...
public:
	MockFileSystem(const ExpectedCalls& calls) :
			calls_(calls)
	{
	}
	...
	Status GetMatchingPaths(const string& dir, std::vector<string>* result) override
	{
