...
private:
	...
	public:
		...
		{
			EXPECT_FALSE(returned_partial_)
...
public:
	RecordioTest() :
			reading_(false), readpos_(0), writer_(new RecordWriter(&dest_)), reader_(
					new RecordReader(&source_))
	{
	}
	...
	void FixChecksum(int header_offset, int len)
	{
		...
		uint32_t crc = crc32c::Value(&dest_.contents_[header_offset + 6],
				1 + len);
		crc = crc32c::Mask(crc);
