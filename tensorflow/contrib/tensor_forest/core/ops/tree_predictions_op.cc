...
public:
			...
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("valid_leaf_threshold",
						&valid_leaf_threshold_));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, node_per_class_weights.shape().dims() == 2,
				errors::InvalidArgument("node_pcw should be two-dimensional"));
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument("Number of nodes should be the same in "
		...
		if (!CheckTensorBounds(context, tree_thresholds))
			return;
		if (!CheckTensorBounds(context, node_per_class_weights))
			return;

		const int32 num_classes =
				static_cast<int32>(node_per_class_weights.shape().dim_size(1));
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, output_shape, &output_predictions));
		...
		for (int i = 0; i < num_data; i++) {
			...
			while (true) {
				...
				if (left_child == LEAF_NODE) {
					...
					tensorforest::GetParentWeightedMean(node_pcw(node_index, 0),
							node_pcw.data() + flat_leaf_index,
							node_pcw(parent, 0),
							node_pcw.data() + flat_parent_index,
				...
				node_index = left_child
						+ tensorforest::DecideNode(input_data,
								sparse_input_indices, sparse_input_values, i,
								feature, thresholds(node_index), input_spec_);
			}
		}
