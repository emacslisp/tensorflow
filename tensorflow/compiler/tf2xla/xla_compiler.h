...
public:
	...
	Status CompileGraph(string const& name, std::unique_ptr<Graph> graph,
			FunctionLibraryRuntime* flr, const std::vector<Argument>& args,
	...
	Status CompileSubComputation(FunctionLibraryRuntime* flr,
			const NameAttrList& fn_name_attrs, const xla::Shape& input_shape,
	...
	XlaCompilationDevice* device() const
	{
		return device_;
	}
	const DeviceMgr* device_mgr() const
	{
		return &device_mgr_;
	}
...
private:
	...
	Status CompileFunctionBody(FunctionLibraryRuntime* flr,
			const FunctionBody& function_body, const string& name,
