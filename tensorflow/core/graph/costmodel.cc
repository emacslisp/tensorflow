		...
		const StepStats& ss)
{
	...
	for (auto& ds : ss.dev_stats()) {
		for (auto& ns : ds.node_stats()) {
			...
			Ensure(global_id);
			int64 elapsed_micros = ns.op_end_rel_micros()
					- ns.op_start_rel_micros();
			count_[global_id]++;
...
void CostModel::SetNumOutputs(const Node* node, int num_outputs)
{
	...
	if (perslot->size() > 0) {
		...
		max_mem_usage->output_port_shape.resize(num_outputs,
				TensorShapeProto());
...
void CostModel::CheckInitialized(const Graph& graph) const
{
	for (const Node* n : graph.nodes()) {
		if (n->IsOp()) {
			CHECK(
					static_cast<size_t>(n->id()) < time_.size()
			...
			for (size_t i = 0; i < perslot.size(); i++) {
				CHECK_GE(perslot[i], Bytes(0))
						<< ": no size estimate for output# " << i << " of "
						<< n->DebugString();
			}
		}
	}
...
void CostModel::RecordMaxMemorySize(const Node* node, int output_slot,
		Bytes bytes, const TensorShapeProto& tensor_shape,
...
{
	CHECK(n->IsOp());
	VLOG(2) << "Node " << n->id() << ": " << n->name() << " type_string: "
			<< n->type_string();
		...
		CostGraphDef* cost_graph) const
{
	...
	for (const Node* n : graph->nodes()) {
		...
		for (int i = 0; i < n->num_outputs(); i++) {
			CostGraphDef::Node::OutputInfo* output_info =
					cnode->add_output_info();
