...
public:
	explicit StridedSliceOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				ValidateStridedSliceOp(&context->input(1), &context->input(2),
						context->input(3),
						ShapeReadWriteFromTensorShape(
								&context->input(0).shape()), begin_mask,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				ValidateStridedSliceOp(&context->input(1), &context->input(2),
						context->input(3),
						ShapeReadWriteFromTensorShape(&input_shape), begin_mask,
						end_mask, ellipsis_mask, new_axis_mask,
						shrink_axis_mask, &wrapped_processing_shape,
						&wrapped_final_shape, &is_identity, &is_simple_slice,
						&slice_dim0, &begin, &end, &strides));
		...
		Tensor* result = nullptr;
		OP_REQUIRES_OK(context,
				context->allocate_output(0, input_shape, &result));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				ValidateStridedSliceOp(&context->input(1), &context->input(2),
						context->input(3),
						ShapeReadWriteFromTensorShape(&old_lhs.shape()),
		...
		if (processing_shape.num_elements()) {
			...
			OP_REQUIRES(context, final_shape == input_shape,
					errors::Unimplemented("sliced l-value shape ",
							final_shape.DebugString(),
