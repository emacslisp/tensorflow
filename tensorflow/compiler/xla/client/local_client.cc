...
public:
	...
	~StreamManager()
	{
		backend_->ReleaseStream(std::move(stream_));
	}
...
LocalExecutable::LocalExecutable(std::unique_ptr<Executable> executable,
		...
		executable_(std::move(executable)), backend_(backend), build_device_ordinal_(
				device_ordinal), build_options_(build_options)
{
}
		...
		if (!computation_layout.parameter_layout(i).MatchesLayoutInShape(
			...
			return InvalidArgument(
					...
					ShapeUtil::HumanString(
							computation_layout.parameter_layout(i).shape()).c_str(),
		...
		const ExecutableRunOptions& options)
{
	...
	if (options.stream() == nullptr) {
		TF_ASSIGN_OR_RETURN(acquired_stream,
				StreamManager::AcquireStream(backend_,
						options.device_ordinal()));
	...
	if (!computation_layout.result_layout().MatchesLayoutInShape(
		...
		return InvalidArgument(
				"result buffer does not match shape or layout of computation result: "
						"expected %s, got %s",
				ShapeUtil::HumanString(
						computation_layout.result_layout().shape()).c_str(),
	...
	for (const ShapedBuffer* argument : arguments) {
		TF_RETURN_IF_ERROR(
				LiteralFromShapedBuffer(*argument,
						session_module->add_arguments()));
	}
	return tensorflow::Status::OK();
}

tensorflow::Status LocalExecutable::RecordResult(const ShapedBuffer* result,
		SessionModule* session_module)
{
		...
		const ShapedBuffer& shaped_buffer, Literal* literal)
{
	...
	return backend_->transfer_manager()->TransferLiteralFromDevice(executor,
			shaped_buffer.buffer( { }), shaped_buffer.shape(),
...
tensorflow::Status LocalClient::ExecuteLocally(const Computation& computation,
		const tensorflow::gtl::ArraySlice<const ShapedBuffer*> arguments,
...
StatusOr<std::vector<std::unique_ptr<AotCompilationResult>>>LocalClient::CompileAheadOfTime(
		...
		const ExecutableBuildOptions& options)
{
	int device_ordinal =
			options.device_ordinal() == -1 ?
					default_device_ordinal() : options.device_ordinal();
	TF_ASSIGN_OR_RETURN(std::unique_ptr < Executable > executable,
			local_service_->CompileExecutable(computation.handle(),
					argument_layouts, options.result_layout(), device_ordinal,
					options.has_hybrid_result()));
	return WrapUnique(
			new LocalExecutable(std::move(executable),
					local_service_->mutable_backend(), device_ordinal, options));
}
