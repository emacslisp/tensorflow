...
// just call the function directly with no multithreading.
	...
	thread_pool->ParallelFor(total_elements, element_cost,
...
struct PerCacheLineParameters {
	...
	PerCacheLineParameters(const PerCacheLineParameters<T1>& other) :
			cache_line_start(other.cache_line_start), input_top_row_start(
					other.input_top_row_start), input_bottom_row_start(
					other.input_bottom_row_start), y_lerp(other.y_lerp)
	{
	}
...
template<class T1>
struct SampleRect {
	EIGEN_ALWAYS_INLINE SampleRect(const T1* in_top_left,
			const T1* in_top_right, const T1* in_bottom_left,
			const T1* in_bottom_right) :
			top_left(in_top_left), top_right(in_top_right), bottom_left(
					in_bottom_left), bottom_right(in_bottom_right)
	{
	}

	EIGEN_ALWAYS_INLINE
	T1 BilinearSample(int channel, T1 x_lerp, T1 y_lerp) const
	{
		const T1 top = top_left[channel]
				+ (top_right[channel] - top_left[channel]) * x_lerp;
...
struct PerCachePixelParameters {
	...
	PerCachePixelParameters(const PerCachePixelParameters<T1>& other) :
			cache_line_pixel(other.cache_line_pixel), left_x_index(
					other.left_x_index), right_x_index(other.right_x_index), x_lerp(
					other.x_lerp)
	{
	}
...
public:
	void operator()(OpKernelContext* context, const Tensor& input,
			...
			int filter_count, int stride_rows, int stride_cols, Padding padding,
			T3* output_data, int output_height, int output_width,
			const ImageResizerState& st, int top_padding, int bottom_padding,
			int left_padding, int right_padding, int pad_offset)
	{
		...
		if ((output_width <= 0) || (output_height <= 0)) {
			LOG(WARNING)
					<< "Conv2D was called with bad output width or height: "
					<< output_width << ", " << output_height;
			return;
		}
		OP_REQUIRES(context,
				((SampleMode == NEAREST) || (SampleMode == BILINEAR)),
				errors::InvalidArgument("Bad sample mode passed in",
						SampleMode));
		...
		if (padding == VALID) {
			filter_left_offset = ((output_width - 1) * stride_cols
					+ filter_width - padded_width + 1) / 2;
			filter_top_offset = ((output_height - 1) * stride_rows
					+ filter_height - padded_height + 1) / 2;
		} else {
			filter_left_offset = ((output_width - 1) * stride_cols
					+ filter_width - padded_width) / 2;
			filter_top_offset = ((output_height - 1) * stride_rows
					+ filter_height - padded_height) / 2;
		}
		...
		const int filter_value_count = filter_width * filter_height
				* input_depth;

		OP_REQUIRES(context, (filter_value_count * sizeof(T1)) <= kMaxChunkSize,
				errors::InvalidArgument("Im2Col patch too large for buffer"));
		const size_t patches_per_chunk = kMaxChunkSize
				/ (filter_value_count * sizeof(T1));
		...
		OP_REQUIRES_OK(context,
				context->resource_manager()->LookupOrCreate("Conv2d",
						"im2col_buffer", &im2col_buffer_resource, creator));
		...
		task_params.cache_end_x = (((output_width - 1) * stride_cols)
				- filter_left_offset) + filter_width;
		task_params.cache_line_width = task_params.cache_end_x
				- task_params.cache_start_x;
		task_params.cache_height = kResizeCacheSize
				/ (task_params.cache_line_width * input_depth);
		const int needed_resize_cache_count = filter_height
				* task_params.cache_line_width * input_depth;
		OP_REQUIRES(context,
		...
		std::function<Status(Im2ColBufferResource<T1, kResizeCacheSize>**)> resize_creator =
		...
		OP_REQUIRES_OK(context,
				context->resource_manager()->LookupOrCreate("Conv2d",
						"resize_cache", &resize_cache_resource,
						resize_creator));
		...
		for (int batch = 0; batch < input_batches; ++batch) {
			task_params.input_batch_start = input_data
					+ (batch * input_height * task_params.input_width
							* input_depth);
			const int in_y_end = ((output_height * stride_rows)
					- filter_top_offset) + filter_height;
			for (int out_y = 0; out_y < output_height; ++out_y) {
				const int in_y_origin = (out_y * stride_rows)
						- filter_top_offset;
				const int cache_start_y = std::max(in_y_origin,
						end_cached_lines);
				const int cache_end_y = std::min(in_y_end,
						std::max((in_y_origin + task_params.cache_height),
								end_cached_lines));
				if (end_cached_lines < (in_y_origin + filter_height)) {
					...
					FusedConvParallelFor(context, cache_start_y, cache_end_y,
				...
				for (int out_x = 0; out_x < output_width; ++out_x) {
					const int in_x_origin = (out_x * stride_cols)
							- filter_left_offset;
					const int patch_index = (batch * output_width
							* output_height) + (out_y * output_width) + out_x;
					const int patch_index_within_chunk = patch_index
							% patches_per_chunk;
					T1* im2col_patch_start = im2col_buffer
					...
					for (int filter_y = 0; filter_y < filter_height;
						...
						if (conv_in_y < 0) {
							...
							cache_index_y = conv_in_y
									% task_params.cache_height;
						}
						T1* cache_line_start = task_params.resize_cache
								+ (cache_index_y * task_params.cache_line_width
										* task_params.input_depth);
						T1* cache_filter_row_start = cache_line_start
								+ ((in_x_origin - task_params.cache_start_x)
										* task_params.input_depth);
						std::copy_n(cache_filter_row_start,
								(filter_width * task_params.input_depth),
								im2col_row_start);
					}
					const bool is_last_in_chunk = (patch_index_within_chunk
							== (patches_per_chunk - 1));
					const bool is_last_overall = ((batch == (input_batches - 1))
					...
					if (is_last_in_chunk || is_last_overall) {
						...
						// matrix.
						const int how_many_patches = patch_index_within_chunk
								+ 1;
						...
						const size_t start_patch_index = patch_index
								- (how_many_patches - 1);
						T3* chunk_output_data = output_data
								+ (start_patch_index * filter_count);
						TGemmFunctor gemm_functor;
						gemm_functor(context, m, n, k, im2col_buffer, lda,
								filter_data, ldb, chunk_output_data, ldc);
					}
				}
			}
		}
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const int fixed_dims =
				(allow_legacy_scalars() && dims == 0
						&& paddings.dim_size(0) == 1) ? 1 : dims;
		OP_REQUIRES(context, fixed_dims == paddings.dim_size(0),
				errors::InvalidArgument(
		...
		for (int d = 0; d < dims; ++d) {
			const int32 before = paddings_matrix(d, 0); // Pad before existing elements.
			const int32 after = paddings_matrix(d, 1); // Pad after exisitng elements.
			...
			if (offset_ == 0) {  // SYMMETRIC mode.
				OP_REQUIRES(context,
						...
						errors::InvalidArgument("paddings must be no greater "
								"than the dimension size: ", before, ", ",
								after, " greater than ",
				...
				OP_REQUIRES(context,
						...
						errors::InvalidArgument("paddings must be less than"
								" the dimension size: ", before, ", ", after,
								" not less than ", resized_shape.dim_size(d)));
			}
			padded_shape.AddDim(before + resized_shape.dim_size(d) + after);
		}

		OP_REQUIRES(context,
				((paddings_matrix(0, 0) == 0) && (paddings_matrix(0, 1) == 0)),
				errors::InvalidArgument(
						"Fused mirror padding only support spatial padding, not batches: ",
						paddings.DebugString()));
		OP_REQUIRES(context,
				((paddings_matrix(3, 0) == 0) && (paddings_matrix(3, 1) == 0)),
				errors::InvalidArgument(
		...
		OP_REQUIRES(context,
				FastBoundsCheck(padded_rows_raw,
						std::numeric_limits<int>::max()),
		...
		OP_REQUIRES(context,
				FastBoundsCheck(padded_cols_raw,
						std::numeric_limits<int>::max()),
		...
		TensorShape out_shape = ShapeFromFormat(FORMAT_NHWC, batch, out_rows,
				out_cols, out_depth);
		...
		VLOG(2) << "FusedConv2D: " << name() << ", in_depth = " << in_depth
				<< ", padded_cols = " << padded_cols << ", resized_cols = "
				<< resized_cols << ", filter_cols = " << filter_cols
				<< ", padded_rows = " << padded_rows << ", resized_rows = "
				<< resized_rows << ", filter_rows = " << filter_rows
				<< ", stride_rows = " << stride_rows << ", stride_cols = "
				<< stride_cols << ", out_depth = " << out_depth << ", DoResize="
				<< DoResize;
		...
		conv_functor(context, input, batch, resized_rows, resized_cols,
				padded_rows, padded_cols, in_depth, filter.flat<T>().data(),
				filter_rows, filter_cols, out_depth, stride_rows, stride_cols,
				padding_, output->flat<T>().data(), out_rows, out_cols, st,
