...
private:
	...
	mutex mu_;condition_variable
	events_pending_ GUARDED_BY(mu_);

	void FlushAccumulatedTensors()
			...
			if (iu.bufrec.buf) {
				if (LogMemory::IsEnabled()) {
					LogMemory::RecordRawDeallocation(iu.bufrec.operation,
							iu.bufrec.step_id, iu.bufrec.buf, iu.bufrec.alloc,
							false);
				}
	...
	void QueueTensors(perftools::gputools::Stream* stream,
			TensorReferenceVector* tensors) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
