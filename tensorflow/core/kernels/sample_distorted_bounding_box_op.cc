...
bool GenerateRandomCrop(int original_width, int original_height,
		float min_relative_crop_area, float max_relative_crop_area,
		float aspect_ratio, SimplePhilox* random, Rectangle* crop_rect)
{
	...
	const float min_area = min_relative_crop_area * original_width
			* original_height;
	const float max_area = max_relative_crop_area * original_width
			* original_height;
	...
	if (lrintf(max_height * aspect_ratio) > original_width) {
		...
		max_height = static_cast<int>((original_width + 0.5 - kEps)
				/ aspect_ratio);
	}
...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("min_object_covered", &min_object_covered_));
		OP_REQUIRES(context, min_object_covered_ >= 0,
				errors::InvalidArgument(
						"Min object covered must be non-negative: ",
						min_object_covered_));

		OP_REQUIRES_OK(context,
				context->GetAttr("use_image_if_no_bounding_boxes",
						&use_image_if_no_bounding_boxes_));

		OP_REQUIRES_OK(context,
				context->GetAttr("aspect_ratio_range", &aspect_ratio_range_));
		OP_REQUIRES(context, aspect_ratio_range_.size() == 2,
				errors::InvalidArgument(
						"Aspect ratio range field must specify 2 dimensions"));

		OP_REQUIRES(context,
				aspect_ratio_range_[0] > 0 && aspect_ratio_range_[1] > 0,
				errors::InvalidArgument(
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("max_attempts", &max_attempts_));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const uint64 height_raw = internal::SubtleMustCopy(
				image_size.flat<T>()(0));
		const uint64 width_raw = internal::SubtleMustCopy(
				image_size.flat<T>()(1));
		OP_REQUIRES(context,
		...
		if (input_boxes.NumElements() > 0) {
			TTypes<float>::ConstMatrix boxes =
					input_boxes.flat_inner_dims<float>();
			for (int b = 0; b < boxes.dimension(0); ++b) {
				for (int i = 0; i < 4; ++i) {
					OP_REQUIRES(context,
							boxes(b, i) >= 0.0 && boxes(b, i) <= 1.0,
							errors::InvalidArgument(
									"All bounding box coordinates must "
											"be in [0.0, 1.0]: ", boxes(b, i)));
				}
		...
		for (int i = 0; i < max_attempts_; ++i) {
			const float sample_aspect_ratio = random.RandFloat()
					* (max_sample_aspect_ratio - min_sample_aspect_ratio)
					+ min_sample_aspect_ratio;

			if (GenerateRandomCrop(width, height, min_sample_area,
					max_sample_area, sample_aspect_ratio, &random,
					&crop_rect)) {
		...
		typename TTypes<float, 3>::Tensor bboxes_data =
				bboxes->tensor<float, 3>();
		...
		bboxes_data(0, 0, 0) = static_cast<float>(crop_rect.min_y_)
				/ static_cast<float>(height);
		bboxes_data(0, 0, 1) = static_cast<float>(crop_rect.min_x_)
				/ static_cast<float>(width);
		bboxes_data(0, 0, 2) = static_cast<float>(crop_rect.max_y_)
				/ static_cast<float>(height);
		bboxes_data(0, 0, 3) = static_cast<float>(crop_rect.max_x_)
				/ static_cast<float>(width);
