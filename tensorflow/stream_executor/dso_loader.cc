...
/* static */port::Status DsoLoader::GetCublasDsoHandle(void** dso_handle)
{
	return GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("cublas",
							GetCudaVersion()), GetCudaLibraryDirPath()),
			dso_handle);
}
...
/* static */port::Status DsoLoader::GetCudnnDsoHandle(void** dso_handle)
{
	...
	return GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("cudnn",
							GetCudnnVersion()), GetCudaLibraryDirPath()),
			dso_handle);
}
...
/* static */port::Status DsoLoader::GetCufftDsoHandle(void** dso_handle)
{
	return GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("cufft",
							GetCudaVersion()), GetCudaLibraryDirPath()),
			dso_handle);
}
...
/* static */port::Status DsoLoader::GetCurandDsoHandle(void** dso_handle)
{
	return GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("curand",
							GetCudaVersion()), GetCudaLibraryDirPath()),
			dso_handle);
}
...
/* static */port::Status DsoLoader::GetLibcudaDsoHandle(void** dso_handle)
{
	...
	port::Status status = GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("cuda", "1"),
					GetCudaDriverLibraryPath()), dso_handle);
...
/* static */port::Status DsoLoader::GetLibcuptiDsoHandle(void** dso_handle)
{
	return GetDsoHandle(
			FindDsoPath(
					port::Env::Default()->FormatLibraryFileName("cupti",
							GetCudaVersion()), GetCudaCuptiLibraryPath()),
			dso_handle);
}
...
/* static */port::Status DsoLoader::GetDsoHandle(port::StringPiece path,
		void** dso_handle, LoadKind load_kind)
{
	...
	port::Status s = port::Env::Default()->LoadLibrary(path_string.c_str(),
			dso_handle);
	if (!s.ok()) {
		...
		return port::Status(port::error::FAILED_PRECONDITION,
				port::StrCat("could not dlopen DSO: ", path, "; dlerror: ",
						s.error_message()));
	}
		...
		port::StringPiece runfiles_relpath)
{
	...
	string binary_directory = GetBinaryDirectory(
			true /* = strip_executable_name */);
	mutex_lock lock { GetRpathMutex() };
	for (const string& rpath : *GetRpaths()) {
		candidate = port::Join(StringPieces { binary_directory, rpath,
				library_name }, "/");
...
{
	static port::StatusOr<void*> result = FetchHandleResult(
			DsoLoader::GetCublasDsoHandle);
	return result;
}
...
{
	static port::StatusOr<void*> result = FetchHandleResult(
			DsoLoader::GetLibcuptiDsoHandle);
	return result;
}
