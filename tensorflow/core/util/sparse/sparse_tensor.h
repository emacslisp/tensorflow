...
public:
	...
	SparseTensor(Tensor ix, Tensor vals, const TensorShape& shape,
			...
			ix_(ix), vals_(vals), shape_(shape), order_(order.begin(),
					order.end()), dims_(GetDimsFromIx(ix))
	{
		CHECK_EQ(ix.dtype(), DT_INT64) << "indices must be type int64 but got: "
				<< ix.dtype();
		CHECK(TensorShapeUtils::IsVector(vals.shape()))
				<< "vals must be a vec, but got: "
		...
		CHECK_EQ(order.size(), dims_)
				<< "Order length must be SparseTensor rank.";
		CHECK_EQ(shape.dims(), dims_)
...
private:
	static int GetDimsFromIx(const Tensor& ix)
	{
		CHECK(TensorShapeUtils::IsMatrix(ix.shape()))
				<< "indices must be a matrix, but got: "
			...
			int n) const
	{
		...
		if (TF_PREDICT_FALSE(!valid || !increasing || !different)) {
			...
			for (int di = 0; di < dims_; ++di) {
				strings::StrAppend(&index, ix_t(n, di),
						di < dims_ - 1 ? "," : "]");
			}
			if (!valid) {
...
bool SparseTensor::ValidateAndInitializeToDense(Tensor* out, bool initialize)
{
	...
	CHECK_EQ(out->dtype(), DataTypeToEnum<T>::v()) << "Output must be type: "
			<< DataTypeToEnum<T>::v() << " but got: " << out->dtype();

	// Make sure the dense output is the same rank and has room
	// to hold the SparseTensor.
	const auto& out_shape = out->shape();
	if (shape_.dims() != out_shape.dims())
		return false;
	for (int d = 0; d < shape_.dims(); ++d) {
		if (shape_.dim_size(d) > out_shape.dim_size(d))
			return false;
	}
...
{
	if (!ValidateAndInitializeToDense<T>(out, initialize))
		return false;
...
template<typename T>
SparseTensor SparseTensor::Concat(const gtl::ArraySlice<SparseTensor>& tensors)
{
	...
	for (const SparseTensor& st : tensors) {
		...
		final_shape.set_dim(primary_dim,
				final_shape.dim_size(primary_dim)
		...
		const int split_dim, const int num_split)
{
	...
	CHECK(num_split > 0 && num_split <= split_dim_size)
			<< "num_split must be in "
					"the interval (0, " << split_dim_size << "]";
	CHECK(split_dim >= 0 && split_dim < num_dim) << "num_dim must be in "
			"the interval [0, " << num_dim << ")";
	...
	for (int i = 0; i < num_split; ++i) {
		...
		output_indices.emplace_back(DT_INT64, TensorShape( { num_values[i],
				num_dim }));
		output_values.emplace_back(DataTypeToEnum<T>::v(), TensorShape( {
				num_values[i] }));
	...
	for (int i = 0; i < input_tensor.indices().dim_size(0); ++i) {
		...
		for (int j = 0; j < num_dim; ++j) {
			...
			output_indices_t[slice_index](slice_dim, j) =
					(j == split_dim) ?
							GetDimensionInSlice(original_dim, split_size,
									residual) :
							original_dim;
		}
	}
