...
struct LaunchConvOp<CPUDevice, T> {
			...
			const Padding padding, Tensor* output)
	{
		functor::CuboidConvolution<CPUDevice, T>()(
				...
				input.tensor<T, 5>(), filter.tensor<T, 5>(), strides[2],
				strides[1], strides[0], BrainPadding2EigenPadding(padding));
	}
};
...
public:
	explicit Conv3DOp(OpKernelConstruction* context) :
			BinaryOp<T>(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		std::array<int64, 3> input_size = { { input.dim_size(1), input.dim_size(
				2), input.dim_size(3) } };
		std::array<int64, 3> filter_size = { { filter.dim_size(0),
				filter.dim_size(1), filter.dim_size(2) } };
		std::array<int64, 3> strides =
				{ { stride_[1], stride_[2], stride_[3] } };
		std::array<int64, 3> out, padding;

		OP_REQUIRES_OK(context,
				Get3dOutputSize(input_size, filter_size, strides, padding_,
						&out, &padding));
		...
		LaunchConvOp<Device, T>::launch(context, input, filter, strides,
				padding_, output);
	}
