...
HloComputation::HloComputation(const string& name, int parameter_count,
		...
		HloInstruction* root_instruction) :
		name_(name), root_instruction_(root_instruction), instruction_name_uniquer_(
				/*separator=*/".")
{
		...
		std::unique_ptr<HloInstruction> instruction)
{
	...
	instruction_iterators_[pinst] = instructions_.insert(instructions_.end(),
...
void HloComputation::set_root_instruction(HloInstruction* new_root_instruction)
{
	...
	CHECK(
			ShapeUtil::Compatible(new_root_instruction->shape(),
...
public:
	...
	static std::list<HloInstruction*> GetOrder(HloInstruction* root,
		...
		std::list<HloComputation*>* post_order)
{
	...
	for (auto& instruction : computation->instructions()) {
		for (auto& called_computation : instruction->MakeCalledComputationsSet()) {
			ComputeComputationPostOrder(called_computation, visited,
					post_order);
		}
	}
...
std::list<HloComputation*> HloComputation::MakeEmbeddedComputationsList() const
{
	tensorflow::gtl::FlatSet<HloComputation*> visited;
	std::list<HloComputation*> post_order;
...
{
	CHECK(
			HloInstruction::FusionKind::kConvBackwardFilter == fusion_kind
					|| HloInstruction::FusionKind::kConvBackwardInput
							== fusion_kind);
	HloInstruction* root = instructions_to_fuse.front();
	HloInstruction* fusion_instruction = AddInstruction(
			HloInstruction::CreateFusionForBackwardConvolution(root->shape(),
					fusion_kind, window, conv_dnums, root));
	FuseInstructionsInto(instructions_to_fuse, fusion_instruction);
	return fusion_instruction;
}
		...
		HloInstruction* instruction)
{
	...
	for (int64 i = 0; i < ShapeUtil::TupleElementCount(instruction->shape());
			++i) {
		HloInstruction* gte = AddInstruction(
		...
		if (ShapeUtil::IsTuple(gte->shape())) {
			...
			element_copy = AddInstruction(
					HloInstruction::CreateUnary(gte->shape(), HloOpcode::kCopy,
							gte));
		}
...
bool HloComputation::ReachabilityMap::IsReachable(const HloInstruction* a,
...
bool HloComputation::ReachabilityMap::IsConnected(const HloInstruction* a,
		const HloInstruction* b) const
{
	const int id_a = FindOrDie(ids_, a);
	const int id_b = FindOrDie(ids_, b);
...
std::unique_ptr<HloComputation::ReachabilityMap> HloComputation::ComputeTransitiveOperands() const
{
...
Status HloComputation::AcceptOrdered(DfsHloVisitor* visitor,
		const std::vector<const HloInstruction*>& order) const
{
	...
	for (const HloInstruction* instruction : order) {
		TF_RET_CHECK(instruction_iterators_.count(instruction) == 1)
				...
				<< " is not in computation " << name();
		TF_RET_CHECK(visited.count(instruction) == 0) << "Instruction "
				<< instruction->name() << " appears more than once in order";
