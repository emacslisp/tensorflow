...
public:
	...
	{
		const TensorShape concat_dim_tensor_shape = ctx->InputShape(
				axis_index_);
		OP_REQUIRES(ctx, IsLegacyScalar(concat_dim_tensor_shape),
		...
		for (int i = 0; i < N; ++i) {
			...
			output_concat_dim +=
					in_shape.dims() > 0 ? in_shape.dim_size(axis) : 1;
		}
...
public:
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, IsLegacyScalar(concat_dim_shape),
				errors::InvalidArgument(
						"Concat dim tensor should be a scalar integer, but got shape ",
		...
		for (int i = 0; i < N; ++i) {
			...
			for (int64 j = 0; j < dims; ++j) {
				if (j == axis) {
					...
					const int32 inp0_element = xla::LiteralUtil::Get<int>(
							inp0_literal, { j });
					const int32 inp_element = xla::LiteralUtil::Get<int>(
							inp_literal, { j });
					OP_REQUIRES(ctx, (inp0_element == inp_element),
							errors::InvalidArgument("input[", i, ",", j,
									"] mismatch: ", inp0_element, " vs. ",
									inp_element));
					out_vec(j) = 0;
				}
			}
