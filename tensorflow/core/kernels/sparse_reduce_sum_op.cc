...
ReduceDetails SparseTensorReduceHelper(const SparseTensor &sp,
		gtl::ArraySlice<int32> axes_slice, bool keep_dims)
{
	...
	// sorted as well.
	std::set_difference(perm.begin(), perm.end(), reduction_axes.begin(),
			reduction_axes.end(),
			std::inserter(reduction.group_by_dims,
					reduction.group_by_dims.begin()));
	...
	std::set_difference(perm.begin(), perm.end(),
			reduction.group_by_dims.begin(), reduction.group_by_dims.end(),
	...
	for (int64 i = 0; i < reduction_axes_flat.size(); i++) {
		...
		if (axis < -shape_t->NumElements() || axis >= shape_t->NumElements()) {
			return errors::InvalidArgument("Invalid reduction dimension ", axis,
					", for input with ", shape_t->NumElements(), " dimensions.");
		}
	}
...
public:
	explicit SparseReduceSumOp(OpKernelConstruction *ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext *ctx) override
	{
		...
		SparseTensor sp(tensor::DeepCopy(*indices_t),
				tensor::DeepCopy(*values_t), TensorShape(shape_vec));
		ReduceDetails reduction = SparseTensorReduceHelper(sp,
				reduction_axes_t->flat<int32>(), keep_dims_);

		Tensor *out_values;
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(0, reduction.reduced_shape, &out_values));
		...
		gtl::InlinedVector<int64, 8> output_strides(
				reduction.group_by_dims.size());
		if (!output_strides.empty()) {  // Do this iff we don't reduce all.
			output_strides.back() = 1;
			for (int d = output_strides.size() - 2; d >= 0; --d) {
				output_strides[d] = output_strides[d + 1]
						* shape_vec(reduction.group_by_dims[d + 1]);
			}
		}
...
public:
	explicit SparseReduceSumSparseOp(OpKernelConstruction *ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("keep_dims", &keep_dims_));
	}
	...
	void Compute(OpKernelContext *ctx) override
	{
		...
		OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape( { nnz,
				reduction.reduced_shape.dims() }), &out_indices_t));
		typename TTypes<int64>::Matrix out_indices_mat = out_indices_t->matrix<
				int64>();
		...
		Tensor *out_shape_t;
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(2,
						TensorShape( { reduction.reduced_shape.dims() }),
						&out_shape_t));
		auto out_shape_flat = out_shape_t->flat<int64>();
		auto out_dim_sizes = reduction.reduced_shape.dim_sizes();
		std::copy(out_dim_sizes.begin(), out_dim_sizes.end(),
				&out_shape_flat(0));
	}
