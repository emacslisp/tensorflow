...
protected:
	...
	{
		TF_EXPECT_OK(
				NodeDefBuilder("resize_area_op", "ResizeArea").Input(
						FakeInput(DT_FLOAT)).Input(FakeInput(DT_INT32)).Attr(
						"align_corners", false).Finalize(node_def()));
		TF_EXPECT_OK(InitOp());
	}
				...
				for (int64 x = 0; x < out_width; ++x) {
					...
					for (int64 i = y_start; i < y_end; ++i) {
						float scale_y =
								i < in_y ?
										(i + 1 > in_y1 ?
												height_scale : i + 1 - in_y) :
										(i + 1 > in_y1 ? in_y1 - i : 1.0);
						for (int64 j = x_start; j < x_end; ++j) {
							float scale_x =
									j < in_x ?
											(j + 1 > in_x1 ?
													width_scale : j + 1 - in_x) :
											(j + 1 > in_x1 ? in_x1 - j : 1.0);
							for (int64 c = 0; c < channels; ++c) {
#define BOUND(val, limit) std::min(((limit)-1ll), (std::max(0ll, (val))))
								sum_data(c) += static_cast<float>(input_data(b,
										BOUND(i, in_height), BOUND(j, in_width),
										c)) * scale_y * scale_x * scale;
#undef BOUND
							}
						}
					}
...
protected:
	...
	{
		const Tensor* input = SetRandomImageInput(TensorShape( { 1, in_height,
				in_width, channels }));
		AddInputFromArray<int32>(TensorShape( { 2 }), { target_height,
				target_width });

		TF_ASSERT_OK(RunOpKernel());
		std::unique_ptr<Tensor> expected(
				new Tensor(device_->GetAllocator(AllocatorAttributes()),
						DataTypeToEnum<float>::v(), TensorShape( { 1,
								target_height, target_width, channels })));
		ResizeAreaBaseline(input->tensor<float, 4>(),
		...
		for (int in_w : { 2, 4, 7, 20, 165 }) {
			for (int in_h : { 1, 3, 5, 8, 100, 233 }) {
				for (int target_height : { 1, 2, 3, 50, 113 }) {
					for (int target_width : { target_height, target_height / 2
							+ 1 }) {
						RunRandomTest(in_w, in_h, target_height, target_width,
								channels);
					}
				}
			}
