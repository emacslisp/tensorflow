...
protected:
	AdjustSaturationOpBase(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, channels == 3,
				errors::InvalidArgument(
						"input must have 3 channels but instead has ", channels,
						" channels."));
...
public:
	...
	void DoCompute(OpKernelContext* context, const ComputeOptions& options) override
	{
		...
		auto input_data = input->shaped<float, 2>(
				{ channel_count, kChannelSize });
		const float scale_h = scale->scalar<float>()();
		auto output_data = output->shaped<float, 2>( { channel_count,
				kChannelSize });
