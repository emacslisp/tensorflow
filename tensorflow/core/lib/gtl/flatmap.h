...
public:
	...
	FlatMap(const FlatMap& src) :
			rep_(src.rep_)
	{
	}

	template<typename InputIter>
	FlatMap(InputIter first, InputIter last, size_t N = 1, const Hash& hf =
			Hash(), const Eq& eq = Eq()) :
			FlatMap(N, hf, eq)
	{
		insert(first, last);
	...
	public:
		iterator() :
				b_(nullptr), end_(nullptr), i_(0)
		{
		}

		// Make iterator pointing at first element at or after b.
		explicit iterator(Bucket* b, Bucket* end) :
				b_(b), end_(end), i_(0)
		{
			SkipUnused();
		}
		...
		iterator(Bucket* b, Bucket* end, uint32 i) :
				b_(b), end_(end), i_(i)
		{
			FillValue();
		}
	...
	private:
		...
		value_type* val()
		{
			return reinterpret_cast<value_type*>(space_);
		}
		void FillValue()
		{
	...
	public:
		...
		explicit const_iterator(Bucket* start, Bucket* end) :
				rep_(start, end)
		{
		}
