		...
		filepattern_(filepattern), open_function_(std::move(open_function))
{
	...
	if (!s.ok()) {
		status_ = errors::InvalidArgument(
				"Unsuccessful TensorSliceReader constructor: "
						"Failed to get matching files on ", filepattern, ": ",
				s.ToString());
		return;
	}
	if (fnames_.empty()) {
		status_ = errors::NotFound(
				"Unsuccessful TensorSliceReader constructor: "
						"Failed to find any matching files for ", filepattern);
		return;
	}
	...
	if (preferred_shard == kLoadAllShards || fnames_.size() == 1
		...
		VLOG(1) << "Loading shard " << preferred_shard << " for "
				<< filepattern_;
...
void TensorSliceReader::LoadShard(int shard) const
{
	...
	status_ = CheckVersions(sts.meta().versions(), TF_CHECKPOINT_VERSION,
			TF_CHECKPOINT_VERSION_MIN_PRODUCER, "Checkpoint", "checkpoint");
	if (!status_.ok())
		return;
	for (const SavedSliceMeta& ssm : sts.meta().tensor()) {
		...
		for (const TensorSliceProto& tsp : ssm.slice()) {
			...
			status_ = RegisterTensorSlice(ssm.name(), ssm_shape, ssm.type(),
					fname, ss_slice, &tensors_);
			if (!status_.ok())
				return;
		}
	}
...
const TensorSliceSet* TensorSliceReader::FindTensorSlice(const string& name,
		...
		DataType* type) const
{
	...
	if (!tss && !all_shards_loaded_) {
		VLOG(1)
				<< "Did not find tensor in preferred shard, loading all shards: "
				<< name;
		LoadAllShards();
...
Status TensorSliceReader::GetTensor(const string& name,
...
TensorSliceReader::VarToShapeMap TensorSliceReader::GetVariableToShapeMap() const
{
