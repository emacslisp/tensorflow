...
namespace tfprof {
static const char* const kOptions[] = { "-max_depth", "-min_bytes",
		"-min_micros", "-min_params", "-min_float_ops", "-device_regexes",
		"-order_by", "-account_type_regexes", "-start_name_regexes",
		"-trim_name_regexes", "-show_name_regexes", "-hide_name_regexes",
		"-account_displayed_op_only", "-select", "-viz", "-dump_to_file", };

static const char* const kOrderBy[] = { "name", "bytes", "micros", "params",
		"float_ops", };

// Append Only.
static const char* const kShown[] = { "bytes", "micros", "params", "float_ops",
		"num_hidden_ops", "tensor_value", "device", "op_types", };

static const char* const kCmds[] = { "scope", "graph", "set", "help", };
...
public:
	...
	Options(int max_depth, tensorflow::int64 min_bytes,
			...
			max_depth(max_depth), min_bytes(min_bytes), min_micros(min_micros), min_params(
					min_params), min_float_ops(min_float_ops), device_regexes(
					device_regexes), order_by(order_by), account_type_regexes(
					account_type_regexes), start_name_regexes(
					start_name_regexes), trim_name_regexes(trim_name_regexes), show_name_regexes(
					show_name_regexes), hide_name_regexes(hide_name_regexes), account_displayed_op_only(
					account_displayed_op_only), select(select.begin(),
					select.end()), viz(viz), dump_to_file(dump_to_file)
	{
	}
