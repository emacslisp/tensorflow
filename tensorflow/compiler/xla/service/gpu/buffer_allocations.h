...
public:
	...
	private:
		std::map<BufferAllocation::Index, perftools::gputools::DeviceMemoryBase> registered_buffers_;
	};
...
private:
	BufferAllocations(BufferAllocation::Index buffer_count,
			...
			int device_ordinal, DeviceMemoryAllocator* memory_allocator) :
			buffers_(buffer_count), temp_buffer_base_(
					perftools::gputools::DeviceMemory<void*>(temp_buffer_base)), device_ordinal_(
					device_ordinal), memory_allocator_(memory_allocator)
	{
	}
