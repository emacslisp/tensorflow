...
public:
	...
	StubResource* resource() LOCKS_EXCLUDED(mu_) {
		mutex_lock lock(mu_);return resource_;
	}
...
protected:
			...
			const string& shared_name)
	{
		NodeDef node_def;
		TF_CHECK_OK(
				NodeDefBuilder(strings::StrCat("test-node", count_++),
						"StubResourceOp").Attr("code", code).Attr("shared_name",
						shared_name).Finalize(&node_def));
		Status status;
		std::unique_ptr<OpKernel> op(
				CreateOpKernel(DEVICE_CPU, &device_,
						device_.GetAllocator(AllocatorAttributes()), node_def,
