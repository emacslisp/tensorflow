		...
		llvm::Value* on_false, llvm::IRBuilder<>* ir_builder)
{
	...
	llvm::LoadInst* pred_value = ir_builder->CreateLoad(pred.GetBasePointer(),
			"load_predicate_value");
	llvm::Value* pred_cond = ir_builder->CreateICmpNE(pred_value,
	...
	for (int i = 0; i < ShapeUtil::TupleElementCount(select.GetShape()); ++i) {
		...
		llvm::Value* on_true_element_address = ir_builder->CreateInBoundsGEP(
				on_true, element_index);
		...
		llvm::Value* on_false_element_address = ir_builder->CreateInBoundsGEP(
				on_false, element_index);
		...
		llvm::Value* output_element_address = ir_builder->CreateInBoundsGEP(
				select.GetBasePointer(), element_index);
		ir_builder->CreateStore(
				ir_builder->CreateSelect(pred_cond, on_true_element,
						on_false_element,
						tensorflow::strings::Printf("select_output_element_%d",
								i).c_str()), output_element_address);
	}
}
		...
		llvm::IRBuilder<>* ir_builder)
{
	for (size_t i = 0; i < operands.size(); ++i) {
		ir_builder->CreateStore(
				...
				ir_builder->CreateInBoundsGEP(tuple.GetBasePointer(), {
						ir_builder->getInt64(0), ir_builder->getInt64(i) }));
	}
}

llvm::Value* EmitGetTupleElement(const Shape& target_shape, int64 index,
		int alignment, llvm::Value* operand, llvm::IRBuilder<>* ir_builder)
{
	llvm::Value* element_ptr = ir_builder->CreateInBoundsGEP(operand, {
			ir_builder->getInt64(0), ir_builder->getInt64(index) });
	...
	llvm::Value* ret_val = ir_builder->CreateBitCast(src_buffer,
