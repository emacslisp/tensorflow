...
Env::Env() :
		file_system_registry_(new FileSystemRegistryImpl)
{
}
...
Status ReadFileToString(Env* env, const string& fname, string* data)
{
	...
	if (!s.ok()) {
		...
		s = errors::Aborted("File ", fname, " changed while reading: ",
				file_size, " vs. ", result.size());
		data->clear();
...
public:
	...
	protobuf_int64 ByteCount() const override
	{
		return pos_;
	}
	Status status() const
		...
		const ::tensorflow::protobuf::Message& proto)
{
	...
	string serialized;
	if (!::tensorflow::protobuf::TextFormat::PrintToString(proto,
			&serialized)) {
