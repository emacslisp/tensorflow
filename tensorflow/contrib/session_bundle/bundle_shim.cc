		...
		const RegressionSignature& regression_signature)
{
	...
	internal::AddOutputToSignatureDef(
			regression_signature.output().tensor_name(), kRegressOutputs,
			&signature_def);
...
Status MaybeBuildPredictSignatureDef(MetaGraphDef* meta_graph_def)
{
	...
	// exist.
	if (!GetNamedSignature(kPredictInputs, *meta_graph_def, &input_signature).ok()
			|| !GetNamedSignature(kPredictOutputs, *meta_graph_def,
					&output_signature).ok()) {
	...
	if (input_signature.type_case() != Signature::TypeCase::kGenericSignature
			|| output_signature.type_case()
					!= Signature::TypeCase::kGenericSignature) {
		return Status(error::Code::INVALID_ARGUMENT,
	...
	const string signature_def_key =
			already_has_default_signature ?
					strings::StrCat(kDefaultServingSignatureDefKey,
							"_from_named") :
					kDefaultServingSignatureDefKey;
	(*meta_graph_def->mutable_signature_def())[signature_def_key] =
			signature_def;
	return Status::OK();
}
		...
		SavedModelBundle* saved_model_bundle)
{
	...
	TF_RETURN_IF_ERROR(
			LoadSessionBundleFromPathUsingRunOptions(session_options,
					run_options, session_bundle_export_dir, &session_bundle));
		...
		MetaGraphDef* meta_graph_def)
{
	...
	if (IsRegressionSignature(signature)) {
		...
		(*meta_graph_def->mutable_signature_def())[kDefaultServingSignatureDefKey] =
				BuildClassificationSignatureDef(
		...
		LOG(WARNING)
				<< "Default signature up-conversion to SignatureDef is only "
						...
						"not up-convert signature: " << signature.DebugString()
		...
		MetaGraphDef* meta_graph_def)
{
	...
	Status predict_signature_def_status = MaybeBuildPredictSignatureDef(
			meta_graph_def);
...
Status ConvertSessionBundleToSavedModelBundle(SessionBundle& session_bundle,
		SavedModelBundle* saved_model_bundle)
{
		...
		SavedModelBundle* saved_model_bundle)
{
	if (MaybeSavedModelDirectory(export_dir)) {
		...
		LOG(INFO)
				<< "Attempting to up-convert SessionBundle to SavedModelBundle "
						"in bundle-shim from: " << export_dir;
		return LoadSavedModelFromLegacySessionBundlePath(session_options,
				run_options, export_dir, saved_model_bundle);
	}
	return Status(error::Code::NOT_FOUND,
