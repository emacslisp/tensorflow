...
{
	stream_executor_ =
			PlatformUtil::GetStreamExecutors(local_client_->platform()).ValueOrDie()[local_client_->default_device_ordinal()];
	transfer_manager_ = TransferManager::GetForPlatform(
			local_client_->platform()).ValueOrDie();
}

std::unique_ptr<ScopedShapedBuffer> LocalClientTestBase::LiteralToScopedShapedBuffer(
		...
		const Literal& literal, int device_ordinal)
{
	...
	auto scoped_buffer = ScopedShapedBuffer::MakeScopedShapedBuffer(
			literal.shape(), GetOrCreateAllocator(local_client_->platform()),
			device_ordinal).ConsumeValueOrDie();
	// The creation of the scoped shaped buffer should allocate the buffer.
	CHECK(
			!scoped_buffer->buffer(/*index=*/{ }).is_null()
					|| ShapeUtil::HasZeroElements(literal.shape()));
	TF_CHECK_OK(
			transfer_manager_->TransferLiteralToDevice(stream_executor_,
					literal, scoped_buffer->mutable_buffer(/*index=*/{ })));
		...
		const ShapedBuffer& shaped_buffer, ShapeIndex* index, Literal* literal)
{
	...
	if (ShapeUtil::IsTuple(shape)) {
		*literal->mutable_shape() = shape;
		...
		ASSERT_IS_OK(
				transfer_manager_->TransferLiteralFromDevice(stream_executor_,
						shaped_buffer.buffer(*index), shape, shape, literal));
	}
}
...
std::unique_ptr<ScopedShapedBuffer> LocalClientTestBase::ShapedBufferToScopedShapedBuffer(
		std::unique_ptr<ShapedBuffer> shaped_buffer,
		DeviceMemoryAllocator* allocator)
{
	std::unique_ptr<ScopedShapedBuffer> scoped_buffer =
			ScopedShapedBuffer::MakeScopedShapedBuffer(shaped_buffer->shape(),
					allocator, shaped_buffer->device_ordinal()).ConsumeValueOrDie();
	...
	for (perftools::gputools::DeviceMemoryBase& memory_base : *scoped_buffer->mutable_buffers()) {
		TF_CHECK_OK(
				allocator->Deallocate(shaped_buffer->device_ordinal(),
						&memory_base));
	}
		...
		const LocalExecuteOptions& options)
{
	return ShapedBufferToScopedShapedBuffer(
			local_client_->ExecuteLocally(computation, arguments, options).ConsumeValueOrDie(),
			options.allocator());
}

void LocalClientTestBase::ExecuteLocally(const Computation& computation,
		tensorflow::gtl::ArraySlice<const ShapedBuffer*> arguments,
		ShapedBuffer* result)
{
	ExecuteLocally(computation, arguments, DefaultLocalExecuteOptions(),
			result);
}
