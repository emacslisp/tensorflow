...
int ShapeReadWriteFromTensorShape::dims() const
{
	return const_shape_->dims();
}
...
{
	DCHECK_NE(size, -1);
	DCHECK(shape_ != nullptr)
			<< "add_dim can only be called on non-const shape";
	shape_->AddDim(size);
}
...
Status ValidateStridedSliceOp(const Tensor* begin_tensor,
		const Tensor* end_tensor, const Tensor& strides_tensor,
		const ShapeReadWriteInterface& input_shape, int32 begin_mask_spec,
		int32 end_mask_spec, const int32 ellipsis_mask, int32 new_axis_mask,
		int32 shrink_axis_mask, ShapeReadWriteInterface* processing_shape,
		...
		gtl::InlinedVector<int64, 4>* strides)
{
	...
	StridedSliceSparseSpec sparse_spec = { strides_tensor.NumElements(), 0,
			begin_tensor, end_tensor, strides_tensor, begin_mask_spec,
	...
	StridedSliceDenseSpec dense_spec = { input_shape.dims(), 0 /* begin_mask */,
			0 /* end_mask */, false /* begin_valid */, false /* end_valid */,
			*begin, *end, *strides };
	...
	*is_simple_slice = true;
	for (int i = 0; i < input_shape.dims(); ++i) {
		...
		const std::array<int64, 2> masks = { { dense_spec.begin_mask & (1 << i),
				dense_spec.end_mask & (1 << i) } };
		const std::array<int64, 2> valid_range = { { stride_i > 0 ? 0 : -1,
				stride_i > 0 ? dim_i : dim_i - 1 } };

		auto canonical =
				[stride_i, i, dim_i, masks, valid_range](int64 x, int c) {
					if (masks[c]) {
		...
		if (dense_spec.begin_valid && dense_spec.end_valid) {
			...
			bool take_all_in_dimension = stride_i == 1 && begin_i == 0
					&& end_i == dim_i;
