...
const string& HostPlatform::Name() const
{
	return name_;
}
...
port::StatusOr<std::unique_ptr<StreamExecutor>> HostPlatform::GetUncachedExecutor(
		const StreamExecutorConfig& config)
{
	auto executor = port::MakeUnique<StreamExecutor>(this,
			new HostExecutor(config.plugin_config));
...
static void InitializeHostPlatform()
{
	...
	SE_CHECK_OK(
			gpu::MultiPlatformManager::RegisterPlatform(std::move(platform)));
}
