		...
		subgraph::NameIndex* name_index)
{
	for (const string& t : fed_outputs) {
		...
		if (iter == name_index->end()) {
			return errors::NotFound("FeedInputs: unable to find feed output ",
					t);
		}
		const Node* n = iter->second;
		DCHECK_EQ(n->name(), id.first);
		if (id.second >= n->num_outputs()) {
			return errors::InvalidArgument("FeedInputs: ", t,
					" should have output index < ", n->num_outputs());
		}

		Node* recv_node;
		TF_RETURN_IF_ERROR(
				NodeBuilder(strings::StrCat("_recv_", id.first, "_", id.second),
						"_Recv").Attr("tensor_type",
						BaseType(n->output_type(id.second))).Attr("tensor_name",
						t).Attr("send_device", device_info.name()).Attr(
						"recv_device", device_info.name()).Attr(
						"send_device_incarnation",
						static_cast<int64>(device_info.incarnation())).Attr(
						"client_terminated", true).Finalize(g, &recv_node));
		...
		std::vector<Node*>* fetch_nodes)
{
	...
	for (const string& t : fetch_outputs) {
		...
		if (n->num_outputs() == 0) {
			return errors::InvalidArgument("Tried to fetch data for '", t,
			...
			return errors::InvalidArgument("FetchOutputs ", t,
					": output index too large, must be < ", n->num_outputs());
		}
		...
		TF_RETURN_IF_ERROR(
				NodeBuilder(strings::StrCat("_send_", id.first, "_", id.second),
						"_Send").Input(n, id.second).Attr("tensor_name", t).Attr(
						"send_device", device_info.name()).Attr("recv_device",
						device_info.name()).Attr("send_device_incarnation",
						static_cast<int64>(device_info.incarnation())).Attr(
						"client_terminated", true).Finalize(g, &send_node));
		...
		const DeviceAttributes& device_info)
{
	...
	if (!fetch_outputs.empty()) {
		TF_RETURN_IF_ERROR(
				FetchOutputs(g, device_info, fetch_outputs, &name_index,
						&fetch_nodes));
	}
