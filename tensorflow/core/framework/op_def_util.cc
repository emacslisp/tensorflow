...
Status AllowedTypeValue(DataType dt, const OpDef::AttrDef& attr)
{
	...
	return errors::InvalidArgument("Value for attr '", attr.name(), "' of ",
			DataTypeString(dt), " is not in the list of allowed values: ",
			allowed_str);
}
...
Status AllowedStringValue(const string& str, const OpDef::AttrDef& attr)
{
	...
	return errors::InvalidArgument("Value for attr '", attr.name(), "' of \"",
			str, "\" is not in the list of allowed values: ", allowed_str);
}
	...
	if (attr.has_minimum()) {
		if (attr.type() == "int") {
			if (attr_value.i() < attr.minimum()) {
				return errors::InvalidArgument("Value for attr '", attr.name(),
						"' of ", attr_value.i(), " must be at least minimum ",
						attr.minimum());
			}
		} else {
			int length = -1;
			...
			if (length < attr.minimum()) {
				return errors::InvalidArgument("Length for attr '", attr.name(),
						"' of ", length, " must be at least minimum ",
						attr.minimum());
			}
		}
	}
	...
	if (attr.has_allowed_values()) {
		if (attr.type() == "type") {
			...
			for (int dt : attr_value.list().type()) {
				TF_RETURN_IF_ERROR(
						AllowedTypeValue(static_cast<DataType>(dt), attr));
			}
		...
		bool output, std::set<string>* names)
{
	...
	if (!arg.number_attr().empty()) {
		...
		VALIDATE(attr != nullptr, "No attr with name '", arg.number_attr(), "'",
				suffix);
		VALIDATE(attr->type() == "int", "Attr '", attr->name(),
				"' used as length", suffix, " has type ", attr->type(),
				" != int");
		VALIDATE(attr->has_minimum(), "Attr '", attr->name(),
				"' used as length", suffix, " must have minimum");
		VALIDATE(attr->minimum() >= 0, "Attr '", attr->name(),
				"' used as length", suffix, " must have minimum >= 0");
		VALIDATE(arg.type_list_attr().empty(),
				"Can't have both number_attr and type_list_attr", suffix);
		VALIDATE(
				(arg.type() != DT_INVALID ? 1 : 0)
						+ (!arg.type_attr().empty() ? 1 : 0) == 1,
...
Status ValidateOpDef(const OpDef& op_def)
{
	...
	if (!StringPiece(op_def.name()).starts_with("_")) {
		VALIDATE(
				Scanner(op_def.name()).One(Scanner::UPPERLETTER).Any(
						Scanner::LETTER_DIGIT).Eos().GetResult(),
	...
	for (const auto& attr : op_def.attr()) {
		...
		if (attr.has_allowed_values()) {
			const string list_type =
					is_list ?
							attr.type() :
							strings::StrCat("list(", attr.type(), ")");
			TF_RETURN_WITH_CONTEXT_IF_ERROR(
					AttrValueHasType(attr.allowed_values(), list_type),
					" for attr '", attr.name(), "' in Op '", op_def.name(),
					"'");
		}
...
Status CheckOpDeprecation(const OpDef& op_def, int graph_def_version)
{
	if (op_def.has_deprecation()) {
		...
		if (graph_def_version >= dep.version()) {
			return errors::Unimplemented("Op ", op_def.name(),
					" is not available in GraphDef version ", graph_def_version,
					". It has been removed in version ", dep.version(), ". ",
					dep.explanation(), ".");
		} else {
...
string ComputeArgSignature(
		...
		std::vector<bool>* ref, bool names)
{
	string s;
	bool add_comma = false;
	for (const OpDef::ArgDef& arg : args) {
		if (!arg.type_list_attr().empty()) {
			const OpDef::AttrDef* old_attr = gtl::FindPtrOrNull(old_attrs,
					arg.type_list_attr());
			if (old_attr) {
				...
				const OpDef::AttrDef* new_attr = gtl::FindPtrOrNull(new_attrs,
						arg.type_list_attr());
				...
				for (int i = 0; i < type_list.size(); ++i) {
					...
					strings::StrAppend(&s,
							DataTypeString(
									static_cast<DataType>(type_list.Get(i))));
					ref->push_back(arg.is_ref());
				}
			}
		} else {
			...
			if (!arg.number_attr().empty()) {
				...
				if (old_attr) {
					...
					// value for the attr from new instead.
					const OpDef::AttrDef* new_attr = gtl::FindPtrOrNull(
							new_attrs, arg.number_attr());
  ...
  if (!(CONDITION)) {                                                       \
	...
	for (const auto& old_attr : old_op.attr()) {
		...
		VALIDATE(old_attr.type() == new_attr->type(), "Attr '", old_attr.name(),
				...
				"'");
		VALIDATE(!MoreRestrictive(old_attr, *new_attr), "Attr '",
				old_attr.name(),
	...
	const string old_out_sig = ComputeArgSignature(old_op.output_arg(),
			old_attrs, new_attrs, &old_out_ref, true /* names */);
	const string new_out_sig = ComputeArgSignature(new_op.output_arg(),
			old_attrs, new_attrs, &new_out_ref, true /* names */);
