...
public:
	explicit PadOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				TensorShapeUtils::IsMatrix(pad_shape)
						&& pad_shape.dim_size(1) == 2,
				errors::InvalidArgument(
						"paddings must be a matrix with 2 columns: ",
						pad_shape.DebugString()));
		const int fixed_dims =
				(allow_legacy_scalars() && dims == 0
						&& pad_shape.dim_size(0) == 1) ? 1 : dims;
		OP_REQUIRES(ctx, fixed_dims == pad_shape.dim_size(0),
				errors::InvalidArgument(
		...
		OP_REQUIRES_OK(ctx,
				ctx->ConstantInputReshaped(1, { fixed_dims, 2 }, &pad_literal));
