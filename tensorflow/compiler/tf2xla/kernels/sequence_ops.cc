...
Status CreateRangeTensor(const xla::Literal& start_literal,
		const xla::Literal& limit_literal, const xla::Literal& delta_literal,
		Tensor* output)
{
	...
	if (delta > 0) {
		if (start > limit) {
			return errors::InvalidArgument(
					"Requires start <= limit when delta > 0: ", start, "/",
					limit);
		}
	} else {
		if (start < limit) {
...
public:
	explicit RangeOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
...
public:
	explicit LinSpaceOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		default:
			ctx->SetStatus(
					errors::InvalidArgument("Invalid argument type ",
							DataTypeString(type)));
			return;
		}
