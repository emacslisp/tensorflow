...
{
	return internal::ExampleFeature(name, example).mutable_int64_list()->mutable_value();
}
...
protobuf::RepeatedField<float>* GetFeatureValues<float>(const string& name,
		Example* example)
{
	return internal::ExampleFeature(name, example).mutable_float_list()->mutable_value();
}
...
protobuf::RepeatedPtrField<string>* GetFeatureValues<string>(const string& name,
		Example* example)
{
	return internal::ExampleFeature(name, example).mutable_bytes_list()->mutable_value();
}
