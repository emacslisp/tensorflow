...
typedef std::unique_ptr<TF_Tensor, decltype(&TF_DeleteTensor)> unique_tensor_ptr;
...
static void TestEncodeDecode(int line, const std::vector<string>& data)
{
	...
	for (const std::vector<tensorflow::int64>& dims : std::vector<
			std::vector<tensorflow::int64>> { { n }, { 1, n }, { n, 1 }, { n
			/ 2, 2 } }) {
		// Create C++ Tensor
...
{
	TF_OperationDescription* desc = TF_NewOperation(graph, "Placeholder",
			"feed");
...
bool GetNodeDef(TF_Operation* oper, NodeDef* node_def)
{
	...
	EXPECT_EQ(TF_OK, TF_GetCode(s)) << TF_Message(s);
	if (ret)
		ret = node_def->ParseFromArray(buffer->data, buffer->length);
	TF_DeleteBuffer(buffer);
	TF_DeleteStatus(s);
	return ret;
}
		...
		tensorflow::AttrValue* attr_value, TF_Status* s)
{
	...
	bool ret = TF_GetCode(s) == TF_OK;
	if (ret)
		ret = attr_value->ParseFromArray(buffer->data, buffer->length);
	TF_DeleteBuffer(buffer);
	return ret;
}
...
public:
	...
	void Run(TF_Status* s)
	{
		...
		const TF_Output* outputs_ptr =
				outputs_.empty() ? nullptr : &outputs_[0];
		...
		TF_SessionRun(session_, nullptr, inputs_ptr, input_values_ptr,
				inputs_.size(), outputs_ptr, output_values_ptr, outputs_.size(),
				targets_ptr, targets_.size(), nullptr, s);
...
TF_Tensor* Int8Tensor(const int64_t* dims, int num_dims, const char* values)
{
	...
	TF_Tensor* t = TF_AllocateTensor(TF_INT8, dims, num_dims,
...
void StringVectorToArrays(const std::vector<string>& v,
		std::unique_ptr<const void*[]>* ptrs, std::unique_ptr<size_t[]>* lens)
{
...
TEST_F(CApiAttributesTest, Shape) {
  ...
  auto oper = TF_FinishOperation(desc, s_);
  ASSERT_EQ(TF_OK, TF_GetCode(s_)) << TF_Message(s_);
  EXPECT_TF_META("v", -1, TF_ATTR_SHAPE, -1);TF_OperationGetAttrShape(oper, "v", nullptr, 10, s_);
  EXPECT_EQ(TF_OK, TF_GetCode(s_)) << TF_Message(s_);
