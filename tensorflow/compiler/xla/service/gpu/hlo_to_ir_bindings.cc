		...
		tensorflow::gtl::ArraySlice<const HloInstruction*> non_io_hlos)
{
	...
	for (const auto* io_hlo : io_hlos) {
		if (!already_bound_for_this_function.count(io_hlo)) {
			if (!is_nested_
					&& io_hlo->opcode() == HloOpcode::kGetTupleElement) {
				BindHloToIrValue(*io_hlo,
						EmitGetTupleElement(io_hlo, &*arg_iter));
			} else {
	...
	for (auto* non_io_hlo : non_io_hlos) {
		...
		if (non_io_hlo->opcode() == HloOpcode::kGetTupleElement) {
			if (!is_nested_) {
				...
				const BufferAllocation* allocation =
						buffer_assignment_->GetUniqueTopLevelAllocation(
								LatestNonGteAncestor(non_io_hlo)).ConsumeValueOrDie();
				// We are not in a nested context, so check non-thread-local allocation.
				CHECK(!allocation->is_thread_local());
				int64 offset = temp_buffer_offsets_->GetOffset(
						allocation->index());
		...
		const BufferAllocation* allocation =
				buffer_assignment_->GetUniqueTopLevelAllocation(non_io_hlo).ConsumeValueOrDie();
		if (allocation->is_thread_local()) {
			llvm::Type* pointee_type = llvm_ir::ShapeToIrType(
					non_io_hlo->shape(), ir_builder_);
			BindHloToIrValue(*non_io_hlo,
					ir_builder_->CreateAlloca(pointee_type));
		} else {
		...
		llvm::Value* base_ptr)
{
	...
	if (gte->operand(0)->opcode() != HloOpcode::kGetTupleElement) {
		return llvm_ir::EmitGetTupleElement(gte->shape(), gte->tuple_index(), /*alignment=*/
				1, GetTypedIrValue(*gte->operand(0), base_ptr), ir_builder_);
	}
	return llvm_ir::EmitGetTupleElement(gte->shape(), gte->tuple_index(), /*alignment=*/
			1, EmitGetTupleElement(gte->operand(0), base_ptr), ir_builder_);
}
		...
		llvm::Value* ir_value)
{
	...
	if (llvm::isa < llvm::GlobalVariable > (ir_value)) {
		...
		typed_ir_value = ir_builder_->CreateBitCast(ir_value,
				pointee_type->getPointerTo());
	}
