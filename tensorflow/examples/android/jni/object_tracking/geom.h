...
public:
	...
	BoundingBox(const BoundingBox& bounding_box) :
			left_(bounding_box.left_), top_(bounding_box.top_), right_(
					bounding_box.right_), bottom_(bounding_box.bottom_)
	{
		SCHECK(left_ < right_, "Bounds out of whack! %.2f vs %.2f!", left_,
				right_);
		SCHECK(top_ < bottom_, "Bounds out of whack! %.2f vs %.2f!", top_,
				bottom_);
	}
	...
	BoundingBox(const Point2f& point1, const Point2f& point2) :
			left_(MIN(point1.x, point2.x)), top_(MIN(point1.y, point2.y)), right_(
					MAX(point1.x, point2.x)), bottom_(MAX(point1.y, point2.y))
	{
	}
	...
	inline BoundingBox Union(const BoundingBox& that) const
	{
		return BoundingBox(MIN(this->left_, that.left_),
				MIN(this->top_, that.top_), MAX(this->right_, that.right_),
	...
	inline float PascalScore(const BoundingBox& that) const
	{
		...
		const float score = intersect_area
				/ (GetArea() + that.GetArea() - intersect_area);
	...
	{
		return InRange(point.x, left_, right_)
				&& InRange(point.y, top_, bottom_);
	}
...
inline BoundingSquare GetCenteredSquare(const BoundingBox& original_box)
{
	return GetCenteredSquare(original_box,
			MIN(original_box.GetWidth(), original_box.GetHeight()));
}
