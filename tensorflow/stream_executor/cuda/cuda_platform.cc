	...
	for (int i = 0; i < VisibleDeviceCount(); i++) {
		...
		if (i == 0) {
			...
			min_numa_node_ = std::min(min_numa_node_,
					exec->GetDeviceDescription().numa_node());
		...
		int bus_ordinal)
{
	...
	return port::Status { port::error::NOT_FOUND, port::Printf(
			"Executor for bus %d not found.", bus_ordinal) };
}
...
const string& CudaPlatform::Name() const
{
	return name_;
}
...
port::StatusOr<std::unique_ptr<StreamExecutor>> CudaPlatform::GetUncachedExecutor(
		const StreamExecutorConfig& config)
{
	auto executor = port::MakeUnique<StreamExecutor>(this,
			new CUDAExecutor(config.plugin_config));
