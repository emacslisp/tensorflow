...
public:
			...
			do_profile_(do_profile), profile_(profile), stream_(stream)
	{
		if (do_profile_) {
			...
			stream->InitTimer(execution_timer_.get()).ThenStartTimer(
	...
	~HloExecutionProfiler()
	{
		if (do_profile_) {
			...
			profile_->set_total_cycles_executed(
					execution_timer_->Nanoseconds() * clock_rate_ghz_);
		}
	}
	...
	void FinishOperation(const HloInstruction* hlo_instruction)
	{
		if (do_profile_) {
			...
			stream_->BlockHostUntilDone();
			profile_->AddProfileResult(hlo_instruction,
					per_op_timer_->Nanoseconds() * clock_rate_ghz_);
		}
	}
...
GpuExecutable::GpuExecutable(tensorflow::StringPiece ptx,
		...
		Executable(std::move(hlo_module), std::move(module_config)), ptx_(ptx), thunk_schedule_(
				std::move(thunk_schedule)), assignment_(std::move(assignment)), temp_buffer_offsets_(
				std::move(temp_buffer_offsets))
{
}

Status GpuExecutable::ExecuteThunks(se::Stream* main_stream,
		...
		HloExecutionProfile* hlo_execution_profile)
{
	...
	HloExecutionProfiler profiler(do_profile, hlo_execution_profile,
			main_stream);
	...
	for (Thunk* thunk : thunk_schedule_->TotalOrder()) {
		...
		int32 stream_no = thunk_schedule_->StreamNumberForHlo(
		...
		for (const Thunk* dependency : thunk_schedule_->DependsOn(thunk)) {
			stream->ThenWaitFor(
					FindOrDie(thunk_to_finish_event, dependency).get());
		}
	...
	if (!main_stream->BlockHostUntilDone()) {
		return InternalError(
				"Failed to complete all kernels launched on stream %p",
				main_stream);
	}
		...
		HloExecutionProfile* hlo_execution_profile)
{
	...
	for (BufferAllocation::Index i = 0; i < assignment_->Allocations().size();
		...
		if (allocation.is_entry_computation_parameter()) {
			buffer_allocations_builder.RegisterBuffer(i,
	...
	if (ShapeUtil::IsTuple(root->shape())) {
		...
		if (GetRootPointsToSet().IsAmbiguous()) {
			...
			TF_ASSIGN_OR_RETURN(referred_by_output,
					transfer_manager->GatherBufferPointersFromTuple(executor,
							output_buffer_address, root->shape()));
		} else {
			...
			// points-to set.
			TF_RETURN_IF_ERROR(
					GetRootPointsToSet().ForEachElement(
		...
		TF_RETURN_IF_ERROR(
				buffer_allocations->TearDown( { output_buffer_address },
						*assignment_));
	}
		...
		if (allocation.is_entry_computation_parameter()) {
			...
			if (ShapeUtil::IsTuple(arguments[param_no]->shape())) {
				return Unimplemented(
						"Tuple ShapedBuffer arguments not supported");
			}
			buffer_allocations_builder.RegisterBuffer(i,
					arguments[param_no]->buffer(/*index=*/{ }));
		}
	}
	se::StreamExecutor* executor = stream->parent();
	TF_ASSIGN_OR_RETURN(auto buffer_allocations,
...
Status GpuExecutable::ExecuteOnStream(const ExecutableRunOptions* run_options,
		tensorflow::gtl::ArraySlice<const ShapedBuffer*> arguments,
