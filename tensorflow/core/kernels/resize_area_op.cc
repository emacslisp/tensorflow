...
public:
	explicit ResizeAreaOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	static void ComputePatchSumOf3Channels(float scale,
			const ImageResizerState& st, const std::vector<const T*>& y_ptrs,
			const std::vector<float>& y_scales,
			const CachedInterpolation& x_interp, float* output_ptr)
	{
...
#define BOUND_IF_NEEDED(x, y) (NeedsXBounding ? Bound(x, y) : (x))
		...
		for (int64 c = 0; c < num_channels; ++c) {
			...
			for (int i = 0; i < y_ptrs.size(); ++i) {
				const T* ptr = y_ptrs[i];
				float scale_x = x_interp.start_scale;
				float sum_y = static_cast<float>(ptr[num_channels
						* BOUND_IF_NEEDED(x_interp.start, st.in_width) + c])
						* scale_x;
				if (x_interp.start + 1 != x_interp.end) {
					for (int64 x = x_interp.start + 1; x < x_interp.end - 1;
							++x) {
						sum_y += static_cast<float>(ptr[num_channels
								* BOUND_IF_NEEDED(x, st.in_width) + c]);
					}
					scale_x = x_interp.end_minus_one_scale;
					sum_y +=
							static_cast<float>(ptr[num_channels
									* BOUND_IF_NEEDED(x_interp.end - 1,
											st.in_width) + c]) * scale_x;
				}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		for (int64 x = 0; x < st.out_width; ++x) {
			...
			x_interp.start_scale =
					v < in_x ?
							(v + 1 > in_x1 ? st.width_scale : v + 1 - in_x) :
			...
			x_interp.end_minus_one_scale =
					v < in_x ?
							(v + 1 > in_x1 ? st.width_scale : v + 1 - in_x) :
							(v + 1 > in_x1 ? in_x1 - v : 1.0);
			x_interp.needs_bounding = Bound(x_interp.start, st.in_width)
					!= x_interp.start
					|| Bound(x_interp.end - 1, st.in_width)
							!= (x_interp.end - 1);
		}
		...
		for (int64 b = 0; b < st.batch_size; ++b) {
			for (int64 y = 0; y < st.out_height; ++y) {
				...
				for (int64 i = y_start; i < y_end; ++i) {
					...
					if (i < in_y) {
						scale_y = (
								i + 1 > in_y1 ? st.height_scale : i + 1 - in_y);
					} else {
					...
					y_ptrs.push_back(
							input_ptr
									+ (b * st.in_height * st.in_width
											* st.channels
											+ Bound(i, st.in_height)
													* st.in_width * st.channels));
				}
				...
				if (kKnownNumChannels == 3) {
					for (int64 x = 0; x < st.out_width; ++x) {
						...
						if (x_interp.needs_bounding) {
							ComputePatchSumOf3Channels<true>(scale, st, y_ptrs,
									y_scales, x_interp, output_ptr);
						} else {
							ComputePatchSumOf3Channels<false>(scale, st, y_ptrs,
									y_scales, x_interp, output_ptr);
						}
						output_ptr += 3;
					}
					...
					for (int64 x = 0; x < st.out_width; ++x) {
						const CachedInterpolation& x_interp = x_interps[x];
						if (x_interp.needs_bounding) {
							ComputePatchSum<true>(scale, st, y_ptrs, y_scales,
									x_interp, output_ptr);
						} else {
							ComputePatchSum<false>(scale, st, y_ptrs, y_scales,
									x_interp, output_ptr);
						}
