...
struct DupRecvKeyHash {
	size_t operator()(const DupRecvKey& k) const
	{
		...
		h = Hash64(reinterpret_cast<const char*>(&k.dst_graph),
				sizeof(k.dst_graph), h);
...
typedef std::unordered_map<std::pair<int, int>, MemoryType, PairIntHash> MemoryTypeMap;
...
NodeDef* AddSend(const PartitionOptions& opts, const GraphInfo& g_info,
		GraphDef* gdef, const Edge* edge, NodeDefBuilder::NodeOut send_from,
		int64 start_time, Status* status)
{
	const DataType dtype = send_from.data_type;
	const DataType cast_dtype =
			opts.should_cast ? opts.should_cast(edge) : dtype;
		...
		GraphDef* gdef, const Edge* edge, NodeDef** real_recv, Status* status)
{
	...
	recv_builder.Device(dst->assigned_device_name()).Attr("tensor_type",
			cast_dtype);
	...
	*real_recv = recv;
	...
	if (dtype != cast_dtype) {
		...
		cast_builder.Device(dst->assigned_device_name()).Input(recv->name(), 0,
				cast_dtype);
		NodeDef* cast = gdef->add_node();
		*status = cast_builder.Finalize(cast);
		if (!status->ok())
		...
		id_builder.Device(dst->assigned_device_name()).Input(recv->name(), 0,
				cast_dtype);
		...
		const Edge* edge, Status* status)
{
	...
	*status = NodeDefBuilder(opts.new_name(src->name()), "Const").Device(
			src->assigned_device_name()).Attr("dtype", DT_FLOAT).Attr("value",
			tensor).Finalize(result);
	return result;
}

// A dummy node for scheduling.
NodeDef* AddControlTrigger(const PartitionOptions& opts, GraphDef* gdef,
		const string& assigned_device_name, int64 epoch, int64 starttime,
		Status* status)
{
	NodeDef* result = gdef->add_node();
	*status = NodeDefBuilder(opts.new_name(strings::StrCat("synch_", epoch)),
			"ControlTrigger").Device(assigned_device_name).Attr("_start_time",
			starttime).Finalize(result);
	return result;
}
...
Node* AddControlMerge(const string& in_name1, const string& in_name2, Graph* g,
		const string& node_name, const string& device_name, Status* status)
{
	NodeBuilder node_builder(node_name, "Merge", g->op_registry());
	node_builder.Input(
			{ { in_name1, 0, DT_FLOAT }, { in_name2, 0, DT_FLOAT } });
	Node* res_node;
	*status = node_builder.Finalize(g, &res_node);
	if (!status->ok())
		return nullptr;
		...
		std::vector<ControlFlowInfo>* cf_info, ControlLoop* loop)
{
	...
	const string& enter_name = ControlLoopName(
			opts.new_name(edge->dst()->name()));
	const string& merge_name = ControlLoopName(
			opts.new_name(edge->dst()->name()));
	const string& switch_name = ControlLoopName(
			opts.new_name(edge->dst()->name()));
	const string& next_name = ControlLoopName(
			opts.new_name(edge->dst()->name()));
	...
	if (!status.ok())
		return status;
	Node* next = AddControlNext( { switch_node, 1 }, device_name,
			bopts.WithName(next_name));
	if (!status.ok())
		return status;
...
Status BuildMemoryDeviceInfo(const Graph& g, GraphInfo* info)
{
	...
	for (const Node* node : g.nodes()) {
		...
		TF_RETURN_IF_ERROR(
				MemoryTypesForNode(g.op_registry(), DeviceType(parsed.type),
...
Status AddControlFlow(const PartitionOptions& opts, Graph* g, GraphInfo* g_info)
{
		...
		while (true) {
			...
			if (curr_frame_name.empty()) {
				...
				if (child_loop.merge != nullptr) {
					const string& node_name = opts.new_name(
							edge->dst()->name());
					const string& device_name =
							edge->dst()->assigned_device_name();
					Node* const_node = AddControlConst(device_name,
							bopts.WithName(node_name));
					if (!status.ok())
						return status;
			...
			const string& cl_key = strings::StrCat(curr_frame_name, "$$",
					dst_device);
			...
			status = AddControlLoop(opts, g, src_frame, edge, loop_cond,
					&cf_info, &curr_loop);
			if (!status.ok())
				return status;
	...
	for (int i = 0; i < num_edge_ids; ++i) {
		...
		if (src_device != dst_device) {
			...
			const string& dst_frame_name = cf_info[dst_frame->id()].frame_name;
			if (!src_frame_name.empty() && src_frame_name == dst_frame_name) {
				const string& cl_key = strings::StrCat(dst_frame_name, "$$",
						dst_device);
		...
		*dst_def = dst->def();
		...
		if (num_input_edges != dst->num_inputs()) {
			return errors::InvalidArgument("Incomplete graph, missing ",
					(dst->num_inputs() - num_input_edges), " inputs for ",
					dst->name());
		}
		...
		for (const Edge* edge : inputs) {
			...
			if (opts.scheduling_for_recvs) {
				if (opts.need_to_record_start_times) {
					...
					status = GetNodeAttr(src->def(), "_start_time",
							&send_start_time);
					if (!status.ok()) {
						return status;
					}
					status = GetNodeAttr(dst->def(), "_start_time",
							&recv_start_time);
					if (!status.ok()) {
						return status;
					}
			...
			if (edge->IsControlEdge()) {
				...
				VLOG(1) << "Send/Recv control: " << src->assigned_device_name()
						<< "[" << src->name() << "] -> "
						<< dst->assigned_device_name() << "[" << dst->name()
						<< "]";
				...
				send_from.Reset(src->name(), edge->src_output(),
						EdgeType(edge));
			}
	...
	if (opts.scheduling_for_recvs) {
		for (auto& it : dup_recv) {
			...
			if (it.second.real_recv != it.second.recv) {
				AddNodeAttr("_start_time", it.second.start_time,
						it.second.real_recv);
			}
		}
	}

	VLOG(1) << "Added send/recv: controls=" << num_control << ", data="
			<< num_data;
	return Status::OK();
}
