...
Master::Master(MasterEnv* env, double session_gc_seconds) :
		env_(env), last_1000_steps_(1000), step_count_(0), session_gc_seconds_(
				session_gc_seconds)
{
...
void Master::GC()
{
	...
	while (true) {
		...
		const int64 num_micros = static_cast<int64>(session_gc_seconds_
				* 1000000);
...
public:
	static Status GetRemoteDevices(
			...
			MasterEnv* env, std::vector<Device*>* out_remote)
	{
...
private:
	...
	mutex mu_;int num_pending_ GUARDED_BY(mu_);
	condition_variable pending_zero_;std::vector<Device*> found_ GUARDED_BY(mu_);
	...
	std::vector<string> targets_;std::vector<bool> seen_targets_ GUARDED_BY(mu_);
	Status status_;
		...
		ExtendSessionResponse* resp, MyClosure done)
{
	...
	if (session == nullptr) {
		mu_.unlock();
				...
				errors::Aborted("Session ", req->session_handle(),
						" is not found."));
		return;
	}
	session->Ref();
...
{
	mu_.lock();
	MasterSession* session = gtl::FindPtrOrNull(sessions_,
			req->session_handle());
	if (session == nullptr) {
		mu_.unlock();
		done(
...
void Master::CleanupWorkers(const ResetRequest& reset)
{
	...
	if (!worker_names.empty()) {
		...
		for (int i = 0; i < num_workers; ++i) {
			...
			if (worker) {
				worker->CleanupAllAsync(&req, &resp[i],
						[this, &n, worker_name, worker, c](Status s) {
							TF_CHECK_OK(s);
