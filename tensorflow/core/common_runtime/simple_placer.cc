...
std::vector<Device*> FilterSupportedDevices(const std::vector<Device*>& devices,
...
void ColocationGroups(const Node& node, std::vector<string>* colocation_groups)
{
...
public:
	ColocationGraph(Graph* graph, const DeviceSet* device_set,
			...
			device_set_(device_set), device_types_(
					device_set->PrioritizedDeviceTypeList()), options_(options)
	{
	...
	Status ColocateNodes(const Node& x, const Node& y)
	{
		...
		if (x_root != y_root) {
			...
			s = DeviceNameUtils::MergeDevNames(&members_[new_root].device_name,
					members_[old_root].device_name,
					options_ == nullptr
							|| options_->config.allow_soft_placement());
			if (!s.ok()) {
				return errors::InvalidArgument("Cannot colocate nodes '",
						x.name(), "' and '", y.name(), ": ", s.error_message());
			}
			...
			if (members_[new_root].supported_device_types.size() == 0) {
				...
				return errors::InvalidArgument("Cannot colocate nodes '",
						...
						"' because no device type supports both of those nodes and the "
								"other nodes colocated with them.", debug_info);
			}
		}
		return Status::OK();
	}
	...
	Status GetDevicesForNode(Node* node, std::vector<Device*>* possible_devices)
	{
		...
		if (DeviceNameUtils::HasSomeDetails(members_[node_root].device_name)) {
			...
			if (!devices.empty()) {
				...
				devices = FilterSupportedDevices(devices,
						members_[node_root].supported_device_types);
			}
				...
				if (DeviceNameUtils::ParseFullName(node->def().device(),
					...
					if (devices_matching_nodedef.empty()) {
						...
						return errors::InvalidArgument(
								"Could not satisfy explicit device specification '",
								node->def().device(),
								"' because no supported kernel for ",
								specified_device_name.type,
								" devices is available.", debug_info);
					} else {
						return errors::InvalidArgument(
								"Could not satisfy explicit device specification '",
			...
			if (devices.empty()) {
				...
				return errors::InvalidArgument(
						...
						"Operation was ", node->type_string(),
						" and inputs were ",
...
private:
	...
	Status InitializeMember(const Node& node, Member* member)
	{
		...
		member->parent = id;
		TF_RETURN_IF_ERROR(
				SupportedDeviceTypesForNode(device_types_, node.def(),
						&member->supported_device_types));
		...
		if (!node.assigned_device_name().empty()) {
			...
			std::vector<Device*> devices;
			const Device* assigned_device = device_set_->FindDeviceByName(
					node.assigned_device_name());
...
Status SimplePlacer::Run()
{
		...
		for (const auto& edge : node->in_edges()) {
			if (!edge->IsControlEdge()
					&& (IsRefType(node->input_type(edge->dst_input()))
							|| node->input_type(edge->dst_input())
									== DT_RESOURCE)) {
				...
				auto source_parsed_name = colocation_graph.DeviceForNode(
						*edge->src());
				...
				if (DeviceNameUtils::HasSomeDetails(source_parsed_name)
					...
					if (!DeviceNameUtils::AreCompatibleDevNames(
						...
						LOG(INFO) << "Ignoring device specification "
								...
								<< "' is a reference connection and already has a device "
										"field set to "
								<< DeviceNameUtils::ParsedNameToString(
										colocation_graph.DeviceForNode(
												*edge->src()));

						// Make 'node' colocated with the source
						colocation_graph.SetDeviceForNode(node,
								source_parsed_name);
					} else {
						bool source_subset_of_dest =
								DeviceNameUtils::IsSpecification(
										source_parsed_name, dest_parsed_name);
						bool dest_subset_of_source =
								DeviceNameUtils::IsSpecification(
										dest_parsed_name, source_parsed_name);

						if (source_subset_of_dest && !dest_subset_of_source) {
							colocation_graph.SetDeviceForNode(edge->src(),
									dest_parsed_name);
						} else {
							colocation_graph.SetDeviceForNode(node,
									source_parsed_name);
	...
	for (Node* node : second_pass) {
		...
		if (IsGeneratorNode(node)) {
			...
			const bool consumers_on_same_device =
					std::all_of(node->out_edges().begin(),
							node->out_edges().end(),
		...
		const std::vector<Device*>& devices) const
{
	if (!candidate_device_name.empty()) {
		...
		const Device* other_device = devices_->FindDeviceByName(
...
void SimplePlacer::AssignAndLog(const string& assigned_device, Node* node) const
{
