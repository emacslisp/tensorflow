	...
	*bucket = bucketp.ToString();
	if (bucket->empty() || *bucket == ".") {
		return errors::InvalidArgument(
				"GCS path doesn't contain a bucket name: ", fname);
	}
	objectp.Consume("/");
	*object = objectp.ToString();
	if (!empty_object_ok && object->empty()) {
		return errors::InvalidArgument(
				"GCS path doesn't contain an object name: ", fname);
	}
	return Status::OK();
}
...
Status GetBoolValue(const Json::Value& parent, const string& name, bool* result)
{
	Json::Value result_value;
	TF_RETURN_IF_ERROR(GetValue(parent, name, &result_value));
...
public:
	GcsRandomAccessFile(const string& bucket, const string& object,
			...
			HttpRequest::Factory* http_request_factory, size_t read_ahead_bytes) :
			bucket_(bucket), object_(object), auth_provider_(auth_provider), http_request_factory_(
					http_request_factory), read_ahead_bytes_(read_ahead_bytes)
	{
	}
			...
			char* scratch) const override
	{
		...
		const bool range_end_included = offset + n
				<= buffer_start_offset_ + buffer_.size();
		if (range_start_included && range_end_included) {
			// The requested range can be filled from the buffer.
			const size_t offset_in_buffer = std::min<uint64>(
					offset - buffer_start_offset_, buffer_.size());
			const auto copy_size = std::min(n,
					buffer_.size() - offset_in_buffer);
...
private:
	...
	Status LoadBufferFromGCS() const EXCLUSIVE_LOCKS_REQUIRED(mu_) {
		string auth_token;TF_RETURN_IF_ERROR(AuthProvider::GetToken(auth_provider_, &auth_token));

		std::unique_ptr<HttpRequest> request(http_request_factory_->Create());TF_RETURN_IF_ERROR(request->Init());TF_RETURN_IF_ERROR(
				request->SetUri(strings::StrCat("https://", bucket_, ".", kStorageHost,
								"/", request->EscapeString(object_))));TF_RETURN_IF_ERROR(request->AddAuthBearerHeader(auth_token));TF_RETURN_IF_ERROR(request->SetRange(
						buffer_start_offset_, buffer_start_offset_ + buffer_.capacity() - 1));TF_RETURN_IF_ERROR(request->SetResultBuffer(&buffer_));TF_RETURN_WITH_CONTEXT_IF_ERROR(request->Send(), " when reading gs://",
				bucket_, "/", object_);return Status::OK();
	}
...
public:
	GcsWritableFile(const string& bucket, const string& object,
			...
			bucket_(bucket), object_(object), auth_provider_(auth_provider), http_request_factory_(
					http_request_factory), sync_needed_(true), initial_retry_delay_usec_(
					initial_retry_delay_usec)
	{
	...
	GcsWritableFile(const string& bucket, const string& object,
			AuthProvider* auth_provider, const string& tmp_content_filename,
			HttpRequest::Factory* http_request_factory,
			int64 initial_retry_delay_usec) :
			bucket_(bucket), object_(object), auth_provider_(auth_provider), http_request_factory_(
					http_request_factory), sync_needed_(true), initial_retry_delay_usec_(
...
private:
	...
	Status CreateNewUploadSession(string* session_uri)
	{
		...
		TF_RETURN_IF_ERROR(
				request->SetUri(
						strings::StrCat(kGcsUploadUriBase, "b/", bucket_,
		...
		TF_RETURN_WITH_CONTEXT_IF_ERROR(request->Send(),
				" when initiating an upload to ", GetGcsPath());
		*session_uri = request->GetResponseHeader("Location");
		if (session_uri->empty()) {
			return errors::Internal(
					"Unexpected response from GCS when writing to ",
	...
	Status RequestUploadSessionStatus(const string& session_uri,
			bool* completed, uint64* uploaded)
	{
		if (completed == nullptr || uploaded == nullptr) {
			return errors::Internal(
					"'completed' and 'uploaded' cannot be nullptr.");
		}
		uint64 file_size;
		TF_RETURN_IF_ERROR(GetCurrentFileSize(&file_size));
		...
		TF_RETURN_IF_ERROR(
				request->AddHeader("Content-Range",
						strings::StrCat("bytes */", file_size)));
		...
		if (received_range.empty()) {
			...
			*uploaded = 0;
			...
			if (!str_util::SplitAndParseAsInts(range_piece, '-', &range_parts)
					|| range_parts.size() != 2) {
				return errors::Internal(
						"Unexpected response from GCS when writing ",
						GetGcsPath(), ": Range header '", received_range,
						"' could not be parsed.");
			}
			if (range_parts[0] != 0) {
				return errors::Internal(
						"Unexpected response from GCS when writing to ",
						GetGcsPath(), ": the returned range '", received_range,
...
GcsFileSystem::GcsFileSystem() :
		auth_provider_(new GoogleAuthProvider()), http_request_factory_(
				new HttpRequest::Factory())
{
}

GcsFileSystem::GcsFileSystem(std::unique_ptr<AuthProvider> auth_provider,
		std::unique_ptr<HttpRequest::Factory> http_request_factory,
		size_t read_ahead_bytes, int64 initial_retry_delay_usec) :
		auth_provider_(std::move(auth_provider)), http_request_factory_(
				std::move(http_request_factory)), read_ahead_bytes_(
				read_ahead_bytes), initial_retry_delay_usec_(
				initial_retry_delay_usec)
{
}
		...
		std::unique_ptr<RandomAccessFile>* result)
{
	...
	result->reset(
			new GcsRandomAccessFile(bucket, object, auth_provider_.get(),
					http_request_factory_.get(), read_ahead_bytes_));
	return Status::OK();
}
		...
		std::unique_ptr<WritableFile>* result)
{
	...
	while (true) {
		status = reader->Read(offset, kReadAppendableFileBufferSize,
				&read_chunk, buffer.get());
		if (status.ok()) {
...
Status GcsFileSystem::NewReadOnlyMemoryRegionFromFile(const string& fname,
		...
		FileStatistics* stat)
{
	...
	TF_RETURN_IF_ERROR(request->SetResultBuffer(&output_buffer));
	TF_RETURN_WITH_CONTEXT_IF_ERROR(request->Send(),
			" when reading metadata of gs://", bucket, "/", object);

	StringPiece response_piece = StringPiece(output_buffer.data(),
			output_buffer.size());
	Json::Value root;
		...
		std::vector<string>* result)
{
	return GetChildrenBounded(dirname, UINT64_MAX, result,
			false /* recursively */, false /* include_self_directory_marker */);
}
		...
		std::vector<string>* results)
{
	...
	const string& fixed_prefix = pattern.substr(0,
	...
	TF_RETURN_IF_ERROR(
			GetChildrenBounded(dir, UINT64_MAX, &all_files,
					true /* recursively */,
...
Status GcsFileSystem::GetChildrenBounded(const string& dirname,
		uint64 max_results, std::vector<string>* result, bool recursive,
		bool include_self_directory_marker)
{
	...
	TF_RETURN_IF_ERROR(
			ParseGcsPath(MaybeAppendSlash(dirname), true, &bucket,
					&object_prefix));
	...
	while (true) {  // A loop over multiple result pages.
		...
		if (max_results - retrieved_results < kGetChildrenDefaultPageSize) {
			uri = strings::StrCat(uri, "&maxResults=",
					max_results - retrieved_results);
		}
		...
		if (items != Json::Value::null) {
			...
			for (size_t i = 0; i < items.size(); i++) {
				...
				if (!relative_path.Consume(object_prefix)) {
					return errors::Internal(
							strings::StrCat(
									"Unexpected response: the returned file name ",
		...
		if (prefixes != Json::Value::null) {
			...
			for (size_t i = 0; i < prefixes.size(); i++) {
				...
				if (!relative_path.Consume(object_prefix)) {
					return errors::Internal(
							"Unexpected response: the returned folder name ",
							prefix_str, " doesn't match the prefix ",
							object_prefix);
				}
				result->emplace_back(relative_path.ToString());
				if (++retrieved_results >= max_results) {
...
Status GcsFileSystem::Stat(const string& fname, FileStatistics* stat)
{
	...
	if (object.empty()) {
		...
		return errors::NotFound("The specified bucket ", fname,
				" was not found.");
	}
...
Status GcsFileSystem::RenameFile(const string& src, const string& target)
{
	...
	std::vector < string > children;
	TF_RETURN_IF_ERROR(
			GetChildrenBounded(src, UINT64_MAX, &children,
					true /* recursively */,
					true /* include_self_directory_marker */));
	for (const string& subpath : children) {
		TF_RETURN_IF_ERROR(
				RenameObject(JoinGcsPath(src, subpath),
						JoinGcsPath(target, subpath)));
	}
	return Status::OK();
}
...
Status GcsFileSystem::RenameObject(const string& src, const string& target)
{
	...
	TF_RETURN_IF_ERROR(
			request->SetUri(
					strings::StrCat(kGcsUriBase, "b/", src_bucket, "/o/",
							request->EscapeString(src_object), "/rewriteTo/b/",
							target_bucket, "/o/",
	...
	if (!done) {
		...
		return errors::Unimplemented("Couldn't rename ", src, " to ", target,
