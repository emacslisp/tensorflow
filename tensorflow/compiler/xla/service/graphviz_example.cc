...
HloComputation* AddScalarConstantComputation(int64 addend, HloModule* module)
{
	auto builder = HloComputation::Builder(
			tensorflow::strings::StrCat("add_", addend));
	auto x_value = builder.AddInstruction(
			HloInstruction::CreateParameter(0, ShapeUtil::MakeShape(F32, { }),
					"x_value"));
	auto half = builder.AddInstruction(
			HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(0.5)));
	builder.AddInstruction(
			HloInstruction::CreateBinary(half->shape(), HloOpcode::kAdd,
					x_value, half));
...
HloComputation* ScalarSumComputation(HloModule* module)
{
	...
	auto rhs = builder.AddInstruction(
			HloInstruction::CreateParameter(1, ShapeUtil::MakeShape(F32, { }),
					"rhs"));
	builder.AddInstruction(
			HloInstruction::CreateBinary(lhs->shape(), HloOpcode::kAdd, lhs,
					rhs));
	return module->AddEmbeddedComputation(builder.Build());
}
...
std::unique_ptr<HloModule> MakeBigGraph()
{
	...
	auto param_v0 = builder.AddInstruction(
			HloInstruction::CreateParameter(0, vshape, "foo"));
	auto param_v1 = builder.AddInstruction(
			HloInstruction::CreateParameter(1, vshape, "bar"));
	auto param_v2 = builder.AddInstruction(
			HloInstruction::CreateParameter(2, vshape, "baz"));
	auto param_s = builder.AddInstruction(
			HloInstruction::CreateParameter(3, sshape, "qux"));
	auto param_m = builder.AddInstruction(
			HloInstruction::CreateParameter(4, mshape, "zzz"));
	...
	auto clamp = builder.AddInstruction(
			HloInstruction::CreateTernary(vshape, HloOpcode::kClamp, copy,
					param_v1, param_v2));
	auto dot = builder.AddInstruction(
			HloInstruction::CreateBinary(vshape, HloOpcode::kDot, clamp,
					param_v0));
	auto tuple = builder.AddInstruction(HloInstruction::CreateTuple( { dot,
			param_s, clamp }));
	...
	auto rng = builder.AddInstruction(
			HloInstruction::CreateRng(vshape, RNG_UNIFORM,
					{ param_m, param_m }));
	auto one = builder.AddInstruction(
			HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(1.0)));
	auto add_computation = ScalarSumComputation(module.get());
	builder.AddInstruction(
			HloInstruction::CreateReduce(vshape, rng, one, { 1 },
					add_computation));
	auto map1 = builder.AddInstruction(HloInstruction::CreateMap(sshape, {
			scalar }, add_one));
	...
	auto fusion = builder.AddInstruction(
			HloInstruction::CreateFusion(sshape,
					HloInstruction::FusionKind::kLoop, map3));
	...
	builder.AddInstruction(
			HloInstruction::CreateCall(fusion->shape(), { fusion },
					call_computation));
...
int main(int argc, char** argv)
{
	...
	printf("Graph URL: %s\n",
			xla::hlo_graph_dumper::DumpGraph(*module->entry_computation(),
					"Example computation",
					/*show_addresses=*/false, /*show_layouts=*/false).c_str());
	return 0;
}
