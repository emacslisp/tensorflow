...
public:
	...
	InlinedVector& operator=(const InlinedVector& v)
	{
		...
		if (s < vs) {  // grow
			reserve(vs);
			if (s)
				std::copy(v.begin(), v.begin() + s, begin());
...
private:
	...
	{
		return u_.data[kSize - 1] != kSentinel;
	}
...
template<typename T, int N>
typename InlinedVector<T, N>::iterator InlinedVector<T, N>::insert(iterator pos,
		const value_type& v)
{
	DCHECK_GE(pos, begin());
	DCHECK_LE(pos, end());
