...
enum JPEGErrors {
	JPEGERRORS_OK, JPEGERRORS_UNEXPECTED_END_OF_DATA, JPEGERRORS_BAD_PARAM
};
...
public:
	FewerArgsForCompiler(int datasize, const UncompressFlags& flags,
			int64* nwarn, std::function<uint8*(int, int, int)> allocate_output) :
			datasize_(datasize), flags_(flags), pnwarn_(nwarn), allocate_output_(
					std::move(allocate_output)), height_read_(0), height_(0), stride_(
					0)
	{
		if (pnwarn_ != nullptr)
			*pnwarn_ = 0;
...
uint8* UncompressLow(const void* srcdata, FewerArgsForCompiler* argball)
{
	...
	uint8* const dstdata = argball->allocate_output_(cinfo.output_width,
			cinfo.output_height, components);
	if (dstdata == nullptr) {
		jpeg_destroy_decompress(&cinfo);
		return nullptr;
	}
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		if (num_lines_read == 0) {
			...
			if (!flags.try_recover_truncated_jpeg) {
				...
				for (size_t line = cinfo.output_scanline;
						line < cinfo.output_height; ++line) {
					if (line == 0) {
				...
				argball->height_read_ = cinfo.output_height; // consider all lines as read
	...
	if (components == 4) {
		...
		JSAMPLE* scanlineptr = static_cast<JSAMPLE*>(dstdata
				+ static_cast<int64>(cinfo.output_height - 1) * stride);
...
{
	uint8* buffer = NULL;
	uint8* result = Uncompress(srcdata, datasize, flags, nwarn,
		...
		const CompressFlags& flags, string* output)
{
	...
	while (cinfo.next_scanline < cinfo.image_height) {
		...
		default: {
			row_pointer[0] =
					reinterpret_cast<JSAMPLE*>(const_cast<JSAMPLE*>(r));
		}
		}
