...
Status CreateSignature(RSA* private_key, StringPiece to_sign, string* signature)
{
	...
	if (EVP_DigestSignUpdate(md_ctx.get(), to_sign.data(), to_sign.size())
			!= 1) {
	...
	return Base64Encode(
			StringPiece(reinterpret_cast<char*>(sig.get()), sig_len), signature);
}
...
Status EncodeJwtClaim(StringPiece client_email, StringPiece scope,
		StringPiece audience, uint64 request_timestamp_sec, string* encoded)
{
	...
	const auto expiration_timestamp_sec = request_timestamp_sec
			+ kRequestedTokenLifetimeSec;
...
OAuthClient::OAuthClient() :
		OAuthClient(
				std::unique_ptr<HttpRequest::Factory>(
						new HttpRequest::Factory()), Env::Default())
{
}
...
Status OAuthClient::GetTokenFromServiceAccountJson(Json::Value json,
		StringPiece oauth_server_uri, StringPiece scope, string* token,
		uint64* expiration_timestamp_sec)
{
	...
	std::unique_ptr<BIO, std::function<void(BIO*)>> bio(BIO_new(BIO_s_mem()),
			[](BIO* ptr) {BIO_free_all(ptr);});
	...
	TF_RETURN_IF_ERROR(
			EncodeJwtClaim(client_email, scope, oauth_server_uri,
	...
	const string request_body = strings::StrCat("grant_type=", kGrantType,
			"&assertion=", jwt);
	...
	TF_RETURN_IF_ERROR(
			request->SetPostFromBuffer(request_body.c_str(),
					request_body.size()));
	TF_RETURN_IF_ERROR(request->SetResultBuffer(&response_buffer));
	TF_RETURN_IF_ERROR(request->Send());

	StringPiece response = StringPiece(response_buffer.data(),
			response_buffer.size());
	TF_RETURN_IF_ERROR(
			ParseOAuthResponse(response, request_timestamp_sec, token,
					expiration_timestamp_sec));
	return Status::OK();
}

Status OAuthClient::GetTokenFromRefreshTokenJson(Json::Value json,
		StringPiece oauth_server_uri, string* token,
		uint64* expiration_timestamp_sec)
{
	if (!token || !expiration_timestamp_sec) {
		return errors::FailedPrecondition(
	...
	const auto request_body = strings::StrCat("client_id=", client_id,
		...
		uint64* expiration_timestamp_sec)
{
	...
	if (token_type != "Bearer") {
		return errors::FailedPrecondition(
				"Unexpected Oauth token type: " + token_type);
	}
	int64 expires_in;
