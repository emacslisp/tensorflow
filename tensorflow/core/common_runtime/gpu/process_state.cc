...
Allocator* ProcessState::GetCPUAllocator(int numa_node)
{
	...
	while (cpu_allocators_.size() <= static_cast<size_t>(numa_node)) {
		Allocator* allocator = new PoolAllocator(100 /*pool_size_limit*/,
				true /*auto_resize*/, new BasicCPUAllocator(), new NoopRounder,
				"cpu_pool");
...
Allocator* ProcessState::GetCUDAHostAllocator(int numa_node)
{
	...
	while (static_cast<int>(cuda_host_allocators_.size()) <= numa_node) {
		...
		Status status = ReadInt64FromEnvVar("TF_CUDA_HOST_MEM_LIMIT_IN_MB",
				1LL << 16 /*64GB max by default*/, &cuda_host_mem_limit_in_mb);
		if (!status.ok()) {
			LOG(ERROR) << "GetCUDAHostAllocator: " << status.error_message();
		}
		int64 cuda_host_mem_limit = cuda_host_mem_limit_in_mb * (1LL << 20);
		Allocator* allocator = new BFCAllocator(new CUDAHostAllocator(se),
				cuda_host_mem_limit, true /*allow_growth*/,
				"cuda_host_bfc" /*name*/);
		...
		if (FLAGS_brain_gpu_record_mem_types) {
			...
			cuda_al_.push_back(
					new internal::RecordingAllocator(&mem_desc_map_,
							cuda_host_allocators_.back(), md, &mu_));
		}
	}
	if (FLAGS_brain_gpu_record_mem_types)
		return cuda_al_[0];
