...
private:
	...
	void FreeArgs()
	EXCLUSIVE_LOCKS_REQUIRED (data_mu_);
	void ClosePipes()
	EXCLUSIVE_LOCKS_REQUIRED (data_mu_);
	...
	mutable mutex proc_mu_;bool running_ GUARDED_BY(proc_mu_);pid_t
	pid_ GUARDED_BY(proc_mu_);

	mutable mutex data_mu_ ACQUIRED_AFTER(proc_mu_);char* exec_path_ GUARDED_BY(data_mu_);char** exec_argv_ GUARDED_BY(data_mu_);ChannelAction action_[kNFds] GUARDED_BY(data_mu_);int parent_pipe_[kNFds] GUARDED_BY(data_mu_);int child_pipe_[kNFds] GUARDED_BY(data_mu_);
