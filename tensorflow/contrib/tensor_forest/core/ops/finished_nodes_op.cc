		...
		std::unordered_set<int32>* final_stale)
{
	...
	for (int32 i = start; i < end; i++) {
		...
		bool finished = params.dominate_method(accumulator,
				simple_philox.get());
		if (finished) {
			finished_leaves.push_back(leaf);
		}
	...
	mutex_lock m(*mutex);
	final_finished_leaves->insert(finished_leaves.begin(),
			finished_leaves.end());
...
public:
			...
			OpKernel(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("num_split_after_samples",
						&num_split_after_samples_));
		OP_REQUIRES_OK(context,
				context->GetAttr("min_split_samples", &min_split_samples_));
		OP_REQUIRES_OK(context,
				context->GetAttr("dominate_fraction", &dominate_fraction_));
		OP_REQUIRES_OK(context,
	...
	void Compute(OpKernelContext* context) override
	{
		...
		// Check tensor bounds.
		if (!CheckTensorBounds(context, leaf_tensor))
			return;
		if (!CheckTensorBounds(context, node_to_accumulator))
			return;
		if (!CheckTensorBounds(context, split_sums))
			return;
		if (!CheckTensorBounds(context, split_squares))
			return;
		if (!CheckTensorBounds(context, accumulator_sums))
			return;
		if (!CheckTensorBounds(context, accumulator_squares))
			return;
		...
		const int32 num_leaves =
				static_cast<int32>(leaf_tensor.shape().dim_size(0));
		...
		if (regression_) {
			...
			if (dominate_method_ == "none") {
				...
				params.dominate_method = std::bind(
						&BestSplitDominatesClassificationHoeffding,
						accumulator_sums, split_sums, _1, dominate_fraction_);
			} else if (dominate_method_ == "chebyshev") {
				params.dominate_method = std::bind(
						&BestSplitDominatesClassificationChebyshev,
						accumulator_sums, split_sums, _1, dominate_fraction_);
				...
				if (params.random_seed == 0) {
					params.random_seed =
							static_cast<uint64>(Env::Default()->NowMicros());
				}

				params.dominate_method = std::bind(
						&BestSplitDominatesClassificationBootstrap,
						accumulator_sums, split_sums, _1, dominate_fraction_,
						_2);
			} else {
		...
		auto work =
				[&params, &finished_leaves, &stale, &m, num_leaves](int64 start,
						int64 end) {
					CHECK(start <= end);
		...
		Shard(num_threads, worker_threads->workers, num_leaves, costPerUnit,
				work);
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, finished_shape, &output_finished));
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(1, stale_shape, &output_stale));
