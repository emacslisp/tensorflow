...
private:
	...
	static Status DumpTensorToEventFile(const string& node_name,
			const int32 output_slot, const string& debug_op,
			const Tensor& tensor, const uint64 wall_time_us,
...
private:
	...
	std::unique_ptr<::grpc::ClientReaderWriterInterface<Event, EventReply>> reader_writer_;
...
private:
	static mutex streams_mu;static std::unordered_map<string, std::shared_ptr<DebugGrpcChannel>>
