...
public:
	...
	{
		out_.reset(
				new llvm::raw_fd_ostream(llvm::StringRef(output_filename_), ec_,
						llvm::sys::fs::F_None));
		if (ec_) {
	...
	for (int i = 0; i < passes_.size(); ++i) {
		...
		if (dump_ir_) {
			...
			const string basename = ReplaceFilenameExtension(
					...
					tensorflow::strings::Printf("pass-%02d.before.%s.ll", i,
							(PI == nullptr ?
									"unknown" : PI->getPassArgument().data())));
			llvm::legacy::PassManager::add(
					new DumpIrPass(
							tensorflow::io::JoinPath(output_dir_, basename)));
		}
