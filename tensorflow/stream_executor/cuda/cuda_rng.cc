...
CUDARng::CUDARng(CUDAExecutor *parent) :
		parent_(parent), rng_(nullptr)
{
}
...
bool CUDARng::Init()
{
	...
	curandStatus_t ret = dynload::curandCreateGenerator(parent_, &rng_,
...
{
	curandStatus_t ret = dynload::curandSetStream(parent_, rng_,
...
template<typename T>
bool CUDARng::DoPopulateRandUniformInternal(Stream *stream, DeviceMemory<T> *v)
{
	mutex_lock lock { mu_ };
	...
	if (std::is_same<T, float>::value
		...
		ret = dynload::curandGenerateUniform(parent_, rng_,
				reinterpret_cast<float *>(CUDAMemoryMutable(v)), element_count);
	} else {
		ret = dynload::curandGenerateUniformDouble(parent_, rng_,
				reinterpret_cast<double *>(CUDAMemoryMutable(v)),
				element_count);
	}
	if (ret != CURAND_STATUS_SUCCESS) {
...
bool CUDARng::DoPopulateRandGaussianInternal(Stream *stream, ElemT mean,
		ElemT stddev, DeviceMemory<ElemT> *v, FuncT func)
{
	mutex_lock lock { mu_ };
	...
	uint64 element_count = v->ElementCount();
	curandStatus_t ret = func(parent_, rng_, CUDAMemoryMutable(v),
			element_count, mean, stddev);

	if (ret != CURAND_STATUS_SUCCESS) {
		LOG(ERROR) << "failed to do gaussian generation of "
				<< v->ElementCount() << " floats at " << v->opaque() << ": "
				<< ret;
		return false;
	}
...
bool CUDARng::SetSeed(Stream *stream, const uint8 *seed, uint64 seed_bytes)
{
	...
	curandStatus_t ret = dynload::curandSetPseudoRandomGeneratorSeed(parent_,
