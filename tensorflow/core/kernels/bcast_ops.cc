...
public:
	explicit BCastArgsOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->MatchSignature( { DT_INT32, DT_INT32 }, {
				DT_INT32 }));
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		OP_REQUIRES(ctx, ctx->num_inputs() == 2,
				errors::Unimplemented(
						"Broadcast for n-ary operations (n > 2)"));
		...
		OP_REQUIRES(ctx, bcast.IsValid(),
				errors::InvalidArgument("Incompatible shapes: [",
						str_util::Join(shapes[0], ","), "] vs. [",
						str_util::Join(shapes[1], ","), "]"));
...
private:
	void Output(OpKernelContext* ctx, int idx, const BCast::Vec& v)
	{
		...
		Tensor* o = nullptr;
		OP_REQUIRES_OK(ctx,
				ctx->allocate_output(idx, TensorShape( { len }), &o));
...
public:
	explicit BCastGradArgsOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->MatchSignature( { DT_INT32, DT_INT32 }, {
				DT_INT32, DT_INT32 }));
