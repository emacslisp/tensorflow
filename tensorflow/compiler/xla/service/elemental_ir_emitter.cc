...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitUnaryOp(const HloInstruction* op,
		...
		const HloInstruction* op, llvm::Value* operand_value) const
{
	...
	case HloOpcode::kAbs: {
		...
		if (is_signed) {
			auto type = llvm_ir::PrimitiveTypeToIrType(
					op->shape().element_type(), ir_builder_);
	...
	case HloOpcode::kLogicalNot:
		...
		return ir_builder_->CreateZExt(
				ir_builder_->CreateNot(
						ir_builder_->CreateTrunc(operand_value,
								ir_builder_->getInt1Ty())),
		...
		const HloInstruction* op, llvm::Value* operand_value) const
{
	...
	case HloOpcode::kConvert: {
		...
		if (primitive_util::IsFloatingPointType(to_type)) {
			return ir_builder_->CreateFPCast(operand_value,
					llvm_ir::PrimitiveTypeToIrType(to_type, ir_builder_));
		}
		if (primitive_util::IsSignedIntegralType(to_type)) {
			return ir_builder_->CreateFPToSI(operand_value,
					llvm_ir::PrimitiveTypeToIrType(to_type, ir_builder_));
		}
		if (primitive_util::IsUnsignedIntegralType(to_type)) {
			return ir_builder_->CreateFPToUI(operand_value,
					llvm_ir::PrimitiveTypeToIrType(to_type, ir_builder_));
		}
	...
	case HloOpcode::kExp:
		return llvm_ir::EmitCallToIntrinsic(llvm::Intrinsic::exp, {
				operand_value }, { operand_value->getType() }, ir_builder_);
	case HloOpcode::kLog:
		return llvm_ir::EmitCallToIntrinsic(llvm::Intrinsic::log, {
				operand_value }, { operand_value->getType() }, ir_builder_);
	...
	case HloOpcode::kSign: {
		...
		return ir_builder_->CreateSelect(oeq, zero,
				ir_builder_->CreateSelect(olt,
		...
		llvm::Value* rhs_value) const
{
	...
	case HloOpcode::kPower:
		return llvm_ir::EmitCallToIntrinsic(llvm::Intrinsic::pow, { lhs_value,
				rhs_value }, { lhs_value->getType() }, ir_builder_);
...
llvm::Value* ElementalIrEmitter::EmitFloatMax(llvm::Value* lhs_value,
		llvm::Value* rhs_value) const
{
	return llvm_ir::EmitCallToIntrinsic(llvm::Intrinsic::maxnum, { lhs_value,
			rhs_value }, { lhs_value->getType() }, ir_builder_);
}
...
llvm::Value* ElementalIrEmitter::EmitFloatMin(llvm::Value* lhs_value,
		llvm::Value* rhs_value) const
{
	return llvm_ir::EmitCallToIntrinsic(llvm::Intrinsic::minnum, { lhs_value,
			rhs_value }, { lhs_value->getType() }, ir_builder_);
}
		...
		llvm::Value* x) const
{
	if (prim_type != F32) {
		return Unimplemented(
				"inverse erf only implemented for F32 (b/34339814)");
	}
	...
	//   }
	//   return p*x
	llvm::Function* logf_fn = llvm::Intrinsic::getDeclaration(module_,
			llvm::Intrinsic::log, { ir_builder_->getFloatTy() });

	llvm::Value* w = ir_builder_->CreateFNeg(
			ir_builder_->CreateCall(logf_fn,
					{ ir_builder_->CreateFMul(
							ir_builder_->CreateFSub(getFloat(1.0f), x),
	...
	llvm_ir::LlvmIfData if_data = llvm_ir::EmitIfThenElse(
			ir_builder_->CreateFCmpOLT(w, getFloat(5.0f)), "w_less_than_five",
			ir_builder_);
	// Handle true BB.
	SetToFirstInsertPoint(if_data.true_block, ir_builder_);
	{
		llvm::Value* lw = ir_builder_->CreateFSub(w, getFloat(2.5f));
		tensorflow::gtl::ArraySlice<float> lq { 2.81022636e-08f,
				3.43273939e-07f, -3.5233877e-06f, -4.39150654e-06f,
				0.00021858087f, -0.00125372503f, -0.00417768164f, 0.246640727f,
				1.50140941f };
	...
	{
		llvm::Function* sqrtf_fn = llvm::Intrinsic::getDeclaration(module_,
				llvm::Intrinsic::sqrt, { ir_builder_->getFloatTy() });

		llvm::Value* gw = ir_builder_->CreateFSub(
				ir_builder_->CreateCall(sqrtf_fn, { w }), getFloat(3.0f));
		tensorflow::gtl::ArraySlice<float> gq { -0.000200214257f,
				0.000100950558f, 0.00134934322f, -0.00367342844f,
				0.00573950773f, -0.0076224613f, 0.00943887047f, 1.00167406f,
				2.83297682f };
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitIntegerBinaryOp(
		const HloInstruction* op, llvm::Value* lhs_value,
		llvm::Value* rhs_value, bool is_signed) const
{
	switch (op->opcode()) {
	...
	case HloOpcode::kDivide:
		return is_signed ?
				ir_builder_->CreateSDiv(lhs_value, rhs_value) :
				ir_builder_->CreateUDiv(lhs_value, rhs_value);
	case HloOpcode::kRemainder:
		return is_signed ?
				ir_builder_->CreateSRem(lhs_value, rhs_value) :
		...
		return ir_builder_->CreateSelect(
				ir_builder_->CreateICmp(
						is_signed ?
								...
								llvm::ICmpInst::ICMP_ULE, lhs_value, rhs_value),
				lhs_value, rhs_value);
	case HloOpcode::kMaximum:
		return ir_builder_->CreateSelect(
				ir_builder_->CreateICmp(
						is_signed ?
								...
								llvm::ICmpInst::ICMP_UGE, lhs_value, rhs_value),
				lhs_value, rhs_value);
...
llvm_ir::ElementGenerator ElementalIrEmitter::MakeRngElementGenerator(
		...
		const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator) const
{
	PrimitiveType param_prim_type = hlo->operand(0)->shape().element_type();
	llvm::Type* param_ir_type = llvm_ir::PrimitiveTypeToIrType(param_prim_type,
			ir_builder_);

	// Same values as PCG library
	// https://github.com/imneme/pcg-c/blob/master/include/pcg_variants.h
	llvm::Value* multiplier = ir_builder_->getInt(llvm::APInt(128, {
			0x4385DF649FCCF645, 0x2360ED051FC65DA4 }));
	llvm::Value* increment = ir_builder_->getInt(llvm::APInt(128, {
			0x14057B7EF767814F, 0x5851F42D4C957F2D }));
	...
	/*Name=*/"state_ptr0");
	uint64 graph_seed =
			hlo_module_config_.seed() != 0 ?
					hlo_module_config_.seed() : random_value();
