...
protected:
	Computation BuildSum()
	{
		...
		auto x_value = builder.Parameter(0, ShapeUtil::MakeShape(F32, { }),
				"x_value");
		auto y_value = builder.Parameter(1, ShapeUtil::MakeShape(F32, { }),
				"y_value");
	...
	void TestLinspaceMax(int64 rows, int64 cols)
	{
		...
		std::unique_ptr<Array2D<float>> alhs = MakeLinspaceArray2D(from, to,
				rows, cols);
		auto arhs = MakeUnique<Array2D<float>>(rows, cols, 1.0);

		ComputationBuilder builder(client_,
				tensorflow::strings::Printf("max_%lldx%lld_linspace", rows,
						cols));
		...
		for (int row = 0; row < rows; ++row) {
			for (int col = 0; col < cols; ++col) {
				aexpected(row, col) = std::max((*alhs)(row, col),
						(*arhs)(row, col));
			}
		}
