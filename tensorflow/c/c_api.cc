...
{
	return static_cast<size_t>(tensorflow::DataTypeSize(
			static_cast<DataType>(dt)));
}
...
{
	void* data = tensorflow::cpu_allocator()->AllocateRaw(EIGEN_MAX_ALIGN_BYTES,
			len);
	if (tensorflow::LogMemory::IsEnabled()) {
		tensorflow::LogMemory::RecordRawAllocation(operation,
...
{
	if (tensorflow::LogMemory::IsEnabled()) {
		tensorflow::LogMemory::RecordRawDeallocation("TensorFlow C Api",
		...
		size_t dst_len, TF_Status* status)
{
	...
	if (dst_len < sz) {
		status->status = InvalidArgument("dst_len (", dst_len,
				") too small to encode a ", src_len, "-byte string");
		return 0;
	}
		...
		size_t* dst_len, TF_Status* status)
{
	...
	const char* p = tensorflow::core::GetVarint64Ptr(src, src + src_len,
			&len64);
	if (p == nullptr) {
		status->status = InvalidArgument(
				"invalid string encoding or truncated src buffer");
		return 0;
	}
	if (len64 > std::numeric_limits<size_t>::max()) {
		status->status = InvalidArgument("encoded string is ", len64,
	...
	*dst = Tensor(static_cast<DataType>(src->dtype), src->shape);
	...
	for (tensorflow::int64 i = 0; i < num_elements; ++i) {
		...
		if (static_cast<ptrdiff_t>(offset) >= (limit - data_start)) {
			status->status = InvalidArgument(
					"Malformed TF_STRING tensor; element ", i, " out of range");
			return false;
		}
		size_t len;
		const char* p;
...
TF_Tensor* TF_Tensor_EncodeStrings(const Tensor& src)
{
	...
	for (int i = 0; i < srcarray.size(); ++i) {
		*offsets = (dst - data_start);
		offsets++;
		const tensorflow::string& s = srcarray(i);
		size_t consumed = TF_StringEncode(s.data(), s.size(), dst, dst_len,
				&status);
	...
	return TF_NewTensor(TF_STRING,
			reinterpret_cast<const int64_t*>(dimvec.data()), dimvec.size(),
			base, size, DeleteArray, base);
}
...
static void TF_Run_Setup(int noutputs, TF_Tensor** c_outputs, TF_Status* status)
{
	status->status = Status::OK();
		...
		TF_Status* status)
{
	...
	for (int i = 0; i < ninputs; ++i) {
		...
		} else if (!tensorflow::TF_Tensor_DecodeStrings(src,
				&(*input_pairs)[i].second, status)) {
...
static void TF_Run_Helper(Session* session, const char* handle,
		...
		TF_Buffer* run_metadata, TF_Status* status)
{
	...
	if (handle == nullptr) {
		...
		result = session->Run(run_options_proto, input_pairs,
				output_tensor_names, target_oper_names, &outputs,
				&run_metadata_proto);
		...
		result = session->PRun(handle, input_pairs, output_tensor_names,
				&outputs);
	}
	if (!result.ok()) {
	...
	for (int i = 0; i < noutputs; ++i) {
		...
		if (src.dtype() != tensorflow::DT_STRING) {
			...
			buf->Ref();
			c_outputs[i] = new TF_Tensor {
					static_cast<TF_DataType>(src.dtype()), src.shape(), buf };
		} else {
...
void TF_Run(TF_DeprecatedSession* s, const TF_Buffer* run_options,
		...
		// Target nodes
		const char** c_target_oper_names, int ntargets, TF_Buffer* run_metadata,
		TF_Status* status)
{
	TF_Run_Setup(noutputs, c_outputs, status);
	std::vector<std::pair<tensorflow::string, Tensor>> input_pairs(ninputs);
	if (!TF_Run_Inputs(c_inputs, &input_pairs, status))
		return;
	for (int i = 0; i < ninputs; ++i) {
...
void TF_PRunSetup(TF_DeprecatedSession* s,
		...
		// Target nodes
		const char** c_target_oper_names, int ntargets, const char** handle,
		TF_Status* status)
{
	status->status = Status::OK();
...
// Input tensors
		...
		const char** c_output_names, TF_Tensor** c_outputs, int noutputs,
		// Target nodes
		const char** c_target_oper_names, int ntargets, TF_Status* status)
{
	TF_Run_Setup(noutputs, c_outputs, status);
	std::vector<std::pair<tensorflow::string, Tensor>> input_pairs(ninputs);
	if (!TF_Run_Inputs(c_inputs, &input_pairs, status))
...
TF_Library* TF_LoadLibrary(const char* library_filename, TF_Status* status)
{
	...
	status->status = tensorflow::LoadLibrary(library_filename,
			&lib_handle->lib_handle, &lib_handle->op_list.data,
...
struct TF_Graph {
	TF_Graph() :
			graph(OpRegistry::Global()), refiner(graph.versions().producer(),
					graph.op_registry()), num_sessions(0), delete_requested(
					false)
	{
	}
	mutex mu;Graph
	...
	// TF_DeleteSession.
	int num_sessions GUARDED_BY(mu);bool delete_requested GUARDED_BY(mu);
		...
		const char* attr_name, TF_Status* status)
{
	...
	if (attr == nullptr) {
		status->status = InvalidArgument("Operation has no attr named '",
				attr_name, "'.");
	}
	return attr;
}
...
void TF_GraphSetTensorShape(TF_Graph* graph, TF_Output output,
		const int64_t* dims, const int num_dims, TF_Status* status)
{
	...
	if (ic == nullptr) {
		status->status = InvalidArgument("Node ", node->name(),
...
void TF_ColocateWith(TF_OperationDescription* desc, TF_Operation* op)
{
	desc->colocation_constraints.emplace_back(
			tensorflow::strings::StrCat(tensorflow::kColocationGroupPrefix,
					op->node.name()));
}
		...
		const int64_t* values, int num_values)
{
	...
	desc->node_builder.Attr(attr_name,
			ArraySlice<const tensorflow::int64>(
					reinterpret_cast<const tensorflow::int64*>(values),
					num_values));
}
	...
	if (num_dims >= 0) {
		...
		shape = PartialTensorShape(
				ArraySlice<tensorflow::int64>(
						reinterpret_cast<const tensorflow::int64*>(dims),
						num_dims));
	}
...
void TF_SetAttrShapeList(TF_OperationDescription* desc, const char* attr_name,
		const int64_t* const * dims, const int* num_dims, int num_shapes)
{
	...
	for (int i = 0; i < num_shapes; ++i) {
		if (num_dims[i] < 0) {
			...
			shapes.emplace_back(
					ArraySlice<tensorflow::int64>(
							reinterpret_cast<const tensorflow::int64*>(dims[i]),
							num_dims[i]));
		}
	}
...
void TF_SetAttrTensorShapeProto(TF_OperationDescription* desc,
		const char* attr_name, const void* proto, size_t proto_len,
		TF_Status* status)
{
	...
	if (proto_len > std::numeric_limits<int>::max()) {
		status->status =
				InvalidArgument("proto_len (", proto_len,
...
void TF_SetAttrTensorShapeProtoList(TF_OperationDescription* desc,
		const char* attr_name, const void* const * protos,
		const size_t* proto_lens, int num_shapes, TF_Status* status)
{
	...
	for (int i = 0; i < num_shapes; ++i) {
		if (proto_lens[i] > std::numeric_limits<int>::max()) {
			status->status =
					InvalidArgument("length of element ", i, " in the list (",
							proto_lens[i],
							" bytes) is too large to be parsed by the protocol buffer library");
			return;
		}
		if (!shapes[i].ParseFromArray(protos[i],
				static_cast<int>(proto_lens[i]))) {
			status->status = InvalidArgument(
		...
		TF_Tensor* value, TF_Status* status)
{
	...
	if (ok)
		desc->node_builder.Attr(attr_name, t);
}

void TF_SetAttrTensorList(TF_OperationDescription* desc, const char* attr_name,
		TF_Tensor* const * values, int num_values, TF_Status* status)
{
	status->status = Status::OK();
	std::vector<Tensor> t;
	...
	for (int i = 0; i < num_values && ok; ++i) {
		if (values[i]->dtype != TF_STRING) {
			t.emplace_back(
					tensorflow::TensorCApi::MakeTensor(values[i]->dtype,
							values[i]->shape, values[i]->buffer));
		} else {
			t.emplace_back(::tensorflow::DT_STRING);
			// TF_STRING tensors require copying since Tensor class expects
			// a sequence of string objects.
			ok = tensorflow::TF_Tensor_DecodeStrings(values[i], &t.back(),
					status);
		}
	}
...
{
	return static_cast<TF_DataType>(oper_out.oper->node.output_type(
			oper_out.index));
}
		...
		TF_Status* status)
{
	...
	if (iter == name_ranges.end()) {
		status->status = InvalidArgument("Input arg '", arg_name,
				"' not found");
		return -1;
	}
...
#define LIST_CASE(field, attr_type, ...)              \
  if (attr->list().field##_size() > 0) {              \
    ...
    break;                                            \
		...
		LIST_CASE(s, TF_ATTR_STRING,
				metadata.total_size = 0; for (int i = 0; i < attr->list().s_size(); ++i) { metadata.total_size += attr->list().s(i).size(); })
		;
		...
		;
		LIST_CASE(shape, TF_ATTR_SHAPE,
				metadata.total_size = 0; for (int i = 0; i < attr->list().shape_size(); ++i) { const auto& s = attr->list().shape(i); metadata.total_size += s.unknown_rank() ? 0 : s.dim_size(); })
		;
		...
		if (metadata.list_size == 0) {
			for (int i = 0; i < oper->node.op_def().attr_size(); ++i) {
				...
				if (typestr == "list(string)") {
					...
					status->status = InvalidArgument("Attribute '", attr_name,
...
void TF_OperationGetAttrStringList(TF_Operation* oper, const char* attr_name,
		void** values, size_t* lengths, int max_values, void* storage,
		size_t storage_size, TF_Status* status)
{
	...
	if (attr->value_case() != tensorflow::AttrValue::kList) {
		status->status = InvalidArgument("Value for '", attr_name,
				"' is not a list");
		return;
	}
...
{
	PartialTensorShape shape;
	status->status = tensorflow::GetNodeAttr(oper->node.def(), attr_name,
			&shape);
	if (!status->status.ok())
		return;
...
void TF_OperationGetAttrShapeList(TF_Operation* oper, const char* attr_name,
		int64_t** values, int* num_dims, int max_values, int64_t* storage,
...
void TF_OperationGetAttrTensorShapeProto(TF_Operation* oper,
		const char* attr_name, TF_Buffer* value, TF_Status* status)
{
	const auto* attr = GetAttrValue(oper, attr_name, status);
	if (!status->status.ok())
		return;
...
void TF_OperationGetAttrTensorShapeProtoList(TF_Operation* oper,
		const char* attr_name, TF_Buffer** values, int max_values,
		TF_Status* status)
{
	const auto* attr = GetAttrValue(oper, attr_name, status);
	if (!status->status.ok())
...
void TF_OperationGetAttrTensorList(TF_Operation* oper, const char* attr_name,
		TF_Tensor** values, int max_values, TF_Status* status)
{
	...
	for (int i = 0; i < len; ++i) {
		const Tensor& t = ts[i];
		values[i] = new TF_Tensor { static_cast<TF_DataType>(t.dtype()),
				t.shape(), tensorflow::TensorCApi::Buffer(t) };
		values[i]->buffer->Ref();
	}
}
...
static void GraphImportGraphDefLocked(TF_Graph* graph, const GraphDef& def,
		const TF_ImportGraphDefOptions* opts, TF_Status* status)
		...
		TF_Graph* graph, TF_Buffer* meta_graph_def, TF_Status* status)
{
	...
	status->status = tensorflow::LoadSavedModel(session_options->options,
			run_options_proto, export_dir, tag_set, &bundle);
	if (!status->status.ok())
		return nullptr;
...
static bool ExtendSessionGraphHelper(TF_Session* session, TF_Status* status)
{
	if (session->graph != nullptr) {
		...
		if (session->last_num_graph_nodes < num_nodes) {
			...
			for (auto id = session->last_num_graph_nodes; id < num_nodes;
					++id) {
		...
		TF_Buffer* run_metadata, TF_Status* status)
{
	...
	std::vector<std::pair<tensorflow::string, Tensor>> input_pairs(ninputs);
	if (!TF_Run_Inputs(input_values, &input_pairs, status))
		return;
	for (int i = 0; i < ninputs; ++i) {
