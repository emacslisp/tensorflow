...
LocalExecuteOptions& LocalExecuteOptions::set_device_ordinal(int device_ordinal)
{
	device_ordinal_ = device_ordinal;
	return *this;
}

int LocalExecuteOptions::device_ordinal() const
{
		...
		const ServiceOptions& options)
{
	...
	std::unique_ptr<LocalService> service(
			new LocalService(std::move(backend),
	...
	if (allocate_space_for_deep_copy) {
		TF_CHECK_OK(
				ShapeUtil::ForEachSubshape(shape,
						[&size, transfer_manager](const Shape& subshape,
		...
		bool allocate_space_for_deep_copy)
{
	...
	return allocation_tracker_.Register(execute_backend_.get(), device_ordinal,
			allocation, shape,
...
{
	TF_ASSIGN_OR_RETURN(std::unique_ptr < ShapedBuffer > null_buffer,
			ExecuteLocallyInternal(computation, arguments, options,
					result_buffer));
...
StatusOr<std::vector<std::unique_ptr<AotCompilationResult>>>LocalService::CompileAheadOfTime(
		...
		const ShapedBuffer* preallocated_result_buffer)
{
	...
	if (options.result_layout()) {
		TF_RETURN_IF_ERROR(
				ValidateResultShapeWithLayout(*options.result_layout(),
						program_shape.result()));
	}
	...
	for (int i = 0; i < argument_layouts.size(); ++i) {
		...
		if (!ShapeUtil::Compatible(argument_shape,
				program_shape.parameters(i))) {
			return InvalidArgument(
					"invalid argument shape for argument %d, expected %s, got %s",
					i,
		...
		const Shape* result_layout, int device_ordinal, bool has_hybrid_result)
{
	...
	if (result_layout != nullptr) {
		TF_RETURN_IF_ERROR(
				ValidateResultShapeWithLayout(*result_layout,
						program_shape->result()));
	}
	...
	/*executable_for_compute_constant=*/false, argument_buffers,
