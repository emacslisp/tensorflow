		...
		const DataLayout& from, const DataLayout& to)
{
	...
	std::tie(d_idx_from, b_idx_from, spatial_idx_from) = GetDimIndices(from,
			input.size());
	std::tie(d_idx_to, b_idx_to, spatial_idx_to) = GetDimIndices(to,
			input.size());
...
BatchDescriptor::BatchDescriptor(int ndims) :
		count_(0), feature_map_count_(0), spatial_size_(ndims, 0), value_max_(
				0.0), value_min_(0.0), layout_(DataLayout::kYXDepthBatch), ndims_(
				ndims), quantized_activation_mode_(
				QuantizedActivationMode::k8Bit)
{
}

BatchDescriptor::BatchDescriptor() :
		BatchDescriptor(/*ndims=*/2)
{
}
...
std::vector<int64> BatchDescriptor::full_dims(const DataLayout& layout) const
{
	...
	std::copy(spatial_size_.begin(), spatial_size_.end(),
			bdyx_dims.begin() + 2);
	return ReorderDims(bdyx_dims, DataLayout::kBatchDepthYX, layout);
}

std::vector<int64> BatchDescriptor::full_strides(const DataLayout& layout) const
{
...
string BatchDescriptor::ToString() const
{
	...
	return port::Printf("{count: %lld feature_map_count: %lld spatial: %s "
			"value_min: %f value_max: %f layout: %s}", count_,
			feature_map_count_, spatial.c_str(), value_min_, value_max_,
...
int64 BatchDescriptor::FullyConnectedWeightCount(const BatchDescriptor& input,
...
FilterDescriptor::FilterDescriptor(int ndims) :
		output_feature_map_count_(0), input_feature_map_count_(0), input_filter_dims_(
				ndims, 0), ndims_(ndims), layout_(FilterLayout::kOutputInputYX)
{
}

FilterDescriptor::FilterDescriptor() :
		FilterDescriptor(/*ndims=*/2)
{
}
...
{
	set_output_feature_map_count(other.output_feature_map_count()).set_input_feature_map_count(
			other.input_feature_map_count()).set_layout(other.layout());
...
string FilterDescriptor::ToString() const
{
	string desc = port::Printf(
			"{output_feature_map_count: %lld input_feature_map_count: %lld "
					"layout: %s shape: ", output_feature_map_count_,
			input_feature_map_count_, FilterLayoutString(layout_).c_str());
	for (int i = 0; i < ndims_; i++) {
...
string ConvolutionDescriptor::ToString() const
{
	...
	return port::Printf("{zero_padding: %s filter_strides: %s}",
			padding.c_str(), strides.c_str());
}
...
PoolingDescriptor::PoolingDescriptor(int ndims) :
		mode_(dnn::PoolingMode::kMaximum), ndims_(ndims), window_(ndims, 0), padding_(
				ndims, 0), strides_(ndims, 1)
{
}

PoolingDescriptor::PoolingDescriptor() :
		PoolingDescriptor(/*ndims=*/2)
{
}
...
NormalizeDescriptor::NormalizeDescriptor() :
		bias_(0.0), range_(0), alpha_(0.0), beta_(0.0), wrap_around_(false), segment_size_(
				0)
{
}
...
string NormalizeDescriptor::ToString() const
{
	return port::Printf(
			"{bias: %f range: %d alpha: %f beta: %f wrap_around: %d "
					"segment_size: %d}", bias_, range_, alpha_, beta_,
			wrap_around_, segment_size_);
}

string NormalizeDescriptor::ToShortString() const
{
	return port::StrCat("bias:", bias_, "_range:", range_, "_alpha:", alpha_,
			"_beta:", beta_, "_wrap:", wrap_around_, "_size:", segment_size_);
}
