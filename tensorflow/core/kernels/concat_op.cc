...
enum AxisArgumentName {
	NAME_IS_AXIS, NAME_IS_CONCAT_DIM
};
...
public:
	...
	void Compute(OpKernelContext* c) override
	{
		...
		const int32 concat_dim = internal::SubtleMustCopy(
...
public:
	explicit ConcatOffsetOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, IsLegacyScalar(concat_dim.shape()),
				errors::InvalidArgument(
		...
		const int64 cdim = internal::SubtleMustCopy(
				concat_dim.scalar<int32>()());
		...
		for (int i = 0; i < N; ++i) {
			...
			OP_REQUIRES(ctx, dims == inp.NumElements(),
					errors::InvalidArgument("input ", i, " should contain ",
							dims, " elements, but got ", inp.NumElements()));
			...
			for (int64 j = 0; j < dims; ++j) {
				if (j == axis) {
					...
					OP_REQUIRES(ctx, (inp0_vec(j) == inp_vec(j)),
							errors::InvalidArgument("All dimensions except ",
									axis, " must match. Input ", i,
									" has shape [", inp.SummarizeValue(10),
