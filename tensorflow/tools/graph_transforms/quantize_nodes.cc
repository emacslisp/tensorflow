...
struct QuantizedOpInfo {
	...
	enum {
		CONTIGUOUS_MIN_MAX, SEPARATE_MIN_MAX
	} min_max_order;
};
...
{
	static const std::vector<QuantizedOpInfo> op_list = { { "AvgPool", {
			"ksize", "strides", "padding" }, { { "T", DT_QUINT8 } }, DT_QUINT8,
			DT_QUINT8, { }, QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "BiasAdd",
			{ }, { { "T1", DT_QUINT8 }, { "T2", DT_QUINT8 }, { "out_type",
					DT_QINT32 } }, DT_QUINT8, DT_QINT32, { },
			QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "Concat", { "N" }, { { "T",
			DT_QUINT8 } }, DT_QUINT8, DT_QUINT8, { 0 },
			QuantizedOpInfo::SEPARATE_MIN_MAX }, { "Conv2D", { "strides",
			"padding" }, { { "Tinput", DT_QUINT8 }, { "Tfilter", DT_QUINT8 }, {
			"out_type", DT_QINT32 } }, DT_QUINT8, DT_QINT32, { },
			QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "MatMul", { "transpose_a",
			"transpose_b" }, { { "T1", DT_QUINT8 }, { "T2", DT_QUINT8 }, {
			"Toutput", DT_QINT32 } }, DT_QUINT8, DT_QINT32, { },
			QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "MaxPool", { "ksize",
			"strides", "padding" }, { { "T", DT_QUINT8 } }, DT_QUINT8,
			DT_QUINT8, { }, QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "Relu",
			{ }, { { "Tinput", DT_QUINT8 } }, DT_QUINT8, DT_QUINT8, { },
			QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "Relu6", { }, { { "Tinput",
			DT_QUINT8 } }, DT_QUINT8, DT_QUINT8, { },
			QuantizedOpInfo::CONTIGUOUS_MIN_MAX }, { "Reshape", { }, { { "T",
			DT_QUINT8 } }, DT_QUINT8, DT_QUINT8, { 1 },
...
Status ExtractRangeFromParams(const TransformFuncContext& context,
		const string& min_name, const string& max_name, float* min_value,
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	do {
		...
		for (const NodeDef& node : current_graph_def.node()) {
			...
			if (!input_names.count(node.name())
					&& !output_names.count(node.name())) {
		...
		for (const std::pair<uint64, std::vector<const NodeDef*>> hashed_node_info : hashed_nodes) {
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	TF_RETURN_IF_ERROR(ReplaceMatchingOpTypes(input_graph_def, // clang-format off
			{ "QuantizeV2", { { "Dequantize" }, { "Min" }, { "Max" }, } }, // clang-format on
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	TF_RETURN_IF_ERROR(
			ExtractRangeFromParams(context, "input_min", "input_max",
					&input_min, &input_max, &has_input_range));
	if (!has_input_range) {
	...
	for (const NodeDef& node : input_graph_def.node()) {
		if (node.op() != "Placeholder") {
			...
			(placeholder_graph_def.mutable_node()->Add())->CopyFrom(
					quantized_placeholder);
			...
			// placeholder.
			inputs_to_rename_second_pass[node.name() + rename_suffix] =
					node.name();
		}
	}
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	TF_RETURN_IF_ERROR(ReplaceMatchingOpTypes(input_graph_def, // clang-format off
			{ "Requantize",
					{ { "QuantizeV2", { { "Dequantize", { { "Requantize", { {
							"*" }, { "*" }, { "*" }, { "RequantizationRange" },
							{ "RequantizationRange" }, } }, { "Requantize" }, {
							"Requantize" }, } }, { "Const" }, { "Const" }, }, },
							{ "QuantizeV2" }, { "QuantizeV2" }, { "Const" }, {
									"Const" }, } },  // clang-format on
			[](const NodeMatch& match, const std::set<string>& input_nodes,
					const std::set<string>& output_nodes,
					std::vector<NodeDef>* new_nodes) {
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	for (int depth = max_depth; depth > 0; --depth) {
		...
		TF_RETURN_IF_ERROR(
				ReplaceMatchingOpTypes(current_graph_def, pattern,
		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	GraphDef placeholder_graph_def;
	TF_RETURN_IF_ERROR(
			QuantizePlaceholders(input_graph_def, context,
					&placeholder_graph_def));
	...
	GraphDef hoisted_graph_def;
	TF_RETURN_IF_ERROR(
			HoistFakeQuants(placeholder_graph_def, context,
					&hoisted_graph_def));
	...
	TF_RETURN_IF_ERROR(
			ConvertFakeQuantsToRequantize(hoisted_graph_def, context,
					&converted_graph_def));
	...
	TF_RETURN_IF_ERROR(
			ExtractRangeFromParams(context, "fallback_min", "fallback_max",
					&fallback_min, &fallback_max, &has_fallback_range));

	// Replace all occurrences of the current float op with its quantized
	// equivalent.
	GraphDef quantized_graph_def;
	TF_RETURN_IF_ERROR(
			ReplaceMatchingOpTypes(converted_graph_def, { op_pattern },
	...
	GraphDef merged_graph_def;
	TF_RETURN_IF_ERROR(
			MergeAdjacentRequantizes(quantized_graph_def, context,
					&merged_graph_def));
	...
	TF_RETURN_IF_ERROR(
			RemoveRedundantQuantizations(deduped_graph_def, context,
					output_graph_def));
	TF_RETURN_IF_ERROR(IsGraphValid(merged_graph_def));
