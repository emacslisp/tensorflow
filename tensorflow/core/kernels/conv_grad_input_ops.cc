...
struct LaunchBackwardInputConvolution {
	bool operator()(OpKernelContext* context, const Device&,
			typename TTypes<T, 4>::Tensor, typename TTypes<T, 4>::ConstTensor,
...
struct LaunchBackwardInputConvolution<CPUDevice, float> {
	bool operator()(OpKernelContext* context, const CPUDevice& d,
			...
			int input_rows, int input_cols, int row_stride, int col_stride,
			TensorFormat data_format) const
	{
		functor::SpatialConvolutionBackwardInput<CPUDevice, float>()(d,
				input_backward, kernel, output_backward, input_rows, input_cols,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				Conv2DBackpropComputeDimensions("Conv2DFastBackpropInput",
		...
		LaunchBackwardInputConvolution<Device, T>()(context,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		Conv2DBackpropDimensions dims;
		OP_REQUIRES_OK(context,
				Conv2DBackpropComputeDimensions("Conv2DCustomBackpropInput",
						input_shape, filter.shape(), out_backprop.shape(),
						strides_, padding_, data_format_, &dims));
		...
		OP_REQUIRES_OK(context,
				GetWindowedOutputSizeVerbose(dims.rows.input_size,
						dims.rows.filter_size, dims.rows.stride, padding_,
						&dims.rows.output_size, &pad_top, &pad_bottom));
		OP_REQUIRES_OK(context,
				GetWindowedOutputSizeVerbose(dims.cols.input_size,
						dims.cols.filter_size, dims.cols.stride, padding_,
						&dims.cols.output_size, &pad_left, &pad_right));

		// The total dimension size of each kernel.
		const int filter_total_size = dims.rows.filter_size
				* dims.cols.filter_size * dims.in_depth;
		// The output image size is the spatial size of the output.
		const int output_image_size = dims.rows.output_size
				* dims.cols.output_size;
		...
		const size_t thread_work_unit_size = work_unit_size
		...
		const size_t shard_size =
				use_parallel_contraction ?
						1 :
						(target_working_set_size + work_unit_size - 1)
								/ work_unit_size;

		Tensor col_buffer;
		OP_REQUIRES_OK(context,
				context->allocate_temp(DataTypeToEnum<T>::value, TensorShape( {
						static_cast<int64>(shard_size),
						static_cast<int64>(output_image_size),
						static_cast<int64>(filter_total_size) }), &col_buffer));

		// The input offset corresponding to a single input image.
		const int input_offset = dims.rows.input_size * dims.cols.input_size
				* dims.in_depth;
		// The output offset corresponding to a single output image.
		const int output_offset = dims.rows.output_size * dims.cols.output_size
				* dims.out_depth;
		...
		if (use_parallel_contraction) {
			...
			for (int image_id = 0; image_id < dims.batch_size; ++image_id) {
				...
				TensorMap C(col_buffer_data, output_image_size,
						filter_total_size);

				ConstTensorMap A(out_backprop_data + output_offset * image_id,
						output_image_size, dims.out_depth);
				ConstTensorMap B(filter_data, filter_total_size,
						dims.out_depth);

				C.device(context->eigen_cpu_device()) = A.contract(B,
						contract_dims);
			...
			for (int image_id = 0; image_id < dims.batch_size; image_id +=
				...
				auto shard =
						[&dims, &pad_top, &pad_left, &pad_bottom, &pad_right,
				...
				Shard(worker_threads.num_threads, worker_threads.workers,
						shard_limit, work_unit_size, shard);
