...
public:
	explicit SparseToDense(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* c) override
	{
		...
		OP_REQUIRES_OK(c,
				TensorShapeUtils::MakeShape(output_shape_vec.data(),
						output_shape_vec.size(), &output_tensor_shape));
		...
		if (indices.dtype() == DT_INT64) {
			...
			indices_shaped.matrix<int64>() = indices.shaped<Index, 2>(
					ix_shape.dim_sizes()).template cast<int64>();
		}
		...
		if (TensorShapeUtils::IsScalar(sparse_values.shape())) {
			OP_REQUIRES_OK(c,
					c->allocate_temp(DataTypeToEnum<T>::value, TensorShape( {
							num_elems }), &sparse_values_b));
		...
		sparse::SparseTensor st(indices_shaped, sparse_values_b,
				output->shape(), order);
