...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context, finished.shape().dims() == 1,
				errors::InvalidArgument("finished should be one-dimensional"));
		...
		// Check tensor bounds.
		if (!CheckTensorBounds(context, finished))
			return;
		if (!CheckTensorBounds(context, non_fertile_leaves))
			return;
		if (!CheckTensorBounds(context, non_fertile_leaf_scores))
			return;
		if (!CheckTensorBounds(context, end_of_tree))
			return;
		if (!CheckTensorBounds(context, accumulator_sums))
			return;
		if (!CheckTensorBounds(context, node_to_accumulator))
			return;
		if (!CheckTensorBounds(context, stale_leaves))
			return;
		...
		const int32 num_finished = static_cast<int32>(finished.shape().dim_size(
				0));
		...
		for (int32 i = 0; i < num_finished; ++i) {
			...
			OP_REQUIRES(context, FastBoundsCheck(node, node_map.size()),
					errors::InvalidArgument(
							"finished node is outside the valid range"));
		...
		for (int32 i = 0; i < stale_vec.size(); ++i) {
			...
			OP_REQUIRES(context, FastBoundsCheck(node, node_map.size()),
					errors::InvalidArgument(
							"stale node is outside the valid range"));
			finished_accumulators.insert(node_map(node));
		}
		...
		OP_REQUIRES(context, FastBoundsCheck(eot, num_nodes + 1),
				errors::InvalidArgument(
						"end-of-tree is outside the valid range"));

		const int32 num_new_leaves = std::min(num_finished * 2,
				num_nodes - eot);

		LeafHeapType leaf_heap(
				static_cast<int32>(non_fertile_leaves.shape().dim_size(0))
						+ num_new_leaves, OrderBySecondGreater());
		ConstructLeafHeap(non_fertile_leaves, non_fertile_leaf_scores, eot,
				num_new_leaves,
		...
		// Allocate leaves.
		std::unique_ptr<HeapValuesType> values(leaf_heap.Extract());
		...
		SetAccumulatorsCleared(finished_accumulators, accumulators_to_node,
				context);
...
private:
	...
	// are also set to -1.
	void SetNodeMapUpdates(const std::set<int32>& finished_accumulators,
			const std::unordered_map<int32, int32>& accumulators_to_node,
			const Tensor& finished, const Tensor& stale,
	...
	void SetAccumulatorsCleared(const std::set<int32>& finished_accumulators,
			const std::unordered_map<int32, int32>& accumulators_to_node,
			OpKernelContext* context)
	{
			...
			OpKernelContext* context)
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(3, allocated_shape,
						&output_allocated));
	...
	void ConstructLeafHeap(const Tensor& non_fertile_leaves,
			const Tensor& non_fertile_leaf_scores, int32 end_of_tree,
			int32 num_new_leaves, int32 num_classes, LeafHeapType* leaf_heap)
	{
		const auto leaf_vec = non_fertile_leaves.unaligned_flat<int32>();
		const auto leaf_score_vec =
				non_fertile_leaf_scores.unaligned_flat<float>();
		...
		// gini (classification).
		const float zero_score =
				regression_ ? 0.0 : WeightedGiniImpurity(zeros);
