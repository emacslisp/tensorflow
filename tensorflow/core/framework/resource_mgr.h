...
private:
	...
	mutable mutex mu_;std::unordered_map<string, Container*> containers_ GUARDED_BY(mu_);
	...
	TF_MUST_USE_RESULT;
	Status DoDelete(const string& container, TypeIndex type, const string& name)
	TF_MUST_USE_RESULT;
...
public:
	explicit IsResourceInitialized(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
...
{
	TF_RETURN_IF_ERROR(internal::ValidateDeviceAndType<T>(ctx, p));
	return ctx->resource_manager()->LookupOrCreate(p.container(), p.name(),
			value, creator);
}
...
void IsResourceInitialized<T>::Compute(OpKernelContext* ctx)
{
	...
	T* unused;
	output->flat<bool>()(0) = LookupResource(ctx, HandleFromInput(ctx, 0),
			&unused).ok();
}
...
void ResourceHandleOp<T>::Compute(OpKernelContext* ctx)
{
	...
	output->scalar<ResourceHandle>()() = MakeResourceHandle<T>(ctx, container_,
			name_);
}
