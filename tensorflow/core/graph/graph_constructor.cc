...
bool IsValidNodeName(StringPiece s, bool allow_internal_ops)
{
	...
	return Scanner(s).One(
			allow_internal_ops ?
					Scanner::LETTER_DIGIT_DOT_UNDERSCORE :
					Scanner::LETTER_DIGIT_DOT).Any(
			Scanner::LETTER_DIGIT_DASH_DOT_SLASH_UNDERSCORE).Eos().GetResult();
}
...
public:
	struct Options {
		Options(const GraphConstructorOptions& in) :
				allow_internal_ops(in.allow_internal_ops), expect_device_spec(
						in.expect_device_spec), importing(false)
		{
		}
		Options(const ImportGraphDefOptions& in) :
				allow_internal_ops(false), expect_device_spec(false), prefix(
						in.prefix.empty()
								|| StringPiece(in.prefix).ends_with("/") ?
								in.prefix : in.prefix + "/"), input_map(
						in.input_map), control_dependencies(
						in.control_dependencies), importing(true)
		{
		}
			...
			ShapeRefiner* refiner)
	{
		TF_RETURN_IF_ERROR(
				CheckVersions(gdef->versions(), TF_GRAPH_DEF_VERSION,
						TF_GRAPH_DEF_VERSION_MIN_PRODUCER, "GraphDef",
						"graph"));
...
private:
	GraphConstructor(const Options& opts, const GraphDef* gdef, Graph* g,
			ShapeRefiner* refiner) :
			opts_(opts), gdef_(gdef), g_(g), original_versions_(g->versions()), refiner_(
					refiner)
	{
	}
	...
	struct NodeInfo {
		explicit NodeInfo(int i) :
				gdef_index(i), node(nullptr)
		{
		}
		...
		const StringPiece& node_name)
{
	for (auto iter = input_map.begin(); iter != input_map.end(); ++iter) {
		if (iter->second.first == node_name)
			return true;
	}
	return false;
}
...
Status GraphConstructor::ValidateInputMapAndControlDependencies()
{
	for (const auto& mapping : opts_.input_map) {
		...
		if (existing_nodes_.count(dst.first) == 0) {
			return errors::InvalidArgument("node '", dst.first,
					"' in input_map does not exist in graph ",
	...
	for (const string& node : opts_.control_dependencies) {
		if (existing_nodes_.count(node) == 0) {
			return errors::InvalidArgument("node '", node,
...
Status GraphConstructor::BuildNodeIndex()
{
	...
	for (int n = 0; n < gdef_->node_size(); ++n) {
		...
		if (!gdef_nodes_.insert(
				std::make_pair(StringPiece(node_def.name()), NodeInfo(n))).second) {
			return errors::InvalidArgument("Node '", node_def.name(),
					"' is not unique");
		}
	...
	for (int n = 0; n < num_nodes; ++n) {
		...
		for (int i = 0; i < node_def.input_size(); ++i) {
			...
			if (iter == gdef_nodes_.end()) {
				return errors::InvalidArgument("Node '", node_def.name(),
						"': Unknown input node '", node_def.input(i), "'");
			}
...
Status GraphConstructor::ValidateShape(Node* node)
{
	...
	if (shape_attrs.size() != node->num_outputs()) {
		return errors::InvalidArgument("Node '", node->name(), "' has ",
				node->num_outputs(), " outputs but the ", kAttrName,
	...
	for (int i = 0; i < shape_attrs.size(); ++i) {
		...
		if (!s.ok()) {
			return errors::InvalidArgument("Node '", node->name(),
					" has an invalid ", kAttrName, " attribute (shape #", i,
		...
		if (!s.ok()) {
			...
			const std::vector<string> whitelist = {
					...
					"PriorityQueue", "QueueSize", "Stack", "Barrier",
					"BarrierReadySize", "BarrierIncompleteSize", "HashTable",
					"MutableHashTable", "MutableHashTableOfTensors", "Mutex",
					"CuckooTable", "IndexTable", "WholeFileReader",
		...
		std::vector<bool>* input_already_exists)
{
	...
	if (!inputs_to_remove.empty())
		RemoveInputs(node_def, inputs_to_remove);
}

void GraphConstructor::AddControlDependencies(NodeDef* node_def,
		std::vector<bool>* input_already_exists)
{
	...
	if (node_def->attr().find(kColocationAttrName) != node_def->attr().end()) {
		...
		for (int i = 0; i < list->s_size(); ++i) {
			...
			if (v.Consume(kColocationGroupPrefix)) {
				list->set_s(i,
						strings::StrCat(kColocationGroupPrefix, prefix, v));
			}
		}
	}
}
	...
	while (!ready_.empty()) {
		...
		if (opts_.importing) {
			...
			if (!opts_.control_dependencies.empty()) {
				...
				AddControlDependencies(&imported_node_def,
		...
		for (int i = 0; i < node_def->input_size(); ++i) {
			...
			if (src_node != nullptr && src_index >= src_node->num_outputs()) {
				return errors::InvalidArgument("Node '", node_def->name(),
						"': Connecting to invalid output ", id.second,
						" of source node ", id.first, " which has ",
						src_node->num_outputs(), " outputs");
			}

			inputs.push_back(
					InputInfo(id.first.ToString(), src_node, src_index));
		}
...
Status GraphConstructor::UpdateVersionDef()
{
	...
	versions.set_min_consumer(
			std::max(versions.min_consumer(),
					gdef_->versions().min_consumer()));
		...
		int input_index)
{
	...
	if (!TypesCompatible(dst_in, src_out)) {
		return errors::InvalidArgument("Input ", input_index, " of node ",
				dst->name(), " was passed ", DataTypeString(src_out), " from ",
				src->name(), ":", output_index, " incompatible with expected ",
...
void CopyGraph(const Graph& src, Graph* dest)
{
	...
	// Copy the nodes
	std::unordered_map<Node*, Node*> node_map; // "Node in src" -> "Node in *dest"
