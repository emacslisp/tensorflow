...
SimpleGraphExecutionState::SimpleGraphExecutionState(GraphDef* graph_def,
		const SimpleGraphExecutionStateOptions& options) :
		stateful_placements_(options.stateful_placements), device_set_(
				options.device_set), session_options_(options.session_options), costs_(
				true /*is_global*/), flib_def_(
				new FunctionLibraryDefinition(OpRegistry::Global(),
						graph_def->library())), graph_(nullptr)
{
		...
		std::unique_ptr<SimpleGraphExecutionState>* out_state)
{
	...
	TF_RETURN_IF_ERROR(
			AddDefaultAttrsToGraphDef(&ret->original_graph_def_,
					*ret->flib_def_.get(), 0));
...
/* static */Status SimpleGraphExecutionState::MakeForPrunedGraph(
		...
		const SimpleGraphExecutionStateOptions& options,
		const GraphDef& graph_def, const BuildGraphOptions& subgraph_options,
		std::unique_ptr<SimpleGraphExecutionState>* out_state,
...
Status SimpleGraphExecutionState::Extend(const GraphDef& extension_def,
		std::unique_ptr<SimpleGraphExecutionState>* out) const
{
	...
	if (gdef.has_versions()) {
		if (gdef.versions().producer() != extension_def.versions().producer()) {
			return errors::InvalidArgument("Can't extend GraphDef at version ",
					gdef.versions().producer(), " with graph at version ",
					extension_def.versions().producer());
		}
		VersionDef* versions = gdef.mutable_versions();
		versions->set_min_consumer(
				std::max(versions->min_consumer(),
						extension_def.versions().min_consumer()));
	...
	*gdef.mutable_library() = flib_def_->ToProto();
	...
	TF_RETURN_IF_ERROR(
			AddDefaultAttrsToGraphDef(&new_execution_state->original_graph_def_,
					*flib_def_.get(), 0));
	if (!session_options_->config.graph_options().place_pruned_graph()) {
		...
		TF_RETURN_IF_ERROR(
				new_execution_state->InitBaseGraph(BuildGraphOptions()));
	}
		...
		const BuildGraphOptions& options)
{
	...
	if (session_options_
		...
		TF_RETURN_IF_ERROR(
				subgraph::RewriteGraphForExecution(new_graph.get(),
						options.feed_endpoints, options.fetch_endpoints,
		...
		std::unique_ptr<SimpleClientGraph>* out)
{
	...
	if (session_options_ == nullptr
		...
		// ops as needed.
		TF_RETURN_IF_ERROR(
				subgraph::RewriteGraphForExecution(ng.get(),
						options.feed_endpoints, options.fetch_endpoints,
						options.target_nodes,
						device_set_->client_device()->attributes()));
	}
