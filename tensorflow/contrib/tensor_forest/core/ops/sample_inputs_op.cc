...
public:
			...
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("split_initializations_per_input",
						&split_initializations_per_input_));
		OP_REQUIRES_OK(context,
				context->GetAttr("split_sampling_random_seed",
						&split_sampling_random_seed_));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (sparse_input) {
			...
			if (have_weights) {
				OP_REQUIRES(context,
						sparse_input_shape.unaligned_flat<int64>()(0)
		...
		OP_REQUIRES(context, leaves.shape().dims() == 1,
				errors::InvalidArgument("leaves should be one-dimensional"));
		...
		OP_REQUIRES(context, split_features.shape() == split_thresholds.shape(),
				errors::InvalidArgument(
		...
		if (!CheckTensorBounds(context, node_to_accumulator))
			return;
		if (!CheckTensorBounds(context, leaves))
			return;
		if (!CheckTensorBounds(context, split_features))
			return;
		if (!CheckTensorBounds(context, split_thresholds))
			return;
		...
		const int32 num_splits =
				static_cast<int32>(split_features.shape().dim_size(1));
		const int32 num_accumulators =
				static_cast<int32>(split_features.shape().dim_size(0));
		...
		for (int32 i = 0; i < num_data; i++) {
			...
			if (accumulator >= 0
					&& !IsAllInitialized(
							split_features.Slice(accumulator,
									accumulator + 1))) {
		...
		int32 num_output_accumulators =
				static_cast<int32>(accumulator_to_leaves.size());
		...
		for (const auto& active : accumulator_to_leaves) {
			...
			VLOG(1) << "Accumulator " << accumulator << " gets new output slot "
					<< output_slot;
			...
			for (int split = 0; split < num_splits; split++) {
				new_split_feature_rows_flat(output_slot, split) = features(
						accumulator, split);
				new_split_threshold_rows_flat(output_slot, split) = thresholds(
						accumulator, split);
			}
			...
			for (int split = 0;
					split < num_splits && it != inputs_for_accumulator.end();
					split++) {
				if (new_split_feature_rows_flat(output_slot, split) < 0) {
					if (have_weights) {
						...
						while (it != inputs_for_accumulator.end()) {
							...
							increment_input(split_initializations_per_input_,
									&it, &input_used_count);
						}
					...
					const int32 rand_feature = rng_->Uniform(
							num_total_features);
