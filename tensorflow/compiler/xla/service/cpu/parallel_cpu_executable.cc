...
ParallelCpuExecutable::ParallelCpuExecutable(std::unique_ptr<SimpleOrcJIT> jit,
		...
		Executable(std::move(hlo_module), std::move(module_config)), jit_(
				std::move(jit)), assignment_(std::move(assignment)), functions_names_(
				std::move(function_names)), hlo_to_profile_idx_(
				std::move(hlo_to_profile_idx)), aligned_constants_(
...
StatusOr<perftools::gputools::DeviceMemoryBase> ParallelCpuExecutable::ExecuteOnStream(
		...
		HloExecutionProfile* hlo_execution_profile)
{
	...
	for (auto* instruction : module().entry_computation()->MakeInstructionPostOrder()) {
		...
		if (instruction->opcode() == HloOpcode::kParameter) {
			...
			unsigned char* aligned_data = FindOrDie(aligned_constants_,
					instruction).get();
	...
	for (auto i = 0; i < device_allocations.size(); ++i) {
		...
		if (marked_addresses.count(alloc.opaque()) == 0
			...
			VLOG(3) << "ParallelCpuExecutable deallocating buffer #" << i
					<< " [" << alloc.opaque() << "]";
			TF_RETURN_IF_ERROR(
					memory_allocator->Deallocate(device_ordinal, &alloc));
		}
	}
...
StatusOr<perftools::gputools::DeviceMemoryBase> ParallelCpuExecutable::ExecuteAsyncOnStream(
		const ExecutableRunOptions* run_options,
		tensorflow::gtl::ArraySlice<se::DeviceMemoryBase> arguments)
{
