...
FakeInputImpl::FakeInputImpl(const OpDef* op_def, int in_index,
		...
		op_def_(op_def), arg_(&op_def->input_arg(in_index)), in_node_(
				FakeNodeName(in_index)), node_def_(node_def), builder_(builder), n_specified_(
				false), dt_specified_(false), dts_specified_(false)
{
}
...
Status FakeInputImpl::AddInputToBuilder()
{
	if (dts_specified_) {
		...
		if (!dt_specified_ && !arg_->type_list_attr().empty()) {
			DataTypeVector dts;
			Status status = GetNodeAttr(*node_def_, arg_->type_list_attr(),
					&dts);
			if (!status.ok()) {
				return errors::InvalidArgument(
						"Could not infer list of types for input '",
						arg_->name(), "': ", status.error_message());
			}
			SourceList(dts);
			return Status::OK();
		}
