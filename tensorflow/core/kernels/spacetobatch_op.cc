...
void SpaceToBatchOpCompute(OpKernelContext* context,
		const Tensor& orig_input_tensor, const Tensor& orig_block_shape,
		const Tensor& orig_paddings)
{
	...
	OP_REQUIRES(context,
			...
			errors::InvalidArgument("paddings should have shape [", block_dims,
	...
	const int internal_block_dims = block_dims - removed_prefix_block_dims
			- removed_suffix_block_dims;
	...
	for (int block_dim = removed_prefix_block_dims;
		...
		const int64 pad_start = paddings[2 * block_dim], pad_end = paddings[2
				* block_dim + 1];
		...
		OP_REQUIRES(context, padded_size % block_shape_value == 0,
				errors::InvalidArgument("padded_shape[", block_dim, "]=",
						padded_size, " is not divisible by block_shape[",
...
public:
	explicit SpaceToBatchOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		OP_REQUIRES_OK(context, context->GetAttr("block_size", &block_size_));
		OP_REQUIRES(context, block_size_ > 1,
				errors::InvalidArgument("Block size should be > 1: ",
						block_size_));
