...
protected:
	void RecvFromRemoteAsync(const Rendezvous::ParsedKey& parsed,
			const Rendezvous::Args& args, DoneCallback done) override;
...
private:
	...
	mutex mu_;std::vector<RpcRecvTensorCall*> objects_ GUARDED_BY(mu_);
};
...
private:
	...
	mutex mu_;std::unordered_map<string, WorkerState> workers_ GUARDED_BY(mu_);
};
		...
		DoneCallback done)
{
	...
	call->Init(rwi, step_id_, parsed.FullKey(), recv_args.alloc_attrs,
...
RpcRendezvousMgr::RpcRendezvousMgr(const WorkerEnv* env) :
		BaseRendezvousMgr(env), cache_(
				new WorkerFreeListCache(env->worker_cache))
{
}
