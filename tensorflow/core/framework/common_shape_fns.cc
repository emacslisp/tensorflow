		...
		int64* padding_before, int64* padding_after)
{
	...
	case Padding::SAME:
		*output_size = (input_size + stride - 1) / stride;
		const int64 padding_needed = std::max(0LL,
				(*output_size - 1) * stride + filter_size - input_size);
...
Status GetWindowedOutputSize(int64 input_size, int64 filter_size, int64 stride,
		Padding padding_type, int64* output_size, int64* padding)
{
	int64 padding_after_unused;
	return GetWindowedOutputSizeVerbose(input_size, filter_size, stride,
			padding_type, output_size, padding, &padding_after_unused);
}
		...
		std::array<int64, 3>* padding_ptr)
{
	for (size_t i = 0; i < input.size(); ++i) {
		TF_RETURN_IF_ERROR(
				GetWindowedOutputSize(input[i], window[i], strides[i],
						padding_type, &(*output_ptr)[i], &(*padding_ptr)[i]));
	}
	return Status::OK();
}
...
Status GetWindowedOutputSizeFromDims(shape_inference::InferenceContext* c,
		...
		Padding padding_type, shape_inference::DimensionHandle* output_size)
{
	...
	case Padding::VALID:
		...
		TF_RETURN_IF_ERROR(
				c->Divide(*output_size, stride, false /* evenly_divisible */,
						output_size));
		break;
	case Padding::SAME:
...
Status BiasAddShape(shape_inference::InferenceContext* c)
{
	...
	if (s.ok() && data_format == "NCHW") {
		...
		TF_RETURN_IF_ERROR(
				c->Merge(input_bias_dim, bias_dim, &merged_bias_dim));
...
Status Conv2DShape(shape_inference::InferenceContext* c)
{
	...
	if (s.ok() && data_format == "NCHW") {
		...
		input_shape = c->MakeShape(
				{ { c->Dim(input_shape, 0), c->Dim(input_shape, 2), c->Dim(
						input_shape, 3), c->Dim(input_shape, 1) } });
	...
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_rows_dim, filter_rows_dim,
					stride_rows, padding, &output_rows));
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_cols_dim, filter_cols_dim,
	...
	if (data_format == "NCHW") {
		...
		output_shape = c->MakeShape( { batch_size_dim, output_rows, output_cols,
				output_depth_dim });
	}
...
Status Conv3DShape(shape_inference::InferenceContext* c)
{
	...
	ShapeHandle output_shape = c->MakeShape( { batch_size_dim, output_planes,
			output_rows, output_cols, output_depth_dim });
	c->set_output(0, output_shape);
	return Status::OK();
}
...
Status DepthwiseConv2DNativeShape(shape_inference::InferenceContext* c)
{
	...
	TF_RETURN_IF_ERROR(
			c->Multiply(input_depth, depth_multiplier, &output_depth));
	...
	ShapeHandle output_shape = c->MakeShape( { batch_size_dim, output_rows,
...
Status AvgPoolShape(shape_inference::InferenceContext* c)
{
	...
	DimensionHandle output_rows, output_cols;
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_rows_dim, kernel_rows,
					stride_rows, padding, &output_rows));
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_cols_dim, kernel_cols,
					stride_cols, padding, &output_cols));
...
Status MaxPoolShape(shape_inference::InferenceContext* c)
{
	...
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_cols_dim, kernel_cols,
					stride_cols, padding, &output_cols));
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_depth_dim, kernel_depth,
					stride_depth, padding, &output_depth));
	...
	if (data_format == "NCHW") {
		...
		output_shape = c->MakeShape(
				{ c->Dim(output_shape, 0), c->Dim(output_shape, 3), c->Dim(
						output_shape, 1), c->Dim(output_shape, 2) });
	}
...
Status Pool3DShape(shape_inference::InferenceContext* c)
{
	...
	DimensionHandle output_planes, output_rows, output_cols;
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_planes_dim, kernel_planes,
					stride_planes, padding, &output_planes));
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSizeFromDims(c, in_rows_dim, kernel_rows,
					stride_rows, padding, &output_rows));
	...
	for (int i = 0; i < reduction_indices_t->NumElements(); ++i) {
		...
		if (reduction_index < -input_rank || reduction_index >= input_rank) {
			return errors::InvalidArgument("Invalid reduction dimension ",
					reduction_index, " for input with ", input_rank,
					" dimensions.");
		}
...
Status ConcatShape(InferenceContext* c, int num_inputs_to_concat)
{
	return ConcatShapeHelper(c, 1 /* start_value_index */,
			1 + num_inputs_to_concat /* end_value_index */, 0 /* dim_index */);
}
...
Status BroadcastBinaryOpShapeFn(InferenceContext* c)
{
	...
	for (int i = 0; i < rank_out; ++i) {
		...
		const auto dim_y =
				dim_y_is_one ?
						dim_one : c->Dim(shape_y, i - (rank_out - rank_y));
		...
		if (c->ValueKnown(shape_rank_dim)) {
			...
			if (index_rank != shape_rank) {
				return errors::InvalidArgument("Index rank (", index_rank,
						") and shape rank (", shape_rank, ") do not match.");
			}
		}
	}
