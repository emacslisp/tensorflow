...
public:
	void ExecuteComputationR0F32(const Computation& computation,
			...
			float expected_result, bool expect_cache_hit)
	{
		...
		std::unique_ptr<Literal> result =
				...
				/*execution_options=*/nullptr, &execution_profile).ConsumeValueOrDie();
		LiteralTestUtil::ExpectNear(
				*LiteralUtil::CreateR0<float>(expected_result), *result,
				error_spec_);
		EXPECT_EQ(expect_cache_hit, execution_profile.compilation_cache_hit());
	}

	void ExecuteComputationR2F32(const Computation& computation,
			tensorflow::gtl::ArraySlice<GlobalData*> arguments,
			std::initializer_list<std::initializer_list<float>> expected_result,
			bool expect_cache_hit)
	{
		ExecutionProfile execution_profile;
		auto data_handle = client_->Execute(computation, arguments, /*execution_options=*/
				nullptr, &execution_profile).ConsumeValueOrDie();
		std::unique_ptr<Literal> result =
				client_->Transfer(*data_handle).ConsumeValueOrDie();
		LiteralTestUtil::ExpectNear(
				*LiteralUtil::CreateR2<float>(expected_result), *result,
				error_spec_);
		EXPECT_EQ(expect_cache_hit, execution_profile.compilation_cache_hit());
	}
