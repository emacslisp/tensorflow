...
public:
	ScopedTracer(StreamExecutor *stream_exec, BeginCallT begin_call,
			...
			stream_exec_(stream_exec), complete_call_(complete_call), result_(
					result)
	{
		if (stream_exec_->tracing_enabled_) {
			correlation_id_ = correlation_id_generator.fetch_add(1,
...
template<typename BeginCallT, typename CompleteCallT, typename ReturnT,
		typename ... BeginArgsT>
ScopedTracer<BeginCallT, CompleteCallT, ReturnT, BeginArgsT...> MakeScopedTracer(
		StreamExecutor *stream_exec, BeginCallT begin_call,
		CompleteCallT complete_call, ReturnT *result, BeginArgsT ... begin_args)
{
...
StreamExecutor::StreamExecutor(PlatformKind platform_kind,
		...
		platform_(nullptr), implementation_(
				StreamExecutorImplementationFromPlatformKind(platform_kind,
						plugin_config)), platform_kind_(platform_kind), device_ordinal_(
				-1), background_threads_(
				new port::ThreadPool(port::Env::Default(), "stream_executor",
						kNumBackgroundThreads)), live_stream_count_(0), tracing_enabled_(
				false)
{
...
StreamExecutor::StreamExecutor(const Platform *platform,
		...
		platform_(platform), implementation_(implementation), device_ordinal_(
				-1), background_threads_(
				new port::ThreadPool(port::Env::Default(), "stream_executor",
						kNumBackgroundThreads)), live_stream_count_(0), tracing_enabled_(
				false)
	...
	if (FLAGS_check_gpu_leaks) {
		for (auto it : mem_allocs_) {
			LOG(INFO) << "Memory alloced at executor exit: addr: "
					<< port::Printf("%p", it.first) << ", bytes: "
					<< it.second.bytes << ", trace: \n"
		...
		SharedMemoryConfig config)
{
	if (config != SharedMemoryConfig::kDefault
		...
		string error_msg = port::Printf(
				"Invalid shared memory config specified: %d",
...
port::StatusOr<std::unique_ptr<dnn::RnnDescriptor>> StreamExecutor::createRnnDescriptor(
		...
		uint64 seed, ScratchAllocator *state_allocator)
{
	...
	return dnn_support->createRnnDescriptor(num_layers, hidden_size, input_size,
			input_mode, direction_mode, rnn_mode, data_type, dropout, seed,
			state_allocator);
}

port::StatusOr<std::unique_ptr<dnn::RnnSequenceTensorDescriptor>> StreamExecutor::createRnnSequenceTensorDescriptor(
		int seq_length, int batch_size, int data_size, dnn::DataType data_type)
{
	...
	return dnn_support->createRnnSequenceTensorDescriptor(seq_length,
			batch_size, data_size, data_type);
}

port::StatusOr<std::unique_ptr<dnn::RnnStateTensorDescriptor>> StreamExecutor::createRnnStateTensorDescriptor(
		...
		const KernelArgsArrayBase &args)
{
	...
	return implementation_->Launch(stream, thread_dims, block_dims, kernel,
			args);
}
...
{
	VLOG(1) << "Called StreamExecutor::HostMemoryUnregister(location="
			<< location << ")" << StackTraceIfVLOG10();
...
bool StreamExecutor::SynchronousMemZero(DeviceMemoryBase *location, uint64 size)
{
		...
		const void *host_src, uint64 size)
{
	VLOG(1) << "Called StreamExecutor::SynchronousMemcpy(gpu_dst="
			<< gpu_dst->opaque() << ", host_src=" << host_src << ", size="
			<< size << ") H2D" << StackTraceIfVLOG10();
		...
		const DeviceMemoryBase &gpu_src, int64 size, void *host_dst)
{
	VLOG(1) << "Called StreamExecutor::SynchronousMemcpyD2H(gpu_src="
			<< gpu_src.opaque() << ", size=" << size << ", host_dst="
			<< host_dst << ")" << StackTraceIfVLOG10();

	port::Status result { port::Status::OK() };
	SCOPED_TRACE(TraceListener::SynchronousMemcpyD2H, &result, gpu_src, size,
			host_dst);
		...
		return port::Status { port::error::INTERNAL,
				port::Printf(
						"failed to synchronously memcpy device-to-host: GPU %p to host %p "
								"size %lld", gpu_src.opaque(), host_dst, size) };
	}
...
port::Status StreamExecutor::SynchronousMemcpyH2D(const void *host_src,
		int64 size, DeviceMemoryBase *gpu_dst)
{
	...
	if (!implementation_->SynchronousMemcpy(gpu_dst, host_src, size)) {
		result = port::Status { port::error::INTERNAL, port::Printf(
				"failed to synchronously memcpy host-to-device: host "
						"%p to GPU %p size %lld", host_src, gpu_dst->opaque(),
				size) };
	}
