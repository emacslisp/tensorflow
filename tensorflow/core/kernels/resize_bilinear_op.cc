...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		typename TTypes<float, 4>::Tensor output_data = st.output->tensor<float,
				4>();

		functor::ResizeBilinear<Device, T>()(context->eigen_device<Device>(),
				image_data, st.height_scale, st.width_scale, output_data);
	}
...
enum ImageScalePattern {
	SCALE_UP, SIMILAR, SCALE_DOWN
};

inline ImageScalePattern compute_image_scale_pattern(const int64 out_height,
		const int64 out_width, const int64 in_height, const int64 in_width)
{
...
inline float compute_lerp(const float top_left, const float top_right,
		const float bottom_left, const float bottom_right, const float x_lerp,
		const float y_lerp)
{
...
inline float image_lerp(const T* input_image, int64 in_x_lower,
		int64 in_x_upper, float xs_lerp, int64 in_y_lower, int64 in_y_upper,
		float ys_lerp, int c)
{
...
void scale_down_image(typename TTypes<T, 4>::ConstTensor images,
		const int batch_size, const int64 out_height, const int64 out_width,
		const int channels, const std::vector<CachedInterpolation>& xs,
...
void scale_down_image(typename TTypes<T, 4>::ConstTensor images,
		const int batch_size, const int64 out_height, const int64 out_width,
		const int channels, const std::vector<CachedInterpolation>& xs_vec,
		const std::vector<CachedInterpolation>& ys,
		typename TTypes<float, 4>::Tensor output)
{
		...
		for (int b = 0; b < batch_size; ++b) {
			...
			for (int64 y = 0; y < out_height; ++y) {
				...
				for (int64 x = 0; x < out_width; ++x) {
					...
					const float out0 = compute_lerp(top_left0, top_right0,
							bottom_left0, bottom_right0, xs_lerp, ys_lerp);
					...
					const float out1 = compute_lerp(top_left1, top_right1,
							bottom_left1, bottom_right1, xs_lerp, ys_lerp);
					...
					const float out2 = compute_lerp(top_left2, top_right2,
							bottom_left2, bottom_right2, xs_lerp, ys_lerp);
				...
				for (int64 x = 0; x < out_width; ++x) {
					for (int c = 0; c < channels; ++c) {
						const float top_left(
								images(b, ys[y].lower, xs[x].lower, c));
						const float top_right(
								images(b, ys[y].lower, xs[x].upper, c));
						const float bottom_left(
								images(b, ys[y].upper, xs[x].lower, c));
						const float bottom_right(
								images(b, ys[y].upper, xs[x].upper, c));
						output(b, y, x, c) = compute_lerp(top_left, top_right,
								bottom_left, bottom_right, xs[x].lerp,
								ys[y].lerp);
					}
				}
			}
	...
	for (int64 y = 0; y < out_height; y += ys[y].consecutive) {
		...
		for (int64 x = 0; x < out_width; x += xs[x].consecutive) {
			...
			for (int c = 0; c < channels; ++c) {
				...
				const float top_right(input_image[in_y_lower + in_x_upper + c]);
				const float bottom_left(
						input_image[in_y_upper + in_x_lower + c]);
				const float bottom_right(
						input_image[in_y_upper + in_x_upper + c]);
				for (int64 y_inner = y; y_inner < y + ys[y].consecutive;
						++y_inner) {
					for (int64 x_inner = x; x_inner < x + xs[x].consecutive;
							++x_inner) {
						output(batch_index, y_inner, x_inner, c) = compute_lerp(
	...
	if (channels == 3) {
		...
		for (int64 y = 0; y < out_height; ++y) {
			...
			for (int64 x = 0; x < out_width; ++x) {
				...
				const float out0 = image_lerp(input_image, in_x_lower,
						in_x_upper, xs_lerp, in_y_lower, in_y_upper, ys_lerp,
						0);
				const float out1 = image_lerp(input_image, in_x_lower,
						in_x_upper, xs_lerp, in_y_lower, in_y_upper, ys_lerp,
						1);
				const float out2 = image_lerp(input_image, in_x_lower,
						in_x_upper, xs_lerp, in_y_lower, in_y_upper, ys_lerp,
						2);
				float* dest = &output(b, y, x, 0);
				dest[0] = out0;
		...
		for (int64 y = 0; y < out_height; ++y) {
			...
			for (int64 x = 0; x < out_width; ++x) {
				...
				for (int c = 0; c < channels; ++c) {
					const float top_left(
							input_image[in_y_lower + in_x_lower + c]);
					const float top_right(
							input_image[in_y_lower + in_x_upper + c]);
					const float bottom_left(
							input_image[in_y_upper + in_x_lower + c]);
					const float bottom_right(
							input_image[in_y_upper + in_x_upper + c]);
					output(b, y, x, c) = compute_lerp(top_left, top_right,
							bottom_left, bottom_right, xs_lerp, ys_lerp);
				}
			}
		}
	}
...
struct ResizeBilinear<CPUDevice, T> {
	void operator()(const CPUDevice& d,
			typename TTypes<T, 4>::ConstTensor images, const float height_scale,
			const float width_scale, typename TTypes<float, 4>::Tensor output)
	{
		...
		const ImageScalePattern scale_pattern = compute_image_scale_pattern(
		...
		if (scale_pattern == SCALE_UP) {
			...
			// Do not eagerly convert all input data points, as we ignore most.
			scale_down_image<T>(images, batch_size, out_height, out_width,
					channels, xs, ys, output);
		} else {
			for (int b = 0; b < batch_size; ++b) {
				scale_similar_image<T>(&images(b, 0, 0, 0), b, out_height,
						out_width, channels, in_height, in_width, xs, ys,
						output);
			}
		}
	}
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		typename TTypes<float, 4>::ConstTensor input_grad = input.tensor<float,
				4>();
		...
		for (int64 b = 0; b < batch; ++b) {
			for (int64 y = 0; y < resized_height; ++y) {
				...
				const int64 bottom_y_index = std::min(
						static_cast<int64>(ceilf(in_y)), original_height - 1);
				...
				for (int64 x = 0; x < resized_width; ++x) {
					...
					const int64 right_x_index = std::min(
							static_cast<int64>(ceilf(in_x)),
							original_width - 1);
					const float x_lerp = in_x - left_x_index;
					const float inverse_x_lerp = (1.0f - x_lerp);
					for (int64 c = 0; c < channels; ++c) {
						output_grad(b, top_y_index, left_x_index, c) += T(
								input_grad(b, y, x, c) * inverse_y_lerp
										* inverse_x_lerp);
						output_grad(b, top_y_index, right_x_index, c) += T(
								input_grad(b, y, x, c) * inverse_y_lerp
										* x_lerp);
						output_grad(b, bottom_y_index, left_x_index, c) += T(
								input_grad(b, y, x, c) * y_lerp
										* inverse_x_lerp);
						output_grad(b, bottom_y_index, right_x_index, c) += T(
