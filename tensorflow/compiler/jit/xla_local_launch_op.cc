		...
		int device_ordinal, uint64 size, bool retry_on_failure)
{
	...
	Tensor t;
	Status status = op_context_->allocate_temp(DT_UINT8, TensorShape( {
			static_cast<int64>(size) }), &t, allocator_attrs, allocation_attrs);
	if (!status.ok()) {
...
Status XlaAllocator::MakeTensorFromBuffer(gpu::DeviceMemoryBase buffer,
		DataType dtype, const TensorShape& shape, Tensor* out_tensor) const
{
...
Status XlaLocalLaunchOp::BuildCompilationCache(XlaCompilationCache** compiler)
{
	...
	if (device_type_ == DeviceType(DEVICE_CPU)) {
		...
		return errors::InvalidArgument(
				"Unknown device type for local _XlaLaunch");
	}
...
void XlaLocalLaunchOp::Compute(OpKernelContext* ctx)
{
	...
	OP_REQUIRES_OK(ctx,
			rm->LookupOrCreate < XlaCompilationCache
					> (rm->default_container(), "xla_compiler", &compiler, [this](XlaCompilationCache** compiler) {
	...
	xla::LocalClient* client =
			static_cast<xla::LocalClient*>(compiler->client());
	...
	if (!kernel->computation.IsNull()) {
		...
		for (int i = 0; i < kernel->xla_input_shapes.size(); ++i) {
			...
			arg_buffers[i] =
					xla::ShapedBuffer::MakeArrayShapedBuffer(shape,
							client->platform(),
							client->default_device_ordinal(), dmem).ConsumeValueOrDie();
		...
		if (kernel->requires_runtime_context) {
			...
			arg_buffers.push_back(
					xla::ShapedBuffer::MakeArrayShapedBuffer(
							...
							local_runtime_context_dmem).ConsumeValueOrDie());
		...
		if (local_runtime_context.error) {
			ctx->CtxFailure(
					errors::InvalidArgument("Compiled kernel returned error: ",
	...
	for (int i = 0; i < ctx->num_outputs(); ++i) {
		if (kernel->outputs[i].is_constant) {
			...
			if (stream && total_bytes > 0) {
				...
				TF_CHECK_OK(
						ctx->allocate_output(i, const_tensor.shape(),
								&output_tensor));
			...
			OP_REQUIRES_OK(ctx,
					xla_allocator.MakeTensorFromBuffer(buffer,
