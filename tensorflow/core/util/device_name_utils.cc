		...
		const ParsedName& more_specific)
{
	if (less_specific.has_job
			&& (!more_specific.has_job
					|| (less_specific.job != more_specific.job))) {
		return false;
	}
	...
	if (less_specific.has_task
			&& (!more_specific.has_task
					|| (less_specific.task != more_specific.task))) {
		return false;
	}
	if (less_specific.has_type
			&& (!more_specific.has_type
					|| (less_specific.type != more_specific.type))) {
		return false;
	}
...
Status DeviceNameUtils::MergeDevNames(ParsedName* target,
		const ParsedName& other, bool allow_soft_placement)
{
	if (other.has_job) {
		...
		task->reserve(
				(pn.has_job ? (5 + pn.job.size()) : 0)
						+ (pn.has_replica ?
								...
								0)
						+ (pn.has_task ?
								(6 + 4 /*estimated UB for # task digits*/) : 0));
		if (pn.has_job) {
