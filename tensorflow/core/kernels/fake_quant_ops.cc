...
// NOLINT This is needed in CUDA mode, do not remove.
...
class FakeQuantWithMinMaxArgsOp: public UnaryElementWiseOp<float,
		FakeQuantWithMinMaxArgsOp<Device>> {
public:
	...
	void Operate(OpKernelContext* context, const Tensor& input, Tensor* output)
	{
		...
		functor(context->eigen_device<Device>(), input.flat<float>(), min_,
				max_, output->flat<float>());
	}
private:
	float min_;
	float max_;
...
// core/ops/array_ops.cc.
template<typename Device>
class FakeQuantWithMinMaxArgsGradientOp: public BinaryElementWiseOp<float,
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		default:
			context->SetStatus(
					InvalidArgument("Only inputs of dimensions 1, 2 or "
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		case 2: {
			...
			functor(context->eigen_device<Device>(), input.dim_size(0),
					input.dim_size(1), gradient.flat<float>(),
					input.flat<float>(), min.vec<float>(), max.vec<float>(),
#ifndef FAKE_QUANT_NO_DEBUG
					check_min_max->scalar<bool>(),
#endif
