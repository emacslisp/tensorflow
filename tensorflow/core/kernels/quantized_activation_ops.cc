...
public:
	explicit QuantizedReluOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const T min_as_quantized = FloatToQuantized<T>(0.0f, min_input,
				max_input);
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const T min_as_quantized = FloatToQuantized<T>(0.0f, min_input,
				max_input);
		const T max_as_quantized = FloatToQuantized<T>(6.0f, min_input,
				max_input);
		...
		if (meta::IsSupportedAndEnabled() && std::is_same<T, quint8>()) {
			...
			output->flat<T>().device(context->eigen_cpu_device()) =
					input.flat<T>().cwiseMax(min_as_quantized).cwiseMin(
							max_as_quantized).template cast<T>();
		}
