...
private:
	...
	mutex mu_;std::unordered_map<string, SharedGrpcChannelPtr> channels_ GUARDED_BY(mu_);
};
...
public:
	SparseGrpcChannelCache(const string& job_id,
			...
			job_id_(job_id), host_ports_(host_ports), channel_func_(
					std::move(channel_func))
	{
	...
	string ToString()
	{
		...
		for (const auto& id_host_port : host_ports_) {
			task_strings.emplace_back(
					strings::StrCat(id_host_port.first, " -> ",
							id_host_port.second));
		}
		return strings::StrCat(job_id_, " -> {",
				str_util::Join(task_strings, ", "), "}");
	}
		...
		ChannelCreationFunction channel_func)
{
	...
	for (auto& job : spec.host_ports_jobs()) {
		caches.push_back(
				new SparseGrpcChannelCache(job.job_id, job.host_ports,
						channel_func));
	}
