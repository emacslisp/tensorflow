...
ObjectTracker::ObjectTracker(const TrackerConfig* const config,
		...
		config_(config), frame_width_(config->image_size.width), frame_height_(
				config->image_size.height), curr_time_(0), num_frames_(0), flow_cache_(
				&config->flow_config), keypoint_detector_(
				&config->keypoint_detector_config), curr_num_frame_pairs_(0), first_frame_index_(
				0), frame1_(new ImageData(frame_width_, frame_height_)), frame2_(
				new ImageData(frame_width_, frame_height_)), detector_(
				detector), num_detected_(0)
{
...
void ObjectTracker::FindCorrespondences(FramePair* const frame_pair) const
{
	...
	for (int i_feat = 0; i_feat < frame_pair->number_of_keypoints_; ++i_feat) {
		...
		if (flow_cache_.FindNewPositionOfPoint(keypoint1->pos_.x,
				keypoint1->pos_.y, &keypoint2->pos_.x, &keypoint2->pos_.y)) {
	...
	LOGV("Found %d of %d keypoint correspondences", num_keypoints_found,
			frame_pair->number_of_keypoints_);
}

void ObjectTracker::NextFrame(const uint8* const new_frame,
		const uint8* const uv_frame, const int64 timestamp,
		const float* const alignment_matrix_2x3)
{
	...
	CHECK_ALWAYS(curr_time_ < timestamp,
			"Timestamp must monotonically increase! Went from %lld to %lld"
					" on frame %d.", curr_time_, timestamp, num_frames_);
...
TrackedObject* ObjectTracker::MaybeAddObject(const std::string& id,
		const Image<uint8>& source_image, const BoundingBox& bounding_box,
		const ObjectModelBase* object_model)
{
	...
	TrackedObject* const object = new TrackedObject(id, source_image,
			bounding_box, model);
...
void ObjectTracker::RegisterNewObjectWithAppearance(const std::string& id,
		const uint8* const new_frame, const BoundingBox& bounding_box)
{
	...
	if (objects_.find(id) == objects_.end()) {
		TrackedObject* const object = MaybeAddObject(id, image, bounding_box,
				object_model);
...
void ObjectTracker::SetPreviousPositionOfObject(const std::string& id,
		const BoundingBox& bounding_box, const int64 timestamp)
{
	CHECK_ALWAYS(timestamp > 0, "Timestamp too low! %lld", timestamp);
	CHECK_ALWAYS(timestamp <= curr_time_, "Timestamp too great! %lld vs %lld",
			timestamp, curr_time_);
...
void ObjectTracker::SetCurrentPositionOfObject(const std::string& id,
		...
		const FramePair& frame_pair) const
{
	...
	frame_pair.AdjustBox(tracked_box, &translation_x, &translation_y, &scale_x,
			&scale_y);
		...
		const int64 timestamp) const
{
	...
	for (int i = 0; i < curr_num_frame_pairs_; ++i) {
		...
		if (frame_pair.end_time_ <= timestamp) {
			...
			if (num_frames_back > 0) {
				LOGV(
						"Went %d out of %d frames before finding frame. (index: %d)",
						num_frames_back, curr_num_frame_pairs_,
						GetNthIndexFromEnd(i));
			}
	...
	for (int i = num_frames_back; i >= 0; --i) {
		const FramePair& frame_pair = frame_pairs_[GetNthIndexFromEnd(i)];
		SCHECK(frame_pair.end_time_ >= timestamp,
				"Frame timestamp was too early!");
...
inline void Convert3x3To4x4(const float* const in_matrix,
		float* const out_matrix)
{
	// X
...
bool ObjectTracker::GetBestObjectForDetection(const Detection& detection,
		TrackedObject** match) const
{
	...
	for (TrackedObjectMap::const_iterator object_iter = objects_.begin();
		...
		const float jump_distance = (tracked_object->GetPosition().GetCenter()
		...
		if (jump_distance < allowed_distance && overlap > best_overlap
...
void ObjectTracker::ProcessDetections(std::vector<Detection>* const detections)
{
	...
	for (std::vector<Detection>::const_iterator it = detections->begin();
		...
		const bool no_collisions = GetBestObjectForDetection(detection,
				&best_match);
		...
		if (best_match != NULL) {
			...
			best_match->OnDetection(model, detection.GetObjectBoundingBox(),
					detection.GetMatchScore(), curr_time_, *frame2_);
		...
		const float square_size = MAX(
				kScanMinSquareSize
						/ (kLastKnownPositionScaleFactor
								* kLastKnownPositionScaleFactor),
				MIN(position.GetWidth(), position.GetHeight()))
				/ kLastKnownPositionScaleFactor;

		FillWithSquares(frame2_->GetImage()->GetContainingBox(),
				tracked_object->GetPosition(), square_size, kScanMinSquareSize,
				kLastKnownPositionScaleFactor, &positions);
	}
#endif
...
void ObjectTracker::TrackObjects()
{
	...
	const bool automatic_removal_allowed =
			detector_.get() != NULL ?
	...
	for (TrackedObjectMap::iterator iter = objects_.begin();
			iter != objects_.end(); iter++) {
		TrackedObject* object = iter->second;
		const BoundingBox tracked_position = TrackBox(object->GetPosition(),
				frame_pairs_[GetNthIndexFromEnd(0)]);
