...
public:
	explicit LRNOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		auto sqr_sum = builder->ReduceWindow(squared,
		...
		auto scale = builder->Pow(
				builder->Add(builder->ConstantR0<float>(bias_),
						builder->Mul(builder->ConstantR0<float>(alpha_),
								sqr_sum)), builder->ConstantR0<float>(-beta_));
...
public:
	explicit LRNGradOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("depth_radius", &depth_radius_));
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				in_grads_shape.dims() == 4 && in_image_shape.dims() == 4,
		...
		auto norm = builder->Add(builder->ConstantR0<float>(bias_),
				builder->Mul(builder->ConstantR0<float>(alpha_), sqr_sum));

		auto dy = builder->Mul(
				builder->Mul(builder->ConstantR0<float>(-2.0f * alpha_ * beta_),
						builder->Div(out_image, norm)), in_grads);

		auto dy_reduced = builder->ReduceWindow(dy,
				XlaHelpers::Zero(builder, input_type(0)),
				*ctx->GetOrCreateAdd(input_type(0)),
				/* window_dimensions = */{ 1, 1, 1, depth_radius_ * 2 + 1 },
				/* window_strides = */{ 1, 1, 1, 1 }, xla::Padding::kSame);
