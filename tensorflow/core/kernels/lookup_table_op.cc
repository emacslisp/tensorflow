...
protected:
	...
	Status DoInsert(const Tensor& keys, const Tensor& values) override
	{
		...
		for (int64 i = 0; i < key_values.size(); ++i) {
			...
			const V& previous_value = gtl::LookupOrInsert(table_.get(), key,
					value);
			if (previous_value != value) {
				return errors::FailedPrecondition(
						"HashTable has different value for same key. Key ", key,
						" has ", previous_value, " and trying to add value ",
						value);
			}
		}
		return Status::OK();
	...
	Status DoFind(const Tensor& key, Tensor* value, const Tensor& default_value) override
	{
		...
		for (int64 i = 0; i < key_values.size(); ++i) {
			value_values(i) = gtl::FindWithDefault(*table_,
...
public:
	...
	Status ExportValues(OpKernelContext* ctx) override
	{
		...
		TF_RETURN_IF_ERROR(
				ctx->allocate_output("values", TensorShape( { size }),
						&values));
...
private:
	...
	mutable mutex mu_;std::unordered_map<K, V> table_ GUARDED_BY(mu_);
};
...
public:
	MutableHashTableOfTensors(OpKernelContext* ctx, OpKernel* kernel)
	{
		...
		OP_REQUIRES(ctx, TensorShapeUtils::IsVector(value_shape_),
				errors::InvalidArgument(
						"Default value must be a vector, got shape ",
			...
			const Tensor& default_value) override
	{
		...
		mutex_lock l(mu_);
		for (int64 i = 0; i < key_values.size(); ++i) {
			ValueArray* value_vec = gtl::FindOrNull(table_,
					SubtleMustCopyUnlessStringOrFloat(key_values(i)));
	...
	Status ExportValues(OpKernelContext* ctx) override
	{
		...
		TF_RETURN_IF_ERROR(
				ctx->allocate_output("keys", TensorShape( { size }), &keys));
		TF_RETURN_IF_ERROR(ctx->allocate_output("values", TensorShape( { size,
				value_dim }), &values));
...
private:
	...
	// TODO(andreasst): consider using a read/write lock or a concurrent map
	mutable mutex mu_;
	typedef gtl::InlinedVector<V, 4> ValueArray;std::unordered_map<K, ValueArray> table_ GUARDED_BY(mu_);
};
...
inline uint64 HashScalar(const string& key)
{
	return Hash64(key);
}
...
public:
	...
	{
		OP_REQUIRES_OK(ctx,
				GetNodeAttr(kernel->def(), "max_load_factor",
						&max_load_factor_));
		...
		empty_key_hash_ = HashKey(empty_key_input->template shaped<K, 2>( { 1,
				key_shape_.num_elements() }), 0);

		int64 initial_num_buckets;
		OP_REQUIRES_OK(ctx,
				GetNodeAttr(kernel->def(), "initial_num_buckets",
						&initial_num_buckets));
		OP_REQUIRES_OK(ctx, AllocateBuckets(ctx, initial_num_buckets));
	}

	size_t size() const override LOCKS_EXCLUDED(mu_) {
		mutex_lock l(mu_);return num_entries_;
	}
	...
	override LOCKS_EXCLUDED(mu_)
	{
		...
		const auto empty_key_matrix =
				empty_key_.AccessTensor(ctx)->template shaped<K, 2>( { 1,
						key_size });
		...
		for (int64 i = 0; i < num_elements; ++i) {
			...
			int64 num_probes = 0;
			while (true) {
				if (IsEqualKey(key_buckets_matrix, bucket_index, key_matrix,
						i)) {
				...
				if (IsEqualKey(key_buckets_matrix, bucket_index,
						empty_key_matrix, 0)) {
					for (int64 j = 0; j < value_size; ++j) {
						value_matrix(i, j) = SubtleMustCopyUnlessStringOrFloat(
								default_flat(j));
	...
	Status Insert(OpKernelContext* ctx, const Tensor& key, const Tensor& value)
	override LOCKS_EXCLUDED(mu_)
	{
	...
	override {return DataTypeToEnum<K>::v();}

	DataType value_dtype() const
	override {return DataTypeToEnum<V>::v();}

	TensorShape key_shape() const
	override {return key_shape_;}

	TensorShape value_shape() const
	override {return value_shape_;}
...
private:
	...
	EXCLUSIVE_LOCKS_REQUIRED(mu_) {
		...
		const auto empty_key_flat = empty_key_.AccessTensor(ctx)->template flat<
				K>();
		for (int64 i = 0; i < num_buckets_; ++i) {
			for (int64 j = 0; j < key_size; ++j) {
	...
	EXCLUSIVE_LOCKS_REQUIRED(mu_) {
		...
		TF_RETURN_IF_ERROR (AllocateBuckets(ctx, num_new_buckets));return
DoInsert(ctx, old_key_buckets, old_value_buckets, true);
}
...
public:
explicit LookupTableFindOp(OpKernelConstruction* ctx) :
OpKernel(ctx)
{
}
...
public:
explicit LookupTableInsertOp(OpKernelConstruction* ctx) :
OpKernel(ctx)
{
}
...
public:
explicit LookupTableSizeOp(OpKernelConstruction* ctx) :
OpKernel(ctx)
{
}
...
public:
explicit LookupTableExportOp(OpKernelConstruction* ctx) :
OpKernel(ctx)
{
}
...
public:
explicit LookupTableImportOp(OpKernelConstruction* ctx) :
OpKernel(ctx)
{
}
