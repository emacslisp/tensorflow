...
public:
	explicit Resource(const string& label) :
			label_(label)
	{
	}
	~Resource() override
...
public:
	explicit Other(const string& label) :
			label_(label)
	{
	}
	~Other() override
...
template<typename T>
string Find(const ResourceMgr& rm, const string& container, const string& name)
{
	T* r;
		...
		const string& name, const string& label)
{
	T* r;
	TF_CHECK_OK(
			rm->LookupOrCreate < T > (container, name, &r, [&label](T** ret) {
				*ret = new T(label);
				return Status::OK();
			}));
	const string ret = r->DebugString();
...
Status ComputePolicy(const string& attr_container,
		const string& attr_shared_name, bool use_node_name_as_default,
		string* result)
{
	ContainerInfo cinfo;
	ResourceMgr rmgr;
		...
		bool use_node_name_as_default)
{
	string ret;
	TF_CHECK_OK(
			ComputePolicy(attr_container, attr_shared_name,
