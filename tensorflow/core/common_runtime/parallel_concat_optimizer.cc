...
public:
	Status Run(const GraphOptimizationPassOptions& options) override
	{
		...
		for (Node* n : matches) {
			...
			auto make_node =
					[n, g, &n_attrs, &base_make_node](string op) {
			...
			Node* start;
			TF_RETURN_IF_ERROR(
					make_node("_ParallelConcatStart").Attr("shape", shape).Attr(
							"dtype", dtype).Finalize(g, &start));
			...
			for (const Edge* input_edge : n->in_edges()) {
				...
				Node* update;
				TF_RETURN_IF_ERROR(
						make_node("_ParallelConcatUpdate").Attr("loc", i).Input(
								start).Input(input_edge->src(),
								input_edge->src_output()).Finalize(g, &update));
