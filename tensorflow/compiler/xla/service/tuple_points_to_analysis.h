...
public:
	explicit PointsToSet(const Shape& shape) :
			ShapeTree<std::vector<const LogicalBuffer*>>(shape), tuple_sources_(
					shape)
	{
	}
	...
	void AddPointedToBuffer(const LogicalBuffer& buffer,
	...
	const std::set<HloInstruction*>& tuple_sources(
			const ShapeIndex& index) const;
...
private:
	...
	tensorflow::gtl::FlatMap<const HloInstruction*, std::unique_ptr<PointsToSet>> points_to_;

	// A map containing the LogicalBuffers defined by each HLO instruction.
	std::unordered_map<const HloInstruction*, std::vector<const LogicalBuffer*>> instruction_defined_buffers_;

	std::unordered_map<const LogicalBuffer*, std::vector<BufferAlias>> buffer_aliases_;
