...
{
	cublasStatus_t ret = dynload::cublasGetPointerMode_v2(parent_, handle_,
			&old_mode_);
	if (ret != CUBLAS_STATUS_SUCCESS) {
		LOG(ERROR) << "failed to get old cublas pointer mode: "
				<< ToString(ret);
		return ok_ = false;
	}
	...
	if (ret != CUBLAS_STATUS_SUCCESS) {
		LOG(ERROR) << "failed to set new cublas pointer mode: "
				<< ToString(ret);
		return ok_ = false;
	}
...
{
	if (ok_) {
		cublasStatus_t ret = dynload::cublasSetPointerMode_v2(parent_, handle_,
				old_mode_);
		if (ret != CUBLAS_STATUS_SUCCESS) {
...
CHECK(blas_ != nullptr);
cublasStatus_t ret = dynload::cublasSetStream_v2(parent_, blas_,
...
bool CUDABlas::DoBlasAsum(Stream *stream, uint64 elem_count,
	const DeviceMemory<float> &x, int incx, DeviceMemory<float> *result)
{
return DoBlasInternal(dynload::cublasSasum, stream,
		false /* = pointer_mode_host */, elem_count, CUDAMemory(x), incx,
		CUDAMemoryMutable(result));
}

bool CUDABlas::DoBlasAsum(Stream *stream, uint64 elem_count,
	const DeviceMemory<double> &x, int incx, DeviceMemory<double> *result)
{
...
{
return DoBlasInternal(dynload::cublasScasum, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
...
{
return DoBlasInternal(dynload::cublasDzasum, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}

bool CUDABlas::DoBlasAxpy(Stream *stream, uint64 elem_count, float alpha,
	const DeviceMemory<float> &x, int incx, DeviceMemory<float> *y, int incy)
{
return DoBlasInternal(dynload::cublasSaxpy, stream,
		true /* = pointer_mode_host */, elem_count, &alpha, CUDAMemory(x), incx,
		CUDAMemoryMutable(y), incy);
}

bool CUDABlas::DoBlasAxpy(Stream *stream, uint64 elem_count, double alpha,
	const DeviceMemory<double> &x, int incx, DeviceMemory<double> *y, int incy)
{
...
bool CUDABlas::DoBlasAxpy(Stream *stream, uint64 elem_count,
	std::complex<float> alpha, const DeviceMemory<std::complex<float>> &x,
	int incx, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasCaxpy, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemoryMutable(y)),
		incy);
}

bool CUDABlas::DoBlasAxpy(Stream *stream, uint64 elem_count,
	std::complex<double> alpha, const DeviceMemory<std::complex<double>> &x,
...
return DoBlasInternal(dynload::cublasScopy, stream,
		true /* = pointer_mode_host */, elem_count, CUDAMemory(x), incx,
		CUDAMemoryMutable(y), incy);
}
...
return DoBlasInternal(dynload::cublasCcopy, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
...
bool CUDABlas::DoBlasDot(Stream *stream, uint64 elem_count,
	const DeviceMemory<float> &x, int incx, const DeviceMemory<float> &y,
	int incy, DeviceMemory<float> *result)
{
return DoBlasInternal(dynload::cublasSdot, stream,
		false /* = pointer_mode_host */, elem_count, CUDAMemory(x), incx,
		CUDAMemory(y), incy, CUDAMemoryMutable(result));
}

bool CUDABlas::DoBlasDot(Stream *stream, uint64 elem_count,
	const DeviceMemory<double> &x, int incx, const DeviceMemory<double> &y,
	int incy, DeviceMemory<double> *result)
{
return DoBlasInternal(dynload::cublasDdot, stream,
		false /* = pointer_mode_host */, elem_count, CUDAMemory(x), incx,
		CUDAMemory(y), incy, CUDAMemoryMutable(result));
}
...
{
return DoBlasInternal(dynload::cublasCdotc, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
...
{
return DoBlasInternal(dynload::cublasZdotc, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(result)));
}
...
bool CUDABlas::DoBlasDotu(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<std::complex<float>> *result)
{
return DoBlasInternal(dynload::cublasCdotu, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(result)));
}
...
bool CUDABlas::DoBlasDotu(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<std::complex<double>> *result)
{
return DoBlasInternal(dynload::cublasZdotu, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(result)));
}
...
bool CUDABlas::DoBlasNrm2(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<float> *result)
{
return DoBlasInternal(dynload::cublasScnrm2, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasNrm2(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<double> *result)
{
return DoBlasInternal(dynload::cublasDznrm2, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}

bool CUDABlas::DoBlasRot(Stream *stream, uint64 elem_count,
	DeviceMemory<float> *x, int incx, DeviceMemory<float> *y, int incy, float c,
	float s)
{
return DoBlasInternal(dynload::cublasSrot, stream,
		true /* = pointer_mode_host */, elem_count, CUDAMemoryMutable(x), incx,
		CUDAMemoryMutable(y), incy, &c, &s);
}

bool CUDABlas::DoBlasRot(Stream *stream, uint64 elem_count,
	DeviceMemory<double> *x, int incx, DeviceMemory<double> *y, int incy,
	double c, double s)
{
return DoBlasInternal(dynload::cublasDrot, stream,
		true /* = pointer_mode_host */, elem_count, CUDAMemoryMutable(x), incx,
		CUDAMemoryMutable(y), incy, &c, &s);
}
...
bool CUDABlas::DoBlasRot(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<std::complex<float>> *y, int incy, float c, float s)
{
...
bool CUDABlas::DoBlasRot(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<std::complex<double>> *y, int incy, double c, double s)
{
...
bool CUDABlas::DoBlasRotg(Stream *stream, DeviceMemory<float> *a,
	DeviceMemory<float> *b, DeviceMemory<float> *c, DeviceMemory<float> *s)
{
...
bool CUDABlas::DoBlasRotg(Stream *stream, DeviceMemory<double> *a,
	DeviceMemory<double> *b, DeviceMemory<double> *c, DeviceMemory<double> *s)
{
return DoBlasInternal(dynload::cublasDrotg, stream,
		false /* = pointer_mode_host */, CUDAComplex(CUDAMemoryMutable(a)),
		CUDAMemoryMutable(b), CUDAMemoryMutable(c), CUDAMemoryMutable(s));
}

bool CUDABlas::DoBlasRotg(Stream *stream, DeviceMemory<std::complex<float>> *a,
	DeviceMemory<std::complex<float>> *b, DeviceMemory<float> *c,
	DeviceMemory<std::complex<float>> *s)
{
return DoBlasInternal(dynload::cublasCrotg, stream,
		false /* = pointer_mode_host */, CUDAComplex(CUDAMemoryMutable(a)),
...
bool CUDABlas::DoBlasRotg(Stream *stream, DeviceMemory<std::complex<double>> *a,
	DeviceMemory<std::complex<double>> *b, DeviceMemory<double> *c,
	DeviceMemory<std::complex<double>> *s)
{
return DoBlasInternal(dynload::cublasZrotg, stream,
		false /* = pointer_mode_host */, CUDAComplex(CUDAMemoryMutable(a)),
		CUDAComplex(CUDAMemoryMutable(b)), CUDAComplex(CUDAMemoryMutable(c)),
		CUDAComplex(CUDAMemoryMutable(s)));
}
...
bool CUDABlas::DoBlasRotm(Stream *stream, uint64 elem_count,
	DeviceMemory<float> *x, int incx, DeviceMemory<float> *y, int incy,
	const DeviceMemory<float> &param)
{
return DoBlasInternal(dynload::cublasSrotm, stream,
		false /* = pointer_mode_host */, elem_count, CUDAMemoryMutable(x), incx,
...
return DoBlasInternal(dynload::cublasSrotmg, stream,
		...
		CUDAMemoryMutable(d2), CUDAMemoryMutable(x1), CUDAMemory(y1),
		CUDAMemoryMutable(param));
}

bool CUDABlas::DoBlasRotmg(Stream *stream, DeviceMemory<double> *d1,
	DeviceMemory<double> *d2, DeviceMemory<double> *x1,
	const DeviceMemory<double> &y1, DeviceMemory<double> *param)
{
...
bool CUDABlas::DoBlasScal(Stream *stream, uint64 elem_count, float alpha,
	DeviceMemory<std::complex<float>> *x, int incx)
{
return DoBlasInternal(dynload::cublasCsscal, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemoryMutable(x)), incx);
}
...
bool CUDABlas::DoBlasScal(Stream *stream, uint64 elem_count, double alpha,
	DeviceMemory<std::complex<double>> *x, int incx)
{
return DoBlasInternal(dynload::cublasZdscal, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemoryMutable(x)), incx);
}

bool CUDABlas::DoBlasScal(Stream *stream, uint64 elem_count,
	std::complex<float> alpha, DeviceMemory<std::complex<float>> *x, int incx)
{
return DoBlasInternal(dynload::cublasCscal, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemoryMutable(x)), incx);
}

bool CUDABlas::DoBlasScal(Stream *stream, uint64 elem_count,
	std::complex<double> alpha, DeviceMemory<std::complex<double>> *x, int incx)
{
return DoBlasInternal(dynload::cublasZscal, stream,
		true /* = pointer_mode_host */, elem_count, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemoryMutable(x)), incx);
}
...
bool CUDABlas::DoBlasIamax(Stream *stream, uint64 elem_count,
	const DeviceMemory<float> &x, int incx, DeviceMemory<int> *result)
{
...
bool CUDABlas::DoBlasIamax(Stream *stream, uint64 elem_count,
	const DeviceMemory<double> &x, int incx, DeviceMemory<int> *result)
{
...
{
return DoBlasInternal(dynload::cublasIcamax, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasIamax(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<int> *result)
{
return DoBlasInternal(dynload::cublasIzamax, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasIamin(Stream *stream, uint64 elem_count,
	const DeviceMemory<float> &x, int incx, DeviceMemory<int> *result)
{
return DoBlasInternal(dynload::cublasIsamin, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasIamin(Stream *stream, uint64 elem_count,
	const DeviceMemory<double> &x, int incx, DeviceMemory<int> *result)
{
return DoBlasInternal(dynload::cublasIdamin, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasIamin(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<int> *result)
{
return DoBlasInternal(dynload::cublasIcamin, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}
...
bool CUDABlas::DoBlasIamin(Stream *stream, uint64 elem_count,
	...
	DeviceMemory<int> *result)
{
return DoBlasInternal(dynload::cublasIzamin, stream,
		false /* = pointer_mode_host */, elem_count, CUDAComplex(CUDAMemory(x)),
		incx, CUDAMemoryMutable(result));
}

bool CUDABlas::DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, uint64 kl, uint64 ku, float alpha, const DeviceMemory<float> &a,
	int lda, const DeviceMemory<float> &x, int incx, float beta,
	DeviceMemory<float> *y, int incy)
{
return DoBlasInternal(dynload::cublasSgbmv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, kl, ku,
		&alpha, CUDAMemory(a), lda, CUDAMemory(x), incx, &beta,
		CUDAMemoryMutable(y), incy);
}

bool CUDABlas::DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, uint64 kl, uint64 ku, double alpha, const DeviceMemory<double> &a,
	int lda, const DeviceMemory<double> &x, int incx, double beta,
	DeviceMemory<double> *y, int incy)
{
return DoBlasInternal(dynload::cublasDgbmv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, kl, ku,
		&alpha, CUDAMemory(a), lda, CUDAMemory(x), incx, &beta,
		CUDAMemoryMutable(y), incy);
}
...
bool CUDABlas::DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, uint64 kl, uint64 ku, std::complex<float> alpha,
	const DeviceMemory<std::complex<float>> &a, int lda,
	const DeviceMemory<std::complex<float>> &x, int incx,
	std::complex<float> beta, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasCgbmv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, kl, ku,
...
bool CUDABlas::DoBlasGbmv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, uint64 kl, uint64 ku, std::complex<double> alpha,
	const DeviceMemory<std::complex<double>> &a, int lda,
	const DeviceMemory<std::complex<double>> &x, int incx,
	std::complex<double> beta, DeviceMemory<std::complex<double>> *y, int incy)
{
return DoBlasInternal(dynload::cublasZgbmv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, kl, ku,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
...
bool CUDABlas::DoBlasGemv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, float alpha, const DeviceMemory<float> &a, int lda,
	const DeviceMemory<float> &x, int incx, float beta, DeviceMemory<float> *y,
	int incy)
{
return DoBlasInternal(dynload::cublasSgemv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, &alpha,
		CUDAMemory(a), lda, CUDAMemory(x), incx, &beta, CUDAMemoryMutable(y),
		incy);
}

bool CUDABlas::DoBlasGemv(Stream *stream, blas::Transpose trans, uint64 m,
	uint64 n, double alpha, const DeviceMemory<double> &a, int lda,
	const DeviceMemory<double> &x, int incx, double beta,
	DeviceMemory<double> *y, int incy)
{
return DoBlasInternal(dynload::cublasDgemv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n, &alpha,
		CUDAMemory(a), lda, CUDAMemory(x), incx, &beta, CUDAMemoryMutable(y),
		incy);
}
...
bool CUDABlas::DoBlasGemv(Stream *stream, blas::Transpose trans, uint64 m,
	...
	std::complex<float> beta, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasCgemv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
}
...
bool CUDABlas::DoBlasGemv(Stream *stream, blas::Transpose trans, uint64 m,
	...
	std::complex<double> beta, DeviceMemory<std::complex<double>> *y, int incy)
{
return DoBlasInternal(dynload::cublasZgemv, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(trans), m, n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
...
bool CUDABlas::DoBlasGer(Stream *stream, uint64 m, uint64 n, float alpha,
	const DeviceMemory<float> &x, int incx, const DeviceMemory<float> &y,
	int incy, DeviceMemory<float> *a, int lda)
{
return DoBlasInternal(dynload::cublasSger, stream,
		true /* = pointer_mode_host */, m, n, &alpha, CUDAMemory(x), incx,
		CUDAMemory(y), incy, CUDAMemoryMutable(a), lda);
}

bool CUDABlas::DoBlasGer(Stream *stream, uint64 m, uint64 n, double alpha,
	const DeviceMemory<double> &x, int incx, const DeviceMemory<double> &y,
	int incy, DeviceMemory<double> *a, int lda)
{
return DoBlasInternal(dynload::cublasDger, stream,
		true /* = pointer_mode_host */, m, n, &alpha, CUDAMemory(x), incx,
		CUDAMemory(y), incy, CUDAMemoryMutable(a), lda);
}
...
bool CUDABlas::DoBlasGerc(Stream *stream, uint64 m, uint64 n,
	std::complex<float> alpha, const DeviceMemory<std::complex<float>> &x,
	int incx, const DeviceMemory<std::complex<float>> &y, int incy,
	DeviceMemory<std::complex<float>> *a, int lda)
{
return DoBlasInternal(dynload::cublasCgerc, stream,
		true /* = pointer_mode_host */, m, n, CUDAComplex(&alpha),
...
bool CUDABlas::DoBlasGerc(Stream *stream, uint64 m, uint64 n,
	std::complex<double> alpha, const DeviceMemory<std::complex<double>> &x,
	int incx, const DeviceMemory<std::complex<double>> &y, int incy,
	DeviceMemory<std::complex<double>> *a, int lda)
{
return DoBlasInternal(dynload::cublasZgerc, stream,
		true /* = pointer_mode_host */, m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(a)), lda);
}
...
bool CUDABlas::DoBlasGeru(Stream *stream, uint64 m, uint64 n,
	...
	DeviceMemory<std::complex<float>> *a, int lda)
{
return DoBlasInternal(dynload::cublasCgeru, stream,
		true /* = pointer_mode_host */, m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(a)), lda);
}
...
bool CUDABlas::DoBlasGeru(Stream *stream, uint64 m, uint64 n,
	...
	DeviceMemory<std::complex<double>> *a, int lda)
{
return DoBlasInternal(dynload::cublasZgeru, stream,
		true /* = pointer_mode_host */, m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemory(y)), incy,
		CUDAComplex(CUDAMemoryMutable(a)), lda);
}
...
bool CUDABlas::DoBlasHbmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	std::complex<float> beta, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasChbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, k,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
}
...
bool CUDABlas::DoBlasHbmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	std::complex<double> beta, DeviceMemory<std::complex<double>> *y, int incy)
{
return DoBlasInternal(dynload::cublasZhbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, k,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
...
bool CUDABlas::DoBlasHemv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	std::complex<float> beta, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasChemv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
}
...
bool CUDABlas::DoBlasHemv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	std::complex<double> beta, DeviceMemory<std::complex<double>> *y, int incy)
{
return DoBlasInternal(dynload::cublasZhemv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(a)), lda,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
...
bool CUDABlas::DoBlasHer(Stream *stream, blas::UpperLower uplo, uint64 n,
	float alpha, const DeviceMemory<std::complex<float>> &x, int incx,
	DeviceMemory<std::complex<float>> *a, int lda)
{
return DoBlasInternal(dynload::cublasCher, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, &alpha,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemoryMutable(a)),
		lda);
}

bool CUDABlas::DoBlasHer(Stream *stream, blas::UpperLower uplo, uint64 n,
	double alpha, const DeviceMemory<std::complex<double>> &x, int incx,
	DeviceMemory<std::complex<double>> *a, int lda)
{
return DoBlasInternal(dynload::cublasZher, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, &alpha,
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(CUDAMemoryMutable(a)),
		lda);
}
...
bool CUDABlas::DoBlasHer2(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	DeviceMemory<std::complex<float>> *a, int lda)
{
return DoBlasInternal(dynload::cublasCher2, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
		CUDAComplex(CUDAMemory(y)), incy, CUDAComplex(CUDAMemoryMutable(a)),
		lda);
}
...
bool CUDABlas::DoBlasHer2(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	DeviceMemory<std::complex<double>> *a, int lda)
{
return DoBlasInternal(dynload::cublasZher2, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
		CUDAComplex(CUDAMemory(y)), incy, CUDAComplex(CUDAMemoryMutable(a)),
		lda);
...
bool CUDABlas::DoBlasHpmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	std::complex<float> alpha, const DeviceMemory<std::complex<float>> &ap,
	const DeviceMemory<std::complex<float>> &x, int incx,
	std::complex<float> beta, DeviceMemory<std::complex<float>> *y, int incy)
{
return DoBlasInternal(dynload::cublasChpmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(ap)),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
}

bool CUDABlas::DoBlasHpmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	std::complex<double> alpha, const DeviceMemory<std::complex<double>> &ap,
	const DeviceMemory<std::complex<double>> &x, int incx,
	std::complex<double> beta, DeviceMemory<std::complex<double>> *y, int incy)
{
return DoBlasInternal(dynload::cublasZhpmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(ap)),
		CUDAComplex(CUDAMemory(x)), incx, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(y)), incy);
...
{
return DoBlasInternal(dynload::cublasChpr, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
...
{
return DoBlasInternal(dynload::cublasZhpr, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
		CUDAComplex(CUDAMemoryMutable(ap)));
}
...
bool CUDABlas::DoBlasHpr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	DeviceMemory<std::complex<float>> *ap)
{
return DoBlasInternal(dynload::cublasChpr2, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
...
bool CUDABlas::DoBlasHpr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	DeviceMemory<std::complex<double>> *ap)
{
return DoBlasInternal(dynload::cublasZhpr2, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n,
		CUDAComplex(&alpha), CUDAComplex(CUDAMemory(x)), incx,
		CUDAComplex(CUDAMemory(y)), incy, CUDAComplex(CUDAMemoryMutable(ap)));
}
...
bool CUDABlas::DoBlasSbmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	int incy)
{
return DoBlasInternal(dynload::cublasSsbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, k, &alpha,
		CUDAMemory(a), lda, CUDAMemory(x), incx, &beta, CUDAMemoryMutable(y),
		incy);
}
...
bool CUDABlas::DoBlasSbmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	...
	DeviceMemory<double> *y, int incy)
{
return DoBlasInternal(dynload::cublasDsbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, k, &alpha,
		CUDAMemory(a), lda, CUDAMemory(x), incx, &beta, CUDAMemoryMutable(y),
		incy);
}
...
bool CUDABlas::DoBlasSpmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	float alpha, const DeviceMemory<float> &ap, const DeviceMemory<float> &x,
...
bool CUDABlas::DoBlasSpmv(Stream *stream, blas::UpperLower uplo, uint64 n,
	double alpha, const DeviceMemory<double> &ap, const DeviceMemory<double> &x,
...
return DoBlasInternal(dynload::cublasSspr, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo), n, &alpha,
		CUDAMemory(x), incx, CUDAMemoryMutable(ap));
}
...
bool CUDABlas::DoBlasSpr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	float alpha, const DeviceMemory<float> &x, int incx,
	const DeviceMemory<float> &y, int incy, DeviceMemory<float> *ap)
{
...
bool CUDABlas::DoBlasSpr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	double alpha, const DeviceMemory<double> &x, int incx,
	const DeviceMemory<double> &y, int incy, DeviceMemory<double> *ap)
{
...
bool CUDABlas::DoBlasSyr(Stream *stream, blas::UpperLower uplo, uint64 n,
	float alpha, const DeviceMemory<float> &x, int incx, DeviceMemory<float> *a,
	int lda)
{
...
bool CUDABlas::DoBlasSyr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	float alpha, const DeviceMemory<float> &x, int incx,
	const DeviceMemory<float> &y, int incy, DeviceMemory<float> *a, int lda)
{
...
bool CUDABlas::DoBlasSyr2(Stream *stream, blas::UpperLower uplo, uint64 n,
	double alpha, const DeviceMemory<double> &x, int incx,
	const DeviceMemory<double> &y, int incy, DeviceMemory<double> *a, int lda)
{
...
bool CUDABlas::DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, blas::Diagonal diag, uint64 n, uint64 k,
	const DeviceMemory<float> &a, int lda, DeviceMemory<float> *x, int incx)
{
return DoBlasInternal(dynload::cublasStbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, k, CUDAMemory(a),
		lda, CUDAMemoryMutable(x), incx);
}

bool CUDABlas::DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, blas::Diagonal diag, uint64 n, uint64 k,
	const DeviceMemory<double> &a, int lda, DeviceMemory<double> *x, int incx)
{
...
bool CUDABlas::DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
	...
	DeviceMemory<std::complex<float>> *x, int incx)
{
return DoBlasInternal(dynload::cublasCtbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, k,
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(x)),
		incx);
}
...
bool CUDABlas::DoBlasTbmv(Stream *stream, blas::UpperLower uplo,
	...
	DeviceMemory<std::complex<double>> *x, int incx)
{
return DoBlasInternal(dynload::cublasZtbmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, k,
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(x)),
		incx);
...
bool CUDABlas::DoBlasTbsv(Stream *stream, blas::UpperLower uplo,
	...
	DeviceMemory<std::complex<float>> *x, int incx)
{
return DoBlasInternal(dynload::cublasCtbsv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, k,
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(x)),
		incx);
...
bool CUDABlas::DoBlasTbsv(Stream *stream, blas::UpperLower uplo,
	...
	DeviceMemory<std::complex<double>> *x, int incx)
{
return DoBlasInternal(dynload::cublasZtbsv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, k,
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(x)),
		incx);
...
bool CUDABlas::DoBlasTpmv(Stream *stream, blas::UpperLower uplo,
	...
	const DeviceMemory<float> &ap, DeviceMemory<float> *x, int incx)
{
return DoBlasInternal(dynload::cublasStpmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, CUDAMemory(ap),
		CUDAMemoryMutable(x), incx);
}

bool CUDABlas::DoBlasTpmv(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, blas::Diagonal diag, uint64 n,
	const DeviceMemory<double> &ap, DeviceMemory<double> *x, int incx)
{
return DoBlasInternal(dynload::cublasDtpmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, CUDAMemory(ap),
		CUDAMemoryMutable(x), incx);
}
...
bool CUDABlas::DoBlasTpsv(Stream *stream, blas::UpperLower uplo,
	...
	const DeviceMemory<float> &ap, DeviceMemory<float> *x, int incx)
{
return DoBlasInternal(dynload::cublasStpsv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, CUDAMemory(ap),
		CUDAMemoryMutable(x), incx);
}
...
bool CUDABlas::DoBlasTpsv(Stream *stream, blas::UpperLower uplo,
	...
	const DeviceMemory<double> &ap, DeviceMemory<double> *x, int incx)
{
return DoBlasInternal(dynload::cublasDtpsv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, CUDAMemory(ap),
		CUDAMemoryMutable(x), incx);
}
...
return DoBlasInternal(dynload::cublasStrmv, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), CUDABlasDiagonal(diag), n, CUDAMemory(a), lda,
		CUDAMemoryMutable(x), incx);
}
...
{
VLOG(1)
		<< port::Printf("doing cuBLAS SGEMM: at=%d bt=%d m=%llu n=%llu "
				"k=%llu alpha=%f a=%p lda=%d b=%p ldb=%d beta=%f "
				"c=%p ldc=%d", static_cast<int>(transa),
				static_cast<int>(transb), m, n, k, alpha, a.opaque(), lda,
...
bool CUDABlas::DoBlasGemm(Stream *stream, blas::Transpose transa,
	...
	std::complex<float> alpha, const DeviceMemory<std::complex<float>> &a,
	int lda, const DeviceMemory<std::complex<float>> &b, int ldb,
	std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasCgemm, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(transa),
...
bool CUDABlas::DoBlasGemm(Stream *stream, blas::Transpose transa,
	blas::Transpose transb, uint64 m, uint64 n, uint64 k,
	std::complex<double> alpha, const DeviceMemory<std::complex<double>> &a,
	int lda, const DeviceMemory<std::complex<double>> &b, int ldb,
	std::complex<double> beta, DeviceMemory<std::complex<double>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasZgemm, stream,
		true /* = pointer_mode_host */, CUDABlasTranspose(transa),
		CUDABlasTranspose(transb), m, n, k, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemory(b)), ldb,
		CUDAComplex(&beta), CUDAComplex(CUDAMemoryMutable(c)), ldc);
...
port::Status CUDABlas::DoBlasGemmBatchedInternal(FuncT cublas_func,
	Stream *stream, blas::Transpose transa, blas::Transpose transb, uint64 m,
	uint64 n, uint64 k, T alpha,
...
bool ok = DoBlasInternal(cublas_func, stream, true /* = pointer_mode_host */,
...
{
SE_RETURN_STATUS_AS_BOOL(
		DoBlasGemmBatchedInternal(dynload::cublasSgemmBatched, stream, transa,
				transb, m, n, k, alpha, a_array, lda, b_array, ldb, beta,
...
{
SE_RETURN_STATUS_AS_BOOL(
		DoBlasGemmBatchedInternal(dynload::cublasDgemmBatched, stream, transa,
				transb, m, n, k, alpha, a_array, lda, b_array, ldb, beta,
				c_array, ldc, batch_count, scratch_allocator));
}
...
{
SE_RETURN_STATUS_AS_BOOL(
		DoBlasGemmBatchedInternal(dynload::cublasCgemmBatched, stream, transa,
				transb, m, n, k, alpha, a_array, lda, b_array, ldb, beta,
				c_array, ldc, batch_count, scratch_allocator));
}
...
{
SE_RETURN_STATUS_AS_BOOL(
		DoBlasGemmBatchedInternal(dynload::cublasZgemmBatched, stream, transa,
				transb, m, n, k, alpha, a_array, lda, b_array, ldb, beta,
				c_array, ldc, batch_count, scratch_allocator));
}

bool CUDABlas::DoBlasHemm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, uint64 m, uint64 n, std::complex<float> alpha,
	const DeviceMemory<std::complex<float>> &a, int lda,
	const DeviceMemory<std::complex<float>> &b, int ldb,
	std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasChemm, stream,
...
bool CUDABlas::DoBlasHemm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, uint64 m, uint64 n, std::complex<double> alpha,
	const DeviceMemory<std::complex<double>> &a, int lda,
	const DeviceMemory<std::complex<double>> &b, int ldb,
	std::complex<double> beta, DeviceMemory<std::complex<double>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasZhemm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemory(b)), ldb,
		CUDAComplex(&beta), CUDAComplex(CUDAMemoryMutable(c)), ldc);
...
bool CUDABlas::DoBlasHerk(Stream *stream, blas::UpperLower uplo,
	...
	const DeviceMemory<std::complex<float>> &a, int lda, float beta,
...
bool CUDABlas::DoBlasHerk(Stream *stream, blas::UpperLower uplo,
	...
	const DeviceMemory<std::complex<double>> &a, int lda, double beta,
...
bool CUDABlas::DoBlasHer2k(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, uint64 n, uint64 k, std::complex<float> alpha,
	const DeviceMemory<std::complex<float>> &a, int lda,
...
bool CUDABlas::DoBlasHer2k(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, uint64 n, uint64 k, std::complex<double> alpha,
	const DeviceMemory<std::complex<double>> &a, int lda,
...
bool CUDABlas::DoBlasSymm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, uint64 m, uint64 n, float alpha,
	const DeviceMemory<float> &a, int lda, const DeviceMemory<float> &b,
	int ldb, float beta, DeviceMemory<float> *c, int ldc)
{
return DoBlasInternal(dynload::cublasSsymm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), m, n, &alpha, CUDAMemory(a), lda,
		CUDAMemory(b), ldb, &beta, CUDAMemoryMutable(c), ldc);
}

bool CUDABlas::DoBlasSymm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, uint64 m, uint64 n, double alpha,
	const DeviceMemory<double> &a, int lda, const DeviceMemory<double> &b,
	int ldb, double beta, DeviceMemory<double> *c, int ldc)
{
return DoBlasInternal(dynload::cublasDsymm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), m, n, &alpha, CUDAMemory(a), lda,
		CUDAMemory(b), ldb, &beta, CUDAMemoryMutable(c), ldc);
}
...
bool CUDABlas::DoBlasSymm(Stream *stream, blas::Side side,
	...
	std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasCsymm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemory(b)), ldb,
		CUDAComplex(&beta), CUDAComplex(CUDAMemoryMutable(c)), ldc);
...
bool CUDABlas::DoBlasSymm(Stream *stream, blas::Side side,
	...
	std::complex<double> beta, DeviceMemory<std::complex<double>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasZsymm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemory(b)), ldb,
		CUDAComplex(&beta), CUDAComplex(CUDAMemoryMutable(c)), ldc);
...
bool CUDABlas::DoBlasSyrk(Stream *stream, blas::UpperLower uplo,
	blas::Transpose trans, uint64 n, uint64 k, float alpha,
	const DeviceMemory<float> &a, int lda, float beta, DeviceMemory<float> *c,
	int ldc)
{
return DoBlasInternal(dynload::cublasSsyrk, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, &alpha, CUDAMemory(a), lda, &beta,
		CUDAMemoryMutable(c), ldc);
}
...
{
return DoBlasInternal(dynload::cublasDsyrk, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, &alpha, CUDAMemory(a), lda, &beta,
		CUDAMemoryMutable(c), ldc);
}
...
bool CUDABlas::DoBlasSyrk(Stream *stream, blas::UpperLower uplo,
	...
	std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasCsyrk, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, CUDAComplex(&alpha),
...
bool CUDABlas::DoBlasSyrk(Stream *stream, blas::UpperLower uplo,
	...
	std::complex<double> beta, DeviceMemory<std::complex<double>> *c, int ldc)
{
return DoBlasInternal(dynload::cublasZsyrk, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(&beta),
		CUDAComplex(CUDAMemoryMutable(c)), ldc);
...
bool CUDABlas::DoBlasSyr2k(Stream *stream, blas::UpperLower uplo,
	...
	int ldb, float beta, DeviceMemory<float> *c, int ldc)
{
return DoBlasInternal(dynload::cublasSsyr2k, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, &alpha, CUDAMemory(a), lda,
		CUDAMemory(b), ldb, &beta, CUDAMemoryMutable(c), ldc);
}
...
bool CUDABlas::DoBlasSyr2k(Stream *stream, blas::UpperLower uplo,
	...
	int ldb, double beta, DeviceMemory<double> *c, int ldc)
{
return DoBlasInternal(dynload::cublasDsyr2k, stream,
		true /* = pointer_mode_host */, CUDABlasUpperLower(uplo),
		CUDABlasTranspose(trans), n, k, &alpha, CUDAMemory(a), lda,
		CUDAMemory(b), ldb, &beta, CUDAMemoryMutable(c), ldc);
}
...
bool CUDABlas::DoBlasTrmm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, blas::Transpose transa, blas::Diagonal diag,
	uint64 m, uint64 n, float alpha, const DeviceMemory<float> &a, int lda,
	DeviceMemory<float> *b, int ldb)
{
return DoBlasInternal(dynload::cublasStrmm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
...
bool CUDABlas::DoBlasTrmm(Stream *stream, blas::Side side,
	blas::UpperLower uplo, blas::Transpose transa, blas::Diagonal diag,
	uint64 m, uint64 n, double alpha, const DeviceMemory<double> &a, int lda,
	DeviceMemory<double> *b, int ldb)
{
return DoBlasInternal(dynload::cublasDtrmm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), CUDABlasTranspose(transa),
		CUDABlasDiagonal(diag), m, n, &alpha, CUDAMemory(a), lda,
		CUDAMemoryMutable(b), ldb, CUDAMemoryMutable(b), ldb);
...
{
return DoBlasInternal(dynload::cublasCtrmm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), CUDABlasTranspose(transa),
...
{
return DoBlasInternal(dynload::cublasZtrmm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), CUDABlasTranspose(transa),
		CUDABlasDiagonal(diag), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(b)), ldb,
...
bool CUDABlas::DoBlasTrsm(Stream *stream, blas::Side side,
	...
	DeviceMemory<std::complex<float>> *b, int ldb)
{
return DoBlasInternal(dynload::cublasCtrsm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), CUDABlasTranspose(transa),
		CUDABlasDiagonal(diag), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(b)), ldb);
...
bool CUDABlas::DoBlasTrsm(Stream *stream, blas::Side side,
	...
	DeviceMemory<std::complex<double>> *b, int ldb)
{
return DoBlasInternal(dynload::cublasZtrsm, stream,
		true /* = pointer_mode_host */, CUDABlasSide(side),
		CUDABlasUpperLower(uplo), CUDABlasTranspose(transa),
		CUDABlasDiagonal(diag), m, n, CUDAComplex(&alpha),
		CUDAComplex(CUDAMemory(a)), lda, CUDAComplex(CUDAMemoryMutable(b)), ldb);
...
{
gpu::port::Status status = gpu::PluginRegistry::Instance()->RegisterFactory<
		gpu::PluginRegistry::BlasFactory>(gpu::cuda::kCudaPlatformId,
		gpu::cuda::kCuBlasPlugin, "cuBLAS",
...
gpu::PluginRegistry::Instance()->SetDefaultFactory(gpu::cuda::kCudaPlatformId,
		gpu::PluginKind::kBlas, gpu::cuda::kCuBlasPlugin);
}
