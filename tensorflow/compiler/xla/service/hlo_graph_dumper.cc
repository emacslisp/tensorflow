...
string WindowToString(const Window& window)
{
	...
	for (const WindowDimension& dimension : window.dimensions()) {
		display_padding |= dimension.padding_low() != 0
		...
		const HloExecutionProfile* hlo_execution_profile)
{
	...
	if (!param_instructions.empty()) {
		...
		param_node_name = StrCat("parameters_",
				InstructionId(param_instructions[0]));
		for (auto& param : param_instructions) {
			...
			param_ports.push_back(
					Printf("<%s> %s", InstructionId(param).c_str(),
							label.c_str()));
		}
	...
	for (auto& instruction : instructions) {
		...
		case HloOpcode::kRng:
			StrAppend(&name, "\\n",
					RandomDistribution_Name(
							instruction->random_distribution()));
			break;
		...
		if (show_layouts && LayoutUtil::HasLayout(instruction->shape())) {
			...
			if (ShapeUtil::IsTuple(instruction->shape())) {
				...
				layout_string = ShapeUtil::HumanStringWithLayout(
						instruction->shape());
			} else {
				layout_string = Join(
		...
		if (hlo_execution_profile != nullptr) {
			auto hlo_cycles_executed = hlo_execution_profile->GetProfileResult(
					*instruction);
		...
		if (instruction->opcode() == HloOpcode::kFusion) {
			string cluster_name = StrCat("cluster_",
			...
			string fusion_edge = StrCat(
					...
					InstructionId(instruction.get()),
					"  [ style = \"dotted\", arrowsize=0.0, ltail=",
					cluster_name, " ];\n");
...
string ComputationToDotGraph(const HloComputation& computation,
		const string& label, bool show_addresses, bool show_layouts,
		const HloExecutionProfile* hlo_execution_profile)
{
	...
	for (auto embedded : computation.MakeEmbeddedComputationsList()) {
		string graph_body = InstructionSequenceGraph(embedded->instructions(),
				show_addresses, show_layouts, &intercomputation_edges,
...
public:
	string RenderGraph(const string& graph) override
	{
		...
		string path = StrCat(flags->xla_hlo_dump_graph_path, "hlo_graph_",
				output_num++, ".dot");
		tensorflow::Status status = tensorflow::WriteStringToFile(
				tensorflow::Env::Default(), path, graph);
		if (!status.ok()) {
