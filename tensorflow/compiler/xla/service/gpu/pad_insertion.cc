...
// kPad and kSlice.
HloInstruction* MaybePaddedAndSlicedInput(const Window& conv_window,
		const ConvolutionDimensionNumbers& conv_dnums, HloInstruction* input)
{
				...
				computation->AddInstruction(
						HloInstruction::CreatePad(
								...
								/*operand_shape=*/input->shape(),
										/*padding_value_shape=*/ShapeUtil::MakeShape(
												element_type, { }),
										padding_config).ConsumeValueOrDie(),
	...
	if (window_util::HasNegativePadding(conv_window)) {
		...
		for (size_t i = 0; i < conv_dnums.spatial_dimensions().size(); ++i) {
			...
			start_indices[dim] += std::max<int64>(0LL,
					-conv_window.dimensions(i).padding_low());
			limit_indices[dim] -= std::max<int64>(0LL,
					-conv_window.dimensions(i).padding_high());
		}
		...
		input =
				computation->AddInstruction(
						HloInstruction::CreateSlice(
								ShapeInference::InferSliceShape(input->shape(),
										start_indices, limit_indices).ConsumeValueOrDie(),
...
HloInstruction* MaybePaddedKernel(const Window& conv_window,
		const ConvolutionDimensionNumbers& conv_dnums, HloInstruction* kernel)
{
	...
	return computation->AddInstruction(
			HloInstruction::CreatePad(
					...
					/*operand_shape=*/kernel->shape(),
							/*padding_value_shape=*/ShapeUtil::MakeShape(
									element_type, { }), padding_config).ConsumeValueOrDie(),
...
bool PadInsertion::CanonicalizeForwardConvolution(HloInstruction* conv)
{
	...
	// kernel operand.
	HloInstruction* new_input = MaybePaddedAndSlicedInput(conv->window(),
			conv->convolution_dimension_numbers(), conv->mutable_operand(0));
	HloInstruction* new_kernel = MaybePaddedKernel(conv->window(),
			conv->convolution_dimension_numbers(), conv->mutable_operand(1));
	...
	TF_CHECK_OK(
			conv->parent()->ReplaceWithNewInstruction(conv,
					HloInstruction::CreateConvolve(conv->shape(), new_input,
							new_kernel, new_conv_window,
		...
		HloInstruction* backward_conv)
{
	...
	PaddingConfig input_padding_config = MakeNoPaddingConfig(
			ShapeUtil::Rank(input->shape()));
	...
	for (size_t i = 0; i < backward_conv->window().dimensions_size(); ++i) {
		...
		int64 padding_high =
				backward_conv->window().dimensions(i).padding_high();
			...
			computation->AddInstruction(
					HloInstruction::CreatePad(
							ShapeInference::InferPadShape(input->shape(),
									padding->shape(), input_padding_config).ConsumeValueOrDie(),
							input, padding, input_padding_config));

	HloInstruction* new_forward_conv = computation->AddInstruction(
			HloInstruction::CreateConvolve(
					ShapeInference::InferConvolveShape(padded_input->shape(),
							output->shape(), new_forward_conv_window,
							forward_conv_dnums).ConsumeValueOrDie(),
					padded_input, output, new_forward_conv_window,
					forward_conv_dnums));
			...
			computation->AddInstruction(
					HloInstruction::CreateTranspose(
							ShapeInference::InferTransposeShape(
									new_forward_conv->shape(),
									transpose->dimensions()).ConsumeValueOrDie(),
		...
		HloInstruction* backward_conv)
{
	...
	HloInstruction* new_reverse_filter = computation->AddInstruction(
			HloInstruction::CreateReverse(filter->shape(), filter,
					reverse_filter->dimensions()));
	HloInstruction* new_forward_conv =
			computation->AddInstruction(
					HloInstruction::CreateConvolve(
							ShapeInference::InferConvolveShape(output->shape(),
									new_reverse_filter->shape(),
									new_forward_conv_window, forward_conv_dnums).ConsumeValueOrDie(),
	...
	CHECK(
			ShapeUtil::Compatible(
					ShapeInference::InferSliceShape(new_backward_conv->shape(),
							start_indices, limit_indices).ConsumeValueOrDie(),
					backward_conv->shape()));
	TF_CHECK_OK(
			computation->ReplaceWithNewInstruction(backward_conv,
					HloInstruction::CreateSlice(backward_conv->shape(),
							new_backward_conv, start_indices, limit_indices)));
	return true;
}
...
{
	bool changed = false;
	for (HloInstruction* instruction : module->entry_computation()->MakeInstructionPostOrder()) {
