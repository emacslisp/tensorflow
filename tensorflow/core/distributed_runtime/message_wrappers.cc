...
Status MutableProtoRunStepRequest::FeedValue(size_t i, Tensor* out_tensor) const
{
...
void InMemoryRunGraphRequest::set_is_last_partial_run(bool is_last_partial_run)
{
...
{
	LOG(FATAL)
			<< "Cannot get a mutable protobuf for an InMemoryRunGraphResponse";
}
...
Status OwnedProtoRunGraphResponse::RecvValue(size_t i, TensorProto* out_tensor)
{
...
void OwnedProtoRunGraphResponse::AddRecv(const string& key, const Tensor& value)
{
...
Status InMemoryRunStepResponse::TensorValue(size_t i, Tensor* out_tensor) const
{
...
{
	LOG(FATAL)
			<< "Cannot get a mutable protobuf for an InMemoryRunStepResponse";
}
		...
		Tensor* out_tensor) const
{
	if (!ParseTensorProtoToTensor(response_.tensor(i).tensor(), out_tensor)) {
		return errors::InvalidArgument("Invalid TensorProto for fetch value ",
				i);
	} else {
		return Status::OK();
	}
