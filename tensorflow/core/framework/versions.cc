		...
		const char* upper_name, const char* lower_name)
{
	...
	if (versions.producer() < min_producer) {
		return errors::InvalidArgument(upper_name, " producer version ",
				versions.producer(), " below min producer ", min_producer,
				" supported by TensorFlow ", TF_VERSION_STRING,
				".  Please regenerate your ", lower_name, ".");
	}
	if (versions.min_consumer() > consumer) {
		return errors::InvalidArgument(upper_name, " min consumer version ",
				versions.min_consumer(), " above current version ", consumer,
	...
	for (const int bad_consumer : versions.bad_consumers()) {
		if (bad_consumer == consumer) {
			return errors::InvalidArgument(upper_name,
					" disallows consumer version ", bad_consumer,
