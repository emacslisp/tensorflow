...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "ScatterNdUpdate").Input(
						FakeInput(variable_ref_type)).Input(
						FakeInput(index_type)).Input(
						FakeInput(RemoveRefType(variable_ref_type))).Finalize(
						node_def()));
		TF_ASSERT_OK(InitOp());
	}
};
...
public:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", op).Input(FakeInput(DT_FLOAT_REF)).Input(
						FakeInput(index_type)).Input(FakeInput(DT_FLOAT)).Finalize(
						node_def()));
		TF_CHECK_OK(InitOp());
	}
};
...
static void BM_ScatterNdHelper(int iters, int embedding_size, const char* op)
{
	...
	bm.AddInputFromArray<float>(TensorShape( { kRows, embedding_size }),
			values);
	...
	testing::ItemsProcessed(
			(static_cast<int64>(kNumUpdates) * embedding_size) * iters);
	testing::StartTiming();
	while (iters-- > 0) {
