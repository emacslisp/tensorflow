...
static int64 GetDeepConvCost(int input_tile_rows, int input_tile_cols,
		int out_tile_rows, int out_tile_cols, int in_depth, int out_depth,
		int out_rows, int out_cols)
{
	// Input transform cost.
	const int64 input_tile_spatial_size = input_tile_rows * input_tile_cols;
	const int64 input_transform_cost = input_tile_spatial_size
			* input_tile_spatial_size * in_depth;
	...
	const int64 output_transform_cost = output_tile_spatial_size
			* input_tile_spatial_size * out_depth;
...
{
	return filter_rows * filter_cols * in_depth * out_depth * out_rows
			* out_cols;
}
...
bool CanUseDeepConv2D(int stride_rows, int stride_cols, int filter_rows,
		int filter_cols, int in_depth, int out_depth, int out_rows,
		int out_cols)
{
	...
	const int64 deep_conv_cost = GetDeepConvCost(t.input_shape().rows,
			t.input_shape().cols, t.output_shape().rows, t.output_shape().cols,
			in_depth, out_depth, out_rows, out_cols);
	const int64 direct_conv_cost = GetDirectConvCost(filter_rows, filter_cols,
			in_depth, out_depth, out_rows, out_cols);

	VLOG(2) << "CanUseDeepConv2D" << " deep_conv_cost: " << deep_conv_cost
...
struct CopyFilterDepth {
	void operator()(const Conv2DArgs& args, const T* filter_in, T* filter_buf)
	{
		...
		for (int64 d = 0; d < vectorized_size; ++d) {
			auto v = Eigen::internal::pgather<T, Packet>(
					filter_in + d * input_stride, args.out_depth);
		...
		for (int64 d = 0; d < scalar_size; ++d) {
			filter_buf[buf_scalar_base + d] = filter_in[in_scalar_base
					+ d * args.out_depth];
		}
	}
};
...
struct ComputeFilterRangeTransform {
	...
	typedef Eigen::Map<
			Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> MatrixMap;
	typedef Eigen::Map<
	...
	void operator()(const Conv2DArgs& args,
			...
			const int64 shard_cols, const T* filter_in, const int64 in_stride,
			const int64 out_stride, const T* transform_matrix, T* out_buffer,
			T* filter_out)
	{
		...
		const int64 base_filter_spatial_size = base_filter_rows
				* base_filter_cols;
		...
		for (int64 od = 0; od < num_filters; ++od) {
			...
			for (int64 s_r = 0; s_r < shard_rows; ++s_r) {
				for (int64 s_c = 0; s_c < shard_cols; ++s_c) {
					const int64 shard_base = shard_stride
							* (s_r * shard_cols + s_c);

					for (int64 i = 0; i < tile_spatial_size; ++i) {
						const int64 in_base = i * in_stride + out_depth_buf_base
								+ shard_base;
						const int64 out_base = i * out_stride + out_depth_base
								+ shard_base;
						// Copy vectorized portion of 'in_depth'.
						for (int64 d = 0; d < vectorized_size; ++d) {
							auto v = ei::ploadu < Packet
									> (out_buffer + in_base + d * kPacketSize);
							ei::pstoreu < T
									> (filter_out + out_base + d * kPacketSize, v);
						}
...
struct TransformFilterRange {
	void operator()(const Conv2DArgs& args,
			const DeepConv2DTransform<T>* transform, const int64 od_start,
			const int64 od_limit, const T* filter_in, const T* transform_matrix,
			T* out_buffer, T* filter_buf, T* filter_out)
	{
		...
		const int64 residual_row = std::max(0LL,
				args.filter_rows - base_filter_rows);
		const int64 shard_rows = 1 + (residual_row + 2 - 1) / 2;

		const int64 residual_col = std::max(0LL,
				args.filter_cols - base_filter_cols);
		...
		const int64 filter_buf_stride = num_filters * shard_rows * shard_cols
				* args.in_depth;
		...
		for (int64 od = 0; od < num_filters; ++od) {
			...
			for (int64 s_r = 0; s_r < shard_rows; ++s_r) {
				...
				for (int64 s_c = 0; s_c < shard_cols; ++s_c) {
					...
					for (int64 b_r = row_offset; b_r < base_filter_rows;
							++b_r) {
						const int64 f_r = f_r_start + b_r;
						if (f_r >= args.filter_rows)
							continue;

						for (int64 b_c = col_offset; b_c < base_filter_cols;
								++b_c) {
							const int64 f_c = f_c_start + b_c;
							if (f_c >= args.filter_cols)
								continue;

							const int64 in_index = args.out_depth
									* (args.in_depth
											* (f_r * args.filter_cols + f_c))
									+ (od_start + od);

							const int64 buf_index = filter_buf_stride
									* (b_r * base_filter_cols + b_c)
		...
		ComputeFilterRangeTransform<T>()(args, transform, od_start, num_filters,
				...
				coord_stride, transform_matrix, out_buffer, filter_out);
	}
};
...
struct TransformFilters {
			...
			const T* filter_in, T* filter_out)
	{
		...
		// Fixed cost.
		const int64 filter_transform_matrix_size = tile_spatial_size
				* base_filter_spatial_size;

		// Per-filter costs.
		const int64 filter_total_size = base_filter_spatial_size * in_depth
				* filter_shards_total;

		const int64 filter_transform_buffer_size = base_filter_spatial_size
				* filter_shards_total * in_depth;

		const int64 filter_out_buf_size = tile_spatial_size
				* filter_shards_total * in_depth;

		// Total per-filter costs.
		const int64 per_filter_cost = filter_total_size
				+ filter_transform_buffer_size + filter_out_buf_size;

		// Remove fixed cost and divide by per-filter cost.
		const int64 num_filters_cache = std::max(1LL,
				(cache_size - filter_transform_matrix_size) / per_filter_cost);
		const int64 num_filters_transform = std::min(out_depth,
				num_filters_cache);
		...
		OP_REQUIRES_OK(ctx,
				ctx->allocate_temp(DataTypeToEnum<T>::value, TensorShape( {
						tile_spatial_size, base_filter_spatial_size }),
						&filter_transform_matrix));
		T* transform_matrix = filter_transform_matrix.template flat<T>().data();
		transform->GetFilterTransformMatrix(tile_spatial_size,
				base_filter_spatial_size, transform_matrix);

		auto shard =
				[&ctx, &args, &transform, &base_filter_rows, &base_filter_cols,
...
public:
	typedef Eigen::internal::const_blas_data_mapper<T, int64, Eigen::RowMajor> LhsMapper;
	typedef Eigen::internal::gebp_traits<T, T> Traits;
	Eigen::internal::gemm_pack_lhs<T, int64, LhsMapper, Traits::mr,
			Traits::LhsProgress, Eigen::RowMajor> pack_lhs;

	GemmFilterPacker(const int64 rows, const int64 depth, const T* lhs_input,
			T* lhs_block) :
			rows_(rows), depth_(depth), lhs_block_(lhs_block), lhs_mapper_(
					lhs_input, depth_)
	{
	}
...
struct PackFilters {
			...
			std::vector<Tensor>* packed_filters)
	{
		const int64 in_depth = args.in_depth;
		const int64 out_depth = args.out_depth;
		const int64 num_filters = filter_shards_row * filter_shards_col
				* out_depth;
		...
		Shard(worker_threads.num_threads, worker_threads.workers,
				tile_spatial_size, num_filters * in_depth, shard);
	}
};
...
class GemmState {
public:
	typedef Eigen::internal::const_blas_data_mapper<T, int64, Eigen::ColMajor> RhsMapper;
	typedef Eigen::internal::blas_data_mapper<T, int64, Eigen::ColMajor> OutputMapper;
	typedef Eigen::internal::gebp_traits<T, T> Traits;

	Eigen::internal::gemm_pack_rhs<T, int64, RhsMapper, Traits::nr,
			Eigen::ColMajor> pack_rhs;
	Eigen::internal::gebp_kernel<T, T, int64, OutputMapper, Traits::mr,
			Traits::nr, false, false> gebp;
	...
	GemmState(const int64 rows, const int64 cols, const int64 depth,
			...
			rows_(rows), cols_(cols), depth_(depth), out_buffer_size_(
					out_buffer_size), lhs_block_(lhs_block), rhs_block_(
					rhs_block), out_buffer_(out_buffer), rhs_mapper_(rhs_input,
					depth_), out_mapper_(out_buffer, rows_)
	{
	}
...
struct CopyInputTile {
	void operator()(const Conv2DArgs& args,
			const DeepConv2DTransform<T>* transform, const int64 num_tiles,
			const int64 in_r_start, const int64 in_c_start, const T* input,
			T* tile_buffer)
	{
		typedef typename Eigen::internal::packet_traits<T>::type Packet;
		static const int64 kPacketSize = (sizeof(Packet) / sizeof(T));
		...
		const int64 input_vectorized_size = (args.in_depth / kPacketSize)
				* kPacketSize;
...
struct TransformInputTiles {
	...
	void operator()(const Conv2DArgs& args,
			const DeepConv2DTransform<T>* transform, const int64 num_tiles,
			const int64 in_r_start, const int64 in_c_start, const T* input,
			const T* transform_matrix, T* tile_buffer, T* tile_transform)
	{
		const int64 tile_rows = transform->input_shape().rows;
		const int64 tile_cols = transform->input_shape().cols;
		const int64 tile_spatial_size = tile_rows * tile_cols;
...
struct TransformOutputTile {
	...
	void operator()(const Conv2DArgs& args,
			const DeepConv2DTransform<T>* transform, const int64 num_tiles,
			const int64 in_r, const int64 in_c, const int64 filter_shards_row,
			const int64 filter_shards_col, const T* out_transform_matrix,
			const T* out_buffer, T* out_transform_buffer, T* output)
	{
		const int64 tile_rows = transform->input_shape().rows;
		const int64 tile_cols = transform->input_shape().cols;
		const int64 tile_spatial_size = tile_rows * tile_cols;

		const int64 out_buf_stride = num_tiles * args.out_depth
				* filter_shards_row * filter_shards_col;
		...
		MatrixMap C(out_transform_buffer, out_tile_spatial_size,
				out_buf_stride);
			...
			for (int64 od = 0; od < args.out_depth; ++od) {
				...
				for (int64 sr = 0; sr < filter_shards_row; ++sr) {
					for (int64 sc = 0; sc < filter_shards_col; ++sc) {
						...
						const int64 out_buf_base = tile_base + out_depth_base
								+ shard_base;

						// Calcuate output indices and outputs to drop (if needed).
						const int64 out_r_start = in_r + args.pad_rows
								- sr * tile_stride_rows;
						...
						if (out_r_start < 0 || out_r_start >= args.out_rows
								|| out_c_start < 0
								|| out_c_start >= args.out_cols) {
							continue;  // Skip un-needed outputs.
						}

						// Increment output if not first filter shard.
						const bool inc_output =
								(sr == 0 && sc == 0) ? false : true;

						for (int64 ot_row = 0; ot_row < out_tile_rows;
								++ot_row) {
							const int64 out_r = out_r_start + ot_row;
							if (out_r >= args.out_rows)
								continue;

							for (int64 ot_col = 0; ot_col < out_tile_cols;
									++ot_col) {
								const int64 out_c = out_c_start + ot_col;
								if (out_c >= args.out_cols)
									continue;

								// Calculate out tile indexl
								const int64 out_buf_index = ot_row
										* out_tile_cols + ot_col;
								// Read output value from buffer.
								const T out_val =
										out_transform_buffer[out_buf_base
												+ out_buf_index * out_buf_stride];
								// Calculate output index.
								const int64 output_index = args.out_depth
										* (out_r * args.out_cols + out_c) + od;
								// Update output.
								if (inc_output) {
...
struct Conv2DState {
	Conv2DState(const int64 tile_spatial_size, const int64 filter_shards_row,
			...
			tile_spatial_size(tile_spatial_size), filter_shards_row(
					filter_shards_row), filter_shards_col(filter_shards_col), input(
					input), tile_transform_matrix(tile_transform_matrix), output_transform_matrix(
					output_transform_matrix), buffer1(buffer1), buffer2(
					buffer2), packed_tile_buffer(packed_tile_buffer), gemm_output_buffer(
					gemm_output_buffer)
	{
	}
...
struct ComputeConv2D {
	void operator()(const Conv2DArgs& args,
			const DeepConv2DTransform<T>* transform, const Conv2DState<T>& cs,
			const int64 in_r, const int64 in_c, const int64 num_tiles,
			...
			T* output)
	{
		...
		const int64 in_depth = args.in_depth;
		const int64 out_depth = args.out_depth;
		const int64 num_filters = cs.filter_shards_row * cs.filter_shards_col
				* out_depth;
		...
		for (int64 i = 0; i < cs.tile_spatial_size; ++i) {
			GemmState<T> gemm(num_filters, num_tiles, in_depth,
					gemm_out_buf_size, packed_filters[i].template flat<T>().data(),
		...
		TransformOutputTile<T>()(args, transform, num_tiles, in_r, in_c,
				...
				cs.output_transform_matrix, cs.buffer1, cs.buffer2, output);
	}
};
...
struct DeepConv2D<CPUDevice, T> {
			...
			const T* input, const T* filter, T* output)
	{
		...
		std::unique_ptr<DeepConv2DTransform<T>> transform(
				new WinogradTransform<T>);
		...
		const int64 filter_residual_row = std::max(0LL,
				args.filter_rows - base_filter_rows);
		const int64 filter_shards_row = 1 + (filter_residual_row + 2 - 1) / 2;

		const int64 filter_residual_col = std::max(0LL,
				args.filter_cols - base_filter_rows);
		...
		OP_REQUIRES_OK(ctx,
				ctx->allocate_temp(DataTypeToEnum<T>::value, TensorShape( {
						tile_rows, tile_cols, out_depth, filter_shards_row,
						filter_shards_col, in_depth }), &filter_transform));
		...
		transform->GetOutputTransformMatrix(out_tile_spatial_size,
				tile_spatial_size, output_transform_matrix);
