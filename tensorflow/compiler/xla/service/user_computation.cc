...
/* static */StatusOr<std::unique_ptr<UserComputation>> UserComputation::MakeWithRemapping(
...
{
	auto user_computation = MakeUnique<UserComputation>(
			session_computation.name(), handle);
	{
		tensorflow::mutex_lock lock(user_computation->mutex_);
		user_computation->session_computation_ = session_computation;
		user_computation->next_handle_value_ = std::max_element(
				session_computation.requests().begin(),
		...
		TF_RETURN_IF_ERROR(
				user_computation->RemapEmbeddedComputations(old_to_new));
	}
...
StatusOr<ComputationDataHandle> UserComputation::AddDynamicUpdateSliceInstruction(
		...
		const VariadicOpRequest& variadic_request)
{
	...
	*request.mutable_output_handle() = handle;
	*request.mutable_output_shape() = shape;
	*request.mutable_request()->mutable_variadic_op_request() =
			variadic_request;
...
StatusOr<std::shared_ptr<const ProgramShape>> UserComputation::ComputeProgramShape(
	...
	if (program_shape_ == nullptr || program_shape_version_ != version) {
		...
		for (int64 request_num = 1; request_num <= version; ++request_num) {
			...
			if (request.request().op_case() == OpRequest::kParameterRequest) {
				...
				*program_shape->mutable_parameters(param_no) =
...
void ConstantVisitor(const SessionComputation& session_computation,
		const ComputationDataHandle& handle, std::set<int64>* visited,
		bool* is_constant)
{
	...
	const OperationRequest& request = session_computation.requests().at(
			handle.handle());
	...
	case OpRequest::kGetTupleElementRequest: {
		...
		ConstantVisitor(session_computation,
				get_tuple_element_request.operand(), visited, is_constant);
		break;
	}
	...
	case OpRequest::kDynamicSliceRequest: {
		...
		ConstantVisitor(session_computation, dynamic_slice_request.operand(),
				visited, is_constant);
		ConstantVisitor(session_computation,
				dynamic_slice_request.start_indices(), visited, is_constant);
		break;
	}
	...
	case OpRequest::kDynamicUpdateSliceRequest: {
		...
		ConstantVisitor(session_computation,
				dynamic_update_slice_request.operand(), visited, is_constant);
		ConstantVisitor(session_computation,
				dynamic_update_slice_request.update(), visited, is_constant);
		ConstantVisitor(session_computation,
	...
	case OpRequest::kReduceRequest: {
		...
		ConstantVisitor(session_computation, reduce_request.operand(), visited,
				is_constant);
		ConstantVisitor(session_computation, reduce_request.init_value(),
				visited, is_constant);
		// TODO(b/32495713): We aren't checking the to_apply computation itself.
		break;
	}
	...
	case OpRequest::kSelectAndScatterRequest: {
		...
		ConstantVisitor(session_computation,
				select_and_scatter_request.operand(), visited, is_constant);
		ConstantVisitor(session_computation,
				select_and_scatter_request.source(), visited, is_constant);
		ConstantVisitor(session_computation,
				select_and_scatter_request.init_value(), visited, is_constant);
	...
	case OpRequest::kBroadcastRequest: {
		...
		ConstantVisitor(session_computation, broadcast_request.operand(),
				visited, is_constant);
		break;
	}
	...
	case OpRequest::kPadRequest: {
		...
		ConstantVisitor(session_computation, pad_request.operand(), visited,
				is_constant);
		ConstantVisitor(session_computation, pad_request.padding_value(),
				visited, is_constant);
		break;
	}
	...
	case OpRequest::kVariadicOpRequest: {
		...
		for (const ComputationDataHandle& handle : variadic_op_request.operands()) {
			ConstantVisitor(session_computation, handle, visited, is_constant);
		}
		break;
	}
	...
	case OpRequest::kUnaryOpRequest: {
		...
		ConstantVisitor(session_computation, unary_op_request.operand(),
				visited, is_constant);
		break;
	}
...
std::vector<VersionedComputationHandle> UserComputation::GetEmbeddedComputations(
		VersionedComputationHandle::Version version) const
{
	tensorflow::mutex_lock lock(mutex_);
...
{
	auto update =
			[&old_to_new](ComputationHandle* to_update) -> Status {
	...
	TF_RETURN_IF_ERROR(
			update(session_computation_.mutable_computation_handle()));
	for (auto& handle_request : *session_computation_.mutable_requests()) {
		...
		case OpRequest::kReduceWindowRequest: {
			...
			ReduceWindowRequest* reduce_window_request =
					request.mutable_request()->mutable_reduce_window_request();
			TF_RETURN_IF_ERROR(
					update(reduce_window_request->mutable_to_apply()));
			break;
		}
		case OpRequest::kSelectAndScatterRequest: {
		...
		const ComputationDataHandle& handle) const
{
	...
	if (session_computation_.requests().count(handle_value) == 0) {
		return InvalidArgument("no ComputationDataHandle value %lld",
				handle_value);
	}
		...
		VersionedComputationHandle::Version version) const
{
	...
	for (int64 request_num = 1; request_num <= version; ++request_num) {
		const OperationRequest& request = session_computation_.requests().at(
				request_num);
		...
		if (request.request().op_case() == OpRequest::kParameterRequest) {
			...
			TF_RET_CHECK(
					0
							== parameter_requests.count(
									parameter_request.parameter()));
			parameter_requests[parameter_request.parameter()] =
					&parameter_request;
		}
	}
...
public:
	static std::unique_ptr<HloComputation> Lower(const string& computation_name,
			const SessionComputation& session_computation,
	...
	{
		ComputationLowerer lowerer(computation_name, session_computation,
				version, std::move(hlo_resolver));
...
private:
	ComputationLowerer(const string& computation_name,
			...
			VersionedComputationHandle::Version version,
			UserComputation::HloComputationResolver hlo_resolver) :
			hlo_builder_(computation_name), session_computation_(
					session_computation), version_(version), hlo_resolver_(
...
private:
	...
	HloComputation* ResolveComputation(const ComputationHandle& handle,
			VersionedComputationHandle::Version version);
		...
		bool include_unused_parameters)
{
	...
	for (const auto& send_request : session_computation_.send_requests()) {
		...
		hlo_builder_.AddInstruction(
				HloInstruction::CreateSend(operand,
						send_request.channel_handle().handle()));
	}
	...
	for (const auto& outfeed_request : session_computation_.outfeed_requests()) {
		...
		hlo_builder_.AddInstruction(
				HloInstruction::CreateOutfeed(operand,
...
HloInstruction* ComputationLowerer::Visit(const ComputationDataHandle& handle,
		std::map<int64, HloInstruction*>* visited)
{
	...
	case OpRequest::kRngRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateRng(request.output_shape(),
						rng_request.distribution(), parameters));
		break;
	}
	...
	case OpRequest::kConstantRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateConstant(
						LiteralUtil::CloneToUnique(
								constant_request.literal())));
		break;
	}
	...
	case OpRequest::kGetTupleElementRequest: {
		const GetTupleElementRequest& get_tuple_element_request =
				request.request().get_tuple_element_request();
		HloInstruction* operand = Visit(get_tuple_element_request.operand(),
				visited);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateGetTupleElement(request.output_shape(),
						operand, get_tuple_element_request.index()));
		break;
	}
	...
	case OpRequest::kSliceRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateSlice(request.output_shape(), operand,
	...
	case OpRequest::kDynamicSliceRequest: {
		const DynamicSliceRequest& dynamic_slice_request =
				request.request().dynamic_slice_request();
		HloInstruction* operand = Visit(dynamic_slice_request.operand(),
				visited);
		HloInstruction* start_indices = Visit(
				dynamic_slice_request.start_indices(), visited);

		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateDynamicSlice(request.output_shape(),
	...
	case OpRequest::kDynamicUpdateSliceRequest: {
		const DynamicUpdateSliceRequest& dynamic_update_slice_request =
				request.request().dynamic_update_slice_request();
		HloInstruction* operand = Visit(dynamic_update_slice_request.operand(),
				visited);
		HloInstruction* update = Visit(dynamic_update_slice_request.update(),
				visited);
		HloInstruction* start_indices = Visit(
				dynamic_update_slice_request.start_indices(), visited);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateDynamicUpdateSlice(request.output_shape(),
	...
	case OpRequest::kConcatenateRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateConcatenate(request.output_shape(),
						operands, concatenate_request.dimension()));
		break;
	}
	...
	case OpRequest::kConvolveRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateConvolve(request.output_shape(), lhs, rhs,
						convolve_request.window(),
						convolve_request.dimension_numbers()));
		break;
	}
	...
	case OpRequest::kCrossReplicaSumRequest: {
		...
		HloInstruction* operand = Visit(cross_replica_sum_request.operand(),
				visited);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateCrossReplicaSum(request.output_shape(),
						operand));
		break;
	}
	...
	case OpRequest::kInfeedRequest: {
		const InfeedRequest& infeed_request =
				request.request().infeed_request();
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateInfeed(request.output_shape(),
						infeed_request.config()));
		break;
	}
	...
	case OpRequest::kMapRequest: {
		...
		VersionedComputationHandle::Version map_version =
				request.embedded_computation_versions(0);
		HloComputation* map_computation = ResolveComputation(
				map_request.to_apply(), map_version);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateMap(request.output_shape(), operands,
						map_computation));
		break;
	}
	...
	case OpRequest::kReduceRequest: {
		...
		HloInstruction* init_value = Visit(reduce_request.init_value(),
				visited);
		CHECK_EQ(1, request.embedded_computation_versions_size());
		VersionedComputationHandle::Version reduce_version =
				request.embedded_computation_versions(0);
		HloComputation* reduce_computation = ResolveComputation(
				reduce_request.to_apply(), reduce_version);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateReduce(request.output_shape(), operand,
						init_value, AsInt64Slice(reduce_request.dimensions()),
						reduce_computation));
		break;
	}
	...
	case OpRequest::kReduceWindowRequest: {
		const ReduceWindowRequest& reduce_window_request =
				request.request().reduce_window_request();
		HloInstruction* operand = Visit(reduce_window_request.operand(),
				visited);
		HloInstruction* init_value = Visit(reduce_window_request.init_value(),
				visited);
		CHECK_EQ(1, request.embedded_computation_versions_size());
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateReduceWindow(request.output_shape(),
						operand, init_value, reduce_window_request.window(),
						reduce_window_computation));
		break;
	}
	...
	case OpRequest::kSelectAndScatterRequest: {
		const SelectAndScatterRequest& select_and_scatter_request =
				request.request().select_and_scatter_request();
		HloInstruction* operand = Visit(select_and_scatter_request.operand(),
				visited);
		HloInstruction* source = Visit(select_and_scatter_request.source(),
				visited);
		HloInstruction* init_value = Visit(
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateSelectAndScatter(request.output_shape(),
						operand, select_computation,
	...
	case OpRequest::kBroadcastRequest: {
		...
		for (int i = 0; i < ShapeUtil::Rank(operand->shape()); ++i) {
			broadcast_dimensions.push_back(
					i + ShapeUtil::Rank(request.output_shape())
							- ShapeUtil::Rank(operand->shape()));
		}
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateBroadcast(request.output_shape(), operand,
	...
	case OpRequest::kReshapeRequest: {
		...
		hlo_instruction =
				hlo_builder_.AddInstruction(
						HloInstruction::CreateReshape(request.output_shape(),
								hlo_builder_.AddInstruction(
	...
	case OpRequest::kReverseRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateReverse(request.output_shape(), operand,
	...
	case OpRequest::kPadRequest: {
		...
		HloInstruction* padding_value = Visit(pad_request.padding_value(),
				visited);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreatePad(request.output_shape(), operand,
						padding_value, pad_request.padding_config()));
		break;
	}
	...
	case OpRequest::kRecvRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateRecv(request.output_shape(),
						recv_request.channel_handle().handle()));
		break;
	}
	...
	case OpRequest::kParameterRequest: {
		const ParameterRequest& parameter_request =
				request.request().parameter_request();
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateParameter(parameter_request.parameter(),
						request.output_shape(), parameter_request.name()));
		break;
	}
	...
	case OpRequest::kWhileRequest: {
		...
		VersionedComputationHandle::Version condition_version =
				request.embedded_computation_versions(0);
		HloComputation* condition = ResolveComputation(
				while_request.condition(), condition_version);
		VersionedComputationHandle::Version body_version =
				request.embedded_computation_versions(1);
		HloComputation* body = ResolveComputation(while_request.body(),
				body_version);
		HloInstruction* init = Visit(while_request.init(), visited);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateWhile(request.output_shape(), condition,
						body, init));
		break;
	}
	...
	case OpRequest::kTernaryOpRequest: {
		...
		auto hlo_opcode = TernaryOperationToHloOpcode(
				ternary_op_request.triop());
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateTernary(request.output_shape(),
						hlo_opcode, lhs, rhs, ehs));
		break;
	}
	...
	case OpRequest::kVariadicOpRequest: {
		...
		auto hlo_opcode = VariadicOperationToHloOpcode(
				variadic_op_request.varop());
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateVariadic(request.output_shape(),
						hlo_opcode, operands));
		break;
	}
	...
	case OpRequest::kCallRequest: {
		...
		VersionedComputationHandle::Version call_version =
				request.embedded_computation_versions(0);
		HloComputation* call_computation = ResolveComputation(
				call_request.to_apply(), call_version);
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateCall(request.output_shape(), operands,
						call_computation));
		break;
	}
	...
	case OpRequest::kUnaryOpRequest: {
		...
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateUnary(request.output_shape(), hlo_opcode,
						operand));
		break;
	}
	...
	case OpRequest::kBinaryOpRequest: {
		...
		if (binary_op_request.broadcast_dimensions_size() > 0) {
			...
			// operation.
			CHECK_NE(ShapeUtil::Rank(lhs->shape()),
					ShapeUtil::Rank(rhs->shape()));
			HloInstruction* operand_to_broadcast =
					ShapeUtil::Rank(lhs->shape())
							< ShapeUtil::Rank(rhs->shape()) ? lhs : rhs;
			...
			HloInstruction* broadcasted_operand = hlo_builder_.AddInstruction(
					HloInstruction::CreateBroadcast(broadcast_shape,
							operand_to_broadcast,
							AsInt64Slice(
									binary_op_request.broadcast_dimensions())));

			lhs = (lhs == operand_to_broadcast) ? broadcasted_operand : lhs;
			rhs = (rhs == operand_to_broadcast) ? broadcasted_operand : rhs;
		}
		hlo_instruction = hlo_builder_.AddInstruction(
				HloInstruction::CreateBinary(request.output_shape(), hlo_opcode,
						lhs, rhs));
		break;
	}
