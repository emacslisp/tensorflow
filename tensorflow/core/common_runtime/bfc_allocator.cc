...
BFCAllocator::BFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
		...
		suballocator_(sub_allocator), name_(name), free_chunks_list_(
				kInvalidChunkHandle), next_allocation_id_(1)
{
	if (allow_growth) {
		// 1MiB smallest initial allocation, unless total memory available
		// is less.
		curr_region_allocation_bytes_ = RoundedBytes(
				std::min(total_memory, size_t { 1048576 }));
	} else {
...
bool BFCAllocator::Extend(size_t rounded_bytes)
{
	...
	available_bytes = (available_bytes / kMinAllocationSize)
			* kMinAllocationSize;
	...
	VLOG(1) << "Extending allocation by "
			<< strings::HumanReadableNumBytes(bytes) << " bytes.";
...
void* BFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes)
{
	...
	if (r != nullptr) {
		...
		return retry_helper_.AllocateRaw([this](size_t a, size_t nb, bool v) {
...
void* BFCAllocator::AllocateRawInternal(size_t unused_alignment,
		size_t num_bytes, bool dump_log_on_failure)
{
	if (num_bytes == 0) {
		...
		for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end();
			...
			if (chunk->size >= rounded_bytes) {
				...
				stats_.max_bytes_in_use = std::max(stats_.max_bytes_in_use,
						stats_.bytes_in_use);
				stats_.max_alloc_size = std::max<std::size_t>(
						stats_.max_alloc_size, chunk->size);
...
size_t BFCAllocator::RequestedSize(void* ptr)
{
	...
	BFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
	CHECK(h != kInvalidChunkHandle)
			<< "Asked for requested size of pointer we never allocated: "
			<< ptr;
	BFCAllocator::Chunk* c = ChunkFromHandle(h);
...
size_t BFCAllocator::AllocatedSize(void* ptr)
{
	...
	BFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
	CHECK(h != kInvalidChunkHandle)
			<< "Asked for allocated size of pointer we never allocated: "
			<< ptr;
	BFCAllocator::Chunk* c = ChunkFromHandle(h);
	return c->size;
}
...
void RenderRegion(char* rendered, const size_t resolution,
		...
		const void* base_ptr, const void* ptr, const size_t size, const char c)
{
	const char* base_ptr_c = static_cast<const char*>(base_ptr);
	const char* ptr_c = static_cast<const char*>(ptr);

	size_t start_location = ((ptr_c - base_ptr_c + offset) * resolution)
			/ total_render_size;
			...
			if (c->in_use()) {
				...
				if (wasted > 0) {
					RenderRegion(rendered, resolution, total_region_size,
							region_offset + c->requested_size, region.ptr(),
							c->ptr, wasted, 'x');
				}
...
void BFCAllocator::DumpMemoryLog(size_t num_bytes)
{
	...
	for (BinNum bin_num = 0; bin_num < kNumBins; bin_num++) {
		...
		LOG(INFO) << "Bin (" << b->bin_size << "): \tTotal Chunks: "
				<< total_chunks_in_bin << ", Chunks in use: "
				<< total_chunks_in_use << " "
