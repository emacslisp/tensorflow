...
void LinearAlgebraOp<Scalar>::ValidateSingleMatrix(OpKernelContext* context,
...
// static
template<typename Scalar>
void LinearAlgebraOp<Scalar>::ValidateSolver(OpKernelContext* context,
		const TensorShapes& input_matrix_shapes)
{
	...
	OP_REQUIRES(context,
			input_matrix_shapes[0].dim_size(0)
					== input_matrix_shapes[1].dim_size(0),
			errors::InvalidArgument("Input matrix and rhs are incompatible."));
}

// static
template<typename Scalar>
void LinearAlgebraOp<Scalar>::ValidateSquareSolver(OpKernelContext* context,
		const TensorShapes& input_matrix_shapes)
{
	OP_REQUIRES(context, input_matrix_shapes.size() == 2,
			errors::InvalidArgument("Expected two input matrices, got %d.",
	...
	OP_REQUIRES(context,
			...
			errors::InvalidArgument(
					"First input (lhs) must be a square matrix."));
	OP_REQUIRES(context, TensorShapeUtils::IsMatrix(input_matrix_shapes[1]),
			errors::InvalidArgument("Second input (rhs) must be a matrix."));
	OP_REQUIRES(context,
			input_matrix_shapes[0].dim_size(0)
...
void LinearAlgebraOp<Scalar>::Compute(OpKernelContext* context)
{
	...
	Shard(worker_threads.num_threads, worker_threads.workers,
			batch_shape.num_elements(), GetCostPerUnit(input_matrix_shapes),
			shard);
}

template<typename Scalar>
void LinearAlgebraOp<Scalar>::AnalyzeInputs(OpKernelContext* context,
		TensorInputs* inputs, TensorShapes* input_matrix_shapes,
		TensorShape* batch_shape)
{
	int input_rank = -1;
	for (int i = 0; i < NumMatrixInputs(context); ++i) {
		...
		if (i == 0) {
			...
			for (int dim = 0; dim < input_rank - 2; ++dim) {
				OP_REQUIRES(context,
						in.dim_size(dim) == batch_shape->dim_size(dim),
						errors::InvalidArgument(
...
template<typename Scalar>
void LinearAlgebraOp<Scalar>::PrepareOutputs(OpKernelContext* context,
		const TensorShapes& input_matrix_shapes, const TensorShape& batch_shape,
		TensorOutputs* outputs, TensorShapes* output_matrix_shapes)
{
	...
	*output_matrix_shapes = GetOutputMatrixShapes(input_matrix_shapes);
	...
	OP_REQUIRES(context, num_outputs <= context->num_outputs(),
			errors::Internal(
	...
	for (int i = 0; i < context->num_outputs(); ++i) {
		...
		if (i < num_outputs) {
			...
			const TensorShape& output_matrix_shape = output_matrix_shapes->at(
					i);
...
template<typename Scalar>
void LinearAlgebraOp<Scalar>::ComputeTensorSlice(OpKernelContext* context,
		int64 matrix_index, const TensorInputs& inputs,
		...
		const TensorShapes& output_matrix_shapes)
{
		...
		matrix_inputs.push_back(
				ConstMatrixMap(
						inputs[i].flat<Scalar>().data()
								+ matrix_index
										* input_matrix_shapes[i].num_elements(),
	...
	for (int i = 0; i < output_matrix_shapes.size(); ++i) {
		...
		int num_output_rows =
				output_matrix_shapes[i].dims() >= 1 ?
						output_matrix_shapes[i].dim_size(0) : 1;
		int num_output_cols =
				output_matrix_shapes[i].dims() == 2 ?
						output_matrix_shapes[i].dim_size(1) : 1;
		matrix_outputs.push_back(
				MatrixMap(
						outputs[i]->flat<Scalar>().data()
								+ matrix_index
										* output_matrix_shapes[i].num_elements(),
