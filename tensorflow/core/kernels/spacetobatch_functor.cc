...
struct SpaceToBatchHelper {
	...
	static void run(T* space_tensor_ptr, const int64* space_tensor_shape,
			...
			const int64* batch_tensor_shape, const int64* batch_tensor_strides,
			T* batch_tensor_ptr)
	{
		for (int64 batch_tensor_pos = 0;
				batch_tensor_pos < batch_tensor_shape[0]; ++batch_tensor_pos) {
			const int64 space_tensor_pos = batch_tensor_pos * block_shape[0]
					+ block_offsets[0] - pad_start[0];
			if (space_tensor_pos >= 0
					&& space_tensor_pos < space_tensor_shape[0]) {
				SpaceToBatchHelper<N - 1, B2S>::run(
						space_tensor_ptr
								+ space_tensor_pos * space_tensor_strides[0],
						space_tensor_shape + 1, space_tensor_strides + 1,
						block_shape + 1, pad_start + 1, block_offsets + 1,
						batch_tensor_shape + 1, batch_tensor_strides + 1,
						batch_tensor_ptr);
			} else {
				if (B2S == false) {
					// Copy in padding.
...
struct SpaceToBatchFunctor<CPUDevice, T, NUM_BLOCK_DIMS, B2S> {
	...
	Status operator()(const CPUDevice& d,
			...
			typename TTypes<BatchT, NUM_BLOCK_DIMS + 2>::Tensor batch_tensor)
	{
		...
		for (int block_dim = 0; block_dim < NUM_BLOCK_DIMS; ++block_dim) {
			...
			space_tensor_shape[block_dim] = space_tensor.dimension(
					block_dim + 1);
			batch_tensor_shape[block_dim] = batch_tensor.dimension(
					block_dim + 1);
		}
		...
		for (int dim = NUM_BLOCK_DIMS; dim >= 0; --dim) {
			space_tensor_strides[dim] = space_tensor_strides[dim + 1]
					* space_tensor.dimension(dim + 1);
			batch_tensor_strides[dim] = batch_tensor_strides[dim + 1]
					* batch_tensor.dimension(dim + 1);
		}
		...
		for (int64 batch_tensor_b = 0; batch_tensor_b < batch_tensor_batch;
			...
			for (int block_dim = NUM_BLOCK_DIMS - 1; block_dim >= 0;
					--block_dim) {
			...
			SpaceToBatchHelper<NUM_BLOCK_DIMS, B2S>::run(
					...
					space_tensor_shape, &space_tensor_strides[1], block_shape,
					pad_start, block_offsets, batch_tensor_shape,
					&batch_tensor_strides[1],
					batch_tensor_ptr
							+ batch_tensor_b * batch_tensor_strides[0]);
		}
		return Status::OK();
	}
};
