...
class TrackedObject {
public:
	TrackedObject(const std::string& id, const Image<uint8>& image,
			const BoundingBox& bounding_box, ObjectModelBase* const model);

	~TrackedObject();

	void UpdatePosition(const BoundingBox& new_position, const int64 timestamp,
