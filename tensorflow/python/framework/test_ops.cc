...
namespace {
enum KernelLabel {
	DEFAULT_LABEL, OVERLOAD_1_LABEL, OVERLOAD_2_LABEL
};
}  // namespace
...
public:
	OldOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	}
...
public:
	ResourceCreateOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
...
public:
	explicit ResourceUsingOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* ctx)
	{
		StubResource * unused;
		OP_REQUIRES_OK(ctx,
				LookupResource<StubResource>(ctx, HandleFromInput(ctx, 0),
						&unused));
	}
};
