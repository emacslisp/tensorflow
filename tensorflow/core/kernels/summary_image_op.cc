...
public:
	...
	explicit SummaryImageOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		int64 max_images_tmp;
		OP_REQUIRES_OK(context,
				context->GetAttr("max_images", &max_images_tmp));
		...
		OP_REQUIRES_OK(context,
				context->device()->MakeTensorFromProto(*proto,
						AllocatorAttributes(), &bad_color_));
		...
		OP_REQUIRES(context, TensorShapeUtils::IsVector(bad_color_.shape()),
				errors::InvalidArgument(
						"bad_color must be a vector, got shape ",
	...
	void Compute(OpKernelContext* c) override
	{
		...
		if (tensor.dtype() == DT_UINT8) {
			...
			auto ith_image =
					[&tensor, batch_size, hw, depth](int i) {
			...
			NormalizeAndAddImages<Eigen::half>(c, tensor, h, w, hw, depth,
					batch_size, base_tag, &s);
		...
		OP_REQUIRES_OK(c,
				c->allocate_output(0, TensorShape( { }), &summary_tensor));
	...
	void NormalizeAndAddImages(OpKernelContext* c, const Tensor& tensor, int h,
			int w, int hw, int depth, int batch_size, const string& base_tag,
			Summary* s)
	{
		...
		typename TTypes<uint8>::ConstVec bad_color(bad_color_full.data(),
				depth);

		// Float images must be scaled and translated.
		Uint8Image image(hw, depth);
		auto ith_image =
				[&tensor, &image, bad_color, batch_size, hw,
				depth](int i) {
	...
	static void NormalizeFloatImage(int hw, int depth,
			typename TTypes<T>::ConstMatrix values,
			typename TTypes<uint8>::ConstVec bad_color, Uint8Image* image)
	{
		if (!image->size())
			return;  // Nothing to do for empty images
		...
		for (int i = 0; i < hw; i++) {
			...
			if (finite) {
				image->chip < 0 > (i) = (values.template chip < 0
						> (i) * scale + offset).template cast<uint8>();
			} else {
