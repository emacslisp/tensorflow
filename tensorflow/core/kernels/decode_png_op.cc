...
public:
	explicit DecodePngOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument("Type must be UINT8 or UINT16, got ",
						dt));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				png::CommonInitDecode(data, channels_, desired_channel_bits_,
						&decode),
		...
		const int64 total_size = static_cast<int64>(width)
				* static_cast<int64>(height);
		if (width != static_cast<int64>(decode.width) || width <= 0
				|| width >= (1LL << 27)
				|| height != static_cast<int64>(decode.height) || height <= 0
				|| height >= (1LL << 27) || total_size >= (1LL << 29)) {
		...
		const auto status = context->allocate_output(0, TensorShape( { height,
				width, decode.channels }), &output);
		if (!status.ok())
			png::CommonFreeDecode(&decode);
		...
		if (desired_channel_bits_ == 8) {
			...
			OP_REQUIRES(context,
					...
					errors::InvalidArgument("Invalid PNG data, size ",
							data.size()));
		} else {
			// Finish decoding image
			OP_REQUIRES(context,
