...
public:
	explicit SparseSoftmaxOp(OpKernelConstruction *context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext *context) override
	{
		...
		SparseTensor st(tensor::DeepCopy(*indices_t),
				tensor::DeepCopy(*values_t),
				TensorShape(shape_t->flat<int64>()));

		Tensor *output_values = nullptr;
		OP_REQUIRES_OK(context,
				context->allocate_output(0, TensorShape( { nnz }),
						&output_values));
		...
		for (const auto &g : st.group(kGroupByDims)) {
			...
			tmp_scalar.device(context->eigen_device<Device>()) =
					group_vals.maximum();
