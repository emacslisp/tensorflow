...
{
	return static_cast<CUDAExecutor *>(stream->parent()->implementation())->cuda_context();
}
...
bool CUDAExecutor::FindOnDiskForComputeCapability(port::StringPiece filename,
		port::StringPiece canonical_suffix, string *found_filename) const
{
		...
		KernelBase *kernel)
{
	...
	if (on_disk_spec != nullptr) {
		...
		const char *orig_ptx = spec.cuda_ptx_in_memory().original_text(
				cc_major_, cc_minor_);
		...
		if (module == nullptr) {
			if (g_cubinate == nullptr) {
				...
				auto load_status = CUDADriver::LoadCubin(context_,
						cubin.c_str(), &module);
				if (!load_status.ok()) {
					LOG(ERROR) << "failed to load cubin via hook: "
							<< load_status;
					return false;
				}
			}
		...
		KernelMetadata *kernel_metadata)
{
	...
	if (!CUDADriver::FuncGetAttribute(CU_FUNC_ATTRIBUTE_NUM_REGS,
			*cuda_kernel->cuda_function_ptr(), &value)) {
		return false;
	}
		...
		const KernelArgsArrayBase &args)
{
	...
	if (cuda_kernel->GetPreferredCacheConfig()
		...
		CUDADriver::FuncSetCacheConfig(cufunc,
				cuda_kernel->GetCUDACacheConfig());
	}
	...
	if (!CUDADriver::LaunchKernel(GetCudaContext(stream), cufunc, block_dims.x,
			...
			thread_dims.z, args.number_of_shared_bytes(), custream,
			kernel_params, nullptr /* = extra */)) {
		LOG(ERROR) << "failed to launch CUDA kernel with args: "
				<< args.number_of_arguments() << "; thread dim: "
				<< thread_dims.ToString() << "; block dim: "
				<< block_dims.ToString();
		return false;
	}
		...
		const ThreadDim &thread_dims, const BlockDim &block_dims)
{
	...
	uint64 blocks_per_sm = CalculateOccupancy(device_description,
			regs_per_thread, smem_per_block, thread_dims);
	...
	int available_blocks_per_sm = port::MathUtil::CeilOfRatio(block_count,
	...
	if (improved_regs_per_thread != 0) {
		VLOG(2) << "Reducing register usage from " << regs_per_thread << " to "
				<< improved_regs_per_thread
		...
		if (reg_reduction
			...
			LOG(INFO)
					<< "Notice: occupancy would increase if register usage was"
					<< " reduced from " << regs_per_thread << " to "
					<< improved_regs_per_thread
...
bool CUDAExecutor::SynchronousMemZero(DeviceMemoryBase *location, uint64 size)
{
	if (reinterpret_cast<uintptr_t>(location->opaque()) % 4 == 0
			&& size % 4 == 0) {
		return CUDADriver::SynchronousMemsetUint32(context_,
				AsCudaDevicePtr(location), 0x0, size / 4);
	}
	return CUDADriver::SynchronousMemsetUint8(context_,
			AsCudaDevicePtr(location), 0x0, size);
}
		...
		uint64 size)
{
	if (reinterpret_cast<uintptr_t>(location->opaque()) % 4 == 0
		...
		return CUDADriver::SynchronousMemsetUint32(context_,
				AsCudaDevicePtr(location), pattern, size / 4);
	}
	return CUDADriver::SynchronousMemsetUint8(context_,
			AsCudaDevicePtr(location), value, size);
}
...
bool CUDAExecutor::SynchronousMemcpyDeviceToDevice(DeviceMemoryBase *gpu_dst,
		const DeviceMemoryBase &gpu_src, uint64 size)
{
		...
		uint8 pattern, uint64 size)
{
	...
	return CUDADriver::AsynchronousMemsetUint8(context_,
			AsCudaDevicePtr(location), pattern, size, AsCUDAStreamValue(stream));
}
		...
		uint32 pattern, uint64 size)
{
	VLOG(2) << "enqueueing memset32 operation onto stream " << stream
			...
			<< " and pattern " << std::hex << pattern;
	CHECK(
			reinterpret_cast<uintptr_t>(location->opaque()) % 4 == 0
					&& size % 4 == 0);
	return CUDADriver::AsynchronousMemsetUint32(context_,
			AsCudaDevicePtr(location), pattern, size / 4,
		...
		const DeviceMemoryBase &gpu_src, uint64 size)
{
	return CUDADriver::AsynchronousMemcpyD2H(context_, host_dst,
			AsCudaDevicePtr(gpu_src), size, AsCUDAStreamValue(stream));
}
		...
		const void *host_src, uint64 size)
{
	return CUDADriver::AsynchronousMemcpyH2D(context_, AsCudaDevicePtr(gpu_dst),
			host_src, size, AsCUDAStreamValue(stream));
}
...
bool CUDAExecutor::HostCallback(Stream *stream, std::function<void()> callback)
{
...
blas::BlasSupport *CUDAExecutor::CreateBlas()
{
	...
	port::StatusOr<PluginRegistry::BlasFactory> status = registry->GetFactory<
			PluginRegistry::BlasFactory>(kCudaPlatformId,
			plugin_config_.blas());
	if (!status.ok()) {
...
{
	PluginRegistry *registry = PluginRegistry::Instance();
	port::StatusOr<PluginRegistry::DnnFactory> status = registry->GetFactory<
			PluginRegistry::DnnFactory>(kCudaPlatformId, plugin_config_.dnn());
	if (!status.ok()) {
...
{
	PluginRegistry *registry = PluginRegistry::Instance();
	port::StatusOr<PluginRegistry::FftFactory> status = registry->GetFactory<
			PluginRegistry::FftFactory>(kCudaPlatformId, plugin_config_.fft());
	if (!status.ok()) {
...
{
	PluginRegistry *registry = PluginRegistry::Instance();
	port::StatusOr<PluginRegistry::RngFactory> status = registry->GetFactory<
			PluginRegistry::RngFactory>(kCudaPlatformId, plugin_config_.rng());
	if (!status.ok()) {
		...
		size_t *bytes)
{
	{  // give limited scope to mutex_lock
		...
		for (auto &it : disk_modules_) {
			if (CUDADriver::GetModuleSymbol(context_, it.second,
					symbol_name.c_str(), reinterpret_cast<CUdeviceptr *>(mem),
					bytes)) {
				return true;
			}
		}
...
bool CUDAExecutor::SupportsBlas() const
{
	return true;
}

bool CUDAExecutor::SupportsFft() const
{
	return true;
}

bool CUDAExecutor::SupportsRng() const
{
	return true;
}

std::unique_ptr<internal::EventInterface> CUDAExecutor::CreateEventImplementation()
{
	return std::unique_ptr<internal::EventInterface>(new CUDAEvent(this));
}

std::unique_ptr<internal::KernelInterface> CUDAExecutor::CreateKernelImplementation()
{
	return std::unique_ptr<internal::KernelInterface>(new CUDAKernel());
}

std::unique_ptr<internal::StreamInterface> CUDAExecutor::GetStreamImplementation()
{
	return std::unique_ptr<internal::StreamInterface>(new CUDAStream(this));
}

std::unique_ptr<internal::TimerInterface> CUDAExecutor::GetTimerImplementation()
{
...
static const UnqueryableDeviceParams kAllUnqueryableDeviceParams[] = { { 3, 5, // compute capability (3.5)
...
DeviceDescription *CUDAExecutor::PopulateDeviceDescription() const
{
...

	{
		...
		string augmented_driver_version =
				port::Printf("%d (%s)", driver_version,
						DriverVersionStatusToString(
								Diagnostician::FindDsoVersion()).c_str());
	...
	for (size_t i = 0; i < ARRAYSIZE(kAllUnqueryableDeviceParams); i++) {
		...
		if (params.cc_major == cc_major_ && params.cc_minor == cc_minor_) {
			...
			builder.set_registers_per_core_limit(
					params.registers_per_core_limit);
			builder.set_registers_per_thread_limit(
					params.registers_per_thread_limit);
			builder.set_warp_alloc_granularity(params.warp_alloc_granularity);
			builder.set_register_alloc_granularity(
					params.register_alloc_granularity);
