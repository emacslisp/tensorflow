...
bool IsChromaDownsampled(const string& jpegdata)
{
	...
	for (int i = 0; i < components; ++i) {
		CHECK_GT(cinfo.comp_info[i].h_samp_factor, 0)
				<< "Invalid sampling factor.";
		CHECK_EQ(cinfo.comp_info[i].h_samp_factor,
	...
	const bool downsampled = cinfo.comp_info[1].h_samp_factor
		...
		const bool try_recover_truncated_jpeg)
{
	...
	std::unique_ptr<uint8[]> imgdata;
	imgdata.reset(
			Uncompress(jpeg.c_str(), jpeg.size(), flags, &width, &height,
					&components, NULL));
