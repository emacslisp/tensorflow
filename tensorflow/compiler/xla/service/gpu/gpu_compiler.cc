...
void DumpPtxasInfo(const string& ptx)
{
	...
	TF_CHECK_OK(
			tensorflow::WriteStringToFile(tensorflow::Env::Default(), ptx_path,
					ptx));
		...
		se::StreamExecutor* stream_exec)
{
	...
	TF_RETURN_IF_ERROR(
			OptimizeHloModule(hlo_module.get(), dump_hlo,
					stream_exec->GetDeviceDescription()));
	TF_RETURN_IF_ERROR(
			PrepareHloModuleForIrEmitting(dump_hlo, hlo_module.get(),
					module_config.get()));
	...
	std::unique_ptr<StreamAssignment> stream_assignment = AssignStreams(
			*hlo_module);
	...
	TF_ASSIGN_OR_RETURN(std::unique_ptr < BufferAssignment > buffer_assignment,
			BufferAssigner::Run(hlo_module.get(),
					hlo_schedule->ConsumeHloOrdering(), pointer_size));
	auto temp_buffer_offsets = MakeUnique<TempBufferOffsets>(
			*buffer_assignment);

	IrEmitterContext ir_emitter_context(hlo_module.get(),
			buffer_assignment.get(), temp_buffer_offsets.get(),
			&stream_exec->GetDeviceDescription(), &llvm_module);

	HloComputation* entry_computation = hlo_module->entry_computation();
	IrEmitterUnnested ir_emitter(*module_config, entry_computation,
			module_config->has_hybrid_result(), &ir_emitter_context);
	TF_RETURN_IF_ERROR(
	...
	TF_ASSIGN_OR_RETURN(*ptx,
			CompileToPtx(&llvm_module, *module_config, libdevice_dir_));
	...
	auto* gpu_executable = new GpuExecutable(*ptx, std::move(thunk_schedule),
			std::move(hlo_module), std::move(module_config),
			std::move(buffer_assignment), std::move(temp_buffer_offsets));
...
StatusOr<std::vector<std::unique_ptr<AotCompilationResult>>>GpuCompiler::CompileAheadOfTime(
