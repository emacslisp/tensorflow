...
{
	return (bits != 0) ?
			(rnd->Rand64() % (1LL << (bits - 1))) + (1LL << (bits - 1)) : 0;
}
...
static void BM_WriteStringIncreasing(int n, int len)
{
BM_WriteString(n, len);
}
static void BM_ReadStringIncreasing(int n, int len)
{
BM_ReadString(n, len);
}
