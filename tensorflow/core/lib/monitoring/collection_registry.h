...
private:
	...
	MetricCollector(
			...
			metric_def_(metric_def), registration_time_millis_(
					registration_time_millis), collector_(collector), point_set_(
					point_set)
	{
...
private:
	...
	MetricCollectorGetter(internal::Collector* const collector,
			...
			const uint64 registration_time_millis) :
			collector_(collector), allowed_metric_def_(allowed_metric_def), registration_time_millis_(
...
public:
	...
	std::unique_ptr<RegistrationHandle> Register(
			...
			const CollectionFunction& collection_function) LOCKS_EXCLUDED(mu_) TF_MUST_USE_RESULT;
...
public:
	Collector(const uint64 collection_time_millis) :
			collected_metrics_(new CollectedMetrics()), collection_time_millis_(
...
// the collection time.
			...
			const uint64 collection_time_millis, Point* const point)
	{
		...
		point->end_timestamp_millis =
				registration_time_millis < collection_time_millis ?
						collection_time_millis : registration_time_millis;
	}
			...
			const MetricDef<metric_kind, Value, NumLabels>* const metric_def)
	{
		if (allowed_metric_def_ != metric_def) {
			LOG(FATAL) << "Expected collection for: "
					<< allowed_metric_def_->name() << " but instead got: "
					<< metric_def->name();
		}

		return collector_->GetMetricCollector(metric_def,
				registration_time_millis_, collector_);
	}
