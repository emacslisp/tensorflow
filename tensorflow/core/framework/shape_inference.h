...
public:
	...
	Status WithRank(ShapeHandle shape, int32 rank, ShapeHandle* out)
	TF_MUST_USE_RESULT;
	Status WithRankAtLeast(ShapeHandle shape, int32 rank, ShapeHandle* out)
	TF_MUST_USE_RESULT;
	Status WithRankAtMost(ShapeHandle shape, int32 rank, ShapeHandle* out)
	TF_MUST_USE_RESULT;
	...
	Status Merge(ShapeHandle in0, ShapeHandle in1, ShapeHandle* out)
	TF_MUST_USE_RESULT;
	...
	Status Merge(DimensionHandle d0, DimensionHandle d1, DimensionHandle* out)
	TF_MUST_USE_RESULT;
	...
	Status Subshape(ShapeHandle s, int64 start, ShapeHandle* out)
	TF_MUST_USE_RESULT;
	...
	Status Subshape(ShapeHandle s, int64 start, int64 end, ShapeHandle* out)
	TF_MUST_USE_RESULT;

	// Returns in <*out> the result of appending the dimensions of <s2> to those
	// of <s1>.
	Status Concatenate(ShapeHandle s1, ShapeHandle s2, ShapeHandle* out)
	TF_MUST_USE_RESULT;
