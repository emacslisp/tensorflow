...
TensorSlice::TensorSlice(std::initializer_list<std::pair<int64, int64>> extents)
{
...
{
	std::vector<string> items = str_util::Split(str, ':',
			str_util::SkipEmpty());
		...
		TensorShape* result_shape) const
{
	...
	if (shape.dims() != dims()) {
		return errors::Internal("Mismatching ranks: shape = ",
