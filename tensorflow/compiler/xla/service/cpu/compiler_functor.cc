...
/* static */CompilerFunctor::VectorIntrinsics CompilerFunctor::AllIntrinsics()
{
...
llvm::object::OwningBinary<llvm::object::ObjectFile> CompilerFunctor::operator()(
		llvm::Module& module) const
{
	...
	if (!dump_path.empty()) {
		...
		TF_CHECK_OK(
				tensorflow::Env::Default()->NewAppendableFile(dump_path, &f));
	...
	llvm::Expected < std::unique_ptr
			< llvm::object::ObjectFile >> object_file_or_error =
			llvm::object::ObjectFile::createObjectFile(
					memory_buffer->getMemBufferRef());
	CHECK (object_file_or_error);

	std::unique_ptr<llvm::object::ObjectFile> object_file = std::move(
		...
		CompilerFunctor::VectorIntrinsics const& available_intrinsics)
{
	...
	const llvm::VecDesc four_wide_vector_functions[] = { { "expf",
			runtime::kExpV4F32, 4 }, { "llvm.exp.f32", runtime::kExpV4F32, 4 },

	{ "logf", runtime::kLogV4F32, 4 },
			{ "llvm.log.f32", runtime::kLogV4F32, 4 },

			{ "tanhf", runtime::kTanhV4F32, 4 }, { "llvm.tanh.f32",
					runtime::kTanhV4F32, 4 }, };

	const llvm::VecDesc eight_wide_vector_functions[] = { { "expf",
			runtime::kExpV8F32, 8 }, { "llvm.exp.f32", runtime::kExpV8F32, 8 },

	{ "logf", runtime::kLogV8F32, 8 },
			{ "llvm.log.f32", runtime::kLogV8F32, 8 },

			{ "tanhf", runtime::kTanhV8F32, 8 }, { "llvm.tanh.f32",
					runtime::kTanhV8F32, 8 }, };
	...
	if (flags->xla_cpu_use_eigen
		...
		if (std::find(features.begin(), features.end(), "+sse4.1")
				!= features.end() && available_intrinsics.sse_intrinsics) {
			vector_functions.insert(vector_functions.end(),
					std::begin(four_wide_vector_functions),
					std::end(four_wide_vector_functions));
		}
		if (std::find(features.begin(), features.end(), "+avx")
				!= features.end() && available_intrinsics.avx_intrinsics) {
			vector_functions.insert(vector_functions.end(),
		...
		llvm::legacy::FunctionPassManager* function_passes) const
{
	...
	auto target_library_info_impl = MakeUnique<llvm::TargetLibraryInfoImpl>(
			target_triple);
	target_library_info_impl->addVectorizableFunctions(
			VectorFunctionsForTargetLibraryInfoImpl(target_triple.getArch(),
					target_machine_->getTargetFeatureString(),
