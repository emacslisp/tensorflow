...
template<>
inline FunctionDefHelper::AttrValueWrapper::AttrValueWrapper(const string& val)
{
	InitFromString(val);
}
...
typedef gtl::ArraySlice<std::pair<string, FunctionDefHelper::AttrValueWrapper>> InstantiateAttrValueSlice;
...
private:
	...
	gtl::FlatMap<string, std::unique_ptr<FunctionDefAndOpRegistration>, HashStr> function_defs_;
...
public:
	...
	virtual Status Instantiate(const string& function_name,
			const InstantiateAttrValueMap& attrs, Handle* handle) = 0;
