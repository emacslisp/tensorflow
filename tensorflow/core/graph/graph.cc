...
Node::Node() :
		id_(-1), cost_id_(-1), class_(NC_UNINITIALIZED), props_(nullptr), assigned_device_name_()
{
}
...
void Node::MaybeCopyOnWrite()
{
	...
	if (!props_->RefCountIsOne()) {
		Properties* new_props = new Properties(props_->op_def_,
				props_->node_def_, props_->input_types_, props_->output_types_);
		props_->Unref();
		props_ = new_props;
	}
}
...
Status Node::input_edge(int idx, const Edge** e) const
{
	if (idx < 0 || idx >= num_inputs()) {
		return errors::InvalidArgument("Invalid input_edge index: ", idx,
				", Node ", name(), " only has ", num_inputs(), " inputs.");
	}
...
Node::Properties::Properties(const OpDef* op_def, const NodeDef& node_def,
		...
		op_def_(op_def), node_def_(node_def), input_types_(inputs.begin(),
				inputs.end()), output_types_(outputs.begin(), outputs.end())
{
}
		...
		*node_def = node->def();
		...
		for (const Edge* edge : node->in_edges()) {
			if (edge->IsControlEdge()) {
				...
				CHECK(inputs[edge->dst_input()] == nullptr) << "Edge "
						<< edge->src()->DebugString() << ":"
						<< edge->dst()->DebugString() << " with dst_input "
						<< edge->dst_input()
						<< " and had pre-existing input edge "
