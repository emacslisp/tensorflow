...
Status CreateSessionFromGraphDef(const SessionOptions& options,
		const GraphDef& graph, std::unique_ptr<Session>* session)
{
...
{
	const string meta_graph_def_path = io::JoinPath(export_dir,
			kMetaGraphDefFilename);
		...
		std::vector<std::pair<string, Tensor>>* inputs)
{
	...
	for (auto& asset : asset_files) {
		...
		inputs->push_back( { asset.tensor_binding().tensor_name(),
				assets_file_tensor });
	}
}
...
string GetVariablesFilename(const StringPiece export_dir)
{
	...
	if (Env::Default()->FileExists(io::JoinPath(export_dir, kVariablesFilename)).ok()
			||
			...
			Env::Default()->FileExists(
					io::JoinPath(export_dir, kVariablesIndexFilename)).ok()) {
...
Status RunRestoreOp(const RunOptions& run_options, const StringPiece export_dir,
		...
		const StringPiece variables_filename_const_op_name, Session* session)
{
		...
		const StringPiece export_dir, SessionBundle* const bundle)
{
	...
	const auto graph_it = bundle->meta_graph_def.collection_def().find(
			kGraphKey);
	if (graph_it != collection_def_map.end()) {
		...
		TF_RETURN_IF_ERROR(
				CreateSessionFromGraphDef(options, graph_def,
						&bundle->session));
	} else {
	...
	if (assets_it != collection_def_map.end()) {
		...
		for (const auto& any_asset : any_assets) {
			...
			TF_RETURN_IF_ERROR(
					ParseAny(any_asset, &asset_file,
							"tensorflow.serving.AssetFile"));
	...
	if (init_op_it != collection_def_map.end()) {
		...
		TF_RETURN_IF_ERROR(
				RunInitOp(run_options, export_dir, asset_files,
...
{
	TF_RETURN_IF_ERROR(
			LoadSessionBundleFromPathUsingRunOptions(options, RunOptions(),
					export_dir, bundle));
	return Status::OK();
}

Status LoadSessionBundleFromPathUsingRunOptions(const SessionOptions& options,
		const RunOptions& run_options, const StringPiece export_dir,
		SessionBundle* const bundle)
{
	...
	load_latency->GetCell(export_dir.ToString())->IncrementBy(
			load_latency_microsecs);
	return status;
}

bool IsPossibleExportDirectory(const StringPiece directory)
{
	const string meta_graph_def_path = io::JoinPath(directory,
			kMetaGraphDefFilename);
