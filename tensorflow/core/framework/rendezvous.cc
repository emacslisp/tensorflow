		...
		const FrameAndIter& frame_iter)
{
	...
	return strings::StrCat(src_device, ";",
			strings::Uint64ToHexString(src_incarnation, buf), ";", dst_device,
			";", name, ";", frame_iter.frame_id, ":", frame_iter.iter_id);
}
			...
			DoneCallback done) override
	{
		...
		if (iter != table_.end()) {
			...
			if (item->has_been_recvd && !tolerate_dup_recv_) {
				mu_.unlock();
				done(errors::Aborted("Duplicated recv: ", key.FullKey()),
						Args(), recv_args, Tensor(), false);
