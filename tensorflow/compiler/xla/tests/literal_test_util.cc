		...
		const Shape& actual)
{
	...
	for (int i = 0; i < expected.dimensions_size(); ++i) {
		ASSERT_EQ(expected.dimensions(i), actual.dimensions(i))
				<< "mismatch in dimension #" << i << " expected: "
				<< ShapeUtil::HumanString(expected) << " actual: "
	...
	if (ulhs != urhs) {
		return testing::AssertionFailure()
				<< tensorflow::strings::Printf(
						...
						lhs,
						tensorflow::strings::StrCat(
								tensorflow::strings::Hex(urhs)).c_str(), rhs,
						rhs);
	}
...
bool ExpectLiteralsEqual(const Literal& expected, const Literal& actual,
		tensorflow::gtl::MutableArraySlice<int64> multi_index, int64 dimension)
{
	if (dimension == expected.shape().dimensions_size()) {
		NativeT expected_value = LiteralUtil::Get<NativeT>(expected,
				multi_index);
		NativeT actual_value = LiteralUtil::Get<NativeT>(actual, multi_index);
		testing::AssertionResult result = CompareEqual<NativeT>(expected_value,
				actual_value);
	...
	for (int64 i = 0; i < expected.shape().dimensions(dimension); ++i) {
		...
		all_match = all_match
				&& ExpectLiteralsEqual<NativeT>(expected, actual, multi_index,
						dimension + 1);
	}
	return all_match;
}
		...
		const Literal& expected, const Literal& actual)
{
	...
	case TUPLE: {
		...
		for (int i = 0; i < actual.tuple_literals_size(); ++i) {
			auto result = Equal(expected.tuple_literals(i),
					actual.tuple_literals(i));
...
public:
	explicit NearComparator(ErrorSpec error) :
			error_(error)
	{
	}
	...
	bool ExpectNear(const Literal& expected, const Literal& actual)
	{
		...
		max_rel_err_ = 0.0;
		max_abs_err_ = 0.0;
		*miscompares_.mutable_shape() = ShapeUtil::ChangeElementType(
				actual.shape(), PRED);
...
private:
	...
	bool ExpectValuesNear(NativeT expected, NativeT actual)
	{
		...
		VLOG(10)
				<< tensorflow::strings::Printf(
						...
						LiteralTestUtil::MultiIndexAsString(multi_index_).c_str(),
						abs_diff, rel_err);
		bool nan_mismatch = std::isnan(actual) != std::isnan(expected);
		bool mismatch = (nan_mismatch
				|| (abs_diff >= error_.abs && rel_err >= error_.rel));
		if (mismatch) {
			...
			if (num_miscompares_ < kMaxFailures) {
				EXPECT_NEAR(expected, actual, error_.abs)
						<< "mismatch at index "
						<< LiteralTestUtil::MultiIndexAsString(multi_index_)
						<< " abs diff " << abs_diff << " rel err " << rel_err
						<< " failure #" << num_miscompares_;
	...
	{
		if (dimension == expected.shape().dimensions_size()) {
			bool near = ExpectValuesNear(
					LiteralUtil::Get<NativeT>(expected, multi_index_),
	...
	void WriteLiteralToTempFile(const Literal& literal, const string& name)
	{
		...
		string filename = tensorflow::io::JoinPath(
				...
				tensorflow::strings::Printf("tempfile-%s-%llx-%s",
						Hostname().c_str(), now_usec, name.c_str()));
		TF_CHECK_OK(
		...
		const Literal& expected, const Literal& actual, const ErrorSpec& error)
{
	...
	if (!ShapeUtil::IsTuple(expected.shape())
		...
		return testing::AssertionFailure()
				...
				<< expected.shape().ShortDebugString() << " actual shape = "
				<< actual.shape().ShortDebugString();
	}
	AssertEqualShapes(expected.shape(), actual.shape());
	for (uint64 i = 0; i < expected.tuple_literals_size(); ++i) {
		const auto& expected_element = expected.tuple_literals(i);
		...
		const Literal& literal)
{
	...
	*new_literal->mutable_shape() = ShapeUtil::MakeShape(
	...
	*shape_with_layout.mutable_layout() = LayoutUtil::MakeLayout(
			minor_to_major);
	...
	for (int64 i = 0; i < ShapeUtil::ElementsIn(literal.shape()); ++i) {
		std::vector<int64> from_multi_index =
				IndexUtil::LinearIndexToMultidimensionalIndex(literal.shape(),
						i);
		std::vector<int64> to_multi_index =
				IndexUtil::LinearIndexToMultidimensionalIndex(shape_with_layout,
						i);
		switch (literal.shape().element_type()) {
		case PRED:
			LiteralUtil::Set<bool>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<bool>(literal, from_multi_index));
			break;
		case U8:
			LiteralUtil::Set<uint8>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<uint8>(literal, from_multi_index));
			break;
		case U32:
			LiteralUtil::Set<uint32>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<uint32>(literal, from_multi_index));
			break;
		case S32:
			LiteralUtil::Set<int32>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<int32>(literal, from_multi_index));
			break;
		case U64:
			LiteralUtil::Set<uint64>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<uint64>(literal, from_multi_index));
			break;
		case S64:
			LiteralUtil::Set<int64>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<int64>(literal, from_multi_index));
			break;
		case F32:
			LiteralUtil::Set<float>(new_literal.get(), to_multi_index,
					LiteralUtil::Get<float>(literal, from_multi_index));
			break;
		case F64:
			LiteralUtil::Set<double>(new_literal.get(), to_multi_index,
