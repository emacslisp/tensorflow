...
protected:
	void TestRequantizeMany(Eigen::ThreadPoolDevice* eigen_device,
			...
			float output_max, const std::vector<qint32>& values_quantized,
			int tolerance = 1)
	{
		...
		for (int value_index = 0; value_index < values_count; ++value_index) {
			expected_values.push_back(
					FloatToQuantized<quint8>(
							QuantizedToFloat(values_quantized[value_index],
									input_min, input_max), output_min,
							output_max));
		}

		Tensor i_tensor = tensorflow::test::AsTensor(
				gtl::ArraySlice<qint32>(values_quantized));
		...
		if (eigen_device == nullptr) {
			...
			RequantizeManyInNewRangeUsingEigen<qint32, quint8>(*eigen_device,
					i_tensor, input_min, input_max, output_min, output_max,
					&o_tensor);
		}

		const string tolerance_str = strings::StrCat("+-", tolerance);
		for (size_t value_index = 0; value_index < values_count;
				++value_index) {
			int e = expected_values[value_index];
			int v = output_values(value_index);
			ASSERT_TRUE(std::abs(e - v) <= tolerance) << "actual=" << v
					<< ", expected=" << e << tolerance_str
					<< ", values_quantized[" << value_index << "]="
					<< values_quantized[value_index] << ", input_min="
					<< input_min << ", input_max=" << input_max
					<< ", output_min=" << output_min << ", output_max="
			...
			Eigen::ThreadPoolDevice* eigen_device)
	{
		...
		const float values[values_count] = { 0.0f, 0.45f, 1.0f, -1.0f, 127.0f,
				255.0f };
		// These are the input and output ranges we'll test.
		const size_t ranges_count = 6;
		const float ranges[ranges_count][4] = { { 0.0f, 255.0f, 0.0f, 255.0f }, //
				{ 0.0f, 1.0f, 0.0f, 1.0f },        //
				{ -1.0f, 1.0f, -1.0f, 1.0f },      //
		...
		for (int i = 0; i < ranges_count; ++i) {
			...
			for (int value_index = 0; value_index < values_count;
					++value_index) {
				const float v = values[value_index];
				values_quantized.push_back(
						FloatToQuantized<qint32>(v, r[0], r[1]));
			}
		...
		TestRequantizeMany(eigen_device, -255.0f, 255.0f, -255.0f, 255.0f,
				vals);
		...
		for (int32 i = low; i <= high; ++i)
			vals.push_back(i);
		TestRequantizeMany(eigen_device, in_min, in_max, out_min, out_max,
				vals);
	}
	...
	{
		thread::ThreadPool threadpool(Env::Default(), "test",
				2 /* num_threads */);
		...
		// Random values.
		for (size_t range_index = 0; range_index < ranges_count;
				++range_index) {
			...
			for (int i = 0; i < values_count; ++i) {
				float v = (rnd.RandFloat() * (input_max - input_min))
						+ input_min;
				values_quantized.push_back(
			...
			Tensor o_tensor_eigen(DataTypeToEnum<OutputType>::v(), TensorShape {
					values_count });
			auto output_values_eigen = o_tensor_eigen.flat<OutputType>();
			Tensor o_tensor_ref(DataTypeToEnum<OutputType>::v(), TensorShape {
					values_count });
			...
			for (int i = 0; i < values_quantized.size(); ++i) {
				...
				ASSERT_TRUE(std::abs(expected - actual) <= tolerance)
						...
						<< values_quantized[i] << " i=" << i << " input_min="
						<< input_min << " input_max=" << input_max
						<< " input_type="
						<< DataTypeString(DataTypeToEnum<InputType>::v())
						<< " output_type="
			...
			Eigen::ThreadPoolDevice* eigen_device)
	{
		...
		FPair(-31.0f, 13.0f) }) {
			...
			FloatTensorToQuantizedInPlaceUsingEigen<T>(*eigen_device, input,
					f_min, f_max, &output);
			auto output_array = output.flat<T>();

			const int tolerance = 1;
			for (int i = 0; i < values_count; ++i) {
				int32 expected = FloatToQuantized<T>(input_array(i), f_min,
						f_max);
			...
			Eigen::ThreadPoolDevice* eigen_device)
	{
		...
		typedef std::pair<float, float> FPair;
		for (FPair min_and_max : std::vector<FPair> { FPair(-255.0f, 255.0f),
				FPair(-1.0f, 1.0f), FPair(-1.0f, 255.0f), FPair(0.0f, 1e6),
				FPair(0.0f, 1.0f), FPair(-31.0f, 13.0f), FPair(-5.89505e+08,
						5.89505e+08), }) {
			const float f_min = min_and_max.first;
			const float f_max = min_and_max.second;
			...
			const double q_range = static_cast<double>(Eigen::NumTraits < T
					> ::highest()) - Eigen::NumTraits < T > ::lowest();
			for (int i = 0; i < values_count; ++i) {
				if (sizeof(T) == 1) {
					input_array(i) = Eigen::NumTraits < T > ::lowest() + i;
				} else {
					int64 offset =
							static_cast<int64>(q_range / values_count * i);
					input_array(i) = static_cast<int32>(std::min<int64>(
							Eigen::NumTraits < T > ::lowest() + offset,
							Eigen::NumTraits < T > ::highest()));
				}
			}

			Tensor output(DT_FLOAT, TensorShape { values_count });
			QuantizedTensorToFloatInPlaceUsingEigen<T>(*eigen_device, input,
					f_min, f_max, &output);
			auto output_array = output.flat<float>();
			const double range = static_cast<double>(f_max) - f_min;
			for (int i = 0; i < values_count; ++i) {
				float expected = QuantizedToFloat<T>(input_array(i), f_min,
						f_max);
				float actual = output_array(i);
				ASSERT_NEAR(expected, actual, range * 1.1e-7) << "expected="
						<< expected << " actual=" << actual << " v="
						<< input_array(i) << " i=" << i << " f_min=" << f_min
						<< " f_max=" << f_max << " type="
						<< DataTypeString(DataTypeToEnum<T>::v());
			}
		}
