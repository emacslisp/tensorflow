...
QueueRunnerDef BuildQueueRunnerDef(const std::string& queue_name,
		const std::vector<std::string>& enqueue_ops,
...
std::unique_ptr<Session> BuildSessionAndInitVariable(const GraphDef& graph_def)
{
...
GraphDef BuildDoubleQueueGraph()
{
	...
	auto dequeue0 = QueueDequeue(root.WithOpName(kDequeueOp0), q0, {
			DataType::DT_INT32 });
	auto enqueue1 = QueueEnqueue(root.WithOpName(kEnqueueOp1), q1,
			{ dequeue0[0] });
	auto dequeue1 = QueueDequeue(root.WithOpName(kDequeueOp1), q1, {
			DataType::DT_INT32 });
