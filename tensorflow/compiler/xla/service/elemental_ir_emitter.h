...
public:
	...
	ElementalIrEmitter(const HloModuleConfig& hlo_module_config,
			...
			ir_builder_(ir_builder), module_(module), hlo_module_config_(
					hlo_module_config)
	{
	}
...
protected:
	...
	llvm_ir::IrArray::Index ElementwiseSourceIndex(
			...
			const HloInstruction& hlo, int64 operand_no) const;
...
private:
	...
	llvm_ir::ElementGenerator MakeRngElementGenerator(const HloInstruction* hlo,
			const HloToElementGeneratorMap& operand_to_generator) const;
};
