...
public:
	explicit SummaryScalarOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
...
public:
	...
	explicit SummaryHistoOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
			...
			if (Eigen::numext::isnan(double_val)) {
				c->SetStatus(
						errors::InvalidArgument(
								"Nan in summary histogram for: ", name()));
				break;
...
public:
	explicit SummaryMergeOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
			...
			for (int i = 0; i < in_vec.dimension(0); i++) {
				...
				for (int v = 0; v < summary_in.value_size(); v++) {
					...
					if ((tag != "") && !tags.insert(tag).second) {
						c->SetStatus(
								errors::InvalidArgument(
										strings::StrCat("Duplicate tag ", tag,
