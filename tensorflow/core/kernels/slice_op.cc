...
static void SharedValidation(OpKernelContext* context,
		TensorShape* output_shape, bool* is_identity, bool* slice_dim0,
		gtl::InlinedVector<int64, 4>* begin, gtl::InlinedVector<int64, 4>* size)
{
	...
	*slice_dim0 = true;
	for (int i = 0; i < input_dims; ++i) {
		...
		if (input.dim_size(i) == 0) {
			OP_REQUIRES(context, b == 0 && s == 0,
					errors::InvalidArgument("Expected begin[", i,
							"] == 0 (got ", b, ") and size[", i, "] == 0 ",
							"(got ", s, ") when ", "input.dim_size(", i,
							") == 0"));
		} else {
...
public:
	explicit SliceOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		SharedValidation(context, &output_shape, &is_identity, &slice_dim0,
				&begin, &size);
		...
		if (output_shape.num_elements() > 0) {
			if (std::is_same<Device, CPUDevice>::value && input_dims == 2
				...
				for (int i = 0; i < size[0]; ++i) {
					...
					memcpy(&output(i, 0), &input(row, begin[1]),
							size[1] * sizeof(T));
				}
				return;
			}
