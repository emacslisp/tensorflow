...
private:
	...
	static tensorflow::Status ForEachHelperMutable(ShapeIndex* index,
			ShapeTree<T>* shape_tree, MutableVisitorFunction func);
	static tensorflow::Status ForEachHelper(ShapeIndex* index,
			const ShapeTree<T>& shape_tree, VisitorFunction func);
...
template<typename T>
ShapeTree<T>::ShapeTree(const Shape& shape) :
...
/* static */
template<typename T>
tensorflow::Status ShapeTree<T>::ForEachHelperMutable(ShapeIndex* index,
		ShapeTree<T>* shape_tree, ShapeTree<T>::MutableVisitorFunction func)
{
	TF_RETURN_IF_ERROR(
	...
	for (int i = 0; i < shape_tree->elements_.size(); ++i) {
		index->push_back(i);
		TF_RETURN_IF_ERROR(
				ForEachHelperMutable(index, shape_tree->elements_[i].get(),
						func));
		index->pop_back();
	}
...
/* static */
template<typename T>
tensorflow::Status ShapeTree<T>::ForEachHelper(ShapeIndex* index,
		const ShapeTree<T>& shape_tree, ShapeTree<T>::VisitorFunction func)
{
	TF_RETURN_IF_ERROR(
			func(*index, shape_tree.elements_.empty(), shape_tree.data_));
	for (int i = 0; i < shape_tree.elements_.size(); ++i) {
		index->push_back(i);
		TF_RETURN_IF_ERROR(
				ForEachHelper(index, *shape_tree.elements_[i], func));
		index->pop_back();
	}
