...
public:
	explicit ImageData(const int width, const int height) :
			uv_frame_width_(width << 1), uv_frame_height_(height << 1), timestamp_(
					0), image_(width, height)
	{
...
public:
	...
	void SetData(const uint8* const new_frame, const uint8* const uv_frame,
			const int stride, const int64 timestamp,
			const int downsample_factor)
	{
		...
		if (uv_frame != NULL) {
			if (u_data_.get() == NULL) {
				u_data_.reset(
						new Image<uint8>(uv_frame_width_, uv_frame_height_));
				v_data_.reset(
						new Image<uint8>(uv_frame_width_, uv_frame_height_));
			}
		...
		if (!pyramid_sqrt2_computed_[level]) {
			...
			if (level == 1) {
				...
				if (pyramid_sqrt2_[level] == NULL) {
					const int new_width =
							(static_cast<int>(upper_level.GetWidth() / sqrtf(2))
									+ 1) / 2 * 2;
					const int new_height =
							(static_cast<int>(upper_level.GetHeight() / sqrtf(2))
									+ 1) / 2 * 2;

					pyramid_sqrt2_[level] = new Image<uint8>(new_width,
							new_height);
				}
				pyramid_sqrt2_[level]->DownsampleInterpolateLinear(upper_level);
			} else {
				const Image<uint8>& upper_level = *GetPyramidSqrt2Level(
						level - 2);
				if (pyramid_sqrt2_[level] == NULL) {
					pyramid_sqrt2_[level] = new Image<uint8>(
							upper_level.GetWidth() / 2,
							upper_level.GetHeight() / 2);
				}
	...
	inline const Image<int32>* GetSpatialX(const int level) const
	{
		if (!spatial_x_computed_[level]) {
			...
			if (spatial_x_[level] == NULL) {
				spatial_x_[level] = new Image<int32>(src.GetWidth(),
						src.GetHeight());
			}
	...
	inline const Image<int32>* GetSpatialY(const int level) const
	{
		if (!spatial_y_computed_[level]) {
			...
			if (spatial_y_[level] == NULL) {
				spatial_y_[level] = new Image<int32>(src.GetWidth(),
						src.GetHeight());
			}
