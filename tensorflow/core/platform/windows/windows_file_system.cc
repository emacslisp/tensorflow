...
{
	return IOError(
			context + string(" : ") + internal::GetWindowsErrorMessage(err),
			err);
}
...
public:
	WinReadOnlyMemoryRegion(const std::string& filename, HANDLE hfile,
			...
			filename_(filename), hfile_(hfile), hmap_(hmap), address_(address), length_(
					length)
	{
	}
...
Status WindowsFileSystem::NewRandomAccessFile(const string& fname,
		std::unique_ptr<RandomAccessFile>* result)
{
	...
	HANDLE hfile = ::CreateFileA(translated_fname.c_str(), GENERIC_READ,
			share_mode, NULL, OPEN_EXISTING, file_flags, NULL);
		...
		std::unique_ptr<WritableFile>* result)
{
	...
	HANDLE hfile = ::CreateFileA(translated_fname.c_str(), GENERIC_WRITE,
			share_mode, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
Status WindowsFileSystem::NewAppendableFile(const string& fname,
		std::unique_ptr<WritableFile>* result)
{
	string translated_fname = TranslateName(fname);
	result->reset();
	...
	DWORD share_mode = FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE;
	HANDLE hfile = ::CreateFileA(translated_fname.c_str(), GENERIC_WRITE,
			share_mode, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
Status WindowsFileSystem::NewReadOnlyMemoryRegionFromFile(const string& fname,
		std::unique_ptr<ReadOnlyMemoryRegion>* result)
{
	...
	if (INVALID_HANDLE_VALUE == hfile) {
		return IOErrorFromWindowsError(
				"NewReadOnlyMemoryRegionFromFile failed to Create/Open: "
						+ fname, ::GetLastError());
	}
	...
	if (s.ok()) {
		...
		if (file_size == 0) {
			return IOError(
					"NewReadOnlyMemoryRegionFromFile failed to map empty file: "
							+ fname,
					EINVAL);
		}

		HANDLE hmap = ::CreateFileMappingA(hfile, NULL, PAGE_READONLY, 0, // Whole file at its present length
				0, NULL);  // Mapping name
		...
		const void* mapped_region = ::MapViewOfFileEx(hmap, FILE_MAP_READ, 0, // High DWORD of access start
				0,  // Low DWORD
				file_size, NULL);  // Let the OS choose the mapping
		...
		result->reset(
				new WinReadOnlyMemoryRegion(fname, hfile, hmap, mapped_region,
						file_size));
...
Status WindowsFileSystem::GetFileSize(const string& fname, uint64* size)
{
	...
	if (TRUE
			== ::GetFileAttributesExA(translated_fname.c_str(),
...
Status WindowsFileSystem::RenameFile(const string& src, const string& target)
{
	...
	if (!::MoveFileExA(TranslateName(src).c_str(),
			TranslateName(target).c_str(), MOVEFILE_REPLACE_EXISTING)) {
		string context(
		...
		std::vector<string>* results)
{
	...
	TF_RETURN_IF_ERROR(
			FileSystem::GetMatchingPaths(converted_pattern, results));
