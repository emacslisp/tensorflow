...
Status ConvBackpropExtractAndVerifyDimension(StringPiece label,
		const TensorShape& input_shape, const TensorShape& filter_shape,
		const TensorShape& output_shape, const std::vector<int32>& strides,
		...
		ConvBackpropSpatialDimension* dim)
{
	...
	TF_RETURN_IF_ERROR(
			GetWindowedOutputSize(dim->input_size, dim->filter_size,
					dim->stride, padding, &out_size, &pad_size));
	...
	dim->pad_after = padded_out_size - dim->expanded_output_size
			- dim->pad_before;
	VLOG(2) << label << ": expanded_out = " << dim->expanded_output_size
			<< ", filter = " << dim->filter_size << ", padded_out = "
			<< padded_out_size << ", pad_before = " << dim->pad_before
			<< ", pad_after = " << dim->pad_after << ", strides = "
			<< dim->stride;
	return Status::OK();
}

Status Conv2DBackpropComputeDimensions(StringPiece label,
		const TensorShape& input_shape, const TensorShape& filter_shape,
		...
		TensorFormat data_format, Conv2DBackpropDimensions* dims)
{
	...
	TF_RETURN_IF_ERROR(
			ConvBackpropExtractAndVerifyDimension(label, input_shape,
					filter_shape, out_backprop_shape, strides, padding, row_dim,
					filter_row_dim, &dims->rows));
	TF_RETURN_IF_ERROR(
			ConvBackpropExtractAndVerifyDimension(label, input_shape,
					filter_shape, out_backprop_shape, strides, padding, col_dim,
					filter_col_dim, &dims->cols));
	return Status::OK();
}
