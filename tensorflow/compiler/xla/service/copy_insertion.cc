...
public:
	...
	const std::vector<HloInstruction*>& copy_users() const
	{
		return copy_users_;
	}
...
InstructionCopier::InstructionCopier(const bool init_value,
		HloInstruction* instruction,
		const std::vector<HloInstruction*>& copy_users) :
		instruction_(instruction), copy_users_(copy_users), indices_to_copy_(
				instruction->shape(), init_value)
{
}
...
tensorflow::Status InstructionCopier::RecordIndicesWhichPointToParamOrConstant(
		const TuplePointsToAnalysis& points_to_analysis)
{
	const PointsToSet& points_to = points_to_analysis.GetPointsToSet(
			instruction_);
...
{
	TF_RETURN_IF_ERROR(
			RecordAmbiguousOrNonDistinctIndices(
					liveness->points_to_analysis()));
	TF_RETURN_IF_ERROR(
			RecordIndicesWhichInterfereWithOtherInstruction(liveness,
					other_instruction));
	return tensorflow::Status::OK();
}
		...
		const TuplePointsToAnalysis& points_to_analysis)
{
	...
	std::unordered_map<const LogicalBuffer*, std::vector<ShapeIndex>> buffer_to_source_indices;
	TF_RETURN_IF_ERROR(
			points_to.ForEachElement([this, &buffer_to_source_indices](
					const ShapeIndex& index, bool /*is_leaf*/,
					const std::vector<const LogicalBuffer*>& buffers) {
				if (buffers.size() > 1) {
	...
	for (auto& buff_to_src : buffer_to_source_indices) {
		...
		for (auto& src_index : buff_to_src.second) {
			...
			if (!indices_to_copy_.element(src_index)) {
				VLOG(2) << "Adding copy of buffer for instruction: "
						<< instruction_->name() << " at index: "
						<< tensorflow::str_util::Join(src_index, ",")
...
tensorflow::Status InstructionCopier::RecordIndicesWhichInterfereWithOtherInstruction(
		BufferLiveness* liveness, HloInstruction* other_instruction)
{
	// Record all buffer indices for 'instruction_', which interfere with
	// 'other_instruction' at the same index.
	TF_RETURN_IF_ERROR(
			ShapeUtil::ForEachSubshape(instruction_->shape(),
		...
		ShapeIndex* index)
{
	...
	const int64 num_tuple_elements = ShapeUtil::TupleElementCount(
			instruction->shape());
		...
		if (ShapeUtil::IsTuple(gte->shape())) {
			...
			if (indices_to_copy_.element(*index)) {
				element_copy = gte->parent()->AddInstruction(
						HloInstruction::CreateUnary(gte->shape(),
								HloOpcode::kCopy, gte));
			} else {
				element_copy = gte;
			}
...
HloInstruction* InstructionCopier::Copy()
{
	...
	if (ShapeUtil::IsTuple(instruction_->shape())) {
		...
		copy = instruction_->parent()->AddInstruction(
				HloInstruction::CreateUnary(instruction_->shape(),
						HloOpcode::kCopy, instruction_));
	}
...
StatusOr<HloInstruction*> CopyInsertion::FindOrInsertCopy(HloInstruction* hlo)
{
	...
	if (copy_it == inserted_copies_.end()) {
		HloInstruction* copy =
				hlo->parent()->DeepCopyInstruction(hlo).ValueOrDie();
...
StatusOr<bool> CopyInsertion::Run(HloModule* module)
{
	...
	std::unordered_map<const HloComputation*, std::vector<HloInstruction*>> while_instructions;
	for (auto& computation : module->computations()) {
		for (auto& instruction : computation->instructions()) {
			...
			while_instructions[computation.get()].emplace_back(
					instruction.get());
		}
	}
	...
	for (auto& computation : module->computations()) {
		...
		if (while_body_computations.count(computation.get()) > 0) {
			...
			TF_RETURN_IF_ERROR(
					root_copier.RecordIndicesToCopyForColocatingBuffers(
							liveness.get(),
							computation->parameter_instruction(0)));
		} else {
			// Record root indices to copy for general computations.
			TF_RETURN_IF_ERROR(
					root_copier.RecordIndicesWhichPointToParamOrConstant(
							liveness->points_to_analysis()));
		}
