...
struct FillPhiloxRandomTask<Distribution, true> {
			...
			int64 start_group, int64 limit_group, Distribution dist)
	{
		...
		static const int kGeneratorSkipPerOutputGroup = kGroupSize
		...
		int index, Tensor** output)
{
	...
	if (shape.dtype() == DataType::DT_INT32) {
		...
		TF_RETURN_IF_ERROR(
				TensorShapeUtils::MakeShape(vec.data(), vec.size(),
						&tensor_shape));
...
public:
	typedef typename Distribution::ResultElementType T;
	explicit PhiloxRandomOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		functor::FillPhiloxRandom<Device, Distribution>()(ctx,
...
public:
	explicit RandomUniformIntOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, generator_.Init(ctx));
	}
...
public:
	explicit RandomGammaOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
...
#undef UNIFORM
		...
		static const int kElementCost = 85 + 2 * Normal::kElementCost
				+ Uniform::kElementCost + 3 * PhiloxRandom::kElementCost;
