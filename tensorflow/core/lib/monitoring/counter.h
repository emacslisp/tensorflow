...
private:
	explicit Counter(
			...
			metric_def_(metric_def), registration_handle_(
	...
	using LabelArray = std::array<string, NumLabels>;std::map<LabelArray, CounterCell> cells_ GUARDED_BY(mu_);
...
template<int NumLabels>
template<typename ... MetricDefArgs>
Counter<NumLabels>* Counter<NumLabels>::New(MetricDefArgs&&... metric_def_args)
{
