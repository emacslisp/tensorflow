...
template<typename CTCBeamState = ctc_beam_search::EmptyBeamState,
		typename CTCBeamComparer = ctc_beam_search::BeamComparer<CTCBeamState>>
...
public:
	...
	CTCBeamSearchDecoder(int num_classes, int beam_width,
			...
			CTCDecoder(num_classes, batch_size, merge_repeated), beam_width_(
					beam_width), leaves_(beam_width), beam_scorer_(
					CHECK_NOTNULL(scorer))
	{
		Reset();
	}
		...
		std::vector<CTCDecoder::Output>* output, ScoreOutput* scores)
{
	...
	for (int b = 0; b < batch_size_; ++b) {
		...
		for (int i = 0; i < branches->size(); ++i) {
			...
			entry->newp.total += beam_scorer_->GetStateEndExpansionScore(
					entry->state);
			leaves_.push(entry);
		}
		...
		const Vector& raw_input)
{
	...
	if (label_selection_margin_ >= 0) {
		...
		label_selection_input_min = std::max(label_selection_input_min,
	...
	for (BeamEntry* b : *branches) {
		if (b->parent != nullptr) {  // if not the root
			if (b->parent->Active()) {
				...
				//                          + P(l=ab @ t=5))
				float previous =
						(b->label == b->parent->label) ?
								b->parent->oldp.blank : b->parent->oldp.total;
				b->newp.label = LogSumExp(b->newp.label,
						beam_scorer_->GetStateExpansionScore(b->state,
								previous));
			}
	...
	for (BeamEntry* b : *branches) {
		...
		for (BeamEntry& c : *b->Children()) {
			if (!c.Active()) {
				...
				beam_scorer_->ExpandState(b->state, b->label, &c.state,
						c.label);
				float previous =
						(c.label == b->label) ? b->oldp.blank : b->oldp.total;
				c.newp.label = input(c.label)
						+ beam_scorer_->GetStateExpansionScore(c.state,
								previous);
...
template<typename CTCBeamState, typename CTCBeamComparer>
void CTCBeamSearchDecoder<CTCBeamState, CTCBeamComparer>::TopPaths(int n,
		std::vector<std::vector<int>>* paths, std::vector<float>* log_probs,
		bool merge_repeated) const
{
	...
	for (auto it = leaves_.unsorted_begin(); it != leaves_.unsorted_end();
			++it) {
