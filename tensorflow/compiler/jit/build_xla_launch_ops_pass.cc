...
static Status BuildLaunchNode(const string& nodename,
		const string& function_name, const AttrValueMap& function_attr,
		const string& device_name, const DataTypeVector& constant_dtypes,
...
static Status ReplaceNodeWithXlaLaunch(Graph* graph, Node* node)
{
	...
	TF_RETURN_IF_ERROR(
			GetNodeAttr(node->def(), kXlaNumConstantArgsAttr,
					&num_constant_args));

	if (num_constant_args < 0
			|| num_constant_args > node->input_types().size()) {
		...
		std::unique_ptr<OpKernel>* kernel)
{
	...
	if (!flr->GetFunctionLibraryDefinition()->GetAttr(ndef, kXlaCompileAttr,
			&xla_compile).ok() || !xla_compile) {
		// Not marked as _XlaCompile=true.
		return errors::InvalidArgument("No ", kXlaCompileAttr, " for ",
				ndef.op());
	}
	...
	*(func.mutable_attr()) = ndef.attr();
	...
	MemoryTypeVector output_memory_types(fbody->ret_types.size(),
			DEVICE_MEMORY);
	...
	OpKernelConstruction construction(DeviceType(dev->device_type()), dev,
			...
			fbody->ret_types, output_memory_types, flr->graph_def_version(),
			&s);
