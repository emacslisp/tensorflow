...
Status GetEnvironmentVariableFileName(string* filename)
{
	...
	if (!result || !IsFile(result)) {
		return errors::NotFound(
				strings::StrCat("$", kGoogleApplicationCredentials,
...
GoogleAuthProvider::GoogleAuthProvider(
		...
		oauth_client_(std::move(oauth_client)), http_request_factory_(
				std::move(http_request_factory)), env_(env), initial_retry_delay_usec_(
...
Status GoogleAuthProvider::GetTokenFromFiles()
{
	...
	if (json.isMember("refresh_token")) {
		TF_RETURN_IF_ERROR(
				oauth_client_->GetTokenFromRefreshTokenJson(json, kOAuthV3Url,
						&current_token_, &expiration_timestamp_sec_));
	} else if (json.isMember("private_key")) {
		TF_RETURN_IF_ERROR(
				oauth_client_->GetTokenFromServiceAccountJson(json, kOAuthV4Url,
						kOAuthScope, &current_token_,
