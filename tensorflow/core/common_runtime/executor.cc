...
private:
	...
	{
		return reinterpret_cast<AllocatorAttributes*>(var()
...
private:
	...
	struct FrameInfo {
		FrameInfo() :
				input_count(0), total_inputs(0), pending_counts(nullptr), nodes(
						nullptr)
		{
		}
...
void GraphView::InitializeNode(const Node* n)
{
	...
	// both be zero.
	const size_t bytes = sizeof(NodeItem)                             // Fixed
	...
	NodeItem* item = reinterpret_cast<NodeItem*>(arena_.AllocAligned(bytes,
			sizeof(NodeItem*)));
	new (item) NodeItem();
	nodes_[id] = item;
...
Status ExecutorImpl::Initialize()
{
	...
	for (const Node* n : graph_->nodes()) {
		...
		item->is_enter_exit_or_next_iter = (IsEnter(n) || IsExit(n)
				|| IsNextIteration(n));
		...
		item->pending_id = frame_info->pending_counts_layout.CreateHandle(
				max_pending, max_dead);
		...
		if (IsEnter(n)) {
			string enter_name;
			TF_RETURN_IF_ERROR(
					GetNodeAttr(n->def(), "frame_name", &enter_name));
		...
		AllocatorAttributes* attr)
{
	...
	if (IsRecv(n)) {
		...
		if (!DeviceNameUtils::ParseFullName(src_name, &parsed_src_name)) {
			s = errors::Internal("Bad send_device attr '", src_name,
					"' in node ", n->name());
			return s;
		}
	...
	if (IsSend(dst)) {
		...
		if (!DeviceNameUtils::ParseFullName(dst_name, &parsed_dst_name)) {
			s = errors::Internal("Bad recv_device attr '", dst_name,
					"' in node ", n->name());
			return s;
		}
		if (!DeviceNameUtils::IsSameAddressSpace(parsed_dst_name,
			...
			attr->set_gpu_compatible(true);
			VLOG(2) << "node " << n->name()
					<< " is the source of a cpu->gpu copy";
		} else {
			VLOG(2) << "default alloc case local type " << local_dev_name.type
...
private:
	...
	struct FrameState {
		...
		void ActivateNexts(const GraphView* gview, int64 iter,
				TaggedNodeSeq* ready)
		EXCLUSIVE_LOCKS_REQUIRED (mu);
	...
	Entry* GetInputTensors(FrameState* input_frame, int64 input_iter) const
...
ExecutorState::ExecutorState(const Executor::Args& args, ExecutorImpl* impl) :
		vlog_(VLOG_IS_ON(1)), log_memory_(LogMemory::IsEnabled()), step_id_(
				args.step_id), rendezvous_(args.rendezvous), session_state_(
				args.session_state), tensor_store_(args.tensor_store), step_container_(
				args.step_container), stats_collector_(args.stats_collector), slice_reader_cache_(
				new checkpoint::TensorSliceReaderCacheWrapper), call_frame_(
				args.call_frame), impl_(impl), cancellation_manager_(
				args.cancellation_manager), runner_(args.runner), sync_on_finish_(
				args.sync_on_finish), num_outstanding_ops_(0)
{
	...
	root_frame_->iterations[0] = new IterationState(root_frame_->pending_counts,
...
struct ExecutorState::AsyncState {
	AsyncState(const OpKernelContext::Params& p, const TaggedNode& _tagged_node,
			...
			saved_inputs(*p.inputs), saved_input_device_contexts(
					*p.input_device_contexts), saved_input_alloc_attrs(
					*p.input_alloc_attrs), params(p), tagged_node(_tagged_node), item(
					_item), first_input(_first_input),
			// ParamsButClearingEigenGPUDevice does equivalent of
			//   params.eigen_gpu_device = nullptr;
			ctx(ParamsButClearingEigenGPUDevice(&params), item->num_outputs), stats(
					_stats)
	{
...
void ExecutorState::Process(TaggedNode tagged_node, int64 scheduled_usec)
{
	...
	while (!inline_ready.empty()) {
		...
		if (vlog_ && VLOG_IS_ON(1)) {
			...
			input_frame->GetIteration(input_iter)->mark_started(
					item.pending_id);
		}
		...
		if (tagged_node.is_dead && !IsTransferNode(node)) {
			...
			s = PrepareInputs(item, first_input, &inputs,
					&input_device_contexts, &input_alloc_attrs, &is_input_dead);
			if (!s.ok()) {
				// Clear inputs.
			...
			if (item.kernel_is_async) {
				...
				AsyncState* state = new AsyncState(params, tagged_node, &item,
						first_input, stats);
	...
	*is_input_dead = false;
	...
	for (int i = 0; i < item.num_inputs; ++i) {
		...
		if (entry->ref == nullptr) {
			if (expect_ref) {
				return AttachDef(
						errors::InvalidArgument(i,
								"-th input expects a ref type"),
						item.kernel->def());
			}
			inp->tensor = entry->val.get();
		} else {
			if (!entry->ref->IsInitialized()
					&& !IsInitializationOp(item.node)) {
				return AttachDef(
	...
	for (int i = 0; i < item.num_outputs; ++i) {
		...
		if (*ctx->is_output_dead() || val.tensor == nullptr) {
			...
			if (!IsSwitch(node) && !IsRecv(node)) {
				s.Update(
						errors::Internal("Missing ", i, "-th output from ",
								SummarizeNodeDef(node->def())));
			}
		} else {
			Entry* out = &((*outputs)[i]);
			...
			if (dtype == item.output_type(i)) {
				...
				if (val.is_ref()) {
					...
					if (impl_->params_.node_outputs_cb != nullptr) {
						impl_->params_.node_outputs_cb(item.node->name(), i,
								out->ref, true, ctx);
					}
				} else {
					...
					if (impl_->params_.node_outputs_cb != nullptr) {
						impl_->params_.node_outputs_cb(item.node->name(), i,
								out->val.get(), false, ctx);
					}
				}
			} else {
				s.Update(
						errors::Internal("Output ", i, " of type ",
...
void ExecutorState::PropagateOutputs(const TaggedNode& tagged_node,
		const NodeItem* item, EntryVector* outputs, TaggedNodeSeq* ready)
{
	const Node* node = tagged_node.node;
	FrameState* input_frame = tagged_node.input_frame;
	int64 input_iter = tagged_node.input_iter;
	...
	if (!item->is_enter_exit_or_next_iter) {
		...
		output_iter = 0;
		{
			...
			if (is_constant) {
				...
				output_frame->ActivateNodes(item, is_dead, output_iter, outputs,
						ready);
			}
		...
		if (is_dead) {
			...
			if (input_iter == input_frame->iteration_count
				...
				input_frame->next_iter_roots.push_back(
						{ node, (*outputs)[0] });
				output_frame = nullptr;
			} else {
	...
	for (auto& tagged_node : ready) {
		...
		if (tagged_node.is_dead || !item.kernel_is_expensive) {
			...
			if (curr_expensive_node) {
				...
				// do for this thread.
				runner_(
						std::bind(&ExecutorState::Process, this,
								*curr_expensive_node, scheduled_usec));
			}
...
void ExecutorState::DumpPendingNodeState(const int node_id,
		const Entry* input_vector, const bool show_nodes_with_no_ready_inputs)
{
		...
		if (tensor->IsInitialized()) {
			LOG(WARNING) << "      Input " << i << ": "
					<< strings::StrCat("Tensor<type: ",
							DataTypeString(tensor->dtype()), " shape: ",
		...
		IterationState* iteration)
{
	...
	for (const Node* node : *nodes) {
		int node_id = node->id();
		PendingCounts::Handle pending_id =
				impl_->gview_.node(node_id)->pending_id;
		if (iteration->node_state(pending_id) == PendingCounts::PENDING_NOTREADY
				|| iteration->node_state(pending_id)
						== PendingCounts::PENDING_READY) {
	...
	for (int i = 0; i < total_input_tensors; ++i) {
		...
		if (tensor->IsInitialized()) {
			LOG(WARNING) << "    Input " << i << ": "
					<< strings::StrCat("Tensor<type: ",
							DataTypeString(tensor->dtype()), " shape: ",
							tensor->shape().DebugString(), ", bytes: ",
							tensor->TotalBytes(), ">");
...
void ExecutorState::FindOrCreateChildFrame(FrameState* frame, int64 iter,
		const Node* node, FrameState** child)
{
	...
	// Initialize iteration 0.
	temp->iterations[0] = new IterationState(temp->pending_counts,
			...
			for (const Edge* e : node->out_edges()) {
				...
				if (IsMerge(dst_node)) {
					if (e->IsControlEdge()) {
						...
						int dead_cnt = parent_iter_state->dead_count(
								dst_pending_id);
						...
						const int dead_cnt = parent_iter_state->dead_count(
								dst_pending_id);
						dst_dead = (dead_cnt == dst_node->num_inputs());
						dst_ready = (parent_iter_state->pending(dst_pending_id)
								== 1) && dst_dead;
					}
				} else {
					parent_iter_state->increment_dead_count(dst_pending_id);
					dst_ready = (parent_iter_state->decrement_pending(
							dst_pending_id, 1) == 0);
				}
				if (dst_ready) {
					if (IsControlTrigger(dst_node))
						dst_dead = false;
					ready->push_back(
							TaggedNode(dst_node, parent_frame, parent_iter,
									dst_dead));
...
void ExecutorState::FrameState::ActivateNodes(const NodeItem* item,
		const bool is_dead, int64 iter, EntryVector* outputs,
		TaggedNodeSeq* ready)
{
		...
		if (dst_item->is_merge) {
			...
			if (is_control_edge) {
				...
				if ((*outputs)[src_slot].has_value) {
					...
					dst_dead = (dead_cnt == dst_item->num_inputs)
							|| item->is_enter;
					dst_ready = (iter_state->pending(dst_pending_id) == 1)
							&& dst_dead;
					dst_need_input = false;
				}
			}
		} else {
			bool increment_dead = (is_dead
		...
		TaggedNodeSeq* ready)
{
	...
	IterationState* iter_state = new IterationState(pending_counts,
			total_input_tensors);
