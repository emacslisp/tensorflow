...
template<typename T>
std::function<AssertionResult(const std::vector<T>& actual)> ApproxVectorMatcher(
		const std::vector<T>& expected, float abs_diff, float rel_diff)
{
...
template<typename T>
std::function<AssertionResult(const std::vector<T>& actual)> UnorderedElementsAre(
		const std::vector<T>& expected)
{
	return [expected](const std::vector<T>& actual) -> AssertionResult {
...
template<typename T>
std::function<AssertionResult(const std::vector<T>& actual)> OrderedMatcher(T a)
{
	std::vector<T> expected;
	expected.push_back(a);
...
inline const ::tensorflow::Status& GetStatus(const ::tensorflow::Status& status)
{
	return status;
}
