...
public:
	SparseTensorColumn(const Tensor& values, std::vector<int64> feature_counts,
			...
			values_(values), feature_counts_(std::move(feature_counts)), feature_start_indices_(
...
public:
	explicit DenseTensorColumn(const Tensor& tensor) :
			tensor_(tensor)
	{
	}
...
public:
	OutputUpdater(const std::vector<int64>& output_start_indices,
			...
			output_start_indices_(output_start_indices), indices_out_(
					indices_out), values_out_(values_out)
	{
	}
	...
	{
		const int64 output_index = output_start_indices_[batch_index]
				+ cross_count;
...
class HashCrosser {
public:
	HashCrosser(const std::vector<std::unique_ptr<ColumnInterface<int64>>>& columns,
...
class HashCrosserV2 {
public:
	HashCrosserV2(const std::vector<std::unique_ptr<ColumnInterface<int64>>>& columns,
...
public:
			...
			OpKernel(context)
	{
		...
		if (VERSION_2) {
			...
			OP_REQUIRES_OK(context,
					context->GetAttr("hash_key", &signed_hash_key_));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->input_list("indices", &indices_list_in));
		...
		typename CrossTraits<HASHED_OUTPUT, InternalType, VERSION_2>::Crosser crosser(
				columns, num_buckets_, hash_key_);
		Tensor* indices_out;
		Tensor* values_out;
		Tensor* shape_out;
		const int64 batch_size = CalculateBatchSize(shapes_list_in,
				dense_list_in);
		...
		typename CrossTraits<HASHED_OUTPUT, InternalType, VERSION_2>::Updater updater(
				output_start_indices, indices_out, values_out);
		auto do_work =
				[this, &columns, crosser, updater](int64 begin, int64 end) {
...
private:
	...
	void ValidateInput(OpKernelContext* context,
			...
			const OpInputList& shapes_list_in, const OpInputList& dense_list_in)
	{
		const auto size = indices_list_in.size();
		// Validates indices_list_in OpInputList.
		for (int i = 0; i < size; i++) {
			OP_REQUIRES(context,
					TensorShapeUtils::IsMatrix(indices_list_in[i].shape()),
					errors::InvalidArgument(
							"Input indices should be a matrix but received shape ",
							indices_list_in[i].shape().DebugString(),
							" at position ", i));
			OP_REQUIRES(context, indices_list_in[i].shape().dim_size(1) == 2,
		...
		OP_REQUIRES(context, values_list_in.size() == size,
				errors::InvalidArgument("Expected ", size,
						" input values, got ", values_list_in.size()));
		for (int i = 0; i < size; i++) {
			OP_REQUIRES(context,
					TensorShapeUtils::IsVector(values_list_in[i].shape()),
					errors::InvalidArgument(
							"Input values should be a std::vector but received shape ",
							values_list_in[i].shape().DebugString(),
							" at position ", i));
			OP_REQUIRES(context,
					indices_list_in[i].shape().dim_size(0)
							== values_list_in[i].shape().dim_size(0),
					errors::InvalidArgument("Expected size of values to be ",
		...
		OP_REQUIRES(context, shapes_list_in.size() == size,
				errors::InvalidArgument("Expected ", size,
						" input shapes, got ", shapes_list_in.size()));
		const auto batch_size = CalculateBatchSize(shapes_list_in,
				dense_list_in);
		for (int i = 0; i < size; i++) {
			OP_REQUIRES(context,
					TensorShapeUtils::IsVector(shapes_list_in[i].shape()),
					errors::InvalidArgument(
							"Input shapes should be a std::vector but received shape ",
							shapes_list_in[i].shape().DebugString(),
							" at position ", i));

			OP_REQUIRES(context, shapes_list_in[i].vec<int64>().size() == 2,
					errors::InvalidArgument(
							"shape should imply a 2D tensor, but got ",
							shapes_list_in[i].shape().DebugString(),
							" at position ", i));
			OP_REQUIRES(context,
					shapes_list_in[i].vec<int64>()(0) == batch_size,
		...
		for (int i = 0; i < dense_list_in.size(); ++i) {
			OP_REQUIRES(context,
					TensorShapeUtils::IsMatrix(dense_list_in[i].shape()),
					errors::InvalidArgument(
	...
	std::vector<std::unique_ptr<ColumnInterface<InternalType>>>GenerateColumnsFromInput(const OpInputList& indices_list_in,
			const OpInputList& values_list_in,
