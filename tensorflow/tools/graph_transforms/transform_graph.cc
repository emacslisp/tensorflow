		...
		TransformParameters* params_list)
{
	params_list->clear();
	enum {
		TRANSFORM_NAME, TRANSFORM_PARAM_NAME, TRANSFORM_PARAM_VALUE,
	...
	while (!remaining.empty()) {
		if (state == TRANSFORM_NAME) {
			...
			Scanner(remaining).Any(Scanner::SPACE).GetResult(&remaining,
					&match);
			// See if we have a valid transform name.
			const bool found_transform_name = Scanner(remaining).Any(
					Scanner::LETTER_DIGIT_UNDERSCORE).GetResult(&remaining,
					&transform_name);
			if (!found_transform_name) {
				return errors::InvalidArgument(
						"Looking for transform name, but found ",
						remaining.ToString().c_str());
			}
			if (Scanner(remaining).OneLiteral("(").GetResult(&remaining,
					&match)) {
				state = TRANSFORM_PARAM_NAME;
			} else {
				// Add a transform with no parameters.
				params_list->push_back( { transform_name.ToString(),
						func_parameters });
				transform_name = "";
				state = TRANSFORM_NAME;
			}
			...
			if (Scanner(remaining).OneLiteral(")").GetResult(&remaining,
				...
				Scanner(remaining).ZeroOrOneLiteral(",").GetResult(&remaining,
						&match);
				Scanner(remaining).Any(Scanner::SPACE).GetResult(&remaining,
						&match);
				// See if we have a valid parameter name.
				const bool found_parameter_name = Scanner(remaining).Any(
						Scanner::LETTER_DIGIT_UNDERSCORE).GetResult(&remaining,
						&parameter_name);
			...
			if (Scanner(remaining).OneLiteral("\"").GetResult(&remaining,
				...
				if (found_parameter_value) {
					Scanner(remaining).OneLiteral("\"").GetResult(&remaining,
							&match);
				}
			} else {
				// See if we have a valid parameter name.
				found_parameter_value =
						Scanner(remaining).Any(
								Scanner::LETTER_DIGIT_DASH_DOT_SLASH_UNDERSCORE).GetResult(
								&remaining, &parameter_value);
			}
			if (!found_parameter_value) {
				return errors::InvalidArgument(
...
int ParseFlagsAndTransformGraph(int argc, char* argv[], bool init_main)
{
	...
	std::vector<Flag> flag_list = { Flag("in_graph", &in_graph,
			"input graph file name"), Flag("out_graph", &out_graph,
			"output graph file name"), Flag("inputs", &inputs_string, "inputs"),
			Flag("outputs", &outputs_string, "outputs"), Flag("transforms",
					&transforms_string, "list of transforms"), Flag(
	...
	Status parse_status = ParseTransformParameters(transforms_string,
			&transform_params);
	if (!parse_status.ok()) {
	...
	Status transform_result = TransformGraph(inputs, outputs, transform_params,
			&graph_def);
		...
		bool* ignore_errors)
{
	*ignore_errors = false;
	if (transform_params.count("ignore_errors")
		...
		const string& ignore_errors_string = str_util::Lowercase(
				transform_params.at("ignore_errors").at(0));
...
Status TransformGraph(const std::vector<string>& inputs,
		...
		const TransformParameters& transform_params, GraphDef* graph_def)
{
	TransformRegistry* transform_registry = GetTransformRegistry();
	for (const auto& transform_info : transform_params) {
		...
		const TransformFunc& transform_func = transform_registry->at(
				transform_name);
		...
		Status transform_result = transform_func(*graph_def, context,
