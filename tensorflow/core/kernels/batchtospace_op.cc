		...
		const Tensor& orig_crops)
{
	...
	OP_REQUIRES(context,
			...
			errors::InvalidArgument("crops should have shape [", block_dims,
					", 2] instead of ", orig_crops.shape().DebugString()));
	...
	OP_REQUIRES(context, orig_input_batch_size % block_shape_product == 0,
			errors::InvalidArgument("Input batch dimension (",
	...
	for (int block_dim = removed_prefix_block_dims;
		...
		const int64 crop_start = crops[2 * block_dim], crop_end = crops[2
				* block_dim + 1];
		...
		const int64 cropped_size = input_size * block_shape_value - crop_start
				- crop_end;
...
public:
	explicit BatchToSpaceOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
