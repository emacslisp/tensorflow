...
public:
	RemoteDevice(Env* env, const DeviceAttributes& da) :
			Device(env, da, nullptr), local_dev_name_(
					GetLocalDeviceName(da.name()))
	{
	}
		...
		const string& worker_name, NewRemoteDevicesDone done)
{
	...
	if (wi == nullptr) {
		...
		done(errors::NotFound("Device ", worker_name, " is not found."),
				&empty);
		return;
	}
	struct Call {
	...
	Call* call = new Call;
	auto cb =
			[env, worker_cache, worker_name, done, wi, call](const Status& s) {
