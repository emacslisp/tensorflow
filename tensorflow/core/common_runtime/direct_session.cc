		...
		const ThreadPoolOptionProto& thread_pool_options, int pool_number)
{
	...
	return new thread::ThreadPool(options.env,
			strings::StrCat("Compute", pool_number), num_threads);
}
...
string GetRendezvousKey(const string& tensor_name,
		const DeviceAttributes& device_info, const FrameAndIter& frame_iter)
{
	return strings::StrCat(device_info.name(), ";",
			strings::FpToString(device_info.incarnation()), ";",
			device_info.name(), ";", tensor_name, ";", frame_iter.frame_id, ":",
			frame_iter.iter_id);
}
...
public:
	...
	Session* NewSession(const SessionOptions& options) override
	{
		...
		Status s = DeviceFactory::AddDevices(options,
		...
		DirectSession* session = new DirectSession(options,
				new DeviceMgr(devices), this);
		{
...
private:
	mutex sessions_lock_;std::vector<DirectSession*> sessions_ GUARDED_BY(sessions_lock_);
};
...
DirectSession::DirectSession(const SessionOptions& options,
		const DeviceMgr* device_mgr, DirectSessionFactory* const factory) :
		options_(options), device_mgr_(device_mgr), factory_(factory), cancellation_manager_(
				new CancellationManager()), operation_timeout_in_ms_(
	...
	if (options_.config.session_inter_op_thread_pool_size() > 0) {
		for (int i = 0; i < options_.config.session_inter_op_thread_pool_size();
			...
			thread_pools_.push_back(
					NewThreadPoolFromThreadPoolOptions(options_,
							options_.config.session_inter_op_thread_pool(i),
							i));
		}
...
Status DirectSession::MaybeInitializeExecutionState(const GraphDef& graph,
		bool* out_already_initialized)
{
	...
	GraphDef temp(graph);
	TF_RETURN_IF_ERROR(
			SimpleGraphExecutionState::MakeForBaseGraph(&temp, options,
					&execution_state_));
	graph_created_ = true;
		...
		RunMetadata* run_metadata)
{
	...
	if (!run_options.debug_options().debug_tensor_watch_opts().empty()) {
		run_state_args.debugger_state = DebuggerStateRegistry::CreateState(
				run_options.debug_options());
	}

	TF_RETURN_IF_ERROR(
			GetOrCreateExecutors(pool, input_tensor_names, output_names,
	...
	// Send inputs.
	TF_RETURN_IF_ERROR(
			SendInputs(inputs, executors_and_keys, run_state.rendez));

	// Start parallel Executors.
	const int num_executors = executors_and_keys->items.size();
	ExecutorBarrier* barrier = new ExecutorBarrier(num_executors,
			run_state.rendez, [&run_state](const Status& ret) {
				{
	...
	if (options_.config.graph_options().build_cost_model() > 0) {
		...
		update_cost_model = ((executors_and_keys->step_count + 1
				- build_cost_model_after) % build_cost_model_every == 0);
	}
	...
	WaitForNotification(&run_state, &step_cancellation_manager,
			run_options.timeout_in_ms() > 0 ?
					run_options.timeout_in_ms() : operation_timeout_in_ms_);
	...
	if (update_cost_model) {
		...
		args.stats_collector->BuildCostModel(&cost_model_manager_,
				device_to_graph);
		...
		for (const auto& item : executors_and_keys->items) {
			TF_RETURN_IF_ERROR(
					cost_model_manager_.AddToCostGraphDef(item.graph,
							cost_graph));
		}
	}
		...
		const std::vector<string>& target_nodes, string* handle)
{
	...
	TF_RETURN_IF_ERROR(
			GetOrCreateExecutors(pool, input_names, output_names, target_nodes,
					&executors_and_keys, &run_state_args));

	// Create the run state and save it for future PRun calls.
	Executor::Args args;
	args.step_id = step_id_counter_.fetch_add(1);
	RunState* run_state = new RunState(input_names, output_names, args.step_id,
			&devices_);
	run_state->rendez = new IntraProcessRendezvous(device_mgr_.get());
	{
		mutex_lock l(executor_lock_);
		...
		if (s.ok()) {
			{
				...
				if (!run_state->status.ok()) {
					LOG(WARNING)
							<< "An error unrelated to this prun has been detected. "
							<< run_state->status;
				}
			}
			for (const auto& it : inputs) {
		...
		const ExecutorsAndKeys* executors_and_keys, const RunState* run_state)
{
	...
	while (!stack.empty()) {
		...
		for (const Edge* in_edge : n->in_edges()) {
			...
			if (pending_feeds.count( { in_node->name(), in_edge->src_output() })
					> 0) {
...
Status DirectSession::GetOrCreateExecutors(thread::ThreadPool* pool,
		...
		ExecutorsAndKeys** executors_and_keys, RunStateArgs* run_state_args)
{
	...
	const string key = strings::StrCat(str_util::Join(inputs_sorted, ","), "->",
			...
			str_util::Join(tn_sorted, ","), "/", run_state_args->is_partial_run,
			"/", debug_tensor_watches_summary);

	// Set the handle.
	run_state_args->handle = strings::StrCat(key, ";",
			handle_name_counter_.fetch_add(1));
	...
	for (auto iter = graphs.begin(); iter != graphs.end(); ++iter) {
		...
		item->flib.reset(
				NewFunctionLibraryRuntime(device_mgr_.get(), options_.env,
						device, graph_def_version, ek->flib_def.get(),
						optimizer_opts));
		...
		if (run_state_args->debugger_state) {
			TF_RETURN_IF_ERROR(
					run_state_args->debugger_state->DecorateGraphForDebug(
							partition_graph.get(), params.device));
		}

		TF_RETURN_IF_ERROR(
				EnsureMemoryTypes(DeviceType(device->device_type()),
						device->name(), partition_graph.get()));
	...
	for (const string& input : inputs) {
		ek->input_keys[input] = GetRendezvousKey(input,
				device_set_.client_device()->attributes(), FrameAndIter(0, 0));
	}
	for (const string& output : outputs) {
		ek->output_keys[output] = GetRendezvousKey(output,
				device_set_.client_device()->attributes(), FrameAndIter(0, 0));
	}
		...
		RunStateArgs* run_state_args)
{
	...
	if (options_.config.graph_options().place_pruned_graph()) {
		...
		TF_RETURN_IF_ERROR(
				SimpleGraphExecutionState::MakeForPrunedGraph(
						execution_state_->original_graph_def().library(),
						prune_options, execution_state_->original_graph_def(),
						subgraph_options, &temp_exec_state_holder,
						&client_graph));
	...
	for (auto placement_pair : current_stateful_placements) {
		...
		if (iter == stateful_placements_.end()) {
			...
			return errors::Internal("Stateful placement mismatch. "
					"Current assignment of ", node_name, " to ", iter->second,
	...
	for (const auto& partition : partitions) {
		const string& local_partition_name = DeviceNameUtils::LocalName(
				partition.first);
		if (std::count(device_names.begin(), device_names.end(),
			...
			return errors::InvalidArgument("Creating a partition for ",
					...
					" which doesn't exist in the list of available devices. Available "
							"devices: ", str_util::Join(device_names, ","));
		}
	}
...
::tensorflow::Status DirectSession::Reset(const std::vector<string>& containers)
{
...
{
	Status status = WaitForNotification(&run_state->executors_done,
			timeout_in_ms);
	if (!status.ok()) {
		{
			mutex_lock l(run_state->mu_);
...
{
	if (timeout_in_ms > 0) {
		bool notified = WaitForNotificationWithTimeout(notification,
				timeout_in_ms);
		if (!notified) {
