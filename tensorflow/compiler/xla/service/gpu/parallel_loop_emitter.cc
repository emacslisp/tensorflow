...
ParallelLoopEmitter::ParallelLoopEmitter(BodyEmitter body_emitter,
		const Shape& shape, const LaunchDimensions& launch_dimensions,
		llvm::IRBuilder<>* ir_builder) :
		LoopEmitter(body_emitter, shape, ir_builder), launch_dimensions_(
				launch_dimensions)
{
}
...
ParallelLoopEmitter::ParallelLoopEmitter(
		...
		llvm::IRBuilder<>* ir_builder) :
		LoopEmitter(target_element_generator, target_array, ir_builder), launch_dimensions_(
				launch_dimensions)
{
}
...
llvm_ir::IrArray::Index ParallelLoopEmitter::EmitIndexAndSetExitBasicBlock()
{
	...
	block_id = ir_builder_->CreateZExt(block_id, ir_builder_->getInt64Ty(),
			"block_id");
	...
	llvm::Value* linear_index = ir_builder_->CreateAdd(
			ir_builder_->CreateMul(block_id,
					ir_builder_->getInt64(
							launch_dimensions_.threads_per_block()), "",
					/*HasNUW=*/true, /*HasNSW=*/true), thread_id,
			"linear_index", /*HasNUW=*/true, /*HasNSW=*/true);

	auto if_in_bounds = llvm_ir::EmitIfThenElse(
			ir_builder_->CreateICmpULT(linear_index,
