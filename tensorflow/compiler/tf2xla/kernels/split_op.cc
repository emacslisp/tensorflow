...
public:
	explicit SplitOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		if (index_shape.dims() == 0) {
			...
			OP_REQUIRES(ctx, index_shape.dims() == 1,
					errors::InvalidArgument(
							"split_index input to Split Op must be a "
									"scalar or a vector with 1 element"));
			OP_REQUIRES(ctx, index_shape.dim_size(0) == 1,
					errors::InvalidArgument(
							"split_index input to Split Op must be a "
									"scalar or a vector with 1 element"));
...
public:
	explicit SplitVOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				...
				errors::InvalidArgument("shape of tensor describing "
		...
		for (int i = 0; i < num_split; ++i) {
			...
			if (slice_size == -1) {
				OP_REQUIRES(ctx, neg_one_dim == -1,
						errors::InvalidArgument(
								"Only one dimensions can have a value of"
		...
		OP_REQUIRES(ctx,
				(neg_one_dim == -1
						...
						|| (neg_one_dim >= 0
								&& total_split_size
										<= input_shape.dim_size(split_dim)),
				errors::InvalidArgument("Determined shape must either match "
						...
						"specified.  Got: ", total_split_size));

		if (neg_one_dim >= 0) {
			split_sizes_vec[neg_one_dim] = input_shape.dim_size(split_dim)
					- total_split_size;
		}
