...
void XlaDeviceAllocator::GetStats(AllocatorStats* stats)
{
	stats->Clear();
}
...
XlaTransferManager::XlaTransferManager(xla::Client* client) :
		client_(client)
{
}

void XlaTransferManager::CopyCPUTensorToDevice(const Tensor* cpu_tensor,
		Device* device, Tensor* device_tensor, StatusCallback done) const
{
...
void XlaTransferManager::CopyDeviceTensorToCPU(const Tensor* device_tensor,
		StringPiece tensor_name, Device* device, Tensor* cpu_tensor,
		StatusCallback done)
{
	if (device_tensor->NumElements() > 0) {
		...
		std::shared_ptr<xla::GlobalData> global_data = GetTensorGlobalData(
				*device_tensor);

		xla::Shape shape;
		Status status = TensorShapeToXLAShape(cpu_tensor->dtype(),
		...
		const void* src_ptr = xla::LiteralUtil::InternalData(
				*result.ValueOrDie());
...
XlaDeviceContext::XlaDeviceContext(xla::Client* client) :
		manager_(client)
{
}
...
void XlaDeviceContext::CopyDeviceTensorToCPU(const Tensor* device_tensor,
		...
		StatusCallback done)
{
	manager_.CopyDeviceTensorToCPU(device_tensor, tensor_name, device,
			cpu_tensor, done);
}
