...
public:
	explicit BitcastOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		...
		int check_size = std::max(in_size_, out_size_)
				% std::min(in_size_, out_size_);
		OP_REQUIRES(context, check_size == 0,
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				in_size_ >= out_size_
						|| (input_tensor.dims() > 0
								&& input_tensor.dim_size(
										input_tensor.dims() - 1)
										== out_size_ / in_size_)
						|| input_tensor.dim_size(input_tensor.dims()) == -1,
				errors::InvalidArgument("Cannot bitcast from ",
						DataTypeString(input_data_type_), " to ",
						DataTypeString(output_data_type_), ": shape ",
