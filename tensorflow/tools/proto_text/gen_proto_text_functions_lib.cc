...
public:
	Generator(const string& tf_header_prefix) :
			tf_header_prefix_(tf_header_prefix), header_(&code_.header), header_impl_(
					&code_.header_impl), cc_(&code_.cc)
	{
	}
...
private:
	...
	void AppendFieldValueAppend(const FieldDescriptor& field,
			const bool omit_default, const string& field_expr);
...
{
	string cpp_class =
			d.cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE ?
					GetQualifiedName(*d.message_type()) : d.cpp_type_name();
...
void Generator::AppendFieldAppend(const FieldDescriptor& field)
{
	...
	if (field.is_map()) {
		...
		const auto& value_type = *field.message_type()->FindFieldByName(
				"value");

		Print("std::vector<", key_type.cpp_type_name(), "> keys;");
		Print("for (const auto& e : msg.", name,
		...
		if (oneof != nullptr) {
			...
			Print("if (msg.", oneof->name(), "_case() == ",
					GetQualifiedName(*oneof->containing_type()), "::k",
					camel_name, ") {");
			Nest();
			AppendFieldValueAppend(field, false /* omit_default */,
					"msg." + name + "()");
...
void Generator::AppendParseMessageFunction(const Descriptor& md)
{
	...
	// Emit success case
	Print("if (nested && (scanner->Peek() == (close_curly ? '}' : '>'))) {").Nest();
	...
	for (int i = 0; i < md.field_count(); ++i) {
		...
		if (map_append) {
			...
			if (field->is_map()) {
				...
				set_value_prefix = "UNREACHABLE"; // generator will never use this value.
			} else {
		...
		if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
			...
			Print("SetProtobufStringSwapAllowed(&str_value, ",
					mutable_value_expr, ");");
		} else if (field->cpp_type() == FieldDescriptor::CPPTYPE_ENUM) {
			Print("StringPiece value;");
			Print("if (!parsed_colon || "
...
void Generator::AppendDebugStringFunctions(const Descriptor& md)
{
	...
	for (int short_pass = 0; short_pass < 2; ++short_pass) {
		...
		const string sig = StrCat("string ",
				short_debug ? "ProtoShortDebugString" : "ProtoDebugString",
	...
	const string sig = StrCat("void AppendProtoDebugString(\n",
		...
		std::set<const Descriptor*>* all_d)
{
	...
	for (int i = 0; i < d->field_count(); ++i) {
		...
		case FieldDescriptor::CPPTYPE_MESSAGE:
			...
			break;
		case FieldDescriptor::CPPTYPE_ENUM:
			GetAllFileDescriptorsFromFile(f->enum_type()->file(), all_fd,
					all_d);
			break;
		}
	}
