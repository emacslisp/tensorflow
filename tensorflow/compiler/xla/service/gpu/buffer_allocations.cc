		...
		DeviceMemoryAllocator* memory_allocator)
{
	...
	if (temp_buffer_offsets.TotalSizeInBytes() > 0) {
		TF_ASSIGN_OR_RETURN(temp_buffer_base,
		...
		if (temp_buffer_base == nullptr) {
			return ResourceExhausted(
					...
					tensorflow::strings::HumanReadableNumBytes(
							temp_buffer_offsets.TotalSizeInBytes()).c_str());
		}
	}
	auto buffer_allocations = WrapUnique(
			new BufferAllocations(buffer_assignment.Allocations().size(),
					temp_buffer_base, device_ordinal, memory_allocator));
	...
	for (BufferAllocation::Index i = 0; i < num_buffers; ++i) {
		...
		if (allocation.maybe_live_out()) {
			...
			if (buffer_size > 0) {
				...
				TF_ASSIGN_OR_RETURN(buffer_address,
						memory_allocator->Allocate(device_ordinal,
								buffer_size));
			...
			se::DeviceMemoryBase temp_buffer_address(
					/*opaque=*/static_cast<char*>(buffer_allocations->GetTempBufferBase().opaque())
		...
		const BufferAssignment& buffer_assignment)
{
	...
	for (auto i = 0; i < buffer_assignment.Allocations().size(); ++i) {
		const BufferAllocation& allocation = buffer_assignment.GetAllocation(i);
		se::DeviceMemoryBase buffer_address = GetDeviceAddress(
				allocation.index());
		if (allocation.maybe_live_out()
			...
			// live out.
			TF_RETURN_IF_ERROR(
					memory_allocator_->Deallocate(device_ordinal_,
							&buffer_address));
		}
	}
