	...
	if (b.tag() != REP_OUT_OF_LINE) {
		...
		if (tag() == REP_OUT_OF_LINE) {
			...
			*(as64()->dims_) = *(b.as64()->dims_);
			...
			as64()->dims_ = new gtl::InlinedVector<int64, 4>(
					*(b.as64()->dims_));
		}
	}
}
...
void TensorShape::AddDim(int64 size)
{
	...
	if (tag() == REP16 && nd < 6 && size < kMaxRep16) {
		...
		if (can_be_rep32) {
			...
			set_tag(REP_OUT_OF_LINE);
			as64()->dims_ = new gtl::InlinedVector<int64, 4>(vals.begin(),
					vals.end());
		}
	}
	set_ndims_byte(nd + 1);
...
string TensorShape::DebugString() const
{
	...
	return strings::StrCat("[",
			str_util::Join(gtl::ArraySlice<int64>(vals), ","), "]");
}
...
{
	if (shape.dims() < prefix.dims())
		return false;
	for (int i = 0; i < prefix.dims(); ++i) {
		if (shape.dim_size(i) != prefix.dim_size(i))
			return false;
	}
	return true;
}
		...
		const TensorShape& suffix)
{
	...
	if (shape.dims() < suffix_size)
		return false;
	for (int i = 0; i < suffix_size; ++i) {
		if (shape.dim_size(shape.dims() - suffix_size + i)
				!= suffix.dim_size(i)) {
			return false;
		}
	}
