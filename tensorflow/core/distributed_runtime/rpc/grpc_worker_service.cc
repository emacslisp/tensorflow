...
private:
	...
	mutex shutdown_mu_;bool is_shutdown_ GUARDED_BY(shutdown_mu_);
...
#undef ENQUEUE_REQUEST
	...
	void EnqueueRecvTensorRequestRaw()
	{
		...
		if (!is_shutdown_) {
			Call<GrpcWorkerService, grpc::WorkerService::AsyncService,
					RecvTensorRequest, ::grpc::ByteBuffer>::EnqueueRequestForMethod(
					&worker_service_, cq_,
...
void GrpcWorker::RecvTensorAsync(CallOptions* opts,
		const RecvTensorRequest* request, ::grpc::ByteBuffer* response,
		StatusCallback done)
{
	...
	env_->rendezvous_mgr->RecvLocalAsync(step_id, parsed,
...
GrpcWorker* NewGrpcWorker(WorkerEnv* env)
{
	return new GrpcWorker(env);
}
