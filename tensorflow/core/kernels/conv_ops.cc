...
struct LaunchGeneric {
			...
			TensorFormat data_format)
	{
		...
		if (filter.dim_size(1) == filter.dim_size(0) && filter.dim_size(0) == 1
			...
			functor::MatMulConvFunctor<Device, T>()(ctx->eigen_device<Device>(),
			...
			functor::SpatialConvolution<Device, T>()(
					...
					input.tensor<T, 4>(), filter.tensor<T, 4>(), row_stride,
					col_stride, padding);
		}
	}
};
...
public:
			...
			TensorFormat data_format)
	{
		LaunchGeneric<CPUDevice, T>::launch(ctx, input, filter, row_stride,
				col_stride, padding, output, data_format);
	}
};
...
public:
	static bool Run(OpKernelContext* ctx, const Tensor& input,
			const Tensor& filter, int batch, int input_rows, int input_cols,
			int in_depth, int filter_rows, int filter_cols, int pad_rows,
			int pad_cols, int out_rows, int out_cols, int out_depth,
			int stride_rows, int stride_cols, Tensor* output,
			TensorFormat data_format)
	{
		return false;
	}
...
public:
			...
			TensorFormat data_format)
	{
		if (data_format != FORMAT_NHWC
				|| !CanUseDeepConv2D(stride_rows, stride_cols, filter_rows,
						filter_cols, in_depth, out_depth, out_rows, out_cols)) {
			return false;
		}
		...
		functor::DeepConv2D<CPUDevice, float>()(ctx, args, input_ptr,
...
public:
			...
			BinaryOp<T>(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("use_cudnn_on_gpu", &use_cudnn_));
	...
	void Compute(OpKernelContext* context) override
	{
		...
		if (LaunchDeepConvOp<Device, T>::Run(context, input, filter, batch,
				input_rows, input_cols, in_depth, filter_rows, filter_cols,
				pad_rows, pad_cols, out_rows, out_cols, out_depth, stride_rows,
				stride_cols, output, data_format_)) {
			return;
		}

		launcher_.launch(context, use_cudnn_, cudnn_use_autotune_, input,
				filter, stride_rows, stride_cols,
