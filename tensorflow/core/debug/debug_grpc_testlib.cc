...
::grpc::Status TestEventListenerImpl::SendEvents(::grpc::ServerContext* context,
		::grpc::ServerReaderWriter<::tensorflow::EventReply, ::tensorflow::Event>* stream)
{
	Event event;
	...
	while (stream->Read(&event)) {
		...
		std::vector<string> name_items = tensorflow::str_util::Split(
				val.node_name(), ':');
		...
		server_port(server_port)
{
	...
	prep_tensor_.reset(new Tensor(DT_FLOAT, TensorShape( { kTensorSize,
			kTensorSize })));
	...
	dump_root = io::JoinPath(tmp_dir,
			strings::StrCat("tfdbg_dump_port", server_port, "_",
...
bool GrpcTestServerClientPair::PollTillFirstRequestSucceeds()
{
	...
	while (n_attempts++ < kMaxAttempts) {
		...
		Status publish_s = DebugIO::PublishDebugTensor("prep_node:0",
				"DebugIdentity", *prep_tensor_, wall_time, urls);
