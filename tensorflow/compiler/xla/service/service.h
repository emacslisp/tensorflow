...
public:
	...
	tensorflow::Status DeconstructTuple(const DeconstructTupleRequest* arg,
	...
	// replica id 0.
	tensorflow::Status GetDeviceHandles(const GetDeviceHandlesRequest* arg,
	...
	// first call.
	tensorflow::Status WaitForExecution(const WaitForExecutionRequest* arg,
			WaitForExecutionResponse* result) override;

	// Requests that global data be transferred to the client in literal form.
	tensorflow::Status TransferToClient(const TransferToClientRequest* arg,
	...
	tensorflow::Status TransferToServer(const TransferToServerRequest* arg,
	...
	tensorflow::Status TransferToInfeed(const TransferToInfeedRequest* arg,
	...
	tensorflow::Status GetComputationStats(const ComputationStatsRequest* arg,
...
protected:
	...
	StatusOr<std::unique_ptr<Executable>> BuildExecutable(
			...
			const tensorflow::gtl::ArraySlice<
					perftools::gputools::DeviceMemoryBase> arguments,
...
template<typename ReturnT>
ReturnT Service::ExecuteOnStreamWrapper(Executable* executable,
		const ExecutableRunOptions* run_options, ExecutionProfile* profile,
		std::function<
				ReturnT(Executable* executable,
						const ExecutableRunOptions* run_options,
						HloExecutionProfile* hlo_execution_profile)> execute_func)
{
	...
	if (profile != nullptr) {
		...
		if (profile->compute_time_ns() == 0) {
			profile->set_compute_time_ns(
					profile->compute_and_transfer_time_ns());
		}
	}
