...
static void PrepareAndValidateInputs(OpKernelContext* c,
		const TensorShape& params_shape, const Tensor& indices,
		const Tensor& updates, int64* indices_nd, Index* num_updates,
		Index* slice_size)
{
	const TensorShape& indices_shape(indices.shape());
	const TensorShape& updates_shape(updates.shape());

	OP_REQUIRES(c, TensorShapeUtils::IsVectorOrHigher(params_shape),
	...
	OP_REQUIRES(c, updates.dim_size(0) == indices.dim_size(0),
			errors::InvalidArgument(
					...
					indices_shape.DebugString(), ", updates.shape ",
	...
	*indices_nd = 1;
	...
	OP_REQUIRES(c, slice_size_big <= std::numeric_limits<Index>::max(),
			errors::InvalidArgument("slice size is too large for indexing: ",
					slice_size_big, " > ", std::numeric_limits<Index>::max()));
...
public:
	explicit ScatterNdOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
...
#undef PARAMS_CASE
		...
		OP_REQUIRES(c, bad_i < 0,
				errors::InvalidArgument("Invalid indices: ",
						SliceDebugString(indices.shape(), bad_i), " = [",
						str_util::Join(
								gtl::ArraySlice<Index>(&indices_flat(bad_i, 0),
										indices_nd), ", "),
						"] does not index into ", shape.DebugString()));
	}
};

template<typename Device, typename T, typename Index, scatter_nd_op::UpdateOp op>
...
public:
			...
			OpKernel(c)
	{
		...
		const DataType index_t = DataTypeToEnum<Index>::v();
		OP_REQUIRES_OK(c,
				c->MatchSignature( { dt_ref, index_t, dt }, { dt_ref }));
