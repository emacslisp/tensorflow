...
public:
	...
	KernelBase(StreamExecutor *parent,
	...
	const string &demangled_name() const
	{
...
public:
	KernelArgIterator(int number_of_argument_addresses,
			...
			const size_t *arg_sizes_data, const size_t *shmem_bytes_data,
			const size_t *shmem_indices_data) :
			arg_index_(0), number_of_arguments_(
					number_of_argument_addresses
							+ number_of_shared_memory_arguments), arg_address_iter_(
					arg_addresses_data), arg_size_iter_(arg_sizes_data), shmem_bytes_iter_(
					shmem_bytes_data), shmem_indices_iter_(shmem_indices_data), shmem_indices_end_(
					shmem_indices_data + number_of_shared_memory_arguments)
	{
	}
...
public:
	explicit KernelArgsArray() :
			total_shared_memory_bytes_(0), number_of_argument_addresses_(0), number_of_shared_memory_arguments_(
					0)
	{
	}
	...
	{
		return KernelArgIterator(number_of_argument_addresses_,
				number_of_shared_memory_arguments_, argument_addresses_.data(),
				argument_sizes_.data(), shared_memory_bytes_.data(),
				shared_memory_indices_.data());
	}
...
struct KernelInvocationChecker {
	...
	static constexpr bool kLengthMatches = std::tuple_size<ParamTuple>::value
			== std::tuple_size<ArgTuple>::value;

	// The (matching) length of the parameters and arguments type lists.
	static constexpr int kTupleLength = static_cast<int>(std::tuple_size<
			ArgTuple>::value);
	...
	{
		return std::is_same<typename std::remove_const<ParamT>::type, ArgT>::value
	...
	static constexpr bool CheckParam(
			typename std::enable_if<(kArgumentNumber < 0)>::type *dummy =
					nullptr)
	{
		return true;
	}
	...
	{
		typedef typename std::tuple_element<kArgumentNumber, ParamTuple>::type ParamT;
...
struct KernelParamsOk<TypedKernel<Params...>, Args...> {
	static constexpr bool kResult =
			KernelInvocationChecker<std::tuple<Params...>, std::tuple<Args...>>::CheckAllNoStaticAssert();
};
