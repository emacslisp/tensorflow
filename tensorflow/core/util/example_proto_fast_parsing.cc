...
public:
	...
	Feature(StringPiece serialized) :
	...
	bool ParseBytesList(Result* bytes_list)
	{
		...
		protobuf::io::CodedInputStream stream(
				reinterpret_cast<const uint8*>(serialized_.data()),
				serialized_.size());
		...
		while (!stream.ExpectAtEnd()) {
			...
			if (!stream.ReadVarint32(&bytes_length))
				return false;
			string bytes;
			if (!stream.ReadString(&bytes, bytes_length))
				return false;
...
bool ParseString(protobuf::io::CodedInputStream* stream, StringPiece* result)
{
	...
	if (static_cast<uint32>(stream_size) < length)
		return false;
...
bool TestFastParse(const string& serialized, Example* example)
{
	...
	for (size_t i = 0; i < parsed_example_size; ++i) {
		...
		if ((*features.mutable_feature()).count(name) > 0)
			continue;

		auto& value = (*features.mutable_feature())[name];
		DataType dtype;
		if (!name_and_feature.second.ParseDataType(&dtype).ok())
			return false;
		switch (dtype) {
		case DT_INVALID:
			break;
		case DT_STRING: {
			...
			if (!name_and_feature.second.ParseBytesList(&list))
				return false;
		...
		case DT_FLOAT: {
			...
			if (!name_and_feature.second.ParseFloatList(&list))
				return false;
		...
		case DT_INT64: {
			...
			if (!name_and_feature.second.ParseInt64List(&list))
				return false;
...
Status FastParseSerializedExample(const string& serialized_example,
		const string& example_name, const size_t example_index,
		const Config& config,
		...
		std::vector<SparseBuffer>* output_sparse)
{
	...
	for (size_t i = 0; i < parsed_example_size; ++i) {
		...
		if (is_dense) {
			...
			if (dense_feature_last_example[d] == example_index) {
				...
				static auto* duplicated_dense_feature =
			...
			const std::size_t num_elements =
					config.dense[d].shape.num_elements();
			const std::size_t offset = example_index * num_elements;

			auto shape_error =
					[&](size_t size, StringPiece type_str) {
			...
			case DT_INT64: {
				...
				if (slice.EndDistance() != 0) {
					return shape_error(num_elements - slice.EndDistance(),
							"int64");
				}
				break;
			}
			...
			if (sparse_feature_last_example[d] == example_index) {
				LOG(WARNING) << "Data loss! Feature '" << feature_name
						<< "' in present in multiple concatenated "
								"tf.Examples. Ignoring all but last one.";
				static auto* duplicated_sparse_feature =
						monitoring::Counter<0>::New(
								"/tensorflow/core/util/example_proto_fast_parsing/"
	...
	for (size_t d = 0; d < config.sparse.size(); ++d) {
		...
		size_t prev_example_end_index =
				out.example_end_indices.empty() ?
						0 : out.example_end_indices.back();
	...
	for (size_t i = 0; i < 1000; ++i) {
		for (size_t d = 0; d < config.dense.size(); ++d) {
			ok &= config_index.InsertUnique(
					hasher(config.dense[d].feature_name), { d, Type::Dense });
		}
		for (size_t d = 0; d < config.sparse.size(); ++d) {
			ok &= config_index.InsertUnique(
					hasher(config.sparse[d].feature_name), { d, Type::Sparse });
		}
		if (ok)
			break;
