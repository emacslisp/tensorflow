...
Status PrepareCopy(Device* device, const DeviceContext* ctx, const Tensor& src,
		const Tensor* dst, const DeviceBase::GpuDeviceInfo** dev_info,
		gpu::Stream** stream)
{
	if (device == nullptr) {
	...
	if (dst != nullptr) {
		...
		if (src.TotalBytes() != dst->TotalBytes()) {
			return errors::Internal("Can't copy ", src.TotalBytes(),
					" bytes of a tensor into another with ", dst->TotalBytes(),
					" bytes buffer.");
		}
...
void GPUUtil::SetProtoFromGPU(const Tensor& tensor, Device* dev,
		const DeviceContext* device_context, TensorProto* proto, bool is_dead,
		StatusCallback done)
{
	...
	if (total_bytes > 0) {
		...
		if (LogMemory::IsEnabled()) {
			LogMemory::RecordRawAllocation("SetProtoFromGPU",
					LogMemory::PROTO_BUFFER_STEP_ID, total_bytes, buf, alloc);
		}
	...
	dev_info->event_mgr->ThenExecute(send_device_to_host_stream,
			[send_device_to_host_stream, done, proto, buf,
...
void GPUUtil::DeviceToDeviceCopy(DeviceContext* send_dev_context,
		...
		AllocatorAttributes src_alloc_attr, AllocatorAttributes dst_alloc_attr,
		const Tensor* input, Tensor* output, StatusCallback done)
{
	...
	auto send_device_to_device_stream =
			static_cast<const GPUDeviceContext*>(send_dev_context)->device_to_device_stream();
	...
	dev_info->event_mgr->ThenExecute(send_device_to_device_stream,
...
static CopyTensor::Registration register_gpu_gpu_copy(DEVICE_GPU, DEVICE_GPU,
		GPUUtil::DeviceToDeviceCopy);

// static
void GPUUtil::CopyGPUTensorToCPU(Device* gpu_device,
		const DeviceContext* device_context, const Tensor* gpu_tensor,
		Tensor* cpu_tensor, StatusCallback done)
{
	...
	if (total_bytes > 0) {
		...
		send_device_to_host_stream->ThenMemcpy(dst_ptr, gpu_src_ptr,
				total_bytes);
	}
...
void GPUUtil::CopyCPUTensorToGPU(const Tensor* cpu_tensor,
		const DeviceContext* device_context, Device* gpu_device,
		Tensor* gpu_tensor, StatusCallback done)
{
	...
	dev_info->event_mgr->ThenExecute(recv_host_to_device_stream,
	...
	if (num_bytes > 0) {
		...
		if (!dev_info) {
			strings::StrAppend(&ret,
					PrintMemory(reinterpret_cast<const char*>(ptr), num_bytes));
		} else {
			string buf;
			buf.resize(num_bytes);
			DeviceMemoryBase gpu_ptr(ptr, num_bytes);
			auto s = dev_info->stream->parent()->SynchronousMemcpyD2H(gpu_ptr,
					num_bytes, gtl::string_as_array(&buf));
			strings::StrAppend(&ret,
...
uint64 GPUUtil::Checksum(Device* gpu_device,
		const DeviceContext* device_context, const Tensor& tensor)
{
...
void GPUUtil::CopyGPUTensorToSameGPU(Device* gpu_device,
		const DeviceContext* device_context, const Tensor* src_gpu_tensor,
		Tensor* dst_gpu_tensor, StatusCallback done)
{
