...
XlaCompilationDevice::XlaCompilationDevice(const SessionOptions& options,
		DeviceType type) :
		LocalDevice(options,
				Device::BuildDeviceAttributes("", type, Bytes(256 << 20),
						DeviceLocality(),
						strings::StrCat("device: XLA JIT device ",
								type.type())), cpu_allocator()), allocator_(
				new XlaCompilationAllocator())
{
}
...
{
	auto platform = perftools::gputools::MultiPlatformManager::PlatformWithId(
			id);
	if (!platform.ok())
		return false;
...
/* static */void XlaOpRegistry::RegisterJitDevice(const string& device_name,
		const string& jit_device_name, bool requires_jit,
		bool enable_jit_by_default)
{
	XlaOpRegistry& registry = Instance();
	mutex_lock lock(registry.mutex_);
	auto result = registry.jit_devices_.emplace(device_name, JitDevice {
			jit_device_name, requires_jit, enable_jit_by_default });
	CHECK(
			result.second
		...
		bool* enable_jit_by_default)
{
	...
	if (it == registry.jit_devices_.end())
		return false;
	if (jit_device_name)
		*jit_device_name = &it->second.jit_device_name;
	if (requires_jit)
		*requires_jit = it->second.requires_jit;
...
XlaOpRegistrar::XlaOpRegistrar(StringPiece name, XlaOpRegistry::Factory factory)
{
