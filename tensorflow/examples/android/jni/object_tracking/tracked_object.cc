...
static void InitNormalized(const Image<uint8>& src_image,
		const BoundingBox& position, Image<float>* const dst_image)
{
...
TrackedObject::TrackedObject(const std::string& id, const Image<uint8>& image,
		const BoundingBox& bounding_box, ObjectModelBase* const model) :
		id_(id), last_known_position_(bounding_box), last_detection_position_(
				bounding_box), position_last_computed_time_(-1), object_model_(
				model), last_detection_thumbnail_(kNormalizedThumbnailSize,
				kNormalizedThumbnailSize), last_frame_thumbnail_(
				kNormalizedThumbnailSize, kNormalizedThumbnailSize), tracked_correlation_(
				0.0f), tracked_match_score_(0.0), num_consecutive_frames_below_threshold_(
				0), allowable_detection_distance_(Square(kInitialDistance))
{
...
void TrackedObject::UpdatePosition(const BoundingBox& new_position,
		const int64 timestamp, const ImageData& image_data,
		const bool authoratative)
{
	last_known_position_ = new_position;
	position_last_computed_time_ = timestamp;

	InitNormalized(*image_data.GetImage(), new_position,
			&last_frame_thumbnail_);

	const float last_localization_correlation = ComputeCrossCorrelation(
			last_detection_thumbnail_.data(), last_frame_thumbnail_.data(),
	...
	if (object_model_ != NULL) {
		tracked_correlation_ = object_model_->GetMaxCorrelation(
				last_frame_thumbnail_);
		LOGV("Tracked correlation to model:               %.6f",
				tracked_correlation_);

		tracked_match_score_ = object_model_->GetMatchScore(new_position,
				image_data);
	...
	if (tracked_correlation_ >= kMinimumCorrelationForTracking
		...
		if (object_model_ != NULL) {
			object_model_->TrackStep(last_known_position_,
					*image_data.GetImage(), *image_data.GetIntegralImage(),
					authoratative);
		}
...
void TrackedObject::OnDetection(ObjectModelBase* const model,
		const BoundingBox& detection_position, const MatchScore match_score,
		const int64 timestamp, const ImageData& image_data)
{
	...
	InitNormalized(*image_data.GetImage(), detection_position,
			&last_detection_thumbnail_);
