...
public:
	explicit GatherOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	}
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, TensorShapeUtils::IsVectorOrHigher(params_shape),
		...
		OP_REQUIRES(ctx, params_shape.dim_size(0) <= limit,
		...
		OP_REQUIRES(ctx, tc.allow_cpu_custom_calls(),
				errors::InvalidArgument(
						"Gather op requires CustomCall on CPU"));
		...
		args.push_back(
				b.ConstantLiteral(
						*xla::LiteralUtil::CreateR0 < int64
		...
		OP_REQUIRES_OK(ctx,
				TensorShapeToXLAShape(DT_FLOAT, result_shape, &xla_out_shape));

		// Call the custom code with args:
		xla::ComputationDataHandle output;
		if (index_type == DT_INT32) {
			output = b.CustomCall("gather_float_int32_xla_impl", args,
					xla_out_shape);
		} else {
			output = b.CustomCall("gather_float_int64_xla_impl", args,
					xla_out_shape);
		}
