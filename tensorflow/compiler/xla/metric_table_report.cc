...
void MetricTableReport::AppendHeader()
{
	...
	AppendLine("There are ", MetricString(UnaccountedMetric()), " ",
			metric_name_, " (", MetricPercent(UnaccountedMetric()),
...
void MetricTableReport::AppendCategoryTable()
{
	...
	for (const auto& category : categories) {
		if (categories_shown >= max_entries_to_show_
				|| metric_sum / expected_metric_sum_
						> max_metric_proportion_to_show) {
			break;
		}
		++categories_shown;
		...
		int64 entries_to_show = std::min<int64>(kMaxToShow,
		...
		for (int64 i = 0; i < entries_to_show; ++i) {
			AppendLine(kIndentPrefix,
					MetricPercent(category.entries[i]->metric), " ",
					category.entries[i]->short_text);
		}
		const int64 remaining_entries = category.entries.size()
				- entries_to_show;
	...
	if (remaining_categories > 0) {
		AppendTableRow(
				tensorflow::strings::StrCat("... (", remaining_categories,
						" more categories)"), expected_metric_sum_ - metric_sum,
				expected_metric_sum_);
	}
}
...
void MetricTableReport::AppendEntryTable()
{
	...
	if (remaining_entries > 0) {
		AppendTableRow(
				tensorflow::strings::StrCat("... (", remaining_entries,
						" more ", entry_name_, ")"),
