...
public:
	explicit DecodeJpegOp(OpKernelConstruction* context) :
			OpKernel(context)
	{
		OP_REQUIRES_OK(context,
				context->GetAttr("channels", &flags_.components));
		OP_REQUIRES(context,
				flags_.components == 0 || flags_.components == 1
						|| flags_.components == 3,
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument("ratio must be 1, 2, 4, or 8, got ",
						flags_.ratio));
		OP_REQUIRES_OK(context,
				context->GetAttr("fancy_upscaling", &flags_.fancy_upscaling));
		OP_REQUIRES_OK(context,
	...
	void Compute(OpKernelContext* context) override
	{
		...
		Tensor* output = NULL;
		OP_REQUIRES(context,
				jpeg::Uncompress(input.data(), input.size(), flags_,
						nullptr /* nwarn */,
				...
				errors::InvalidArgument("Invalid JPEG data, size ",
						input.size()));
	}
