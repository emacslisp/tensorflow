...
Status ReplaceSendRecvs(const GraphDef& original_graph_def,
		const GraphDef& rewritten_graph_def, const std::vector<string>& inputs,
		const std::vector<string>& outputs, GraphDef* output_graph_def);
...
protected:
	void TestSimpleAdd()
	{
		...
		Output a_const = Const(root.WithOpName("a_expect_removed"),
				Input::Initializer(a_data));

		Tensor b_data(DT_FLOAT, TensorShape( { width }));
		test::FillIota<float>(&b_data, 1.0f);
		Output b_const = Const(root.WithOpName("b_expect_removed"),
				Input::Initializer(b_data));

		Output add = Add(root.WithOpName("add_expect_removed"), a_const,
				b_const);

		Output placeholder = Placeholder(
				root.WithOpName("placeholder_expect_remains"), DT_FLOAT);

		Output mul = Mul(root.WithOpName("output_expect_remains"), add,
				placeholder);
		...
		TestConstantFolding(graph_def, { { "placeholder_expect_remains",
				placeholder_tensor } }, { "output_expect_remains" });
	}
			...
			std::vector<string> outputs)
	{
		...
		TF_ASSERT_OK(
				unfolded_session->Run(inputs, outputs, { }, &unfolded_tensors));
		...
		TF_ASSERT_OK(
				graph_transforms::FoldConstants(graph_def, context,
						&folded_graph_def));
		...
		TF_ASSERT_OK(
				folded_session->Run(inputs, outputs, { }, &folded_tensors));

		EXPECT_EQ(unfolded_tensors.size(), folded_tensors.size());
		for (int i = 0; i < unfolded_tensors.size(); ++i) {
			test::ExpectTensorNear<float>(unfolded_tensors[i],
					folded_tensors[i], 1e-5);
		}
	...
	void TestReplaceSendRecvs()
	{
		...
		Output o_a_const = Const(o_root.WithOpName("a_const"),
				Input::Initializer(a_const_data));
		...
		Output n_a_const = Const(n_root.WithOpName("a_const"),
				Input::Initializer(a_const_data));
		...
		TF_ASSERT_OK(
				graph_transforms::ReplaceSendRecvs(o_graph_def, n_graph_def, {
						"placeholder" }, { "a_const" }, &result_graph_def));
	...
	void TestRemoveUnusedNodes()
	{
		...
		Output unused_const = Const(root.WithOpName("unused"),
				Input::Initializer(unused_data));

		GraphDef graph_def;
		TF_ASSERT_OK(root.ToGraphDef(&graph_def));
		GraphDef result_graph_def;
		TF_ASSERT_OK(graph_transforms::RemoveUnusedNodes(graph_def, { {
				"placeholder" }, { "output" } }, &result_graph_def));
