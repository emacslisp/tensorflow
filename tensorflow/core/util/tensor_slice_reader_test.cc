		...
		TensorSliceReader::OpenTableFunction open_function)
{
	...
	const string filepattern = strings::StrCat(fname_base, "_*");
	const TensorSliceReader* reader = cache.GetReader(filepattern,
			open_function, TensorSliceReader::kLoadAllShards);
	...
	const TensorSliceReader* reader2 = cache.GetReader(filepattern,
			open_function, TensorSliceReader::kLoadAllShards);
...
static void VersionTest(const VersionDef& versions, const string& error)
{
	...
	EXPECT_TRUE(
			reader.status().code() == error::INVALID_ARGUMENT
					&& StringPiece(reader.status().error_message()).starts_with(
							error)) << "Expected error starting with '"
			<< errors::InvalidArgument(error) << "', got '" << reader.status()
			<< "'";
}
