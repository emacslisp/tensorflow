...
public:
	...
	{
		return retry_.AllocateRaw([this](size_t a, size_t nb, bool v) {
			mutex_lock l(mu_);
...
private:
	...
	mutex mu_;size_t
	memory_capacity_ GUARDED_BY(mu_);
	int millis_to_wait_;
};
...
private:
	void IncrementTurn() EXCLUSIVE_LOCKS_REQUIRED(mu_) {
		int skipped = 0;while (skipped < num_users_) {
...
protected:
	...
	void LaunchConsumerThreads(int num_consumers, int cap_needed)
	{
		...
		for (int i = 0; i < num_consumers; ++i) {
			consumers_.push_back(
					Env::Default()->StartThread(ThreadOptions(), "anon_thread",
							[this, i, cap_needed]() {
								do {
									void* ptr = nullptr;
	...
	mutex mu_;bool has_failed_ GUARDED_BY(mu_) = false;int count_ GUARDED_BY(mu_) = 0;
};
