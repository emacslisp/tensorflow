...
protected:
	...
	{
		TF_ASSERT_OK(
				NodeDefBuilder("myop", "ImageSummary").Input(FakeInput()).Input(
						FakeInput()).Attr("max_images", max_images).Finalize(
						node_def()));
		TF_ASSERT_OK(InitOp());
	}
	...
	void CheckAndRemoveEncodedImages(Summary* summary)
	{
		for (int i = 0; i < summary->value_size(); ++i) {
			...
			ASSERT_TRUE(value->has_image()) << "No image for value: "
					<< value->tag();
			...
			if (VLOG_IS_ON(2)) {
				...
				TF_CHECK_OK(
						WriteStringToFile(Env::Default(),
								strings::StrCat("/tmp/", value->tag(), ".png"),
