...
RecordYielder::RecordYielder(OpKernelConstruction* context,
		...
		opts_(opts), thread_(
				new thread::ThreadPool(context->env(), "record_yielder",
						1 + opts.parallelism)), epoch_(0), rnd_(opts.seed)
{
...
static Status MatchFiles(const string& patterns, std::vector<string>* filenames)
{
...
void RecordYielder::MainLoop()
{
	while (true) {
		...
		std::mt19937_64 shuffle_rnd(
				Hash64(reinterpret_cast<char*>(&epoch_), sizeof(epoch_),
						opts_.seed));
