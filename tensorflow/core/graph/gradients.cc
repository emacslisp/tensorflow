...
SymbolicGradientBuilder::SymbolicGradientBuilder(
		...
		gtl::ArraySlice<NodeOut> y_grad_node_outputs,
		std::vector<NodeOut>* x_grad_node_outputs, Graph* graph) :
		y_node_outputs_(y_node_outputs), x_node_outputs_(x_node_outputs), y_grad_node_outputs_(
				y_grad_node_outputs), x_grad_node_outputs_(x_grad_node_outputs), graph_(
				graph)
{
...

	{
		...
		for (int i = 0; i < num_y; ++i) {
			...
			for (const Edge* e : y->in_edges()) {
				if (e->IsControlEdge())
					continue;
				BackpropAlongEdge(y_grad_node_outputs_[i],
						{ e->src(), e->src_output() });
			}
		}
	}
...
Status SymbolicGradientBuilder::Compute()
{
	...
	while (!ready_.empty()) {
		...
		for (const Edge* e : n->in_edges()) {
			if (e->IsControlEdge())
				continue;
			BackpropAlongEdge( { grad, e->dst_input() },
					{ e->src(), e->src_output() });
		}
	}
