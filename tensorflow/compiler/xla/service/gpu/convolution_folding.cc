...
std::tuple<bool, std::vector<HloInstruction*>, Window,
		ConvolutionDimensionNumbers> MatchBackwardFilter(HloInstruction* conv)
{
	const auto no_match_result = std::make_tuple(false,
			std::vector<HloInstruction*>(), Window(),
	...
	for (int i = 0; i < 2; ++i) {
		...
		int64 input_size = conv->operand(0)->shape().dimensions(
				spatial_dims[i]);
		...
		int64 padded_input_size = filter_size
				+ (output_size - 1) * dim->stride();
		int64 min_padding_high = padded_input_size - input_size
				- dim->padding_low();
	...
	if (transpose == nullptr) {
		...
		transpose = parent_computation->AddInstruction(
				HloInstruction::CreateTranspose(conv->shape(), conv,
	...
	return std::make_tuple(true,
			std::vector<HloInstruction*>( { transpose, conv }),
...
// Precondition: "conv" is a kConvolution.
std::tuple<bool, std::vector<HloInstruction*>, Window,
		ConvolutionDimensionNumbers> MatchBackwardInput(HloInstruction* conv)
{
	const auto no_match_result = std::make_tuple(false,
			std::vector<HloInstruction*>(), Window(),
			ConvolutionDimensionNumbers());
	...
	if (reverse_filter->opcode() == HloOpcode::kReverse) {
		...
		for (int64 i = 0; i < kernel_spatial_dims.size(); ++i) {
			if (conv->window().dimensions(i).size() != 1) {
				VLOG(1)
						<< "The reverse filter is neither a kReverse nor a 1x1 filter: "
	...
	for (size_t i = 0; i < spatial_dims.size(); ++i) {
		...
		auto backward_padding_low = kernel_size - 1
				- old_window.dimensions(i).padding_low();
		...
		auto output_size = conv->operand(0)->shape().dimensions(
				spatial_dims[i]);
		auto padded_input_size = kernel_size
				+ dim->stride() * (output_size - 1);
	...
	if (reverse_filter->opcode() != HloOpcode::kReverse) {
		reverse_filter = reverse_filter->parent()->AddInstruction(
				HloInstruction::CreateReverse(reverse_filter->shape(),
						reverse_filter, AsInt64Slice(kernel_spatial_dims)));
	...
	return std::make_tuple(true, std::vector<HloInstruction*>( { conv,
			reverse_filter }), new_window, dnums);
}
}  // namespace
...
StatusOr<bool> ConvolutionFolding::Run(HloModule* module)
{
	...
	for (HloInstruction* conv : convs) {
		...
		std::tie(match, hlos_to_fuse, window, dnums) = MatchBackwardFilter(
				conv);
		if (match) {
