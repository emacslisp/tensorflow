...
/* static */Shape ShapeUtil::MakeShapeWithLayout(PrimitiveType element_type,
		tensorflow::gtl::ArraySlice<int64> dimensions,
...
/* static */void ShapeUtil::PopulateShape(PrimitiveType element_type,
		tensorflow::gtl::ArraySlice<int64> dimensions, Shape* shape)
{
	shape->Clear();
...
{
	return IsTuple(shape)
			&& std::any_of(shape.tuple_shapes().begin(),
...
/* static */bool ShapeUtil::ShapeIs(const Shape& shape,
		PrimitiveType element_type, std::initializer_list<int64> dimensions)
{
	TF_DCHECK_OK(ValidateShapeWithOptionalLayout(shape));
	...
	for (auto& shape : program_shape.parameters()) {
		...
		parameters.push_back(
				tensorflow::strings::StrCat(
						i < program_shape.parameter_names_size() ?
...
/* static */StatusOr<Shape> ShapeUtil::ParseShapeString(const string& s)
{
	...
	if (RE2::FullMatch(s, "([fsu]32)\\[([\\d,]*)\\](?: {([\\d,]*)})?",
		...
		if (element_type_string == "f32") {
			...
			LOG(FATAL) << "unhandled element type string: "
					<< element_type_string;
		}
		Shape result;
		if (layout_string.empty()) {
			...
			result = ShapeUtil::MakeShapeWithLayout(primitive_type, dimensions,
					min2maj);
		}
...
/* static */int64 ShapeUtil::ByteSizeOf(const Shape& shape, int64 pointer_size)
{
	TF_DCHECK_OK(ValidateShape(shape));
	...
	if (shape.layout().padded_dimensions_size() > 0) {
		CHECK_EQ(ShapeUtil::Rank(shape),
				shape.layout().padded_dimensions_size());
	...
	if (ShapeUtil::Rank(shape) != shape.dimensions_size()) {
		return InvalidArgument(
				"shape's rank is mismatched with dimension count; rank=%lld "
						"dimensions_size=%d", ShapeUtil::Rank(shape),
...
/* static */Shape ShapeUtil::StripDegenerateDimensions(const Shape& shape)
{
	...
	for (int64 i : shape.layout().minor_to_major()) {
		if (std::find(degenerate_dimensions.begin(),
				degenerate_dimensions.end(), i)
				== degenerate_dimensions.end()) {
...

	{
		...
		DCHECK(
				minor_to_major.size() == dims.size()
						&& std::is_permutation(minor_to_major.begin(),
								minor_to_major.end(), dims.begin()));
	}
	Shape stripped_shape =
			shape.has_layout() ?
					MakeShapeWithLayout(shape.element_type(), dimension_sizes,
							minor_to_major) :
...
Status ForEachSubshapeHelper(const Shape& shape,
		const ShapeUtil::VisitorFunction func, ShapeIndex* index)
{
		...
		for (int64 i = 0; i < ShapeUtil::TupleElementCount(shape); ++i) {
			...
			TF_RETURN_IF_ERROR(
					ForEachSubshapeHelper(
							ShapeUtil::GetTupleElementShape(shape, i), func,
							index));
			index->pop_back();
		}
	}
...
Status ForEachMutableSubshapeHelper(Shape* shape,
		const ShapeUtil::MutatingVisitorFunction func, ShapeIndex* index)
{
	TF_RETURN_IF_ERROR(func(shape, *index));
	if (ShapeUtil::IsTuple(*shape)) {
		for (int64 i = 0; i < ShapeUtil::TupleElementCount(*shape); ++i) {
			index->push_back(i);
			TF_RETURN_IF_ERROR(
					ForEachMutableSubshapeHelper(shape->mutable_tuple_shapes(i),
							func, index));
			index->pop_back();
		}
	}
...
/* static */Status ShapeUtil::ForEachMutableSubshape(Shape* shape,
...
/* static */std::tuple<bool, std::vector<int64>, std::vector<int64>> ShapeUtil::InsertedOrDeleted1SizedDimensions(
		const Shape& shape_pre, const Shape& shape_post)
{
	auto nil = std::make_tuple(false, std::vector<int64>(),
			std::vector<int64>());
	...
	auto check_modified_dims =
			[&shape_pre, &shape_post, &deleted_indices,
			&inserted_indices](
	...
	for (size_t i = 0; i <= unmodified_dims.size(); ++i) {
		...
		if (!check_modified_dims(prior_unmodified_dim_pair,
				unmodified_dim_pair)) {
			return nil;
		}
	}
...
/* static */std::vector<std::pair<int64, int64>> ShapeUtil::DimensionsUnmodifiedByReshape(
		const Shape& input_shape, const Shape& output_shape)
{
	...
	for (size_t i = 0; i < common_factors.size() - 1;) {
		if (1 != common_factors[i + 1].first - common_factors[i].first
				|| 1
						!= common_factors[i + 1].second
								- common_factors[i].second) {
...
/* static */bool ShapeUtil::TransposeIsBitcast(const Shape& input_shape,
		...
		tensorflow::gtl::ArraySlice<int64> dimension_mapping)
{
	...
	if (LayoutUtil::IsPadded(input_shape)
			&& LayoutUtil::IsPadded(output_shape)) {
		return false;
	}
...
/* static */Shape ShapeUtil::DeleteDimension(int64 dim_to_delete, Shape shape)
{
	shape.mutable_dimensions()->erase(
			shape.dimensions().begin() + dim_to_delete);
	if (LayoutUtil::HasLayout(shape)) {
