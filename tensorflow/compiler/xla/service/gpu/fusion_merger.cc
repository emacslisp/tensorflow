...
double CalculateBytesReadByFusionParameter(HloInstruction* param)
{
	...
	for (auto& instruction : instructions) {
		if (std::all_of(instruction->users().begin(),
...
double GetCurrentBytesTransferred(HloInstruction* fusion)
{
	...
	const double bytes_written = ShapeUtil::ByteSizeOf(
...
Status FusionInstructionMerger::Run()
{
	...
	VLOG(1) << "FusionInstructionMerger EXIT" << " computation: "
			<< computation_->name() << " total_visited: " << total_visited_
			<< " total_merged: " << total_merged_ << " merge failures { "
			<< " no_users: " << num_fail_no_users_ << " not_loop_fusion: "
			<< num_fail_not_loop_fusion_ << " merge_all_users: "
			<< num_fail_merge_all_users_ << " expensive_instruction: "
			<< num_fail_expensive_fused_instruction_ << " flops_to_byte_ratio: "
			<< num_fail_flops_to_byte_ratio_ << " net_bytes_transferred: "
...
Status FusionInstructionMerger::HandleFusion(HloInstruction* fusion)
{
	...
	const double merged_to_current_bytes_ratio = merged_bytes_transferred
			/ std::max(1.0, current_bytes_transferred);
