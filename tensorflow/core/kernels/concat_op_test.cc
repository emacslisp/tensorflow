...
static void ConcatHelper(int iters, int concat_dimension, int dim2)
{
	...
	Node* node;
	TF_CHECK_OK(
			NodeBuilder(g->NewName("n"), "Concat").Input(
					test::graph::Constant(g, concat_dim)).Input(
					{ test::graph::Constant(g, in0), test::graph::Constant(g,
							in1) }).Attr("N", 2).Attr("T", dt).Finalize(g,
					&node));
...
static void ConcatManyHelper(int iters, int concat_dimension, int dim2)
{
	...
	TF_CHECK_OK(
			NodeBuilder(g->NewName("n"), "Concat").Input(
					test::graph::Constant(g, concat_dim)).Input(inputs).Attr(
					"N", 64).Attr("T", dt).Finalize(g, &node));
	testing::BytesProcessed(
			static_cast<int64>(iters) * kDim1 * dim2 * kNumInputs * sizeof(T));
	testing::StartTiming();
	test::Benchmark("cpu", g).Run(iters);
	testing::UseRealTime();
}
...
static void MemcpyManyAlternative1(int iters, int dim2)
{
	...
	testing::BytesProcessed(
			static_cast<int64>(iters) * kDim1 * dim2 * kNumCopies
					* sizeof(bfloat16));
	testing::StartTiming();
	while (iters-- > 0) {
