			...
			if (IsEnter(out)) {
				if (is_visited) {
					...
					if (parent_frame != frame_name) {
						return errors::InvalidArgument("The node '",
								out->name(), "' has inputs from different "
										"frames. The input '",
								curr_node->name(), "' is in frame '",
								frame_name, "'. The input '",
								parent_nodes[out->id()]->name(),
					...
					TF_RETURN_IF_ERROR(
							GetNodeAttr(out->def(), "frame_name",
									&out_info->frame_name));
					if (out_info->frame_name.empty()) {
						return errors::InvalidArgument("The Enter node ",
								out->name(), " must have a frame name.");
					}
				}
			} else {
				if (is_visited) {
