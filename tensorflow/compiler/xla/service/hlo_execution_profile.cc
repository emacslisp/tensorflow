		...
		const HloCostAnalysis& cost_analysis) const
{
	...
	tensorflow::strings::StrAppend(&result,
			tensorflow::strings::Printf(
					"HLO execution profile: (%s @ f_nom)\n\t",
					tensorflow::strings::HumanReadableElapsedTime(
							total_cycles / clock_rate_ghz / 1e9).c_str()));
	append_item(total_cycles, -1, "[total]");
	for (const auto& item : items) {
		tensorflow::strings::StrAppend(&result, "\n\t");
		auto flops =
				item.first == nullptr ?
						-1 : cost_analysis.hlo_to_flop_count(*item.first);
		string display =
				item.first == nullptr ? "<none>" : item.first->ToString();
