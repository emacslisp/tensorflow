...
public:
	ReadVariableOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
...
public:
	AssignVariableOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				LookupOrCreateResource<Var>(context,
						HandleFromInput(context, 0), &variable,
...
public:
	AssignAddVariableOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		add_functor(context->eigen_device<Device>(),
				variable->tensor()->flat<T>(), value.flat<T>());
	}
};
...
public:
	explicit ResourceGatherOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	...
	void Compute(OpKernelContext* c) override
	{
		...
		if (N > 0) {
			...
			OP_REQUIRES(c, bad_i < 0,
					errors::InvalidArgument("indices",
...
public:
	explicit ResourceScatterUpdateOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
	...
	void Compute(OpKernelContext* c) override
	{
		...
		if (N > 0) {
			auto indices_flat = indices.flat<Index>();
			auto params_flat = params->flat_outer_dims<T>();
			auto updates_flat = updates.shaped<T, 2>(
					{ N, updates.NumElements() / N });
