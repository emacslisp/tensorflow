		...
		llvm::Module* llvm_module)
{
	...
	llvm::Function* ir_kernel = llvm_module->getFunction(
			kernel_thunk->kernel_name().c_str());
	...
	nvvm_annotations_node->addOperand(
			llvm::MDNode::get(llvm_context,
					{ llvm::ConstantAsMetadata::get(ir_kernel),
							llvm::MDString::get(llvm_context, "maxntidx"),
							llvm::ConstantAsMetadata::get(
									threads_per_block_ir_value) }));
}
}  // namespace

IrEmitterUnnested::IrEmitterUnnested(const HloModuleConfig& hlo_module_config,
		const HloComputation* hlo_computation, bool has_hybrid_result,
		IrEmitterContext* ir_emitter_context) :
		IrEmitter(hlo_module_config, ir_emitter_context, /*is_nested=*/false), hlo_computation_(
				hlo_computation), has_hybrid_result_(has_hybrid_result)
{
		...
		tensorflow::gtl::ArraySlice<const HloInstruction*> escaped_hlos)
{
	...
	llvm::FunctionType* kernel_type = llvm::FunctionType::get(
			...
			std::vector<llvm::Type*>(num_escaped_hlos + 1,
					ir_builder_.getInt8PtrTy()), false); // Not a variadic argument function.
	llvm::Function* kernel = llvm::Function::Create(kernel_type,
			llvm::GlobalValue::ExternalLinkage, kernel_name.c_str(), module);
	...
	for (size_t arg_no = 0; arg_no < escaped_hlos.size(); ++arg_no) {
		...
		int64 escaped_hlo_size = llvm_ir::ByteSizeOf(escaped_hlo_shape,
	...
	llvm::NamedMDNode* nvvm_annotations_node = module->getOrInsertNamedMetadata(
			"nvvm.annotations");
	nvvm_annotations_node->addOperand(
			llvm::MDNode::get(context, { llvm::ConstantAsMetadata::get(kernel),
					llvm::MDString::get(context, "kernel"),
					llvm::ConstantAsMetadata::get(ir_builder_.getInt32(1)) }));

	// Update the insert point to the entry basic block.
	llvm::BasicBlock* entry_bb = llvm::BasicBlock::Create(context, "entry", // The name of the basic block.
...
Status IrEmitterUnnested::HandleDot(HloInstruction* dot,
		HloInstruction* lhs_instruction, HloInstruction* rhs_instruction)
{
...
Status IrEmitterUnnested::HandleFusion(HloInstruction* fusion)
{
	...
	if (HloInstruction::FusionKind::kInput == fusion->fusion_kind()) {
		...
		case HloOpcode::kReduce: {
			VLOG(3) << "Emitting fused reduction to vector: "
					<< fusion->ToString();
			std::vector<std::unique_ptr<Thunk>> thunks;
			thunks.emplace_back(BuildKernelThunk(fusion));
			TF_RETURN_IF_ERROR(
					EmitInitializer(fusion,
			...
			GpuElementalIrEmitter elemental_emitter(hlo_module_config_,
					ir_emitter_context_->llvm_module(), &ir_builder_,
					GetNestedComputer());
			...
			return EmitReductionToVector(root, input_shape,
					fused_emitter.GetGenerator(root->operand(0)),
					fused_emitter.GetGenerator(root->operand(1)),
					root->dimensions(), root->to_apply());
			break;
		}
		default:
		...
		const Shape& shape)
{
	...
	for (size_t i = 1; i <= segs.size(); ++i) {
		dimensions.push_back(
				std::accumulate(shape.dimensions().begin() + segs[i - 1],
						shape.dimensions().begin()
								+ (segs.size() == i ?
										shape.dimensions().size() : segs[i]), 1,
						std::multiplies<int64>()));
	}
	return ShapeUtil::MakeShapeWithMonotonicDim0MajorLayout(
...
bool AreShapesForTranspose021(const Shape& a, const Shape& b)
{
	return 3 == b.dimensions().size()
			&& ShapeUtil::Compatible(
					...
					ShapeUtil::PermuteDimensions( { 0, 2, 1 },
							ShapeUtil::NormalizeShapeToMonotonicDim0MajorLayout(
									b)));
}
		...
		const int64 tile_size, llvm::IRBuilder<>* builder)
{
	...
	Shape input_shape = ShapeUtil::NormalizeShapeToMonotonicDim0MajorLayout(
			input.GetShape());
	Shape output_shape = ShapeUtil::NormalizeShapeToMonotonicDim0MajorLayout(
			output.GetShape());
	...
	// array.
	auto emit_cp_tile =
			[builder, tile_size, x, &offset_dim](
	...
	for (int i = 1; i < 3; ++i) {
		input_dims_in_tiles[i] = CeilOfRatio<int64>(input_dims_in_tiles[i],
				tile_size);
	}
	int64 num_tiles = std::accumulate(input_dims_in_tiles.begin(),
			input_dims_in_tiles.end(), 1, std::multiplies<int64>());
			...
			/*linear=*/builder->CreateIntCast(
					llvm_ir::AddRangeMetadata(0, num_tiles,
							static_cast<llvm::Instruction*>(llvm_ir::EmitCallToIntrinsic(
									...
									{ }, { }, builder))), builder->getInt64Ty(), /*isSigned=*/
					true, "block.id.x"),
			ShapeUtil::MakeShapeWithMonotonicDim0MajorLayout(PRED /*arbitrary*/,
					AsInt64Slice(input_dims_in_tiles)), builder);
	...
	const llvm_ir::IrArray::Index input_index = offset_dim(input_tile_origin, x, /*dim=*/
			2);
	...
	const llvm_ir::IrArray::Index output_index = offset_dim(output_tile_origin,
			x, /*dim=*/2);
		...
		HloInstruction* operand)
{
	...
	if (is_transpose_021
			&& reduced_input_shape.dimensions(1)
					>= kMinDimensionToTransposeTiled
			&& reduced_input_shape.dimensions(2)
					>= kMinDimensionToTransposeTiled) {
		...
		int64 num_tiles = EmitTranspose021Tiled(
				GetIrArray(*operand).CastToShape(reduced_input_shape,
						&ir_builder_),
				GetIrArray(*copy).CastToShape(reduced_output_shape,
						&ir_builder_), tile_size, &ir_builder_);
		UpdateLaunchDimensions(LaunchDimensions(num_tiles, tile_size),
...
Status IrEmitterUnnested::EmitColumnReduction(int64 height, int64 width,
		...
		HloComputation* reducer)
{
	...
	Shape tiled_input_shape = ShapeUtil::MakeShapeWithLayout(
			reduce->shape().element_type(), { height_in_tiles, width },
			{ 1, 0 });
	...
	return ParallelLoopEmitter(loop_body_emitter, tiled_input_shape,
			launch_dimensions, &ir_builder_).EmitLoop();
}

Status IrEmitterUnnested::EmitRowReduction(int64 depth, int64 height,
		int64 width, HloInstruction* reduce, const Shape& input_shape,
		const llvm_ir::ElementGenerator& input_gen,
		const llvm_ir::ElementGenerator& init_value_gen,
		HloComputation* reducer)
{
	...
	const int64 width_in_tiles = RoundUpToNearest(CeilOfRatio(width, kTileSize),
			kWarpSize);
...
//               elementwise.
Status IrEmitterUnnested::EmitReductionToVector(HloInstruction* reduce,
		...
		HloComputation* reducer)
{
	...
	CHECK(input_shape.has_layout()) << "LayoutAssignment or InstructionFusion "
			"doesn't set the input layout of " << reduce->ToString();
	...
	if (input_dims_to_keep.empty()
		...
		for (int64 input_dim = 0; input_dim < ShapeUtil::Rank(input_shape);
				++input_dim) {
			if (!std::count(input_dims_to_keep.begin(),
					input_dims_to_keep.end(), input_dim)) {
				height *= input_shape.dimensions(input_dim);
			}
		}
		return EmitColumnReduction(height, width, reduce, input_shape,
		...
		for (int64 input_dim = 0; input_dim < ShapeUtil::Rank(input_shape);
			...
			if (PositionInContainer(input_shape.layout().minor_to_major(),
				...
				depth *= input_shape.dimensions(input_dim);
					...
					input_shape.layout().minor_to_major(), input_dim)
					< PositionInContainer(input_shape.layout().minor_to_major(),
							input_dims_to_keep.front())) {
...
Status IrEmitterUnnested::HandleReduce(HloInstruction* reduce,
		HloInstruction* input, HloInstruction* init_value,
		tensorflow::gtl::ArraySlice<int64> dimensions_to_reduce,
		HloComputation* reducer)
{
	...
	// NVPTX backend can't do atomic cmpxchg any narrower than 32 bits
		...
		TF_RETURN_IF_ERROR(
				EmitInitializer(reduce,
						static_cast<KernelThunk*>(thunks.back().get())));
		bindings_.UnbindAllLocalIrValues();
		thunks.emplace_back(BuildKernelThunk(reduce));
		thunk_sequence_->emplace_back(
				MakeUnique<SequentialThunk>(std::move(thunks), reduce));
		return EmitReductionToVector(reduce, input->shape(),
		...
		tensorflow::gtl::ArraySlice<HloInstruction*> operands)
{
	bool all_tuple_elements_have_buffer =
			std::all_of(operands.begin(), operands.end(),
					[this](HloInstruction* tuple_element) {
	...
	if (all_tuple_elements_have_buffer) {
		...
		thunk_sequence_->emplace_back(
				MakeUnique<TupleThunk>(tuple_element_buffers,
						GetAllocationIndex(*tuple), tuple));
		return Status::OK();
	}
	...
	if (has_hybrid_result_
			&& ReachRootViaOnlyTuples(*tuple,
					*hlo_computation_->root_instruction())) {
		return Status::OK();
	}
		...
		HloInstruction* select_and_scatter)
{
...

	{
		...
		TF_RETURN_IF_ERROR(
				EmitInitializer(select_and_scatter,
						static_cast<KernelThunk*>(thunks.back().get())));
		bindings_.UnbindAllLocalIrValues();
		thunks.emplace_back(BuildKernelThunk(select_and_scatter));
		thunk_sequence_->emplace_back(
				MakeUnique<SequentialThunk>(std::move(thunks),
						select_and_scatter));
	}
...
Status IrEmitterUnnested::HandleWhile(HloInstruction* xla_while,
		HloInstruction* init, HloComputation* condition, HloComputation* body)
{
	TF_RET_CHECK(
	...
	if (result.ok()) {
		...
		const int64 loop_trip_count = (std::get<1>(tuple) - std::get<0>(tuple)
				+ std::get<2>(tuple) - 1) / std::get<2>(tuple);
		thunk_sequence_->emplace_back(
				BuildForThunk(xla_while, loop_trip_count));
		...
		const HloInstruction& hlo, std::vector<const HloInstruction*>* io_hlos)
{
	...
	for (const HloInstruction* operand : hlo.operands()) {
		...
		if (buffer_assignment.HasTopLevelAllocation(to_lookup)
				&& buffer_assignment.GetUniqueTopLevelAllocation(to_lookup).ConsumeValueOrDie()->IsInputOrOutput()) {
	...
	if (buffer_assignment.HasTopLevelAllocation(&hlo)
			&& buffer_assignment.GetUniqueTopLevelAllocation(&hlo).ConsumeValueOrDie()->IsInputOrOutput()) {
		...
		const HloInstruction* inst)
{
	...
	llvm::Function* kernel = EmitBasePointersForHloAndItsOperands(*inst,
			&io_hlos);
		...
		const HloInstruction* inst)
{
	...
	/*destination_buffer=*/GetAllocationIndex(*inst),
			/*mem_size=*/llvm_ir::ByteSizeOf(operand->shape(),
					ir_emitter_context_->llvm_module()->getDataLayout()), inst);
}
		...
		const HloInstruction* inst)
{
	if (inst->opcode() == HloOpcode::kDot) {
		...
		return MakeUnique<GemmThunk>(GetAllocationIndex(*lhs), // The buffer assigned to LHS.
	...
	if (inst->opcode() == HloOpcode::kFusion) {
		...
		const HloInstruction* lhs = inst->operand(
				lhs_parameter->parameter_number());
		const HloInstruction* rhs = inst->operand(
		...
		KernelThunk* thunk)
{
	...
	return EmitTargetElementLoopInThunk(*hlo,
...
Status CheckWhileBuffersShareAllocation(const HloInstruction* xla_while,
		const BufferAssignment& buffer_assignment)
{
	return ShapeUtil::ForEachSubshape(xla_while->shape(),
		...
		const HloInstruction* hlo)
{
	...
	IrEmitterUnnested ir_emitter_body(hlo_module_config_, body,
			false /* has_hybrid_result */, ir_emitter_context_);
...
{
	LaunchDimensions launch_dimensions = CalculateLaunchDimensions(hlo.shape(),
...
Status IrEmitterUnnested::EmitTargetElementLoop(const HloInstruction& hlo,
