...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				st.in_height < (1 << 24) && st.in_width < (1 << 24),
		...
		for (int b = 0; b < st.batch_size; ++b) {
			for (int y = 0; y < st.out_height; ++y) {
				const int64 in_y = std::min(
						static_cast<int64>(floorf(y * st.height_scale)),
						(st.in_height - 1));
				for (int x = 0; x < st.out_width; ++x) {
					const int64 in_x = std::min(
							static_cast<int64>(floorf(x * st.width_scale)),
							(st.in_width - 1));
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(0, TensorShape( { input.dim_size(0),
						sizes(0), sizes(1), input.dim_size(3) }), &output));
		...
		const float height_scale = CalculateResizeScale(out_height, in_height,
				align_corners_);
		const float width_scale = CalculateResizeScale(out_width, in_width,
				align_corners_);
		output_data.setZero();
			...
			for (int y = 0; y < in_height; ++y) {
				...
				for (int x = 0; x < in_width; ++x) {
					...
					for (int b = 0; b < batch_size; ++b) {
						output_data(b, out_y, out_x, c) += input_data(b, y, x,
								c);
					}
				}
			}
