		...
		const TransformFuncContext& context, GraphDef* output_graph_def)
{
	...
	for (const NodeDef& node : input_graph_def.node()) {
		for (const string& input : node.input()) {
			...
			const string output_index_string = suffix.substr(1,
					suffix.size() - 1);
			int32 output_index;
			if (!strings::safe_strto32(output_index_string, &output_index)) {
				return errors::InvalidArgument(
						"Couldn't understand output number in ", input);
			}
	...
	for (const NodeDef& node : input_graph_def.node()) {
		...
		if ((!has_ops || op_matches) && (!has_prefixes || prefix_matches)) {
			...
			TF_RETURN_IF_ERROR(
					GetInOutTypes(node, &input_types, &output_types));
			...
			for (int output_index : node_outputs[node.name()]) {
				print_node->add_input(
						strings::StrCat(node.name(), ":", output_index));
			}
			...
			inputs_to_rename[node.name() + ":0"] = strings::StrCat(node.name(),
					name_suffix, ":0");
		}
	}
