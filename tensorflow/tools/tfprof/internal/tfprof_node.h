...
public:
	TFNode(const NodeDef* node) :
			node_(node), step_stat_(nullptr), op_start_micros_(0), op_exec_micros_(
					0), all_spent_micros_(0), requested_bytes_(0), float_ops_(0)
	{
		if (!node)
			return;
	...
	void AddInput(TFNode* input)
	{
		inputs_[input->node_def()->name()] = input;
	}
	...
	void AddFloatOps(int64 float_ops)
	{
	...
	const std::map<string, TFNode*>& inputs()
	{
		return inputs_;
	}
	int64 op_start_micros()
	{
		return op_start_micros_;
	}
	int64 op_exec_micros()
	{
		return op_exec_micros_;
	}
	int64 all_spent_micros()
	{
		return all_spent_micros_;
	}
	int64 requested_byptes()
	{
	...
	void update_shape(const std::vector<int64>& shape)
	{
		shape_ = shape;
	}
