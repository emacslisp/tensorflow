...
ZlibInputStream::ZlibInputStream(InputStreamInterface* input_stream,
		...
		input_stream_(input_stream), input_buffer_capacity_(input_buffer_bytes), output_buffer_capacity_(
				output_buffer_bytes), z_stream_input_(
				new Bytef[input_buffer_capacity_]), z_stream_output_(
				new Bytef[output_buffer_capacity_]), zlib_options_(
				zlib_options), z_stream_(new z_stream)
{
	InitZlibBuffer();
}
...
Status ZlibInputStream::ReadFromStream()
{
	...
	if (z_stream_->avail_in > 0) {
		...
		if (read_bytes > 0) {
			memmove(z_stream_input_.get(), z_stream_->next_in,
					z_stream_->avail_in);
		}
...
size_t ZlibInputStream::ReadBytesFromCache(size_t bytes_to_read, string* result)
{
	size_t unread_bytes = reinterpret_cast<char*>(z_stream_->next_out)
			- next_unread_byte_;
...
{
	size_t read_bytes = next_unread_byte_
			- reinterpret_cast<char*>(z_stream_output_.get());
...
int64 ZlibInputStream::Tell() const
{
	return -1;
}
