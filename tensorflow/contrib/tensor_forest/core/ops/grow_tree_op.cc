...
public:
	explicit GrowTree(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				finished.shape().dim_size(0) == best_splits.shape().dim_size(0),
				errors::InvalidArgument(
		...
		if (!CheckTensorBounds(context, finished))
			return;
		if (!CheckTensorBounds(context, best_splits))
			return;
		...
		const auto split_thresholds =
				candidate_split_thresholds.tensor<float, 2>();
		...
		const int32 num_accumulators =
				static_cast<int32>(candidate_split_features.shape().dim_size(0));
		const int32 num_splits =
				static_cast<int32>(candidate_split_features.shape().dim_size(1));
		...
		int32 nodes_we_can_allocate = std::min(num_finished,
		...
		auto threshold_updates_flat =
				threshold_updates_tensor->tensor<float, 1>();
		...
		for (int32 i = 0; i < nodes_we_can_allocate; i++) {
			...
			OP_REQUIRES(context, FastBoundsCheck(node, node_map.size()),
					errors::InvalidArgument(
							"finished node not in valid range."))
			...
			tree_updates_flat(output_slot, FEATURE_INDEX) = split_features(
					accumulator, best);
			threshold_updates_flat(output_slot) = split_thresholds(accumulator,
					best);
			output_slot++;
		...
		OP_REQUIRES_OK(context,
				context->allocate_output(3, new_end_of_tree_shape,
