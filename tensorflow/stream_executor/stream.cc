...
string ToVlogString(blas::Transpose t)
{
	return blas::TransposeString(t);
}

string ToVlogString(blas::UpperLower ul)
{
	return blas::UpperLowerString(ul);
}

string ToVlogString(blas::Diagonal d)
{
	return blas::DiagonalString(d);
}

string ToVlogString(blas::Side s)
{
...
string ToVlogString(int i)
{
	return port::StrCat(i);
}

string ToVlogString(uint32 i)
{
	return port::StrCat(i);
}

string ToVlogString(uint64 i)
{
	return port::StrCat(i);
}

string ToVlogString(int64 i)
{
	return port::StrCat(i);
}

string ToVlogString(float f)
{
	return port::StrCat(f);
}

string ToVlogString(double d)
{
	return port::StrCat(d);
}
...
Stream::Stream(StreamExecutor *parent) :
		parent_(parent), implementation_(
				parent->implementation()->GetStreamImplementation()), allocated_(
				false), ok_(false), temporary_memory_manager_(this)
{
...
Stream::Stream(StreamExecutor *parent,
		...
		parent_(parent), implementation_(implementation), allocated_(false), ok_(
				false), temporary_memory_manager_(this)
{
...
Stream &Stream::ThenRecordEvent(Event *event)
{
	...
	if (!status.ok()) {
		LOG(ERROR) << "Error recording event in stream: "
...
Stream &Stream::ThenBatchNormalizationForward(const DeviceMemory<float> &x,
		...
		std::function<void()> inv_var_to_var)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoBatchNormalizationForward(this, x, scale, offset,
							estimated_mean, estimated_variance, x_desc,
							scale_offset_desc, epsilon, y, batch_mean,
							batch_var, saved_mean, saved_inv_var, is_training,
							std::move(var_to_inv_var),
		...
		DeviceMemory<float> *offset_backprop)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoBatchNormalizationBackward(this, y_backprop, x,
							scale, mean, variance, x_desc, scale_offset_desc,
							epsilon, x_backprop, scale_backprop,
							offset_backprop));
		} else {
			SetError();
			LOG(WARNING)
		...
		dnn::ProfileResult *output_profile_result)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			auto status = dnn->DoConvolve(this, input_descriptor, input_data,
					filter_descriptor, filter_data, convolution_descriptor,
					output_descriptor, output, scratch_allocator,
...
Stream &Stream::ThenConvolve(const dnn::BatchDescriptor &input_descriptor,
		const DeviceMemory<float> &input_data,
		const dnn::FilterDescriptor &filter_descriptor,
		const DeviceMemory<float> &filter_data,
		const dnn::ConvolutionDescriptor &convolution_descriptor,
		...
		DeviceMemory<float> *output)
{
	return ThenConvolveWithScratch(input_descriptor, input_data,
			filter_descriptor, filter_data, convolution_descriptor,
			output_descriptor, output, /*scratch_allocator=*/nullptr);
}
		...
		DeviceMemory<float> *output)
{
	VLOG_CALL(PARAM(batch_descriptor), PARAM(input_data),
			PARAM(filter_descriptor), PARAM(depth_multiplier),
			PARAM(first_weights), PARAM(second_weights),
			PARAM(convolution_descriptor), PARAM(output_descriptor),
			PARAM(output));
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoSeparableConvolve(this, batch_descriptor, input_data,
							filter_descriptor, depth_multiplier, first_weights,
							second_weights, convolution_descriptor,
		...
		ScratchAllocator *scratch_allocator)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoConvolveBackwardData(this, filter_descriptor,
							filter_data, output_descriptor,
							backward_output_data, convolution_descriptor,
							input_descriptor, backward_input_data,
							scratch_allocator, dnn::AlgorithmConfig(),
							nullptr));
		} else {
			SetError();
			LOG(WARNING)
		...
		dnn::ProfileResult *output_profile_result)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			auto status = dnn->DoConvolveBackwardData(this, filter_descriptor,
					filter_data, output_descriptor, backward_output_data,
...
Stream &Stream::ThenConvolveBackwardData(
		...
		DeviceMemory<float> *backward_input_data)
{
	return ThenConvolveBackwardDataWithScratch(filter_descriptor, filter_data,
		...
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoConvolveBackwardFilter(this, input_descriptor,
							input_data, output_descriptor, backward_output_data,
		...
		dnn::ProfileResult *output_profile_result)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			auto status = dnn->DoConvolveBackwardFilter(this, input_descriptor,
					input_data, output_descriptor, backward_output_data,
					convolution_descriptor, filter_descriptor,
...
Stream &Stream::ThenConvolveBackwardFilter(
		...
		DeviceMemory<float> *backward_filter_data)
{
	return ThenConvolveBackwardFilterWithScratch(input_descriptor, input_data,
			output_descriptor, backward_output_data, convolution_descriptor,
...
{
	VLOG_CALL(PARAM(input_descriptor), PARAM(input_data),
			PARAM(bias_descriptor), PARAM(backward_bias_data));

	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoConvolveBackwardBias(this, input_descriptor,
							input_data, bias_descriptor, backward_bias_data));
		} else {
			SetError();
			LOG(WARNING)
					<< "attempting to perform DNN operation using StreamExecutor "
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoMatMul(this, input_data, weights, input_dimensions,
...
Stream &Stream::ThenMatMulQuantized(const DeviceMemory<float> &input_data,
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoMatMulQuantized(this, input_data, weights,
							weight_scales, input_dimensions, output_dimensions,
							output_data));
		} else {
			SetError();
			LOG(WARNING)
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoBiasAdd(this, input_data, biases, dimensions,
							output_data));
		} else {
			SetError();
			LOG(WARNING)
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoPoolForward(this, pooling_dimensions,
		...
		DeviceMemory<float> *output_diff_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoPoolBackward(this, pooling_dimensions,
							input_dimensions, input_data, output_dimensions,
...
{
	VLOG_CALL(PARAM(normalize_descriptor), PARAM(input_data),
			PARAM(output_data));

	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoNormalize(this, normalize_descriptor, input_data,
							output_data));
		} else {
			SetError();
			LOG(WARNING)
		...
		const DeviceMemory<float> &input_data, DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoNormalizeWithDimensions(this, normalize_descriptor,
							dimensions, input_data, output_data));
		} else {
			SetError();
			LOG(WARNING)
					<< "attempting to perform DNN operation using StreamExecutor "
		...
		DeviceMemory<float> *raw_variable_gradient)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoNormalizeBackwardWithDimensions(this,
							normalize_descriptor, dimensions, raw_data,
							normalized_data, normalized_variable_gradient,
							raw_variable_gradient));
		} else {
			SetError();
			LOG(WARNING)
					<< "attempting to perform DNN operation using StreamExecutor "
		...
		const DeviceMemory<float> &input_data, DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoActivate(this, activation_mode, dimensions,
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoDepthConcatenate(this, input_dimensions, input_data,
							output_data));
		} else {
			SetError();
			LOG(WARNING)
...
Stream &Stream::ThenElementwiseOperate(dnn::ElementwiseOperation operation,
		port::ArraySlice<dnn::BatchDescriptor> input_dimensions,
		port::ArraySlice<const DeviceMemory<float> *> input_data,
		const dnn::BatchDescriptor &output_dimensions,
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoElementwiseOperate(this, operation, input_dimensions,
		...
		int64 top_pad, int64 bottom_pad, DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoXYPad(this, dimensions, input_data, left_pad,
							right_pad, top_pad, bottom_pad, output_data));
		} else {
			SetError();
			LOG(WARNING)
					<< "attempting to perform DNN operation using StreamExecutor "
		...
		DeviceMemory<float> *output_data)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoXYSlice(this, dimensions, input_data, left_trim,
		...
		dnn::QuantizedActivationMode mode, void *host_dst, uint64 size)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoMemcpyD2HQuantized(this, gpu_unquantized_src, mode,
							host_dst, size));
		} else {
			SetError();
			LOG(WARNING)
...
Stream &Stream::ThenMemcpyH2DQuantized(const void *host_src, uint64 size,
		...
		DeviceMemory<float> *gpu_unquantized_dst)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoMemcpyH2DQuantized(this, host_src, size, mode,
							gpu_unquantized_dst));
		} else {
			SetError();
			LOG(WARNING)
...
Stream *Stream::GetOrCreateSubStream()
{
	...
	sub_streams_.emplace_back(
			std::unique_ptr<Stream> { new Stream { parent_ } }, false);
...
Stream &Stream::ThenStartTimer(Timer *t)
{
	...
	if (ok()) {
		...
		LOG(INFO) << "stream " << this << " did not enqueue 'start timer': "
				<< t;
	}
	return *this;
}
...
Stream &Stream::ThenStopTimer(Timer *t)
{
	...
	if (ok()) {
		...
		LOG(INFO) << "stream " << this << " did not enqueue 'stop timer': "
				<< t;
	}
	return *this;
}
...
template<typename ... Args>
Stream &ThenBlasImpl<Args...>::operator()(Stream *stream,
		bool (blas::BlasSupport::*blas_func)(Stream *, Args...), Args ... args)
{
	if (stream->ok()) {
		...
		int incx, DeviceMemory<float> *result)
{
	...
	ThenBlasImpl<uint64, const DeviceMemory<float> &, int, DeviceMemory<float> *> impl;
...
Stream &Stream::ThenBlasAxpy(uint64 elem_count, float alpha,
		const DeviceMemory<float> &x, int incx, DeviceMemory<float> *y,
		int incy)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasAxpy, elem_count, alpha, x,
			incx, y, incy);
}

Stream &Stream::ThenBlasAxpy(uint64 elem_count, double alpha,
		const DeviceMemory<double> &x, int incx, DeviceMemory<double> *y,
		int incy)
{
	VLOG_CALL(PARAM(elem_count), PARAM(alpha), PARAM(x), PARAM(incx), PARAM(y),
			PARAM(incy));
		...
		DeviceMemory<float> *result)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasDot, elem_count, x, incx, y,
			incy, result);
}
		...
		DeviceMemory<float> *y, int incy, float c, float s)
{
	...
	ThenBlasImpl<uint64, DeviceMemory<float> *, int, DeviceMemory<float> *, int,
			float, float> impl;
	return impl(this, &blas::BlasSupport::DoBlasRot, elem_count, x, incx, y,
			incy, c, s);
}

Stream &Stream::ThenBlasRot(uint64 elem_count, DeviceMemory<double> *x,
		int incx, DeviceMemory<double> *y, int incy, double c, double s)
{
	VLOG_CALL(PARAM(elem_count), PARAM(x), PARAM(incx), PARAM(y), PARAM(incy),
			PARAM(c), PARAM(s));

	ThenBlasImpl<uint64, DeviceMemory<double> *, int, DeviceMemory<double> *,
			int, double, double> impl;
	return impl(this, &blas::BlasSupport::DoBlasRot, elem_count, x, incx, y,
			incy, c, s);
}
		...
		const DeviceMemory<double> &param)
{
	...
	ThenBlasImpl<uint64, DeviceMemory<double> *, int, DeviceMemory<double> *,
			int, const DeviceMemory<double> &> impl;
	return impl(this, &blas::BlasSupport::DoBlasRotm, elem_count, x, incx, y,
			incy, param);
}
		...
		DeviceMemory<float> *x, int incx)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasScal, elem_count, alpha, x,
			incx);
}
		...
		int incx, DeviceMemory<float> *y, int incy)
{
	...
	ThenBlasImpl<uint64, DeviceMemory<float> *, int, DeviceMemory<float> *, int> impl;
		...
		int incx, DeviceMemory<int> *result)
{
	...
	ThenBlasImpl<uint64, const DeviceMemory<float> &, int, DeviceMemory<int> *> impl;
		...
		int incx, DeviceMemory<int> *result)
{
	...
	ThenBlasImpl<uint64, const DeviceMemory<double> &, int, DeviceMemory<int> *> impl;
	return impl(this, &blas::BlasSupport::DoBlasIamax, elem_count, x, incx,
			result);
}
...
Stream &Stream::ThenBlasGbmv(blas::Transpose trans, uint64 m, uint64 n,
		uint64 kl, uint64 ku, float alpha, const DeviceMemory<float> &a,
		...
		DeviceMemory<float> *y, int incy)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasGbmv, trans, m, n, kl, ku,
			alpha, a, lda, x, incx, beta, y, incy);
}

Stream &Stream::ThenBlasGbmv(blas::Transpose trans, uint64 m, uint64 n,
		uint64 kl, uint64 ku, double alpha, const DeviceMemory<double> &a,
		...
		int incy)
{
	...
	ThenBlasImpl<blas::Transpose, uint64, uint64, uint64, uint64,
			...
			std::complex<double>, DeviceMemory<std::complex<double>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasGbmv, trans, m, n, kl, ku,
			alpha, a, lda, x, incx, beta, y, incy);
}
		...
		DeviceMemory<float> *y, int incy)
{
	...
	ThenBlasImpl<blas::Transpose, uint64, uint64, float,
			const DeviceMemory<float> &, int, const DeviceMemory<float> &, int,
			float, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasGemv, trans, m, n, alpha, a,
		...
		int incy, DeviceMemory<float> *a, int lda)
{
	...
	ThenBlasImpl<uint64, uint64, float, const DeviceMemory<float> &, int,
			const DeviceMemory<float> &, int, DeviceMemory<float> *, int> impl;
		...
		int incy, DeviceMemory<double> *a, int lda)
{
	...
	ThenBlasImpl<uint64, uint64, double, const DeviceMemory<double> &, int,
			const DeviceMemory<double> &, int, DeviceMemory<double> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasGer, m, n, alpha, x, incx, y,
			incy, a, lda);
}
...
Stream &Stream::ThenBlasHbmv(blas::UpperLower uplo, uint64 n, uint64 k,
		...
		int incy)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(k), PARAM(alpha), PARAM(a),
			PARAM(lda), PARAM(x), PARAM(incx), PARAM(beta), PARAM(y),
			PARAM(incy));
...
Stream &Stream::ThenBlasHemv(blas::UpperLower uplo, uint64 n,
		...
		int incy)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(a), PARAM(lda),
		...
		DeviceMemory<std::complex<float>> *a, int lda)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, std::complex<float>,
			...
			const DeviceMemory<std::complex<float>> &, int,
			DeviceMemory<std::complex<float>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasHer2, uplo, n, alpha, x, incx,
			y, incy, a, lda);
}
		...
		int incy)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, std::complex<float>,
			...
			const DeviceMemory<std::complex<float>> &, int, std::complex<float>,
			DeviceMemory<std::complex<float>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasHpmv, uplo, n, alpha, ap, x,
			incx, beta, y, incy);
}
		...
		DeviceMemory<std::complex<float>> *ap)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, float,
			const DeviceMemory<std::complex<float>> &, int,
			DeviceMemory<std::complex<float>> *> impl;
	return impl(this, &blas::BlasSupport::DoBlasHpr, uplo, n, alpha, x, incx,
			ap);
}
		...
		DeviceMemory<std::complex<float>> *ap)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, std::complex<float>,
			...
			const DeviceMemory<std::complex<float>> &, int,
			DeviceMemory<std::complex<float>> *> impl;
	return impl(this, &blas::BlasSupport::DoBlasHpr2, uplo, n, alpha, x, incx,
			y, incy, ap);
}
...
Stream &Stream::ThenBlasSpmv(blas::UpperLower uplo, uint64 n, float alpha,
		const DeviceMemory<float> &ap, const DeviceMemory<float> &x, int incx,
		float beta, DeviceMemory<float> *y, int incy)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, float, const DeviceMemory<float> &,
			const DeviceMemory<float> &, int, float, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSpmv, uplo, n, alpha, ap, x,
			incx, beta, y, incy);
}

Stream &Stream::ThenBlasSpmv(blas::UpperLower uplo, uint64 n, double alpha,
		const DeviceMemory<double> &ap, const DeviceMemory<double> &x, int incx,
...
Stream &Stream::ThenBlasSpr(blas::UpperLower uplo, uint64 n, float alpha,
		const DeviceMemory<float> &x, int incx, DeviceMemory<float> *ap)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(ap));
	...
	return impl(this, &blas::BlasSupport::DoBlasSpr, uplo, n, alpha, x, incx,
			ap);
}

Stream &Stream::ThenBlasSpr(blas::UpperLower uplo, uint64 n, double alpha,
		const DeviceMemory<double> &x, int incx, DeviceMemory<double> *ap)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(ap));
...
Stream &Stream::ThenBlasSpr2(blas::UpperLower uplo, uint64 n, float alpha,
		const DeviceMemory<float> &x, int incx, const DeviceMemory<float> &y,
		int incy, DeviceMemory<float> *ap)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(y), PARAM(incy), PARAM(ap));

	ThenBlasImpl<blas::UpperLower, uint64, float, const DeviceMemory<float> &,
			int, const DeviceMemory<float> &, int, DeviceMemory<float> *> impl;
	return impl(this, &blas::BlasSupport::DoBlasSpr2, uplo, n, alpha, x, incx,
			y, incy, ap);
}

Stream &Stream::ThenBlasSpr2(blas::UpperLower uplo, uint64 n, double alpha,
		const DeviceMemory<double> &x, int incx, const DeviceMemory<double> &y,
		int incy, DeviceMemory<double> *ap)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(y), PARAM(incy), PARAM(ap));

	ThenBlasImpl<blas::UpperLower, uint64, double, const DeviceMemory<double> &,
			int, const DeviceMemory<double> &, int, DeviceMemory<double> *> impl;
	return impl(this, &blas::BlasSupport::DoBlasSpr2, uplo, n, alpha, x, incx,
			y, incy, ap);
}
		...
		int incx, float beta, DeviceMemory<float> *y, int incy)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, float, const DeviceMemory<float> &,
			int, const DeviceMemory<float> &, int, float, DeviceMemory<float> *,
			int> impl;
...
Stream &Stream::ThenBlasSyr(blas::UpperLower uplo, uint64 n, float alpha,
		const DeviceMemory<float> &x, int incx, DeviceMemory<float> *a, int lda)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(a), PARAM(lda));
...
Stream &Stream::ThenBlasSyr(blas::UpperLower uplo, uint64 n, double alpha,
		const DeviceMemory<double> &x, int incx, DeviceMemory<double> *a,
		int lda)
{
	VLOG_CALL(PARAM(uplo), PARAM(n), PARAM(alpha), PARAM(x), PARAM(incx),
			PARAM(a), PARAM(lda));
		...
		int incy, DeviceMemory<float> *a, int lda)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, float, const DeviceMemory<float> &,
			int, const DeviceMemory<float> &, int, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSyr2, uplo, n, alpha, x, incx,
			y, incy, a, lda);
}
		...
		int incy, DeviceMemory<double> *a, int lda)
{
	...
	ThenBlasImpl<blas::UpperLower, uint64, double, const DeviceMemory<double> &,
			int, const DeviceMemory<double> &, int, DeviceMemory<double> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSyr2, uplo, n, alpha, x, incx,
			y, incy, a, lda);
}

Stream &Stream::ThenBlasTbmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, uint64 k, const DeviceMemory<float> &a,
		int lda, DeviceMemory<float> *x, int incx)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, blas::Diagonal, uint64,
			uint64, const DeviceMemory<float> &, int, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasTbmv, uplo, trans, diag, n, k,
			a, lda, x, incx);
}

Stream &Stream::ThenBlasTbmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, uint64 k, const DeviceMemory<double> &a,
		...
		int lda, DeviceMemory<float> *x, int incx)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, blas::Diagonal, uint64,
			uint64, const DeviceMemory<float> &, int, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasTbsv, uplo, trans, diag, n, k,
			a, lda, x, incx);
}
...
Stream &Stream::ThenBlasTpmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, const DeviceMemory<float> &ap,
		DeviceMemory<float> *x, int incx)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasTpmv, uplo, trans, diag, n, ap,
			x, incx);
}

Stream &Stream::ThenBlasTpmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, const DeviceMemory<double> &ap,
		DeviceMemory<double> *x, int incx)
{
	VLOG_CALL(PARAM(uplo), PARAM(trans), PARAM(diag), PARAM(n), PARAM(ap),
			PARAM(x), PARAM(incx));
		...
		DeviceMemory<float> *x, int incx)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, blas::Diagonal, uint64,
			const DeviceMemory<float> &, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasTpsv, uplo, trans, diag, n, ap,
			x, incx);
}
...
Stream &Stream::ThenBlasTrmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, const DeviceMemory<float> &a, int lda,
		DeviceMemory<float> *x, int incx)
{
...
Stream &Stream::ThenBlasTrmv(blas::UpperLower uplo, blas::Transpose trans,
		blas::Diagonal diag, uint64 n, const DeviceMemory<double> &a, int lda,
		DeviceMemory<double> *x, int incx)
{
	VLOG_CALL(PARAM(uplo), PARAM(trans), PARAM(diag), PARAM(n), PARAM(a),
			PARAM(lda), PARAM(x), PARAM(incx));
		...
		DeviceMemory<Eigen::half> *c, int ldc)
{
	...
	ThenBlasImpl<blas::Transpose, blas::Transpose, uint64, uint64, uint64,
			float, const DeviceMemory<Eigen::half> &, int,
			const DeviceMemory<Eigen::half> &, int, float,
			DeviceMemory<Eigen::half> *, int> impl;
		...
		DeviceMemory<float> *c, int ldc)
{
	...
	ThenBlasImpl<blas::Transpose, blas::Transpose, uint64, uint64, uint64,
			float, const DeviceMemory<float> &, int,
			const DeviceMemory<float> &, int, float, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasGemm, transa, transb, m, n, k,
			alpha, a, lda, b, ldb, beta, c, ldc);
}
		...
		int ldb, double beta, DeviceMemory<double> *c, int ldc)
{
	...
	ThenBlasImpl<blas::Transpose, blas::Transpose, uint64, uint64, uint64,
			double, const DeviceMemory<double> &, int,
			const DeviceMemory<double> &, int, double, DeviceMemory<double> *,
			int> impl;
	return impl(this, &blas::BlasSupport::DoBlasGemm, transa, transb, m, n, k,
			alpha, a, lda, b, ldb, beta, c, ldc);
		...
		std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
	...
	ThenBlasImpl<blas::Side, blas::UpperLower, uint64, uint64,
			...
			const DeviceMemory<std::complex<float>> &, int, std::complex<float>,
			DeviceMemory<std::complex<float>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasHemm, side, uplo, m, n, alpha,
		...
		DeviceMemory<std::complex<float>> *c, int ldc)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, uint64, uint64, float,
			...
			DeviceMemory<std::complex<float>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasHerk, uplo, trans, n, k, alpha,
			a, lda, beta, c, ldc);
}
		...
		DeviceMemory<float> *c, int ldc)
{
	...
	ThenBlasImpl<blas::Side, blas::UpperLower, uint64, uint64, float,
			const DeviceMemory<float> &, int, const DeviceMemory<float> &, int,
			float, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSymm, side, uplo, m, n, alpha,
			a, lda, b, ldb, beta, c, ldc);
}
		...
		float beta, DeviceMemory<float> *c, int ldc)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, uint64, uint64, float,
			const DeviceMemory<float> &, int, float, DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSyrk, uplo, trans, n, k, alpha,
			a, lda, beta, c, ldc);
}

Stream &Stream::ThenBlasSyrk(blas::UpperLower uplo, blas::Transpose trans,
		uint64 n, uint64 k, double alpha, const DeviceMemory<double> &a,
		...
		std::complex<float> beta, DeviceMemory<std::complex<float>> *c, int ldc)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, uint64, uint64,
			std::complex<float>, const DeviceMemory<std::complex<float>> &, int,
			std::complex<float>, DeviceMemory<std::complex<float>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSyrk, uplo, trans, n, k, alpha,
			a, lda, beta, c, ldc);
}
		...
		int ldc)
{
	...
	ThenBlasImpl<blas::UpperLower, blas::Transpose, uint64, uint64,
			std::complex<double>, const DeviceMemory<std::complex<double>> &,
			int, std::complex<double>, DeviceMemory<std::complex<double>> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasSyrk, uplo, trans, n, k, alpha,
			a, lda, beta, c, ldc);
}
...
Stream &Stream::ThenBlasSyr2k(blas::UpperLower uplo, blas::Transpose trans,
		uint64 n, uint64 k, double alpha, const DeviceMemory<double> &a,
		int lda, const DeviceMemory<double> &b, int ldb, double beta,
		DeviceMemory<double> *c, int ldc)
{
	VLOG_CALL(PARAM(uplo), PARAM(trans), PARAM(n), PARAM(k), PARAM(alpha),
			PARAM(a), PARAM(lda), PARAM(b), PARAM(ldb), PARAM(beta), PARAM(c),
...
Stream &Stream::ThenBlasTrmm(blas::Side side, blas::UpperLower uplo,
		blas::Transpose transa, blas::Diagonal diag, uint64 m, uint64 n,
		float alpha, const DeviceMemory<float> &a, int lda,
		DeviceMemory<float> *b, int ldb)
{
	...
	ThenBlasImpl<blas::Side, blas::UpperLower, blas::Transpose, blas::Diagonal,
			...
			DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasTrmm, side, uplo, transa, diag,
...
Stream &Stream::ThenBlasTrmm(blas::Side side, blas::UpperLower uplo,
		blas::Transpose transa, blas::Diagonal diag, uint64 m, uint64 n,
		std::complex<float> alpha, const DeviceMemory<std::complex<float>> &a,
		int lda, DeviceMemory<std::complex<float>> *b, int ldb)
{
	VLOG_CALL(PARAM(side), PARAM(uplo), PARAM(transa), PARAM(diag), PARAM(m),
			PARAM(n), PARAM(alpha), PARAM(a), PARAM(lda), PARAM(b), PARAM(ldb));
...
Stream &Stream::ThenBlasTrmm(blas::Side side, blas::UpperLower uplo,
		blas::Transpose transa, blas::Diagonal diag, uint64 m, uint64 n,
		std::complex<double> alpha, const DeviceMemory<std::complex<double>> &a,
		int lda, DeviceMemory<std::complex<double>> *b, int ldb)
{
	VLOG_CALL(PARAM(side), PARAM(uplo), PARAM(transa), PARAM(diag), PARAM(m),
			PARAM(n), PARAM(alpha), PARAM(a), PARAM(lda), PARAM(b), PARAM(ldb));
		...
		DeviceMemory<float> *b, int ldb)
{
	...
	ThenBlasImpl<blas::Side, blas::UpperLower, blas::Transpose, blas::Diagonal,
			uint64, uint64, float, const DeviceMemory<float> &, int,
			DeviceMemory<float> *, int> impl;
	return impl(this, &blas::BlasSupport::DoBlasTrsm, side, uplo, transa, diag,
			m, n, alpha, a, lda, b, ldb);
}
...
Stream &Stream::ThenBlasGemmBatched(blas::Transpose transa,
		...
		int batch_count)
{
	return ThenBlasGemmBatchedWithScratch(transa, transb, m, n, k, alpha, a,
			lda, b, ldb, beta, c, ldc, batch_count, nullptr);
}

Stream &Stream::ThenBlasGemmBatchedWithScratch(blas::Transpose transa,
		blas::Transpose transb, uint64 m, uint64 n, uint64 k, float alpha,
		const port::ArraySlice<DeviceMemory<float> *> &a, int lda,
		const port::ArraySlice<DeviceMemory<float> *> &b, int ldb, float beta,
		const port::ArraySlice<DeviceMemory<float> *> &c, int ldc,
		int batch_count, ScratchAllocator *scratch_allocator)
{
	...
	return impl(this, &blas::BlasSupport::DoBlasGemmBatched, transa, transb, m,
			n, k, alpha, a, lda, b, ldb, beta, c, ldc, batch_count,
			scratch_allocator);
}
		...
		int ldc, int batch_count, ScratchAllocator *scratch_allocator)
{
	...
	ThenBlasImpl<blas::Transpose, blas::Transpose, uint64, uint64, uint64,
			std::complex<double>,
			const port::ArraySlice<DeviceMemory<std::complex<double>> *> &, int,
			const port::ArraySlice<DeviceMemory<std::complex<double>> *> &, int,
			std::complex<double>,
			const port::ArraySlice<DeviceMemory<std::complex<double>> *> &, int,
			int, ScratchAllocator *> impl;
...
Stream &Stream::ThenSetRngSeed(const uint8 *seed, uint64 seed_bytes)
{
	...
	if (ok()) {
		...
		LOG(INFO) << "stream " << this << " did not set RNG seed: "
				<< static_cast<const void *>(seed) << "; bytes: " << seed_bytes;
	}
	return *this;
}
		...
		uint64 size)
{
	...
	if (ok()) {
		...
		LOG(INFO) << "stream " << this
				<< " did not memcpy device-to-host; source: "
				<< gpu_src.opaque();
	}
	return *this;
}
		...
		const DeviceMemoryBase &gpu_src, uint64 size)
{
	...
	if (ok()) {
		...
		LOG(INFO) << "stream " << this << " did not memcpy gpu-to-gpu; source: "
				<< &gpu_src;
	}
	return *this;
}
...
Stream &Stream::ThenRnnForward(const dnn::RnnDescriptor &rnn_desc,
		...
		ScratchAllocator *workspace_allocator)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoRnnForward(this, rnn_desc, input_desc, input_data,
							input_h_desc, input_h_data, input_c_desc,
							input_c_data, params, output_desc, output_data,
							output_h_desc, output_h_data, output_c_desc,
							output_c_data, is_training, reserve_space_allocator,
							workspace_allocator));
		} else {
			SetError();
			LOG(WARNING)
					<< "Attempting to call ThenRnnForward without DNN support";
		}
	}
	return *this;
}

Stream &Stream::ThenRnnBackward(const dnn::RnnDescriptor &rnn_desc,
		const dnn::RnnSequenceTensorDescriptor &input_desc,
		const DeviceMemory<float> &input_data,
		const dnn::RnnStateTensorDescriptor &input_h_desc,
		const DeviceMemory<float> &input_h_data,
		...
		ScratchAllocator *workspace_allocator)
{
	...
	if (ok()) {
		if (dnn::DnnSupport *dnn = parent_->AsDnn()) {
			CheckError(
					dnn->DoRnnBackward(this, rnn_desc, input_desc, input_data,
							input_h_desc, input_h_data, input_c_desc,
							input_c_data, params, output_desc, output_data,
							output_h_desc, output_h_data, output_c_desc,
			...
			SetError();
			LOG(WARNING)
					<< "Attempting to call ThenRnnBackward without DNN support";
		}
	}
	return *this;
}
