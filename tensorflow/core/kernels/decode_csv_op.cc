...
public:
	explicit DecodeCSVOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
		string delim;
			...
			for (int f = 0; f < static_cast<int>(out_type_.size()); ++f) {
				...
				case DT_INT32: {
					...
					if (fields[f].empty()) {
						...
						int32 value;
						OP_REQUIRES(ctx,
								strings::safe_strto32(fields[f], &value),
								errors::InvalidArgument("Field ", f,
										" in record ", i,
				...
				case DT_INT64: {
					...
					if (fields[f].empty()) {
						...
						int64 value;
						OP_REQUIRES(ctx,
								strings::safe_strto64(fields[f], &value),
								errors::InvalidArgument("Field ", f,
										" in record ", i,
				...
				case DT_FLOAT: {
					...
					if (fields[f].empty()) {
						...
						float value;
						OP_REQUIRES(ctx,
								strings::safe_strtof(fields[f].c_str(), &value),
								errors::InvalidArgument("Field ", f,
										" in record ", i,
