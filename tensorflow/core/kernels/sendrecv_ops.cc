...
static string GetRendezvousKeyPrefix(const string& send_device,
		const string& recv_device, const uint64 send_device_incarnation,
		const string& tensor_name)
{
	return strings::StrCat(send_device, ";",
			strings::FpToString(send_device_incarnation), ";", recv_device, ";",
			tensor_name);
}
...
SendOp::SendOp(OpKernelConstruction* ctx) :
		OpKernel(ctx)
{
	string send_device;
...
void SendOp::Compute(OpKernelContext* ctx)
{
	...
	OP_REQUIRES_OK(ctx,
			ctx->rendezvous()->Send(parsed, args, ctx->input(0),
					ctx->is_input_dead()));
}
...
RecvOp::RecvOp(OpKernelConstruction* ctx) :
		AsyncOpKernel(ctx)
{
	string send_device;
	OP_REQUIRES_OK(ctx, ctx->GetAttr("send_device", &send_device));
...
void RecvOp::ComputeAsync(OpKernelContext* ctx, DoneCallback done)
{
	...
	Rendezvous::DoneCallback done_cb = std::bind([ctx](DoneCallback done,
