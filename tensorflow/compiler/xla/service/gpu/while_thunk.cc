...
WhileThunk::WhileThunk(BufferAllocation::Index condition_result_buffer_index,
		...
		Thunk(Kind::kWhile, hlo), condition_result_buffer_index_(
				condition_result_buffer_index), condition_thunk_sequence_(
				MakeUnique<SequentialThunk>(
						std::move(*condition_thunk_sequence), hlo)), body_thunk_sequence_(
				MakeUnique<SequentialThunk>(std::move(*body_thunk_sequence),
						hlo))
{
}
		...
		perftools::gputools::Stream* stream)
{
	...
	while (true) {
		...
		TF_RETURN_IF_ERROR(
				condition_thunk_sequence_->ExecuteOnStream(buffer_allocations,
						stream));

		// Copy the result of condition computation and break the loop if 'false'.
		bool condition_result;
		stream->ThenMemcpy(&condition_result, condition_result_data,
				sizeof(bool));
