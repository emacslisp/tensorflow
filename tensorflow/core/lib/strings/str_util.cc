...
inline bool is_octal_digit(unsigned char c)
{
	return c >= '0' && c <= '7';
}
...
bool SplitAndParseAsInts(StringPiece text, char delim,
		std::function<bool(StringPiece, T*)> converter, std::vector<T>* result)
{
	result->clear();
...
{
	return SplitAndParseAsInts<int32>(text, delim, strings::safe_strto32,
			result);
}
...
{
	return SplitAndParseAsInts<int64>(text, delim, strings::safe_strto64,
			result);
}
