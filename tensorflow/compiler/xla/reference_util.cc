...
{
	return ConvArray4DGeneralDimensions(lhs, rhs, kernel_stride, padding,
			ComputationBuilder::CreateDefaultConvDimensionNumbers());
}

/* static */std::unique_ptr<Array4D<float>> ReferenceUtil::SeparableConvArray4D(
			...
			for (int64 kz = 0; kz < input.depth(); ++kz) {
				for (int64 out = 0; out < pointwise_weights.planes(); ++out) {
					...
					for (int64 depth = 0; depth < depth_multiplier; ++depth) {
						weight += depthwise_weights(depth, kz, ky, kx)
								* pointwise_weights(out,
										depth + kz * depth_multiplier, 0, 0);
					}
...
/* static */int64 ReferenceUtil::WindowCount(int64 unpadded_width,
		int64 window_len, int64 stride, Padding padding)
{
		...
		const tensorflow::gtl::ArraySlice<int64>& stride, Padding padding)
{
	...
	for (int64 i = 0; i < window.size(); ++i) {
		window_counts[i] = WindowCount(dim_lengths[i], window[i], stride[i],
				padding);
				...
				for (int64 i3 = 0; i3 < window_counts[3]; ++i3) {
					...
					for (int64 i0_win = 0; i0_win < window[0]; ++i0_win) {
						for (int64 i1_win = 0; i1_win < window[1]; ++i1_win) {
							for (int64 i2_win = 0; i2_win < window[2];
									++i2_win) {
								for (int64 i3_win = 0; i3_win < window[3];
										++i3_win) {
									if (i0_base + i0_win >= 0
											&& i1_base + i1_win >= 0
											&& i2_base + i2_win >= 0
...
/* static  */std::unique_ptr<Array4D<float>> ReferenceUtil::SelectAndScatter4DGePlus(
		...
		for (int64 i1 = 0; i1 < window_counts[1]; ++i1) {
			for (int64 i2 = 0; i2 < window_counts[2]; ++i2) {
				for (int64 i3 = 0; i3 < window_counts[3]; ++i3) {
					...
					float val = operand(scatter_0, scatter_1, scatter_2,
							scatter_3);
					for (int64 i0_win = 0; i0_win < window[0]; ++i0_win) {
						for (int64 i1_win = 0; i1_win < window[1]; ++i1_win) {
							for (int64 i2_win = 0; i2_win < window[2];
								...
								for (int64 i3_win = 0; i3_win < window[3];
									...
									if (i0_base + i0_win >= 0
											...
											&& i3_base + i3_win
													< operand.n4()) {
										float tmp = operand(i0_base + i0_win,
												i1_base + i1_win,
												i2_base + i2_win,
												i3_base + i3_win);
										if (tmp >= val) {
...
/* static */std::unique_ptr<Array4D<float>> ReferenceUtil::ConvArray4DGeneralDimensions(
		const Array4D<float>& lhs, const Array4D<float>& rhs,
		std::pair<int64, int64> kernel_stride, Padding padding,
...
/* static */std::unique_ptr<Array4D<float>> ReferenceUtil::ConvArray4DGeneralDimensionsDilated(
		const Array4D<float>& lhs, const Array4D<float>& rhs,
		std::pair<int64, int64> kernel_stride, Padding padding,
		std::pair<int64, int64> lhs_dilation,
		std::pair<int64, int64> rhs_dilation, ConvolutionDimensionNumbers dnums)
{
	std::array<int64, 4> lhs_dimensions { { lhs.n1(), lhs.n2(), lhs.n3(),
			lhs.n4() } };
	std::array<int64, 4> rhs_dimensions { { rhs.n1(), rhs.n2(), rhs.n3(),
			rhs.n4() } };
	...
	const int64 ox =
			padding == Padding::kSame ?
					ix : window_util::StridedBound(ix, kx, ksx);
	const int64 oy =
			padding == Padding::kSame ?
					iy : window_util::StridedBound(iy, ky, ksy);
	const int64 istartx = padding == Padding::kValid ? 0 :
							kx % 2 == 0 ? -(kx / 2 - 1) : -kx / 2;
	const int64 istarty = padding == Padding::kValid ? 0 :
							ky % 2 == 0 ? -(ky / 2 - 1) : -ky / 2;
	...
	auto result = MakeUnique<Array4D<float>>(result_dimensions[0],
			result_dimensions[1], result_dimensions[2], result_dimensions[3]);
	result->Fill(0.0);
...
/* static */std::unique_ptr<std::vector<float>> ReferenceUtil::ReduceToColArray2D(
...
/* static */std::unique_ptr<std::vector<float>> ReferenceUtil::ReduceToRowArray2D(
		const Array2D<float>& matrix, float init,
		std::function<float(float, float)> reduce_function)
{
	int64 rows = matrix.height();
		...
		std::function<float(float, float)> reduce_function)
{
	...
	for (int64 a0 = 0; a0 == 0 || (!dim_set.count(0) && a0 < array.n1());
			++a0) {
		for (int64 a1 = 0; a1 == 0 || (!dim_set.count(1) && a1 < array.n2());
				++a1) {
			for (int64 a2 = 0;
					a2 == 0 || (!dim_set.count(2) && a2 < array.n3()); ++a2) {
				for (int64 a3 = 0;
						a3 == 0 || (!dim_set.count(3) && a3 < array.n4());
						++a3) {
					float accumulator = init;
					for (int64 i0 = 0;
							i0 == 0 || (dim_set.count(0) && i0 < array.n1());
							++i0) {
						for (int64 i1 = 0;
								i1 == 0 || (dim_set.count(1) && i1 < array.n2());
								++i1) {
							for (int64 i2 = 0;
									i2 == 0
											|| (dim_set.count(2)
	...
	for (int i0 = 0; i0 < array.n1(); ++i0) {
		for (int i1 = 0; i1 < array.n2(); ++i1) {
			for (int i2 = 0; i2 < array.n3(); ++i2) {
				...
				(*result)(row, col) = reduce_function((*result)(row, col),
						array(i0, i1, i2));
			}
		}
	}
		...
		const float pad)
{
	...
	int64 out0 = in0 + low_padding0 + high_padding0
			+ (in0 - 1) * interior_padding0;
	...
	int64 out1 = in1 + low_padding1 + high_padding1
			+ (in1 - 1) * interior_padding1;
