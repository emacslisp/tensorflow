...
{
	const string saved_model_pb_path = io::JoinPath(export_dir,
			kSavedModelFilenamePb);
	if (Env::Default()->FileExists(saved_model_pb_path).ok()) {
		return ReadBinaryProto(Env::Default(), saved_model_pb_path,
				saved_model_proto);
	}
	const string saved_model_pbtxt_path = io::JoinPath(export_dir,
			kSavedModelFilenamePbTxt);
		...
		std::vector<std::pair<string, Tensor>>* inputs)
{
	...
	for (auto& asset_file_def : asset_file_defs) {
		Tensor assets_file_path_tensor = CreateStringTensor(
				io::JoinPath(export_dir, kSavedModelAssetsDirectory,
						asset_file_def.filename()));
		inputs->push_back( { asset_file_def.tensor_info().name(),
				assets_file_path_tensor });
	}
}
...
Status RunRestore(const RunOptions& run_options, const string& export_dir,
		...
		const std::vector<AssetFileDef>& asset_file_defs, Session* session)
{
	LOG(INFO) << "Restoring SavedModel bundle.";
	// Find path to variables to be restored in export directory.
	const string variables_directory = io::JoinPath(export_dir,
			kSavedModelVariablesDirectory);
	...
	const string variables_index_path = io::JoinPath(variables_directory,
			MetaFilename(kSavedModelVariablesFilename));
	if (!Env::Default()->FileExists(variables_index_path).ok()) {
		return errors::NotFound(
				"Checkpoint index file not found in SavedModel directory.");
	}
	const string variables_path = io::JoinPath(variables_directory,
	...
	std::vector<std::pair<string, Tensor>> inputs =
			{ { variable_filename_const_op_name.ToString(),
					variables_path_tensor } };

	AddAssetsTensorsToInputs(export_dir, asset_file_defs, &inputs);

	RunMetadata run_metadata;
	return session->Run(run_options, inputs, { },
			{ restore_op_name.ToString() }, nullptr /* outputs */,
			&run_metadata);
}
	...
	if (init_op_it != collection_def_map.end()) {
		if (init_op_it->second.node_list().value_size() != 1) {
			return errors::FailedPrecondition(
					strings::StrCat(
							"Expected exactly one serving init op in : ",
							export_dir));
		}
		std::vector<std::pair<string, Tensor>> inputs;
		AddAssetsTensorsToInputs(export_dir, asset_file_defs, &inputs);
		...
		std::vector<AssetFileDef>* asset_file_defs)
{
	...
	for (const auto& any_asset : any_assets) {
		...
		TF_RETURN_IF_ERROR(
				ParseAny(any_asset, &asset_file_def,
		...
		const std::unordered_set<string>& tags, SavedModelBundle* const bundle)
{
	...
	TF_RETURN_IF_ERROR(
			FindMetaGraphDefToLoad(saved_model_proto, tags,
					&bundle->meta_graph_def));

	TF_RETURN_IF_ERROR(
			LoadMetaGraphIntoSession(bundle->meta_graph_def, session_options,
					&bundle->session));
	...
	TF_RETURN_IF_ERROR(
			RunLegacyInitOp(run_options, export_dir, bundle->meta_graph_def,
					asset_file_defs, bundle->session.get()));
	return Status::OK();
}
