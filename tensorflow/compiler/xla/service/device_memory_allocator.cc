...
StreamExecutorMemoryAllocator::StreamExecutorMemoryAllocator(
		...
		tensorflow::gtl::ArraySlice<perftools::gputools::StreamExecutor*> stream_executors) :
		DeviceMemoryAllocator(platform), stream_executors_(
				stream_executors.begin(), stream_executors.end())
{
}

StatusOr<perftools::gputools::DeviceMemoryBase> StreamExecutorMemoryAllocator::Allocate(
...
tensorflow::Status StreamExecutorMemoryAllocator::Deallocate(int device_ordinal,
		perftools::gputools::DeviceMemoryBase* mem)
{
	if (!mem->is_null()) {
		TF_ASSIGN_OR_RETURN(
				perftools::gputools::StreamExecutor * stream_executor,
				GetStreamExecutor(device_ordinal));
...
StatusOr<perftools::gputools::StreamExecutor*> StreamExecutorMemoryAllocator::GetStreamExecutor(
		int device_ordinal)
{
	if (device_ordinal < 0) {
	...
	if (device_ordinal >= stream_executors_.size()) {
		return InvalidArgument(
				"device ordinal value (%d) >= number of devices (%zu)",
				device_ordinal, stream_executors_.size());
	}
