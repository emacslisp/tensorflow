...
Status GetTensorArray(OpKernelContext* ctx, TensorArray** tensor_array)
{
	...
	if (ctx->input_dtype(0) != DT_RESOURCE) {
		...
		TF_RETURN_IF_ERROR(
				rm->Lookup(ctx->step_container()->name(), container + ta_handle,
						tensor_array));
		return Status::OK();
	} else {
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, rm != nullptr,
				errors::Internal("No resource manager."));

		TensorArray* output_tensor_array;
		OP_REQUIRES_OK(ctx,
				CreateTensorArray(ctx, rm, &tensor_array_output_handle,
						&output_tensor_array));
		...
		if (ctx->num_outputs() == 2) {
			...
			Tensor* flow;
			OP_REQUIRES_OK(ctx,
					ctx->allocate_output(1, TensorShape( { }), &flow));
		}
	}
...
public:
			...
			TensorArrayCreationOp(context)
	{
		...
		OP_REQUIRES_OK(context,
				context->GetAttr("element_shape", &element_shape_));
		OP_REQUIRES_OK(context,
				context->GetAttr("dynamic_size", &dynamic_size_));
		OP_REQUIRES_OK(context,
				context->GetAttr("clear_after_read", &clear_after_read_));
		OP_REQUIRES_OK(context,
				context->GetAttr("tensor_array_name", &tensor_array_name_));
		if (tensor_array_name_ == "")
			tensor_array_name_ = name();
	}
			...
			TensorArray** output_tensor_array) override
	{
		...
		string unique_tensor_array_name = strings::StrCat(tensor_array_name_,
				"_", TensorArray::tensor_array_counter.fetch_add(1));
		handle(0) = "_tensor_arrays";
		handle(1) = unique_tensor_array_name;

		auto key = strings::StrCat(handle(0), unique_tensor_array_name);

		TensorArray* tensor_array = new TensorArray(key, dtype_,
				*tensor_array_output_handle, size, element_shape_,
...
public:
			...
			TensorArray** output_tensor_array) override
	{
		...
		if (ctx->input_dtype(0) != DT_RESOURCE) {
			...
			tensor_array_name = StringPiece(resource.name()).substr(
		...
		TF_RETURN_IF_ERROR(
				rm->Lookup(ctx->step_container()->name(),
		...
		if (!tensor_array->GradientsAllowed()) {
			return errors::InvalidArgument(
					"Unable to create a gradients TensorArray for ",
					tensor_array_name,
		...
		auto creator =
				[this, key, tensor_array, array_size, marked_size,
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx, tensor_value->dtype() == tensor_array->ElemType(),
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		const int32 index = tensor_index->scalar<int32>()();
		OP_REQUIRES(ctx, dtype_ == tensor_array->ElemType(),
				errors::InvalidArgument("TensorArray dtype is ",
						DataTypeString(tensor_array->ElemType()),
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		if (LEGACY_PACK) {
			...
			OP_REQUIRES(ctx,
					TensorShapeUtils::IsVector(tensor_indices->shape()),
					errors::InvalidArgument(
		...
		if (num_indices == 0) {
			...
			Tensor* empty_unused;
			OP_REQUIRES_OK(ctx,
					ctx->allocate_output(0, empty_shape, &empty_unused));
			return;
		}
		...
		auto output_flat = output_tensor->shaped<T, 2>(
				{ 1, output_shape.num_elements() });

		// Insert the first value
		input_tensors_flat.emplace_back(
				new ConstMatrix(
						value_0_t->shaped<T, 2>(
								{ 1, value_0_t->NumElements() })));

		for (int i = 1; i < num_indices; ++i) {
			const Tensor* value_t = values[i].AccessTensor(ctx);
			OP_REQUIRES(ctx, value_0_t->shape() == value_t->shape(),
					errors::InvalidArgument(
...
public:
			...
			OpKernel(context)
	{
		OP_REQUIRES_OK(context, context->GetAttr("dtype", &dtype_));
		OP_REQUIRES_OK(context,
				context->GetAttr("element_shape_except0",
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		if (array_size == 0) {
			OP_REQUIRES(ctx, element_shape_except0_.IsFullyDefined(),
					errors::Unimplemented(
		...
		OP_REQUIRES_OK(ctx, ctx->allocate_output(1, TensorShape( {
				static_cast<int64>(values.size()) }), &lengths_tensor));
		...
		for (std::size_t i = 0; i < values.size(); ++i) {
			...
			OP_REQUIRES(ctx, TensorShapeUtils::IsVectorOrHigher(value_shape_t),
					errors::InvalidArgument(
			...
			if (i == 0) {
				...
				OP_REQUIRES(ctx,
						element_shape_except0_.IsCompatibleWith(
								output_shape_except0),
						errors::InvalidArgument(
				...
				OP_REQUIRES(ctx, output_shape_except0 == value_shape_t_except0,
						errors::InvalidArgument(
								...
								output_shape_except0.DebugString(),
								" but index ", i,
								" has (excepting dimension 0) shape: ",
								value_shape_t_except0.DebugString()));
				// Store the previous maximum length as the offset for this tensor.
				output_shape.set_dim(0,
						output_shape.dim_size(0) + value_shape_t.dim_size(0));
			}
		}
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		if (LEGACY_UNPACK) {
			OP_REQUIRES(ctx, element_shape.dim_size(0) == array_size,
					errors::InvalidArgument(
			...
			OP_REQUIRES(ctx, max_index < array_size,
					errors::InvalidArgument(
							"Max scatter index must be <= array size (",
							max_index, " vs. ", array_size, ")"));
		}
		element_shape.RemoveDim(0);

		auto tensor_value_t = tensor_value->shaped<T, 3>( { 1, num_values,
				element_shape.num_elements() });

		Eigen::DSizes < Eigen::DenseIndex, 3 > indices { 0, 0, 0 };
		Eigen::DSizes < Eigen::DenseIndex, 3
				> sizes { 1, 1, element_shape.num_elements() };
		...
		for (int i = 0; i < num_values; ++i) {
			...
			OP_REQUIRES_OK(ctx,
					ctx->allocate_persistent(tensor_array->ElemType(),
							element_shape, &persistent_tensor,
							&tensor_value_i));
			auto tensor_value_i_t = tensor_value_i->shaped<T, 3>( { 1, 1,
					element_shape.num_elements() });
			indices[1] = i;

			if (element_shape.num_elements() > 0) {
				functor::Split<Device, T>()(ctx->eigen_device<Device>(),
						tensor_value_i_t, tensor_value_t, indices, sizes);
			}
		...
		Status s = tensor_array->WriteOrAggregateMany<Device, T>(ctx,
				write_indices, &write_values);
		OP_REQUIRES_OK(ctx, s);
	}
};
...
public:
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		OP_REQUIRES(ctx,
				TensorShapeUtils::IsVectorOrHigher(tensor_value->shape()),
				errors::InvalidArgument(
		...
		int64 elements_per_row =
				(total_length == 0) ?
						0 : (tensor_value->NumElements() / total_length);

		int32 array_size;
		OP_REQUIRES_OK(ctx, tensor_array->Size(&array_size));
		bool dynamic_size = tensor_array->HasDynamicSize();

		std::vector<TensorShape> element_shapes(num_tensors,
				tensor_value->shape());
		...
		OP_REQUIRES(ctx, array_size == num_tensors,
				errors::InvalidArgument(
						"TensorArray's size is not equal to the size of lengths (",
						array_size, " vs. ", num_tensors,
						"), and the TensorArray is not ",
		...
		auto tensor_value_t = tensor_value->shaped<T, 3>( { 1, total_length,
				elements_per_row });
		...
		for (int i = 0; i < array_size; ++i) {
			...
			Eigen::DSizes < Eigen::DenseIndex, 3 > indices { 0, previous_length,
					0 };
			Eigen::DSizes < Eigen::DenseIndex, 3
					> sizes { 1, tensor_lengths_t(i), elements_per_row };

			OP_REQUIRES_OK(ctx,
					ctx->allocate_persistent(tensor_array->ElemType(),
							element_shapes[i], &persistent_tensor,
							&tensor_value_i));

			if (tensor_lengths_t(i) > 0) {
				auto tensor_value_i_t = tensor_value_i->shaped<T, 3>( { 1,
						tensor_lengths_t(i), elements_per_row });
