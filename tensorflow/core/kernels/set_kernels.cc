...
sparse::SparseTensor SparseTensorFromContext(OpKernelContext* ctx,
		const int32 base_index, bool validate_indices)
{
	// Assume row-major order.
	const TensorShape shape = TensorShape(
			ctx->input(base_index + 2).vec<int64>());
		...
		const std::map<std::vector<int64>, std::set<T>>& sets)
{
	...
	OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape( { num_values,
			output_shape.dims() }), &out_indices_t));
	OP_REQUIRES_OK(ctx,
			ctx->allocate_output(1, TensorShape( { num_values }),
					&out_values_t));
	OP_REQUIRES_OK(ctx, ctx->allocate_output(2, TensorShape( {
			output_shape.dims() }), &out_shape_t));
	...
	for (auto it = sets.begin(); it != sets.end(); ++it) {
		...
		OP_REQUIRES(ctx, group_indices.size() == output_shape.dims() - 1,
				errors::Internal("Invalid number of indices ",
						group_indices.size(), ", expected ",
						output_shape.dims() - 1, "."));
		...
		for (auto value = set.begin(); value != set.end();
			...
			out_indices_mat(value_index, group_indices.size()) =
					group_value_index;
...
void PopulateFromDenseGroup(OpKernelContext* ctx, const Tensor& input_tensor,
		...
		const std::vector<int64>& group_indices, std::set<T>* result)
{
	...
	const auto start = std::inner_product(group_indices.begin(),
			group_indices.end(), input_strides.begin(), 0L);
...
void PopulateFromSparseGroup(OpKernelContext* ctx, const sparse::Group& group,
		const TensorShape& sparse_tensor_shape, std::set<T>* result)
{
...
{
	const sparse::SparseTensor set_st = SparseTensorFromContext(ctx, 0,
			validate_indices_);
	...
	for (const auto& group : set_st.group(group_ix)) {
		...
		const auto output_index = std::inner_product(group_key.begin(),
				group_key.end(), output_strides.begin(), 0L);
...
enum SetOperation {
	A_MINUS_B = 0, B_MINUS_A = 1, INTERSECTION = 2, UNION = 3
};
...
SetOperation SetOperationFromContext(OpKernelConstruction* ctx)
{
	...
	if (!ctx->GetAttr("set_operation", &set_operation_str).ok()) {
		...
		if ("union" != set_operation_str) {
			ctx->CtxFailure(
					errors::InvalidArgument("Invalid set_operation ",
							set_operation_str, "."));
		}
	}
...
public:
	SetOperationOp(OpKernelConstruction* ctx, InputTypes input_types) :
			OpKernel(ctx), set_operation_(SetOperationFromContext(ctx)), validate_indices_(
					ValidateIndicesFromContext(ctx)), input_types_(input_types)
	{
	}
...
void SetOperationOp<T>::ApplySetOperation(const std::set<T>& set1,
		const std::set<T>& set2, std::set<T>* result) const
{
...
const TensorShape GroupShapeFromInputs(OpKernelContext* ctx,
		const TensorShape& shape1, const TensorShape& shape2)
{
...
void PopulateGroupIndices(const int64 flat_group_index,
		const TensorShape& group_shape, std::vector<int64>* group_indices)
{
	group_indices->clear();
...
void SetOperationOp<T>::ComputeDenseToDense(OpKernelContext* ctx) const
{
	...
	const TensorShape group_shape = GroupShapeFromInputs(ctx, set1_t.shape(),
			set2_t.shape());
...
{
	const Tensor& set1_t = ctx->input(0);
	const sparse::SparseTensor set2_st = SparseTensorFromContext(ctx, 1,
			validate_indices_);
	...
	auto set2_grouper = set2_st.group(
			sparse::SparseTensor::VarDimArray(set2_st.order(), 0,
					set2_st.order().size() - 1));
	...
	for (int64 flat_group_index = 0;
		...
		if (set2_group_it != set2_grouper.end()) {
			...
			OP_REQUIRES(ctx, set2_group_indices.size() == group_indices.size(),
					errors::InvalidArgument("Invalid number of group indices ",
							set2_group_indices.size(), ", expected ",
							group_indices.size(), "."));
			bool group_match = true;
			for (int32 i = 0; group_match && (i < set2_group_indices.size());
					++i) {
...
void CompareGroups(OpKernelContext* ctx,
		...
		const std::vector<int64>& set2_group_indices, int64* result)
{
...
{
	const sparse::SparseTensor set1_st = SparseTensorFromContext(ctx, 0,
			validate_indices_);
	const sparse::SparseTensor set2_st = SparseTensorFromContext(ctx, 3,
			validate_indices_);
	// The following should stay in sync with `_sparse_to_sparse_shape` shape
	// assertions in python/ops/set_ops.py, and `SetShapeFn` for
	// `SparseToSparseSetOperation` in ops/set_ops.cc.
	const TensorShape group_shape = GroupShapeFromInputs(ctx, set1_st.shape(),
			set2_st.shape());
	...
	auto set1_grouper = set1_st.group(
			sparse::SparseTensor::VarDimArray(set1_st.order(), 0,
					set1_st.order().size() - 1));
	...
	while ((set1_group_it != set1_grouper.end())
		...
		const std::vector<int64>& set1_group_indices =
				(set1_group_it == set1_grouper.end()) ?
						GROUP_ITER_END : (*set1_group_it).group();
		const std::vector<int64>& set2_group_indices =
				(set2_group_it == set2_grouper.end()) ?
						GROUP_ITER_END : (*set2_group_it).group();

		int64 compare_groups;
		CompareGroups(ctx, set1_group_indices, set2_group_indices,
				&compare_groups);
