...
Status ReadGraphDef(Env* env, const string& fname, GraphDef* graph_def)
{
	...
	if (protobuf::TextFormat::ParseFromString(out, graph_def)) {
		return Status();
	} else if (ReadBinaryProto(tensorflow::Env::Default(), fname, graph_def).ok()) {
		return Status();
	}
...
{
	std::vector<string> pieces = str_util::Split(line, ' ',
			str_util::SkipEmpty());
	...
	*cmd = pieces[0];
	...
	for (int i = 1; i < pieces.size(); ++i) {
		if (pieces[i] == string(tensorflow::tfprof::kOptions[0])) {
			if (pieces.size() <= i + 1
					|| !strings::safe_strto32(pieces[i + 1],
							&opts->max_depth)) {
			...
			std::set<string> order_by_set(kOrderBy,
					kOrderBy + sizeof(kOrderBy) / sizeof(*kOrderBy));
			...
			opts->account_type_regexes = str_util::Split(
			...
			opts->start_name_regexes = str_util::Split(
					StripQuote(pieces[i + 1]), ',', str_util::SkipEmpty());
			++i;
