...
public:
	...
	{
		return props_->node_def_.name();
	}
	const string& type_string() const
	{
		return props_->node_def_.op();
	}
	...
	{
		return props_->node_def_;
	}
	const OpDef& op_def() const
	{
		return *props_->op_def_;
	}

	// input and output types
	int num_inputs() const
	{
		return props_->input_types_.size();
	}
	DataType input_type(int i) const
	{
		return props_->input_types_[i];
	}
	const DataTypeVector& input_types() const
	{
		return props_->input_types_;
	}

	int num_outputs() const
	{
		return props_->output_types_.size();
	}
	DataType output_type(int o) const
	{
		return props_->output_types_[o];
	}
	...
	string assigned_device_name() const
	{
		return assigned_device_name_;
	}
...
inline void NodeIter::operator++()
{
	while (1) {
		...
		++id_;
		if (id_ >= graph_->num_node_ids()
				|| graph_->FindNodeId(id_) != nullptr) {
			return;
		}
	}
}
