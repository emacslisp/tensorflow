...
void GemmlowpMultiply(OpKernelContext* op_context, const quint8* a_data,
		const quint8* b_data, qint32* c_data, int m, int n, int k, int offset_a,
		int offset_b, int lda, int ldb, int ldc)
{
	...
	static const gemmlowp::MapOrder ResultOrder =
			!TransposeC ?
					gemmlowp::MapOrder::RowMajor : gemmlowp::MapOrder::ColMajor;
	...
	gemmlowp::MatrixMap<const std::uint8_t, RhsOrder> rhs(b_data_as_uint8, k, n,
			ldb);
	gemmlowp::MatrixMap < std::int32_t, ResultOrder
			> result(c_data_as_int32, m, n, ldc);
	...
	gemmlowp::GemmWithOutputPipeline<std::uint8_t, std::int32_t,
			gemmlowp::DefaultL8R8BitDepthParams>(&context, lhs, rhs, &result,
			-offset_a, -offset_b, empty_pipeline);
...
public:
	...
	void Compute(OpKernelContext* context) override
	{
		...
		const int32 offset_a = FloatToQuantizedUnclamped<T1>(0.0f, min_a,
				max_a);
		const int32 offset_b = FloatToQuantizedUnclamped<T2>(0.0f, min_b,
				max_b);
		...
		if (meta::IsSupportedAndEnabled() && std::is_same<T1, quint8>()
			...
			meta::QuantizedGemm(context, transpose_a_, transpose_b_, a_data,
					b_data, c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
			...
			if (transpose_a_) {
				if (transpose_b_) {
					GemmlowpMultiply<true, true, false>(context, a_data, b_data,
							c_data, m, n, k, offset_a, offset_b, lda, ldb, ldc);
				} else {
					GemmlowpMultiply<true, false, false>(context, a_data,
							b_data, c_data, m, n, k, offset_a, offset_b, lda,
							ldb, ldc);
				}
			} else {
				if (transpose_b_) {
					GemmlowpMultiply<false, true, false>(context, a_data,
							b_data, c_data, m, n, k, offset_a, offset_b, lda,
							ldb, ldc);
				} else {
					GemmlowpMultiply<false, false, false>(context, a_data,
							b_data, c_data, m, n, k, offset_a, offset_b, lda,
							ldb, ldc);
				}
			}
		} else {
			ReferenceGemm<T1, T2, Toutput>(transpose_a_, transpose_b_,
					transpose_c, m, n, k, a_data, offset_a, lda, b_data,
					offset_b, ldb, c_data, shift_c, offset_c, mult_c, ldc);
		}

		float min_c_value;
		float max_c_value;
		QuantizationRangeForMultiplication<T1, T2, Toutput>(min_a, max_a, min_b,
				max_b, &min_c_value, &max_c_value);
