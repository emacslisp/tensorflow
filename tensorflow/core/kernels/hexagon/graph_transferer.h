...
public:
	...
	Status LoadGraphFromProtoFile(
			...
			const std::vector<InputNodeInfo>& input_node_info_list,
			const std::vector<string>& output_node_names,
			const bool is_text_proto, const bool dry_run_for_unknown_shape,
			OutputTensorInfo* output_tensor_info);

	// Dry run inference and cache the result to get memory mapping
	static Status DryRunInference(const GraphDef& graph_def,
			const std::vector<InputNodeInfo>& input_node_info_list,
			const std::vector<string>& output_node_names,
	...
	// std::vector
	static Status DryRunInferenceForAllNode(const GraphDef& graph_def,
			const std::vector<InputNodeInfo>& input_node_info_list,
			const bool initialize_by_zero,
			OutputTensorInfo* output_tensor_info);
...
private:
	...
	void AppendNodeParamsWithIoParams(const ShapeRefiner& shape_refiner,
			const OutputTensorMap& output_tensor_map, const Node& node,
			const string& name, const int id, const string& type,
			const int type_id, const string& padding_str, const int inputs_size,
			const std::vector<int>& extra_inputs, const int outputs_size,
	...
	static void FillDependencyRec(int node_id,
