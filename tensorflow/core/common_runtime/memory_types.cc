...
static Status ProcessMemoryTypes(DeviceType device_type, const Graph* g,
		std::function<Status(const Edge*, MemoryType, MemoryType)> fn)
{
	...
	typedef std::unordered_map<Endpoint, MemoryType, EndpointHash, EndpointEq> MemTypeMap;
	MemTypeMap inp;
	MemTypeMap out;
	...
	for (const Node* n : g->nodes()) {
		TF_RETURN_IF_ERROR(
				MemoryTypesForNode(g->op_registry(), device_type, n->def(),
						&inp_mvec, &out_mvec));
	...
	for (const Edge* e : g->edges()) {
		...
		MemoryType sm = gtl::FindWithDefault(out,
				{ e->src()->id(), e->src_output() }, DEVICE_MEMORY);
		MemoryType dm = gtl::FindWithDefault(inp, { e->dst()->id(),
				e->dst_input() }, DEVICE_MEMORY);
...
{
	return ProcessMemoryTypes(device_type, g,
			[g](const Edge* e, MemoryType sm,
					MemoryType dm) {
				if (sm == dm) {
					return Status::OK();
				}
...
static Node* Send(Graph* g, const string& device_name, bool host,
		const Edge* edge)
{
	const string tensor_name = strings::StrCat("edge_", edge->id(), "_",
			edge->src()->name());
	Node* ret;
	TF_CHECK_OK(
			NodeBuilder(g->NewName("n"), host ? "_HostSend" : "_Send").Input(
					edge->src(), edge->src_output()).Attr("tensor_name",
					tensor_name).Attr("send_device", device_name).Attr(
					"send_device_incarnation", 0)  // Do not care.
			.Attr("recv_device", device_name).Finalize(g, &ret));
	return ret;
}
		...
		const Edge* edge)
{
	...
	Node* ret;
	TF_CHECK_OK(
			NodeBuilder(g->NewName("n"), host ? "_HostRecv" : "_Recv").Attr(
					"tensor_type", edge->src()->output_type(edge->src_output())).Attr(
					"tensor_name", tensor_name).Attr("send_device", device_name).Attr(
					"send_device_incarnation", 0).Attr("recv_device",
					device_name).Finalize(g, &ret));
	return ret;
}
		...
		Graph* g)
{
	...
	TF_RETURN_IF_ERROR(
			ProcessMemoryTypes(device_type, g,
					[g, &edges](const Edge* e, MemoryType sm, MemoryType dm) {
						if (sm == dm) {
							return Status::OK();
						}
		...
		const Node* n, int index, MemoryType* memory_type)
{
	...
	if (out_mvec.size() <= index) {
		return errors::Internal("Trying to get the memory type for ", index,
				"'th output of node ", n->DebugString(), " that has only ",
				out_mvec.size(), " outputs");
	}
