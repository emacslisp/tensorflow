...
StatusOr<bool> ParallelizationPreparation::Run(HloModule* module)
{
	...
	for (HloInstruction* instruction : entry_computation->MakeInstructionPostOrder()) {
		...
		if (all_bitcasts) {
			...
			HloInstruction* copy = entry_computation->AddInstruction(
					HloInstruction::CreateUnary(head_operand->shape(),
							HloOpcode::kCopy, head_operand));
			TF_RETURN_IF_ERROR(head->ReplaceOperandWith(0, copy));
			instructions_to_outline.insert(instructions_to_outline.begin(),
					copy);
		}
		...
		module->OutlineExpressionFromComputation(instructions_to_outline,
	...
	for (auto& computation : module->computations()) {
		...
		/*index=*/{ })) {
			HloInstruction* copy = computation->AddInstruction(
					HloInstruction::CreateUnary(root->shape(), HloOpcode::kCopy,
							root));
