...
public:
	...
	virtual void **CudaStreamMemberHack()
	{
		return nullptr;
	}
...
public:
	...
	virtual bool SynchronousMemZero(DeviceMemoryBase *location,
			uint64 size) = 0;
	...
	virtual bool MemcpyDeviceToDevice(Stream *stream, DeviceMemoryBase *gpu_dst,
			const DeviceMemoryBase &host_src, uint64 size) = 0;
