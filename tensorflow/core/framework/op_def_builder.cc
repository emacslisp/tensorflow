...
{
	return Scanner(*sp).One(Scanner::LETTER).Any(
			Scanner::LETTER_DIGIT_UNDERSCORE).StopCapture().AnySpace().OneLiteral(
			":").AnySpace().GetResult(sp, out);
}

bool ConsumeListPrefix(StringPiece* sp)
{
	return Scanner(*sp).OneLiteral("list").AnySpace().OneLiteral("(").AnySpace().GetResult(
			sp);
}
...
bool ConsumeQuotedString(char quote_ch, StringPiece* sp, StringPiece* out)
{
	...
	return Scanner(*sp).OneLiteral(quote_str.c_str()).RestartCapture().ScanEscapedUntil(
			quote_ch).StopCapture().OneLiteral(quote_str.c_str()).AnySpace().GetResult(
			sp, out);
}

bool ConsumeAttrType(StringPiece* sp, StringPiece* out)
{
	return Scanner(*sp).Many(Scanner::LOWERLETTER_DIGIT).StopCapture().AnySpace().GetResult(
			sp, out);
}
...
bool ConsumeAttrNumber(StringPiece* sp, int64* out)
{
	...
	if (!scan.Many(Scanner::DIGIT).StopCapture().AnySpace().GetResult(
			&remaining, &match)) {
		return false;
	}
	int64 value = 0;
...
void FinalizeAttr(StringPiece spec, OpDef* op_def, std::vector<string>* errors)
{
	...
	if (spec.Consume("=")) {
		str_util::RemoveLeadingWhitespace(&spec);
		VERIFY(
				ParseAttrValue(attr->type(), spec,
						attr->mutable_default_value()),
...
{
	return strings::StrCat(" from ", is_output ? "Output" : "Input", "(\"",
			orig, "\") for Op ", op_name);
}

bool ConsumeInOutName(StringPiece* sp, StringPiece* out)
{
	return Scanner(*sp).One(Scanner::LOWERLETTER).Any(
			Scanner::LOWERLETTER_DIGIT_UNDERSCORE).StopCapture().AnySpace().OneLiteral(
			":").AnySpace().GetResult(sp, out);
}

bool ConsumeInOutRefOpen(StringPiece* sp)
{
	return Scanner(*sp).OneLiteral("Ref").AnySpace().OneLiteral("(").AnySpace().GetResult(
			sp);
}
...
{
	return Scanner(*sp).One(Scanner::LETTER).Any(
			Scanner::LETTER_DIGIT_UNDERSCORE).StopCapture().AnySpace().GetResult(
			sp, out);
}

bool ConsumeInOutTimesType(StringPiece* sp, StringPiece* out)
{
	return Scanner(*sp).OneLiteral("*").AnySpace().RestartCapture().One(
			Scanner::LETTER).Any(Scanner::LETTER_DIGIT_UNDERSCORE).StopCapture().AnySpace().GetResult(
			sp, out);
}
	...
	{  // Parse "<name|type>" or "<name>*<name|type>".
		...
		if (DataTypeFromString(type_or_attr, &dt)) {
			...
			if (attr->type() == "type") {
				...
				arg->set_type_list_attr(type_or_attr.data(),
						type_or_attr.size());
			}
		}
	}
...
void FinalizeDoc(const string& text, OpDef* op_def, std::vector<string>* errors)
{
	...
	while (static_cast<size_t>(l) < lines.size()) {
		...
		if (!found) {
			errors->push_back(
					strings::StrCat("No matching input/output/attr for name '",
							name, "' from Doc() for Op ", op_def->name()));
			return;
		}
	}
}
...
OpDefBuilder& OpDefBuilder::Doc(StringPiece text)
{
	if (!doc_.empty()) {
		errors_.push_back(
				strings::StrCat("Extra call to Doc() for Op ",
						op_def()->name()));
	} else {
...
OpDefBuilder& OpDefBuilder::Deprecated(int version, StringPiece explanation)
{
	if (op_def()->has_deprecation()) {
		errors_.push_back(
				strings::StrCat("Deprecated called twice for Op ",
						op_def()->name()));
	} else {
		...
		Status (*fn)(shape_inference::InferenceContext*))
{
	if (op_reg_data_.shape_inference_fn != nullptr) {
		errors_.push_back(
				strings::StrCat("SetShapeFn called twice for Op ",
						op_def()->name()));
	} else {
