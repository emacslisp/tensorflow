...
public:
	explicit ScanOp(OpKernelConstruction* ctx) :
			OpKernel(ctx)
	{
	...
	void Compute(OpKernelContext* ctx) override
	{
		...
		const int axis_arg = internal::SubtleMustCopy(
				tensor_axis.scalar<int>()());
		...
		functor::Scan<Device, Reducer, T>()(d,
				input.shaped<T, 3>(reduced_shape),
				output->shaped<T, 3>(reduced_shape), reducer, reverse_,
				exclusive_);
	}
