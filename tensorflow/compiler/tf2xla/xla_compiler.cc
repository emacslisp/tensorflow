	...
	if (xla::ShapeUtil::IsTuple(xla_shape)) {
		...
		for (int i = 0; i < tf_types.size(); ++i) {
			...
			if (type
				...
				return errors::Internal("element ", i, " has XLA type ",
						xla::ShapeUtil::GetTupleElementShape(xla_shape, i).element_type(),
		...
		if (tf_types.size() != 1) {
			return errors::Internal("Expected singleton type, got ",
					tf_types.size(), " types");
		}
		xla::PrimitiveType type;
...
XlaCompiler::XlaCompiler(const XlaCompiler::Options& options) :
		client_(options.client), allow_cpu_custom_calls_(
				options.allow_cpu_custom_calls), local_executable_has_hybrid_result_(
				options.local_executable_has_hybrid_result), resolve_compile_time_constants_(
				options.resolve_compile_time_constants), next_step_id_(1), device_(
				new XlaCompilationDevice(SessionOptions(),
						options.device_type)), device_mgr_( { device_ })
{
}
...
Status XlaCompiler::CompileFunction(FunctionLibraryRuntime* flr,
...
Status XlaCompiler::CompileSubComputation(FunctionLibraryRuntime* flr,
		const NameAttrList& function, const xla::Shape& input_shape,
		const xla::Shape& output_shape, xla::Computation* computation)
{
	const string function_id = Canonicalize(function.name(), function.attr());
	...
	if (use_tuple_arg) {
		for (int i = 0; i < args.size(); ++i) {
			xla::Shape xla_shape = xla::ShapeUtil::GetTupleElementShape(
					input_shape, i);
	...
	TF_RETURN_IF_ERROR(
			CompileFunctionBody(flr, *fbody, function_id, args, use_tuple_arg,
					&result));
...
Status XlaCompiler::CompileFunctionBody(FunctionLibraryRuntime* flr,
		const FunctionBody& fbody, const string& function_id,
		const std::vector<XlaCompiler::Argument>& args, bool use_tuple_arg,
		XlaCompiler::CompilationResult* result)
{
	VLOG(1) << "XlaCompiler::CompileFunctionBody " << function_id;

	std::unique_ptr<Graph> graph(
			new Graph(flr->GetFunctionLibraryDefinition()));
	...
	VLOG(1) << "====================================================";
	TF_RETURN_IF_ERROR(
			CompileGraph(function_id, std::move(graph), flr, args,
		...
		int64 step_id)
{
	...
	TF_RETURN_IF_ERROR(
			device->resource_manager()->Create(step_container->name(),
					XlaContext::kXlaContextResourceName, xla_context));
	...
	TF_RETURN_IF_ERROR(
			NewLocalExecutor(exec_params, graph.release(), &exec_ptr));
	...
	TF_RETURN_WITH_CONTEXT_IF_ERROR(exec->Run(exec_args),
...
Status XlaCompiler::CompileGraph(string const& name,
		std::unique_ptr<Graph> graph, FunctionLibraryRuntime* flib,
		const std::vector<XlaCompiler::Argument>& args, bool use_tuple_arg,
		CompilationResult* result)
{
	...
	for (int i = 0; i < args.size(); ++i) {
		...
		TF_RETURN_IF_ERROR(
				TensorShapeToXLAShape(args[i].type, args[i].shape,
						&result->xla_input_shapes.back().second));
	}

	XlaContext* xla_context = new XlaContext(this, client(), name,
			allow_cpu_custom_calls_, resolve_compile_time_constants_);
	core::ScopedUnref xla_context_unref(xla_context);

	TF_RETURN_IF_ERROR(xla_context->BuildArguments(args, use_tuple_arg));

	TF_RETURN_IF_ERROR(
			ExecuteGraph(xla_context, std::move(graph), device_, flib,
					NextStepId()));

	std::vector<XlaContext::ConstRetVal> compile_time_constants;
	int num_nonconst_outputs;
	TF_RETURN_IF_ERROR(
			xla_context->CollectResults(&result->computation,
					&result->requires_runtime_context, &compile_time_constants,
					&num_nonconst_outputs));

	VLOG(2) << "Outputs: constant: " << compile_time_constants.size()
			<< " nonconstant: " << num_nonconst_outputs;
	result->outputs.resize(
			compile_time_constants.size() + num_nonconst_outputs);
	...
	auto num_non_constant_outputs =
			(xla::ShapeUtil::IsTuple(result->xla_output_shape)) ?
					xla::ShapeUtil::TupleElementCount(
	...
	if (1 == num_non_constant_outputs) {
		...
		for (xla::Shape& s : *result->xla_output_shape.mutable_tuple_shapes()) {
			auto& minor_to_major =
					*s.mutable_layout()->mutable_minor_to_major();
			minor_to_major.Resize(xla::ShapeUtil::Rank(s), 0);
			std::iota(minor_to_major.rbegin(), minor_to_major.rend(), 0);
		}
	}
	...
	for (int i = 0; i < result->outputs.size(); ++i) {
		if (!result->outputs[i].is_constant) {
			...
			if (num_non_constant_outputs > 1) {
				result->outputs[i].shape = XLAShapeToTensorShape(
						xla::ShapeUtil::GetTupleElementShape(
