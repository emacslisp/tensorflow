...
public:
	explicit StridedSliceOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		OP_REQUIRES_OK(ctx,
				LiteralToHostTensor(begin_literal, index_type_, &begin_tensor));
		OP_REQUIRES_OK(ctx,
				LiteralToHostTensor(end_literal, index_type_, &end_tensor));
		OP_REQUIRES_OK(ctx,
				LiteralToHostTensor(strides_literal, index_type_,
						&strides_tensor));
		...
		bool dummy = false;
		OP_REQUIRES_OK(ctx,
				ValidateStridedSliceOp(&begin_tensor, &end_tensor,
						strides_tensor,
						ShapeReadWriteFromTensorShape(&input_shape),
						begin_mask_, end_mask_, ellipsis_mask_, new_axis_mask_,
						shrink_axis_mask_, &wrapped_dummy_processing_shape,
						&wrapped_final_shape, &dummy, &dummy, &dummy, &begin,
						&end, &strides));
		...
		xla::ComputationDataHandle slice = ctx->builder()->Slice(ctx->Input(0),
...
public:
	explicit StridedSliceGradOp(OpKernelConstruction* ctx) :
			XlaOpKernel(ctx)
	{
		OP_REQUIRES_OK(ctx, ctx->GetAttr("begin_mask", &begin_mask_));
		OP_REQUIRES_OK(ctx, ctx->GetAttr("end_mask", &end_mask_));
	...
	void Compile(XlaOpKernelContext* ctx) override
	{
		...
		bool dummy = false;
		ShapeReadWriteFromTensorShape wrapped_final_shape(&final_shape);
		ShapeReadWriteFromTensorShape wrapped_processing_shape(
				&processing_shape);
		OP_REQUIRES_OK(ctx,
				ValidateStridedSliceOp(&begin_tensor, &end_tensor,
						strides_tensor,
						ShapeReadWriteFromTensorShape(&input_shape),
						begin_mask_, end_mask_, ellipsis_mask_, new_axis_mask_,
						shrink_axis_mask_, &wrapped_processing_shape,
						&wrapped_final_shape, &dummy, &dummy, &dummy, &begin,
						&end, &strides));
		...
		OP_REQUIRES(ctx, final_shape == dy_shape,
				errors::InvalidArgument("shape of dy was ",
						dy_shape.DebugString(), " instead of ",
						final_shape.DebugString()));

		OP_REQUIRES(ctx, input_shape.dims() == processing_shape.dims(),
				errors::Internal(
						"input shape and processing shape must have same number of dims"));

		auto zero = XlaHelpers::Zero(ctx->builder(),
		...
		for (int i = 0; i < processing_shape.dims(); ++i) {
			...
			if (strides[i] > 0) {
				...
				int64 size = dims->edge_padding_low()
						+ processing_shape.dim_size(i)
						+ (processing_shape.dim_size(i) - 1)
								* dims->interior_padding();
				...
				int64 size = dims->edge_padding_high()
						+ processing_shape.dim_size(i)
						+ (processing_shape.dim_size(i) - 1)
								* dims->interior_padding();
