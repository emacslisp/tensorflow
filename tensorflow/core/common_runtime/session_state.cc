...
Status SessionState::DeleteTensor(const string& handle)
{
	...
	if (tensors_.erase(handle) == 0) {
		return errors::InvalidArgument(
				"Failed to delete a tensor with handle '", handle,
...
Status TensorStore::AddTensor(const string& name, const TensorAndKey& tk)
{
	...
	if (!tensors_.insert( { name, tk }).second) {
		return errors::InvalidArgument("Failed to add a tensor with name '",
				name, "' to the tensor store.");
	}
	return Status::OK();
}
	...
	if (tensors_.size() != 0) {
		...
		for (const string& name : output_names) {
			...
			if (it != tensors_.end()) {
				...
				TF_RETURN_IF_ERROR(
						session_state->AddTensor(key, it->second.tensor));
			}
		}
	}
	return Status::OK();
