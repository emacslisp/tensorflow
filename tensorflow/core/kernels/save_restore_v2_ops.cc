		...
		const Tensor& shape_and_slices)
{
	...
	OP_REQUIRES(context,
			...
			errors::InvalidArgument("Input tensor_names and shape_and_slices "
	...
	OP_REQUIRES(context, shape_and_slices.NumElements() == num_tensors,
			errors::InvalidArgument("Expected ", num_tensors,
					" elements in shapes_and_slices, but got ",
					context->input(2).NumElements()));
	if (is_save_op) {
		OP_REQUIRES(context,
				context->num_inputs() == num_tensors + kFixedInputs,
				errors::InvalidArgument("Got ", num_tensors,
						" tensor names but ",
						context->num_inputs() - kFixedInputs, " tensors."));
		OP_REQUIRES(context,
				context->num_inputs() == num_tensors + kFixedInputs,
				errors::InvalidArgument("Expected a total of ",
						num_tensors + kFixedInputs,
						" inputs as input #1 (which is a string "
								"tensor of saved names) contains ", num_tensors,
						" names, but received ", context->num_inputs(),
						" inputs"));
	}
}
...
public:
	explicit SaveV2(OpKernelConstruction* context) :
			OpKernel(context)
	{
	}
			...
			if (!shape_and_slices_flat(i).empty()) {
				...
				OP_REQUIRES(context, slice_shape.IsSameSize(tensor.shape()),
						errors::InvalidArgument("Slice in shape_and_slice "
								...
								", tensor: ", tensor.shape().DebugString()));
...
public:
	explicit RestoreV2(OpKernelConstruction* context) :
			OpKernel(context)
	{
	...
	void Compute(OpKernelContext* context) override
	{
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument(
						...
						checkpoint_prefixes.shape().DebugString(),
						" instead."));
		OP_REQUIRES(context,
				TensorShapeUtils::IsScalar(destination_prefix.shape()),
				errors::InvalidArgument(
						"Input destination_prefix should be a scalar tensor, got ",
						destination_prefix.shape().DebugString(), " instead."));

		const gtl::ArraySlice<string> input_prefixes = gtl::ArraySlice<string>(
				checkpoint_prefixes.flat<string>());
		Env* env = Env::Default();
