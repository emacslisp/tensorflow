...
std::ostream& operator<<(std::ostream& out, const LaunchDimensions& launch_dims)
{
	out
			<< tensorflow::strings::Printf("[block: %lld, thread: %lld]",
	...
	if (partition_strategy == PartitionStrategy::kLatency) {
		...
		if (threads_per_block_limit_due_to_registers < threads_per_block) {
			...
			// efficiently.
					warp_size
							* std::max(1LL,
									threads_per_block_limit_due_to_registers
											/ warp_size);
			VLOG(2)
					<< "Update # of threads per block due to register pressure = "
					<< threads_per_block;
		}
	}
	...
	VLOG(2)
			<< tensorflow::strings::Printf(
					"Initialized the block count to ceil(# of elements / threads per "
							"block) = ceil(%lld/%lld) = %lld", num_elements,
