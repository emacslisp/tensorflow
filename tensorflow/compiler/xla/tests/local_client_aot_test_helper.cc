...
int main(int argc, char** argv)
{
	...
	if (target_cpu == "k8") {
		...
		triple_string = xla::llvm_ir::AsString(
	...
	xla::LocalClient::AheadOfTimeComputationInstance instance { &computation, /*argument_layouts=*/
			{ &opaque_shape }, &r0f32 };

	xla::cpu::CpuAotCompilationOptions options(triple_string,
	...
	if (triple.isOSBinFormatELF()) {
		...
		CHECK_EQ(result->object_file_data()[5],
				triple.isLittleEndian() ? 1 : 2);
