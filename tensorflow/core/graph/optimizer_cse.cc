...
size_t OptimizerCSE::NodeHash(const Node* n)
{
	...
	if (h == kIllegalNodeHash)
		h = kIllegalNodeHash + 1;
	return h;
}
...
bool OptimizerCSE::Optimize(std::function<bool(const Node*)> consider_fn)
{
	...
	for (Node* n : order) {
		...
		if (consider_fn != nullptr && !consider_fn(n))
			continue;
		...
		if (*candidate == nullptr) {
			...
			*candidate = n;
			...
			for (const Edge* e : n->out_edges()) {
				g_->AddEdge(*candidate, e->src_output(), e->dst(),
						e->dst_input());
			}
			g_->RemoveNode(n);
			changed = true;
