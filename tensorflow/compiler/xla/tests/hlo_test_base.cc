...
StatusOr<perftools::gputools::DeviceMemoryBase> HloTestBase::Execute(
		...
		tensorflow::gtl::ArraySlice<perftools::gputools::DeviceMemoryBase> arguments,
		Shape* result_shape)
{
		...
		Shape* result_shape)
{
	VLOG(3) << "module_config layout "
			<< LayoutUtil::HumanString(
					module_config->entry_computation_layout().result_layout().layout());
	...
	if (ShapeUtil::IsTuple(*result_shape)) {
		...
		TF_ASSIGN_OR_RETURN(
				...
				backend_->transfer_manager()->ShallowCopyTupleFromDevice(
						backend_->default_stream_executor(), result,
						*result_shape));
...
se::DeviceMemoryBase HloTestBase::TransferToDevice(const Literal& literal)
{
	...
	int64 allocation_size =
			backend_->transfer_manager()->GetByteSizeRequirement(
					literal.shape());
	...
	TF_CHECK_OK(
			backend_->transfer_manager()->TransferLiteralToDevice(
					backend_->default_stream_executor(), literal, &allocation));

	return allocation;
}

std::unique_ptr<Literal> HloTestBase::TransferFromDevice(const Shape& shape,
		se::DeviceMemoryBase device_base)
{
	auto literal = MakeUnique<Literal>();
	TF_CHECK_OK(
			backend_->transfer_manager()->TransferLiteralFromDevice(
					backend_->default_stream_executor(), device_base, shape,
					shape, literal.get()));
	return literal;
}
...
{
	Shape result_shape;
	se::DeviceMemoryBase device_base = Execute(std::move(module), arguments,
			&result_shape).ValueOrDie();
...
{
	Shape result_shape;
	se::DeviceMemoryBase device_base = Execute(std::move(module),
			std::move(module_config), arguments, &result_shape).ValueOrDie();
	return TransferFromDevice(result_shape, device_base);
}
