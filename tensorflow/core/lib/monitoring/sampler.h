...
public:
	...
	static Sampler* New(
			const MetricDef<MetricKind::kCumulative, HistogramProto, NumLabels>& metric_def,
...
private:
	...
	Sampler(
			const MetricDef<MetricKind::kCumulative, HistogramProto, NumLabels>& metric_def,
			const std::vector<double>& bucket_limits) :
			metric_def_(metric_def), bucket_limits_(bucket_limits), registration_handle_(
					CollectionRegistry::Default()->Register(&metric_def_,
	...
	const MetricDef<MetricKind::kCumulative, HistogramProto, NumLabels> metric_def_;
	...
	using LabelArray = std::array<string, NumLabels>;std::map<LabelArray, SamplerCell> cells_ GUARDED_BY(mu_);
