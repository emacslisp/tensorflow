...
// finalizer.
bool XlaReductionOp::BuildFinalizer(xla::ComputationBuilder* builder,
...
void XlaReductionOp::Compile(XlaOpKernelContext* ctx)
{
	...
	OP_REQUIRES_OK(ctx,
			ctx->ConstantInputReshaped(1, { axes_tensor_shape.num_elements() },
					&axes_literal));
	...
	xla::ComputationDataHandle rx = r.Parameter(0,
			xla::ShapeUtil::MakeShape(type, { }), "x");
	xla::ComputationDataHandle ry = r.Parameter(1,
	...
	xla::ComputationDataHandle reduce = ctx->builder()->Reduce(data, initial,
			reduction_computation, xla_axes);
	...
	xla::ComputationDataHandle fx = f.Parameter(0,
			xla::ShapeUtil::MakeShape(type, { }), "x");
	...
	if (has_finalizer) {
		...
		pre_reshaped_data = ctx->builder()->Map( { reduce },
