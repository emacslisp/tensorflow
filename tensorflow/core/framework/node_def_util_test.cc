...
{
	EXPECT_EQ(Status::OK(), ValidateNodeDef(good, op_def)) << "NodeDef: "
			<< SummarizeNodeDef(good) << "; OpDef: " << SummarizeOpDef(op_def);
}
		...
		const string& message)
{
	...
	EXPECT_TRUE(errors::IsInvalidArgument(status)) << status << "; NodeDef: "
			<< SummarizeNodeDef(bad) << "; OpDef: " << SummarizeOpDef(op_def);

	LOG(INFO) << "Message: " << status.error_message();
	EXPECT_TRUE(StringPiece(status.ToString()).contains(message)) << "NodeDef: "
			<< SummarizeNodeDef(bad) << "; OpDef: " << SummarizeOpDef(op_def)
			<< "\nActual error: " << status << "\nDoes not contain: "
			<< message;
}
...
{
	EXPECT_EQ(Status::OK(), ValidateExternalNodeDefSyntax(good)) << "NodeDef: "
			<< SummarizeNodeDef(good);
}
...
void ExpectInvalidSyntax(const NodeDef& bad, const string& message)
{
	...
	EXPECT_TRUE(errors::IsInvalidArgument(status)) << status << "; NodeDef: "
			<< SummarizeNodeDef(bad);

	EXPECT_TRUE(StringPiece(status.ToString()).contains(message)) << "NodeDef: "
			<< SummarizeNodeDef(bad) << ", " << status << ", " << message;
}
