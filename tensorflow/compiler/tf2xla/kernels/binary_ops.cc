...
static xla::ComputationDataHandle FloorDivImpl(xla::ComputationBuilder* b,
		DataType dtype, xla::ComputationDataHandle x,
		xla::ComputationDataHandle y, const BCast& broadcast_helper)
{
...
XLA_MAKE_BINARY_MAP(SoftplusGrad,
		b->Div(lhs,
				b->Add(b->Exp(b->Neg(rhs)),
						XlaHelpers::One(b, input_type(1)))));
XLA_MAKE_BINARY_MAP(TanhGrad,
		b->Mul(rhs,
				b->Sub(XlaHelpers::One(b, input_type(0)), b->Mul(lhs, lhs))));
