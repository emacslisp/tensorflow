...
void XlaBinaryOp::Compile(XlaOpKernelContext* ctx)
{
	...
	xla::ComputationDataHandle output = Computation(ctx, lhs_handle,
			lhs_shape.dim_sizes(), rhs_handle, rhs_shape.dim_sizes(), bcast,
			extend_dimension);
...
/* static */std::pair<xla::ComputationDataHandle, xla::ComputationDataHandle> XlaBinaryOp::Broadcast(
		xla::ComputationBuilder* builder, const xla::ComputationDataHandle& lhs,
		const xla::ComputationDataHandle& rhs, const BCast& broadcast_helper)
{
	...
	auto lhs_broadcast = builder->Broadcast(lhs_shaped,
			broadcast_helper.x_bcast());
	int lhs_size = broadcast_helper.x_bcast().size();
	auto rhs_broadcast = builder->Broadcast(rhs_shaped,
			broadcast_helper.y_bcast());
...
xla::ComputationDataHandle XlaBinaryMapOp::Computation(XlaOpKernelContext* ctx,
		...
		const std::vector<int64>& extend_dimensions)
{
	...
	xla::ComputationDataHandle x = l.Parameter(0,
			xla::ShapeUtil::MakeShape(type, { }), "x");
	xla::ComputationDataHandle y = l.Parameter(1,
	...
	if (lhs_shape == rhs_shape) {
		...
		std::tie(lhs_broadcast, rhs_broadcast) = Broadcast(builder, lhs, rhs,
				broadcast_helper);
	}
