...
struct LocalDevice::EigenThreadPoolInfo {
	EigenThreadPoolInfo(const SessionOptions& options)
	{
		...
		eigen_worker_threads_.workers = new thread::ThreadPool(options.env,
				"Eigen", intra_op_parallelism_threads);
		eigen_threadpool_wrapper_.reset(
				new EigenThreadPoolWrapper(eigen_worker_threads_.workers));
		eigen_device_.reset(
				new Eigen::ThreadPoolDevice(eigen_threadpool_wrapper_.get(),
...
LocalDevice::LocalDevice(const SessionOptions& options,
		const DeviceAttributes& attributes, Allocator* device_allocator) :
		Device(options.env, attributes, device_allocator), owned_tp_info_(
				nullptr)
{
