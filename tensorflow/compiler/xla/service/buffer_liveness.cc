...
tensorflow::Status BufferLiveness::Analyze()
{
	TF_ASSIGN_OR_RETURN(points_to_analysis_,
			TuplePointsToAnalysis::Run(module_));
	for (auto& computation : module_->computations()) {
		...
		for (const auto& instruction : computation->instructions()) {
			for (const LogicalBuffer* aliased_buffer : points_to_analysis_->GetPointsToSet(
					instruction.get()).CreateFlattenedSet()) {
		...
		if (computation.get() == module_->entry_computation()) {
			for (const LogicalBuffer* live_out_buffer : points_to_analysis_->GetPointsToSet(
...
string BufferLiveness::ToString() const
{
	...
	pieces.push_back(
			tensorflow::strings::Printf("BufferLiveness(module=%s):",
					module_->name().c_str()));
