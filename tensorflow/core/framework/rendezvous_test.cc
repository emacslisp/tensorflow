...
Rendezvous::ParsedKey MakeKey(const string& name)
{
	string s = Rendezvous::CreateKey("/job:mnist/replica:1/task:2/CPU:0", 7890,
			"/job:mnist/replica:1/task:2/GPU:0", name, FrameAndIter(0, 0));
	Rendezvous::ParsedKey k;
	TF_EXPECT_OK(Rendezvous::ParseKey(s, &k));
	return k;
}

Rendezvous::ParsedKey KeyFoo()
{
	return MakeKey("foo");
}
Rendezvous::ParsedKey KeyBar()
{
	return MakeKey("bar");
}
...
public:
	explicit DummyDeviceContext(int stream_id) :
...
{
	thread::ThreadPool* pool = new thread::ThreadPool(Env::Default(), "test",
			1);
