...
public:
	...
	void TryDequeueMany(int num_elements, OpKernelContext* ctx,
			bool allow_small_batch, CallbackWithTuple callback) override;
...
private:
	...
	static Status GetElementComponentFromBatch(const Tuple& tuple, int64 index,
			int component, OpKernelContext* ctx, PersistentTensor* out_tensor);

	const int32 min_after_dequeue_;
	const int64 original_seed_;
	const int64 original_seed2_;

	random::PhiloxRandom parent_generator_ GUARDED_BY(mu_);random::SingleSampleAdapter<random::PhiloxRandom> generator_ GUARDED_BY(mu_);

	TF_DISALLOW_COPY_AND_ASSIGN (RandomShuffleQueue);
};

RandomShuffleQueue::RandomShuffleQueue(int32 capacity, int32 min_after_dequeue,
		...
		TypedQueue(capacity, component_dtypes, component_shapes, name), min_after_dequeue_(
				min_after_dequeue), original_seed_(seed), original_seed2_(
				seed2), generator_(&parent_generator_)
{
	if (seed == 0 && seed2 == 0) {
		...
		DoneCallback callback)
{
	...
	{
		mutex_lock l(mu_);
		already_cancelled = !cm->RegisterCallback(token,
				[this, cm, token]() {Cancel(kEnqueue, cm, token);});
...
Status RandomShuffleQueue::GetElementComponentFromBatch(const Tuple& tuple,
	int64 index, int component, OpKernelContext* ctx,
...
TF_RETURN_IF_ERROR(
		ctx->allocate_persistent(tuple[component].dtype(), element_shape,
				out_tensor, &element_access));
TF_RETURN_IF_ERROR(CopySliceToElement(tuple[component], element_access, index));
return Status::OK();
}
...
mutex_lock l(mu_);
already_cancelled = !cm->RegisterCallback(token,
		[this, cm, token]() {Cancel(kDequeue, cm, token);});
if (!already_cancelled) {
...
bool allow_small_batch, CallbackWithTuple callback)
{
if (!specified_shapes()) {
ctx->SetStatus(
...
"' has min_after_dequeue ", min_after_dequeue_,
...
return errors::InvalidArgument("Shared queue '", name_, "' has random seeds (",
original_seed_, ", ", original_seed2_, ") but requested seeds are (", seed,
", ", seed2, ").");
}
...
component_shapes_, cinfo_.name());return CreateTypedQueue(queue, ret);
}
