...
struct MatrixDescriptor {
	MatrixDescriptor(se::DeviceMemoryBase matrix_data, bool needs_transpose,
			...
			data(matrix_data), transpose(needs_transpose), num_rows(
					matrix_num_rows), num_cols(matrix_num_cols)
	{
	}
		...
		se::Stream* stream)
{
	...
	bool launch_ok = stream->ThenBlasGemm(
			lhs_matrix.transpose ?
					se::blas::Transpose::kTranspose :
					se::blas::Transpose::kNoTranspose,
			rhs_matrix.transpose ?
					se::blas::Transpose::kTranspose :
					se::blas::Transpose::kNoTranspose, output_matrix.num_rows,
			output_matrix.num_cols,
			lhs_matrix.transpose ? lhs_matrix.num_rows : lhs_matrix.num_cols, // Size of the reduce dimension.
			/*alpha=*/1.0, lhs_data, lhs_matrix.num_rows, // The leading dimension of LHS.
			rhs_data, rhs_matrix.num_rows,  // The leading dimension of RHS.
			/*beta=*/0.0, &output_data, output_matrix.num_rows) // The leading dimension of the output matrix.
	.ok();
	if (!launch_ok) {
		return InternalError("Unable to launch cuBLAS gemm on stream %p",
				stream);
	}
...
std::function<
		tensorflow::Status(MatrixDescriptor, MatrixDescriptor, MatrixDescriptor,
				se::Stream*)> FindGemmExecutor(PrimitiveType type)
{
	switch (type) {
	case F32:
		return &DoGemm<float> ;
...
GemmThunk::GemmThunk(Index lhs_buffer, Index rhs_buffer, Index output_buffer,
		...
		Thunk(Kind::kGemm, hlo_instruction), lhs_buffer_(lhs_buffer), rhs_buffer_(
				rhs_buffer), output_buffer_(output_buffer), lhs_shape_(
				lhs_shape), rhs_shape_(rhs_shape), output_shape_(output_shape), transpose_lhs_(
				transpose_lhs), transpose_rhs_(transpose_rhs)
{
}
		...
		const BufferAllocations& buffer_allocations, se::Stream* stream)
{
	...
	se::DeviceMemoryBase lhs_data = buffer_allocations.GetDeviceAddress(
			lhs_buffer_);
	se::DeviceMemoryBase rhs_data = buffer_allocations.GetDeviceAddress(
			rhs_buffer_);
	se::DeviceMemoryBase output_data = buffer_allocations.GetDeviceAddress(
			output_buffer_);
	...
	const MatrixDescriptor lhs_descriptor = make_descriptor(lhs_data,
			lhs_shape_, transpose_lhs_);
	const MatrixDescriptor rhs_descriptor = make_descriptor(rhs_data,
			rhs_shape_, transpose_rhs_);
	if (output_shape_.layout().minor_to_major(0) == 0) {
		return executor(lhs_descriptor, rhs_descriptor,
				MatrixDescriptor(output_data, false, output_num_rows,
						output_num_cols), stream);
	} else {
		return executor(rhs_descriptor, lhs_descriptor,
				MatrixDescriptor(output_data, false, output_num_cols,
						output_num_rows), stream);
	}
}
