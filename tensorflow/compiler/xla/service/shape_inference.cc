...
tensorflow::Status VerifyReducerShape(const ProgramShape& reducer_shape,
		const Shape& init_value_shape, const PrimitiveType& input_element_type)
{
	if (reducer_shape.parameters_size() != 2) {
		return InvalidArgument("Reduction function must take 2 parameters, but "
				"takes %d parameter(s).", reducer_shape.parameters_size());
	}
	...
	if (!ShapeUtil::Compatible(accumulator_shape,
			reducer_shape.parameters(0))) {
	...
	const Shape& input_element_shape = ShapeUtil::MakeShape(input_element_type,
			{ });
...
StatusOr<Shape> InferWindowOutputShape(const Shape& base_shape,
		const Window& window, PrimitiveType element_type,
		bool allow_negative_padding)
{
	...
	for (int64 i = 0; i < window.dimensions_size(); ++i) {
		...
		if (dim.size() <= 0) {
			return InvalidArgument(
					"Window has a non-positive dimension. Window: %s",
					window.DebugString().c_str());
		}
		if (dim.stride() <= 0) {
			return InvalidArgument(
					"Window has a non-positive stride. Window: %s",
					window.DebugString().c_str());
		}
		if (!allow_negative_padding && dim.padding_low() < 0) {
			return InvalidArgument(
					"Window has a negative low padding. Window: %s",
					window.DebugString().c_str());
		}
		if (!allow_negative_padding && dim.padding_high() < 0) {
			return InvalidArgument(
					"Window has a negative high padding. Window: %s",
					window.DebugString().c_str());
		}
		...
		const int64 padded_dilated_base = dim.padding_low() + dilated_base
				+ dim.padding_high();
		const int64 dilated_window = window_util::DilatedBound(dim.size(),
				dim.window_dilation());

		output_dimensions[i] = window_util::StridedBound(padded_dilated_base,
				dilated_window, dim.stride());
	}
...
{
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(arg, "operand of unary operation"));
		...
		const int64 dimension)
{
	...
	if (dimension < 0 || dimension >= ShapeUtil::Rank(*arg_shapes[0])) {
		return InvalidArgument(
				"dimension to concatenate along out of bounds: %lld", dimension);
	}
	...
	for (const Shape* shape : arg_shapes) {
		...
		for (int64 dimension_number = 0;
				dimension_number < ShapeUtil::Rank(*arg_shape);
				++dimension_number) {
...
/* static */StatusOr<Shape> ShapeInference::InferDegenerateDimensionBroadcastShape(
		...
		tensorflow::gtl::ArraySlice<int64> broadcast_dimensions)
{
	if (broadcast_dimensions.empty() && !ShapeUtil::IsScalar(smaller_shape)) {
		...
		return InvalidArgument(
				"automatic shape inference not supported: %s and %s",
		...
		return InvalidArgument(
				"size of broadcast_dimensions has to match lower-rank operand's "
						...
						"%zu", ShapeUtil::Rank(smaller_shape),
	...
	for (int i = 0; i < smaller_shape.dimensions_size(); ++i) {
		...
		if (dimension_to_match >= larger_shape.dimensions_size()) {
			return InvalidArgument(
					"broadcast dimension number (%lld) too large; higher-rank "
							"operand has rank %d", dimension_to_match,
					larger_shape.dimensions_size());
		}
		int64 small_dimension_size = smaller_shape.dimensions(i);
		int64 large_dimension_size = larger_shape.dimensions(
				dimension_to_match);
		...
		if (small_dimension_size != large_dimension_size
			...
			return InvalidArgument(
					"broadcast dimension %d mismatch: %lld != %lld; %s and %s",
					i, small_dimension_size, large_dimension_size,
					ShapeUtil::HumanString(smaller_shape).c_str(),
					ShapeUtil::HumanString(larger_shape).c_str());
		}
		...
		tensorflow::gtl::ArraySlice<int64> broadcast_dimensions)
{
	...
	if (!ShapeUtil::SameElementType(lhs, rhs)) {
		return InvalidArgument(
				"binary op with different element types: %s and %s",
				ShapeUtil::HumanString(lhs).c_str(),
				ShapeUtil::HumanString(rhs).c_str());
	}
	...
	if (ShapeUtil::Rank(lhs) == ShapeUtil::Rank(rhs)) {
		...
		return InferDegenerateDimensionBroadcastShape(operation,
				indim_broadcast_shape, larger_shape);
	}
}
		...
		tensorflow::gtl::ArraySlice<int64> broadcast_dimensions)
{
	...
	case BINOP_INDEX:
		if (ShapeUtil::Rank(lhs) > 0 && ShapeUtil::Rank(rhs) == 0) {
			tensorflow::gtl::ArraySlice<int64> dimensions = AsInt64Slice(
					lhs.dimensions());
		...
		const Shape& ehs)
{
	...
	case TRIOP_CLAMP:
		...
		return Unimplemented("not yet implemented: %s, %s <clamp> %s",
				lhs.ShortDebugString().c_str(), ehs.ShortDebugString().c_str(),
	...
	for (size_t i = 1; i < arg_shapes.size(); ++i) {
		...
		return InvalidArgument(
				"Map operation requires all operands to have the same shape; got: "
						"%s", tensorflow::str_util::Join(pieces, ", ").c_str());
	}
	...
	if (arg_shapes.size() != to_apply.parameters_size()) {
		return InvalidArgument(
				"Map applied function arity must match number of arguments; got: "
						"arity: %d, arguments: %zu", to_apply.parameters_size(),
				arg_shapes.size());
	}
	...
	if (!ShapeUtil::IsScalar(output_shape)) {
		return InvalidArgument(
				"mapped computation's result has to be a scalar; "
						"got: %s", ShapeUtil::HumanString(output_shape).c_str());
	}
		...
		if (!ShapeUtil::IsScalar(parameter_shape)) {
			return InvalidArgument(
					"mapped computation's parameter has to be a scalar; "
							"got parameter %d shape: %s", i,
					ShapeUtil::HumanString(parameter_shape).c_str());
		}
...
/* static */StatusOr<Shape> ShapeInference::InferConvolveShape(const Shape& lhs,
		const Shape& rhs, const Window& window,
		const ConvolutionDimensionNumbers& dnums)
{
	...
	if (dnums.spatial_dimensions_size()
		...
		return InvalidArgument(
				"Both arguments to convolution must have same number of dimensions.\n"
						"Window: %s", window.DebugString().c_str());
	}
	...
	if (!std::all_of(input_dnums.begin(), input_dnums.end(), in_range)
			|| !std::all_of(window_dnums.begin(), window_dnums.end(),
					in_range)) {
		return InvalidArgument(
	...
	if (input_dnums != expected_dnums) {
		return InvalidArgument(
				"Input dimensions of convolution must contain each dimension exactly "
						"once: %s", dnums.DebugString().c_str());
	}
	...
	for (int i = 0; i < num_spatial_dims; ++i) {
		kernel_spatial_dims[i] = rhs.dimensions(
				dnums.kernel_spatial_dimensions(i));
	}
	const int64 kernel_input_features = rhs.dimensions(
			dnums.kernel_input_feature_dimension());
	const int64 kernel_output_features = rhs.dimensions(
	...
	if (input_features != kernel_input_features) {
		return InvalidArgument(
				"Expected LHS feature dimension (value %lld) to match RHS "
						...
						"Dimension numbers: {%s}", input_features,
				kernel_input_features, ShapeUtil::HumanString(lhs).c_str(),
				ShapeUtil::HumanString(rhs).c_str(),
				dnums.DebugString().c_str());
	}
	...
	if (kernel_spatial_dims != window_dims) {
		return InvalidArgument("Window dimensions do not match RHS shape:\n\t"
				"RHS shape: %s\n\t"
				"Window: {%s}\n\t"
				"Dimension numbers: {%s}", ShapeUtil::HumanString(rhs).c_str(),
				window.ShortDebugString().c_str(),
	...
	for (int i = 0; i < num_spatial_dims; ++i) {
		dimensions[dnums.spatial_dimensions(i)] =
...
/* static */StatusOr<Shape> ShapeInference::InferReduceShape(const Shape& arg,
		...
		const ProgramShape& to_apply)
{
	...
	for (int i = 0; i < ShapeUtil::Rank(arg); ++i) {
		if (dimensions_to_reduce_set.find(i)
				== dimensions_to_reduce_set.end()) {
		...
		const Window& window, const ProgramShape& to_apply_shape)
{
	...
	TF_RETURN_IF_ERROR(
			VerifyReducerShape(to_apply_shape, init_value_shape,
...
{
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(operand_shape,
					"operand of select-and-scatter"));

	// Check if the select function has a proper shape of (T,T) -> PRED.
	if (select_shape.parameters_size() != 2) {
		return InvalidArgument("select function must take 2 parameters, but "
				"takes %d parameter(s).", select_shape.parameters_size());
	}
	...
	const Shape& operand_element_shape = ShapeUtil::MakeShape(
			operand_shape.element_type(), { });
	...
	TF_RETURN_IF_ERROR(
			VerifyReducerShape(scatter_shape, init_value_shape,
...
/* static */StatusOr<Shape> ShapeInference::InferSliceShape(const Shape& arg,
		...
		if (limit_index > arg.dimensions(dimension)) {
			return InvalidArgument(
					"limit index (%lld) must be less than or equal to dimension "
							"size (%lld)", limit_index,
					arg.dimensions(dimension));
		}
		if (start_index > limit_index) {
			return InvalidArgument(
					"limit index (%lld) must be greater or equal to "
							"start index (%lld) in slice", limit_index,
					start_index);
		}
		VLOG(2)
				<< tensorflow::strings::Printf("starts[%lld] = %lld", dimension,
						start_index);
		VLOG(2)
				<< tensorflow::strings::Printf("limits[%lld] = %lld", dimension,
						limit_index);
		...
		tensorflow::gtl::ArraySlice<int64> slice_sizes)
{
	...
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(start_indices_shape,
	...
	for (int64 dim = 0; dim < slice_sizes.size(); ++dim) {
		...
		VLOG(2)
				<< tensorflow::strings::Printf("slice_sizes[%lld] = %lld", dim,
						slice_dim_size);
	}
		...
		const Shape& start_indices_shape)
{
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(operand_shape,
					"operand of dynamic update slice"));
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(update_shape,
					"update of dynamic update slice"));
	TF_RETURN_IF_ERROR(
			ExpectNotTupleOrOpaque(start_indices_shape,
	...
	if (ShapeUtil::Rank(operand_shape) != start_num_dims) {
		return InvalidArgument(
				"dynamic update slice start number of dimensions %lld must match "
						"rank %lld of slice input", start_num_dims,
				ShapeUtil::Rank(operand_shape));
	}
	...
	if (ShapeUtil::Rank(update_shape) != ShapeUtil::Rank(operand_shape)) {
		return InvalidArgument(
				"dynamic update slice update rank does not match argument rank: "
						"%lld vs %lld", ShapeUtil::Rank(update_shape),
				ShapeUtil::Rank(operand_shape));
	}
	...
	for (int64 dim = 0; dim < ShapeUtil::Rank(operand_shape); ++dim) {
		...
		VLOG(2)
				<< tensorflow::strings::Printf("update_sizes[%lld] = %lld", dim,
						update_dim_size);
	}
	...
	if (index >= arg.tuple_shapes_size()) {
		return InvalidArgument(
				"cannot infer shape: attempt to index out of tuple bounds: %lld "
						">= %d in shape %s", index, arg.tuple_shapes_size(),
				ShapeUtil::HumanString(arg).c_str());
	}
		...
		const Shape& init)
{
	...
	string shape_string = tensorflow::strings::Printf(
			...
			condition.ShortDebugString().c_str(),
		...
		tensorflow::gtl::ArraySlice<int64> broadcast_sizes)
{
	...
	for (int64 size : broadcast_sizes) {
		if (size < 0) {
			return InvalidArgument(
					"Broadcast with negative dimension size %lld.", size);
		}
	}

	std::vector<int64> dimensions(
			operand.dimensions_size() + broadcast_sizes.size());
	std::copy(broadcast_sizes.begin(), broadcast_sizes.end(),
			dimensions.begin());
		...
		tensorflow::gtl::ArraySlice<int64> new_sizes)
{
	...
	Shape inferred_shape = ShapeUtil::MakeShape(operand.element_type(),
			new_sizes);

	if (ShapeUtil::ElementsIn(operand)
...
/* static */StatusOr<Shape> ShapeInference::InferSelectShape(const Shape& pred,
		const Shape& on_true, const Shape& on_false)
{
	...
	if (ShapeUtil::SameDimensions(pred, on_true)
		...
		if (!ShapeUtil::Compatible(arg_shape, param_shape)) {
			return InvalidArgument(
					"Call parameter must match argument; got parameter %d shape: %s, "
							"argument shape: %s", i,
					ShapeUtil::HumanString(param_shape).c_str(),
					ShapeUtil::HumanString(arg_shape).c_str());
		}
	}
