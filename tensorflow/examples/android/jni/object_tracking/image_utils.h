...
inline void GetUV(const uint8* const input, Image<uint8>* const u,
		Image<uint8>* const v)
{
...
inline static void MarkImage(const int x, const int y, const int radius,
		Image<U>* const img)
{
	SCHECK(img->ValidPixel(x, y), "Marking invalid pixel in image! %d, %d", x,
			y);
	...
	for (int d_y = 0; d_y <= radius; ++d_y) {
		...
		for (int d_x = radius - d_y; d_x <= radius; ++d_x) {
			...
			if (squared_y_dist + Square(d_x) >= squared_radius) {
				...
				memset(bottom_row_start, true,
						sizeof(*bottom_row_start) * x_width);
...
inline void CalculateGInt16(const int16* const vals_x,
		const int16* const vals_y, const int num_vals, int* const G)
{
	// Non-accelerated version.
	for (int i = 0; i < num_vals; ++i) {
		G[0] += Square(vals_x[i]);
...
inline void CalculateG(const int window_radius, const float center_x,
		const float center_y, const Image<int32>& I_x, const Image<int32>& I_y,
		float* const G)
{
	...
	static const int kWindowBufferSize = (kMaxWindowRadius * 2 + 1)
			* (kMaxWindowRadius * 2 + 1);
...
inline float ImageCrossCorrelation(const Image<float>& image1,
		const Image<float>& image2, const int x_offset, const int y_offset)
{
	SCHECK(
...
inline void CopyArea(const Image<uint8>& image, const BoundingBox& area_to_copy,
		Image<float>* const patch_image)
{
	...
	for (int y_index = 0; y_index < patch_height; ++y_index) {
		const float sample_y = y_index * y_dist_between_samples
				+ area_to_copy.top_;

		for (int x_index = 0; x_index < patch_width; ++x_index) {
			const float sample_x = x_index * x_dist_between_samples
					+ area_to_copy.left_;

			if (image.ValidInterpPixel(sample_x, sample_y)) {
				// TODO(andrewharp): Do area averaging when downsampling.
				(*patch_image)[y_index][x_index] = image.GetPixelInterp(
						sample_x, sample_y);
			} else {
