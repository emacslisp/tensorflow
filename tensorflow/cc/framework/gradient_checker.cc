...
template<typename T>
Status ComputeTheoreticalJacobianTranspose(const Scope& scope,
		const OutputList& xs, const std::vector<TensorShape>& x_shapes,
		...
		T* max_error)
{
	...
	TF_RETURN_IF_ERROR(
			ComputeTheoreticalJacobianTranspose<T>(scope, xs, x_shapes, x_datas,
					ys, y_shapes, jacobian_ts));
	...
	TF_RETURN_IF_ERROR(
			ComputeNumericJacobianTranspose<T>(scope, xs, x_shapes, ys,
					y_shapes, 1e-3, x_datas, jacobian_ns));
...
Status ComputeGradientError(const Scope& scope, const Output& x,
		const Tensor& x_init_value, const Output& y, const TensorShape& y_shape,
		T* max_error)
{
	// Initialize 'x_data' from 'x_init_value'.
	std::vector<Tensor> x_datas(1, Tensor(x_init_value));
	// Compute gradient error.
	return ComputeGradientErrorInternal(scope, { x }, { x_datas[0].shape() }, {
			y }, { y_shape }, x_datas, max_error);
}
