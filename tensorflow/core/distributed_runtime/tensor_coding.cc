...
Status TensorResponse::InitFrom(RecvTensorResponse* response)
{
	...
	if (on_host_) {
		...
		s = device_->MakeTensorFromProto(meta_.tensor(), alloc_attrs_,
				&tensor_);
	}
	{
		TensorProto empty;
...
Status TensorResponse::ParseFrom(Source* source)
{
	if (!on_host_) {
		...
		if (!meta_.ParseFromCodedStream(&input)
				|| !input.ConsumedEntireMessage()) {
			return errors::InvalidArgument("Cannot parse tensor from response");
		}
		Status s = device_->MakeTensorFromProto(meta_.tensor(), alloc_attrs_,
				&tensor_);
...
enum WireType {
	WIRETYPE_VARINT = 0, WIRETYPE_LENGTH_DELIMITED = 2,
};
...
{
	int length;
	if (!ReadVarintSizeAsInt(input, &length))
		return false;
		...
		protobuf::io::CodedInputStream* input, TensorProto* tensor_meta)
{
	...
	while (true) {
		...
		case TensorProto::kDtypeFieldNumber: {
			...
			if (!DataTypeCanUseMemcpy(tensor_meta->dtype()))
				return false;
			break;
		}
		case TensorProto::kTensorShapeFieldNumber: {
			if ((wt != WIRETYPE_LENGTH_DELIMITED)
					|| !ReadNestedMessage(input,
							tensor_meta->mutable_tensor_shape()))
				return false;
			if (seen_tensor_content)
				return false;
			break;
		...
		case TensorProto::kTensorContentFieldNumber: {
			...
			int num_bytes;
			if (!ReadVarintSizeAsInt(input, &num_bytes))
				return false;
...
bool TensorResponse::ParseFast(Source* source)
{
	...
	while (true) {
		...
		case RecvTensorResponse::kTransportOptionsFieldNumber: {
			if ((wt != WIRETYPE_LENGTH_DELIMITED)
					|| !ReadNestedMessage(&input,
							meta_.mutable_transport_options()))
				return false;
			break;
		}
		default: {
