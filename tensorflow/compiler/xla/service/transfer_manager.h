...
public:
	...
	virtual Status ResetDevices(
			tensorflow::gtl::ArraySlice<perftools::gputools::StreamExecutor*> executor) = 0;
	...
	GatherBufferPointersFromTuple(perftools::gputools::StreamExecutor* executor,
			const perftools::gputools::DeviceMemoryBase& source,
			const Shape& shape);
	...
	virtual Status TransferBufferToDevice(
			...
			const void* source,
			perftools::gputools::DeviceMemoryBase* destination);
