...
public:
	...
	// given input shapes.
	static StatusOr<Shape> InferBinaryOpShape(BinaryOperation operation,
	...
	static StatusOr<Shape> InferTernaryOpShape(TernaryOperation operation,
			const Shape& lhs, const Shape& rhs, const Shape& ehs);

	// Infers the shape produced by applying the given variadic operation to the
	// given input operand shapes.
	static StatusOr<Shape> InferVariadicOpShape(VariadicOperation operation,
	...
	static StatusOr<Shape> InferConvolveShape(const Shape& lhs,
	...
	static StatusOr<Shape> InferReduceShape(const Shape& arg,
	...
	static StatusOr<Shape> InferReduceWindowShape(const Shape& operand_shape,
			const Shape& init_value, const Window& window,
	...
	static StatusOr<Shape> InferDynamicSliceShape(const Shape& operand_shape,
	...
	static StatusOr<Shape> InferWhileShape(const ProgramShape& condition,
			const ProgramShape& body, const Shape& init);

	// Infers the shape produced by a broadcast operation.
	static StatusOr<Shape> InferBroadcastShape(const Shape& operand,
...
private:
	...
	static StatusOr<Shape> InferInDimBroadcastShape(BinaryOperation operation,
