...
public:
	explicit SplitVOpBase(OpKernelConstruction* c) :
			OpKernel(c)
	{
	}
			...
			std::vector<Tlen>* split_sizes_vec)
	{
		...
		std::copy(split_sizes_d.data(),
				split_sizes_d.data() + split_sizes_d.size(),
		...
		OP_REQUIRES(context, 0 <= split_dim && split_dim < input.dims(),
				errors::InvalidArgument(
						"0 <= split_dim < number of input dimensions (",
		...
		OP_REQUIRES(context,
				...
				errors::InvalidArgument("Determined shape must either match "
						...
						"specified.  Got: ", determined_size));

		if (neg_one_dim >= 0)
			(*split_sizes_vec)[neg_one_dim] = input_size_split_dim
					- determined_size;
			...
			const TensorShape& input_shape, const int32 split_dim) const
	{
		...
		IndexType split_dim_size = static_cast<IndexType>(input_shape.dim_size(
				split_dim));
...
public:
	...
	explicit SplitVOpCPU(OpKernelConstruction* c) :
			Base(c)
	{
	}
	...
	void Compute(OpKernelContext* context) override
	{
		...
		std::tie(prefix_dim_size, split_dim_size, suffix_dim_size) =
				Base::template SetDims<Eigen::DenseIndex>(input_shape,
						split_dim);
		auto input_reshaped = input.shaped<T, 3>( { prefix_dim_size,
		...
		for (int i = 0; i < num_split; ++i) {
			...
			Eigen::DSizes < Eigen::DenseIndex, 3 > sizes { prefix_dim_size,
					split_sizes_vec[i], suffix_dim_size };

			if (sizes.TotalSize() > 0) {
				auto result_shaped = result->shaped<T, 3>( { prefix_dim_size,
						split_sizes_vec[i], suffix_dim_size });

				functor::Split<CPUDevice, T>()(
