...
public:
	explicit GatherNdOp(OpKernelConstruction* c) :
			OpKernel(c)
	{
	...
	void Compute(OpKernelContext* c) override
	{
		...
		OP_REQUIRES(c, indices.dim_size(indices.dims() - 1) <= params.dims(),
				errors::InvalidArgument(
						"index innermost dimension length must be <= params rank; saw: ",
						indices.dim_size(indices.dims() - 1), " vs. ",
						params.dims()));

		const TensorShape& indices_shape(indices.shape());
		const int64 indices_nd = indices_shape.dim_size(
		...
		if (N_result > 0) {
			...
			Tensor scratch;
			OP_REQUIRES_OK(c,
					c->allocate_temp(DT_INT32, TensorShape(), &scratch));
...
#undef PARAMS_CASE
			...
			OP_REQUIRES(c, bad_i < 0,
					errors::InvalidArgument("flat indices[", bad_i, ", :] = [",
							str_util::Join(
									gtl::ArraySlice<Index>(
