...
{
	static std::vector<double>* default_bucket_limits =
			InitDefaultBucketsInner();
	return *default_bucket_limits;
}

Histogram::Histogram() :
		bucket_limits_(InitDefaultBuckets())
{
	Clear();
}
...
Histogram::Histogram(gtl::ArraySlice<double> custom_bucket_limits) :
		custom_bucket_limits_(custom_bucket_limits.begin(),
				custom_bucket_limits.end()), bucket_limits_(
...
bool Histogram::DecodeFromProto(const HistogramProto& proto)
{
	...
	custom_bucket_limits_.insert(custom_bucket_limits_.end(),
			proto.bucket_limit().begin(), proto.bucket_limit().end());
	bucket_limits_ = custom_bucket_limits_;
	buckets_.clear();
	buckets_.insert(buckets_.end(), proto.bucket().begin(),
			proto.bucket().end());
	return true;
}
...
{
	int b = std::upper_bound(bucket_limits_.begin(), bucket_limits_.end(),
			value) - bucket_limits_.begin();
...
double Histogram::Percentile(double p) const
{
	...
	for (size_t i = 0; i < buckets_.size(); i++) {
		...
		if (cumsum >= threshold) {
			...
			double lhs =
					(i == 0 || cumsum_prev == 0) ? min_ : bucket_limits_[i - 1];
