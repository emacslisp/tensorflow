...
KernelThunk::KernelThunk(tensorflow::gtl::ArraySlice<Index> io_buffers,
		...
		Thunk(Kind::kKernel, hlo_instruction), io_buffers_(io_buffers.begin(),
				io_buffers.end()), kernel_name_(kernel_name)
{
}
		...
		const BufferAllocations& buffer_allocations, se::Stream* stream)
{
	...
	LaunchDimensions launch_dimensions;
	{
		...
		if (!executor->GetKernel(*loader_spec_, &kernel)) {
			return InternalError("Unable to load kernel %s",
					kernel_name_.c_str());
		}
	...
	if (!stream->parent()->Launch(stream,
			se::ThreadDim(launch_dimensions.threads_per_block()),
