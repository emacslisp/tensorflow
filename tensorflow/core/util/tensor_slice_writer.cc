...
TensorSliceWriter::TensorSliceWriter(const string& filename,
		...
		filename_(filename), create_builder_(std::move(create_builder)), tmpname_(
				strings::StrCat(filename, ".tempstate", random::New64())), slices_(
				0)
{
...
Status TensorSliceWriter::Finish()
{
	...
	if (s.ok()) {
		...
		if (s.ok()) {
			...
			LOG(ERROR) << "Failed to rename file " << tmpname_ << " to "
					<< filename_;
		}
	} else {
